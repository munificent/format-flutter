...
void main() {
  group('verifyVersion()', () {
    ...
    test('passes for valid version strings', () async {
      ...
      for (final String version in valid_versions) {
        ...
        expect(await verifyVersion(file), isNull, reason: '$version is valid but verifyVersionFile said it was bad');
      }
    });
  ...
  group('flutter/pacakges version', () {
    ...
    test('commit hash', () async {
      ...
      final String actualHash =
          await getFlutterPackagesVersion(fileSystem: memoryFileSystem, packagesVersionFile: packagesVersionFile.path);
  ...
  group('test.dart script', () {
    ...
    Future<ProcessResult> runScript([
      ...
      final ProcessResult scriptProcess = processManager
          .runSync(<String>[dart, 'test.dart', ...otherArgs], environment: environment);
    ...
    test('subshards tests correctly', () async {
      ...
      ProcessResult result = await runScript(<String, String>{'SHARD': kTestHarnessShardName, 'SUBSHARD': '1_3'});
    ...
    test('exits with code 1 when SUBSHARD index greater than total', () async {
      final ProcessResult result =
          await runScript(<String, String>{'SHARD': kTestHarnessShardName, 'SUBSHARD': '100_99'});
    ...
    test('exits with code 255 when invalid SUBSHARD name', () async {
      final ProcessResult result =
          await runScript(<String, String>{'SHARD': kTestHarnessShardName, 'SUBSHARD': 'invalid_name'});
