...
void main() {
  ...
  test('analyze.dart - verifyGoldenTags', () async {
    ...
    final List<String> lines = <String>[
      ...
      (String line) => line.replaceAll('/', Platform.isWindows ? r'\' : '/'),
    ).toList();
    expect(
      result.length,
      4 + lines.length,
  ...
  test('analyze.dart - verifySpacesAfterFlowControlStatements', () async {
    final String result = await capture(
      () => verifySpacesAfterFlowControlStatements(
        testRootPath,
        minimumMatches: 2,
      ),
      shouldHaveErrors: true,
  ...
  test(
    'analyze.dart - verifyInternationalizations - comparison fails',
    () async {
      final String result = await capture(
