...
final String engineVersionFile = path.join(
  flutterRoot,
  'bin',
  'internal',
  'engine.version',
...
String get shuffleSeed {
  if (_shuffleSeed == null) {
    ...
    final DateTime seedTime = DateTime.now().toUtc().subtract(
  ...
  try {
    ...
    for (final String arg in args) {
      if (arg.startsWith('--local-engine=')) {
        localEngineEnv['FLUTTER_LOCAL_ENGINE'] = arg.substring(
          '--local-engine='.length,
        );
...
Future<void> _runTestHarnessTests() async {
  ...
  // success.
  final String automatedTests = path.join(
    flutterRoot,
    'dev',
    'automated_tests',
  );
...
Future<void> _runIntegrationToolTests() async {
  final List<String> allTests = Directory(
      ...
      .whereType<File>()
      .map<String>(
        (FileSystemEntity entry) => path.relative(entry.path, from: _toolsPath),
      )
      .where(
...
Future<void> _runExampleProjectBuildTests(
  ...
  if (Directory(path.join(examplePath, 'android')).existsSync()) {
    ...
    print(
      'Example project ${path.basename(
        examplePath,
      )} has no android directory, skipping apk',
    );
  }
  if (Platform.isMacOS) {
...
Future<void> _flutterBuild(
  ...
  if (verifyCaching) {
    ...
    final File file = File(
      path.join(flutterRoot, relativePathToApplication, 'perf.json'),
    );
...
bool _allTargetsCached(File performanceFile) {
  ...
  return targets.every(
    (Map<String, Object?> element) => element['skipped'] == true,
  );
}
...
Future<void> _runFrameworkTests() async {
  ...
  Future<void> runLibraries() async {
    final List<String> tests =
        Directory(path.join(flutterRoot, 'packages', 'flutter', 'test'))
            ...
            .toList();
    printProgress(
      '${green}Running packages/flutter tests$reset for $cyan${tests.join(
        ", ",
      )}$reset',
    );
    for (final String trackWidgetCreationOption
  ...
  Future<void> runTracingTests() async {
    final String tracingDirectory = path.join(
      flutterRoot,
      'dev',
      'tracing_tests',
    );
    ...
    Future<List<String>> verifyTracingAppBuild({
      ...
      try {
        ...
        final String libappStrings = utf8.decode(
          libappBytes,
          allowMalformed: true,
        );
        await runCommand(flutter, <String>[
    ...
    results.addAll(
      await verifyTracingAppBuild(
        ...
        allowed: <String>{
          ...
          'RenderTest.performResize called', // controls
          'BUILD', 'LAYOUT',
  ...
  Future<void> runMisc() async {
    ...
    await _runFlutterTest(
      ...
      printOutput: false,
      outputChecker: (CommandResult result) {
        final Iterable<Match> matches = httpClientWarning.allMatches(
          result.flattenedStdout!,
        );
  ...
  final List<String> allTests = flutterPackageTestDirectory
      ...
      .map<String>(
        (File file) => path.relative(
          file.path,
          from: flutterPackageDirectory.path,
        ),
      )
      .where(
        (String filePath) => !kWebTestFileKnownFailures[webRenderer]!.contains(
...
Future<void> _runWebTreeshakeTest() async {
  ...
  final File mainDartJs = File(
    path.join(testAppDirectory, 'build', 'web', 'main.dart.js'),
  );
...
Future<String> getFlutterPackagesVersion({
  ...
  final File versionFile = fileSystem.file(
    packagesVersionFile ?? flutterPackagesVersionFile,
  );
...
Future<void> _runFlutterPackagesTests() async {
  Future<void> runAnalyze() async {
    ...
    final String toolScript = path.join(
      toolDir,
      'bin',
...
Future<void> _ensureChromeDriverIsRunning() async {
  ...
  final Map<String, dynamic> webDriverStatus = json.decode(
...
Future<void> _runDartTest(
  ...
  final File metricFile = fileSystem.file(
    path.join(flutterRoot, 'metrics.json'),
  );
  final List<String> args = <String>[
    'run',
    'test',
  ...
  await runCommand(
    ...
    workingDirectory: workingDirectory,
    environment: environment,
    removeLine: useBuildRunner
        ? (String line) => line.startsWith('[INFO]')
        : null,
  );
...
/// 2_3
/// 3_3
List<T> _selectIndexOfTotalSubshard<T>(
  List<T> tests, {
...
Future<void> _runShardRunnerIndexOfTotalSubshard(
  List<ShardRunner> tests,
) async {
  final List<ShardRunner> sublist = _selectIndexOfTotalSubshard<ShardRunner>(
    tests,
  );
...
Future<void> selectShard(Map<String, ShardRunner> shards) => _runFromList(
  shards,
  kShardKey,
  'shard',
  0,
);
Future<void> selectSubshard(Map<String, ShardRunner> subshards) => _runFromList(
  subshards,
  kSubshardKey,
  'subshard',
  1,
