...
Future<void> _generateEntrypoint() async {
  ...
  await runCommand(_flutter, <String>['create', 'generated_entrypoint_test'], workingDirectory: tempDirectory.path);
  ...
  final File testEntrypoint = File(
    path.join(_testAppWebDirectory, _testTypeToIndexFile(ServiceWorkerTestType.generatedEntrypoint)),
  );
...
Future<void> _setAppVersion(int version) async {
  ...
  await targetFile.writeAsString(
    (await targetFile.readAsString()).replaceFirst(RegExp(r'CLOSE\?version=\d+'), 'CLOSE?version=$version'),
  );
}
...
Future<void> runWebServiceWorkerTest({required bool headless, required ServiceWorkerTestType testType}) async {
  final Map<String, int> requestedPathCounts = <String, int>{};
  void expectRequestCounts(Map<String, int> expectedCounts) => _expectRequestCounts(
    expectedCounts,
    requestedPathCounts,
  );

  AppServer? server;
  Future<void> waitForAppToLoad(Map<String, int> waitForCounts) async => _waitForAppToLoad(
    waitForCounts,
    requestedPathCounts,
    server,
  );
  ...
  await runCommand('mv', <String>['index.html', 'index_og.html'], workingDirectory: _testAppWebDirectory);
  ...
  try {
    ...
    expectRequestCounts(<String, int>{
      ...
      if (!headless) ...<String, int>{'manifest.json': 1, 'favicon.ico': 1},
    });
    expect(reportedVersion, '1');
    reportedVersion = null;
    ...
    await runCommand('mv', <String>['index_og.html', 'index.html'], workingDirectory: _testAppWebDirectory);
...
Future<void> runWebServiceWorkerTestWithCachingResources({
  ...
  try {
    ...
    print('With cache: test first page load');
    await startAppServer(cacheControl: 'max-age=3600');
    await waitForAppToLoad(<String, int>{'assets/fonts/MaterialIcons-Regular.otf': 1, 'flutter_service_worker.js': 1});
    ...
    await server!.chrome.reloadPage();
    await waitForAppToLoad(<String, int>{'assets/fonts/MaterialIcons-Regular.otf': 1, 'flutter_service_worker.js': 1});
    expectRequestCounts(<String, int>{'assets/fonts/MaterialIcons-Regular.otf': 1, 'flutter_service_worker.js': 1});
    ...
    expectRequestCounts(<String, int>{
      ...
      'assets/fonts/MaterialIcons-Regular.otf': 1,
      // In headless mode Chrome does not load 'manifest.json' and 'favicon.ico'.
      if (!headless) ...<String, int>{'favicon.ico': 1},
    });
  } finally {
    await runCommand('mv', <String>['index_og.html', 'index.html'], workingDirectory: _testAppWebDirectory);
    await server?.stop();
