...
Future<void> main(List<String> arguments) async {
  ...
  argParser.addFlag('verbose', negatable: false, help: 'Print verbose output for the analysis process.');
  argParser.addOption(
    'dart-ui-location',
...
class _SnippetCheckerException extends _ErrorBase implements Exception {
  ...
  bool operator ==(Object other) {
    ...
    return other is _SnippetCheckerException && other.message == message && other.file == file && other.line == line;
  }
...
class _SnippetChecker {
  ...
  _SnippetChecker(this._flutterPackage, {String? tempDirectory, this.verbose = false, Directory? dartUiLocation})
  ...
  static List<File> _listDartFiles(Directory directory, {bool recursive = false}) {
    return directory.listSync(recursive: recursive, followLinks: false).whereType<File>().where(
  ...
  Future<List<Object>> _extractSnippets(List<File> files, {required Map<String, _SnippetFile> snippetMap}) async {
  ...
  _SnippetFile _processBlock(
    ...
    if (firstCodeLine.startsWith('import ')) {
      ...
      return _SnippetFile.fromStrings(
        ...
        postfix: '}',
        importPreviousExample ? <_Line>[] : headersWithImports,
        <_Line>[...preamble, const _Line.generated(code: '// ignore_for_file: avoid_classes_with_only_static_members')],
        'class declaration',
        filename,
      );
    } else {
  ...
  List<Object> _analyze(Map<String, _SnippetFile> snippets) {
    ...
    for (final String error in analyzerOutput) {
      ...
      if (lineNumber < 1 || lineNumber > fileContents.length + 1) {
        errors.add(_AnalysisError(file.path, lineNumber, columnNumber, message, errorCode, _Line(line: lineNumber)));
  ...
  List<String> _runAnalyzer() {
    ...
    // output.
    Process.runSync(_flutter, <String>['pub', 'get'], workingDirectory: _tempDirectory.absolute.path);
...
class _SnippetFile {
  ...
  factory _SnippetFile.fromStrings(
    ...
    final List<_Line> codeLines = <_Line>[
      ...
      for (int i = 0; i < code.length; i += 1) _Line(code: code[i], line: firstLine.line + i, indent: firstLine.indent),
