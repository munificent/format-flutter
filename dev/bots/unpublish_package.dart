...
class ArchiveUnpublisher {
  ...
  Future<void> unpublishArchive() async {
    ...
    final List<Map<String, String>> releases =
        (jsonData['releases'] as List<dynamic>).map<Map<String, String>>(
          (dynamic entry) {
    ...
    for (final Channel channel in channels) {
      ...
      print(
        '${confirmed ? 'Reverting' : 'Would revert'} current ${getChannelName(
          channel,
        )} '
        '${getPublishedPlatform(platform)} release to ${replacementRelease[
          'hash'
        ]} (version ${replacementRelease['version']}).',
      );
    }
  ...
  Future<void> _cloudRemoveArchive(
    Map<Channel, Map<String, String>> paths,
  ) async {
...
Future<void> main(List<String> rawArguments) async {
  final List<String> allowedChannelValues = Channel.values.map<String>(
    (Channel channel) => getChannelName(channel),
  ).toList();
  ...
  final List<String> channelOptions = channelArg.isNotEmpty
      ? channelArg
      : allowedChannelValues;
  final Set<Channel> channels = channelOptions.map<Channel>(
    (String value) => fromChannelName(value),
  ).toSet();
  final List<String> platformArg = parsedArguments['platform'] as List<String>;
  final List<String> platformOptions = platformArg.isNotEmpty
      ? platformArg
