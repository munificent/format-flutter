...
Future<void> run(List<String> arguments) async {
  ...
  final Directory outDir = Directory.systemTemp.createTempSync(
    'flutter_mega_gallery.',
  );
  try {
    await runCommand(dart, <String>[
...
Future<void> verifyTokenTemplatesUpdateCorrectFiles(
  ...
  String getMaterialDirPath(List<String> lines) {
    final String line = lines.firstWhere(
      (String line) => line.contains('String materialLib'),
    );
    final String relativePath = line.substring(
      line.indexOf("'") + 1,
      line.lastIndexOf("'"),
    );
...
Future<void> verifyToolTestsEndInTestDart(String workingDirectory) async {
  ...
  await for (final File file
    ...
    final bool callsTestFunction = file.readAsStringSync().contains(
...
Future<void> verifyNoSyncAsyncStar(
  String workingDirectory, {
  int minimumMatches = 2000,
}) async {
  final RegExp syncPattern = RegExp(r'\s*?a?sync\*\s*?{');
  final RegExp ignorePattern = RegExp(
  ...
  await for (final File file
    ...
    for (int index = 0; index < lines.length; index += 1) {
      ...
      if (line.contains(syncPattern)) {
        ...
        if (!hasExplanation) {
          errors.add(
            '${file.path}:$index: sync*/async* without an explanation.',
          );
        }
      }
    }
...
final RegExp _findGoldenDefinitionPattern = RegExp(
...
Future<void> verifyDeprecations(
  String workingDirectory, {
  int minimumMatches = 2000,
}) async {
  final List<String> errors = <String>[];
  await for (final File file
    ...
    for (int lineNumber in linesWithDeprecations) {
      try {
        final RegExpMatch? startMatch = _deprecationStartPattern.firstMatch(
          lines[lineNumber],
        );
        if (startMatch == null) {
        ...
        String? message;
        do {
          final RegExpMatch? messageMatch = _deprecationMessagePattern
              .firstMatch(lines[lineNumber]);
...
Future<void> _verifyNoMissingLicenseForExtension(
  ...
  final String licensePattern = RegExp.escape(
    '$license\n${trailingBlank ? '\n' : ''}',
  );
  final List<String> errors = <String>[];
  ...
  // Fail if any errors
  if (errors.isNotEmpty) {
    final String fileDoes = errors.length == 1
        ? 'file does'
        : '${errors.length} files do';
    foundError(<String>[
...
final RegExp _testImportPattern = RegExp(
  r'''import (['"])([^'"]+_test\.dart)\1''',
);
...
Future<void> verifyNoBadImportsInFlutter(String workingDirectory) async {
  final List<String> errors = <String>[];
  final String libPath = path.join(
    workingDirectory,
    'packages',
    'flutter',
    'lib',
  );
  final String srcPath = path.join(
    workingDirectory,
    'packages',
    'flutter',
    'lib',
  ...
  final List<String> packages = Directory(libPath)
      ...
      .map<String>(
        (FileSystemEntity entity) => path.basenameWithoutExtension(entity.path),
      )
      .toList()
    ..sort();
  ...
  for (final String package in dependencyMap.keys) {
    ...
    if (loop != null) {
      errors.add(
        '${yellow}Dependency loop:$reset ${loop.join(' depends on ')}',
      );
    }
  }
  // Fail if any errors
...
Future<void> verifyInternationalizations(
  ...
  final EvalResult
  materialGenResult = await _evalCommand(dartExecutable, <String>[
    path.join('dev', 'tools', 'localization', 'bin', 'gen_localizations.dart'),
    '--material',
    '--remove-undefined',
  ], workingDirectory: workingDirectory);
  final EvalResult
  cupertinoGenResult = await _evalCommand(dartExecutable, <String>[
    path.join('dev', 'tools', 'localization', 'bin', 'gen_localizations.dart'),
    '--cupertino',
    '--remove-undefined',
  ], workingDirectory: workingDirectory);
...
Future<void> verifyNoRuntimeTypeInToString(String workingDirectory) async {
  final String flutterLib = path.join(
    workingDirectory,
    'packages',
    'flutter',
    'lib',
  ...
  final RegExp toStringRegExp = RegExp(
    ...
    while ((start = contents.indexOf(issueLinkPrefix, start)) >= 0) {
      ...
      if (url == issueLinkPrefix) {
        if (file.path !=
          ...
          problems.add(
            '${file.path} contains a direct link to $issueLinkPrefix.',
          );
...
Future<void> verifyNoBinaries(
  String workingDirectory, {
  Set<Hash256>? legacyBinaries,
}) async {
...
Future<List<File>> _gitFiles(
  ...
  final EvalResult evalResult = await _evalCommand('git', <String>[
    'ls-files',
    '-z',
  ], workingDirectory: workingDirectory, runSilently: runSilently);
...
Stream<File> _allFiles(
  ...
  gitFileNamesSet.addAll((await _gitFiles(workingDirectory)).map(
    (File f) => path.canonicalize(f.absolute.path),
  ));
...
Future<EvalResult> _evalCommand(
  ...
  final String commandDescription = '${path.relative(
    executable,
  ...
  if (!runSilently) {
    print(
      'ELAPSED TIME: $bold${prettyPrintDuration(
        time.elapsed,
      )}$reset for $commandDescription in $relativeWorkingDir',
    );
  }
...
Future<void> _checkConsumerDependencies() async {
  ...
  if (added.isNotEmpty) {
    foundError(<String>[
      'The transitive closure of package dependencies contains ${plural(
        added.length,
        "a non-allowlisted package",
  ...
  if (removed.isNotEmpty) {
    foundError(<String>[
      'Excellent news! ${plural(
        removed.length,
        "A package dependency has been removed!",
        "Multiple package dependencies have been removed!",
      )}',
      '  ${removed.join(', ')}',
      'To make sure we do not accidentally add ${plural(
        removed.length,
        "this dependency",
        "these dependencies",
      )} back in the future,',
      'please remove ${plural(
        removed.length,
        "this",
        "these",
      )} packages from the allow-list in dev/bots/allowlist.dart.',
      'Thanks!',
    ]);
  }
}
...
final RegExp _nullInitializedField = RegExp(
  r'kDebugMode \? [\w<> ,{}()]+ : null;',
);
...
final RegExp tabooPattern = RegExp(
  r'^ *///.*\b(simply|note:|note that)\b',
  caseSensitive: false,
);

Future<void> verifyTabooDocumentation(
  String workingDirectory, {
  ...
  await for (final File file
    ...
    for (int index = 0; index < lines.length; index += 1) {
      ...
      if (match != null) {
        errors.add(
          '${file.path}:${index + 1}: Found use of the taboo word "${match.group(
            1,
...
final String _kIntegrationTestsRelativePath = path.join(
  'dev',
  'integration_tests',
);
...
Future<void> verifyIntegrationTestTemplateFiles(String flutterRoot) async {
  ...
  final String integrationTestsPath = path.join(
    flutterRoot,
    _kIntegrationTestsRelativePath,
  );
...
Future<CommandResult> _runFlutterAnalyze(
  ...
  return runCommand(flutter, <String>[
    'analyze',
    ...options,
  ], workingDirectory: workingDirectory, failureMessage: failureMessage);
}
...
final RegExp _importPattern = RegExp(
  r'''^\s*import (['"])package:flutter/([^.]+)\.dart\1''',
);
final RegExp _importMetaPattern = RegExp(
  r'''^\s*import (['"])package:meta/meta\.dart\1''',
);
...
Future<Set<String>> _findFlutterDependencies(
  ...
  return _allFiles(srcPath, 'dart', minimumMatches: 1).map<Set<String>>(
    (File file) {
