...
Future<void> main() async {
  await task(() async {
    ...
    try {
      ...
      final Directory pluginADirectory = Directory(
        path.join(tempDir.path, 'plugin_a'),
      );
      ...
      final Directory pluginBDirectory = Directory(
        path.join(tempDir.path, 'plugin_b'),
      );
      await inDirectory(tempDir, () async {
        await flutter(
          'create',
      ...
      final Directory pluginCDirectory = Directory(
        path.join(tempDir.path, 'plugin_c'),
      );
      await inDirectory(tempDir, () async {
        await flutter(
          'create',
      ...
      final File pluginCpubspec = File(
        path.join(pluginCDirectory.path, 'pubspec.yaml'),
      );
...
''', flush: true);
      ...
      final Directory pluginDDirectory = Directory(
        path.join(tempDir.path, 'plugin_d'),
      );
      await inDirectory(tempDir, () async {
        await flutter(
          'create',
...
''', flush: true);
      ...
      final File pluginApubspec = File(
        path.join(pluginADirectory.path, 'pubspec.yaml'),
      );
...
''', flush: true);
      ...
      final Directory exampleApp = Directory(
        path.join(pluginADirectory.path, 'example'),
      );
      ...
      final Map<String, dynamic> jsonContent = json.decode(
      ...
      if (Platform.isMacOS) {
        ...
        checkDirectoryExists(
          path.join(appBundle.path, 'Frameworks', 'plugin_a.framework'),
        );
        checkDirectoryExists(
          path.join(appBundle.path, 'Frameworks', 'plugin_b.framework'),
        );
        checkDirectoryExists(
          path.join(appBundle.path, 'Frameworks', 'plugin_c.framework'),
        );

        // Plugin D is Android only and should not be embedded.
        checkDirectoryNotExists(
          path.join(appBundle.path, 'Frameworks', 'plugin_d.framework'),
        );
      }
