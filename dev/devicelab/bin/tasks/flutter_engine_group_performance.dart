...
Future<void> _withApkInstall(String apkPath, String bundleName, Function(AndroidDevice) body) async {
...
Future<TaskResult> _doTest() async {
  try {
    ...
    final String multipleFluttersPath = path.join(flutterDirectory, 'dev', 'benchmarks', 'multiple_flutters');
    ...
    await _withApkInstall(apkPath, _bundleName, (AndroidDevice device) async {
      ...
      for (int i = 0; i < _numberOfIterations; ++i) {
        await device.adb(<String>['shell', 'am', 'start', '-n', '$_bundleName/$_bundleName.$_activityName']);
      ...
      final Map<String, dynamic> results = <String, dynamic>{...totalMemoryStatistics.asMap('totalMemory')};
