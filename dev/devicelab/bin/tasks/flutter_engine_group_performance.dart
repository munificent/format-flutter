...
void _copyGradleFromModule(String source, String destination) {
  ...
  final String windowsWrapperPath = path.join(
    source,
    '.android',
    'gradlew.bat',
  );
  final String wrapperDestinationPath = path.join(destination, 'gradlew');
  final String windowsWrapperDestinationPath = path.join(
    destination,
    'gradlew.bat',
  );
...
Future<TaskResult> _doTest() async {
  try {
    ...
    await utils.eval(gradlewExecutable, <String>[
      'assembleRelease',
    ...
    await _withApkInstall(apkPath, _bundleName, (AndroidDevice device) async {
      ...
      final ListStatistics totalMemoryStatistics = ListStatistics(
        totalMemorySamples,
      );
