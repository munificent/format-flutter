...
Future<void> main() async {
  await task(() async {
    ...
    try {
      ...
      await inDirectory(tempDir, () async {
        await exec('pod', <String>['lib', 'lint', objcPodspecPath, '--allow-warnings', '--verbose']);
      });

      section('Lint Objective-C iOS podspec plugin as library');

      await inDirectory(tempDir, () async {
        await exec('pod', <String>['lib', 'lint', objcPodspecPath, '--allow-warnings', '--use-libraries', '--verbose']);
      });
      ...
      await inDirectory(tempDir, () async {
        await exec('pod', <String>['lib', 'lint', swiftPodspecPath, '--allow-warnings', '--verbose']);
      });
      ...
      await inDirectory(tempDir, () async {
        await exec('pod', <String>['lib', 'lint', macOSPodspecPath, '--allow-warnings', '--verbose']);
      });
      ...
      await inDirectory(tempDir, () async {
        await flutter(
          'create',
          options: <String>['--org', 'io.flutter.devicelab', '--platforms=ios', '--ios-language=objc', objcAppName],
        );
      });
...
void _validateMacOSPodfile(String appPath) {
  ...
  final String pluginSymlinks = path.join(appPath, 'macos', 'Flutter', 'ephemeral', '.symlinks', 'plugins');
