...
Future<void> main() async {
  await task(() async {
    ...
    try {
      ...
      final String objcPodspecPath = path.join(
        objcPluginPath,
        'ios',
        '$objcPluginName.podspec',
      );
      await inDirectory(tempDir, () async {
        await exec('pod', <String>[
          'lib',
      ...
      final String swiftPodspecPath = path.join(
        swiftPluginPath,
        'ios',
      ...
      swiftPodfileContent = swiftPodfileContent.replaceAll(
        'use_frameworks!',
        '',
      );
      ...
      final File macOSPodfile = File(
        path.join(swiftAppPath, 'macos', 'Podfile'),
      );
      ...
      macosPodfileContent = macosPodfileContent.replaceAll(
        'use_frameworks!',
        '',
      );
      ...
      final File pluginPubspec = File(
        path.join(objcPluginPath, 'pubspec.yaml'),
      );
      ...
      pluginPubspecContent = pluginPubspecContent.replaceFirst(
        iosPlatformMap,
        '',
      );
      ...
      final File podfileLockFile = File(
        path.join(swiftAppPath, 'ios', 'Podfile.lock'),
      );
      final String podfileLockOutput = podfileLockFile.readAsStringSync();
      if (!podfileLockOutput.contains(
      ...
      checkDirectoryExists(
        path.join(pluginSymlinks, 'url_launcher_ios', 'ios'),
      );

      checkDirectoryExists(
        path.join(pluginSymlinks, 'test_plugin_swift', 'ios'),
      );
...
void _validateIosPodfile(String appPath) {
  ...
  checkDirectoryNotExists(
    path.join(appPath, 'ios', 'Flutter', 'Flutter.framework'),
  );
...
void _validateMacOSPodfile(String appPath) {
  ...
  final File podfileLockFile = File(
    path.join(appPath, 'macos', 'Podfile.lock'),
  );
  final String podfileLockOutput = podfileLockFile.readAsStringSync();
  ...
  checkFileExists(
    path.join(appPath, 'macos', 'Flutter', 'ephemeral', 'FlutterMacOS.podspec'),
  );
  ...
  checkDirectoryExists(
    path.join(pluginSymlinks, 'url_launcher_macos', 'macos'),
  );
