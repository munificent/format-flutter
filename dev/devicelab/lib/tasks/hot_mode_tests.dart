...
TaskFunction createHotModeTest({String? deviceIdOverride, bool checkAppRunningOnLocalDevice = false}) {
  ...
  return () async {
          ...
          smallReloadData = await captureReloadData(
            ...
            onLine: (String line, Process process) {
              ...
              if (hotReloadCount == 0) {
                ...
                final File appDartSource = file(path.join(_editedFlutterGalleryDir.path, 'lib/gallery/app.dart'));
                appDartSource.writeAsStringSync(
                  appDartSource.readAsStringSync().replaceFirst("'Flutter Gallery'", "'Updated Flutter Gallery'"),
                );
          ...
          largeReloadData = await captureReloadData(
            ...
            onLine: (String line, Process process) async {
              ...
              if (hotReloadCount == 2) {
                ...
                flutterFrameworkSource.writeAsStringSync('${flutterFrameworkSource.readAsStringSync()}\n');
                process.stdin.writeln('r');
                hotReloadCount += 1;
              } else {
    ...
    return TaskResult.success(
      <String, dynamic>{
        ...
        // ignore: avoid_dynamic_calls
        'hotReloadFlutterReassembleMillisecondsAfterChange': smallReloadData['hotReloadFlutterReassembleMilliseconds']
            [1],
        // ignore: avoid_dynamic_calls
...
Future<void> _checkAppRunning(bool shouldBeRunning) async {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    galleryProcesses =
        await getRunningProcesses(processName: 'Flutter Gallery$exe', processManager: const LocalProcessManager());
