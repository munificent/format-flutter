...
Future<TaskResult> flutterToolStartupBenchmarkTask() async {
  final Directory projectParentDirectory = Directory.systemTemp.createTempSync('flutter_tool_startup_benchmark');
  ...
  final Map<String, dynamic> data = <String, dynamic>{
    ...
    ...(await _Benchmark(projectDirectory, 'test startup', 'test').run()).asMap('flutter_tool_startup_test'),
    ...
    // `flutter -h`.
    ...(await _Benchmark(projectDirectory, 'help startup', '-h').run()).asMap('flutter_tool_startup_help'),
  };
...
class _Benchmark {
  _Benchmark(this.directory, this.title, this.command, {this.options = const <String>[], this.environment});
