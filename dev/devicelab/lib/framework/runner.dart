  ...
  for (final String taskName in taskNames) {
    ...
    while (retry <= Cocoon.retryNumber) {
      ...
      if (!result.succeeded) {
        ...
        if (retry > 0) {
          print(
            'Total ${retry + 1} executions: $retry failures and 1 false positive.',
          );
          print('flaky: true');
...
Future<TaskResult> runTask(
  ...
  try {
    ...
    final Map<String, dynamic> taskResultJson = (await result.vmService
...
Future<VmService> vmServiceConnectUri(String wsUri, {Log? log}) async {
  ...
  socket.listen(
    ...
    onError: (Object err, StackTrace stackTrace) => controller.addError(
      err,
      stackTrace,
    ),
