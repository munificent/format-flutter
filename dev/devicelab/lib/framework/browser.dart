...
Future<Uri> _getRemoteDebuggerUrl(Uri base) async {
  ...
  return base.resolve(
    (jsonObject.first as Map<String, dynamic>)['webSocketDebuggerUrl']
        as String,
  );
}
...
class BlinkTraceSummary {
  ...
  static BlinkTraceSummary? fromJson(List<Map<String, dynamic>> traceJson) {
    try {
      ...
      events = events.where(
        (BlinkTraceEvent element) => element.pid == tabPid,
      ).toList();
      ...
      return BlinkTraceSummary._(
        averageBeginFrameTime: _computeAverageDuration(
          frames.map(
            (BlinkFrame frame) => frame.beginFrame,
          ).whereType<BlinkTraceEvent>().toList(),
        ),
        averageUpdateLifecyclePhasesTime: _computeAverageDuration(
          frames.map(
            (BlinkFrame frame) => frame.updateAllLifecyclePhases,
          ).whereType<BlinkTraceEvent>().toList(),
        ),
      );
    } catch (_) {
...
Duration _computeAverageDuration(List<BlinkTraceEvent> events) {
  ...
  final double sum = events.skip(
    math.max(events.length - _kMeasuredSampleCount, 0),
