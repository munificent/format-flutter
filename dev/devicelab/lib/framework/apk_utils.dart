...
Future<void> runProjectTest(
  Future<void> Function(FlutterProject project) testFunction,
) async {
...
class ApkExtractor {
  ...
  Future<void> _extractDex() async {
    ...
    final String packages =
        await _evalApkAnalyzer(<String>['dex', 'packages', apkFile.path]);
...
Future<String> getAndroidManifest(String apk) async {
  return _evalApkAnalyzer(<String>[
    'manifest',
    'print',
    apk,
...
class FlutterProject {
  ...
  Future<void> addCustomBuildType(
    String name, {
    required String initWith,
  }) async {
...
android {
  ...
  Future<void> introducePubspecError() async {
    ...
    final String newContents = contents.replaceFirst(
      '${platformLineSep}flutter:$platformLineSep',
      '''
...
''',
  ...
  Future<ProcessResult> resultOfGradleTask(
    String task, {
    List<String>? options,
  }) {
...
class FlutterPluginProject {
  ...
  static Future<FlutterPluginProject> create(
    ...
    await inDirectory(directory, () async {
      await flutter(
        'create',
        options: <String>['--template=plugin', '--platforms=ios,android', name],
      );
    });
...
String? validateSnapshotDependency(
  ...
  final File snapshotBlob = File(path.join(
    project.rootPath,
    'build',
    'app',
    'intermediates',
