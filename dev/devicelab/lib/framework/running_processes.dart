...
class RunningProcessInfo {
  ...
  Future<bool> terminate({required ProcessManager processManager}) async {
    ...
    if (Platform.isWindows) {
      ...
      final ProcessResult result = await processManager.run(<String>['taskkill.exe', '/pid', '$pid', '/f']);
...
Future<Set<RunningProcessInfo>> getRunningProcesses({String? processName, required ProcessManager processManager}) {
  if (Platform.isWindows) {
...
@visibleForTesting
Future<Set<RunningProcessInfo>> windowsRunningProcesses(String? processName, ProcessManager processManager) async {
...
@visibleForTesting
Future<Set<RunningProcessInfo>> posixRunningProcesses(String? processName, ProcessManager processManager) async {
  ...
  final ProcessResult result = await processManager.run(<String>['ps', '-eo', 'lstart,pid,command']);
  if (result.exitCode != 0) {
    print('Could not list processes!');
    print(result.stderr);
    print(result.stdout);
