...
class Cocoon {
  ...
  String _readCommitSha() {
    final ProcessResult result = processRunSync('git', <String>[
      'rev-parse',
      'HEAD',
    ]);
  ...
  Future<void> sendTaskStatus({
    ...
    if (_shouldUpdateCocoon(resultsJson, builderBucket ?? 'prod')) {
      await retry(
        () async => _sendUpdateTaskRequest(resultsJson).timeout(
          Duration(seconds: requestTimeoutLimit),
        ),
        retryIf: (Exception e) =>
  ...
  Future<Map<String, dynamic>> _sendCocoonRequest(
    String apiPath, [
    dynamic jsonData,
  ]) async {
