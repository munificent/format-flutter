...
Future<bool> run(List<String> arguments) async {
  ...
  final int? shardIndex = int.tryParse(
    parsedArguments['shard-index'] as String,
  );
  final List<File> files = parsedArguments.rest
      .expand(
        (String path) => Glob(path).listFileSystemSync(const LocalFileSystem()),
      )
      .whereType<File>()
      .where(
        (File file) =>
  ...
  if (help ||
    ...
    if (verbose) {
      if (repeat == null) {
        print(
          'Error: Could not parse repeat count ("${parsedArguments[
            'repeat'
          ]}")',
        );
      }
      if (numberShards == null) {
        print(
          'Error: Could not parse shards count ("${parsedArguments[
            'shards'
          ]}")',
        );
      ...
      if (shardIndex == null) {
        print(
          'Error: Could not parse shard index ("${parsedArguments[
            'shard-index'
          ]}")',
        );
      ...
      if (parsedArguments.rest.isEmpty) {
        ...
        print(
          'Error: File arguments ("${parsedArguments.rest.join(
            '", "',
