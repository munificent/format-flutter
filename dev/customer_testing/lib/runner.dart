...
Future<bool> runTests({
  ...
  for (final File file in shardedFiles) {
    ...
    try {
      ...
      if (success) {
        final Directory customerRepo = Directory(
          path.join(checkout.path, 'tests'),
        );
        ...
        if (success) {
          ...
          for (int iteration = 0; iteration < repeat; iteration += 1) {
            ...
            for (final String testCommand in instructions.tests) {
              ...
              if (!success) {
                failure(
                  'One or more tests from ${path.basenameWithoutExtension(
                    file.path,
                  )} failed.',
                );
                break;
          ...
          if (verbose && success) {
            print(
              'Tests finished in ${(stopwatch.elapsed.inSeconds / repeat).toStringAsFixed(
                2,
...
Future<bool> shell(
  ...
  utf8.decoder.bind(process.stdout).transform(const LineSplitter()).listen(
    verbose ? printLog : output.add,
  );
  utf8.decoder.bind(process.stderr).transform(const LineSplitter()).listen(
    verbose ? printLog : output.add,
  );
