...
void main() {
  ...
  setUp(() {
    ...
    framework = FrameworkRepository(checkouts, mirrorRemote: const Remote(name: RemoteName.mirror, url: mirrorUrl));
  ...
  test('GitHub token is redacted from exceptions while pushing', () async {
    ...
    processManager.addCommands(<FakeCommand>[
      ...
      const FakeCommand(
        command: <String>['$checkoutsParentDirectory/flutter_conductor_checkouts/framework/bin/flutter', 'help'],
      ),
      const FakeCommand(
        command: <String>[
...
''',
    ...
    await expectLater(() async {
      ...
      await rollFuture;
    }, throwsA(isA<Exception>().having((Exception exc) => exc.toString(), 'message', isNot(contains(token)))));
  });
