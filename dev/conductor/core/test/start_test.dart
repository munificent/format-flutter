...
void main() {
  group('start command', () {
    ...
    CommandRunner<void> createRunner({
      ...
      platform = FakePlatform(environment: environment, operatingSystem: operatingSystem, pathSeparator: pathSeparator);
      ...
      final StartCommand command = StartCommand(checkouts: checkouts, conductorVersion: conductorVersion);
    ...
    test('throws if provided an invalid --$kVersionOverrideOption', () async {
      ...
      final String stateFilePath = fileSystem.path.join(platform.environment['HOME']!, kStateFileName);
    ...
    test('creates state file if provided correct inputs', () async {
      ...
      final List<FakeCommand> engineCommands = <FakeCommand>[
        ...
        const FakeCommand(command: <String>['git', 'checkout', '-b', 'cherrypicks-$candidateBranch']),
        const FakeCommand(command: <String>['git', 'status', '--porcelain'], stdout: 'MM path/to/DEPS'),
        const FakeCommand(command: <String>['git', 'add', '--all']),
        const FakeCommand(command: <String>['git', 'commit', '--message', 'Update Dart SDK to $nextDartRevision']),
        const FakeCommand(command: <String>['git', 'rev-parse', 'HEAD'], stdout: revision2),
        const FakeCommand(command: <String>['git', 'rev-parse', 'HEAD'], stdout: revision2),
      ];
      ...
      final List<FakeCommand> frameworkCommands = <FakeCommand>[
        FakeCommand(
          command: <String>[
            ...
            fileSystem.path.join(checkoutsParentDirectory, 'flutter_conductor_checkouts', 'framework'),
          ],
        ),
        ...
        const FakeCommand(command: <String>['git', 'checkout', '-b', 'cherrypicks-$candidateBranch']),
        const FakeCommand(
          command: <String>['git', 'describe', '--match', '*.*.*', '--tags', 'refs/remotes/upstream/$candidateBranch'],
      ...
      final CommandRunner<void> runner = createRunner(commands: <FakeCommand>[...engineCommands, ...frameworkCommands]);
    ...
    test('logs to STDERR but does not fail on an unexpected candidate branch', () async {
      ...
      expect(
        stdio.error,
        contains('Parsed version $previousVersion.42 has a different x value than candidate branch $candidateBranch'),
      );
    });
    ...
    test('StartContext gets engine and framework checkout directories after run', () async {
      ...
      final List<FakeCommand> frameworkCommands = <FakeCommand>[
        ...
        const FakeCommand(command: <String>['git', 'rev-parse', 'HEAD'], stdout: branchPointRevision),
        const FakeCommand(
          command: <String>['git', 'merge-base', 'upstream/$candidateBranch', 'upstream/master'],
          stdout: branchPointRevision,
        ),
        ...
        const FakeCommand(command: <String>['git', 'push', FrameworkRepository.defaultUpstream, branchPointTag]),
      ];
      ...
      processManager = FakeProcessManager.list(<FakeCommand>[...engineCommands, ...frameworkCommands]);
      checkouts = Checkouts(
        fileSystem: fileSystem,
        parentDirectory: fileSystem.directory(checkoutsParentDirectory),
        platform: platform,
