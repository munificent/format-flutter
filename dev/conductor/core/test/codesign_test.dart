...
void main() {
  group('codesign command', () {
    ...
    const String flutterCache = '${checkoutsParentDirectory}flutter_conductor_checkouts/framework/bin/cache';
    const String flutterBin = '${checkoutsParentDirectory}flutter_conductor_checkouts/framework/bin/flutter';
    ...
    const List<String> binariesWithoutEntitlements = <String>['$flutterCache/engine/darwin-x64/font-subset'];
    const List<String> allBinaries = <String>[...binariesWithEntitlements, ...binariesWithoutEntitlements];

    void createRunner({String operatingSystem = 'macos', List<FakeCommand>? commands}) {
      stdio = TestStdio();
    ...
    test('does not fail if --revision flag not provided', () async {
      ...
      for (final String bin in binariesWithEntitlements) {
        codesignCheckCommands.add(FakeCommand(command: <String>['codesign', '-vvv', bin]));
      ...
      createRunner(
        commands: <FakeCommand>[
          ...
          const FakeCommand(command: <String>[flutterBin, 'help']),
          const FakeCommand(command: <String>[flutterBin, 'precache', '--android', '--ios', '--macos']),
          FakeCommand(
          ...
          for (final String bin in allBinaries)
            FakeCommand(command: <String>['file', '--mime-type', '-b', bin], stdout: 'application/x-mach-binary'),
    ...
    test('framework cloned from repo provided by --$kUpstream', () async {
      ...
      await runner.run(<String>['codesign', '--$kVerify', '--$kRevision', revision, '--$kUpstream', upstreamRepo]);
      expect(processManager, hasNoRemainingExpectations);
      expect(
        stdio.stdout,
        contains('Verified that binaries for commit $revision are codesigned and have expected entitlements'),
      );
    });
    ...
    test('fails if a single binary is not codesigned', () async {
      ...
      codesignCheckCommands.add(
        const FakeCommand(command: <String>['codesign', '-vvv', '$flutterCache/dart-sdk/bin/dart']),
      );
      codesignCheckCommands.add(FakeCommand(
      ...
      codesignCheckCommands.add(
        const FakeCommand(command: <String>['codesign', '-vvv', '$flutterCache/engine/darwin-x64/font-subset']),
      );
    ...
    test('fails if a single binary has the wrong entitlements', () async {
      ...
      codesignCheckCommands.add(
        const FakeCommand(command: <String>['codesign', '-vvv', '$flutterCache/dart-sdk/bin/dartaotruntime']),
      );
      // No entitlements
      codesignCheckCommands.add(
        const FakeCommand(
    ...
    test('does not check signatures or entitlements if --no-$kSignatures specified', () async {
      ...
      await runner.run(<String>['codesign', '--$kVerify', '--no-$kSignatures', '--$kRevision', revision]);
      expect(processManager.hasRemainingExpectations, false);
    });
  });
}
