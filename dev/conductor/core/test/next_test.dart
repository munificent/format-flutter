...
void main() {
  ...
  group(
    ...
    () {
      ...
      test('throws if no state file found', () async {
        ...
        expect(
          () async => runner
              .run(<String>['next', '--$kStateOption', stateFile]),
      ...
      group('APPLY_ENGINE_CHERRYPICKS to CODESIGN_ENGINE_BINARIES', () {
        test(
          ...
          () async {
            ...
            final File ciYaml = fileSystem.file(
              '$checkoutsParentDirectory/engine/.ci.yaml',
            )..createSync(recursive: true);
            ...
            expect(
              ...
              ReleasePhase.CODESIGN_ENGINE_BINARIES,
            );
        ...
        test(
          'confirms to stdout when all engine cherrypicks were auto-applied',
          () async {
            stdio.stdin.add('n');
            final File ciYaml = fileSystem.file(
              '$checkoutsParentDirectory/engine/.ci.yaml',
      ...
      group('CODESIGN_ENGINE_BINARIES to APPLY_FRAMEWORK_CHERRYPICKS', () {
        ...
        test('updates currentPhase if user responds yes', () async {
          ...
          expect(
            finalState.currentPhase,
            ReleasePhase.APPLY_FRAMEWORK_CHERRYPICKS,
          );
        });
      });
      ...
      group('APPLY_FRAMEWORK_CHERRYPICKS to PUBLISH_VERSION', () {
        ...
        setUp(() {
          ...
          state = pb.ConductorState(
            ...
            engine: pb.Repository(
              ...
              workingBranch: workingBranch,
              upstream:
                  pb.Remote(name: 'upstream', url: engineUpstreamRemoteUrl),
            ),
          ...
          final Directory frameworkDir = fileSystem.directory(
          ...
          () async {
            ...
            processManager.addCommands(<FakeCommand>[
              ...
              FakeCommand(
                ...
                onRun: () {
                  final File file = fileSystem.file(
                    '$checkoutsParentDirectory/framework/.ci.yaml',
                  )..createSync();
                  _initializeCiYamlFile(file);
                },
              ),
        ...
        test(
          'does not update state.currentPhase if user responds no',
          () async {
            stdio.stdin.add('n');
            processManager.addCommands(<FakeCommand>[
              const FakeCommand(command: <String>['git', 'fetch', 'upstream']),
      ...
      test(
        'throws exception if state.currentPhase is RELEASE_COMPLETED',
        () async {
  ...
  group('prompt', () {
    test('can be overridden for different frontend implementations', () async {
      ...
      final bool response =
          await context.prompt('A prompt that will immediately be agreed to');
...
''';
        ...
        final Repository testRepository = _TestRepository.fromCheckouts(
          checkouts,
        );
