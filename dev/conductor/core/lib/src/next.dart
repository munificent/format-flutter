...
class NextCommand extends Command<void> {
  NextCommand({required this.checkouts}) {
    final String defaultPath = state_import.defaultStateFilePath(
      checkouts.platform,
    );
    argParser.addOption(
      kStateOption,
...
class NextContext extends Context {
  ...
  Future<void> run(pb.ConductorState state) async {
    ...
    final ReleasePhase nextPhase = state_import.getNextPhase(
      state.currentPhase,
    );
    stdio.printStatus(
