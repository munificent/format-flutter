...
abstract class Repository {
    ...
    if (previousCheckoutLocation != null) {
      ...
      if (initialRef != null) {
        ...
        await git.run(<String>[
          'fetch',
          upstreamRemote.name,
        ], 'Fetch ${upstreamRemote.name} to ensure we have latest refs', workingDirectory: _checkoutDirectory!.path);
        // If [initialRef] is a remote ref, the checkout will be left in a detached HEAD state.
        await git.run(<String>[
          'checkout',
          initialRef!,
        ], 'Checking out initialRef $initialRef', workingDirectory: _checkoutDirectory!.path);
      }
  ...
  Future<void> lazilyInitialize(Directory checkoutDirectory) async {
    ...
    if (mirrorRemote != null) {
      await git.run(<String>[
        ...
        mirrorRemote!.name,
        mirrorRemote!.url,
      ], 'Adding remote ${mirrorRemote!.url} as ${mirrorRemote!.name}', workingDirectory: checkoutDirectory.path);
      await git.run(<String>[
        'fetch',
        mirrorRemote!.name,
      ], 'Fetching git remote ${mirrorRemote!.name}', workingDirectory: checkoutDirectory.path);
    }
    if (localUpstream) {
      ...
      for (final String channel in requiredLocalBranches) {
        await git.run(<String>[
          ...
          channel,
          '--',
        ], 'check out branch $channel locally', workingDirectory: checkoutDirectory.path);
      }
    }
    ...
    if (initialRef != null) {
      await git.run(<String>[
        'checkout',
        initialRef!,
      ], 'Checking out initialRef $initialRef', workingDirectory: checkoutDirectory.path);
    }
  ...
  Future<String> remoteUrl(String remoteName) async {
    return git.getOutput(<String>[
      'remote',
      'get-url',
      remoteName,
    ], 'verify the URL of the $remoteName remote', workingDirectory: (await checkoutDirectory).path);
  }
  ...
  Future<bool> gitCheckoutClean() async {
    final String output = await git.getOutput(<String>[
      'status',
      '--porcelain',
    ], 'check that the git checkout is clean', workingDirectory: (await checkoutDirectory).path);
    return output == '';
  }
  ...
  Future<void> fetch(String remoteName) async {
    await git.run(<String>[
      ...
      remoteName,
      '--tags',
    ], 'fetch $remoteName --tags', workingDirectory: (await checkoutDirectory).path);
  }
  ...
  Future<void> newBranch(String branchName) async {
    await git.run(<String>[
      ...
      '-b',
      branchName,
    ], 'create & checkout new branch $branchName', workingDirectory: (await checkoutDirectory).path);
  }

  /// Check out the given ref.
  Future<void> checkout(String ref) async {
    await git.run(<String>['checkout', ref], 'checkout ref', workingDirectory: (await checkoutDirectory).path);
  }
  ...
  Future<String> getFullTag(String remoteName, String branchName, {bool exact = true}) async {
    ...
    return git.getOutput(<String>[
      ...
      '--tags',
      ref,
    ], 'obtain last released version number', workingDirectory: (await checkoutDirectory).path);
  }
  ...
  Future<void> tag(String commit, String tagName, String remote) async {
    ...
    await git.run(<String>[
      ...
      tagName,
      commit,
    ], 'tag the commit with the version label', workingDirectory: (await checkoutDirectory).path);
    ...
    await git.run(<String>[
      ...
      remote,
      tagName,
    ], 'publish the tag to the repo', workingDirectory: (await checkoutDirectory).path);
    stdio.printStatus('Tag push successful.');
  }

  /// List commits in reverse chronological order.
  Future<List<String>> revList(List<String> args) async {
    return (await git.getOutput(<String>[
          'rev-list',
          ...args,
        ], 'rev-list with args ${args.join(' ')}', workingDirectory: (await checkoutDirectory).path))
        .trim()
        .split('\n');
  }
  ...
  Future<String> reverseParse(String ref) async {
    final String revisionHash = await git.getOutput(<String>[
      'rev-parse',
      ref,
    ], 'look up the commit for the ref $ref', workingDirectory: (await checkoutDirectory).path);
  ...
  Future<void> reset(String ref) async {
    await git.run(<String>['reset', ref, '--hard'], 'reset to $ref', workingDirectory: (await checkoutDirectory).path);
  }
  ...
  Future<String> commit(String message, {bool addFirst = false, String? author}) async {
    final bool hasChanges = (await git.getOutput(<String>[
          'status',
          '--porcelain',
        ], 'check for uncommitted changes', workingDirectory: (await checkoutDirectory).path))
        .trim()
        .isNotEmpty;
    if (!hasChanges) {
    ...
    if (addFirst) {
      await git.run(<String>[
        'add',
        '--all',
      ], 'add all changes to the index', workingDirectory: (await checkoutDirectory).path);
    }
    String? authorArg;
    if (author != null) {
      if (author.contains('"')) {
  ...
  Future<String> authorEmptyCommit([String message = 'An empty commit']) async {
    await git.run(<String>[
      ...
      '-m',
      "'$message'",
    ], 'create an empty commit', workingDirectory: (await checkoutDirectory).path);
    return reverseParse('HEAD');
  }
...
class FrameworkRepository extends Repository {
  FrameworkRepository(
    ...
    super.upstreamRemote = const Remote(name: RemoteName.upstream, url: FrameworkRepository.defaultUpstream),
    super.localUpstream,
  ...
  factory FrameworkRepository.localRepoAsUpstream(
    ...
    return FrameworkRepository(
      checkouts,
      name: name,
      upstreamRemote: Remote(name: RemoteName.upstream, url: 'file://$upstreamPath/'),
  ...
  Future<void> _ensureToolReady() async {
    ...
    // Build tool
    await processManager.run(<String>[fileSystem.path.join((await checkoutDirectory).path, 'bin', 'flutter'), 'help']);
  }

  Future<io.ProcessResult> runFlutter(List<String> args) async {
    await _ensureToolReady();
    return processManager
        .run(<String>[fileSystem.path.join((await checkoutDirectory).path, 'bin', 'flutter'), ...args]);
  }
  ...
  Future<io.Process> streamFlutter(
    ...
    await _ensureToolReady();
    final io.Process process = await processManager
        .start(<String>[fileSystem.path.join((await checkoutDirectory).path, 'bin', 'flutter'), ...args]);
    process.stdout.transform(utf8.decoder).transform(const LineSplitter()).listen(stdoutCallback ?? stdio.printTrace);
    process.stderr.transform(utf8.decoder).transform(const LineSplitter()).listen(stderrCallback ?? stdio.printError);
    return process;
  }
  ...
  Future<Version> flutterVersion() async {
    ...
    final Map<String, dynamic> versionJson = jsonDecode(stdoutToString(result.stdout)) as Map<String, dynamic>;
  ...
  Future<bool> updateCandidateBranchVersion(String branch, {@visibleForTesting File? versionFile}) async {
    assert(branch.isNotEmpty);
    versionFile ??= (await checkoutDirectory).childDirectory('bin').childDirectory('internal').childFile(
    ...
    if (versionFile.existsSync()) {
      ...
      if (oldCandidateBranch.trim() == branch.trim()) {
        stdio.printTrace('Tried to update the candidate branch but version file is already up to date at: $branch');
        return false;
      }
    }
  ...
  ///
  /// Returns [true] if the version file was updated and a commit is needed.
  Future<bool> updateEngineRevision(String newEngine, {@visibleForTesting File? engineVersionFile}) async {
    assert(newEngine.isNotEmpty);
    engineVersionFile ??= (await checkoutDirectory).childDirectory('bin').childDirectory('internal').childFile(
      'engine.version',
    );
    ...
    if (oldEngine.trim() == newEngine.trim()) {
      stdio.printTrace('Tried to update the engine revision but version file is already up to date at: $newEngine');
      return false;
    }
...
class HostFrameworkRepository extends FrameworkRepository {
  HostFrameworkRepository({required Checkouts checkouts, String name = 'host-framework', required String upstreamPath})
    : super(
        checkouts,
        name: name,
        upstreamRemote: Remote(name: RemoteName.upstream, url: 'file://$upstreamPath/'),
  ...
  void updateChannel(String commit, String remote, String branch, {bool force = false, bool dryRun = false}) {
...
class EngineRepository extends Repository {
  EngineRepository(
    ...
    super.upstreamRemote = const Remote(name: RemoteName.upstream, url: EngineRepository.defaultUpstream),
    super.localUpstream,
    super.previousCheckoutLocation,
    super.mirrorRemote,
    List<String>? additionalRequiredLocalBranches,
  ...
  Future<void> updateDartRevision(String newRevision, {@visibleForTesting File? depsFile}) async {
    ...
    final String updatedFileContent = fileContent.replaceFirst(dartPattern, "  'dart_revision': '$newRevision',");
