...
[Pub packages autoroller](https://github.com/flutter/flutter/blob/main/dev/conductor/core/bin/packages_autoroller.dart).''';
  ...
  Future<bool> updatePackages({bool verbose = true}) async {
    ...
    final io.Process flutterProcess = await framework
        .streamFlutter(<String>[if (verbose) '--verbose', 'update-packages', '--force-upgrade']);
    ...
    await framework.commit('roll packages', addFirst: true, author: author);
    return true;
  }
  ...
  Future<void> pushBranch() async {
    ...
    await framework.pushRef(fromRef: await featureBranchName, toRef: await featureBranchName, remote: remote);
  }
  ...
  Future<String> cli(List<String> args, {bool allowFailure = false, String? stdin, String? workingDirectory}) async {
    log('Executing "$githubClient ${args.join(' ')}" in $workingDirectory');
    final io.Process process = await processManager
        .start(<String>[githubClient, ...args], workingDirectory: workingDirectory, environment: <String, String>{});
    final List<String> stderrStrings = <String>[];
    final List<String> stdoutStrings = <String>[];
    final Future<void> stdoutFuture = process.stdout.transform(utf8.decoder).forEach(stdoutStrings.add);
    final Future<void> stderrFuture = process.stderr.transform(utf8.decoder).forEach(stderrStrings.add);
    if (stdin != null) {
