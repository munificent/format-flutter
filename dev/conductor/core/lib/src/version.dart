...
class Version {
  ...
  factory Version.fromString(String versionString) {
    ...
    // stable tag
    Match? match = versionPatterns[VersionType.stable]!.firstMatch(
      versionString,
    );
    if (match != null) {
      // parse stable
      final List<int> parts = match.groups(<int>[1, 2, 3]).map(
    ...
    if (match != null) {
      // parse development
      final List<int> parts = match.groups(<int>[1, 2, 3, 4, 5]).map(
        (String? s) => int.parse(s!),
      ).toList();
      return Version(
        x: parts[0],
    ...
    if (match != null) {
      // parse latest
      final List<int> parts = match.groups(<int>[1, 2, 3, 4, 5, 6]).map(
        (String? s) => int.parse(s!),
      ).toList();
      return Version(
        x: parts[0],
  ...
  void ensureValid(String candidateBranch, ReleaseType releaseType) {
    final RegExpMatch? branchMatch = releaseCandidateBranchRegex.firstMatch(
      candidateBranch,
    );
    if (branchMatch == null) {
      throw ConductorException(
