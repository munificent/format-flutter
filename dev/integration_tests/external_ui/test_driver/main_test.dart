...
final RegExp statsRegExp = RegExp(
  'Produced: (.*)fps\nConsumed: (.*)fps\nWidget builds: (.*)',
);
...
Future<void> main() async {
  group('texture suite', () {
    ...
    test('texture rendering', () async {
      ...
      final Match? matchCalibration = calibrationRegExp.matchAsPrefix(
        calibrationResult,
      );
      expect(matchCalibration, isNotNull);
      final double flutterFrameRate = double.parse(
        matchCalibration?.group(1) ?? '0',
      );
      ...
      expect(
        double.parse(matchSlow.group(1)!),
        closeTo(flutterFrameRate / 2.0, 5.0),
      );
      expect(
        double.parse(matchSlow.group(2)!),
        closeTo(flutterFrameRate / 2.0, 5.0),
      );
      ...
      expect(
        double.parse(matchFast.group(1)!),
        closeTo(flutterFrameRate * 2.0, 5.0),
      );
      expect(
        double.parse(matchFast.group(2)!),
        closeTo(flutterFrameRate, 10.0),
      );
