            ...
            Expanded(
              child: ElevatedButton(
                ...
                onPressed: () {
                  ...
                  saveRecordedEvents(
                    codec.encodeMessage(flutterViewEvents)!,
                    context,
                  );
                },
              ),
  ...
  Future<String> playEventsFile() async {
    ...
    try {
      ...
      for (int i = 0; i < flutterViewEvents.length; ++i) {
        final String currentDiff = diffMotionEvents(
          flutterViewEvents[i],
          embeddedViewEvents[i],
        );
  ...
  Widget buildEventTile(BuildContext context, int index) {
    if (embeddedViewEvents.length > index) {
      return TouchEventDiff(
        flutterViewEvents[index],
        embeddedViewEvents[index],
      );
    }
    return Text(
...
class TouchEventDiff extends StatelessWidget {
  ...
  void prettyPrintEvent(Map<String, dynamic> event) {
    ...
    for (int i = 0; i < coords.length; i++) {
      buffer.write(
        'p$i x: ${coords[i]['x']} y: ${coords[i]['y']}, pressure: ${coords[i][
          'pressure'
        ]} ',
      );
    }
