...
class _AndroidSemanticsMatcher extends Matcher {
  ...
  @override
  bool matches(
    covariant AndroidSemanticsNode item,
    ...
    if (contentDescription != null &&
      ...
      return _failWithMessage(
        'Expected contentDescription: $contentDescription',
        matchState,
      );
    }
    ...
    if (actions != null) {
      ...
      if (!unorderedEquals(actions!).matches(itemActions, matchState)) {
        final List<String> actionsString = actions!.map<String>(
          (AndroidSemanticsAction action) => action.toString(),
        ).toList()
          ..sort();
        final List<String> itemActionsString = itemActions.map<String>(
          (AndroidSemanticsAction action) => action.toString(),
        ).toList()
          ..sort();
        final Set<String> unexpectedInString = itemActionsString
            .toSet()
            .difference(actionsString.toSet());
    ...
    if (isLongClickable != null && isLongClickable != item.isLongClickable) {
      return _failWithMessage(
        'Expected longClickable: $isLongClickable',
        matchState,
      );
    }
    return true;
