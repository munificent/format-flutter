...
class _AllSectionsLayout extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    for (int index = 0; index < cardCount!; index++) {
      ...
      final Offset columnIndicatorOrigin = Offset(
        columnIndicatorX,
        columnIndicatorY,
      );
      final Rect titleRect = Rect.fromPoints(
        titleOrigin,
        titleSize.bottomRight(titleOrigin),
      );
      ...
      final Offset rowIndicatorOrigin = Offset(
...
class _AllSectionsView extends AnimatedWidget {
  ...
  Widget _build(BuildContext context, BoxConstraints constraints) {
    ...
    final double tColumnToRow = 1.0 -
        ((size.height - midHeight!) / (maxHeight! - midHeight!)).clamp(
          0.0,
          1.0,
        );
    ...
    final double tCollapsed = 1.0 -
        ((size.height - minHeight!) / (midHeight! - minHeight!)).clamp(
          0.0,
          1.0,
        );
...
class _AnimationDemoHomeState extends State<AnimationDemoHome> {
  ...
  void _maybeScroll(double midScrollOffset, int pageIndex, double xOffset) {
    if (_scrollController.offset < midScrollOffset) {
      ...
      final int newPageIndex = xOffset > centerX
          ? pageIndex + 1
          : pageIndex - 1;
      _headingPageController.animateToPage(
        newPageIndex,
  ...
  Iterable<Widget> _detailItemsFor(Section section) {
    final Iterable<Widget> detailItems = section.details!.map<Widget>(
