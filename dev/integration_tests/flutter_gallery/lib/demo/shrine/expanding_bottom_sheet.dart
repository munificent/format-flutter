...
Animation<T> _getEmphasizedEasingAnimation<T>({
  ...
  return TweenSequence<T>(<TweenSequenceItem<T>>[
    TweenSequenceItem<T>(
      weight: firstWeight,
      tween: Tween<T>(begin: begin, end: peak).chain(
        CurveTween(curve: firstCurve),
      ),
    ),
    TweenSequenceItem<T>(
      weight: secondWeight,
      tween:
          Tween<T>(begin: peak, end: end).chain(CurveTween(curve: secondCurve)),
    ),
  ]).animate(parent);
}
...
class _ProductThumbnailRowState extends State<ProductThumbnailRow> {
  ...
  Widget _buildThumbnail(
    ...
    final Animation<double> thumbnailSize = Tween<double>(begin: 0.8, end: 1.0)
        .animate(CurvedAnimation(
