...
class Board extends IterableMixin<BoardPoint?> {
  ...
  Vertices getVerticesForBoardPoint(BoardPoint boardPoint, Color color) {
    ...
    final List<Offset>
    positions = positionsForHexagonAtOrigin.map((Offset offset) {
  ...
  Board copyWithBoardPointColor(BoardPoint boardPoint, Color color) {
    ...
    final List<BoardPoint> nextBoardPoints = List<BoardPoint>.from(
      _boardPoints,
    );
...
class BoardPoint {
  ...
  BoardPoint copyWithColor(Color nextColor) => BoardPoint(
    q,
    r,
    color: nextColor,
  );
