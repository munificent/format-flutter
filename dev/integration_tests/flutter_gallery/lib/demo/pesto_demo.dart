...
final ThemeData _kTheme = ThemeData(brightness: Brightness.light, primarySwatch: Colors.teal);
    ...
    return Theme(
      ...
      child: Scaffold(
        floatingActionButton: FloatingActionButton(
          ...
          onPressed: () {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Not supported.')));
          },
          child: const Icon(Icons.edit),
        ),
        body: CustomScrollView(
          semanticChildCount: widget.recipes!.length,
          slivers: <Widget>[_buildAppBar(context, statusBarHeight), _buildBody(context, statusBarHeight)],
        ),
      ),
    );
  }
    ...
    return SliverAppBar(
      ...
      flexibleSpace: LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          ...
          return Padding(
            padding: EdgeInsets.only(top: statusBarHeight + 0.5 * extraPadding, bottom: extraPadding),
  ...
  void showRecipePage(BuildContext context, Recipe? recipe) {
    Navigator.push(context, MaterialPageRoute<void>(
      ...
      builder: (BuildContext context) {
        return Theme(data: _kTheme.copyWith(platform: Theme.of(context).platform), child: RecipePage(recipe: recipe));
      },
    ));
  }
}
          ...
          child: Stack(
            ...
            children: <Widget>[
              Positioned.fromRect(
                ...
                child: Image.asset(_kSmallLogoImage, package: _kGalleryAssetsPackage, fit: BoxFit.contain),
              ),
              Positioned.fromRect(
...
class _RecipePageState extends State<RecipePage> {
  ...
  PopupMenuItem<String> _buildMenuItem(IconData icon, String label) {
    return PopupMenuItem<String>(
      child: Row(
        children: <Widget>[
          Padding(padding: const EdgeInsets.only(right: 24.0), child: Icon(icon, color: Colors.black54)),
...
class RecipeSheet extends StatelessWidget {
  ...
  TableRow _buildItemRow(String left, String right) {
    return TableRow(
      children: <Widget>[
        Padding(padding: const EdgeInsets.symmetric(vertical: 4.0), child: Text(left, style: itemAmountStyle)),
        Padding(padding: const EdgeInsets.symmetric(vertical: 4.0), child: Text(right, style: itemStyle)),
      ],
    );
  }
}
