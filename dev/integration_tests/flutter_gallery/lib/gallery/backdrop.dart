...
class _CrossFadeTransition extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final double opacity1 = CurvedAnimation(parent: ReverseAnimation(progress), curve: const Interval(0.5, 1.0)).value;

    final double opacity2 = CurvedAnimation(parent: progress, curve: const Interval(0.5, 1.0)).value;

    return Stack(
      alignment: alignment,
      children: <Widget>[
        Opacity(opacity: opacity1, child: Semantics(scopesRoute: true, explicitChildNodes: true, child: child1)),
        Opacity(opacity: opacity2, child: Semantics(scopesRoute: true, explicitChildNodes: true, child: child0)),
      ],
    );
  }
}
...
class _BackAppBar extends StatelessWidget {
  const _BackAppBar({this.leading = const SizedBox(width: 56.0), required this.title, this.trailing});
        ...
        child: SizedBox(
          ...
          child: Row(
            children: <Widget>[
              ...
              if (trailing != null) Container(alignment: Alignment.center, width: 56.0, child: trailing),
            ],
          ),
        ),
      ),
            ...
            builder: (BuildContext context, Widget? child) {
              return PhysicalShape(
                ...
                clipper: ShapeBorderClipper(
                  shape: BeveledRectangleBorder(borderRadius: _kFrontHeadingBevelRadius.transform(_controller!.value)!),
                ),
