...
void main() {
  group('FlutterDriver', () {
    ...
    test('waitForAbsent should time out waiting for text "present" to disappear', () async {
      await expectLater(() => driver.waitForAbsent(presentText, timeout: const Duration(seconds: 1)), throwsA(
    ...
    test('waitFor times out waiting for "present" to reappear', () async {
      await expectLater(() => driver.waitFor(presentText, timeout: const Duration(seconds: 1)), throwsA(
        isA<DriverError>().having(
          (DriverError error) => error.message,
          'message',
    ...
    test('waitFor should resolve when text "present" reappears', () async {
      ...
      driver.waitFor(presentText).then(whenWaitForResolves.complete, onError: whenWaitForResolves.completeError);
