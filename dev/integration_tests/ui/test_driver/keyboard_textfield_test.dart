    ...
    test(
      ...
      () async {
        await driver.setTextEntryEmulation(
          enabled: false,
        ); // we want the keyboard to come up

        final SerializableFinder listViewFinder = find.byValueKey(
          keys.kListView,
        );
        final SerializableFinder textFieldFinder = find.byValueKey(
          keys.kDefaultTextField,
        );
        final SerializableFinder offsetFinder = find.byValueKey(
          keys.kOffsetText,
        );
        final SerializableFinder keyboardVisibilityIndicatorFinder = find
            .byValueKey(keys.kKeyboardVisibleView);
        ...
        final double scrollOffsetWithoutKeyboard = double.parse(
          await driver.getText(offsetFinder),
        );
        ...
        await driver.waitFor(textFieldFinder);
        final double scrollOffsetWithKeyboard = double.parse(
          await driver.getText(offsetFinder),
        );
        ...
        expect(
          ...
          greaterThan(scrollOffsetWithoutKeyboard),
        );
      },
