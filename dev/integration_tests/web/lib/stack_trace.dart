...
void main() {
  ...
  try {
    try {
      ...
      final List<StackFrame> parsedFrames = StackFrame.fromStackTrace(
        expectedStackTrace,
      );
...
void _checkStackFrameContents(
  ...
  final List<StackFrame> actual = parsedFrames.where(
    (StackFrame frame) => callChain.contains(frame.method),
  ).toList();
