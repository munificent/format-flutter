...
Future<double> _runBasicBinary(BasicMessageChannel<ByteData> basicBinary, ByteData buffer, int count) async {
  int size = 0;
  final Stopwatch watch = Stopwatch();
  watch.start();
  for (int i = 0; i < count; ++i) {
...
Future<void> _runTest({
  ...
  // Prime test.
  await test(1);
  printer.addResult(description: description, value: await test(numMessages), unit: 'µs', name: name);
}
