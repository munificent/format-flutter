...
typedef DartMmap = ffi.Pointer<ffi.Void> Function(ffi.Pointer<ffi.Void>, int, int, int, int, int);
...
final DartMprotect mprotect = ffi.DynamicLibrary.process().lookupFunction<CMprotect, DartMprotect>('mprotect');
...
final GetStackPointerCallback getStackPointer = () {
  ...
  region.cast<ffi.Uint8>().asTypedList(kMemorySize).setAll(kMemoryStartingIndex, <int>[
  ...
  return region.cast<ffi.NativeFunction<ffi.IntPtr Function()>>().asFunction<int Function()>();
}();
...
class StackSizePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const Material(child: Column(children: <Widget>[SizedBox(width: 200, height: 100, child: ParentWidget())]));
  }
}
...
class ChildWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Text('${parentStackSize - myStackSize}', key: const ValueKey<String>(kStackSizeKey));
  }
}
