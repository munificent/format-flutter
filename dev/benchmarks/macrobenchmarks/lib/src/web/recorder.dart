...
abstract class WidgetRecorder extends Recorder implements FrameRecorder {
  ...
  Future<Profile> run() async {
    ...
    registerEngineBenchmarkValueListener(kProfilePrerollFrame, (num value) {
      localProfile.addDataPoint(kProfilePrerollFrame, Duration(microseconds: value.toInt()), reported: false);
    });
    registerEngineBenchmarkValueListener(kProfileApplyFrame, (num value) {
      localProfile.addDataPoint(kProfileApplyFrame, Duration(microseconds: value.toInt()), reported: false);
    });
...
class Timeseries {
  ...
  TimeseriesStats computeStats() {
    ...
    final List<AnnotatedSample> annotatedValues = <AnnotatedSample>[
      for (final double warmUpValue in warmUpValues)
        AnnotatedSample(magnitude: warmUpValue, isOutlier: warmUpValue > outlierCutOff, isWarmUpValue: true),
      for (final double candidate in candidateValues)
        AnnotatedSample(magnitude: candidate, isOutlier: candidate > outlierCutOff, isWarmUpValue: false),
    ];
  ...
  void add(double value, {required bool isWarmUpValue}) {
    ...
    if (isWarmUpValue) {
      if (!_isWarmingUp) {
        throw StateError('A warm-up value was added to the timeseries after the warm-up phase finished.');
      }
...
class AnnotatedSample {
  const AnnotatedSample({required this.magnitude, required this.isOutlier, required this.isWarmUpValue});
...
class Profile {
  ...
  void addDataPoint(String key, Duration duration, {required bool reported}) {
    scoreData.putIfAbsent(key, () => Timeseries(key, reported)).add(
      duration.inMicroseconds.toDouble(),
      isWarmUpValue: isWarmingUp,
    );
