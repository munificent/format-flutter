...
final RegExp kPluralRegexp = RegExp(
  r'(\w*)(' + kPluralSuffixes.skip(1).join(r'|') + r')$',
);
    ...
    if (plural != null) {
      ...
      if (!bundle.containsKey(resourceIdOther)) {
        errorMessages.writeln(
          'Default plural resource $resourceIdOther undefined',
        );
      }
    } else {
      if (!optional && !bundle.containsKey(resourceId)) {
        errorMessages.writeln(
          'No matching $resourceId defined for $atResourceId',
        );
      }
    }
  }
...
void removeUndefinedLocalizations(
  ...
  final Set<String> canonicalKeys = Set<String>.from(
    canonicalLocalizations.keys,
  );
  ...
  localeToResources.forEach((LocaleInfo locale, Map<String, String> resources) {
    ...
    resources.removeWhere(
      (String key, String value) => invalidKeys.contains(key),
    );
  });
}
