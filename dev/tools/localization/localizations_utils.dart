...
class LocaleInfo implements Comparable<LocaleInfo> {
  ...
  String camelCase() {
    return originalString.split('_').map<String>(
      (String part) =>
    ...
    if (type == 'language' || type == 'region' || type == 'script') {
      ...
      if (description.contains(kParentheticalPrefix)) {
        description = description.substring(
          0,
          description.indexOf(kParentheticalPrefix),
        );
      }
      if (description.contains(kProvincePrefix)) {
        description = description.substring(
          0,
          description.indexOf(kProvincePrefix),
        );
      }
...
/// The translations for ${describeLocale(
...
String generateString(String value) {
  if (<String>['\n', '\f', '\t', '\r', '\b'].every(
    (String pattern) => !value.contains(pattern),
  )) {
