...
String generateArbBasedLocalizationSubclasses({
  ...
  for (final LocaleInfo locale in localeToResources.keys.toList()..sort()) {
    ...
    if (locale.countryCode != null && locale.scriptCode != null) {
      final LocaleInfo key = LocaleInfo.fromString(
    ...
    allResourceIdentifiers.addAll(
      localeToResources[locale]!.keys.toList()..sort(),
    );
  }
  ...
  for (final String languageName in languageCodes) {
    ...
    output.writeln(
      generateClassDeclaration(languageLocale, generatedClassPrefix, baseClass),
    );
    ...
    for (final String key in allKeys) {
      ...
      output.writeln(
        generateGetter(key, languageResources[key], attributes, languageLocale),
      );
    }
    output.writeln('}');
    ...
    if (languageToScriptCodes.containsKey(languageName)) {
      ...
      for (final String scriptCode in languageToScriptCodes[languageName]!) {
        final LocaleInfo scriptBaseLocale = LocaleInfo.fromString(
          '${languageName}_$scriptCode',
        );
    ...
    if (countryCodeCount == 0) {
      if (scriptCodeCount == 0) {
        ...
        supportedLocales.writeln(
          '///  * `$languageName` - ${describeLocale(
            languageName,
          )} (plus $scriptCodeCount script${scriptCodeCount == 1 ? '' : 's'})',
        );
      }
      ...
      supportedLocales.writeln(
        '///  * `$languageName` - ${describeLocale(
          languageName,
        )} (plus one country variation$scriptCodeMessage)',
      );
    } else {
      supportedLocales.writeln(
        '///  * `$languageName` - ${describeLocale(
          languageName,
        )} (plus $countryCodeCount country variations$scriptCodeMessage)',
      );
    }
  }
...
${languageCodes.map<String>(
    (String value) => "  '$value', // ${describeLocale(value)}",
  ).toList().join('\n')}
]);
...
$factoryDeclaration
  ...
  for (final String language in languageToLocales.keys) {
    ...
    if (languageToLocales[language]!.length == 1) {
      output.writeln('''
    case '$language':
      return ${callsFactoryWithConst ? 'const ' : ''}$generatedClassPrefix${languageToLocales[
        language
      ]![0].camelCase()}($factoryArguments);''');
    ...
    case '$language': {
      ...
      return ${callsFactoryWithConst ? 'const ' : ''}$generatedClassPrefix${LocaleInfo.fromString(
        language,
      ).camelCase()}($factoryArguments);
    }''');
    } else {
...
void main(List<String> rawArgs) {
  ...
  // https://github.com/googlei18n/app-resource-bundle/wiki/ApplicationResourceBundleSpecification#resource-attributes
  final Map<LocaleInfo, Map<String, dynamic>>
  cupertinoLocaleToResourceAttributes = <LocaleInfo, Map<String, dynamic>>{};
  ...
  if (options.writeToFile) {
    ...
    widgetsLocalizationsFile.writeAsStringSync(
    ...
    materialLocalizationsFile.writeAsStringSync(
    ...
    cupertinoLocalizationsFile.writeAsStringSync(
