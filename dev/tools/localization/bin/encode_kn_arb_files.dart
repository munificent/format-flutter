...
void _checkEncodedTranslations(
  Map<String, dynamic> encodedBundle,
  ...
  for (final String key in bundle.keys) {
    if (decoder.convert('"${encodedBundle[key]}"') != bundle[key]) {
      stderr.writeln(
        '  encodedTranslation for $key does not match original value "${bundle[
          key
        ]}"',
      );
      errorFound = true;
...
void encodeKnArbFiles(Directory directory) {
  ...
  final File materialArbFile = File(
    path.join(directory.path, 'material_kn.arb'),
  );
  final File cupertinoArbFile = File(
    path.join(directory.path, 'cupertino_kn.arb'),
  );
