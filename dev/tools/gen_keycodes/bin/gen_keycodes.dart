...
Future<void> main(List<String> rawArguments) async {
  ...
  if (parsedArguments['collect'] as bool) {
    ...
    final String supplementalHidCodes = readDataFile(
      'supplemental_hid_codes.inc',
    );
    final String androidScanCodes = await getAndroidScanCodes();
    final String androidToDomKey = readDataFile(
    ...
    final String supplementalKeyData = readDataFile(
    ...
    final String windowsToDomKey = readDataFile(
      'windows_logical_to_window_vk.json',
    );
    final String macosLogicalToPhysical = readDataFile(
      'macos_logical_to_physical.json',
    );
    final String iosLogicalToPhysical = readDataFile(
    ...
    File(parsedArguments['physical-data'] as String).writeAsStringSync(
      '$physicalJson\n',
    );
    final String logicalJson = encoder.convert(logicalData.toJson());
    File(parsedArguments['logical-data'] as String).writeAsStringSync(
      '$logicalJson\n',
    );
  } else {
    physicalData = PhysicalKeyData.fromJson(
      json.decode(
        await File(parsedArguments['physical-data'] as String).readAsString(),
      ) as Map<String, dynamic>,
    );
    logicalData = LogicalKeyData.fromJson(
      json.decode(
        await File(parsedArguments['logical-data'] as String).readAsString(),
      ) as Map<String, dynamic>,
    );
  }

  final Map<String, bool> layoutGoals = parseMapOfBool(
