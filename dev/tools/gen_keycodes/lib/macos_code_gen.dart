...
class MacOSCodeGenerator extends PlatformCodeGenerator {
  ...
  String get _scanCodeMap {
    ...
    for (final PhysicalKeyEntry entry in keyData.entries) {
      if (entry.macOSScanCode != null) {
        lines.add(
          entry.macOSScanCode!,
          '  @${toHex(entry.macOSScanCode)} : @${toHex(
            entry.usbHidCode,
          )},  // ${entry.constantName}',
        );
      }
  ...
  String get _keyCodeToLogicalMap {
    ...
    for (final LogicalKeyEntry entry in logicalData.entries) {
      zipStrict(
        entry.macOSKeyCodeValues,
        entry.macOSKeyCodeNames,
        (int macOSValue, String macOSName) {
          lines.add(
            macOSValue,
            '  @${toHex(macOSValue)} : @${toHex(
              entry.value,
              digits: 11,
            )},  // $macOSName -> ${entry.constantName}',
          );
        },
      );
    }
  ...
  String get _keyToModifierFlagMap {
    ...
    for (final String name in kModifiersOfInterest) {
      modifierKeyMap.writeln(
        '  @${toHex(
          logicalData.entryByName(name).macOSKeyCodeValues[0],
  ...
  String get _modifierFlagToKeyMap {
    ...
    for (final String name in kModifiersOfInterest) {
      modifierKeyMap.writeln(
        '  @(kModifierFlag${lowerCamelToUpperCamel(name)}) : @${toHex(
          logicalData.entryByName(name).macOSKeyCodeValues[0],
        )},',
      );
    }
