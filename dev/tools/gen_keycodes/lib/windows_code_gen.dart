...
class WindowsCodeGenerator extends PlatformCodeGenerator {
    ...
    for (final PhysicalKeyEntry entry in keyData.entries) {
      if (entry.windowsScanCode != null) {
        lines.add(
          ...
          '        {${toHex(entry.windowsScanCode)}, ${toHex(
            entry.usbHidCode,
          )}},  // ${entry.constantName}',
        );
      }
  ...
  String get _scanCodeToLogicalMap {
    final OutputLines<int> lines = OutputLines<int>(
      'Windows scancode to logical map',
    );
    _scancodeToLogical.forEach((String scanCodeName, String logicalName) {
      ...
      lines.add(
        ...
        '        {${toHex(physicalEntry.windowsScanCode)}, ${toHex(
          logicalEntry.value,
          digits: 11,
        )}},  '
  ...
  String get _maskConstants {
    ...
    for (final MaskConstant constant in maskConstants) {
      buffer.writeln(
        'const uint64_t KeyboardKeyEmbedderHandler::${constant.lowerCamelName} = ${toHex(
          constant.value,
          digits: 11,
        )};',
      );
