...
class PhysicalKeyData {
  ...
  static Map<String, List<int>> _readAndroidScanCodes(
    ...
    for (final RegExpMatch match in keyEntry.allMatches(keyboardLayout)) {
      ...
      androidNameToScanCodes.putIfAbsent(name, () => <int>[]).add(
    ...
    final Map<String, List<int>> result = nameToAndroidNames.map(
      (String name, List<String> androidNames) {
  ...
  static Map<String, PhysicalKeyEntry> _readHidEntries(
    ...
    for (final RegExpMatch match in usbMapRegExp.allMatches(input)) {
      ...
      final String enumName = match.namedGroup('enum')!.replaceAll(
        'MINIMIUM',
        'MINIMUM',
      );
    ...
    return entries.map(
      (int code, PhysicalKeyEntry entry) => MapEntry<String, PhysicalKeyEntry>(
        entry.name,
        entry,
      ),
    );
...
class PhysicalKeyEntry {
  ...
  static String getCommentName(String constantName) {
    ...
    return upperCamel.replaceAllMapped(
      RegExp(r'([A-Z])'),
  ...
  String toString() {
    ...
    return """'$constantName': (name: "$name", usbHidCode: ${toHex(
      usbHidCode,
    )}, """
        'linuxScanCode: ${toHex(evdevCode)}, xKbScanCode: ${toHex(
      xKbScanCode,
    )}, '
        'windowsKeyCode: ${toHex(windowsScanCode)}, macOSScanCode: ${toHex(
      macOSScanCode,
    )}, '
        'windowsScanCode: ${toHex(
      windowsScanCode,
    )}, chromiumSymbolName: $chromiumCode '
        'iOSScanCode: ${toHex(iOSScanCode)})$otherWebStr';
  }

  static int compareByUsbHidCode(
    PhysicalKeyEntry a,
    PhysicalKeyEntry b,
