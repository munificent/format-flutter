...
''');
  ...
  String get _logicalDefinitions {
    ...
    void printKey(
      ...
      lines.add(flutterId, '''
$firstComment  ///
$otherComments  static const LogicalKeyboardKey $constantName = LogicalKeyboardKey(${toHex(
        flutterId,
        digits: 11,
      )});
''');
  ...
  String? _otherComments(String name) {
    if (synonyms.containsKey(name)) {
      final Set<String> unionNames = synonyms[name]!.keys.map(
        (LogicalKeyEntry entry) => entry.constantName,
      ).toSet();
      return _wrapString(
  ...
  String get _logicalSynonyms {
    ...
    for (final SynonymKeyInfo synonymInfo in synonyms.values) {
      for (final LogicalKeyEntry key in synonymInfo.keys) {
        final LogicalKeyEntry synonym = logicalData.entryByName(
          synonymInfo.name,
        );
...
${_wrapString(constant.description)}  ///
  ...
  static const int ${constant.lowerCamelName} = ${toHex(
        constant.value,
        digits: 11,
      )};
''');
