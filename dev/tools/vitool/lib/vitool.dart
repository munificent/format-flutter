...
FrameData interpretSvg(String svgFilePath) {
  ...
  final List<SvgPath> paths = _interpretSvgGroup(
    svgElement.children,
    _Transform(),
  );
...
List<Point<double>> parsePoints(String points) {
  ...
  while (unParsed.isNotEmpty && _pointMatcher.hasMatch(unParsed)) {
    ...
    result.add(
      Point<double>(double.parse(m.group(1)!), double.parse(m.group(2)!)),
    );
    unParsed = m.group(3)!;
  }
  return result;
...
class SvgPathCommandBuilder {
  ...
  SvgPathCommand build(String type, List<Point<double>> points) {
    ...
    if (_isRelativeCommand(type)) {
      absPoints = points.map<Point<double>>(
        (Point<double> p) => p + lastPoint,
      ).toList();
    }
...
String _extractAttr(XmlElement element, String name) {
  try {
    return element.attributes.singleWhere(
      (XmlAttribute x) => x.name.local == name,
    ).value;
  } catch (e) {
    throw ArgumentError(
...
bool _hasAttr(XmlElement element, String name) {
  return element.attributes.where(
    (XmlAttribute a) => a.name.local == name,
  ).isNotEmpty;
}
