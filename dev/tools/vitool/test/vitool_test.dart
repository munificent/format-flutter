...
void main() {
  ...
  test('parsePoints', () {
    expect(parsePoints('1.0, 2.0'), const <Point<double>>[
  ...
  group('parseSvg', () {
    ...
    test('SVG group translate', () {
      final FrameData frameData = interpretSvg(
        testAsset('bar_group_translate.svg'),
      );
      expect(frameData.paths, const <SvgPath>[
        SvgPath('path_1', <SvgPathCommand>[
          SvgPathCommand('M', <Point<double>>[Point<double>(0.0, 34.0)]),
    ...
    test('SVG group rotate scale', () {
      final FrameData frameData = interpretSvg(
        testAsset('bar_group_rotate_scale.svg'),
      );
      expect(frameData.paths, const <PathMatcher>[
        PathMatcher(SvgPath('path_1', <SvgPathCommand>[
    ...
    test('SVG group opacity', () {
      final FrameData frameData = interpretSvg(
        testAsset('bar_group_opacity.svg'),
      );
      expect(frameData.paths, const <SvgPath>[
        SvgPath('path_1', <SvgPathCommand>[
          SvgPathCommand('M', <Point<double>>[Point<double>(0.0, 19.0)]),
    ...
    test('horizontal bar relative', () {
      ...
      final FrameData frameData = interpretSvg(
        testAsset('horizontal_bar_relative.svg'),
      );
      expect(frameData.paths, const <SvgPath>[
        SvgPath('path_1', <SvgPathCommand>[
          SvgPathCommand('M', <Point<double>>[Point<double>(0.0, 19.0)]),
    ...
    test('close in middle of path', () {
      // This asset uses the relative 'l' command instead of 'L'.
      final FrameData frameData = interpretSvg(
        testAsset('close_path_in_middle.svg'),
      );
      expect(frameData.paths, const <SvgPath>[
        SvgPath('path_1', <SvgPathCommand>[
...
class PathAnimationMatcher extends Matcher {
  ...
  bool matches(dynamic item, Map<dynamic, dynamic> matchState) {
    ...
    if (!const ListEquality<double>().equals(
      other.opacities,
      expected.opacities,
    )) {
      return false;
  ...
  bool commandsMatch(
    PathCommandAnimation expected,
    PathCommandAnimation other,
  ) {
    ...
    for (int i = 0; i < other.points.length; i += 1) {
      if (!const ListEquality<Point<double>>().equals(
        other.points[i],
        expected.points[i],
      )) {
        return false;
