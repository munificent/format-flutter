...
class IconButtonTemplate extends TokenTemplate {
  ...
  String _overlayColor() {
    ...
    MaterialStateProperty.resolveWith((Set<MaterialState> states) {
      if (states.contains(MaterialState.selected)) {
        if (states.contains(MaterialState.pressed)) {
          return ${componentColor('$tokenGroup.toggle.selected.pressed.state-layer')}.withOpacity(${opacity(
          '$tokenGroup.pressed.state-layer.opacity',
        )});
        }
        if (states.contains(MaterialState.hovered)) {
          return ${componentColor('$tokenGroup.toggle.selected.hover.state-layer')}.withOpacity(${opacity(
          '$tokenGroup.hover.state-layer.opacity',
        )});
        }
        if (states.contains(MaterialState.focused)) {
          return ${componentColor('$tokenGroup.toggle.selected.focus.state-layer')}.withOpacity(${opacity(
          '$tokenGroup.focus.state-layer.opacity',
        )});
        }
      }
      if (toggleable) { // toggleable but unselected case
        if (states.contains(MaterialState.pressed)) {
          return ${componentColor('$tokenGroup.toggle.unselected.pressed.state-layer')}.withOpacity(${opacity(
          '$tokenGroup.pressed.state-layer.opacity',
        )});
        }
        if (states.contains(MaterialState.hovered)) {
          return ${componentColor('$tokenGroup.toggle.unselected.hover.state-layer')}.withOpacity(${opacity(
          '$tokenGroup.hover.state-layer.opacity',
        )});
        }
        if (states.contains(MaterialState.focused)) {
          return ${componentColor('$tokenGroup.toggle.unselected.focus.state-layer')}.withOpacity(${opacity(
          '$tokenGroup.focus.state-layer.opacity',
        )});
        }
      }
    ...
    MaterialStateProperty.resolveWith((Set<MaterialState> states) {
      if (states.contains(MaterialState.selected)) {
        if (states.contains(MaterialState.pressed)) {
          return ${componentColor('$tokenGroup.selected.pressed.state-layer')}.withOpacity(${opacity(
          '$tokenGroup.pressed.state-layer.opacity',
        )});
        }
        if (states.contains(MaterialState.hovered)) {
          return ${componentColor('$tokenGroup.selected.hover.state-layer')}.withOpacity(${opacity(
          '$tokenGroup.hover.state-layer.opacity',
        )});
        }
        if (states.contains(MaterialState.focused)) {
          return ${componentColor('$tokenGroup.selected.focus.state-layer')}.withOpacity(${opacity(
          '$tokenGroup.focus.state-layer.opacity',
        )});
        }
      }
      if (states.contains(MaterialState.pressed)) {
        return ${componentColor('$tokenGroup.unselected.pressed.state-layer')}.withOpacity(${opacity(
          '$tokenGroup.pressed.state-layer.opacity',
        )});
      }
      if (states.contains(MaterialState.hovered)) {
        return ${componentColor('$tokenGroup.unselected.hover.state-layer')}.withOpacity(${opacity(
          '$tokenGroup.hover.state-layer.opacity',
        )});
      }
      if (states.contains(MaterialState.focused)) {
        return ${componentColor('$tokenGroup.unselected.focus.state-layer')}.withOpacity(${opacity(
          '$tokenGroup.focus.state-layer.opacity',
        )});
      }
      return Colors.transparent;
  ...
  String _minimumSize() {
    ...
    const MaterialStatePropertyAll<Size>(Size(${getToken('$tokenGroup.container.size')}, ${getToken(
        '$tokenGroup.container.size',
      )}))''';
    } else {
      return '''

    const MaterialStatePropertyAll<Size>(Size(${getToken('$tokenGroup.state-layer.size')}, ${getToken(
