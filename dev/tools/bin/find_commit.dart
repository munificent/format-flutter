...
String findCommit({
  ...
  if (primaryBranch == primaryTrunk) {
    ...
    final List<Commit> branchCommits = Commit.parseList(
      git(primaryRepoDirectory, <String>['log', Commit.formatArgument, primaryBranch, '--']),
    );
    final List<Commit> trunkCommits = Commit.parseList(
      git(primaryRepoDirectory, <String>['log', Commit.formatArgument, primaryTrunk, '--']),
    );
    ...
    if (branchCommits.last.hash == trunkCommits.last.hash) {
      ...
      log(
        '$primaryBranch branched from $primaryTrunk ${branchCommits.length - index} commits ago, trunk has advanced by ${trunkCommits.length - index} commits since then.',
      );
