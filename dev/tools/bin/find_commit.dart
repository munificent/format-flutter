...
class Commit {
  ...
  static List<Commit> parseList(String lines) {
    return lines.split('\n').where((String line) => line.isNotEmpty).map(
      parse,
...
String findCommit({
  ...
  if (primaryBranch == primaryTrunk) {
    ...
    final List<Commit> trunkCommits = Commit.parseList(git(
      primaryRepoDirectory,
    ...
    if (branchCommits.last.hash == trunkCommits.last.hash) {
      log(
        '$primaryBranch is even with $primaryTrunk, using last commit as anchor',
      );
  ...
  if (arguments.isEmpty ||
    ...
    print(
      findCommit(
        ...
        primaryBranch: git(primaryRepo, <String>[
          'rev-parse',
          '--abbrev-ref',
          'HEAD',
        ]).trim(),
