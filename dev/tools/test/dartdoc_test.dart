...
void main() {
  ...
  test('getBranchName does not call git if env LUCI_BRANCH provided', () {
    final Platform platform = FakePlatform(environment: <String, String>{'LUCI_BRANCH': branchName});

    final ProcessManager processManager = FakeProcessManager.empty();

    expect(getBranchName(platform: platform, processManager: processManager), branchName);
  });
  ...
  test('getBranchName calls git if env LUCI_BRANCH not provided', () {
    ...
    final ProcessManager processManager = FakeProcessManager.list(<FakeCommand>[
      const FakeCommand(command: <String>['git', 'status', '-b', '--porcelain'], stdout: '## $branchName'),
    ]);
  ...
  test("runPubProcess doesn't use the pub binary", () {
    final ProcessManager processManager = FakeProcessManager.list(<FakeCommand>[
      const FakeCommand(command: <String>['dart', 'pub', '--one', '--two']),
    ]);

    runPubProcess(dartBinaryPath: 'dart', arguments: <String>['--one', '--two'], processManager: processManager);
