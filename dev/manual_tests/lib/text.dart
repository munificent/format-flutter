...
class _FuzzerState extends State<Fuzzer> with SingleTickerProviderStateMixin {
  ...
  TextSpan _fiddleWith(TextSpan node) {
    return TextSpan(
      ...
      children: _fiddleWithChildren(
        node.children?.map(
              (InlineSpan child) => _fiddleWith(child as TextSpan),
            ).toList() ??
            <TextSpan>[],
      ),
    );
  ...
  double? _fiddleWithDouble(double? value, double defaultValue, double max) {
    switch (_random.nextInt(10)) {
      case 0:
        if (value == null) {
          return math.min(
            defaultValue * (0.95 + _random.nextDouble() * 0.1),
            max,
          );
        }
  ...
  List<TextSpan>? _fiddleWithChildren(List<TextSpan> children) {
    switch (_random.nextInt(100)) {
      ...
      case 4:
        children.insert(
          _random.nextInt(children.length + 1),
          _createRandomTextSpan(),
        );
      case 10:
      ...
      child: ListView(
        ...
        children: <Widget>[
          ...
          const Text(
            ...
            textAlign: TextAlign.center,
            style:
                TextStyle(inherit: false, fontSize: 14.0, color: Colors.white),
          ),
        ],
      ),
    );
                      ...
                      TextButton(
                        onPressed: _ticker.isActive
                            ...
                            : () {
                                ...
                                print(_text?.runes
                                    .map<String>(
                                      (int value) => 'U+${value.toRadixString(
                                        16,
                                      ).padLeft(4, '0').toUpperCase()}',
                                    )
                                    .join(' '));
                              },
