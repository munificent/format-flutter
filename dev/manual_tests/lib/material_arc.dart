...
class _PointDemoState extends State<_PointDemo> {
  ...
  Widget build(BuildContext context) {
    ...
    final MaterialPointArcTween arc = MaterialPointArcTween(
      begin: _begin,
      end: _end,
    );
...
class _RectangleDemoState extends State<_RectangleDemo> {
  ...
  Widget build(BuildContext context) {
    ...
    final MaterialRectArcTween arc = MaterialRectArcTween(
      begin: _begin,
      end: _end,
    );
    return RawGestureDetector(
      ...
      child: ClipRect(
        child: CustomPaint(
          key: _painterKey,
          foregroundPainter:
              _RectangleDemoPainter(repaint: _animation, arc: arc),
          // Watch out: if this IgnorePointer is left out, then gestures that
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          bottom: TabBar(
            tabs: _allDemos.map<Tab>(
              (_ArcDemo demo) => Tab(text: demo.title),
            ).toList(),
          ),
        ),
        ...
        body: TabBarView(
          children: _allDemos.map<Widget>(
            (_ArcDemo demo) => demo.builder(demo),
          ).toList(),
        ),
      ),
    );
