...
void main() {
  runApp(MaterialApp(
    ...
    home: Scaffold(
      ...
      body: const Center(child: RawKeyboardDemo()),
    ),
  ));
}
        ...
        builder: (BuildContext context, Widget? child) {
          ...
          if (data is RawKeyEventDataAndroid) {
            ...
            final String codePointChar = String.fromCharCode(
              combiningCharacterMask & data.codePoint,
            );
            dataText.add(Text(
              'codePoint: ${data.codePoint} (${_asHex(
                data.codePoint,
              )}: $codePointChar)',
            ));
            ...
            dataText.add(Text(
              'plainCodePoint: ${data.plainCodePoint} (${_asHex(
                data.plainCodePoint,
              )}: $plainCodePointChar)',
            ));
            dataText.add(
            ...
            dataText.add(
              Text('metaState: ${data.metaState} (${_asHex(data.metaState)})'),
            );
            dataText.add(
              Text('source: ${data.eventSource} (${_asHex(data.eventSource)})'),
            );
            dataText.add(
              Text('vendorId: ${data.vendorId} (${_asHex(data.vendorId)})'),
            );
            dataText.add(
              Text('productId: ${data.productId} (${_asHex(data.productId)})'),
            );
            ...
            dataText.add(
              Text('codePoint: ${data.codePoint} (${_asHex(data.codePoint)})'),
            );
            dataText.add(
              Text('hidUsage: ${data.hidUsage} (${_asHex(data.hidUsage)})'),
            );
            dataText.add(
              Text('modifiers: ${data.modifiers} (${_asHex(data.modifiers)})'),
            );
            ...
            dataText.add(
              Text('unicodeScalarValues: ${data.unicodeScalarValues}'),
            );
            dataText.add(
              Text('modifiers: ${data.modifiers} (${_asHex(data.modifiers)})'),
            );
            ...
            dataText.add(Text('scanCode: ${data.scanCode}'));
            dataText.add(
              Text('characterCodePoint: ${data.characterCodePoint}'),
            );
            dataText.add(
              Text('modifiers: ${data.modifiers} (${_asHex(data.modifiers)})'),
            );
          ...
          for (final ModifierKey modifier in data!.modifiersPressed.keys) {
            for (final KeyboardSide side in KeyboardSide.values) {
              if (data.isModifierPressed(modifier, side: side)) {
                dataText.add(Text(
                  '${_getEnumName(side)} ${_getEnumName(modifier).replaceAll(
                    'Modifier',
                    '',
                  )} pressed',
                ));
