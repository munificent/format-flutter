...
class FrameTimingSummarizer {
  ...
  Map<String, dynamic> get summary => <String, dynamic>{
    ...
    'frame_build_times': frameBuildTime.map<int>(
      (Duration datum) => datum.inMicroseconds,
    ).toList(),
    'frame_rasterizer_times': frameRasterizerTime.map<int>(
      (Duration datum) => datum.inMicroseconds,
    ).toList(),
...
int _countExceed<T extends Comparable<T>>(List<T> data, T threshold) {
  final int exceedsThresholdIndex = data.indexWhere(
    (T datum) => datum.compareTo(threshold) > 0,
  );
