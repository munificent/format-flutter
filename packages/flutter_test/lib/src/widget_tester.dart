...
class WidgetTester extends WidgetController
  ...
  Future<void> restartAndRestore() async {
    ...
    return TestAsyncUtils.guard<void>(() async {
      final Widget widget =
          ((binding.rootElement! as RenderObjectToWidgetElement<RenderObject>)
  ...
  void dispatchEvent(PointerEvent event, HitTestResult result) {
    if (event is PointerDownEvent) {
      final RenderObject innerTarget = result.path.map(
        (HitTestEntry candidate) => candidate.target,
      ).whereType<RenderObject>().first;
      ...
      for (final Element element in candidates) {
        ...
        if (!_isPrivate(widget.runtimeType)) {
          ...
          if (descendantText != null && numberOfWithTexts < 5) {
            final Iterable<Element> matches = find.widgetWithText(
              widget.runtimeType,
              descendantText,
            ).evaluate();
            if (matches.length == 1) {
  ...
  Future<void> showKeyboard(Finder finder) async {
    return TestAsyncUtils.guard<void>(() async {
      ...
      editable = state<EditableTextState>(find.descendant(
        of: finder,
