...
mixin TestDefaultBinaryMessengerBinding on BindingBase, ServicesBinding {
  ...
  static TestDefaultBinaryMessengerBinding
  get instance => BindingBase.checkInstance(_instance);
...
abstract class TestWidgetsFlutterBinding extends BindingBase
  ...
  static TestWidgetsFlutterBinding get instance => BindingBase.checkInstance(
    _instance,
  );
  ...
  @override
  Future<ui.AppExitResponse> exitApplication(
    ui.AppExitType exitType, [
    int exitCode = 0,
  ]) async {
    switch (exitType) {
  ...
  Future<void> _runTest(
    ...
    final Zone testZone = _parentZone!.fork(
      specification: errorHandlingZoneSpecification,
    );
    testZone
  ...
  Future<void> _runTestBody(
    ...
    runApp(
      Container(key: UniqueKey(), child: _preTestMessage),
...
class AutomatedTestWidgetsFlutterBinding extends TestWidgetsFlutterBinding {
  ...
  static AutomatedTestWidgetsFlutterBinding
  get instance => BindingBase.checkInstance(_instance);
  ...
  @override
  test_package.Timeout defaultTestTimeout = const test_package.Timeout(
    Duration(minutes: 10),
  );
          ...
          if (_phase != EnginePhase.compositingBits) {
            ...
            if (_phase != EnginePhase.paint && sendFramesToEngine) {
              ...
              if (_phase != EnginePhase.composite) {
                pipelineOwner.flushSemantics();
                assert(
                  _phase == EnginePhase.flushSemantics ||
...
class LiveTestWidgetsFlutterBinding extends TestWidgetsFlutterBinding {
  ...
  /// you can ensure a binding has been constructed by calling the
  /// [TestWidgetsFlutterBinding.ensureInitialized] function.
  static LiveTestWidgetsFlutterBinding
  get instance => BindingBase.checkInstance(_instance);
  ...
  int get microtaskCount {
    ...
    assert(
      false,
      'microtaskCount cannot be reported when running in real time',
    );
    return -1;
  }
  ...
  void _handleRenderViewPaint(
    ...
    if (_pointerIdToPointerRecord.isNotEmpty) {
      ...
      bool dirty = false;
      for (final _LiveTestPointerRecord record
          in _pointerIdToPointerRecord.values) {
      ...
      _pointerIdToPointerRecord.keys
          .where(
            (int pointer) => _pointerIdToPointerRecord[pointer]!.decay == 0,
          )
          .toList()
    ...
    switch (pointerEventSource) {
      case TestBindingEventSource.test:
        final _LiveTestPointerRecord? record =
            _pointerIdToPointerRecord[event.pointer];
        if (record != null) {
  ...
  Offset localToGlobal(Offset point, RenderView view) {
    ...
    final Matrix4 transform = view.configuration.toMatrix();
    final Offset pointInPhysicalPixels = MatrixUtils.transformPoint(
      transform,
      point,
    );
...
class TestViewConfiguration extends ViewConfiguration {
  ...
  TestViewConfiguration.fromView({
    ...
    super.size = _kDefaultTestViewportSize,
...
class _LiveTestPointerRecord {
  _LiveTestPointerRecord(this.pointer, this.position)
    : color =
          HSVColor.fromAHSV(0.8, (35.0 * pointer) % 360.0, 1.0, 1.0).toColor(),
      decay = 1;
  final int pointer;
  final Color color;
  Offset position;
  int
  decay; // >0 means down, <0 means up, increases by one each time, removed at 0
}
