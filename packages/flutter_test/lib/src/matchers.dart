...
Matcher coversSameAreaAs(Path expectedPath, {required Rect areaToCompare, int sampleSize = 20}) => _CoversSameAreaAs(
  expectedPath,
...
class _IsMethodCall extends Matcher {
  ...
  @override
  Description describe(Description description) {
    return description.add('has method name: ').addDescriptionOf(name).add(' with arguments: ').addDescriptionOf(
      arguments,
    );
  }
}
  ...
  bool matches(covariant Path actualPath, Map<dynamic, dynamic> matchState) {
    for (int i = 0; i < sampleSize; i += 1) {
      for (int j = 0; j < sampleSize; j += 1) {
        final Offset offset = Offset(i * (areaToCompare.width / sampleSize), j * (areaToCompare.height / sampleSize));

        if (!_samplePoint(matchState, actualPath, offset)) {
          return false;
        }

        final Offset noise = Offset(maxHorizontalNoise * random.nextDouble(), maxVerticalNoise * random.nextDouble());
  ...
  bool matches(dynamic node, Map<dynamic, dynamic> matchState) {
    ...
    if (actions.isNotEmpty) {
      ...
      if (unexpectedActions.isNotEmpty || missingActions.isNotEmpty) {
        return failWithDescription(
          matchState,
          'missing actions: ${_createEnumsSummary(missingActions)} unexpected actions: ${_createEnumsSummary(
            unexpectedActions,
          )}',
        );
      }
    ...
    if (customActions != null || hintOverrides != null) {
      ...
      if (hintOverrides?.onTapHint != null) {
        expectedCustomActions.add(
          CustomSemanticsAction.overridingAction(hint: hintOverrides!.onTapHint!, action: SemanticsAction.tap),
        );
      }
    ...
    if (flags.isNotEmpty) {
      ...
      if (unexpectedFlags.isNotEmpty || missingFlags.isNotEmpty) {
        return failWithDescription(
          matchState,
          'missing flags: ${_createEnumsSummary(missingFlags)} unexpected flags: ${_createEnumsSummary(
            unexpectedFlags,
          )}',
        );
      }
