...
class MinimumTextContrastGuideline extends AccessibilityGuideline {
  ...
  Future<Evaluation> _evaluateNode(
    ...
    for (final Element element in elements) {
      result +=
          await _evaluateElement(node, element, tester, image, byteData, view);
    }
    return result;
  }
  ...
  Future<Evaluation> _evaluateElement(
    ...
    tester.binding.renderView.applyPaintTransform(
      tester.binding.renderView.child!,
      rootTransform,
    );
    rootTransform.multiply(globalTransform);
    screenBounds = MatrixUtils.transformRect(
      rootTransform,
    ...
    final double targetContrastRatio = this.targetContrastRatio(
      fontSize,
      bold: isBold,
    );
