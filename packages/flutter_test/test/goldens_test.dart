...
void main() {
  ...
  void test(String description, FutureOr<void> Function() body) {
    test_package.test(description, () async {
      await io.IOOverrides.runZoned<FutureOr<void>>(
        ...
        fseGetType: (String path, bool followLinks) => fs.type(
          path,
          followLinks: followLinks,
        ),
        fseGetTypeSync: (String path, bool followLinks) => fs.typeSync(
          path,
          followLinks: followLinks,
        ),
  ...
  group('LocalFileComparator', () {
    ...
    test(
      'can be instantiated with uri that represents file in same folder',
      () {
        comparator = LocalFileComparator(
      ...
      group('fails', () {
        test(
          ...
          () async {
            ...
            await fs.file(fix('/golden.png')).writeAsBytes(
              _kColorFailurePngBytes,
            );
            await expectLater(() => doComparison(), throwsA(isFlutterError
                .having(
        ...
        test(
          ...
          () async {
            ...
            await expectLater(() => doComparison('subdir/golden.png'), throwsA(
        ...
        test('when golden file does not exist', () async {
          await expectLater(() => doComparison(), throwsA(isA<TestFailure>()
              .having(
        ...
        test('when images are not the same size', () async {
          ...
          await expectLater(() => doComparison(), throwsA(isFlutterError.having(
            (FlutterError error) => error.message,
            'message',
