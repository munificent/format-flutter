...
void main() {
  ...
  group('text', () {
    ...
    testWidgets('finds Text.rich widgets', (WidgetTester tester) async {
      await tester.pumpWidget(_boilerplate(
        const Text.rich(TextSpan(text: 't', children: <TextSpan>[TextSpan(text: 'e'), TextSpan(text: 'st')])),
      ));
    ...
    group('findRichText', () {
      testWidgets('finds RichText widgets when enabled', (WidgetTester tester) async {
        await tester.pumpWidget(
          _boilerplate(RichText(text: const TextSpan(text: 't', children: <TextSpan>[TextSpan(text: 'est')]))),
        );
      ...
      testWidgets('does not find Text and RichText separated by semantics widgets twice', (WidgetTester tester) async {
      ...
      testWidgets('finds Text.rich widgets when enabled', (WidgetTester tester) async {
        await tester.pumpWidget(_boilerplate(
          const Text.rich(TextSpan(text: 't', children: <TextSpan>[TextSpan(text: 'est'), TextSpan(text: '3')])),
        ));
  ...
  group('textContaining', () {
    ...
    testWidgets('finds Text.rich widgets', (WidgetTester tester) async {
      await tester.pumpWidget(_boilerplate(
        const Text.rich(TextSpan(
          text: 'this',
          children: <TextSpan>[TextSpan(text: 'is'), TextSpan(text: 'a'), TextSpan(text: 'test')],
        )),
      ));
    ...
    testWidgets('finds EditableText widgets', (WidgetTester tester) async {
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: _boilerplate(TextField(controller: TextEditingController()..text = 'this is test'))),
      ));
  ...
  group('semantics', () {
    ...
    testWidgets('finds Semantically labeled widgets', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        _boilerplate(Semantics(label: 'Add', button: true, child: const TextButton(onPressed: null, child: Text('+')))),
      );
    ...
    testWidgets('finds Semantically labeled widgets by RegExp', (WidgetTester tester) async {
      final SemanticsHandle semanticsHandle = tester.ensureSemantics();
      await tester.pumpWidget(
        _boilerplate(Semantics(container: true, child: const Row(children: <Widget>[Text('Hello'), Text('World')]))),
      );
  ...
  group('hitTestable', () {
    testWidgets('excludes non-hit-testable widgets', (WidgetTester tester) async {
      await tester.pumpWidget(_boilerplate(IndexedStack(
  ...
  testWidgets('ChainedFinders chain properly', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      _boilerplate(Column(children: <Widget>[Container(key: key1, child: const Text('1')), const Text('2')])),
    );
    ...
    final Text text =
        find.descendant(of: find.byKey(key1), matching: find.byType(Text)).last.evaluate().single.widget as Text;

    expect(text.data, '1');
  });

  testWidgets('finds multiple subtypes', (WidgetTester tester) async {
    await tester.pumpWidget(_boilerplate(Row(
      children: <Widget>[
