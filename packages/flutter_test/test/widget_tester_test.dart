...
void main() {
  ...
  group('findsNothing', () {
    ...
    testWidgets('fails with a descriptive message', (WidgetTester tester) async {
      ...
      expect(message, contains(
        'Actual: _TextFinder:<exactly one widget with text "foo": Text("foo", textDirection: ltr, dependencies: [MediaQuery])>\n',
      ));
    ...
    testWidgets('fails with a descriptive message when skipping', (WidgetTester tester) async {
      ...
      expect(message, contains('Expected: no matching nodes in the widget tree\n'));
      expect(message, contains(
        'Actual: _TextFinder:<exactly one widget with text "foo" (ignoring offstage widgets): Text("foo", textDirection: ltr, dependencies: [MediaQuery])>\n',
      ));
      expect(message, contains('Which: means one was found but none were expected\n'));
    });
  });
  ...
  group('pumping', () {
    testWidgets('pumping', (WidgetTester tester) async {
      ...
      final AnimationController test = AnimationController(duration: const Duration(milliseconds: 5100), vsync: tester);
  ...
  group('find.descendant', () {
    testWidgets('finds one descendant', (WidgetTester tester) async {
      await tester.pumpWidget(
        const Row(textDirection: TextDirection.ltr, children: <Widget>[Column(children: fooBarTexts)]),
      );

      expect(find.descendant(of: find.widgetWithText(Row, 'foo'), matching: find.text('bar')), findsOneWidget);
    });
    ...
    testWidgets('finds two descendants with different ancestors', (WidgetTester tester) async {
      ...
      expect(find.descendant(of: find.widgetWithText(Column, 'foo'), matching: find.text('bar')), findsNWidgets(2));
    });
    ...
    testWidgets('fails with a descriptive message', (WidgetTester tester) async {
      ...
      late TestFailure failure;
      try {
        expect(find.descendant(of: find.widgetWithText(Column, 'foo'), matching: find.text('bar')), findsOneWidget);
      } on TestFailure catch (e) {
        failure = e;
      }
  ...
  group('find.ancestor', () {
    testWidgets('finds one ancestor', (WidgetTester tester) async {
      ...
      expect(find.ancestor(of: find.text('bar'), matching: find.widgetWithText(Row, 'foo')), findsOneWidget);
    });
    ...
    testWidgets('fails with a descriptive message', (WidgetTester tester) async {
      ...
      late TestFailure failure;
      try {
        expect(find.ancestor(of: find.text('bar'), matching: find.widgetWithText(Column, 'foo')), findsOneWidget);
      } on TestFailure catch (e) {
        failure = e;
      }
    ...
    testWidgets('Root not matched by default', (WidgetTester tester) async {
      ...
      expect(find.ancestor(of: find.byType(Column), matching: find.widgetWithText(Column, 'foo')), findsNothing);
    });
    ...
    testWidgets('Match the root', (WidgetTester tester) async {
      ...
      expect(
        find.descendant(of: find.byType(Column), matching: find.widgetWithText(Column, 'foo'), matchRoot: true),
        findsOneWidget,
      );
    });
    ...
    testWidgets('is fast in deep tree', (WidgetTester tester) async {
      await tester.pumpWidget(Directionality(
        ...
        child: _deepWidgetTree(
          depth: 1000,
          child: Row(children: <Widget>[_deepWidgetTree(depth: 1000, child: const Column(children: fooBarTexts))]),
        ),
      ));
  ...
  group('showKeyboard', () {
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        await tester.pumpWidget(MaterialApp(home: Material(child: Offstage(child: TextFormField()))));
  ...
  group('Accessibility announcements testing API', () {
    testWidgets('Returns the list of announcements', (WidgetTester tester) async {
      ...
      expect(TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger.checkMockMessageHandler(
    ...
    test('New test API is not breaking existing tests', () async {
      ...
      await SemanticsService.announce('announcement 1', TextDirection.rtl, assertiveness: Assertiveness.assertive);
      expect(log, equals(<Map<String, dynamic>>[
        <String, dynamic>{
          'type': 'announce',
          'data': <String, dynamic>{'message': 'announcement 1', 'textDirection': 0, 'assertiveness': 1},
        },
      ]));
