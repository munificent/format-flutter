...
mixin CommandHandlerFactory {
  ...
  Future<Health> _getHealth(Command command) async => const Health(
    HealthStatus.ok,
  );
  ...
  Future<Result> _waitForCondition(Command command) async {
    ...
    final WaitCondition condition = deserializeCondition(
      waitForConditionCommand.condition,
    );
  ...
  Future<DiagnosticsTreeResult> _getDiagnosticsTree(
    ...
    return DiagnosticsTreeResult(diagnosticsNode.toJsonMap(
  ...
  Future<Result> _scroll(
    ...
    await Future<
      void
    >.value(); // so that down and move don't happen in the same microtask
