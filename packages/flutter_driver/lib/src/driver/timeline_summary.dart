...
class TimelineSummary {
  ...
  ]) => _extractFrameDurations().where(
    (Duration duration) => duration > kBuildBudget,
  ).length;
  ...
  int computeMissedFrameRasterizerBudgetCount([
    Duration frameBuildBudget = kBuildBudget,
  ]) => _extractGpuRasterizerDrawDurations().where(
    (Duration duration) => duration > kBuildBudget,
  ).length;
  ...
  Map<String, dynamic> get summaryJson {
    ...
    final Map<String, dynamic> timelineSummary = <String, dynamic>{
      ...
      'frame_build_times': _extractFrameDurations().map<int>(
        (Duration duration) => duration.inMicroseconds,
      ).toList(),
      'frame_rasterizer_times': _extractGpuRasterizerDrawDurations().map<int>(
        (Duration duration) => duration.inMicroseconds,
      ).toList(),
  ...
  Future<void> writeTimelineToFile(
    ...
    final File file = fs.file(
      path.join(destinationDirectory, '$traceName.timeline.json'),
    );
  ...
  List<TimelineEvent> _extractNamedEvents(String name) {
    return _timeline.events!.where(
      (TimelineEvent event) => event.name == name,
    ).toList();
  }

  List<TimelineEvent> _extractEventsWithNames(Set<String> names) {
    return _timeline.events!.where(
      (TimelineEvent event) => names.contains(event.name),
    ).toList();
  }
  ...
  List<Duration> _extractBeginEndEvents(String name) {
    return _extractDurations(
      ...
      (TimelineEvent beginEvent, TimelineEvent endEvent) {
        return Duration(
          microseconds: endEvent.timestampMicros! - beginEvent.timestampMicros!,
        );
      },
    );
  }
  ...
  double _percentileInMillis(List<Duration> durations, double percentile) {
    ...
    final List<double> doubles = durations.map<double>(
  ...
  _sceneDisplayLagSummarizer() => SceneDisplayLagSummarizer(
    _extractNamedEvents(kSceneDisplayLagEvent),
  );

  List<Duration> _extractGpuRasterizerDrawDurations() => _extractBeginEndEvents(
    kRasterizeFrameEventName,
  );

  ProfilingSummarizer _profilingSummarizer() => ProfilingSummarizer.fromEvents(
    _extractEventsWithNames(kProfilingEvents),
  );

  List<Duration> _extractFrameDurations() => _extractBeginEndEvents(
  ...
  RasterCacheSummarizer _rasterCacheSummarizer() => RasterCacheSummarizer(
    _extractNamedEvents(kRasterCacheEvent),
  );

  GCSummarizer _gcSummarizer() => GCSummarizer.fromEvents(
    _extractEventsWithNames(kGCRootEvents),
  );
}
