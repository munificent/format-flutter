  ...
  group('TimelineSummary', () {
    ...
    group('percentile_frame_rasterizer_time_millis', () {
      ...
      test('computes 90th frame rasterizer time in milliseconds', () {
        for (int i = 0; i < sequences.length; ++i) {
          expect(summarize(rasterizeTimeSequenceInMillis(sequences[i]))
              .computePercentileFrameRasterizerTimeMillis(90.0), p90s[i]);
        }
      });
      ...
      test('compute 99th frame rasterizer time in milliseconds', () {
        ...
        expect(summarize(rasterizeTimeSequenceInMillis(sequence))
            .computePercentileFrameRasterizerTimeMillis(99.0), 99);
      });
    });
    ...
    group('writeTimelineToFile', () {
      ...
      test('writes timeline to JSON file without summary', () async {
        ...
        final String written = await fs.file(
          path.join(tempDir.path, 'test.timeline.json'),
        ).readAsString();
      ...
      test('writes summary to JSON file', () async {
        ...
        final String written = await fs.file(
          path.join(tempDir.path, 'test.timeline_summary.json'),
        ).readAsString();
    ...
    group('SceneDisplayLagSummarizer tests', () {
      ...
      test('average_vsyncs_missed', () async {
        final SceneDisplayLagSummarizer summarizer = summarize(
          <Map<String, dynamic>>[
            lagBegin(1000, 4),
            lagEnd(2000, 4),
            lagBegin(1200, 12),
    ...
    group('VsyncFrameLagSummarizer tests', () {
      ...
      test('average_vsync_frame_lag', () async {
        final VsyncFrameLagSummarizer summarizer = summarize(
          <Map<String, dynamic>>[
            platformVsync(10),
            vsyncCallback(12),
            platformVsync(16),
      ...
      test('malformed_event_ordering', () async {
        final VsyncFrameLagSummarizer summarizer = summarize(
          <Map<String, dynamic>>[vsyncCallback(10), platformVsync(10)],
        );
      ...
      test('penalize_consecutive_vsyncs', () async {
        final VsyncFrameLagSummarizer summarizer = summarize(
          <Map<String, dynamic>>[platformVsync(10), platformVsync(12)],
        );
      ...
      test('pick_nearest_platform_vsync', () async {
        final VsyncFrameLagSummarizer summarizer = summarize(<
          Map<String, dynamic>
        >[platformVsync(10), platformVsync(12), vsyncCallback(18)]);
    ...
    group('RefreshRateSummarizer tests', () {
      ...
      List<Map<String, dynamic>> populateEvents({
        ...
        for (int i = 0; i < numberOfEvents; i++) {
          final int randomMargin = margin >= 1
              ? (-margin + Random().nextInt(margin * 2))
              : 0;
          final int endTime = startTimeInNanoseconds + interval + randomMargin;
          events.add(vsyncCallback(
            0,
      ...
      test('Identify illegal refresh rates.', () async {
        ...
        expect(
          summary.framesWithIllegalRefreshRate.first,
