...
void main() {
  ...
  group('waitForCondition', () {
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        driverExtension
            .call(
              const WaitForCondition(NoPendingPlatformMessages()).serialize(),
            )
            .then<void>(expectAsync1((Map<String, dynamic> r) {
              result = r;
            }));
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        const MethodChannel channel1 = MethodChannel(
          'helloChannel1',
          JSONMethodCodec(),
        );
        tester.binding.defaultBinaryMessenger.setMockMessageHandler(
  ...
  group('getSemanticsId', () {
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        expect(
          response['response'],
          contains('Bad state: No semantics data found'),
        );
      },
  ...
  testWidgets('getOffset', (WidgetTester tester) async {
    ...
    expect(
      await getOffset(OffsetType.bottomLeft),
      const Offset(40, 30 + 120.0),
    );
    expect(
      await getOffset(OffsetType.bottomRight),
      const Offset(40 + 100.0, 30 + 120.0),
    );
    expect(
      await getOffset(OffsetType.center),
  ...
  testWidgets('ancestor finder', (WidgetTester tester) async {
    await silenceDriverLogger(() async {
      ...
      // Find nothing
      Future<Offset?> result = getAncestorTopLeft(
        of: 'leftchild',
        matching: 'leftchild',
      );
      await tester.pump(const Duration(seconds: 2));
  ...
  group('enableTextEntryEmulation', () {
    ...
    const Widget testWidget = MaterialApp(
      home: Material(
        child: Center(
          child: TextField(key: ValueKey<String>('foo'), autofocus: true),
        ),
      ),
    );
  ...
  group('waitForTappable', () {
    ...
    testWidgets(
      'returns true when widget is tappable',
      (WidgetTester tester) async {
        driverExtension = FlutterDriverExtension(
          (String? arg) async => '',
          true,
