...
void main() {
  ...
  group('VMServiceFlutterDriver.connect', () {
    ...
    setUp(() {
      ...
      fakeClient.responses['get_health'] = makeFakeResponse(<String, dynamic>{
        'status': 'ok',
      });
    });
    ...
    test('Retries while Dart VM service is not available', () async {
      ...
      expect(
        vmServiceConnectFunction('http://foo.bar', <String, dynamic>{}).timeout(
    ...
    test('Retries connections if isolate is not available', () async {
      fakeIsolate.pauseEvent = vms.Event(
        kind: vms.EventKind.kPauseStart,
        timestamp: 0,
      );
    ...
    test('connects to isolate paused mid-flight', () async {
      fakeIsolate.pauseEvent = vms.Event(
        kind: vms.EventKind.kPauseBreakpoint,
        timestamp: 0,
      );
  ...
  group('VMServiceFlutterDriver', () {
    ...
    group('BySemanticsLabel', () {
      ...
      test('finds by Semantic label using RegExp', () async {
        await driver.tap(
          find.bySemanticsLabel(RegExp('^foo')),
    ...
    group('getText', () {
      test('sends the getText command', () async {
        fakeClient.responses['get_text'] = makeFakeResponse(<String, dynamic>{
          'text': 'hello',
        });
        final String result =
    ...
    group('sendTextInputAction', () {
      test('sends the SendTextInputAction command with action done', () async {
        fakeClient.responses['send_text_input_action'] = makeFakeResponse(
          <String, dynamic>{},
        );
    ...
    group('getLayerTree', () {
      test('sends the getLayerTree command', () async {
        fakeClient.responses['get_layer_tree'] = makeFakeResponse(
          <String, String>{'tree': 'hello'},
        );
    ...
    group('getWidgetDiagnostics', () {
      test('sends the getWidgetDiagnostics command', () async {
        fakeClient.responses['get_diagnostics_tree'] = makeFakeResponse(
          <String, dynamic>{},
        );
        await driver.getWidgetDiagnostics(
          find.byTooltip('foo'),
          timeout: _kTestTimeout,
        );
        expect(fakeClient.commandLog, <String>[
    ...
    group('getRenderObjectDiagnostics', () {
      test('sends the getRenderObjectDiagnostics command', () async {
        ...
        await driver.getRenderObjectDiagnostics(
          find.byTooltip('foo'),
          timeout: _kTestTimeout,
        );
        expect(fakeClient.commandLog, <String>[
    ...
    group('waitForCondition', () {
      test('sends the wait for NoPendingFrameCondition command', () async {
        fakeClient.responses['waitForCondition'] = makeFakeResponse(
          <String, dynamic>{},
        );
    ...
    group('traceAction', () {
      ...
      test('with time interval', () async {
        ...
        fakeClient.timelineResponses[1000001] = vms.Timeline.parse(
          <String, dynamic>{
            'traceEvents': <dynamic>[
        ...
        expect(
          timeline.events!.map((TimelineEvent event) => event.name),
          <String>['test event', 'test event 2'],
        );
      });
    });
    ...
    group('sendCommand error conditions', () {
      ...
      test('remote error', () async {
        ...
        await expectLater(() => driver.waitFor(find.byTooltip('foo')), throwsA(
    ...
    group('setSemantics', () {
      test('can be enabled', () async {
        fakeClient.responses['set_semantics'] = makeFakeResponse(
          <String, Object>{'changedState': true},
        );
      ...
      test('can be disabled', () async {
        fakeClient.responses['set_semantics'] = makeFakeResponse(
          <String, Object>{'changedState': false},
        );
    ...
    group('runUnsynchronized', () {
      test('wrap waitFor with runUnsynchronized', () async {
        fakeClient.responses['waitFor'] = makeFakeResponse(<String, dynamic>{});
        fakeClient.responses['set_frame_sync'] = makeFakeResponse(
          <String, dynamic>{},
        );
  ...
  group('WebFlutterDriver with logCommunicationToFile', () {
    ...
    test('logCommunicationToFile = true', () async {
      ...
      await driver.waitFor(
        find.byTooltip('logCommunicationToFile test'),
        timeout: _kTestTimeout,
      );
  ...
  group('WebFlutterDriver with printCommunication', () {
    ...
    test('printCommunication = true', () async {
      ...
      await driver.waitFor(
        find.byTooltip('printCommunication test'),
        timeout: _kTestTimeout,
      );
      expect(log, <String>[
  ...
  group('WebFlutterDriver', () {
    ...
    group('ByValueKey', () {
      ...
      test('finds by ValueKey', () async {
        fakeConnection.responses['tap'] = jsonEncode(
...
class FakeVmService extends Fake implements vms.VmService {
  ...
  Future<vms.Response> callServiceExtension(
    ...
    assert(
      response != null,
      'Failed to create a response for ${args['command']}',
    );
    return response!;
  }
