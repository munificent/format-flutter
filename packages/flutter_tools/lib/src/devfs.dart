...
class DevFSByteContent extends DevFSContent {
  ...
  @override
  Stream<List<int>> contentsAsStream() => Stream<List<int>>.fromIterable(
    <List<int>>[_bytes],
  );
}
...
class DevFS {
  ...
  Uri deviceUriToHostUri(Uri deviceUri) {
    ...
    if (deviceUriString.startsWith(baseUriString)) {
      final String deviceUriSuffix = deviceUriString.substring(
        baseUriString.length,
      );
  ...
  Future<Uri> create() async {
    _logger.printTrace(
      'DevFS: Creating new filesystem on the device ($_baseUri)',
    );
    try {
    ...
    _logger.printTrace(
      'DevFS: Created new filesystem on the device ($_baseUri)',
    );
    return _baseUri!;
  }
  ...
  Future<UpdateFSReport> update({
    ...
    _widgetCacheOutputFile = _fileSystem.file(
      '$dillOutputPath.incremental.dill.widget_cache',
    );
      ...
      bundle.entries.forEach((String archivePath, DevFSContent content) {
        ...
        switch (bundle.entryKinds[archivePath]) {
          case AssetKind.shader:
            final Future<DevFSContent?> pending = shaderCompiler
                .recompileShader(content);
          ...
          case AssetKind.model:
            ...
            final Future<DevFSContent?> pending = sceneImporter.reimportScene(
              content,
            );
            pendingAssetBuilds.add(pending);
            pending.then((DevFSContent? content) {
    ...
    if (!bundleFirstUpload) {
      ...
      if (compiledBinary.isNotEmpty) {
        ...
        final DevFSFileContent content = DevFSFileContent(
          _fileSystem.file(compiledBinary),
        );
        syncedBytes += content.size;
    ...
    if (dirtyEntries.isNotEmpty) {
      await (devFSWriter ?? _httpWriter).write(
        dirtyEntries,
        _baseUri!,
        _httpWriter,
      );
