...
class GitHubTemplateCreator {
  ...
  static String toolCrashSimilarIssuesURL(String errorString) {
    return 'https://github.com/flutter/flutter/issues?q=is%3Aissue+${Uri.encodeQueryComponent(
      errorString,
    )}';
  }
  ...
  static String sanitizedCrashException(Object error) {
    if (error is ProcessException) {
      ...
      return '${error.runtimeType}: ${LineSplitter.split(
        error.stackTrace.toString(),
      ).take(1)}';
    } else if (error is String) {
      // Force comma separator to standardize.
      return 'String: <${NumberFormat(null, 'en_US').format(
        error.length,
      )} characters>';
    }
    // Exception, other.
...
''';
  ...
  String _projectMetadataInformation() {
    ...
    try {
      project = _flutterProjectFactory.fromDirectory(
    ...
    try {
      ...
      if (file.existsSync()) {
        ...
        for (final String plugin
          ...
          description.writeln(
            pathParts.isEmpty ? pluginParts.first : pathParts.last,
          );
        }
      }
