...
class CachedArtifacts implements Artifacts {
  ...
  FileSystemEntity getHostArtifact(HostArtifact artifact) {
    switch (artifact) {
      ...
      case HostArtifact.idevicesyslog:
      case HostArtifact.idevicescreenshot:
        final String artifactFileName = _hostArtifactToFileName(
          artifact,
          _platform,
        );
        return _cache.getArtifactDirectory('libimobiledevice').childFile(
          artifactFileName,
        );
      case HostArtifact.skyEnginePath:
      ...
      case HostArtifact.iosDeploy:
        ...
        return _cache.getArtifactDirectory('ios-deploy').childFile(
          artifactFileName,
        );
      case HostArtifact.iproxy:
        final String artifactFileName = _hostArtifactToFileName(
        ...
        return _cache.getArtifactDirectory('usbmuxd').childFile(
          artifactFileName,
        );
      case HostArtifact.impellerc:
      case HostArtifact.scenec:
      case HostArtifact.libtessellator:
        ...
        return _fileSystem.file(
          _fileSystem.path.join(engineDir, artifactFileName),
        );
    }
  }
  ...
  String _getAndroidArtifactPath(
    ...
    switch (artifact) {
      case Artifact.genSnapshot:
        ...
        final String hostPlatform = getNameForHostPlatform(
  ...
  String _getFuchsiaArtifactPath(
    ...
    switch (artifact) {
      case Artifact.genSnapshot:
        final String genSnapshot = mode.isRelease
  ...
  String _getHostArtifactPath(
    ...
    switch (artifact) {
      ...
      case Artifact.fontSubset:
      case Artifact.constFinder:
        return _cache.getArtifactDirectory('engine').childDirectory(
          _enginePlatformDirectoryName(platform),
        ).childFile(_artifactToFileName(artifact, _platform, mode)!).path;
      case Artifact.flutterFramework:
      case Artifact.flutterXcframework:
      case Artifact.fuchsiaFlutterRunner:
      case Artifact.fuchsiaKernelCompiler:
...
String _getIosEngineArtifactPath(
  ...
  for (final Directory platformDirectory
    ...
    final bool simulatorDirectory = platformDirectory.basename.endsWith(
      '-simulator',
    );
  ...
  return flutterFrameworkSource.childDirectory(
...
class CachedLocalEngineArtifacts implements Artifacts {
  ...
  String _getDartSdkPath() {
    final String builtPath = _fileSystem.path.join(
      _hostEngineOutPath,
      'dart-sdk',
    );
  ...
  String _getPrebuiltTarget() {
    final TargetPlatform hostPlatform = _currentHostPlatform(
      _platform,
...
class CachedLocalWebSdkArtifacts implements Artifacts {
  ...
  @override
  String getEngineType(TargetPlatform platform, [BuildMode? mode]) => _parent
      .getEngineType(platform, mode);
  ...
  String _getFlutterPrebuiltsPath() {
    final String engineSrcPath = _fileSystem.path.dirname(
      _fileSystem.path.dirname(_webSdkPath),
    );
    return _fileSystem.path.join(engineSrcPath, 'flutter', 'prebuilts');
  }
...
class OverrideArtifacts implements Artifacts {
  ...
  @override
  String getEngineType(TargetPlatform platform, [BuildMode? mode]) => parent
      .getEngineType(platform, mode);
