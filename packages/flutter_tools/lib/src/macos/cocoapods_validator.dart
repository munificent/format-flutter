...
class CocoaPodsValidator extends DoctorValidator {
  ...
  Future<ValidationResult> validate() async {
    ...
    if (cocoaPodsStatus == CocoaPodsStatus.recommended) {
      messages.add(
        ValidationMessage(_userMessages.cocoaPodsVersion((await _cocoaPods.cocoaPodsVersionText).toString())),
      );
    } else {
      if (cocoaPodsStatus == CocoaPodsStatus.notInstalled) {
        status = ValidationType.missing;
        messages.add(
          ValidationMessage.error(_userMessages.cocoaPodsMissing(noCocoaPodsConsequence, cocoaPodsInstallInstructions)),
        );
