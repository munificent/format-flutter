...
class Template {
  ...
  Template._(
    ...
    for (final FileSystemEntity entity
      ...
      if (relativePath.contains(templateExtension)) {
        ...
        _templateFilePaths[relativePath] = fileSystem.path.absolute(
          entity.path,
        );
      }
    }
  ...
  int render(
    ...
    String? renderPath(String relativeDestinationPath) {
      final Match? match = _kTemplateLanguageVariant.matchAsPrefix(
    ...
    _templateFilePaths.forEach((
      ...
      final String relativePathForLogging = _fileSystem.path.relative(
      ...
      if (finalDestinationFile.existsSync()) {
        if (overwriteExisting) {
          ...
          if (printStatusWhenWriting) {
            _logger.printTrace(
              '  $relativePathForLogging (existing - skipped)',
            );
          }
          return;
        }
      ...
      if (sourceFile.path.endsWith(imageTemplateExtension)) {
        ...
        if (potentials.any((File file) => file.existsSync())) {
          final File imageSourceFile = potentials.firstWhere(
      ...
      if (sourceFile.path.endsWith(templateExtension)) {
        ...
        if (finalDestinationFile.path.endsWith('.kt') &&
          ...
          context['androidIdentifier'] = _escapeKotlinKeywords(
            androidIdentifier,
          );
        }
