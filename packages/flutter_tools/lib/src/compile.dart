...
class StdoutHandler {
  ...
  void handler(String message) {
    ...
    if (messageBoundaryKey != null && message.startsWith(messageBoundaryKey)) {
      ...
      final String fileName = message.substring(
        messageBoundaryKey.length + 1,
        spaceDelimiter,
      );
      final int errorCount = int.parse(
...
List<String> buildModeOptions(BuildMode mode, List<String> dartDefines) =>
    switch (mode) {
      BuildMode.debug => <String>[
          ...
          if (!dartDefines.any(
            (String define) => define.startsWith('dart.vm.profile'),
          ))
            '-Ddart.vm.profile=false',
          if (!dartDefines.any(
            (String define) => define.startsWith('dart.vm.product'),
          ))
...
class _RecompileRequest extends _CompilationRequest {
  ...
  @override
  Future<CompilerOutput?> _run(
    DefaultResidentCompiler compiler,
  ) async => compiler._recompile(this);
}
...
class _CompileExpressionRequest extends _CompilationRequest {
  ...
  Future<CompilerOutput?> _run(
    DefaultResidentCompiler compiler,
  ) async => compiler._compileExpression(this);
}
...
class _CompileExpressionToJsRequest extends _CompilationRequest {
  ...
  Future<CompilerOutput?> _run(
    DefaultResidentCompiler compiler,
  ) async => compiler._compileExpressionToJs(this);
}
...
class DefaultResidentCompiler implements ResidentCompiler {
  ...
  Future<CompilerOutput?> _recompile(_RecompileRequest request) async {
    ...
    if (request.additionalSourceUri != null) {
      additionalSourceUri = request.packageConfig.toPackageUri(
