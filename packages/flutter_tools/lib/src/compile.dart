...
class StdoutHandler {
  StdoutHandler({required Logger logger, required FileSystem fileSystem}) : _logger = logger, _fileSystem = fileSystem {
    reset();
  }
  ...
  void handler(String message) {
    ...
    if (messageBoundaryKey != null && message.startsWith(messageBoundaryKey)) {
      ...
      final CompilerOutput output = CompilerOutput(fileName, errorCount, sources, expressionData: expressionData);
...
List<String> buildModeOptions(BuildMode mode, List<String> dartDefines) => switch (mode) {
      ...
      BuildMode.release => <String>['-Ddart.vm.profile=false', '-Ddart.vm.product=true', ...kDartCompilerExperiments],
...
class KernelCompiler {
  ...
  Future<CompilerOutput?> compile({
    ...
    final List<String> command = <String>[
      ...
      if (initializeFromDill != null) ...<String>['--incremental', '--initialize-from-dill', initializeFromDill],
    ...
    server.stdout.transform<String>(utf8.decoder).transform<String>(const LineSplitter()).listen(
...
class DefaultResidentCompiler implements ResidentCompiler {
  ...
  Future<CompilerOutput?> _compile(String scriptUri, String? outputPath, {String? additionalSourceUri}) async {
    final TargetPlatform? platform = (targetModel == TargetModel.dartdevc) ? TargetPlatform.web_javascript : null;
    final String frontendServer = _artifacts.getArtifactPath(
      Artifact.frontendServerSnapshotForEngineDartSdk,
      platform: platform,
    ...
    final List<String> command = <String>[
      ...
      if (librariesSpec != null && platformDill == null) ...<String>['--libraries-spec', librariesSpec!],
      if (packagesPath != null) ...<String>['--packages', packagesPath!],
      ...buildModeOptions(buildMode, dartDefines),
      if (trackWidgetCreation) '--track-widget-creation',
      for (final String root in fileSystemRoots) ...<String>['--filesystem-root', root],
    ...
    _server?.stdout.transform<String>(utf8.decoder).transform<String>(const LineSplitter()).listen(
      _stdoutHandler.handler,
      onDone: () {
