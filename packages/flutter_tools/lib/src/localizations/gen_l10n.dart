...
Future<LocalizationsGenerator> generateLocalizations({
  ...
  if (options.format) {
    ...
    final String dartBinary = artifacts.getArtifactPath(
...
String _defaultSyntheticPackagePath(FileSystem fileSystem) => fileSystem.path
    .join('.dart_tool', 'flutter_gen');
...
/// localizations tool.
String _syntheticL10nPackagePath(FileSystem fileSystem) => fileSystem.path.join(
  _defaultSyntheticPackagePath(fileSystem),
  'gen_l10n',
);
...
List<String> generateMethodArguments(Message message) {
  return message.placeholders.values.map(
...
String generateNumberFormattingLogic(Message message) {
  ...
  final Iterable<String> formatStatements = message.placeholders.values
      ...
      .map((Placeholder placeholder) {
        ...
        final Iterable<String> parameters = placeholder.optionalParameters
            .map<String>((OptionalParameter parameter) {
              if (parameter.value is num) {
                return '${parameter.name}: ${parameter.value}';
              } else {
                return "${parameter.name}: '${generateString(
...
String generateBaseClassMethod(Message message, LocaleInfo? templateArbLocale) {
  ...
  if (message.placeholders.isNotEmpty) {
    return baseClassMethodTemplate
        ...
        .replaceAll(
          '@(parameters)',
          generateMethodParameters(message).join(', '),
        );
  }
...
String _generateLookupByAllCodes(
  ...
  final Iterable<LocaleInfo> localesWithAllCodes = allBundles.locales.where(
    (LocaleInfo locale) {
  ...
  final Iterable<String> switchClauses = localesWithAllCodes.map<String>(
    (LocaleInfo locale) {
      return generateSwitchClauseTemplate(locale).replaceAll(
        '@(case)',
        locale.toString(),
      );
    },
...
String _generateLookupByScriptCode(
  ...
  final Iterable<String> switchClauses =
      allBundles.languages.map((String language) {
        final Iterable<LocaleInfo> locales = allBundles.localesForLanguage(
          language,
        );
        final Iterable<LocaleInfo> localesWithScriptCodes = locales.where(
          (LocaleInfo locale) {
            return locale.scriptCode != null && locale.countryCode == null;
          },
        );
        ...
        return _addSpaces(nestedSwitchTemplate
            ...
            .replaceAll(
              '@(switchClauses)',
              _addSpaces(localesWithScriptCodes.map((LocaleInfo locale) {
                return generateSwitchClauseTemplate(locale).replaceAll(
                  '@(case)',
                  locale.scriptCode!,
                );
...
String _generateLookupByCountryCode(
  ...
  final Iterable<String> switchClauses =
      allBundles.languages.map((String language) {
        ...
        final Iterable<LocaleInfo> localesWithCountryCodes = locales.where(
          (LocaleInfo locale) {
        ...
        return _addSpaces(nestedSwitchTemplate
            ...
            .replaceAll(
              '@(switchClauses)',
              _addSpaces(localesWithCountryCodes.map((LocaleInfo locale) {
                return generateSwitchClauseTemplate(locale).replaceAll(
                  '@(case)',
                  locale.countryCode!,
                );
...
String _generateLookupByLanguageCode(
  ...
  final Iterable<String> switchClauses =
      allBundles.languages.map((String language) {
        ...
        final Iterable<LocaleInfo> localesWithLanguageCode = locales.where(
          (LocaleInfo locale) {
            return locale.countryCode == null && locale.scriptCode == null;
          },
        );
...
String _generateLookupBody(
  ...
  return lookupBodyTemplate.replaceAll(
...
String _generateDelegateClass({
  ...
  final String loadBody = (useDeferredLoading
...
class LocalizationsGenerator {
  ...
  factory LocalizationsGenerator({
    ...
    final Directory? projectDirectory = projectDirFromPath(
      fileSystem,
      projectPathString,
    );
  ...
  late final AppResourceBundle _templateBundle = AppResourceBundle(
    templateArbFile,
  );
  ...
  List<String> get arbPathStrings {
    return _allBundles.bundles.map(
      (AppResourceBundle bundle) => bundle.file.path,
    ).toList();
  }
  ...
  static String _getAbsoluteProjectPath(
    String relativePath,
    Directory projectDirectory,
  ) => projectDirectory.fileSystem.path.join(
  ...
  static File? _inputsAndOutputsListFileFromPath(
    ...
    return fileSystem.file(fileSystem.path.join(
      inputsAndOutputsListPath,
  ...
  void loadResources() {
    ...
    _allMessages = _templateBundle.resourceIds.map(
      (String id) => Message(
        _templateBundle,
        _allBundles,
        id,
    ...
    final List<LocaleInfo> allLocales = List<LocaleInfo>.from(
      _allBundles.locales,
    );
  ...
  String _generateCode() {
    ...
    final Iterable<String> supportedLocalesCode = supportedLocales.map(
      (LocaleInfo locale) {
    ...
    for (final LocaleInfo locale in allLocales) {
      if (isBaseClassLocale(locale, locale.languageCode)) {
        ...
        final List<LocaleInfo> localesForLanguage = getLocalesForLanguage(
          locale.languageCode,
        );

        // Generate every subclass that is needed for the particular language
        final Iterable<String> subclasses = localesForLanguage.map<String>(
          (LocaleInfo locale) {
    ...
    final List<String> sortedClassImports = supportedLocales
        .where(
          (LocaleInfo locale) => isBaseClassLocale(locale, locale.languageCode),
        )
    ...
    return fileTemplate
        .replaceAll('@(header)', header.isEmpty ? '' : '$header\n')
        .replaceAll('@(class)', className)
        .replaceAll('@(methods)', _allMessages
            .map(
              (Message message) => generateBaseClassMethod(
                message,
                _templateArbLocale,
              ),
            )
        ...
        .replaceAll(
          '@(requiresIntlImport)',
          requiresIntlImport ? "import 'package:intl/intl.dart' as intl;" : '',
        )
      ...
      if (message.placeholders.isEmpty) {
        ...
        return getterTemplate
            ...
            .replaceAll(
              '@(message)',
              "'${generateString(
                node.children.map((Node child) => child.value!).join(),
              )}'",
            );
      }
          ...
          case ST.pluralExpr:
            ...
            for (final Node pluralPart in pluralParts.children.reversed) {
              ...
              if (!pluralLogicArgs.containsKey(pluralCases[pluralCase])) {
                final String pluralPartExpression = generateVariables(
                  pluralMessage,
                );
...
[${_inputFileNames[
                  locale
                ]}:${message.resourceId}] ICU Syntax Warning: The plural part specified below is overridden by a later plural part.
    $translationForMessage
    ${Parser.indentForError(pluralPart.positionInMessage)}''');
          ...
          case ST.selectExpr:
            ...
            for (final Node selectPart in selectParts.children) {
              ...
              final String selectPartExpression = generateVariables(
                selectMessage,
              );
              selectLogicArgs.add(
                "        '$selectCase': $selectPartExpression,",
              );
            }
            final String tempVarName = getTempVariableName();
      ...
      final String tempVarLines = tempVariables.isEmpty
          ? ''
          : '${tempVariables.join('\n')}\n';
      return methodTemplate
          ...
          .replaceAll(
            '@(numberFormatting)',
            generateNumberFormattingLogic(message),
          )
  ...
  List<String> writeOutputFiles({bool isFromYaml = false}) {
    ...
    if (useSyntheticPackage) {
      ...
      final File flutterGenPubspec = syntheticPackageDirectory.childFile(
        'pubspec.yaml',
      );
    ...
    if (messagesFile != null) {
      ...
      _unimplementedMessages.forEach(
        (LocaleInfo locale, List<String> messages) {
          logger.printStatus(
    ...
    if (inputsAndOutputsListFileLocal != null) {
      ...
      inputsAndOutputsListFileLocal.writeAsStringSync(json.encode(
        <String, Object>{'inputs': _inputFileList, 'outputs': _outputFileList},
      ));
    }
