...
class Placeholder {
  ...
  bool get hasNumberFormatWithParameters => _numberFormatsWithNamedParameters
      .contains(format);
...
class Message {
  ...
  bool get placeholdersRequireFormatting => placeholders.values.any(
    (Placeholder p) => p.requiresFormatting,
  );
  ...
  void _inferPlaceholders(Map<LocaleInfo, String> filenames) {
    ...
    for (final LocaleInfo locale in parsedMessages.keys) {
      ...
      while (traversalStack.isNotEmpty) {
        ...
        if (<ST>[ST.placeholderExpr, ST.pluralExpr, ST.selectExpr].contains(
          node.type,
        )) {
    ...
    placeholders.addEntries(
      undeclaredPlaceholders.entries.toList()
        ..sort(
          (
            MapEntry<String, Placeholder> p1,
...
class AppResourceBundle {
  factory AppResourceBundle(File file) {
    ...
    final String fileName = file.fileSystem.path.basenameWithoutExtension(
      file.path,
    );
