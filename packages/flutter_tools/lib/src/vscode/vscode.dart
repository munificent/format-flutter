...
class VsCode {
  ...
  static List<VsCode> allInstalled(FileSystem fileSystem, Platform platform, ProcessManager processManager) {
    if (platform.isMacOS) {
  ...
  static List<VsCode> _installedMacOS(FileSystem fileSystem, Platform platform, ProcessManager processManager) {
    ...
    try {
      final ProcessResult vsCodeSpotlightQueryResult = processManager
          .runSync(<String>['mdfind', 'kMDItemCFBundleIdentifier="com.microsoft.VSCode"']);
      vsCodeSpotlightResult = vsCodeSpotlightQueryResult.stdout as String;
      final ProcessResult vsCodeInsidersSpotlightQueryResult = processManager
          .runSync(<String>['mdfind', 'kMDItemCFBundleIdentifier="com.microsoft.VSCodeInsiders"']);
    ...
    return _findInstalled(<VsCodeInstallLocation>{
      VsCodeInstallLocation(fileSystem.path.join('/Applications', 'Visual Studio Code.app', 'Contents'), '.vscode'),
      if (homeDirPath != null)
      ...
      if (homeDirPath != null)
        VsCodeInstallLocation(
          fileSystem.path.join(homeDirPath, 'Applications', 'Visual Studio Code - Insiders.app', 'Contents'),
          '.vscode-insiders',
        ),
      ...
      for (final String vsCodeInsidersPath in LineSplitter.split(vsCodeInsiderSpotlightResult))
        VsCodeInstallLocation(fileSystem.path.join(vsCodeInsidersPath, 'Contents'), '.vscode-insiders'),
    }, fileSystem, platform);
  }
  ...
  static List<VsCode> _installedWindows(FileSystem fileSystem, Platform platform) {
    ...
    final List<VsCodeInstallLocation> searchLocations = <VsCodeInstallLocation>[
      if (localAppData != null)
        VsCodeInstallLocation(fileSystem.path.join(localAppData, r'Programs\Microsoft VS Code'), '.vscode'),
  ...
  @override
  String toString() => 'VS Code ($version)${_extensionVersion != null ? ', Flutter ($_extensionVersion)' : ''}';
