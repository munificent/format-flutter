...
class MDnsVmServiceDiscovery {
  ...
  Future<MDnsVmServiceDiscoveryResult?> queryForAttach({
    ...
    if (results.isEmpty) {
      ...
      for (final MDnsVmServiceDiscoveryResult result in results) {
        buffer.writeln(
          '  flutter attach --app-id "${result.domainName.replaceAll(
            '.$dartVmServiceName',
            '',
          )}" --device-vmservice-port ${result.port}',
        );
      }
    ...
    try {
      ...
      await for (final PtrResourceRecord ptr in ptrResourceStream) {
        ...
        final List<SrvResourceRecord> srvRecords =
            await client.lookup<SrvResourceRecord>(ResourceRecordQuery.service(domainName)).toList();
        ...
        if (useDeviceIPAsHost) {
          List<IPAddressResourceRecord> ipAddresses = await client.lookup<IPAddressResourceRecord>(
            ipv6
        ...
        results.add(MDnsVmServiceDiscoveryResult(domainName, srvRecord.port, authCode, ipAddress: ipAddress));
  ...
  String _getAuthCode(String txtRecord) {
    ...
    final Iterable<String> matchingRecords = LineSplitter.split(txtRecord).where(
      (String record) => record.startsWith(authCodePrefix),
    );
  ...
  Future<Uri?> _handleResult(
    ...
    return buildVMServiceUri(device, host, result.port, hostVmservicePort, result.authCode, useDeviceIPAsHost);
  }
  ...
  Future<void> _checkForIPv4LinkLocal(Device device) async {
    ...
    final List<NetworkInterface> interfaces =
        await listNetworkInterfaces(includeLinkLocal: true, type: InternetAddressType.IPv4);
