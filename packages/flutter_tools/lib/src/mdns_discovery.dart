...
class MDnsVmServiceDiscovery {
  ...
  Future<List<MDnsVmServiceDiscoveryResult>> _pollingVmService(
    ...
    try {
      ...
      final Stream<PtrResourceRecord> ptrResourceStream = client
      ...
      await for (final PtrResourceRecord ptr in ptrResourceStream) {
        ...
        if (applicationId != null) {
          ...
          if (ptr.domainName.toLowerCase().startsWith(
        ...
        if (useDeviceIPAsHost) {
          ...
          if (ipAddresses.length > 1) {
            ipAddresses = ipAddresses.where(
              (IPAddressResourceRecord element) => !element.address.isLinkLocal,
            ).toList();
          }
    ...
    for (final NetworkInterface interface in interfaces) {
      if (_logger.isVerbose) {
        ...
        for (final InternetAddress address in interface.addresses) {
          ...
          _logger.printTrace(
            '\tBound address: "${address.address}" $linkLocal',
          );
        }
      }
    }
