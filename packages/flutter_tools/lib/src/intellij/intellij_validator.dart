...
abstract class IntelliJValidator extends DoctorValidator {
  ...
  Future<ValidationResult> validate() async {
    ...
    if (pluginsPath == null) {
      ...
      messages.add(
        ValidationMessage(_userMessages.intellijLocation(installPath)),
      );
      ...
      plugins.validatePackage(messages, <String>[
        'Dart',
      ], 'Dart', IntelliJPlugins.kIntellijDartPluginUrl);
    ...
    for (final Directory dir in homeDir.listSync().whereType<Directory>()) {
      ...
      IntelliJValidator._idToTitle.forEach((String id, String title) {
        if (name.startsWith('.$id')) {
          ...
          String? installPath;
          try {
            installPath = fileSystem.file(
              fileSystem.path.join(dir.path, 'system', '.home'),
            ).readAsStringSync();
          } on FileSystemException {
            // ignored
          }
          if (installPath != null && fileSystem.isDirectorySync(installPath)) {
            final String pluginsPath = fileSystem.path.join(
              dir.path,
              'config',
              'plugins',
            );
    ...
    for (final Directory dir in cacheDir.listSync().whereType<Directory>()) {
      ...
      IntelliJValidator._idToTitle.forEach((String id, String title) {
        if (name.startsWith(id)) {
          ...
          try {
            installPath = fileSystem.file(
              fileSystem.path.join(dir.path, '.home'),
            ).readAsStringSync();
          } on FileSystemException {
            // ignored
          }
  ...
  static Iterable<DoctorValidator> installed({
    ...
    try {
      ...
      try {
        ...
        final ProcessResult ultimateQueryResult = processManager.runSync(<
          String
