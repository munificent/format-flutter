...
abstract class IntelliJValidator extends DoctorValidator {
  IntelliJValidator(super.title, this.installPath, {required FileSystem fileSystem, required UserMessages userMessages})
  ...
  Future<ValidationResult> validate() async {
    ...
    if (pluginsPath == null) {
      ...
      plugins.validatePackage(messages, <String>[
        'flutter-intellij',
        'flutter-intellij.jar',
      ], 'Flutter', IntelliJPlugins.kIntellijFlutterPluginUrl, minVersion: IntelliJPlugins.kMinFlutterPluginVersion);
      plugins.validatePackage(messages, <String>['Dart'], 'Dart', IntelliJPlugins.kIntellijDartPluginUrl);
...
class IntelliJValidatorOnMac extends IntelliJValidator {
  ...
  static Iterable<DoctorValidator> installed({
    ...
    try {
      ...
      try {
        final ProcessResult ceQueryResult = processManager
            .runSync(<String>['mdfind', 'kMDItemCFBundleIdentifier="com.jetbrains.intellij.ce"']);
        ceSpotlightResult = ceQueryResult.stdout as String;
        final ProcessResult ultimateQueryResult = processManager
            .runSync(<String>['mdfind', 'kMDItemCFBundleIdentifier="com.jetbrains.intellij*"']);
      ...
      validators.add(ValidatorWithResult(
        ...
        ValidationResult(ValidationType.missing, <ValidationMessage>[ValidationMessage.error(e.message)]),
      ));
    }
    return validators;
  }
  ...
  String get version {
    return _version ??=
        _plistParser.getValueFromFile<String>(plistFile, PlistParser.kCFBundleShortVersionStringKey) ?? 'unknown';
  }
  ...
  String? get pluginsPath {
    ...
    final String? altLocation = _plistParser.getValueFromFile<String>(plistFile, 'JetBrainsToolboxApp');
    ...
    if (homeDirPath != null) {
      ...
      if (!_fileSystem.isDirectorySync(pluginsPath)) {
        pluginsPath = _fileSystem.path.join(homeDirPath, 'Library', 'Application Support', '$id$major.$minor');
      }
