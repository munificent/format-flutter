...
class DevelopmentArtifact {
  ...
  static const DevelopmentArtifact iOS = DevelopmentArtifact._(
    'ios',
    feature: flutterIOSFeature,
  );

  /// Artifacts required for web development.
  static const DevelopmentArtifact web = DevelopmentArtifact._(
    'web',
    feature: flutterWebFeature,
  );
  ...
  static const DevelopmentArtifact universal = DevelopmentArtifact._(
    'universal',
  );
...
class Cache {
  ...
  ArtifactUpdater _createUpdater() {
    return ArtifactUpdater(
      ...
      allowedBaseUrls: <String>[storageBaseUrl, cipdBaseUrl],
    );
  }
  ...
  String get devToolsVersion {
    if (_devToolsVersion == null) {
      ...
      final Directory devToolsDir = getCacheDir(
        devToolsDirPath,
        shouldCreate: false,
      );
  ...
  String get dartSdkVersion {
    if (_dartSdkVersion == null) {
      ...
      _dartSdkVersion = justVersion.replaceFirstMapped(
        RegExp(r'(\d+\.\d+\.\d+)(.+)'),
        (Match match) {
  ...
  String get dartSdkBuild {
    if (_dartSdkBuild == null) {
      ...
      // Changes '2.1.0-dev.8.0.flutter-4312ae32' to '2.1.0 (build 2.1.0-dev.8.0 4312ae32)'
      final String justVersion = _platform.version.split(' ')[0];
      _dartSdkBuild = justVersion.replaceFirstMapped(
        RegExp(r'(\d+\.\d+\.\d+)(.+)'),
        (Match match) {
          final String noFlutter = match[2]!.replaceAll('.flutter-', ' ');
  ...
  Directory getRoot() {
    if (_rootOverride != null) {
      ...
      return _fileSystem.directory(
        _fileSystem.path.join(flutterRoot!, 'bin', 'cache'),
      );
    }
  }
  ...
  Directory getCacheDir(String name, {bool shouldCreate = true}) {
    final Directory dir = _fileSystem.directory(
      _fileSystem.path.join(getRoot().path, name),
    );
  ...
  File getLicenseFile() => _fileSystem.file(
    _fileSystem.path.join(flutterRoot!, 'LICENSE'),
  );
  ...
  MapEntry<String, String> get dyLdLibEntry {
    ...
    _dyLdLibEntry = MapEntry<String, String>(
      'DYLD_LIBRARY_PATH',
      paths.join(':'),
    );
    return _dyLdLibEntry!;
  ...
  File getStampFileFor(String artifactName) {
    return _fileSystem.file(
      _fileSystem.path.join(getRoot().path, '$artifactName.stamp'),
    );
  }
  ...
  Future<void> updateAll(
    ...
    for (final ArtifactSet artifact in _artifacts) {
      if (!requiredArtifacts.contains(artifact.developmentArtifact)) {
        _logger.printTrace(
          'Artifact $artifact is not required, skipping update.',
        );
        continue;
      }
      if (await artifact.isUpToDate(_fileSystem)) {
  ...
  bool get ignorePlatformFiltering {
    return cache.includeAllPlatforms ||
        (cache.platformOverrideArtifacts != null &&
            cache.platformOverrideArtifacts!.contains(
              developmentArtifact.name,
            ));
  }
...
abstract class EngineCachedArtifact extends CachedArtifact {
  ...
  bool isUpToDateInner(FileSystem fileSystem) {
    ...
    for (final List<String> toolsDir in getBinaryDirs()) {
      final Directory dir = fileSystem.directory(
        fileSystem.path.join(location.path, toolsDir[0]),
      );
      if (!dir.existsSync()) {
        return false;
      }
  ...
  Future<void> updateInner(
    ...
    for (final List<String> toolsDir in getBinaryDirs()) {
      ...
      final Directory dir = fileSystem.directory(
        fileSystem.path.join(location.path, cacheDir),
      );

      // Avoid printing things like 'Downloading linux-x64 tools...' multiple times.
      final String friendlyName = urlPath
          .replaceAll('/artifacts.zip', '')
          .replaceAll('.zip', '');
      await artifactUpdater.downloadZipArchive(
      ...
      final File frameworkZip = fileSystem.file(
        fileSystem.path.join(dir.path, 'FlutterMacOS.framework.zip'),
      );
...
class ArtifactUpdater {
  ...
  File _createDownloadFile(String name) {
    final File tempFile = _fileSystem.file(
      _fileSystem.path.join(_tempStorage.path, name),
    );
...
String flattenNameSubdirs(Uri url, FileSystem fileSystem) {
  ...
  final Iterable<String> convertedPieces = pieces.map<String>(
