...
class LinuxDoctorValidator extends DoctorValidator {
  ...
  Future<ValidationResult> validate() async {
    ...
    if (installedVersions.values.any(
      (_VersionInfo? versionInfo) => versionInfo?.number == null,
    )) {
    ...
    // Messages for libraries.
    {
      ...
      if (libraryMissing) {
        validationType = ValidationType.missing;
        messages.add(
          ValidationMessage.error(_userMessages.gtkLibrariesMissing),
        );
      }
    }
  ...
  Future<bool> _libraryIsPresent(String library) async {
    ProcessResult? result;
    try {
      result = await _processManager
          .run(<String>['pkg-config', '--exists', library]);
    } on ArgumentError {
      // ignore error.
    }
