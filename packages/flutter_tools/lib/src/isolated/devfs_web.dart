...
class WebExpressionCompiler implements ExpressionCompiler {
  ...
  Future<ExpressionCompilationResult> compileExpressionToJs(
    ...
    final CompilerOutput? compilerOutput = await _generator
...
class WebAssetServer implements AssetReader {
  ...
  static Future<WebAssetServer> start(
    ...
    if (buildInfo.mode != BuildMode.debug) {
      ...
      runZonedGuarded(
        ...
        (Object e, StackTrace s) {
          globals.printTrace(
            'Release asset server: error serving requests: $e:$s',
          );
        },
      );
      return server;
    ...
    final shelf.Cascade cascade = shelf.Cascade().add(dwds.handler).add(
      dwdsHandler,
    );
    runZonedGuarded(
      () {
  ...
  Future<shelf.Response> handleRequest(shelf.Request request) async {
    ...
    final String webServerPath = requestPath.replaceFirst(
      '.dart.js',
      '.dart.lib.js',
    );
    ...
    if (!file.existsSync() && requestPath.startsWith('canvaskit/')) {
      final Directory canvasKitDirectory = globals.fs.directory(globals.fs.path
          .join(
  ...
  File _resolveDartFile(String path) {
    ...
    final File dartFile = globals.fs.file(
      globals.fs.currentDirectory.uri.resolve(path),
    );
    ...
    final Directory dartSdkParent = globals.fs.directory(globals.artifacts!
        .getArtifactPath(
...
class WebDevFS implements DevFS {
  ...
  Future<UpdateFSReport> update({
    ...
    if (bundleFirstUpload) {
      webAssetServer.entrypointCacheDirectory = globals.fs.directory(
        outputDirectoryPath,
      );
      ...
      webAssetServer.writeBytes(
        ...
        stackTraceMapper.readAsBytesSync(),
      );
      webAssetServer.writeFile(
        'manifest.json',
        '{"info":"manifest not generated in run mode."}',
      );
      final String fileGeneratorsPath = globals.artifacts!.getArtifactPath(
      ...
      webAssetServer.writeFile(
        'version.json',
        FlutterProject.current().getVersionInfo(),
      );
      webAssetServer.writeFile('main.dart.js', generateBootstrapScript(
    ...
    try {
      final Directory parentDirectory = globals.fs.directory(
        outputDirectoryPath,
      );
      codeFile = parentDirectory.childFile(
        '${compilerOutput.outputFilename}.sources',
      );
      manifestFile = parentDirectory.childFile(
        '${compilerOutput.outputFilename}.json',
      );
      sourcemapFile = parentDirectory.childFile(
        '${compilerOutput.outputFilename}.map',
      );
      metadataFile = parentDirectory.childFile(
  ...
  @visibleForTesting
  final File requireJS = globals.fs.file(globals.fs.path.join(globals.artifacts!
      .getArtifactPath(
        Artifact.engineDartSdkPath,
        platform: TargetPlatform.web_javascript,
...
class ReleaseAssetServer {
  ...
  Future<shelf.Response> handle(shelf.Request request) async {
    ...
    final File file = _fileSystem.file(
      _fileSystem.path.join(_webBuildDirectory!, 'index.html'),
    );
    return shelf.Response.ok(
...
void log(logging.LogRecord event) {
  ...
  if (event.level >= logging.Level.SEVERE) {
    globals.printError(
      '${event.loggerName}: ${event.message}$error',
...
IndexHtml _getIndexHtml() {
  final File indexHtml = globals.fs.currentDirectory
      .childDirectory('web')
