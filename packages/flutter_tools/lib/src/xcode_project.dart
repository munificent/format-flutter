...
abstract class XcodeBasedProject extends FlutterProjectPlatform {
  ...
  late final String hostAppProjectName = () {
    ...
    final Directory? xcodeProjectDirectory = _xcodeDirectoryWithExtension(
      '.xcodeproj',
    );
    return xcodeProjectDirectory != null
        ? xcodeProjectDirectory.fileSystem.path.basenameWithoutExtension(
            xcodeProjectDirectory.path,
          )
        : _defaultHostAppName;
  }();
  ...
  File get defaultHostInfoPlist => hostAppRoot
      .childDirectory(_defaultHostAppName)
  ...
  File get podManifestLock => hostAppRoot.childDirectory('Pods').childFile(
    'Manifest.lock',
  );
...
class IosProject extends XcodeBasedProject {
  ...
  Directory get ephemeralDirectory => _flutterLibRoot
      .childDirectory('Flutter')
      .childDirectory('ephemeral');

  @override
  File xcodeConfigFor(String mode) => _flutterLibRoot
      .childDirectory('Flutter')
      .childFile('$mode.xcconfig');
  ...
  File get infoPlist => _editableDirectory.childDirectory('Runner').childFile(
    'Info.plist',
  );
        ...
        if (entitlement.existsSync()) {
          ...
          if (domains != null) {
            ...
            for (final String domain in domains) {
              final RegExpMatch? match = _associatedDomainPattern.firstMatch(
                domain,
              );
              if (match != null) {
  ...
  Future<Map<String, String>?> buildSettingsForBuildInfo(
    ...
    return _buildSettingsForXcodeProjectBuildContext(XcodeProjectBuildContext(
  ...
  Future<Map<String, String>?> _xcodeProjectBuildSettings(
    ...
    final Map<String, String> buildSettings = await xcodeProjectInterpreter
        .getBuildSettings(xcodeProject.path, buildContext: buildContext);
  ...
  Future<bool> containsWatchCompanion({
    ...
    for (final String target in projectInfo.targets) {
      ...
      final File infoFile = hostAppRoot.childDirectory(target).childFile(
        'Info.plist',
      );
  ...
  Future<void> _regenerateFromTemplateIfNeeded() async {
    ...
    final bool toolingChanged = globals.cache.isOlderThanToolsStamp(
  ...
  Directory get pluginRegistrantHost {
    return isModule
        ? _flutterLibRoot.childDirectory('Flutter').childDirectory(
...
class MacOSProject extends XcodeBasedProject {
  ...
  Directory get ephemeralDirectory => managedDirectory.childDirectory(
    'ephemeral',
  );

  /// The xcfilelist used to track the inputs for the Flutter script phase in
  /// the Xcode build.
  File get inputFileList => ephemeralDirectory.childFile(
    'FlutterInputs.xcfilelist',
  );

  /// The xcfilelist used to track the outputs for the Flutter script phase in
  /// the Xcode build.
  File get outputFileList => ephemeralDirectory.childFile(
    'FlutterOutputs.xcfilelist',
  );

  @override
  File get generatedXcodePropertiesFile => ephemeralDirectory.childFile(
    'Flutter-Generated.xcconfig',
  );

  File get pluginRegistrantImplementation => managedDirectory.childFile(
    'GeneratedPluginRegistrant.swift',
  );

  @override
  File xcodeConfigFor(String mode) => managedDirectory.childFile(
    'Flutter-$mode.xcconfig',
  );

  @override
  File get generatedEnvironmentVariableExportScript => ephemeralDirectory
      .childFile('flutter_export_environment.sh');
