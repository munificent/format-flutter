...
Future<T> runInContext<T>(FutureOr<T> Function() runner, {Map<Type, Generator>? overrides}) async {
  ...
  return context.run<T>(
    ...
    fallbacks: <Type, Generator>{
      ...
      AndroidWorkflow: () => AndroidWorkflow(androidSdk: globals.androidSdk, featureFlags: featureFlags),
      ...
      BuildSystem: () => FlutterBuildSystem(fileSystem: globals.fs, logger: globals.logger, platform: globals.platform),
      Cache: () => FlutterCache(
            fileSystem: globals.fs,
            logger: globals.logger,
            platform: globals.platform,
      ...
      CustomDevicesConfig:
          () => CustomDevicesConfig(fileSystem: globals.fs, logger: globals.logger, platform: globals.platform),
      ...
      IOSWorkflow: () => IOSWorkflow(featureFlags: featureFlags, xcode: globals.xcode!, platform: globals.platform),
      Java: () => Java.find(
            config: globals.config,
            androidStudio: globals.androidStudio,
            logger: globals.logger,
      ...
      MDnsVmServiceDiscovery: () => MDnsVmServiceDiscovery(logger: globals.logger, flutterUsage: globals.flutterUsage),
      ...
      PersistentToolState:
          () => PersistentToolState(fileSystem: globals.fs, logger: globals.logger, platform: globals.platform),
      ProcessInfo: () => ProcessInfo(globals.fs),
      ProcessManager:
          () => ErrorHandlingProcessManager(delegate: const LocalProcessManager(), platform: globals.platform),
      ProcessUtils: () => ProcessUtils(processManager: globals.processManager, logger: globals.logger),
      Pub: () => Pub(
            fileSystem: globals.fs,
            logger: globals.logger,
            processManager: globals.processManager,
