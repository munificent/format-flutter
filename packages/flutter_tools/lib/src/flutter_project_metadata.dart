...
enum FlutterProjectType implements CliEnum {
  ...
  String get helpText => switch (this) {
        ...
        FlutterProjectType.package => 'Generate a shareable Flutter project containing modular Dart code.',
        FlutterProjectType.plugin => 'Generate a shareable Flutter project containing an API '
        ...
        FlutterProjectType.pluginFfi => 'Generate a shareable Flutter project containing an API '
...
class FlutterProjectMetadata {
  ...
  FlutterProjectMetadata(this.file, Logger logger) : _logger = logger, migrateConfig = MigrateConfig() {
    if (!file.existsSync()) {
    ...
    if (_validateMetadataMap(yamlRoot, <String, Type>{'version': YamlMap}, _logger)) {
      ...
      if (versionYamlMap is YamlMap &&
          _validateMetadataMap(versionYamlMap, <String, Type>{'revision': String, 'channel': String}, _logger)) {
...
class MigrateConfig {
  ...
  static const List<String> kDefaultUnmanagedFiles = <String>['lib/main.dart', 'ios/Runner.xcodeproj/project.pbxproj'];
  ...
  String getOutputFileString() {
    ...
    for (final MapEntry<SupportedPlatform, MigratePlatformConfig> entry in platformConfigs.entries) {
      platformsString += '\n    - platform: ${entry.key.toString().split(
        '.',
...
class MigratePlatformConfig {
  ...
  bool equals(MigratePlatformConfig other) {
    return platform == other.platform && createRevision == other.createRevision && baseRevision == other.baseRevision;
  }
}
