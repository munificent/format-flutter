...
class SourceVisitor implements ResolvedFiles {
  ...
  Iterable<File> _processList(String rawText) {
    return rawText
        ...
        .map<String>(
          (String path) => path.replaceAllMapped(
            _escapeExpr,
            (Match match) => match.group(1)!,
          ).trim(),
        )
  ...
  void visitPattern(String pattern, bool optional) {
    ...
    for (final FileSystemEntity entity
      ...
      if (wildcardSegments.isEmpty) {
        ...
        if (filename.substring(wildcardSegments[0].length).endsWith(
          wildcardSegments[1],
        )) {
          sources.add(environment.fileSystem.file(entity.absolute));
        }
  ...
  void visitHostArtifact(HostArtifact artifact) {
    ...
    final FileSystemEntity entity = environment.artifacts.getHostArtifact(
      artifact,
    );
...
class _ArtifactSource implements Source {
  ...
  @override
  void accept(SourceVisitor visitor) => visitor.visitArtifact(
    artifact,
    platform,
    mode,
  );
