...
abstract class Target {
  ...
  T fold<T>(
    T initialValue,
    T Function(T previousValue, Target target) combine,
  ) {
...
class FlutterBuildSystem extends BuildSystem {
  ...
  Future<BuildResult> build(
    ...
    final FileStore fileCache = FileStore(cacheFile: cacheFile, logger: _logger)
      ..initialize();
  ...
  Future<BuildResult> buildIncremental(
    ...
    if (previousBuild == null || _incrementalFileStore[previousBuild] == null) {
      final File cacheFile = environment.buildDir.childFile(
        FileStore.kFileCache,
      );
      fileCache = FileStore(
        cacheFile: cacheFile,
        logger: _logger,
  ...
  void trackSharedBuildDirectory(
    ...
    final String currentBuildId = fileSystem.path.basename(
      environment.buildDir.path,
    );
    final File lastBuildIdFile = environment.outputDir.childFile(
      '.last_build_id',
    );
...
class _BuildInstance {
  _BuildInstance({
    ...
    Platform? platform,
         ...
         buildSystemConfig.resourcePoolSize ??
  ...
  Future<bool> invokeTarget(Node node) async {
    ...
    final AsyncMemoizer<bool> memoizer = pending[node.target.name] ??=
        AsyncMemoizer<bool>();
...
class Node {
  ...
  bool computeChanges(
    ...
    for (final File file in inputs) {
      ...
      if (fileStore.currentAssetKeys.containsKey(absolutePath)) {
        ...
        if (currentHash != previousAssetKey) {
          final InvalidatedReason reason = _invalidate(
    ...
    if (missingInputs.isNotEmpty) {
      _dirty = true;
      final String missingMessage = missingInputs
