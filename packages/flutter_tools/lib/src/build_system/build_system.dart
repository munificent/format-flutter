...
abstract class Target {
  ...
  Node _toNode(Environment environment) {
    ...
    return Node(this, inputsFiles.sources, outputFiles.sources, <Node>[
      for (final Target target in dependencies) target._toNode(environment),
    ], environment, inputsFiles.containsNewDepfile);
  }
  ...
  void _writeStamp(List<File> inputs, List<File> outputs, Environment environment) {
    ...
    final Map<String, Object> result = <String, Object>{'inputs': inputPaths, 'outputs': outputPaths};
    if (!stamp.existsSync()) {
      stamp.createSync();
    }
...
abstract class BuildSystem {
  ...
  /// initialized.
  Future<BuildResult> buildIncremental(Target target, Environment environment, BuildResult? previousBuild);
}

class FlutterBuildSystem extends BuildSystem {
  const FlutterBuildSystem({required FileSystem fileSystem, required Platform platform, required Logger logger})
  ...
  Future<BuildResult> buildIncremental(Target target, Environment environment, BuildResult? previousBuild) async {
    ...
    if (previousBuild == null || _incrementalFileStore[previousBuild] == null) {
      final File cacheFile = environment.buildDir.childFile(FileStore.kFileCache);
      fileCache = FileStore(cacheFile: cacheFile, logger: _logger, strategy: FileStoreStrategy.timestamp)..initialize();
    } else {
  ...
  @visibleForTesting
  void trackSharedBuildDirectory(Environment environment, FileSystem fileSystem, Map<String, File> currentOutputs) {
...
class Node {
  Node(this.target, this.inputs, this.outputs, this.dependencies, Environment environment, this.missingDepfile) {
  ...
  bool computeChanges(Environment environment, FileStore fileStore, FileSystem fileSystem, Logger logger) {
