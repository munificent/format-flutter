...
abstract class AotAssemblyBase extends Target {
  ...
  Future<void> build(Environment environment) async {
    ...
    for (final DarwinArch darwinArch in darwinArchs) {
      final List<String> archExtraGenSnapshotOptions = List<String>.of(
      ...
      if (codeSizeDirectory != null) {
        ...
        archExtraGenSnapshotOptions.add(
...
abstract class UnpackIOS extends Target {
  ...
  void _thinFramework(
    ...
    final ProcessResult infoResult = environment.processManager
        .runSync(<String>['lipo', '-info', frameworkBinaryPath]);
    ...
    if (lipoInfo.startsWith('Non-fat file:')) {
      environment.logger.printTrace(
        'Skipping lipo for non-fat file $frameworkBinaryPath',
      );
      return;
    }

    // Thin in-place.
    final ProcessResult extractResult = environment.processManager
        .runSync(<String>[
          'lipo',
          '-output',
          frameworkBinaryPath,
  ...
  void _bitcodeStripFramework(
    ...
    final ProcessResult stripResult = environment.processManager
        .runSync(<String>[
          'xcrun',
          'bitcode_strip',
          frameworkBinaryPath,
...
abstract class IosAssetBundle extends Target {
  ...
  Future<void> build(Environment environment) async {
    ...
    final Directory assetDirectory = frameworkDirectory.childDirectory(
      'flutter_assets',
    );
    ...
    if (buildMode == BuildMode.debug) {
      ...
      environment.buildDir.childFile('app.dill').copySync(
        assetDirectory.childFile('kernel_blob.bin').path,
      );
    ...
    // Copy the dSYM
    if (environment.buildDir.childDirectory(
      'App.framework.dSYM',
    ).existsSync()) {
    ...
    flutterProject.ios.appFrameworkInfoPlist.copySync(
      environment.outputDir.childDirectory('App.framework').childFile(
        'Info.plist',
      ).path,
    );
...
Future<void> _signFramework(
  ...
  await removeFinderExtendedAttributes(binary, ProcessUtils(
    processManager: environment.processManager,
    logger: environment.logger,
  ), environment.logger);
