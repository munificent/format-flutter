...
Future<Depfile> copyAssets(
  ...
  await Future.wait<void>(assetEntries.entries.map<Future<void>>(
    (MapEntry<String, DevFSContent> entry) async {
  ...
  if (environment.defines[kDeferredComponents] == 'true' && buildMode != null) {
    ...
    >((MapEntry<String, Map<String, DevFSContent>> componentEntries) async {
      ...
      await Future.wait<void>(componentEntries.value.entries.map<Future<void>>(
        (MapEntry<String, DevFSContent> entry) async {
          final PoolResource resource = await pool.request();
          try {
            // This will result in strange looking files, for example files with `/`
  ...
  final Depfile depfile = Depfile(
    inputs + assetBundle.additionalDependencies,
    outputs,
  );
...
DevFSContent? processSkSLBundle(
  ...
  return DevFSStringContent(json
      .encode(<String, Object?>{'data': bundle['data']}));
}
