  ...
  Future<void> build(Environment environment) async {
    ...
    if (buildMode == BuildMode.debug) {
      ...
      environment.buildDir.childFile('app.dill').copySync(
        outputDirectory.childFile('kernel_blob.bin').path,
      );
      environment.fileSystem.file(vmSnapshotData).copySync(
        outputDirectory.childFile('vm_snapshot_data').path,
      );
      environment.fileSystem.file(isolateSnapshotData).copySync(
        outputDirectory.childFile('isolate_snapshot_data').path,
      );
    }
...
class AndroidAot extends AotElfBase {
  ...
  Future<void> build(Environment environment) async {
    ...
    final Directory output = environment.buildDir.childDirectory(
      _androidAbiName,
    );
    final String? buildModeEnvironment = environment.defines[kBuildMode];
    if (buildModeEnvironment == null) {
    ...
    if (codeSizeDirectory != null) {
      ...
      extraGenSnapshotOptions.add(
        '--write-v8-snapshot-profile-to=${codeSizeFile.path}',
      );
      extraGenSnapshotOptions.add(
        '--trace-precompiler-to=${precompilerTraceFile.path}',
      );
    }
...
const AndroidAot androidArmProfile = AndroidAot(
  TargetPlatform.android_arm,
  BuildMode.profile,
);
const AndroidAot androidArm64Profile = AndroidAot(
  TargetPlatform.android_arm64,
  BuildMode.profile,
);
const AndroidAot androidx64Profile = AndroidAot(
  TargetPlatform.android_x64,
  BuildMode.profile,
);
const AndroidAot androidArmRelease = AndroidAot(
  TargetPlatform.android_arm,
  BuildMode.release,
);
const AndroidAot androidArm64Release = AndroidAot(
  TargetPlatform.android_arm64,
  BuildMode.release,
);
const AndroidAot androidx64Release = AndroidAot(
  TargetPlatform.android_x64,
  BuildMode.release,
);
...
class AndroidAotBundle extends Target {
  ...
  @override
  Future<void> build(Environment environment) async {
    final Directory buildDir = environment.buildDir.childDirectory(
      _androidAbiName,
    );
    final Directory outputDirectory = environment.outputDir.childDirectory(
      _androidAbiName,
...
const AndroidAotBundle androidArmProfileBundle = AndroidAotBundle(
  androidArmProfile,
);
const AndroidAotBundle androidArm64ProfileBundle = AndroidAotBundle(
  androidArm64Profile,
);
const AndroidAotBundle androidx64ProfileBundle = AndroidAotBundle(
  androidx64Profile,
);
const AndroidAotBundle androidArmReleaseBundle = AndroidAotBundle(
  androidArmRelease,
);
const AndroidAotBundle androidArm64ReleaseBundle = AndroidAotBundle(
  androidArm64Release,
);
const AndroidAotBundle androidx64ReleaseBundle = AndroidAotBundle(
  androidx64Release,
);
  ...
  for (final String abi in abis) {
    for (final DeferredComponent component in components) {
      ...
      for (final LoadingUnit unit in loadingUnits) {
        ...
        final List<String>? splitPath = unit.path?.split(
