  ...
  FlutterProject fromDirectory(Directory directory) {
    return projects.putIfAbsent(directory.path, () {
      ...
      final FlutterManifest exampleManifest = FlutterProject._readManifest(
        FlutterProject._exampleDirectory(directory).childFile(
          bundle.defaultManifestPath,
        ).path,
        logger: _logger,
        fileSystem: _fileSystem,
      );
...
class FlutterProject {
  ...
  /// Returns a [FlutterProject] view of the given directory or a ToolExit error,
  /// if `pubspec.yaml` or `example/pubspec.yaml` is invalid.
  static FlutterProject fromDirectory(
    Directory directory,
  ) => globals.projectFactory.fromDirectory(directory);

  /// Returns a [FlutterProject] view of the current directory or a ToolExit error,
  /// if `pubspec.yaml` or `example/pubspec.yaml` is invalid.
  static FlutterProject current() => globals.projectFactory.fromDirectory(
  ...
  /// version information.
  File get packageConfigFile => directory
  ...
  File get flutterPluginsDependenciesFile => directory.childFile(
  ...
  static Directory _exampleDirectory(Directory directory) => directory
...
class AndroidProject extends FlutterProjectPlatform {
  ...
  static final RegExp _declarativeKotlinPluginPattern = RegExp(
    '^\\s*id\\s+[\'"]kotlin-android[\'"]\\s*\$',
  );
  ...
  Directory get _editableHostAppDirectory => parent.directory.childDirectory(
    'android',
  );
  ...
  Future<AndroidAppLinkSettings> getAppLinksSettings({
    ...
    return AndroidAppLinkSettings(
      applicationId: await androidBuilder!.getApplicationIdForVariant(
        variant,
        project: parent,
      ),
      domains: await androidBuilder!.getAppLinkDomainsForVariant(
        variant,
        project: parent,
      ),
    );
  ...
  bool _computeSupportedVersion() {
    ...
    final File plugin = hostAppGradleRoot.childFile(fileSystem.path.join(
      'buildSrc',
      'src',
      'main',
      'groovy',
    ...
    for (final String line in appGradle.readAsLinesSync()) {
      final bool fileBasedApply = line.contains(
        RegExp(r'apply from: .*/flutter.gradle'),
      );
      final bool declarativeApply = line.contains(
        'dev.flutter.flutter-gradle-plugin',
      );
      final bool managed = line.contains(
  ...
  bool get isKotlin {
    final File gradleFile = hostAppGradleRoot.childDirectory('app').childFile(
      'build.gradle',
    );
    final bool imperativeMatch =
        firstMatchInFile(gradleFile, _imperativeKotlinPluginPattern) != null;
    final bool declarativeMatch =
        firstMatchInFile(gradleFile, _declarativeKotlinPluginPattern) != null;
  ...
  File get gradleAppOutV1File => gradleAppOutV1Directory.childFile(
    'app-debug.apk',
  );
...
''';
    ...
    return CompatibilityResult(
      compatibleJavaGradle && compatibleGradleAgp,
      description,
    );
  }
  ...
  Directory get pluginRegistrantHost => _flutterLibGradleRoot.childDirectory(
    isModule ? 'Flutter' : 'app',
  );
  ...
  AndroidEmbeddingVersionResult computeEmbeddingVersion() {
    ...
    for (final XmlElement metaData in document.findAllElements('meta-data')) {
      ...
      if (name == 'flutterEmbedding') {
        final String? embeddingVersionString = metaData.getAttribute(
          'android:value',
        );
...
class WebProject extends FlutterProjectPlatform {
  ...
  File get indexFile => parent.directory.childDirectory('web').childFile(
    'index.html',
  );
