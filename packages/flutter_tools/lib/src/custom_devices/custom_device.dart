...
class CustomDevicePortForwarder extends DevicePortForwarder {
  ...
  Future<ForwardedPort?> tryForward(int devicePort, int hostPort) async {
    ...
    logLinesSubscription = reader.logLines.listen((String line) {
  ...
  Future<int> forward(int devicePort, {int? hostPort}) async {
    int actualHostPort = (hostPort == 0 || hostPort == null)
        ? devicePort
        : hostPort;
    int tries = 0;
    ...
    while ((numTries == null) || (tries < numTries!)) {
      ...
      // choose another one
      while (_forwardedPorts.any(
        (ForwardedPort port) => port.hostPort == actualHostPort,
      )) {
        actualHostPort += 1;
      }
  ...
  @override
  List<ForwardedPort> get forwardedPorts => List<ForwardedPort>.unmodifiable(
    _forwardedPorts,
  );
...
class CustomDeviceAppSession {
  ...
  Future<LaunchResult> start({
    ...
    final ProtocolDiscovery discovery = ProtocolDiscovery.vmService(
      logReader,
      portForwarder: _device._config.usesPortForwarding
          ? _device.portForwarder
          : null,
      logger: _logger,
      ipv6: ipv6,
    );
...
class CustomDevice extends Device {
  ...
  Future<bool> installApp(
    ...
    final bool result =
        await tryInstall(localPath: getAssetBuildDirectory(), appName: appName);
