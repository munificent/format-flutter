...
class CustomDeviceLogReader extends DeviceLogReader {
  ...
  void listenToProcessOutput(Process process, {Encoding encoding = systemEncoding}) {
    ...
    subscriptions.add(
      process.stderr.transform<String>(decoder).transform<String>(const LineSplitter()).listen(logLinesController.add),
    );
  }
  ...
  List<String> _getEngineOptions(DebuggingOptions debuggingOptions, bool traceStartup, String? route) {
    ...
    return <String>[
      ...
      if (debuggingOptions.debuggingEnabled) ...<String>[
        ...
        if (debuggingOptions.buildInfo.isDebug) ...<String>['enable-checked-mode=true', 'verify-entry-points=true'],
...
class CustomDevice extends Device {
  CustomDevice({required CustomDeviceConfig config, required Logger logger, required ProcessManager processManager})
    : _config = config,
      _logger = logger,
      _processManager = processManager,
      _processUtils = ProcessUtils(processManager: processManager, logger: logger),
  ...
  Future<bool> _tryPostBuild({
    ...
    final List<String> interpolated = interpolateCommand(_config.postBuildCommand!, <String, String>{
      'appName': appName,
      'localPath': localPath,
...
class CustomDevices extends PollingDeviceDiscovery {
  ...
  List<CustomDevice> get _enabledCustomDevices {
    return _customDevicesConfig.tryGetDevices().where((CustomDeviceConfig element) => element.enabled).map(
      (CustomDeviceConfig config) => CustomDevice(config: config, logger: _logger, processManager: _processManager),
    ).toList();
  }
