  ...
  final StreamSubscription<Uint8List> subscription = source.listen(
    (Uint8List chunk) {
      ...
      while (start < chunk.length) {
        final int sizeToTake = min(
          chunkSize - bytesBuilder.length,
          chunk.length - start,
        );
        assert(sizeToTake > 0);
...
class FileTransfer {
  ...
  Future<BlockHashes> calculateBlockHashesOfFile(
    File file, {
    int? blockSize,
  }) async {
    ...
    final Stream<Uint8List> fileContentStream = file.openRead().map(
      (List<int> chunk) => Uint8List.fromList(chunk),
    );
  ...
  Future<List<FileDeltaBlock>> computeDelta(
    ...
    await for (final List<int> chunk in fileContentStream) {
      for (final int c in chunk) {
        ...
        final String md5Hash = base64.encode(
          md5.convert(adler32.currentBlock()).bytes,
        );
  ...
  Future<Uint8List> binaryForRebuilding(
    ...
    final int totalSize = toTransfer.map((FileDeltaBlock i) => i.size).reduce(
      (int a, int b) => a + b,
    );
