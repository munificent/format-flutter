...
class PubDependencies extends ArtifactSet {
  ...
  Future<bool> isUpToDate(FileSystem fileSystem) async {
    final File toolPackageConfig = fileSystem.file(fileSystem.path.join(
      _flutterRoot(),
      'packages',
      'flutter_tools',
      '.dart_tool',
    ...
    for (final Package package in packageConfig.packages) {
      if (!fileSystem.directory(package.root).childFile(
        'pubspec.yaml',
      ).existsSync()) {
        return false;
      }
...
class MaterialFonts extends CachedArtifact {
  MaterialFonts(Cache cache)
    : super('material_fonts', cache, DevelopmentArtifact.universal);
...
class FlutterWebSdk extends CachedArtifact {
  FlutterWebSdk(Cache cache)
    : super('flutter_web_sdk', cache, DevelopmentArtifact.web);
...
class LegacyCanvasKitRemover extends ArtifactSet {
  ...
  Directory _getLegacyCanvasKitDirectory(FileSystem fileSystem) => fileSystem
      .directory(fileSystem.path.join(cache.getRoot().path, 'canvaskit'));
...
class GradleWrapper extends CachedArtifact {
  GradleWrapper(Cache cache)
    : super('gradle_wrapper', cache, DevelopmentArtifact.universal);
  ...
  Future<void> updateInner(
    ...
    final File noticeFile = fileSystem.file(
      fileSystem.path.join(location.path, 'NOTICE'),
    );
  ...
  bool isUpToDateInner(FileSystem fileSystem) {
    final String gradleWrapper = fileSystem.path.join(
      'gradle',
      'wrapper',
      'gradle-wrapper.jar',
    );
    ...
    for (final String scriptName in _gradleScripts) {
      final File scriptFile = fileSystem.file(
        fileSystem.path.join(wrapperDir.path, scriptName),
      );
