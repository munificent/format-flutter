...
class PubDependencies extends ArtifactSet {
  ...
  Future<bool> isUpToDate(FileSystem fileSystem) async {
    ...
    final PackageConfig packageConfig =
        await loadPackageConfigWithLogging(toolPackageConfig, logger: _logger, throwOnError: false);
...
class LegacyCanvasKitRemover extends ArtifactSet {
  ...
  Directory _getLegacyCanvasKitDirectory(FileSystem fileSystem) => fileSystem.directory(
    fileSystem.path.join(cache.getRoot().path, 'canvaskit'),
  );

  @override
  Future<bool> isUpToDate(FileSystem fileSystem) async => !(await _getLegacyCanvasKitDirectory(fileSystem).exists());
...
class FlutterRunnerDebugSymbols extends CachedArtifact {
  FlutterRunnerDebugSymbols(Cache cache, {required Platform platform, VersionedPackageResolver? packageResolver})
    : _platform = platform,
...
class LinuxFuchsiaSDKArtifacts extends _FuchsiaSDKArtifacts {
  LinuxFuchsiaSDKArtifacts(Cache cache, {required Platform platform}) : _platform = platform, super(cache, 'linux');
...
class MacOSFuchsiaSDKArtifacts extends _FuchsiaSDKArtifacts {
  MacOSFuchsiaSDKArtifacts(Cache cache, {required Platform platform}) : _platform = platform, super(cache, 'mac');
