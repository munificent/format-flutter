...
class AndroidStudioJavaGradleConflictMigration extends ProjectMigrator {
  AndroidStudioJavaGradleConflictMigration(
    ...
    required Java? java,
  }) : _gradleWrapperPropertiesFile = getGradleWrapperFile(
  ...
  String migrateFileContents(String fileContents) {
    ...
    final RegExpMatch? gradleDistributionUrl = gradleOrgVersionMatch.firstMatch(
      fileContents,
    );
    ...
    if (gradleVersionsToUpgradeFrom.contains(existingVersionString)) {
      ...
      final String? gradleDistributionUrlString = gradleDistributionUrl.group(
        0,
      );
      if (gradleDistributionUrlString != null) {
        ...
        fileContents = fileContents.replaceFirst(
          gradleOrgVersionMatch,
          upgradedDistributionUrl,
        );
      } else {
        logger.printTrace(gradleVersionNotFound);
      }
