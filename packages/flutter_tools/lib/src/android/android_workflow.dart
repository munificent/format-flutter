...
final RegExp licenseCounts = RegExp(
  r'(\d+) of (\d+) SDK package licenses? not accepted.',
);
final RegExp licenseNotAccepted = RegExp(
  r'licenses? not accepted',
  caseSensitive: false,
);
...
class AndroidValidator extends DoctorValidator {
  ...
  Future<bool> _checkJavaVersion(List<ValidationMessage> messages) async {
    ...
    try {
      ...
      messages.add(
        ValidationMessage(_userMessages.androidJdkLocation(_java!.binaryPath)),
      );
      if (!_java!.canRun()) {
  ...
  Future<ValidationResult> validate() async {
    ...
    if (!androidSdk.cmdlineToolsAvailable) {
      messages.add(
        ValidationMessage.error(_userMessages.androidMissingCmdTools),
      );
      return ValidationResult(ValidationType.missing, messages);
    }
    ...
    if (androidSdkLatestVersion != null) {
      if (androidSdkLatestVersion.sdkLevel < kAndroidSdkMinVersion ||
        ...
        messages.add(ValidationMessage.error(_userMessages
...
class AndroidLicenseValidator extends DoctorValidator {
  ...
  Future<ValidationResult> validate() async {
    ...
    switch (await licensesAccepted) {
      ...
      case LicensesAccepted.none:
        messages.add(
          ValidationMessage.error(_userMessages.androidLicensesNone),
        );
        return ValidationResult(
          ValidationType.partial,
          messages,
  ...
  Future<bool> _checkJavaVersionNoOutput() async {
    ...
    String? javaVersion;
    try {
      final ProcessResult result = await _processManager
          .run(<String>[javaBinary, '-version']);
      if (result.exitCode == 0) {
        final List<String> versionLines = (result.stderr as String).split('\n');
        javaVersion = versionLines.length >= 2
            ? versionLines[1]
            : versionLines[0];
      }
    } on Exception catch (error) {
