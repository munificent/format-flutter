...
class AndroidSdk {
  ...
  static bool sdkDirectoryHasPlatformTools(String dir) {
    return globals.fs.isDirectorySync(
      globals.fs.path.join(dir, 'platform-tools'),
    );
  }
  ...
  late final String? adbPath = getPlatformToolsPath(
    globals.platform.isWindows ? 'adb.exe' : 'adb',
  );
  ...
  List<String> validateSdkWellFormed() {
    ...
    if (sdkVersions.isEmpty || latestVersion == null) {
      ...
      if (_platforms.isEmpty) {
        ...
        msg.write(
          _platforms.map((Directory dir) => '  - ${dir.basename}').join('\n'),
        );
      }
  ...
  String? getEmulatorPath() {
    final String binaryName = globals.platform.isWindows
        ? 'emulator.exe'
        : 'emulator';
  ...
  void reinitialize() {
    ...
    _sdkVersions = _platforms.map<AndroidSdkVersion?>((Directory platformDir) {
      ...
      try {
        final Match? numberedVersion = _numberedAndroidPlatformRe.firstMatch(
          platformName,
        );
