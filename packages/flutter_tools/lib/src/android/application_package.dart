...
class AndroidApk extends ApplicationPackage
  ...
  static AndroidApk? fromApk(
    ...
    String apptStdout;
    try {
      apptStdout = processUtils.runSync(<String>[
        aaptPath,
        'dump',
        'xmltree',
        apk.path,
  ...
  static Future<AndroidApk?> fromAndroidProject(
    ...
    if (androidProject.isUsingGradle && androidProject.isSupportedVersion) {
      ...
      apkFile = fileSystem.file(
        fileSystem.path.join(getAndroidBuildDirectory(), filename),
      );
    }
...
class _Element extends _Entry {
  ...
  Iterable<_Element> allElements(String name) {
    return children.whereType<_Element>().where(
      (_Element e) => e.name?.startsWith(name) ?? false,
    );
  }
}
...
class ApkManifestData {
  ...
  static ApkManifestData? parseFromXmlDump(String data, Logger logger) {
    ...
    final int manifestLine = lines.indexWhere(
      (String line) => line.contains('E: manifest'),
    );
    ...
    for (final _Element activity in activities) {
      ...
      final Iterable<_Element> intentFilters = activity.allElements(
        'intent-filter',
      );
    ...
    final String? packageName = package?.value?.substring(
      1,
      package.value?.indexOf('" '),
    );

    if (launchActivity == null) {
      logger.printError(
        'Error running $packageName. Default activity not found',
      );
      return null;
    }

    final _Attribute? nameAttribute = launchActivity.firstAttribute(
      'android:name',
    );
    ...
    final _Attribute? versionCodeAttr = manifest.firstAttribute(
      'android:versionCode',
    );
  ...
  get data => UnmodifiableMapView<String, Map<String, String>>(_data);
