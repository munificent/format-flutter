...
class AndroidDevice extends Device {
  ...
  late final Future<Map<String, String>> _properties = () async {
    ...
    final List<String> propCommand = adbCommandForDevice(<String>[
      'shell',
      'getprop',
    ]);
    ...
    try {
      ...
      _logger.printError(
        'Error retrieving device properties for $name: $error',
      );
    }
    return properties;
  }();
  ...
  Future<RunResult> runAdbCheckedAsync(
    ...
    return _processUtils.run(
      ...
      allowedFailures:
          (int value) => _allowHeapCorruptionOnWindows(value, _platform),
    );
  }

  bool _isValidAdbVersion(String adbVersion) {
    // Sample output: 'Android Debug Bridge version 1.0.31'
    final Match? versionFields = RegExp(r'(\d+)\.(\d+)\.(\d+)').firstMatch(
      adbVersion,
    );
  ...
  Future<bool> _checkForSupportedAdbVersion() async {
    ...
    try {
      final RunResult adbVersion = await _processUtils
          .run(<String>[adbPath, 'version'], throwOnError: true);
  ...
  Future<bool> _checkForSupportedAndroidVersion() async {
    ...
    try {
      ...
      await _processUtils
          .run(<String>[adbPath, 'start-server'], throwOnError: true);
  ...
  String _getSourceSha1(AndroidApk apk) {
    final File shaFile = _fileSystem.file(
      '${apk.applicationPackage.path}.sha1',
    );
  ...
  Future<bool> _installApp(AndroidApk app, {String? userIdentifier}) async {
    ...
    final RunResult installResult = await _processUtils
        .run(adbCommandForDevice(<String>[
          'install',
          '-t',
          '-r',
          if (userIdentifier != null) ...<String>['--user', userIdentifier],
  ...
  Future<bool> uninstallApp(
    ...
    String uninstallOut;
    try {
      final RunResult uninstallResult = await _processUtils
          .run(adbCommandForDevice(<String>[
            'uninstall',
            if (userIdentifier != null) ...<String>['--user', userIdentifier],
            app.id,
  ...
  Future<MemoryInfo> queryMemoryInfo() async {
    ...
    final RunResult runResult = await _processUtils.run(adbCommandForDevice(
      <String>['shell', 'dumpsys', 'meminfo', package.id, '-d'],
    ));
...
AndroidMemoryInfo parseMeminfoDump(String input) {
  ...
  final List<String> times = timelineData.trim().split(
    '${AndroidMemoryInfo._kRealTimeKey}:',
  );
...
class AdbLogReader extends DeviceLogReader {
  ...
  static final RegExp _fatalLog = RegExp(
    r'^F\/libc\s*\(\s*\d+\):\sFatal signal (\d+)',
  );

  // 'I/DEBUG(pid): ...'
  static final RegExp _tombstoneLine = RegExp(
    r'^[IF]\/DEBUG\s*\(\s*\d+\):\s(.+)$',
  );

  // 'I/DEBUG(pid): Tombstone written to: '
  static final RegExp _tombstoneTerminator = RegExp(
