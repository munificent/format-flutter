...
final RegExp _kBuildVariantRegex = RegExp(
  '^BuildVariant: (?<$_kBuildVariantRegexGroupName>.*)\$',
);
...
final RegExp _kApplicationIdRegex = RegExp(
  '^ApplicationId: (?<$_kApplicationIdRegexGroupName>.*)\$',
);
...
final RegExp _kAppLinkDomainsRegex = RegExp(
  '^Domain: (?<$_kAppLinkDomainsGroupName>.*)\$',
);
...
String _taskFor(String prefix, BuildInfo buildInfo) {
  ...
  return _taskForBuildVariant(
    prefix,
    '$productFlavor${sentenceCase(buildType)}',
  );
}

String _taskForBuildVariant(
  String prefix,
  String buildVariant, [
  String suffix = '',
]) {
...
class AndroidGradleBuilder implements AndroidBuilder {
  ...
  Future<void> buildGradleApp({
    ...
    final bool usesAndroidX = isAppUsingAndroidX(
        ...
        switch (status) {
          case GradleBuildStatus.retry:
            ...
            final int waitTime = min(
              pow(2, retry).toInt() * 100,
              kMaxRetryTime.inMicroseconds,
            );
            retry += 1;
            _logger.printStatus(
    ...
    if (isBuildingBundle) {
      ...
      _logger.printStatus(
        '${_logger.terminal.successMark} Built ${_fileSystem.path.relative(
          bundleFile.path,
        )}$appSize.',
  ...
  Future<void> _performCodeSizeAnalysis(
    ...
    final Map<String, Object?> output = await sizeAnalyzer
    ...
    final File outputFile = _fileSystemUtils.getUniqueFile(
      _fileSystem.directory(_fileSystemUtils.homeDirPath).childDirectory(
        '.flutter-devtools',
      ),
  ...
  Future<void> buildGradleAar({
    ...
    if (buildInfo.dartObfuscation && buildInfo.mode != BuildMode.release) {
      _logger.printStatus(
        'Dart obfuscation is not supported in ${sentenceCase(
...
File findBundleFile(
  ...
  final List<File> fileCandidates = <File>[
    getBundleDirectory(project).childDirectory(
      camelCase(buildInfo.modeName),
    ).childFile('app.aab'),
    getBundleDirectory(project).childDirectory(
      camelCase(buildInfo.modeName),
  ...
  if (buildInfo.flavor != null) {
    ...
    fileCandidates.add(getBundleDirectory(project)
        .childDirectory(
          '${buildInfo.uncapitalizedFlavor}${camelCase(
            '_${buildInfo.modeName}',
          )}',
        )
        .childFile(
    ...
    fileCandidates.add(getBundleDirectory(project)
        ...
        .childFile(
          'app-${kebabCase(
            buildInfo.uncapitalizedFlavor!,
          )}-${buildInfo.modeName}.aab',
        ));
  }
