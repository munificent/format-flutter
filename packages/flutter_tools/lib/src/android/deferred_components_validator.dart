...
abstract class DeferredComponentsValidator {
  DeferredComponentsValidator(
    ...
    String? title,
  }) : outputDir = projectDir.childDirectory('build').childDirectory(
  ...
  void displayResults() {
    if (changesNeeded) {
      ...
      logger.printStatus(
        title,
        indent: (_thickDivider.length - title.length) ~/ 2,
        emphasis: true,
      );
      logger.printStatus(_thickDivider);
      ...
      if (generatedFiles.isNotEmpty) {
        ...
        for (final String filePath in generatedFiles) {
          final String shortenedPath = filePath.substring(
            projectDir.parent.path.length + 1,
          );
