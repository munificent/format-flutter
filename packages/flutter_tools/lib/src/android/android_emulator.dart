...
class AndroidEmulators extends EmulatorDiscovery {
  ...
  Future<List<AndroidEmulator>> _getEmulatorAvds() async {
    ...
    final String listAvdsOutput = (await _processUtils
            .run(<String>[emulatorPath, '-list-avds']))
        .stdout
        .trim();
  ...
  AndroidEmulator _loadEmulatorInfo(String id) {
    ...
    final File iniFile = _fileSystem.file(
      _fileSystem.path.join(avdPath, '$id.ini'),
    );
    ...
    final File configFile = _fileSystem.file(
      _fileSystem.path.join(path, 'config.ini'),
    );
    if (!configFile.existsSync()) {
      return androidEmulatorWithoutProperties;
    }
    final Map<String, String> properties = parseIniLines(
      configFile.readAsLinesSync(),
    );
    return AndroidEmulator(
      id,
