...
class MultiRootFileSystem extends ForwardingFileSystem {
  ...
  @override
  File file(dynamic path) => MultiRootFile(fileSystem: this, delegate: delegate.file(_resolve(path)));
  ...
  @override
  Link link(dynamic path) => MultiRootLink(fileSystem: this, delegate: delegate.link(_resolve(path)));
  ...
  @override
  Future<io.FileSystemEntityType> type(String path, {bool followLinks = true}) => delegate.type(
    _resolve(path).toString(),
    followLinks: followLinks,
  );

  @override
  io.FileSystemEntityType typeSync(String path, {bool followLinks = true}) => delegate.typeSync(
    _resolve(path).toString(),
    followLinks: followLinks,
  );
  ...
  @override
  String toString() => 'MultiRootFileSystem(scheme = $_scheme, roots = $_roots, delegate = $delegate)';
}
...
abstract class MultiRootFileSystemEntity<T extends FileSystemEntity, D extends io.FileSystemEntity>
  ...
  @override
  Directory wrapDirectory(io.Directory delegate) => MultiRootDirectory(fileSystem: fileSystem, delegate: delegate);
...
class MultiRootDirectory extends MultiRootFileSystemEntity<Directory, io.Directory>
  ...
  @override
  Directory childDirectory(String basename) => fileSystem.directory(fileSystem.path.join(delegate.path, basename));
