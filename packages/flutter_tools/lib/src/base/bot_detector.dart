...
class BotDetector {
  ...
  Future<bool> get isRunningOnBot async {
    ...
    final bool result = _platform.environment['BOT'] == 'true'
        ...
        ||
        (_platform.environment.containsKey('AWS_REGION') && _platform.environment.containsKey('CODEBUILD_INITIATOR'))
...
@visibleForTesting
class AzureDetector {
  AzureDetector({required HttpClientFactory httpClientFactory}) : _httpClientFactory = httpClientFactory;
  ...
  Future<bool> get isRunningOnAzure async {
    ...
    try {
      final HttpClientRequest request = await client.getUrl(Uri.parse(_serviceUrl)).timeout(requestTimeout);
