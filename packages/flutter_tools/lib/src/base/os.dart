...
class _PosixUtils extends OperatingSystemUtils {
  ...
  void chmod(FileSystemEntity entity, String mode) {
    ...
    try {
      final ProcessResult result = _processManager
          .runSync(<String>['chmod', mode, entity.path]);
  ...
  HostPlatform get hostPlatform {
    if (_hostPlatform == null) {
      final RunResult hostPlatformCheck = _processUtils
          .runSync(<String>['uname', '-m']);
      ...
      if (hostPlatformCheck.exitCode != 0) {
        ...
        _logger.printError(
          ...
          'Assuming host platform is ${getNameForHostPlatform(
            _hostPlatform!,
          )}.',
        );
  ...
  String get name {
    if (_name == null) {
      ...
      try {
        ...
        final List<String> osVersionSplit = _platform.operatingSystemVersion
            .split(' ');
...
class _MacOSUtils extends _PosixUtils {
  ...
  String get name {
    if (_name == null) {
      ...
      if (results.every((RunResult result) => result.exitCode == 0)) {
        ...
        _name = '${results[0].stdout.trim()} ${results[
          1
        ].stdout.trim()} ${results[2].stdout.trim()} $osName';
      }
      _name ??= super.name;
    }
    return _name!;
  ...
  HostPlatform get hostPlatform {
    if (_hostPlatform == null) {
      ...
      final RunResult arm64Check = _processUtils
          .runSync(<String>[sysctlPath, 'hw.optional.arm64']);
...
class _WindowsUtils extends OperatingSystemUtils {
  ...
  void _unpackArchive(Archive archive, Directory targetDirectory) {
    for (final ArchiveFile archiveFile in archive.files) {
      ...
      final File destFile = _fileSystem.file(_fileSystem.path.canonicalize(
  ...
  String get name {
    if (_name == null) {
      final ProcessResult result = _processManager
          .runSync(<String>['ver'], runInShell: true);
      if (result.exitCode == 0) {
