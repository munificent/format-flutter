...
class ErrorHandlingFileSystem extends ForwardingFileSystem {
  ...
  @override
  File file(dynamic path) => ErrorHandlingFile(platform: _platform, fileSystem: this, delegate: delegate.file(path));
  ...
  @override
  Link link(dynamic path) => ErrorHandlingLink(platform: _platform, fileSystem: this, delegate: delegate.link(path));
...
class ErrorHandlingFile extends ForwardingFileSystemEntity<File, io.File>
  ...
  @override
  File wrapFile(io.File delegate) => ErrorHandlingFile(platform: _platform, fileSystem: fileSystem, delegate: delegate);
  ...
  @override
  Link wrapLink(io.Link delegate) => ErrorHandlingLink(platform: _platform, fileSystem: fileSystem, delegate: delegate);
  ...
  Future<File> writeAsString(
    ...
    return _run<File>(
      () async => wrap(await delegate.writeAsString(contents, mode: mode, encoding: encoding, flush: flush)),
      platform: _platform,
      failureMessage: 'Flutter failed to write to a file at "${delegate.path}"',
      posixPermissionSuggestion: _posixPermissionSuggestion(<String>[delegate.path]),
    );
...
class ErrorHandlingDirectory extends ForwardingFileSystemEntity<Directory, io.Directory>
  ...
  // ignore: prefer_mixin
  ErrorHandlingDirectory({required Platform platform, required this.fileSystem, required this.delegate})
    : _platform = platform;
...
class ErrorHandlingProcessManager extends ProcessManager {
  ...
  Future<io.ProcessResult> run(
    ...
    }, platform: _platform, failureMessage: 'Flutter failed to run "${command.join(' ')}"');
  }
