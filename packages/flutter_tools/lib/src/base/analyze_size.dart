...
class SizeAnalyzer {
  ...
  Future<Map<String, Object?>> analyzeAotSnapshot({
    ...
    final Object? decodedAotSnapshot = json.decode(
      aotSnapshot.readAsStringSync(),
    );
    if (decodedAotSnapshot == null) {
      throwToolExit('AOT snapshot is invalid for analysis');
    }
    final Map<String, Object?> processedAotSnapshotJson = treemapFromJson(
      decodedAotSnapshot,
    );
    final _SymbolNode? aotSnapshotJsonRoot = _parseAotSnapshot(
      ...
      for (final String path in paths) {
        ...
        if (childWithPathAsName == null) {
          ...
          if (matchesPattern(path, pattern: _appFilenamePattern) != null) {
            ...
            _locatedAotFilePath = _buildNodeName(
              childWithPathAsName,
              currentNode,
            );
  ...
  void _printLibChildrenPaths(
    ...
    if (currentNode.children.isNotEmpty &&
      ...
      // larger than 1KB
      final bool isAotSnapshotPath = _locatedAotFilePath.join('/').contains(
        totalPath,
      );
  ...
  _SymbolNode? _buildNodeWithChildren(Map<String, Object?> aotSnapshotJson) {
    ...
    for (final Object? child in rawChildren) {
      ...
      final _SymbolNode? childTreemapNode = _parseAotSnapshot(
...
class _SymbolNode {
  ...
  _SymbolNode addChild(_SymbolNode child) {
    ...
    assert(
      !_children.containsKey(child.name),
