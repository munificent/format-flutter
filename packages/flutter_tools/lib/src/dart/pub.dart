...
class PubContext {
  ...
  static PubContext getVerifyContext(String commandName) => PubContext._(
    <String>['verify', commandName.replaceAll('-', '_')],
  );
  ...
  static final PubContext createPlugin = PubContext._(<String>[
    'create_plugin',
  ]);
  ...
  static final PubContext pubPassThrough = PubContext._(<String>[
    'passthrough',
  ]);
  static final PubContext runTest = PubContext._(<String>['run_test']);
  static final PubContext flutterTests = PubContext._(<String>[
    'flutter_tests',
  ]);
  static final PubContext updatePackages = PubContext._(<String>[
    'update_packages',
  ]);
...
class _DefaultPub implements Pub {
  ...
  Future<void> get({
    ...
    if (checkUpToDate &&
        ...
        pubspecYaml.lastModifiedSync().isBefore(
          pubLockFile.lastModifiedSync(),
        ) &&
        pubspecYaml.lastModifiedSync().isBefore(
          packageConfigFile.lastModifiedSync(),
        ) &&
  ...
  void _preloadPubCache() {
    ...
    final Directory preloadCacheDir = flutterRoot.childDirectory(
      '.pub-preload-cache',
    );
...
It can be reset by running `dart pub cache clean`.''');
    ...
    if (home != null) {
      final String dartServerCachePath = _fileSystem.path.join(
        home,
        '.dartServer',
      );
...
It can be reset by deleting $dartServerCachePath.''');
  ...
  Future<void> _updatePackageConfig(FlutterProject project) async {
    ...
    if (packageConfig.packages.any(
      (Package package) => package.name == 'flutter_gen',
    )) {
      return;
    }
    ...
    final Map<String, dynamic> jsonContents = json.decode(
