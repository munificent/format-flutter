...
class PubContext {
  ...
  static PubContext getVerifyContext(String commandName) => PubContext._(<String>[
    'verify',
    commandName.replaceAll('-', '_'),
  ]);
...
enum PubOutputMode { none, all, summaryOnly }
...
class _DefaultPub implements Pub {
  ...
  Future<void> get({
    ...
    if (shouldSkipThirdPartyGenerator && project.packageConfigFile.existsSync()) {
      ...
      try {
        packageConfigMap = jsonDecode(project.packageConfigFile.readAsStringSync()) as Map<String, Object?>;
      } on FormatException {
    ...
    try {
      if (outputMode != PubOutputMode.none) {
        ...
        if (stdio == null) {
          ...
          final StreamSubscription<List<int>> stdoutSubscription = process.stdout.listen(stdio.stdout.add);
          final StreamSubscription<List<int>> stderrSubscription = process.stderr.listen(stdio.stderr.add);
      ...
      if (exception is io.ProcessException) {
        ...
        throw io.ProcessException(exception.executable, exception.arguments, buffer.toString(), exception.errorCode);
      }
      rethrow;
    }
  ...
  Future<void> batch(
    ...
    final Map<String, String> pubEnvironment =
        await _createPubEnvironment(context: context, flutterRootOverride: flutterRootOverride);
    final List<String> pubCommand = <String>[..._pubCommand, ...arguments];
  ...
  List<String> _computePubCommand() {
    ...
    final String sdkPath = _fileSystem.path
        .joinAll(<String>[Cache.flutterRoot!, 'bin', 'cache', 'dart-sdk', 'bin', 'dart']);
  ...
  void _preloadPubCache() {
    ...
    if (preloadCacheDir.existsSync()) {
      ...
      final Iterable<String> cacheFiles = preloadCacheDir.listSync().map((FileSystemEntity f) => f.path).where(
        (String path) => path.endsWith('.tar.gz'),
      );
...
It can be reset by deleting $dartServerCachePath.''');
  ...
  Future<void> _updateVersionAndPackageConfig(FlutterProject project) async {
    ...
    final File lastVersion = _fileSystem.file(_fileSystem.path.join(project.directory.path, '.dart_tool', 'version'));
    final File currentVersion = _fileSystem.file(_fileSystem.path.join(Cache.flutterRoot!, 'version'));
  ...
  Future<void> _updatePackageConfig(FlutterProject project) async {
    ...
    packageConfigFile.parent.childFile('package_config_subset').writeAsStringSync(
      _computePackageConfigSubset(packageConfig, _fileSystem),
    );
    ...
    (jsonContents['packages'] as List<dynamic>)
        .add(<String, dynamic>{'name': 'flutter_gen', 'rootUri': 'flutter_gen', 'languageVersion': '2.12'});
