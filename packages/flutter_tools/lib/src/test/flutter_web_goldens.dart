...
class TestGoldenComparator {
  ...
  Future<Process?> _startProcess(String testBootstrap) async {
    ...
    final File listenerFile = (await tempDir.createTemp('listener')).childFile(
      'listener.dart',
    );
    ...
    final List<String> command = <String>[
      ...
      '--disable-vm-service',
      '--non-interactive',
      '--packages=${_fileSystem.path.join(
        '.dart_tool',
        'package_config.json',
      )}',
      output,
    ...
    final Map<String, String> environment = <String, String>{
      ...
      'FLUTTER_WEB_RENDERER': webRenderer == WebRendererMode.html
          ? 'html'
          : 'canvaskit',
    };
...
class TestGoldenComparatorProcess {
  ...
  static String generateBootstrap(
    File testFile,
    Uri testUri, {
    required Logger logger,
  }) {
...
${testConfigFile != null ? "import '${Uri.file(
                testConfigFile.path,
              )}' as test_config;" : ""}
