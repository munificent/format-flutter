...
class FlutterPlatform extends PlatformPlugin {
  ...
  StreamChannel<dynamic> loadChannel(String path, SuitePlatform platform) {
    ...
    testCompleteCompleter.complete(
      _startTest(path, localChannel, ourTestCount),
    );
  ...
  Future<_AsyncError?> _startTest(
    ...
    try {
        ...
        if (integrationTestDevice == null) {
          ...
          if (mainDart == null) {
            testHarnessChannel.sink.addError(
              'Compilation failed for testPath=$testPath',
            );
            return null;
          }
        } else {
      ...
      final Stopwatch? testTimeRecorderStopwatch = testTimeRecorder?.start(
        TestTimePhases.Run,
      );
      final Future<StreamChannel<String>> remoteChannelFuture = testDevice
          .start(mainDart!);
      ...
      globals.printTrace(
        'test $ourTestCount: awaiting connection to test device',
      );
      await Future.any<void>(<Future<void>>[
        ...
        () async {
          ...
          testTimeRecorder?.stop(
            TestTimePhases.Run,
            testTimeRecorderStopwatch!,
          );
          final Stopwatch? watchTestTimeRecorderStopwatch = testTimeRecorder
              ?.start(TestTimePhases.WatcherFinishedTest);
    ...
    if (outOfBandError != null) {
      globals.printTrace(
        'test $ourTestCount: finished with out-of-band failure',
      );
    } else {
      globals.printTrace('test $ourTestCount: finished');
    }
...
class _FlutterPlatformStreamSinkWrapper<S> implements StreamSink<S> {
  ...
  @override
  void addError(Object errorEvent, [StackTrace? stackTrace]) => _parent
