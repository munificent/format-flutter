...
Future<vm_service.VmService> setUpVmService({
  ...
  vmService.registerServiceCallback(kFlutterVersionServiceName, (Map<String, Object?> params) async {
    final FlutterVersion version =
        context.get<FlutterVersion>() ?? FlutterVersion(fs: globals.fs, flutterRoot: Cache.flutterRoot!);
  ...
  if (flutterProject != null) {
    ...
    registrationRequests.add(vmService.registerService(kFlutterGetIOSBuildOptionsServiceName, kFlutterToolAlias));
    ...
    vmService.registerServiceCallback(kFlutterGetAndroidBuildVariantsServiceName, (Map<String, Object?> params) async {
      ...
      return <String, Object>{
        'result': <String, Object>{kResultType: kResultTypeSuccess, 'variants': options},
      };
    });
    registrationRequests.add(vmService.registerService(kFlutterGetAndroidBuildVariantsServiceName, kFlutterToolAlias));
    ...
    registrationRequests.add(vmService.registerService(kFlutterGetAndroidAppLinkSettingsName, kFlutterToolAlias));
  }

  if (printStructuredErrorLogMethod != null) {
    vmService.onExtensionEvent.listen(printStructuredErrorLogMethod);
    registrationRequests.add(vmService.streamListen(vm_service.EventStreams.kExtension).then<vm_service.Success?>(
...
Future<FlutterVmService> _connect(
  ...
  final vm_service.VmService delegateService =
      await createVmServiceDelegate(wsUri, compression: compression, logger: logger);
...
String _validateRpcStringParam(String methodName, Map<String, Object?> params, String paramName) {
  ...
  if (value is! String || value.isEmpty) {
    throw vm_service.RPCError(methodName, RPCErrorCodes.kInvalidParams, "Invalid '$paramName': $value");
  }
  return value;
}
...
class FlutterVmService {
  ...
  Future<void> setAssetDirectory({
    ...
    await callMethodWrapper(
      ...
      args: <String, Object?>{'viewId': viewId, 'assetDirectory': assetsDirectory.toFilePath(windows: windows)},
    );
  }
  ...
  Future<Map<String, Object?>?> getSkSLs({required String viewId}) async {
    final vm_service.Response? response =
        await callMethodWrapper(kGetSkSLsMethod, args: <String, String>{'viewId': viewId});
    if (response == null) {
      return null;
    }
  ...
  Future<void> flushUIThreadTasks({required String uiIsolateId}) async {
    await callMethodWrapper(kFlushUIThreadTasksMethod, args: <String, String>{'isolateId': uiIsolateId});
  }
  ...
  /// engine.
  Future<void> runInView({required String viewId, required Uri main, required Uri assetsDirectory}) async {
    try {
  ...
  Future<String> flutterDebugDumpApp({required String isolateId}) async {
    final Map<String, Object?>? response =
        await invokeFlutterExtensionRpcRaw('ext.flutter.debugDumpApp', isolateId: isolateId);
  ...
  Future<String> flutterDebugDumpLayerTree({required String isolateId}) async {
    final Map<String, Object?>? response =
        await invokeFlutterExtensionRpcRaw('ext.flutter.debugDumpLayerTree', isolateId: isolateId);
    return response?['data']?.toString() ?? '';
  }

  Future<String> flutterDebugDumpFocusTree({required String isolateId}) async {
    final Map<String, Object?>? response =
        await invokeFlutterExtensionRpcRaw('ext.flutter.debugDumpFocusTree', isolateId: isolateId);
    return response?['data']?.toString() ?? '';
  }

  Future<String> flutterDebugDumpSemanticsTreeInTraversalOrder({required String isolateId}) async {
    final Map<String, Object?>? response =
        await invokeFlutterExtensionRpcRaw('ext.flutter.debugDumpSemanticsTreeInTraversalOrder', isolateId: isolateId);
    return response?['data']?.toString() ?? '';
  }

  Future<String> flutterDebugDumpSemanticsTreeInInverseHitTestOrder({required String isolateId}) async {
    final Map<String, Object?>? response = await invokeFlutterExtensionRpcRaw(
  ...
  Future<Map<String, Object?>?> _flutterToggle(String name, {required String isolateId}) async {
    Map<String, Object?>? state = await invokeFlutterExtensionRpcRaw('ext.flutter.$name', isolateId: isolateId);
  ...
  Future<Map<String, Object?>?> flutterToggleDebugPaintSizeEnabled({required String isolateId}) => _flutterToggle(
    'debugPaint',
    isolateId: isolateId,
  );

  Future<Map<String, Object?>?> flutterTogglePerformanceOverlayOverride({required String isolateId}) => _flutterToggle(
    'showPerformanceOverlay',
    isolateId: isolateId,
  );

  Future<Map<String, Object?>?> flutterToggleWidgetInspector({required String isolateId}) => _flutterToggle(
    'inspector.show',
    isolateId: isolateId,
  );

  Future<Map<String, Object?>?> flutterToggleInvertOversizedImages({required String isolateId}) => _flutterToggle(
    'invertOversizedImages',
    isolateId: isolateId,
  );

  Future<Map<String, Object?>?> flutterToggleProfileWidgetBuilds({required String isolateId}) => _flutterToggle(
    'profileWidgetBuilds',
    isolateId: isolateId,
  );
  ...
  Future<Map<String, Object?>?> flutterFastReassemble({required String isolateId, required String className}) {
    return invokeFlutterExtensionRpcRaw(
  ...
  Future<bool> flutterAlreadyPaintedFirstUsefulFrame({required String isolateId}) async {
    final Map<String, Object?>? result =
        await invokeFlutterExtensionRpcRaw('ext.flutter.didSendFirstFrameRasterizedEvent', isolateId: isolateId);
  ...
  Future<bool> flutterExit({required String isolateId}) async {
    try {
      final Map<String, Object?>? result = await invokeFlutterExtensionRpcRaw('ext.flutter.exit', isolateId: isolateId);
  ...
  /// is updated with this value.
  Future<Brightness?> flutterBrightnessOverride({Brightness? brightness, required String isolateId}) async {
    final Map<String, Object?>? result = await invokeFlutterExtensionRpcRaw(
  ...
  Future<vm_service.Response?> _checkedCallServiceExtension(String method, {Map<String, Object?>? args}) async {
    try {
  ...
  Future<Map<String, Object?>?> invokeFlutterExtensionRpcRaw(
    ...
    final vm_service.Response? response =
        await _checkedCallServiceExtension(method, args: <String, Object?>{'isolateId': isolateId, ...?args});
    return response?.json;
  }
  ...
  Future<void> reloadAssetFonts({required String isolateId, required String viewId}) async {
    await callMethodWrapper(kReloadAssetFonts, isolateId: isolateId, args: <String, Object?>{'viewId': viewId});
  }
