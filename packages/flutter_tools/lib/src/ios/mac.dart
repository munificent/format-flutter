...
class IMobileDevice {
  ...
  Future<Process> startLogger(String deviceID) {
    return _processUtils.start(
      <String>[_idevicesyslogPath, '-u', deviceID],
...
Future<XcodeBuildResult> buildXcodeProject({
  ...
  final String? configuration = projectInfo.buildConfigurationFor(
    buildInfo,
    scheme,
  );
  ...
  final Map<String, String> buildSettings = await app.project
  ...
  try {
    ...
    buildResult =
        await _runBuildWithRetries(buildCommands, app, resultBundleFile);
    ...
    initialBuildStatus = null;
    globals.printStatus(
      'Xcode ${xcodeBuildActionToString(buildAction)} done.'.padRight(
            kDefaultStatusPadding + 1,
          ) +
...
Future<void> diagnoseXcodeBuildFailure(
  ...
  // Handle errors.
  final bool issueDetected = _handleIssues(
    result.xcResult,
    logger,
    xcodeBuildExecution,
  );
  ...
  for (final String line in lines) {
    ...
    if (match != null) {
      if (printedStatuses.add(match.group(1)!)) {
        logger.printStatus(
          'Removing obsolete reference to ${match.group(
            1,
          )} from ${project.xcodeProject.basename}',
        );
      }
    } else {
      buffer.writeln(line);
...
bool _handleIssues(
  ...
  if (xcResult != null && xcResult.parseSuccess) {
    ...
    globals.printTrace(
      'XCResult parsing error: ${xcResult.parsingErrorMessage}',
    );
  }
