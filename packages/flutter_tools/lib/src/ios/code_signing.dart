...
Future<String?> _getCodeSigningIdentityDevelopmentTeam({
  ...
  try {
    findIdentityStdout = (await processUtils.run(findIdentityCommand, throwOnError: true)).stdout.trim();
  ...
  try {
    signingCertificateStdout = (await processUtils
            .run(<String>['security', 'find-certificate', '-c', signingCertificateId, '-p'], throwOnError: true))
        .stdout
        .trim();
  } on ProcessException catch (error) {
