...
final RegExp _securityFindIdentityCertificateCnExtractionPattern = RegExp(
  r'.*\(([a-zA-Z0-9]+)\)',
);
final RegExp _certificateOrganizationalUnitExtractionPattern = RegExp(
  r'OU=([a-zA-Z0-9]+)',
);
...
Future<Map<String, String>?> getCodeSigningIdentityDevelopmentTeamBuildSetting({
  ...
  if (_isNotEmpty(buildSettings[_developmentTeamBuildSettingName])) {
    logger.printStatus(
      ...
      'team in Xcode project: ${buildSettings[
...
Future<String?> _getCodeSigningIdentityDevelopmentTeam({
  ...
  try {
    findIdentityStdout =
        (await processUtils.run(findIdentityCommand, throwOnError: true))
            .stdout
            .trim();
  ...
  try {
    signingCertificateStdout = (await processUtils.run(<String>[
          'security',
          'find-certificate',
          '-c',
  ...
  final Process opensslProcess = await processUtils
      .start(const <String>['openssl', 'x509', '-subject']);
...
Future<String?> _chooseSigningIdentity(
  ...
  if (validCodeSigningIdentities.length > 1) {
    ...
    for (int i = 0; i < count; i++) {
      logger.printStatus(
        '  ${i + 1}) ${validCodeSigningIdentities[i]}',
        emphasis: true,
      );
    }
