...
String flutterMacOSFrameworkDir(
  ...
  return fileSystem.path.normalize(
    fileSystem.path.dirname(flutterMacOSFramework),
  );
}
...
Future<List<String>> _xcodeBuildSettingsLines({
  ...
  xcodeBuildSettings.add(
    'FLUTTER_APPLICATION_PATH=${globals.fs.path.normalize(
  ...
  xcodeBuildSettings.add(
    'FLUTTER_BUILD_DIR=${buildDirOverride ?? getBuildDirectory()}',
  );
  ...
  if (localEngineInfo != null) {
    ...
    xcodeBuildSettings.add(
      'FLUTTER_ENGINE=${globals.fs.path.dirname(globals.fs.path.dirname(
        engineOutPath,
      ))}',
    );
  ...
  if (!useMacOSConfig) {
    ...
    xcodeBuildSettings.add(
      'EXCLUDED_ARCHS[sdk=iphonesimulator*]=$excludedSimulatorArchs',
    );
