    ...
    try {
      if (_versionText == null) {
        final RunResult result = _processUtils
            .runSync(<String>[...xcrunCommand(), 'xcodebuild', '-version']);
      ...
      final int minorVersion = components.length < 2
          ? 0
          : int.parse(components[1]);
      final int patchVersion = components.length < 3
          ? 0
...
List<String> environmentVariablesAsXcodeBuildSettings(Platform platform) {
  ...
  return platform.environment.entries.where(
...
String substituteXcodeVariables(
  String str,
  Map<String, String> xcodeBuildSettings,
) {
  ...
  return str.replaceAllMapped(
    _varExpr,
    (Match m) => xcodeBuildSettings[m[1]!] ?? m[0]!,
  );
}
...
class XcodeProjectInfo {
  ...
  String? buildConfigurationFor(BuildInfo? buildInfo, String scheme) {
    ...
    final String expectedConfiguration = expectedBuildConfigurationFor(
      buildInfo,
      scheme,
    );
  ...
  static String? _uniqueMatch(
    ...
    bool Function(String s) matches,
  ) {
