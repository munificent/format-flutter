  ...
  Future<bool> uninstallApp(
    ...
    try {
      uninstallationResult =
          await _iosDeploy.uninstallApp(deviceId: id, bundleId: app.id);
    } on ProcessException catch (e) {
      _logger.printError(e.message);
      return false;
    }
  ...
  Future<LaunchResult> startApp(
    ...
    if (!prebuiltApplication) {
      ...
      packageId = buildResult
          .xcodeBuildExecution
          ?.buildSettings[IosProject.kProductBundleIdKey];
    }
    ...
    try {
      ...
      final Timer timer = Timer(
        discoveryTimeout ?? Duration(seconds: defaultTimeout),
        () {
          _logger.printError(
      ...
      if (isWirelesslyConnected) {
        ...
        localUri = await MDnsVmServiceDiscovery.instance!
...
class IOSDeviceLogReader extends DeviceLogReader {
  IOSDeviceLogReader._(
    ...
    bool usingCISystem,
      ...
      _runnerLineRegex = RegExp(
        appName + r'(\(Flutter\))?\[[\d]+\] <[A-Za-z]+>: ',
      ),
  ...
  Future<void> _listenToUnifiedLoggingEvents(
    ...
    try {
      ...
      await Future.wait(<Future<void>>[
        connectedVmService.service.streamListen(
          vm_service.EventStreams.kStdout,
        ),
        connectedVmService.service.streamListen(
          vm_service.EventStreams.kStderr,
        ),
      ]);
  ...
  void _listenToSysLog() {
    ...
    if (!useBothLogDeviceReaders &&
        _majorSdkVersion >= minimumUniversalLoggingSdkVersion) {
      return;
    }
