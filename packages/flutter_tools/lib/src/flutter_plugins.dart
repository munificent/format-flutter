...
void _renderTemplateToFile(
  ...
  final String renderedTemplate = templateRenderer.renderString(
    template,
    context,
  );
...
bool _writeFlutterPluginsList(FlutterProject project, List<Plugin> plugins) {
  ...
  if (oldPluginsFileStringContent != null) {
    pluginsChanged = oldPluginsFileStringContent.contains(
      pluginsMap.toString(),
    );
  }
...
bool _writeFlutterPluginsListLegacy(
  ...
  for (final Plugin plugin in plugins) {
    flutterPluginsBuffer.write(
      '${plugin.name}=${globals.fsUtils.escapePath(plugin.path)}\n',
    );
  }
    ...
    case AndroidEmbeddingVersion.v2:
      ...
      if (pluginsUsingV1.length > 1) {
        globals.printWarning(
          'The plugins `${pluginsUsingV1.join(
            ', ',
...
Future<void> _writeLinuxPluginFiles(
  ...
  final List<Plugin> ffiPlugins = _filterFfiPlugins(
    plugins,
    LinuxPlugin.kConfigKey,
...
List<Plugin> _filterPluginsByVariant(
  ...
  return plugins.where((Plugin element) {
    ...
    return (platformPlugin as VariantPlatformPlugin).supportedVariants.contains(
      variant,
    );
  }).toList();
}
...
Future<void> _writeWebPluginRegistrant(
  ...
  final List<Map<String, Object?>> webPlugins = _extractPlatformMaps(
    plugins,
...
void createPluginSymlinks(
  ...
  final String? pluginFileContent = _readFileContent(
...
void handleSymlinkException(
  ...
  if (platform.isWindows) {
    ...
    if (e.osError?.errorCode == 1314) {
      final String? versionString = RegExp(r'[\d.]+')
          .firstMatch(os.name)
          ?.group(0);
...
List<PluginInterfaceResolution> resolvePlatformImplementation(
  ...
  for (final Plugin plugin in plugins) {
    for (final String platform in platforms) {
      ...
      if (implementsPackage == null || implementsPackage.isEmpty) {
        ...
        final String defaultImplementationKey = getResolutionKey(
          platform: platform,
      ...
      final String resolutionKey = getResolutionKey(
        platform: platform,
        packageName: implementsPackage,
      );
      if (!possibleResolutions.containsKey(resolutionKey)) {
        possibleResolutions[resolutionKey] = <PluginInterfaceResolution>[];
      }
      possibleResolutions[resolutionKey]!.add(
        PluginInterfaceResolution(plugin: plugin, platform: platform),
      );
    }
  }
  ...
  for (final MapEntry<String, List<PluginInterfaceResolution>> entry
    ...
    final Iterable<PluginInterfaceResolution> directDependencies = candidates
        .where((PluginInterfaceResolution r) {
      ...
      if (directDependencies.length > 1) {
        globals.printError(
          ...
          '${directDependencies.map(
            (PluginInterfaceResolution r) => '  ${r.plugin.name}\n',
          ).join()}'
...
Future<void> generateMainDartWithPluginRegistrant(
  ...
  for (final PluginInterfaceResolution resolution in resolutions) {
    ...
    (templateContext[resolution.platform] as List<Object?>?)?.add(
      resolution.toMap(),
    );
  }
  try {
