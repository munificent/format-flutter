...
class ChromiumLauncher {
  ...
  Future<Chromium> launch(
    ...
    if (_logger.isVerbose && !_platform.isWindows) {
      ...
      final ProcessResult versionResult = await _processManager
          .run(<String>[chromeExecutable, '--version']);
  ...
  Future<Process?> _spawnChromiumProcess(
    ...
    if (_operatingSystemUtils.hostPlatform == HostPlatform.darwin_arm64) {
      final ProcessResult result = _processManager
          .runSync(<String>['file', chromeExecutable]);
  ...
  String get _preferencesPath => _fileSystem.path.join(
    'Default',
    'preferences',
  );
