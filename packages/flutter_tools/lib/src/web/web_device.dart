...
abstract class ChromiumDevice extends Device {
  ...
  Future<LaunchResult> startApp(
    ...
    _logger.sendEvent('app.webLaunchUrl', <String, Object>{
      'url': url,
    ...
    if (_platform.isWindows) {
      if (_processManager.canRun('reg')) {
        ...
        if (result.exitCode == 0) {
          final List<String> parts = (result.stdout as String).split(
            RegExp(r'\s+'),
          );
          if (parts.length > 2) {
...
class MicrosoftEdgeDevice extends ChromiumDevice {
  ...
  Future<bool> _meetsVersionConstraint() async {
    final String rawVersion = (await sdkNameAndVersion).replaceFirst(
      'Microsoft Edge ',
      '',
    );
