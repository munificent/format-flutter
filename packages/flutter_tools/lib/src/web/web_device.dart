  ...
  Future<LaunchResult> startApp(
    ...
    if (launchChrome) {
      _chrome = await chromeLauncher.launch(
        url,
        cacheDir: _fileSystem.currentDirectory.childDirectory('.dart_tool').childDirectory('chrome-device'),
    ...
    if (_platform.isWindows) {
      if (_processManager.canRun('reg')) {
        final ProcessResult result = await _processManager
            .run(<String>[r'reg', 'query', r'HKEY_CURRENT_USER\Software\Google\Chrome\BLBeacon', '/v', 'version']);
...
class MicrosoftEdgeDevice extends ChromiumDevice {
  ...
  Future<String> _getSdkNameAndVersion() async {
    if (_processManager.canRun('reg')) {
      final ProcessResult result = await _processManager
          .run(<String>[r'reg', 'query', r'HKEY_CURRENT_USER\Software\Microsoft\Edge\BLBeacon', '/v', 'version']);
      if (result.exitCode == 0) {
        final List<String> parts = (result.stdout as String).split(RegExp(r'\s+'));
        if (parts.length > 2) {
...
class WebServerDevice extends Device {
  WebServerDevice({required Logger logger})
    : _logger = logger,
