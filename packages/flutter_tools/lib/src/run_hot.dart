...
class HotRunner extends ResidentRunner {
        ...
        startupTasks.add(device.generator!
            .recompile(
              ...
              dartPluginRegistrant:
                  FlutterProject.current().dartPluginRegistrant,
  ...
  Future<void> _launchFromDevFS() async {
    ...
    for (final FlutterDevice? device in flutterDevices) {
      ...
      futures.add(
        _launchInView(device, deviceEntryUri, deviceAssetsDirectoryUri),
      );
    }
    await Future.wait(futures);
  }
  ...
  Future<OperationResult> _restartFromSources({String? reason}) async {
          ...
          if ((isolate != null) && isPauseEvent(isolate.pauseEvent!.kind!)) {
            ...
            final List<Future<void>> breakpointAndExceptionRemoval =
              ...
              for (final vm_service.Breakpoint breakpoint
                ...
                device.vmService!.service.removeBreakpoint(
                  isolate.id!,
                  breakpoint.id!,
                ),
            ];
    ...
    restartTimer.stop();
    globals.printTrace(
      'Hot restart performed in ${getElapsedAsMilliseconds(
  ...
  static bool validateReloadReport(
    ...
    final ReloadReportContents contents = ReloadReportContents.fromReloadReport(
      reloadReport,
    );
  ...
  Future<OperationResult> restart({
    ...
    if (fullRestart) {
      ...
      if (!silent) {
        globals.printStatus(
          'Restarted application in ${getElapsedAsMilliseconds(
            timer.elapsed,
          )}.',
        );
      }
  ...
  Future<OperationResult> _fullRestartHelper({
    ...
    try {
      ...
      return OperationResult(
        1,
        'hot restart failed to complete: $err\n$st',
        fatal: true,
      );
  ...
  Future<OperationResult> _reloadSources({
    ...
    extraTimings.add(OperationResultExtraTiming(
      'compile',
      updatedDevFS.compileDuration.inMilliseconds,
    ));
    ...
    reloadVMTimer.stop();
    extraTimings.add(
      OperationResultExtraTiming('reload', reloadVMTimer.elapsedMilliseconds),
    );
    ...
    extraTimings.add(OperationResultExtraTiming(
      'reassemble',
      reassembleTimer.elapsedMilliseconds,
    ));
...
Future<OperationResult> _defaultReloadSourcesHelper(
  ...
  if (!HotRunner.validateReloadReport(reloadReport)) {
    ...
    return OperationResult(
      1,
      'Reload rejected: ${contents.notices.join("\n")}',
    );
  }
...
class ReassembleResult {
  ReassembleResult(
    this.reassembleViews,
    this.failedReassemble,
    this.shouldReportReloadTime,
  );
      ...
      for (final Uri uri in urisToScan) {
        waitList.add(pool.withResource<void>(
          ...
          () => (uri.hasScheme && uri.scheme != 'file'
                  ...
                  : _fileSystem.stat(
                      uri.toFilePath(windows: _platform.isWindows),
                    ))
              .then((FileStat stat) {
