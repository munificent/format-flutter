...
class FlutterManifest {
  ...
  @visibleForTesting
  static FlutterManifest? createFromString(
    String manifest, {
    required Logger logger,
  }) {
  ...
  List<Map<String, Object?>> get _rawFontsDescriptor {
    ...
    return fontList == null
        ...
        : fontList.map<Map<String, Object?>?>(
            castStringKeyedMap,
  ...
  List<Font> _extractFonts() {
    ...
    for (final Map<String, Object?> fontFamily in _rawFontsDescriptor) {
      ...
      if (fontFiles == null) {
        _logger.printWarning(
          'Warning: No fonts specified for font $familyName',
          emphasis: true,
        );
        continue;
      }
  ...
  late final List<Uri> models = kIs3dSceneSupported
      ? _extractAssetUris('models', 'Model')
      : <Uri>[];
  ...
  List<Uri> _extractAssetUris(String key, String singularName) {
    ...
    for (final Object? item in items) {
      if (item is! String || item == '') {
        _logger.printError(
          '$singularName manifest contains a null or empty uri.',
        );
        continue;
      }
      try {
        ...
        _logger.printError(
          '$singularName manifest contains invalid uri: $item.',
        );
      }
    }
    return results;
...
class Font {
  ...
  Map<String, Object?> get descriptor {
    return <String, Object?>{
      'family': familyName,
      'fonts': fontAssets.map<Map<String, Object?>>(
...
void _validateListType<T>(
  ...
  for (int i = 0; i < yamlList.length; i++) {
    if (yamlList[i] is! T) {
      ...
      errors.add(
        'Expected $context to be a list of $typeAlias, but element $i was a ${yamlList[
          i
        ].runtimeType}',
      );
    }
  ...
  if (yamlList != null && (yamlList is! YamlList || yamlList[0] is! YamlMap)) {
    ...
    for (int i = 0; i < yamlList.length; i++) {
      ...
      if (valueMap is! YamlMap) {
        // ignore: avoid_dynamic_calls
        errors.add(
          'Expected the $i element in "${kvp.key}" to be a map, but got ${yamlList[
            i
          ]} (${yamlList[i].runtimeType}).',
        );
        continue;
      }
      ...
      if (valueMap.containsKey('assets')) {
        ...
        if (assets is! YamlList) {
          ...
          _validateListType<String>(
            assets,
            errors,
            '"assets" key in the $i element of "${kvp.key}"',
            'file paths',
          );
        }
      }
