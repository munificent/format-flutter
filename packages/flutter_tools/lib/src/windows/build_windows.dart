...
Future<void> buildWindows(
  ...
  final bool badPath = _kBadCharacters.runes.any((int i) => projectPath.contains(String.fromCharCode(i)));
  if (badPath) {
    throwToolExit(
  ...
  final File appFile = buildDirectory.childDirectory('runner').childDirectory(sentenceCase(buildModeName)).childFile(
    '$binaryName.exe',
  );
  ...
  if (buildInfo.codeSizeDirectory != null && sizeAnalyzer != null) {
    ...
    globals.printStatus('A summary of your Windows bundle analysis can be found at: ${outputFile.path}');
...
Future<void> _runCmakeGeneration({
  ...
  int result;
  try {
    result = await globals.processUtils
        .stream(<String>[cmakePath, '-S', sourceDir.path, '-B', buildDir.path, '-G', generator], trace: true);
  } on ArgumentError {
...
Future<void> _runBuild(String cmakePath, Directory buildDir, String buildModeName, {bool install = true}) async {
  final Stopwatch sw = Stopwatch()..start();
...
void _writeGeneratedFlutterConfig(WindowsProject windowsProject, BuildInfo buildInfo, String? target) {
...
void _fixBrokenCmakeGeneration(Directory buildDirectory) {
  final File assembleProject = buildDirectory.childDirectory('flutter').childFile('flutter_assemble.vcxproj');
  if (assembleProject.existsSync()) {
    ...
    final RegExp assembleCallRegex = RegExp(r'^.*/tool_backend\.bat windows[^ ]* (Debug|Profile|Release)');
      ...
      if (commandMatch != null) {
        ...
        if (assembleCallMatch != null) {
          ...
          if (callConfig != lastCommandConditionConfig) {
            ...
            final String correctedLine = line.replaceFirst(callConfig, lastCommandConditionConfig, badConfigIndex);
