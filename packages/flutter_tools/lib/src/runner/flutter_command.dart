...
abstract class FlutterCommand extends Command<void> {
  ...
  DeviceConnectionInterface? get deviceConnectionInterface {
    if ((argResults?.options.contains(FlutterOptions.kDeviceConnection) ??
      ...
      final String? connectionType = stringArg(
        FlutterOptions.kDeviceConnection,
      );
  ...
  Future<BuildInfo> getBuildInfo({
    ...
    final String? performanceMeasurementFile = argParser.options.containsKey(
            FlutterOptions.kPerformanceMeasurementFile,
          )
    ...
    List<String> dartDefines = extractDartDefines(
    ...
    if (argParser.options.containsKey(FlutterOptions.kWebRendererFlag)) {
      webRenderer = WebRendererMode.values.byName(
    ...
    return BuildInfo(
      ...
      initializeFromDill:
          argParser.options.containsKey(FlutterOptions.kInitializeFromDill)
              ...
              : null,
      assumeInitializeFromDillUpToDate: argParser.options.containsKey(
            FlutterOptions.kAssumeInitializeFromDillUpToDate,
          ) &&
  ...
  static List<String> updateDartDefines(
    ...
    if (!dartDefines.any(
          (String d) => d.startsWith('FLUTTER_WEB_AUTO_DETECT='),
        ) &&
        dartDefines.any((String d) => d.startsWith('FLUTTER_WEB_USE_SKIA='))) {
      dartDefinesSet.removeWhere(
        (String d) => d.startsWith('FLUTTER_WEB_USE_SKIA='),
      );
    }
