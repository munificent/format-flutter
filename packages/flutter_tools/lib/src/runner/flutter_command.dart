...
abstract class FlutterCommand extends Command<void> {
  ...
  void usesFilesystemOptions({required bool hide}) {
    argParser
      ..addOption('output-dill', hide: hide, help: 'Specify the path to frontend server output kernel file.')
      ..addMultiOption(
  ...
  int _tryParseHostVmservicePort() {
    final String? vmServicePort = stringArg(vmServicePortOption) ?? stringArg(observatoryPortOption);
  ...
  BuildMode getBuildMode() {
    ...
    final List<bool> modeFlags = <bool>[debugResult, jitReleaseResult, boolArg('profile'), releaseResult];
  ...
  Future<BuildInfo> getBuildInfo({BuildMode? forcedBuildMode, File? forcedTargetFile}) async {
    ...
    final bool treeShakeIcons =
        argParser.options.containsKey('tree-shake-icons') && buildMode.isPrecompiled && boolArg('tree-shake-icons');
    ...
    if (argParser.options.containsKey(FlutterOptions.kDartDefineFromFileOption)) {
      ...
      for (final String path in configJsonPaths) {
        if (!globals.fs.isFileSync(path)) {
          throwToolExit(
            'Json config define file "--${FlutterOptions.kDartDefineFromFileOption}=$path" is not a file, '
            'please fix first!',
          );
        }
  ...
  void _registerSignalHandlers(String commandPath, DateTime startTime) {
    void handler(io.ProcessSignal s) {
      ...
      _sendPostUsage(commandPath, const FlutterCommandResult(ExitStatus.killed), startTime, globals.systemClock.now());
    }
  ...
  void _sendPostUsage(String commandPath, FlutterCommandResult commandResult, DateTime startTime, DateTime endTime) {
    // Send command result.
    ...
    final String label = labels.where((String? label) => label != null && !_isBlank(label)).join('-');
...
''');
    ...
    if (refreshWirelessDevices) {
      ...
      _targetDevices.startExtendedWirelessDeviceDiscovery(deviceDiscoveryTimeout: deviceDiscoveryTimeout);
    }
    ...
    if (shouldRunPub) {
      ...
      await generateLocalizationsSyntheticPackage(environment: environment, buildSystem: globals.buildSystem);

      await pub.get(context: PubContext.getVerifyContext(name), project: project, checkUpToDate: cachePubGet);
    ...
    if (commandPath != null) {
      Usage.command(
        commandPath,
        parameters: CustomDimensions(commandHasTerminal: globals.stdio.hasTerminal).merge(await usageValues),
      );
    }
  ...
  Future<List<Device>?> findAllTargetDevices({bool includeDevicesUnsupportedByProject = false}) async {
  ...
  Future<Device?> findTargetDevice({bool includeDevicesUnsupportedByProject = false}) async {
    List<Device>? deviceList =
        await findAllTargetDevices(includeDevicesUnsupportedByProject: includeDevicesUnsupportedByProject);
    if (deviceList == null) {
      return null;
    }
...
mixin DeviceBasedDevelopmentArtifacts on FlutterCommand {
  ...
  Future<Set<DevelopmentArtifact>> get requiredArtifacts async {
    ...
    final List<Device> devices =
        await globals.deviceManager!.getDevices(filter: DeviceDiscoveryFilter(excludeDisconnected: false));
    if (devices.isEmpty) {
