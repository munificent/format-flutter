...
class FlutterCommandRunner extends CommandRunner<void> {
  FlutterCommandRunner({
    ...
    argParser.addFlag(FlutterGlobalOptions.kVersionFlag, negatable: false, help: 'Reports the version of this tool.');
    argParser.addFlag(
  ...
  Future<void> runCommand(ArgResults topLevelResults) async {
    ...
    if (engineBuildPaths != null) {
      contextOverrides.addAll(<Type, Object?>{Artifacts: Artifacts.getLocalEngine(engineBuildPaths)});
    }
    ...
    await context.run<void>(
      ...
      body: () async {
        ...
        final bool redirectedCompletion =
        ...
        if ((topLevelResults[FlutterGlobalOptions.kVersionFlag] as bool?) ?? false) {
          ...
          final FlutterVersion version = globals.flutterVersion.fetchTagsAndGetVersion(clock: globals.systemClock);
          final String status;
          if (machineFlag) {
  ...
  List<Directory> getRepoPackages() {
    return getRepoRoots().expand<String>((String root) => _gatherProjectPaths(root)).map<Directory>(
      (String dir) => globals.fs.directory(dir),
    ).toList();
  }
  ...
  static List<String> _gatherProjectPaths(String rootPath) {
    ...
    final List<String> projectPaths =
        globals.fs.directory(rootPath).listSync(followLinks: false).expand((FileSystemEntity entity) {
