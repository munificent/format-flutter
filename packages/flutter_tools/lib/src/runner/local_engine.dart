...
class LocalEngineLocator {
  ...
  Future<EngineBuildPaths?> findEnginePath({
    ...
    if (engineSourcePath == null) {
      ...
      engineSourcePath ??= _tryEnginePath(_fileSystem.path.join(
        _fileSystem.directory(_flutterRoot).parent.path,
        'engine',
        'src',
      ));
    }
    ...
    if (localEngine != null || localWebSdk != null) {
      throwToolExit(_userMessages.runnerNoEngineSrcDir(
  ...
  Future<String?> _findEngineSourceByPackageConfig(String? packagePath) async {
    final PackageConfig packageConfig =
        await loadPackageConfigWithLogging(_fileSystem.file(
    ...
    if (engineUriPath != null) {
      engineSourcePath = _fileSystem.directory(
        engineUriPath,
  ...
  EngineBuildPaths _findEngineBuildPath(
    ...
    if (localEngine != null) {
      engineBuildPath = _fileSystem.path.normalize(
        _fileSystem.path.join(enginePath, 'out', localEngine),
      );
      if (!_fileSystem.isDirectorySync(engineBuildPath)) {
        throwToolExit(
          _userMessages.runnerNoEngineBuild(engineBuildPath),
          exitCode: 2,
        );
      }
      ...
      engineHostBuildPath = _fileSystem.path.normalize(_fileSystem.path.join(
        _fileSystem.path.dirname(engineBuildPath),
        hostBasename,
      ));
      if (!_fileSystem.isDirectorySync(engineHostBuildPath)) {
        throwToolExit(
          _userMessages.runnerNoEngineBuild(engineHostBuildPath),
          exitCode: 2,
        );
      }
    }
    ...
    if (localWebSdk != null) {
      webSdkPath = _fileSystem.path.normalize(
        _fileSystem.path.join(enginePath, 'out', localWebSdk),
      );
