...
class DriveCommand extends RunCommandBase {
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    final PackageConfig packageConfig =
        await loadPackageConfigWithLogging(_fileSystem.file('.packages'), logger: _logger, throwOnError: false);
    final DriverService driverService = _flutterDriverFactory!.createDriverService(web);
    final BuildInfo buildInfo = await getBuildInfo();
    final DebuggingOptions debuggingOptions = await createDebuggingOptions(web);
    final File? applicationBinary = applicationBinaryPath == null ? null : _fileSystem.file(applicationBinaryPath);
    ...
    try {
      if (stringArg('use-existing-app') == null) {
        ...
        await driverService.reuseApplication(uri, device, debuggingOptions, ipv6 ?? false);
      }
