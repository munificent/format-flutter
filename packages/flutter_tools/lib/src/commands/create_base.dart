...
abstract class CreateBase extends FlutterCommand {
  ...
  FlutterProjectType? determineTemplateType() {
    ...
    bool exists(List<String> path) {
      return globals.fs.directory(globals.fs.path
          .joinAll(<String>[projectDir.absolute.path, ...path])).existsSync();
    }
  ...
  void validateProjectDir({bool overwrite = false}) {
    ...
    switch (type) {
      ...
      case FileSystemEntityType.file:
        ...
        throwToolExit(
          "Invalid project name: '$projectDirPath' - file exists.",
          exitCode: 2,
        );
  ...
  Map<String, Object?> createTemplateContext({
    ...
    final String appleIdentifier = createUTIIdentifier(
      organization,
      projectName,
    );
    final String androidIdentifier = createAndroidIdentifier(
      organization,
      projectName,
    );
    final String windowsIdentifier = createWindowsIdentifier(
      organization,
      projectName,
    );
    ...
    return <String, Object?>{
      ...
      'flutterChannel': escapeYamlString(
        globals.flutterVersion.getBranchName(),
      ), // may contain PII
      'ios': ios,
      'android': android,
      'web': web,
  ...
  static String createAndroidIdentifier(String organization, String name) {
    ...
    final List<String> segments = tmpIdentifier.split('.').where(
  ...
  Set<Uri> _computeTemplateManifest() {
    ...
    return Set<Uri>.from((manifest['files']! as List<Object?>)
        .cast<String>()
        .map<Uri>(
          (String path) => Uri.file(
            globals.fs.path.join(flutterToolsAbsolutePath, path),
          ),
        ));
  }
