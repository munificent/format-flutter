...
class BuildIOSArchiveCommand extends _BuildIOSSubCommand {
  ...
  Map<_ImageAssetFileKey, String> _parseImageAssetContentsJson(
    ...
    final Directory contentsJsonDirectory = globals.fs.directory(
      contentsJsonDirName,
    );
    if (!contentsJsonDirectory.existsSync()) {
      return <_ImageAssetFileKey, String>{};
    }
    final File contentsJsonFile = contentsJsonDirectory.childFile(
      'Contents.json',
    );
    final Map<String, dynamic> contents = json.decode(
          contentsJsonFile.readAsStringSync(),
    ...
    for (final dynamic image in images) {
      ...
      if (size != null) {
        ...
        final Iterable<double> parsedSizes = size.split('x').map(
          (String element) => double.tryParse(element),
        ).whereType<double>();
      ...
      // for example, "3x".
      final Iterable<int> parsedScales = scale.split('x').map(
        (String element) => int.tryParse(element),
      ).whereType<int>();
  ...
  bool _isAssetStillUsingTemplateFiles({
    ...
    return projectImageInfoMap.entries.any(
      (MapEntry<_ImageAssetFileKey, String> entry) {
  ...
  List<String> _imageFilesWithWrongSize({
    ...
    return imageInfoMap.entries.where(
      (MapEntry<_ImageAssetFileKey, String> entry) {
        final String fileName = entry.value;
        final File imageFile = globals.fs.file(
          globals.fs.path.join(imageDirName, fileName),
        );
  ...
  _validateXcodeBuildSettingsAfterArchive() async {
    ...
    final bool usesDefaultBundleIdentifier =
        xcodeProjectSettingsMap['Bundle Identifier']?.startsWith(
              'com.example',
            ) ??
            false;
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    final String absoluteOutputPath = globals.fs.path.absolute(
      relativeOutputPath,
    );
    final String absoluteArchivePath = globals.fs.path.absolute(
    ...
    try {
      final String exportMethodDisplayName = isAppStoreUpload
          ? 'App Store'
          : exportMethod;
      status = globals.logger.startProgress(
        'Building $exportMethodDisplayName IPA...',
      );
...
abstract class _BuildIOSSubCommand extends BuildSubCommand {
  ...
  @override
  Future<Set<DevelopmentArtifact>> get requiredArtifacts async => const <
    DevelopmentArtifact
  ...
  late final Future<BuildableIOSApp> buildableIOSApp = () async {
    final BuildableIOSApp? app = await applicationPackages
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    final String logTarget = environmentType == EnvironmentType.simulator
        ? 'simulator'
        : 'device';
    ...
    if (buildInfo.codeSizeDirectory != null) {
      ...
      final Directory outputAppDirectoryCandidate = _outputAppDirectory(
        resultOutput,
      );
      ...
      final File outputFile = globals.fsUtils.getUniqueFile(
        globals.fs.directory(globals.fsUtils.homeDirPath).childDirectory(
          '.flutter-devtools',
        ),
