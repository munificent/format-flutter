...
abstract class RunCommandBase extends FlutterCommand
  ...
  ImpellerStatus get enableImpeller => ImpellerStatus.fromBool(
    argResults!['enable-impeller'] as bool?,
  );
...
class RunCommand extends RunCommandBase {
  ...
  Future<String?> get usagePath async {
    ...
    return '$command/${getNameForTargetPlatform(
      await devices![0].targetPlatform,
    )}';
  }
    ...
    if (anyIOSDevices) {
      ...
      if (iosProject.exists) {
        final Iterable<File> swiftFiles = iosProject.hostAppRoot
            ...
            .whereType<File>()
            .where(
              (File file) => globals.fs.path.extension(file.path) == '.swift',
            );
  ...
  Daemon createMachineDaemon() {
    final Daemon daemon = Daemon(
      ...
      notifyingLogger: (globals.logger is NotifyingLogger)
          ...
          : NotifyingLogger(
              verbose: globals.logger.isVerbose,
              parent: globals.logger,
            ),
      logToStdout: true,
    );
    return daemon;
