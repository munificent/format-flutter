...
abstract class RunCommandBase extends FlutterCommand with DeviceBasedDevelopmentArtifacts {
  RunCommandBase({required bool verboseHelp}) {
    ...
    argParser
      ...
      ..addFlag('verbose-system-logs', negatable: false, help: 'Include verbose logging from the Flutter engine.')
      ..addFlag(
        'cache-sksl',
        negatable: false,
    ...
    if (buildInfo.mode.isRelease) {
      return DebuggingOptions.disabled(
        ...
        webUseSseForInjectedClient:
            featureFlags.isWebEnabled && stringArg('web-server-debug-injected-client-protocol') == 'sse',
      ...
      return DebuggingOptions.enabled(
        ...
        enableSoftwareRendering:
            argParser.options.containsKey('enable-software-rendering') && boolArg('enable-software-rendering'),
        skiaDeterministicRendering:
            argParser.options.containsKey('skia-deterministic-rendering') && boolArg('skia-deterministic-rendering'),
        ...
        fastStart:
            argParser.options.containsKey('fast-start') && boolArg('fast-start') && !runningWithPrebuiltApplication,
...
class RunCommand extends RunCommandBase {
  ...
  Future<void> validateCommand() async {
    ...
    if (userIdentifier != null && devices!.every((Device device) => device.platformType != PlatformType.android)) {
      throwToolExit(
  ...
  Future<ResidentRunner> createRunner({
    ...
    if (hotMode && !webMode) {
      return HotRunner(
        ...
        applicationBinary: applicationBinaryPath == null ? null : globals.fs.file(applicationBinaryPath),
