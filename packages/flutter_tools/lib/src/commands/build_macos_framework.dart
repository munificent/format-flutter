...
class BuildMacOSFrameworkCommand extends BuildFrameworkCommand {
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    if (hasPlugins(project)) {
      ...
      globals.printStatus(
        '\nCopy ${globals.fs.path.basename(
          pluginRegistrantImplementation.path,
        )} into your project.',
      );
    }
...
''';
  ...
  Future<void> _producePlugins(
    ...
    try {
      ...
      final Directory buildConfiguration = buildOutput.childDirectory(
        xcodeBuildConfiguration,
      );

      final Iterable<Directory> products = buildConfiguration.listSync(
        followLinks: false,
      ).whereType<Directory>();
