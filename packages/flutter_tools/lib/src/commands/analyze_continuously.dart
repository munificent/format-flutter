...
class AnalyzeContinuously extends AnalyzeBase {
  ...
  void _handleAnalysisStatus(AnalysisServer server, bool isAnalyzing) {
    if (isAnalyzing) {
      ...
      analysisErrors.removeWhere(
        (String path, _) => pathsToRemove.contains(path),
      );
      ...
      dumpErrors(sortedErrors.map<String>(
        (AnalysisError error) => error.toLegacyString(),
      ));

      final int issueCount = sortedErrors.length;
      final int issueDiff = issueCount - lastErrorCount;
      lastErrorCount = issueCount;
      final String seconds = (analysisTimer.elapsedMilliseconds / 1000.0)
          .toStringAsFixed(2);
