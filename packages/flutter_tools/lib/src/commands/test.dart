...
class TestCommand extends FlutterCommand with DeviceBasedDevelopmentArtifacts {
  ...
  Future<FlutterCommandResult> verifyThenRunCommand(String? commandPath) {
    ...
    if (testUris.isEmpty) {
      ...
      for (final Uri uri in testUris) {
        ...
        if (globals.fs.isDirectorySync(testPath)) {
          _testFileUris.addAll(
            _findTests(globals.fs.directory(testPath)).map(Uri.file),
          );
        } else {
    ...
    final List<String> testFilePaths = _testFileUris.map(
      (Uri uri) => uri.replace(query: '').toFilePath(),
    ).toList();
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    if (buildTestAssets) {
      ...
      testAssetDirectory = globals.fs.path.join(
    ...
    int? jobs = concurrencyString == null
        ? null
        : int.tryParse(concurrencyString);
    ...
    testTimeRecorder?.stop(
      TestTimePhases.TestRunner,
      testRunnerTimeRecorderStopwatch!,
    );
  ...
  Uri _parseTestArgument(String arg) {
    ...
    final String queryPart = queryStart == -1
        ? ''
        : arg.substring(queryStart + 1);

    filePart = globals.fs.path.absolute(filePart);
    filePart = globals.fs.path.normalize(filePart);

    return Uri.file(filePart).replace(
