...
class CreateCommand extends CreateBase {
  CreateCommand({super.verboseHelp = false}) {
    addPlatformsOptions(customHelp: kPlatformHelp);
    argParser.addOption(
      'template',
      abbr: 't',
      allowed: FlutterProjectType.values.map<String>(
        (FlutterProjectType e) => e.cliName,
      ),
  ...
  Future<void> _writeSamplesJson(String outputFilePath) async {
    try {
      ...
      throwToolExit(
        'Failed to write samples JSON to "$outputFilePath": $e',
        exitCode: 2,
      );
    }
  }
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    if (generatePackage) {
      final String relativeMainPath = globals.fs.path.normalize(globals.fs.path
          .join(
            relativeDirPath,
            'lib',
      ...
      final List<String> platformsToWarn = _getPlatformWarningList(
        requestedPlatforms,
      );
      if (platformsToWarn.isNotEmpty) {
        _printWarningDisabledPlatform(platformsToWarn);
      }
      final String template = generateMethodChannelsPlugin
          ? 'plugin'
          : 'plugin_ffi';
...
''');
  ...
  void _applySample(Directory directory, String sampleCode) {
    final File mainDartFile = directory.childDirectory('lib').childFile(
      'main.dart',
    );
...
List<String> _getSupportedPlatformsInPlugin(Directory projectDir) {
  final String pubspecPath = globals.fs.path.join(
...
void _printPluginDirectoryLocationMessage(
  ...
  final String relativePluginMain = globals.fs.path.join(
    pluginPath,
    'lib',
    '$projectName.dart',
  );
...
void _printWarningDisabledPlatform(List<String> platforms) {
  ...
  if (desktop.isNotEmpty) {
    ...
    globals.printStatus('''
The desktop $platforms: ${desktop.join(
      ', ',
    )} $verb currently not supported on your local environment.
