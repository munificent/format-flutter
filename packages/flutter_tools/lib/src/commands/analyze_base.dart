...
class PackageDependencyTracker {
  ...
  void addDependenciesFromPackagesFileIn(Directory directory) {
    final String dotPackagesPath = globals.fs.path.join(
      directory.path,
      '.packages',
    );
    ...
    if (dotPackages.existsSync()) {
      ...
      for (final String line in lines) {
        ...
        if (colon > 0) {
          ...
          final String packagePath = globals.fs.path.fromUri(
          ...
          if (!_vendedSdkPackages.contains(packageName) ||
            ...
            add(packageName, globals.fs.path.normalize(
  ...
  void addCanonicalCase(
    ...
    getPackageDependency(packageName).addCanonicalCase(
      packagePath,
      pubSpecYamlPath,
    );
  }
  ...
  void checkForConflictingDependencies(
    ...
    for (final Directory directory in pubSpecDirectories) {
      final String pubSpecYamlPath = globals.fs.path.join(
        directory.path,
        'pubspec.yaml',
      );
  ...
  bool get hasConflicts {
    return packages.values.any(
      (PackageDependency dependency) => dependency.hasConflict,
    );
  }
...
Set<String> findDirectories(ArgResults argResults, FileSystem fileSystem) {
  final Set<String> items = Set<String>.of(argResults.rest.map<String>(
    (String path) => fileSystem.path.canonicalize(path),
  ));
  if (items.isNotEmpty) {
