...
class DowngradeCommand extends FlutterCommand {
  DowngradeCommand({
    ...
    argParser.addFlag('prompt', defaultsTo: true, hide: !verboseHelp, help: 'Show the downgrade prompt.');
  }
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    final RunResult parseResult = await processUtils
        .run(<String>['git', 'describe', '--tags', lastFlutterVersion], workingDirectory: workingDirectory);
    ...
    if (stdio.hasTerminal && boolArg('prompt')) {
      ...
      final String result = await terminal.promptForCharInput(const <String>[
        'y',
        'n',
      ], prompt: 'Downgrade flutter to version $humanReadableVersion?', logger: _logger);
      if (result == 'n') {
  ...
  Future<String> _createErrorMessage(String workingDirectory, Channel currentChannel) async {
    ...
    for (final Channel channel in Channel.values) {
      ...
      final RunResult parseResult = await _processUtils!
          .run(<String>['git', 'describe', '--tags', sha], workingDirectory: workingDirectory);
      if (parseResult.exitCode == 0) {
