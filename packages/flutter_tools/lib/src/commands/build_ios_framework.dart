...
abstract class BuildFrameworkCommand extends BuildSubCommand {
  ...
  static Future<void> produceXCFramework(
    ...
    final ProcessResult xcframeworkResult =
        await processManager.run(xcframeworkCommand);
...
class BuildIOSFrameworkCommand extends BuildFrameworkCommand {
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    for (final BuildInfo buildInfo in buildInfos) {
      ...
      final String xcodeBuildConfiguration = sentenceCase(
        buildInfo.mode.cliName,
      );
      final Directory modeDirectory = outputDirectory.childDirectory(
    ...
    if (!project.isModule && hasPlugins(project)) {
      ...
      globals.printStatus(
        '\nCopy the ${globals.fs.path.basenameWithoutExtension(
          pluginRegistrantHeader.path,
...
''';
  ...
  Future<void> _producePlugins(
    ...
    try {
      ...
      final Directory iPhoneBuildConfiguration = iPhoneBuildOutput
          .childDirectory('$xcodeBuildConfiguration-iphoneos');
      final Directory simulatorBuildConfiguration = simulatorBuildOutput
          .childDirectory('$simulatorConfiguration-iphonesimulator');

      final Iterable<Directory> products = iPhoneBuildConfiguration.listSync(
        followLinks: false,
      ).whereType<Directory>();
