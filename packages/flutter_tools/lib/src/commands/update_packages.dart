...
class UpdatePackagesCommand extends FlutterCommand {
  ...
  late final Directory _syntheticPackageDir = (() {
    ...
    if (optionPath == null) {
      return globals.fs.systemTempDirectory.createTempSync(
      ...
      packages.retainWhere((Directory directory) {
        return consumerPackages.any((String package) {
          return directory.path.endsWith(
            'packages${globals.fs.path.separator}$package',
          );
        });
      });
    }
    ...
    if (cherryPickPackage != null) {
      ...
      for (final String package in allDependencies.keys) {
        if (package == cherryPickPackage) {
          ...
          adjustedDependencies.add(
            pubspec.copyWith(version: cherryPickVersion),
          );
        } else {
    ...
    try {
      ...
      for (final Directory dir in packages) {
        unawaited(queue.add(() async {
          ...
          final String relativeDir = globals.fs.path.relative(
            dir.path,
            from: Cache.flutterRoot,
          );
          globals.printStatus(
...
class PubspecYaml {
  ...
  static PubspecYaml _parse(File file, List<String> lines) {
    ...
    for (int index = 0; index < lines.length; index += 1) {
      ...
      if (lastDependency == null) {
        ...
        final PubspecHeader? header = PubspecHeader.parse(
          line,
  ...
  void apply(PubDependencyTree versions, Set<String> specialDependencies) {
        ...
        switch (section) {
          ...
          case Section.dependencies:
            ...
            if (!data.isTransitive) {
              ...
              assert(
                !directDependencies.contains(data.name) &&
              ...
              if (data.kind == DependencyKind.normal) {
                ...
                assert(
                  versions.contains(data.name),
                  "versions doesn't contain ${data.name}",
                );
                output.add(
                  '  ${data.name}: ${versions.versionFor(
                    data.name,
                  )}${data.suffix}',
                );
              } else {
    ...
    for (final String package in directDependencies) {
      transitiveDependencies.addAll(versions.getTransitiveDependenciesFor(
        package,
        seen: done,
        exclude: implied,
      ));
    ...
    for (final String package in devDependencies) {
      transitiveDevDependencies.addAll(versions.getTransitiveDependenciesFor(
        package,
        seen: done,
        exclude: implied,
      ));
    ...
    String computeTransitiveDependencyLineFor(String package) {
      return '  $package: ${versions.versionFor(
        package,
    ...
    for (final String package in transitiveDependenciesAsList) {
      transitiveDependencyOutput.add(
        computeTransitiveDependencyLineFor(package),
      );
    }
    ...
    final String checksumString = _computeChecksum(
  ...
  static PubspecDependency? parse(
    ...
    if (colonIndex != -1) {
      version = line.substring(
        colonIndex + 1,
        hashIndex != -1 ? hashIndex : line.length,
      ).trim();
    }
...
String generateFakePubspec(
  ...
  for (final PubspecDependency dependency in dependencies) {
    if (!dependency.pointsToSdk) {
      dependency.describeForFakePubspec(
        result,
        overrides,
    ...
    if (message.startsWith('- ')) {
      ...
      if (!contains(package)) {
        ...
        if (space3 < message.length) {
          ...
          final String allDependencies = message.substring(
            space3 + 2,
            message.length - 1,
          );
...
String _computeChecksum(
  ...
  String Function(String name) getVersion,
) {
  int lowerCheck = 0;
  int upperCheck = 0;
...
Directory createTemporaryFlutterSdk(
  ...
  realFlutter.childFile('version').copySync(
    directory.childFile('version').path,
  );
  ...
  for (final String flutterPackage in currentPackages) {
    final File pubspecFile = packages.childDirectory(flutterPackage).childFile(
      'pubspec.yaml',
    )..createSync(recursive: true);
