...
abstract class FlutterVersion {
  ...
  FlutterVersion._({required SystemClock clock, required this.flutterRoot, required this.fs}) : _clock = clock;
  ...
  Future<DateTime?> _getLatestAvailableFlutterDate() async {
    ...
    if (versionCheckStamp.lastTimeVersionWasChecked != null) {
      final Duration timeSinceLastCheck = now.difference(versionCheckStamp.lastTimeVersionWasChecked!);
    ...
    try {
      final DateTime remoteFrameworkCommitDate = DateTime.parse(await fetchRemoteFrameworkCommitDate());
      await versionCheckStamp.store(newTimeVersionWasChecked: now, newKnownRemoteVersion: remoteFrameworkCommitDate);
...
String _gitCommitDate({String gitRef = 'HEAD', bool lenient = false, required String? workingDirectory}) {
  final List<String> args = FlutterVersion.gitLog(<String>[gitRef, '-n', '1', '--pretty=format:%ad', '--date=iso']);
  try {
...
class _FlutterVersionGit extends FlutterVersion {
  ...
  String? get repositoryUrl {
    if (_repositoryUrl == null) {
      ...
      if (slash != -1) {
        ...
        _repositoryUrl = _runGit('git ls-remote --get-url $remote', globals.processUtils, flutterRoot);
      }
    }
  ...
  String get channel {
    ...
    assert(
      kOfficialChannels.contains(channel) || kObsoleteBranches.containsKey(channel) || channel == kUserBranch,
...
String _runSync(List<String> command, {bool lenient = true, required String? workingDirectory}) {
  final ProcessResult results = globals.processManager.runSync(command, workingDirectory: workingDirectory);
...
class GitTagVersion {
  ...
  static GitTagVersion determine(
    ...
    return parse(_runGit('git describe --match *.*.* --long --tags $gitRef', processUtils, workingDirectory));
  }
