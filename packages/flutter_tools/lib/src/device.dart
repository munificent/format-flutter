...
abstract class DeviceManager {
  ...
  Future<List<Device>> getDevicesById(
    ...
    final Set<String> wellKnownIds = _platformDiscoverers.expand(
      (DeviceDiscovery discovery) => discovery.wellKnownIds,
    ).toSet();
  ...
  Iterable<DeviceDiscovery> get _platformDiscoverers {
    return deviceDiscoverers.where(
      (DeviceDiscovery discoverer) => discoverer.supportsPlatform,
    );
  }
  ...
  Future<List<Device>> getAllDevices({DeviceDiscoveryFilter? filter}) async {
    filter ??= DeviceDiscoveryFilter();
    final List<List<Device>> devices = await Future.wait<List<Device>>(
      <Future<List<Device>>>[
        for (final DeviceDiscovery discoverer in _platformDiscoverers)
  ...
  bool get canListAnything {
    return _platformDiscoverers.any(
      (DeviceDiscovery discoverer) => discoverer.canListAnything,
    );
  }
...
abstract class DeviceDiscovery {
  ...
  Future<List<String>> getDiagnostics() => Future<List<String>>.value(
    <String>[],
  );
...
abstract class Device {
  ...
  Future<void> takeScreenshot(File outputFile) => Future<void>.error(
    'unimplemented',
  );
  ...
  static Future<List<String>> descriptions(List<Device> devices) async {
    ...
    for (final Device device in devices) {
      ...
      if (await device.isLocalEmulator) {
        final String type = targetPlatform == TargetPlatform.ios
            ? 'simulator'
            : 'emulator';
    ...
    for (final List<String> row in table) {
      widths = indices.map<int>(
        (int i) => math.max(widths[i], row[i].length),
      ).toList();
    }
    ...
    return <String>[
      for (final List<String> row in table)
        indices.map<String>((int i) => row[i].padRight(widths[i])).followedBy(
          <String>[row.last],
        ).join(' • '),
    ];
  }
  ...
  static List<String> devicesPlatformTypes(List<Device> devices) {
    return devices.map((Device d) => d.platformType.toString()).toSet().toList()
      ..sort();
  }
