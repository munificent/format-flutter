...
class FlutterDriverFactory {
  ...
  DriverService createDriverService(bool web) {
    if (web) {
      return WebDriverService(logger: _logger, processUtils: _processUtils, dartSdkPath: _dartSdkPath);
    }
...
abstract class DriverService {
  ...
  Future<void> reuseApplication(Uri vmServiceUri, Device device, DebuggingOptions debuggingOptions, bool ipv6);
...
class FlutterDriverService extends DriverService {
  ...
  Future<void> start(
    ...
    return reuseApplication(result.vmServiceUri!, device, debuggingOptions, ipv6);
  }
