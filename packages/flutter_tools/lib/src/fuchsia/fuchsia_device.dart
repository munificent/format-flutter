...
class _FuchsiaLogReader extends DeviceLogReader {
  ...
  Stream<String>? _processLogs(Stream<String>? lines) {
    ...
    return Stream<String>.eventTransformed(
      lines,
      (EventSink<String> output) => _FuchsiaLogSink(
        output,
        matchRegExp,
        startTime,
      ),
...
class FuchsiaDevice extends Device {
  ...
  Future<LaunchResult> startApp(
    ...
    try {
      ...
      final File flutterRunnerArchive = globals.fs.file(globals.artifacts!
          .getArtifactPath(
      ...
      if (!await fuchsiaDeviceTools.pkgctl.resolve(
        this,
        fuchsiaPackageServer.name,
        flutterRunnerName,
      )) {
        globals.printError(
          'Failed to get pkgctl to prefetch the flutter_runner',
        );
        return LaunchResult.failed();
      }
...
class _FuchsiaPortForwarder extends DevicePortForwarder {
  ...
  @override
  Future<void> dispose() async {
    final List<ForwardedPort> forwardedPortsCopy = List<ForwardedPort>.of(
      forwardedPorts,
    );
