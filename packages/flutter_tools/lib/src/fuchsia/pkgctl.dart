...
class FuchsiaPkgctl {
  ...
  Future<bool> addRepo(FuchsiaDevice device, FuchsiaPackageServer server) async {
    ...
    final RunResult result = await device.shell('pkgctl repo add url -n ${server.name} $configUrl');
  ...
  Future<bool> rmRepo(FuchsiaDevice device, FuchsiaPackageServer server) async {
    final RunResult result = await device.shell('pkgctl repo rm fuchsia-pkg://${server.name}');
    return result.exitCode == 0;
  }
