...
void main(List<String> args) {
  runInContext<void>(
    () => run(args),
    overrides: <Type, Generator>{Usage: () => DisabledUsage()},
  );
}
...
Future<void> run(List<String> args) async {
  ...
  if (_kRequiredOptions.any(
    (String option) => !argResults.options.contains(option),
  )) {
  ...
  try {
    ...
    final Directory sdkRootSrc = globals.fs.directory(
      argResults[_kOptionSdkRoot],
    );
    ...
    final Link testerDestLink = globals.fs.link(
    ...
    for (final FileSystemEntity artifact in sdkRootSrc.listSync()) {
      globals.fs.link(sdkRootDest.childFile(artifact.basename).path).createSync(
        artifact.path,
      );
    }
    // TODO(tvolkert): Remove once flutter_tester no longer looks for this.
    globals.fs.link(sdkRootDest.childFile('platform.dill').path).createSync(
    ...
    final List<Map<String, dynamic>> jsonList = List<Map<String, dynamic>>.from(
      (json.decode(
            globals.fs.file(argResults[_kOptionTests]).readAsStringSync(),
          ) as List<dynamic>)
