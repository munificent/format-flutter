...
class Context {
  Context({
    ...
    if (scriptOutputStreamFile != null) {
      scriptOutputStream = scriptOutputStreamFile.openSync(
        mode: FileMode.write,
      );
    }
  }
  ...
  String parseFlutterBuildMode() {
    ...
    echoError(
      '.xcconfig file for the current build configuration (${environment[
        'CONFIGURATION'
      ]}).',
    );
    echoError(
  ...
  void embedFlutterFrameworks() {
    ...
    final String xcodeFrameworksDir = '${environment[
      'TARGET_BUILD_DIR'
    ]}/${environment['FRAMEWORKS_FOLDER_PATH']}';
  ...
  void addVmServiceBonjourService() {
    ...
    final String builtProductsPlist = '${environment[
          'BUILT_PRODUCTS_DIR'
        ] ?? ''}/${environment['INFOPLIST_PATH'] ?? ''}';
  ...
  void buildApp() {
    ...
    if (environment['FLUTTER_ENGINE'] != null &&
      ...
      flutterArgs.add(
        '--local-engine-src-path=${environment['FLUTTER_ENGINE']}',
      );
    }
    ...
    flutterArgs.addAll(<String>[
      ...
      '--ExtraGenSnapshotOptions=${environment[
    ...
    if (environment['PERFORMANCE_MEASUREMENT_FILE'] != null &&
      ...
      flutterArgs.add(
        '--performance-measurement-file=${environment[
    ...
    if (environment['CODE_SIZE_DIRECTORY'] != null &&
      ...
      flutterArgs.add(
        '-dCodeSizeDirectory=${environment['CODE_SIZE_DIRECTORY']}',
      );
    }
