...
class LogLine {
  ...
  bool get couldBeCrash => message.contains(
...
class ProcessTestResult {
  ...
  List<String> get stdout {
    return logs.where((LogLine log) => log.channel == 'stdout').map<String>(
      (LogLine log) => log.message,
    ).toList();
  }

  List<String> get stderr {
    return logs.where((LogLine log) => log.channel == 'stderr').map<String>(
      (LogLine log) => log.message,
    ).toList();
  }
...
Future<ProcessTestResult> runFlutter(
  ...
  String stamp() => '[${(clock.elapsed.inMilliseconds / 1000.0).toStringAsFixed(
    1,
  ).padLeft(5)}s]';
  ...
  if (nextTransition < transitions.length) {
    ...
    debugPrint(
      '${nextTransition.toString().padLeft(
        5,
      )} NOW WAITING FOR> ${transitions[nextTransition]}',
    );
    if (!streamingLogs) {
      describeStatus();
