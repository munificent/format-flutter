...
void main() {
  ...
  setUp(() {
    ...
    tempDir = createResolvedTempDirectorySync(
      'android_plugin_new_output_dir_test.',
    );
  });
  ...
  test(
    "error logged when plugin's build output dir was not private.",
    () async {
      ...
      final String projectPath = fileSystem.path.join(
        tempDir.path,
        'flutter_project',
      );

      final File modulePubspec = fileSystem.file(
        fileSystem.path.join(projectPath, 'pubspec.yaml'),
      );
