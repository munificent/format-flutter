...
void main() {
  ...
  testUsingContext(
    ...
    () async {
      ...
      final Directory buildDir = projectDir.childDirectory('.dart_tool/flutter_build').listSync().firstWhere(
  ...
  testUsingContext(
    ...
    () async {
      await _createProject(projectDir, <String>[]);
      // We need a dependency so the plugin registrant is not completely empty.
      await _editPubspecFile(projectDir, _composeEditors(<PubspecEditor>[
  ...
  testUsingContext(
    ...
    () async {
      ...
      await _editPubspecFile(
        projectDir,
        _addDependencyEditor('test_web_plugin_with_a_purposefully_extremely_long_package_name', path: '../test_plugin'),
      );
...
Future<void> _ensureFlutterToolsSnapshot() async {
  ...
  final String dotPackages = globals.fs.path.absolute(globals.fs.path.join('.dart_tool/package_config.json'));
