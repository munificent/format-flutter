...
void main() {
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(
        contents,
        contains('SharedPreferencesPlugin.registerWith(registrar);'),
      );
  ...
  testUsingContext(
    ...
    () async {
      ...
      final Directory buildDir = projectDir.childDirectory(
  ...
  testUsingContext(
    ...
    () async {
      await _createProject(projectDir, <String>[]);
      await _addAnalysisOptions(projectDir, <String>[
      ...
      await _editPubspecFile(projectDir, _addDependencyEditor(
        'test_web_plugin_with_a_purposefully_extremely_long_package_name',
        path: '../test_plugin',
      ));
...
Future<void> _ensureFlutterToolsSnapshot() async {
  ...
  final String dotPackages = globals.fs.path.absolute(
...
PubspecEditor _addDependencyEditor(
  String packageToAdd, {
  String? version,
  String? path,
}) {
...
Future<void> _runFlutterSnapshot(
  ...
  final ProcessResult exec = await Process.run(globals.artifacts!
      .getArtifactPath(
        Artifact.engineDartBinary,
        platform: TargetPlatform.web_javascript,
      ), args, workingDirectory: workingDir.path);
