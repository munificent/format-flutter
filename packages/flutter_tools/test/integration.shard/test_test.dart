...
void main() {
  ...
  testWithoutContext('flutter test should run all tests when tags are unspecified', () async {
    ...
    expect(result, ProcessResultMatcher(exitCode: 1, stdoutPattern: RegExp(r'\+\d+ -1: Some tests failed\.')));
  });
  ...
  testWithoutContext('flutter test should run a test by line number in URI format', () async {
    final ProcessResult result =
        await _runFlutterTest('uri_format', automatedTestsDirectory, flutterTestDirectory, query: 'line=11');
    expect(result, ProcessResultMatcher(stdoutPattern: RegExp(r'\+\d+: All tests passed!')));
  });
...
Future<void> _testFile(
  ...
  final ProcessResult exec =
      await _runFlutterTest(testName, workingDirectory, testDirectory, extraArguments: extraArguments);
