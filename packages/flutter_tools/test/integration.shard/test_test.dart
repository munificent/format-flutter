...
final String automatedTestsDirectory = fileSystem.path.join(
  '..',
  '..',
  'dev',
  'automated_tests',
...
final String flutterTestDirectory = fileSystem.path.join(
...
void main() {
  ...
  testWithoutContext(
    'integration test should not have extraneous error messages',
    () async {
      return _testFile(
        'trivial_widget',
        automatedTestsDirectory,
  ...
  testWithoutContext(
    ...
    () async {
      ...
      expect(result, ProcessResultMatcher(
        stdoutPattern: RegExp(r'\+\d+: All tests passed!'),
      ));
    },
  );
      ...
      try {
        ...
        sub = process.stdout.transform(utf8.decoder).listen((String e) {
          if (!completer.isCompleted && vmServiceUriRegExp.hasMatch(e)) {
            completer.complete(
              Uri.parse(vmServiceUriRegExp.firstMatch(e)!.group(0)!),
            );
          }
        });
    ...
    try {
      ...
      sub = process.stdout.transform(utf8.decoder).listen((String e) {
        if (!completer.isCompleted && devToolsUriRegExp.hasMatch(e)) {
          completer.complete(
            Uri.parse(devToolsUriRegExp.firstMatch(e)!.group(1)!),
          );
        }
      });
