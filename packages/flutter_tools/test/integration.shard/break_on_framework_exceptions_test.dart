...
void main() {
  ...
  Future<void> expectException(TestProject project, String exceptionMessage) async {
    await _timeoutAfter(message: 'Timed out setting up project in $tempDir', work: () => project.setUpIn(tempDir));
    ...
    await _timeoutAfter(message: 'Timed out waiting for VM service pause debug event', work: flutter.waitForPause);
...
class TestProject extends Project {
  ...
  @override
  String get test => _test.replaceFirst('// SETUP', setup ?? '').replaceFirst('// TEST_BODY', testBody).replaceFirst(
    '// CLASSES',
    classes ?? '',
  );
