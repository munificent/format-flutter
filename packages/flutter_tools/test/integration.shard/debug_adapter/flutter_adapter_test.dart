...
void main() {
  ...
  group('launch', () {
    ...
    testWithoutContext('logs to client when sendLogsToClient=true', () async {
      ...
      final List<String> logMessages = logEvents.map(
        (Event l) => (l.body! as Map<String, Object?>)['message']! as String,
      ).toList();
      expect(logMessages, containsAll(<Matcher>[
    ...
    testWithoutContext(
      'can run and terminate a Flutter app in noDebug mode',
      () async {
        final BasicProject project = BasicProject();
        await project.setUpIn(tempDir);

        // Once the "topLevelFunction" output arrives, we can terminate the app.
        unawaited(dap.client.stdoutOutput
            .firstWhere(
              (String output) => output.startsWith('topLevelFunction'),
            )
            .whenComplete(() => dap.client.terminate()));
    ...
    testWithoutContext(
      'outputs useful message on invalid DAP protocol messages',
      () async {
    ...
    testWithoutContext(
      'correctly outputs launch errors and terminates',
      () async {
        final CompileErrorProject project = CompileErrorProject();
        await project.setUpIn(tempDir);
    ...
    testWithoutContext(
      'correctly outputs exceptions in debug mode',
      () => testExceptionOutput(noDebug: false),
    );

    testWithoutContext(
      'correctly outputs exceptions in noDebug mode',
    ...
    testWithoutContext(
      ...
      () async {
        ...
        final List<Map<String, Object?>> eventBodies = progressEvents.map(
          (Event event) => event.body,
        ).cast<Map<String, Object?>>().toList();
        final ProgressStartEventBody start = ProgressStartEventBody.fromMap(
          eventBodies[0],
        );
        final ProgressEndEventBody end = ProgressEndEventBody.fromMap(
          eventBodies[1],
        );
    ...
    testWithoutContext(
      ...
      () async {
        ...
        await Future.wait(<Future<void>>[
          ...
          dap.client.stoppedEvents.first.then(
            (StoppedEventBody event) => newThreadId = event.threadId!,
          ),
  ...
  group('attach', () {
    ...
    setUp(() async {
      ...
      breakpointFilePath = globals.fs.path.join(
        project.dir.path,
        'lib',
        'main.dart',
      );
    ...
    testWithoutContext(
      'can attach to an already-running Flutter app and reload',
      () async {
    ...
    testWithoutContext('resumes and removes breakpoints on detach', () async {
      ...
      await Future.wait(<Future<void>>[
        ...
        testProcess.output.firstWhere(
          (String output) => output.contains('topLevelFunction'),
        ),
        // Trigger the detach.
...
String _uniqueOutputLines(List<OutputEventBody> outputEvents) {
  String? lastItem;
  return outputEvents.map((OutputEventBody e) => e.output).where(
    (String output) {
