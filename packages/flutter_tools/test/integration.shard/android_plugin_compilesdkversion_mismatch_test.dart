...
void main() {
  ...
  test('error logged when plugin Android compileSdkVersion higher than project', () async {
    ...
    final File projectGradleFile = pluginExampleAppDir.childDirectory('android').childDirectory('app').childFile(
      'build.gradle',
    );
    ...
    expect(result.stderr, contains('One or more plugins require a higher Android SDK version.'));
    expect(result.stderr, contains('Fix this issue by adding the following to ${projectGradleFile.path}'));
  });
}
