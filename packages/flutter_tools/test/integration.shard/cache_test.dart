    ...
    testWithoutContext(
      ...
      () async {
        ...
        try {
          ...
          final File cacheFile = fileSystem.file(fileSystem.path.join(
            Cache.flutterRoot!,
            'bin',
            'cache',
            'lockfile',
          ))..createSync(recursive: true);
          final File script = fileSystem.file(fileSystem.path.join(
            Cache.flutterRoot!,
            'bin',
            'cache',
            'test_lock.dart',
...
''');
          ...
          while (!locked) {
            ...
            final RandomAccessFile lock = cacheFile.openSync(
              mode: FileMode.write,
            );
            try {
    ...
    testWithoutContext(
      ...
      () async {
        ...
        try {
          ...
          final FakeVersionlessArtifact artifact = FakeVersionlessArtifact(
            cache,
          );
  ...
  testWithoutContext('Dart SDK target arch matches host arch', () async {
    ...
    final ProcessResult dartResult = await const LocalProcessManager()
        .run(<String>[dart, '--version']);
    ...
    final String dartTargetArch = (dartResult.stdout as String)
        ...
        .replaceAll('"', '')
        .split('_')[1];
    final ProcessResult unameResult = await const LocalProcessManager()
        .run(<String>['uname', '-m']);
