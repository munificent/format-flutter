...
void main() {
  group('Cache.lock', () {
    ...
    testWithoutContext('should log a message to stderr when lock is not acquired', () async {
      ...
      final BufferLogger logger = BufferLogger(terminal: Terminal.test(), outputPreferences: OutputPreferences());
...
''');
        ...
        process = await const LocalProcessManager()
  ...
  testWithoutContext('Dart SDK target arch matches host arch', () async {
    ...
    final ProcessResult dartResult = await const LocalProcessManager().run(<String>[dart, '--version']);
    ...
    final ProcessResult unameResult = await const LocalProcessManager().run(<String>['uname', '-m']);
    final String unameArch = (unameResult.stdout as String).trim().replaceAll('aarch64', 'arm64').replaceAll(
      'x86_64',
      'x64',
    );
