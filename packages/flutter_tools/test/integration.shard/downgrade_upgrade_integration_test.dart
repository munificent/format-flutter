...
void main() {
  ...
  testWithoutContext('Can upgrade and downgrade a Flutter checkout', () async {
    ...
    final RunResult versionResult = await processUtils
        .run(<String>['git', 'describe', '--match', '*.*.*', '--long', '--tags'], workingDirectory: testDirectory.path);
    ...
    final RunResult oldVersionResult = await processUtils
        .run(<String>['git', 'describe', '--match', '*.*.*', '--long', '--tags'], workingDirectory: testDirectory.path);
