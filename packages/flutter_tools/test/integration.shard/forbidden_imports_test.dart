...
void main() {
  final String flutterTools = fileSystem.path.join(
    getFlutterRoot(),
    'packages',
    'flutter_tools',
  );
  ...
  test('no imports of commands/* or test/* in lib/src/*', () {
    ...
    bool isNotSkipped(FileSystemEntity entity) => skippedPaths.every(
      (String path) => !entity.path.startsWith(path),
    );
  ...
  test('no imports of globals without a global prefix', () {
    ...
    final Iterable<File> files = fileSystem
        .directory(fileSystem.path.join(flutterTools, 'lib', 'src'))
        .listSync(recursive: true)
        .followedBy(fileSystem.directory(
          fileSystem.path.join(flutterTools, 'test'),
        ).listSync(recursive: true))
        .where(_isDartFile)
        .where(isNotSkipped)
        .map(_asFile);
  ...
  test('no unauthorized imports of dart:io', () {
    ...
    bool isNotAllowed(FileSystemEntity entity) => allowedPaths.every(
      (String path) => path != entity.path,
    );
  ...
  test('no unauthorized imports of build_runner/dwds/devtools', () {
    ...
    bool isNotAllowed(FileSystemEntity entity) => allowedPaths.every(
      (String path) => !entity.path.contains(path),
    );
