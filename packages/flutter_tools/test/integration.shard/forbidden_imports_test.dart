...
void main() {
  ...
  test('no imports of commands/* or test/* in lib/src/*', () {
    ...
    final Iterable<File> files = fileSystem.directory(
      fileSystem.path.join(flutterTools, 'lib', 'src'),
    ).listSync(recursive: true).where(_isDartFile).where(isNotSkipped).map(_asFile);
    for (final File file in files) {
      for (final String line in file.readAsLinesSync()) {
        if (line.startsWith(RegExp(r'import.*package:'))) {
          continue;
        }
        if (line.startsWith(RegExp(r'import.*commands/')) || line.startsWith(RegExp(r'import.*test/'))) {
  ...
  test('no unauthorized imports of dart:io', () {
    ...
    for (final String dirName in <String>['lib', 'bin']) {
      final Iterable<File> files = fileSystem.directory(
        fileSystem.path.join(flutterTools, dirName),
      ).listSync(recursive: true).where(_isDartFile).where(isNotAllowed).map(_asFile);
      for (final File file in files) {
        for (final String line in file.readAsLinesSync()) {
          if (line.startsWith(RegExp(r'import.*dart:io')) && !line.contains('flutter_ignore: dart_io_import')) {
            final String relativePath = fileSystem.path.relative(file.path, from: flutterTools);
