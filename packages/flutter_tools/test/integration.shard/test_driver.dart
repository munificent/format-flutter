...
abstract class FlutterTestDriver {
  ...
  Future<void> _setupProcess(
    ...
    _stdout.stream.listen(
      (String message) => _debugPrint(message, topic: '<=stdout='),
    );
    _stderr.stream.listen(
      (String message) => _debugPrint(message, topic: '<=stderr='),
    );
  }
  ...
  Future<Event> subscribeToPauseEvent(
    String isolateId,
  ) => subscribeToDebugEvent('Pause', isolateId);
  Future<Event> subscribeToResumeEvent(
    String isolateId,
  ) => subscribeToDebugEvent('Resume', isolateId);

  Future<Isolate> waitForPauseEvent(
    String isolateId,
    Future<Event> event,
  ) => waitForDebugEvent('Pause', isolateId, event);
  Future<Isolate> waitForResumeEvent(
    String isolateId,
    Future<Event> event,
  ) => waitForDebugEvent('Resume', isolateId, event);

  Future<Isolate> waitForPause() async => subscribeAndWaitForDebugEvent(
    'Pause',
    await _getFlutterIsolateId(),
  );
  Future<Isolate> waitForResume() async => subscribeAndWaitForDebugEvent(
    'Resume',
    await _getFlutterIsolateId(),
  );
  ...
  Future<Isolate?> resume({bool waitForNextPause = false}) => _resume(
    null,
    waitForNextPause,
  );
  Future<Isolate?> stepOver({bool waitForNextPause = true}) => _resume(
    StepOption.kOver,
    waitForNextPause,
  );
  Future<Isolate?> stepOverAsync({bool waitForNextPause = true}) => _resume(
    StepOption.kOverAsyncSuspension,
    waitForNextPause,
  );
  Future<Isolate?> stepInto({bool waitForNextPause = true}) => _resume(
    StepOption.kInto,
    waitForNextPause,
  );
  Future<Isolate?> stepOut({bool waitForNextPause = true}) => _resume(
    StepOption.kOut,
    waitForNextPause,
  );
  ...
  Future<SourcePosition?> getSourceLocation() async {
    ...
    final Script script = await _vmService!.getObject(
      flutterIsolateId,
  ...
  Future<Map<String, Object?>> _waitFor({
    ...
    assert(event == null || id == null);
    final String interestingOccurrence = event != null
        ? '$event event'
  ...
  Future<T> _timeoutWithMessages<T>(
    ...
    final StreamSubscription<String> subscription = _allMessages.stream.listen(
      logMessage,
    );
  ...
  Future<int> detach() async {
    ...
    if (_currentRunningAppId != null) {
      ...
      await Future.any<void>(<Future<void>>[
        process.exitCode,
        _sendRequest('app.detach', <String, Object?>{
          'appId': _currentRunningAppId,
        }),
      ]).timeout(
        quitTimeout,
...
class FlutterTestTestDriver extends FlutterTestDriver {
  ...
  Future<void> _setupProcess(
    ...
    if (withDebugger) {
      ...
      _vmServiceWsUri = Uri.parse(vmServiceHttpString).replace(
        scheme: 'ws',
        path: '/ws',
      );
      await connectToVmService(pauseOnExceptions: pauseOnExceptions);
...
Stream<String> transformToLines(Stream<List<int>> byteStream) {
  return byteStream.transform<String>(utf8.decoder).transform<String>(
    const LineSplitter(),
  );
}
