...
void main() {
  ...
  void createDotPackages(String projectPath, [bool nullSafe = false]) {
    ...
    final String canonicalizedFlutterRootPath = fileSystem.path.canonicalize(
      getFlutterRoot(),
    );
...
''';
  ...
  setUp(() {
    ...
    final String projectWithErrors = fileSystem.path.join(
      tempDir.path,
    ...
    errorFile = fileSystem.file(
      fileSystem.path.join(projectWithErrors, 'other', 'error.dart'),
    )
      ..createSync(recursive: true)
  ...
  testWithoutContext('analyze once no duplicate issues', () async {
    final File foo = fileSystem.file(
      fileSystem.path.join(projectPath, 'foo.dart'),
    );
...
''');
    ...
    final File bar = fileSystem.file(
      fileSystem.path.join(projectPath, 'bar.dart'),
    );
...
analyzer:
  ...
  ''');
      ...
      await runCommand(
        arguments: <String>['analyze', '--no-pub'],
        statusTextContains: <String>['info', 'missing_return'],
        exitMessageContains: '1 issue found.',
        exitCode: 1,
      );
    },
...
analyzer:
  ...
  ''');
      ...
      await runCommand(
        ...
        statusTextContains: <String>['warning', 'missing_return'],
        exitMessageContains: '1 issue found.',
      );
    },
  );
