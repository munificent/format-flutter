...
class MigrateProject extends Project {
  ...
  Future<void> setUpIn(
    ...
    if (Platform.isWindows) {
      await processManager
          .run(<String>['robocopy', tempDir.path, dir.path, '*', '/E', '/mov']);
      ...
      await processManager
          .run(<String>['cp', '-R', '-L', '-f', '${tempDir.path}/.', dir.path]);

      await processManager
          .run(<String>['rm', '-rf', '.cipd'], workingDirectory: dir.path);
  ...
  @override
  String get pubspec => fileSystem.file(
    fileSystem.path.join(_appPath, 'pubspec.yaml'),
  ).readAsStringSync();
