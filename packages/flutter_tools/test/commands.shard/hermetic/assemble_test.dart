...
void main() {
  ...
  testUsingContext(
    ...
    () async {
      ...
      await commandRunner
          .run(<String>['assemble', '-o Output', '-dFooBar=fizz=2', 'debug_macos_bundle_flutter_assets']);
  ...
  testUsingContext(
    ...
    () async {
      final AssembleCommand command = AssembleCommand(buildSystem: TestBuildSystem.all(BuildResult(success: true)));
      final CommandRunner<void> commandRunner = createTestCommandRunner(command);
  ...
  testUsingContext(
    ...
    () async {
      ...
      await expectLater(commandRunner
          .run(<String>['assemble', '-o Output', 'debug_macos_bundle_flutter_assets']), throwsToolExit());
    ...
    () async {
      final BuildSystem buildSystem = TestBuildSystem.list(<BuildResult>[
        ...
        BuildResult(
          ...
          outputFiles: <File>[
            globals.fs.file('bar'),
            globals.fs.file(globals.fs.path.join('.dart_tool', 'fizz2'))..createSync(recursive: true),
          ],
        ),
      ]);
  ...
  testWithoutContext('writePerformanceData outputs performance data in JSON form', () {
    ...
    expect(json.decode(outFile.readAsStringSync()), <String, Object>{
      'targets': <Object>[
        <String, Object>{'name': 'foo', 'skipped': false, 'succeeded': true, 'elapsedMilliseconds': 123},
      ],
    });
  });

  testUsingContext(
    'test --dart-define-from-file option with err file format',
    () {
  ...
  testUsingContext(
    'test --dart-define-from-file option with err json format',
    () async {
