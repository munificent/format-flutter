...
''');
  ...
  FakeCommand setUpXCResultCommand({String stdout = '', void Function()? onRun}) {
    return FakeCommand(
      command: const <String>['xcrun', 'xcresulttool', 'get', '--path', _xcBundleFilePath, '--format', 'json'],
      stdout: stdout,
      onRun: onRun,
    );
  }
  ...
  FakeCommand exportArchiveCommand({String exportOptionsPlist = '/ExportOptions.plist', File? cachePlist}) {
    return FakeCommand(
      command: <String>[
        'xcrun',
        'xcodebuild',
...
''';
  ...
  testUsingContext(
    ...
    () async {
      ...
      await createTestCommandRunner(command).run(const <String>['build', 'ipa', '--no-pub', '--no-codesign']);
      expect(fakeProcessManager, hasNoRemainingExpectations);
...
[
  {
    ...
    "s": 2400
      ...
      expect(usage.events, contains(const TestUsageEvent('code-size-analysis', 'ios')));
      expect(fakeProcessManager, hasNoRemainingExpectations);
    },
    overrides: <Type, Generator>{
      FileSystem: () => fileSystem,
  ...
  testUsingContext(
    ...
    () async {
      ...
      fakeProcessManager.addCommands(<FakeCommand>[xattrCommand, setUpFakeXcodeBuildHandler(), exportArchiveCommand()]);
      createMinimalMockProjectFiles();
  ...
  testUsingContext(
    ...
    () async {
      ...
      await expectLater(createTestCommandRunner(command)
          .run(const <String>['build', 'ipa', '--no-pub']), throwsToolExit());
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(testLogger.warningText, isNot(contains(
  ...
  testUsingContext(
    ...
    () async {
      ...
      fakeProcessManager.addCommands(<FakeCommand>[xattrCommand, setUpFakeXcodeBuildHandler(exitCode: 1)]);
      createMinimalMockProjectFiles();
  ...
  testUsingContext(
    ...
    () async {
      ...
      plistUtils.fileContents[plistPath] = <String, String>{'CFBundleIdentifier': 'io.flutter.someProject'};
  ...
  testUsingContext(
    ...
    () async {
      ...
      plistUtils.fileContents[plistPath] = <String, String>{'CFBundleIdentifier': 'com.example.my_app'};
  ...
  testUsingContext(
    ...
    () async {
      ...
      plistUtils.fileContents[plistPath] = <String, String>{'CFBundleIdentifier': 'com.my_company.my_app'};
