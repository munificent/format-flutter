...
final Platform notWindowsPlatform = FakePlatform(environment: <String, String>{'FLUTTER_ROOT': flutterRoot});
...
void main() {
  ...
  // files.
  FakeCommand cmakeGenerationCommand({void Function()? onRun, String generator = _defaultGenerator}) {
    return FakeCommand(
      command: <String>[
        _cmakePath,
        '-S',
  ...
  FakeCommand buildCommand(String buildMode, {bool verbose = false, void Function()? onRun, String stdout = ''}) {
    return FakeCommand(
      command: <String>[
        _cmakePath,
        '--build',
  ...
  testUsingContext(
    ...
    () async {
      ...
      final BuildWindowsCommand command = BuildWindowsCommand(logger: BufferLogger.test())..visualStudioOverride =
          fakeVisualStudio;
      setUpMockCoreProjectFiles();
...
''');
  ...
  testUsingContext(
    ...
    () async {
      ...
      processManager = FakeProcessManager.list(<FakeCommand>[cmakeGenerationCommand(), buildCommand('Release')]);
...
''';
  ...
  testUsingContext(
    ...
    () async {
      ...
      processManager = FakeProcessManager.list(<FakeCommand>[cmakeGenerationCommand(), buildCommand('Profile')]);
  ...
  testUsingContext(
    ...
    () async {
      ...
      await createTestCommandRunner(command).run(const <String>['windows', '--no-pub', '--build-name=1.2.3']);
  ...
  testUsingContext(
    ...
    () async {
      ...
      await createTestCommandRunner(command).run(const <String>['windows', '--no-pub', '--build-number=4']);
  ...
  }, overrides: <Type, Generator>{FeatureFlags: () => TestFeatureFlags(), Platform: () => windowsPlatform});
...
[
  {
    ...
    "s": 2400
      ...
      expect(usage.events, contains(const TestUsageEvent('code-size-analysis', 'windows')));
    },
    overrides: <Type, Generator>{
      FeatureFlags: () => TestFeatureFlags(isWindowsEnabled: true),
      FileSystem: () => fileSystem,
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(
        createTestCommandRunner(command).run(const <String>['windows', '--no-pub']),
        throwsToolExit(
          message: 'Path $absPath contains invalid characters in "\'#!\$^&*=|,;<>?". '
