  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(createTestCommandRunner(command)
          .run(const <String>['windows', '--no-pub']), throwsToolExit());
    },
...
''');
  ...
  testUsingContext(
    ...
    () async {
      ...
      fileSystem.file(fileSystem.path.join('lib', 'other.dart')).createSync(
        recursive: true,
      );
      fileSystem.file(fileSystem.path.join('foo', 'bar.sksl.json')).createSync(
        recursive: true,
      );
...
[
  {
    ...
    "s": 2400
      ...
      expect(
        usage.events,
        contains(const TestUsageEvent('code-size-analysis', 'windows')),
      );
    },
    overrides: <Type, Generator>{
      FeatureFlags: () => TestFeatureFlags(isWindowsEnabled: true),
