  ...
  group('ide_config', () {
    ...
    Future<void> updateIdeConfig({
      ...
      for (final String path in expectedContents.keys) {
        final String absPath = globals.fs.path.join(tempDir.absolute.path, path);
        expect(fileOrDirectoryExists(globals.fs.path.join(dir.path, path)), true, reason: "$path doesn't exist");
    ...
    testUsingContext("doesn't touch existing files without --overwrite", () async {
      final Map<String, String> templateManifest = getManifest(intellijDir, 'template', isTemplate: true);
    ...
    testUsingContext('creates non-existent files', () async {
      ...
      populateDir(templateManifest);
      final Map<String, String> expectedContents = <String, String>{...templateManifest, ...flutterManifest};
      return updateIdeConfig(expectedContents: expectedContents);
    });
    ...
    testUsingContext('overwrites existing files with --overwrite', () async {
      ...
      final Map<String, String> expectedContents = <String, String>{...templateManifest, ...overwrittenManifest};
    ...
    testUsingContext('only adds new templates without --overwrite', () async {
      ...
      final Map<String, String> flutterManifest = getManifest(tempDir, 'existing');
      populateDir(flutterManifest);
      final Map<String, String> expectedContents = <String, String>{...flutterManifest, ...templateManifest};
      return updateIdeConfig(args: <String>['--update-templates'], expectedContents: expectedContents);
    });
    ...
    testUsingContext('update all templates with --overwrite', () async {
      ...
      final Map<String, String> flutterManifest = getManifest(tempDir, 'existing');
      populateDir(flutterManifest);
      final Map<String, String> updatedTemplates = getManifest(intellijDir, 'existing', isTemplate: true);
      final Map<String, String> expectedContents = <String, String>{...flutterManifest, ...updatedTemplates};
      return updateIdeConfig(args: <String>['--update-templates', '--overwrite'], expectedContents: expectedContents);
    });
