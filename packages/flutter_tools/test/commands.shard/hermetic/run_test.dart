...
void main() {
  ...
  group('run', () {
    ...
    testUsingContext(
      ...
      () async {
        ...
        await createTestCommandRunner(command)
            .run(const <String>['run', '--use-application-binary=app/bar/faz', '--no-sound-null-safety']);
      },
      overrides: <Type, Generator>{
    ...
    testUsingContext(
      ...
      () async {
        ...
        final RunCommand command = RunCommand();
        await expectLater(() => createTestCommandRunner(command).run(<String>['run', '--no-pub']), throwsToolExit());
        final BufferLogger bufferLogger = globals.logger as BufferLogger;
        expect(bufferLogger.statusText, containsIgnoringWhitespace('Changing current working directory to:'));
      },
      overrides: <Type, Generator>{
        FileSystem: () => fileSystem,
        ProcessManager: () => FakeProcessManager.any(),
    ...
    group('run app', () {
      ...
      testUsingContext(
        ...
        () async {
          ...
          final TestRunCommandThatOnlyValidates command = TestRunCommandThatOnlyValidates();
          await expectLater(
            createTestCommandRunner(command).run(<String>['run', '--no-pub', '--device-user', '10']),
            throwsToolExit(
      ...
      testUsingContext(
        ...
        () async {
          ...
          final TestRunCommandThatOnlyValidates command = TestRunCommandThatOnlyValidates();
          await createTestCommandRunner(command).run(<String>['run', '--no-pub', '--device-user', '10']);
      ...
      testUsingContext(
        ...
        ''', flush: true);
          ...
          final RunCommand command = RunCommand();
          await expectLater(
            createTestCommandRunner(command).run(<String>['run', '--pub']),
      ...
      testUsingContext(
        ...
        () async {
          ...
          expect(testLogger.statusText, containsIgnoringWhitespace(userMessages
              .flutterMissPlatformProjects(Device.devicesPlatformTypes(<Device>[mockDevice]))));
        },
        overrides: <Type, Generator>{
          DeviceManager: () => testDeviceManager,
          FileSystem: () => fs,
      ...
      testUsingContext(
        ...
        () async {
          final RunCommand command = RunCommand();
          final FakeDevice mockDevice = FakeDevice(sdkNameAndVersion: 'iOS 13')..startAppSuccess = false;
          ...
          await expectToolExitLater(createTestCommandRunner(command)
              .run(<String>['run', '--no-pub', '--no-hot', '--uninstall-first']), isNull);
      ...
      testUsingContext(
        ...
        () async {
          ...
          await expectToolExitLater(createTestCommandRunner(command)
              .run(<String>['run', '--no-pub', '--no-hot']), isNull);

          expect(usage.commands, contains(TestUsageCommand(
            'run',
      ...
      testUsingContext(
        ...
        () async {
          ...
          await expectToolExitLater(createTestCommandRunner(command)
              .run(<String>['run', '--no-pub', '--no-hot', 'test/widget_test.dart']), isNull);
      ...
      group('--machine', () {
        testUsingContext(
          ...
          () async {
            ...
            await expectLater(
              () => createTestCommandRunner(command).run(<String>['run', '--no-pub', '--machine', '-d', device.id]),
              throwsToolExit(),
            );
        ...
        testUsingContext(
          ...
          () async {
            ...
            await expectLater(
              () => createTestCommandRunner(command)
                  .run(<String>['run', '--no-pub', '--no-multidex', '--machine', '-d', device.id]),
              throwsToolExit(),
            );
        ...
        testUsingContext(
          ...
          () async {
            ...
            await expectLater(
              () => createTestCommandRunner(command)
                  .run(<String>['run', '--no-pub', '--machine', '--device-user', '10', '-d', device.id]),
              throwsToolExit(),
            );
    ...
    group('Fatal Logs', () {
      ...
      }, overrides: <Type, Generator>{FileSystem: () => fs, ProcessManager: () => FakeProcessManager.any()});
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(
          ...
          unorderedEquals(<DevelopmentArtifact>{DevelopmentArtifact.universal, DevelopmentArtifact.androidGenSnapshot}),
        );
        ...
        expect(
          await RunCommand().requiredArtifacts,
          unorderedEquals(<DevelopmentArtifact>{DevelopmentArtifact.universal, DevelopmentArtifact.iOS}),
        );

        testDeviceManager.devices = <Device>[FakeDevice(), FakeDevice(targetPlatform: TargetPlatform.android_arm)];
        ...
        expect(
          await RunCommand().requiredArtifacts,
          unorderedEquals(<DevelopmentArtifact>{DevelopmentArtifact.universal, DevelopmentArtifact.web}),
        );
      },
      overrides: <Type, Generator>{
        DeviceManager: () => testDeviceManager,
    ...
    group('usageValues', () {
      testUsingContext(
        ...
        () async {
          ...
          try {
            ...
            // command.usageValues.
            expect(error, isA<ToolExit>().having(
      ...
      testUsingContext(
        ...
        () async {
          final List<Device> devices = <Device>[
            FakeIOSDevice(connectionInterface: DeviceConnectionInterface.wireless, sdkNameAndVersion: 'iOS 16.2'),
          ];
          final TestRunCommandForUsageValues command = TestRunCommandForUsageValues(devices: devices);
          final CommandRunner<void> runner = createTestCommandRunner(command);
          try {
  ...
  testUsingContext(
    ...
    () async {
      ...
      await expectLater(
        () => createTestCommandRunner(command).run(<String>['run', '--no-pub', '--bundle-sksl-path=foo.json']),
  ...
  testUsingContext('fails when "--web-launch-url" is not supported', () async {
    final RunCommand command = RunCommand();
    await expectLater(
      () => createTestCommandRunner(command).run(<String>['run', '--web-launch-url=http://flutter.dev']),
      throwsA(isException.having(
        (Exception exception) => exception.toString(),
        'toString',
  ...
  }, overrides: <Type, Generator>{ProcessManager: () => FakeProcessManager.any(), Logger: () => BufferLogger.test()});
}
...
class TestRunCommandForUsageValues extends RunCommand {
  TestRunCommandForUsageValues({this.devices});
