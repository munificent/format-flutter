...
void main() {
  ...
  group('run', () {
    ...
    group('run app', () {
      ...
      testUsingContext(
        ...
        () async {
          ...
          await expectToolExitLater(createTestCommandRunner(command).run(
            <String>['run', '--no-pub', '--no-hot', '--uninstall-first'],
          ), isNull);
      ...
      testUsingContext(
        ...
        () async {
          ...
          await expectToolExitLater(createTestCommandRunner(command).run(
            <String>['run', '--no-pub', '--no-hot', 'test/widget_test.dart'],
          ), isNull);
      ...
      group('--machine', () {
        testUsingContext(
          ...
          () async {
            final DaemonCapturingRunCommand command =
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(await RunCommand().requiredArtifacts, unorderedEquals(
          <DevelopmentArtifact>{
        ...
        expect(await RunCommand().requiredArtifacts, unorderedEquals(<
          DevelopmentArtifact
        >{DevelopmentArtifact.universal, DevelopmentArtifact.iOS}));
        ...
        expect(await RunCommand().requiredArtifacts, unorderedEquals(<
          DevelopmentArtifact
        >{DevelopmentArtifact.universal, DevelopmentArtifact.web}));
      },
      overrides: <Type, Generator>{
        DeviceManager: () => testDeviceManager,
        Cache: () => Cache.test(processManager: FakeProcessManager.any()),
  ...
  testUsingContext(
    ...
    () async {
      ...
      final DebuggingOptions options =
          await command.createDebuggingOptions(true);
...
class FakeDevice extends Fake implements Device {
  ...
  Never _throwToolExit(int code) => throwToolExit(
    'FakeDevice tool exit',
    exitCode: code,
  );
