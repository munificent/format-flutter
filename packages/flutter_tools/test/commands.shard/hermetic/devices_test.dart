  ...
  group('devices', () {
    ...
    group('ensure factory', () {
      ...
      testWithoutContext('returns DevicesCommandOutputWithExtendedWirelessDeviceDiscovery on MacOS', () async {
        ...
        final DevicesCommandOutput devicesCommandOutput = DevicesCommandOutput(platform: platform, logger: fakeLogger);
          ...
          group('when deviceConnectionInterface', () {
            ...
            testUsingContext(
              ...
              () async {
                ...
                expect(json.decode(testLogger.statusText), <Map<String, Object>>[fakeDevices[2].json]);
              },
              overrides: <Type, Generator>{
                DeviceManager: () => _FakeDeviceManager(devices: deviceList),
                ProcessManager: () => FakeProcessManager.any(),
...
''');
    ...
    group('when Platform is MacOS', () {
      ...
      testUsingContext('returns 0 when called', () async {
        final DevicesCommand command = DevicesCommand();
        await createTestCommandRunner(command).run(<String>['devices']);
      }, overrides: <Type, Generator>{Cache: () => cache, Artifacts: () => Artifacts.test(), Platform: () => platform});
...
''');
      ...
      group('when includes both attached and wireless devices', () {
        List<FakeDeviceJsonData>? deviceList;
        setUp(() {
          deviceList = <FakeDeviceJsonData>[fakeDevices[0], fakeDevices[1], fakeDevices[2], fakeDevices[3]];
        });
...
class _FakeDeviceManager extends DeviceManager {
  ...
  @override
  Future<List<Device>> refreshAllDevices({Duration? timeout, DeviceDiscoveryFilter? filter}) => getAllDevices(
    filter: filter,
  );
