  ...
  group('daemon', () {
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(response.data['result'], isNotEmpty);
        expect(
          (response.data['result']! as Map<String, Object?>)['platforms'],
          <String>{'macos'},
        );
      },
    ...
    testUsingContext(
      'device.discoverDevices should respond with list',
      () async {
        daemon = Daemon(daemonConnection, notifyingLogger: notifyingLogger);
        daemonStreams.inputs.add(DaemonMessage(<String, Object?>{
          'id': 0,
          'method': 'device.discoverDevices',
        }));
        final DaemonMessage response =
            await daemonStreams.outputs.stream.firstWhere(_notEvent);
        expect(response.data['id'], 0);
    ...
    testUsingContext(
      'device.discoverDevices reports available devices',
      () async {
        daemon = Daemon(daemonConnection, notifyingLogger: notifyingLogger);
        final FakePollingDeviceDiscovery discoverer =
            FakePollingDeviceDiscovery();
    ...
    testUsingContext(
      'device.supportsRuntimeMode returns correct value',
      () async {
        daemon = Daemon(daemonConnection, notifyingLogger: notifyingLogger);
        final FakePollingDeviceDiscovery discoverer =
            FakePollingDeviceDiscovery();
    ...
    group('device.startApp and .stopApp', () {
      ...
      testUsingContext(
        ...
        () async {
          ...
          expect(
            applicationPackageFactory.applicationBinaryRequested!.basename,
            'test_file',
          );
          expect(
          ...
          final Uri vmServiceUri = Uri.parse(
            'http://127.0.0.1:12345/vmService',
          );
          device.launchResult = LaunchResult.succeeded(
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(
          device.dds.startVMServiceUri,
          Uri.parse('http://fake_uri/auth_code'),
        );
    ...
    testUsingContext(
      ...
      () async {
        ...
        unawaited(daemonStreams.outputs.stream
            .firstWhere(
              (DaemonMessage request) =>
                  request.data['method'] == 'app.exposeUrl',
            )
            .then((DaemonMessage request) {
              expect(
                (request.data['params']! as Map<String, Object?>)['url'],
                equals(originalUrl),
              );
              daemonStreams.inputs.add(DaemonMessage(<String, Object?>{
                'id': request.data['id'],
    ...
    testUsingContext(
      ...
      overrides: <Type, Generator>{
        DevtoolsLauncher: () => FakeDevtoolsLauncher(
          DevToolsServerAddress('127.0.0.1', 1234),
        ),
      },
    );
    ...
    testUsingContext(
      ...
      () async {
        ...
        await io.IOOverrides.runWithIOOverrides(() async {
          ...
          final DaemonMessage disconnectEvent = await broadcastOutput
              .firstWhere(
                (DaemonMessage message) =>
                    message.data['event'] != null &&
    ...
    testWithoutContext(
      ...
      () async {
        await _runFakeAsync((FakeAsync time) async {
          final List<Future<int>> operations = <Future<int>>[
            queue.queueAndDebounce('OP1', debounceDuration, () async => 1),
            Future<void>.delayed(debounceDuration * 2).then(
              (_) => queue.queueAndDebounce(
                'OP1',
                debounceDuration,
                () async => 2,
              ),
