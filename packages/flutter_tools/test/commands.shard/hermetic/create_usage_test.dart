  ...
  group('usageValues', () {
    ...
    testUsingContext(
      ...
      () => testbed.run(() async {
        ...
        await runner
            .run(<String>['create', '--no-pub', '--template=module', 'testy']);
        expect((await command.usageValues).commandCreateProjectType, 'module');

        await runner
            .run(<String>['create', '--no-pub', '--template=app', 'testy1']);
        ...
        expect(
          (await command.usageValues).commandCreateProjectType,
          'skeleton',
        );
        ...
        await runner
            .run(<String>['create', '--no-pub', '--template=plugin', 'testy4']);
    ...
    testUsingContext(
      ...
      () => testbed.run(() async {
        ...
        await runner
            .run(<String>['create', '--no-pub', '--template=app', 'testy']);
    ...
    testUsingContext(
      ...
      () => testbed.run(() async {
        ...
        await runner
            .run(<String>['create', '--no-pub', '--template=app', 'testy']);
        expect(
          (await command.usageValues).commandCreateAndroidLanguage,
          'kotlin',
        );
    ...
    testUsingContext(
      ...
      }, overrides: <Type, Generator>{Pub: () => fakePub}),
    );
  });
}
