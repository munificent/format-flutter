...
void main() {
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(
        logger.errorText,
        contains('Screenshot not supported for FakeDevice'),
      );
  ...
  testUsingContext(
    ...
    () async {
      ...
      FakeAsync().run<void>((FakeAsync time) {
        ...
        expect(screenshotDevice.screenshots, contains(isA<File>().having(
          (File file) => file.path,
          'path',
...
class NeverEndingFlutterDriverFactory extends Fake
  ...
  @override
  DriverService createDriverService(bool web) => NeverEndingDriverService(
    callback,
  );
}
