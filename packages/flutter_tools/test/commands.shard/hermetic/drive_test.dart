...
void main() {
    ...
    () async {
      ...
      await expectLater(
        () => createTestCommandRunner(command)
            .run(<String>['drive', '--no-pub', '-d', screenshotDevice.id, '--screenshot', 'drive_screenshots']),
    ...
    () async {
      ...
      FakeAsync().run<void>((FakeAsync time) {
        ...
        expect(
          ...
          contains(isA<File>().having((File file) => file.path, 'path', 'drive_screenshots/drive_01.png')),
        );
      });
  ...
  testUsingContext(
    ...
    () async {
      ...
      unawaited(createTestCommandRunner(command).run(<String>[
        'drive',
        '--no-pub',
        '-d',
        screenshotDevice.id,
  ...
  testUsingContext(
    ...
    () async {
      ...
      final Device wirelessDevice = FakeIosDevice()..connectionInterface = DeviceConnectionInterface.wireless;
      fakeDeviceManager.wirelessDevices = <Device>[wirelessDevice];

      await expectLater(() => createTestCommandRunner(command).run(<String>['drive']), throwsToolExit());
  ...
  testUsingContext(
    ...
    () async {
      ...
      final Device usbDevice = FakeIosDevice()..connectionInterface = DeviceConnectionInterface.attached;
