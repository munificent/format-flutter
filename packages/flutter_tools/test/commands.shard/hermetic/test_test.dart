...
void main() {
  ...
  setUp(() {
    ...
    package.childDirectory('test').childFile('some_test.dart').createSync(
      recursive: true,
    );
    package
...
dev_dependencies:
  flutter_test:
    ...
    ''');
      ...
      expect(
        () => commandRunner
            .run(const <String>['test', '--no-pub', 'integration_test']),
        throwsToolExit(),
      );
    },
    overrides: <Type, Generator>{
    ...
    () async {
      ...
      expect(
        ...
        throwsA(isA<ToolExit>().having(
          (ToolExit toolExit) => toolExit.message,
          'message',
          isNull,
        )),
      );
  ...
  testUsingContext(
    ...
    () async {
      ...
      await commandRunner
          .run(const <String>['test', '--no-pub', '--', 'test/fake_test.dart']);
  ...
  testUsingContext(
    ...
    () async {
      ...
      final List<String> logPhaseMessages = logger.messages.where(
        (String m) => m.startsWith('Runtime for phase '),
      ).toList();
      ...
      final List<String> logPhaseMessagesNonZero = logPhaseMessages.where(
        (String m) => !m.contains(Duration.zero.toString()),
      ).toList();
  ...
  group('Required artifacts', () {
    ...
    testUsingContext(
      ...
      () async {
        ...
        await commandRunner
            .run(const <String>['test', '--no-pub', '--platform=chrome']);
  ...
  testUsingContext(
    ...
    () async {
      ...
      await commandRunner
          .run(const <String>['test', '--no-pub', '--no-test-assets']);
