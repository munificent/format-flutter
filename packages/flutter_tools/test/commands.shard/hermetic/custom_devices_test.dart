...
void main() {
  ...
  group('linux', () {
    ...
    testUsingContext('custom-devices add command fails when feature is not enabled', () async {
      final CommandRunner<void> runner = createCustomDevicesCommandRunner();
      expect(runner.run(const <String>['custom-devices', 'add']), throwsToolExit(message: featureNotEnabledMessage));
    });
    ...
    testUsingContext('custom-devices list command fails when feature is not enabled', () async {
      final CommandRunner<void> runner = createCustomDevicesCommandRunner();
      expect(runner.run(const <String>['custom-devices', 'list']), throwsToolExit(message: featureNotEnabledMessage));
    });

    testUsingContext('custom-devices reset command fails when feature is not enabled', () async {
      final CommandRunner<void> runner = createCustomDevicesCommandRunner();
      expect(runner.run(const <String>['custom-devices', 'reset']), throwsToolExit(message: featureNotEnabledMessage));
    });
    ...
    testUsingContext('custom-devices add command correctly adds ipv4 ssh device config', () async {
      ...
      expect(config.devices, contains(CustomDeviceConfig(
        ...
        forwardPortSuccessRegex: RegExp('Port forwarding success'),
        screenshotCommand: const <String>['ssh', '-o', 'BatchMode=yes', 'testuser@192.168.178.1', 'testscreenshot'],
      )));
    });
    ...
    testUsingContext('custom-devices add command correctly adds ipv6 ssh device config', () async {
      ...
      expect(config.devices, contains(CustomDeviceConfig(
        ...
        forwardPortSuccessRegex: RegExp('Port forwarding success'),
        screenshotCommand: const <String>['ssh', '-o', 'BatchMode=yes', '-6', 'testuser@[::1]', 'testscreenshot'],
      )));
    });
  ...
  group('windows', () {
    ...
    testUsingContext('custom-devices add command correctly adds ssh device config on windows', () async {
