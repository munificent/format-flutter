...
void main() {
  ...
  group('config', () {
    testUsingContext('throws error on excess arguments', () {
      ...
      expect(
        () => commandRunner.run(<String>['config', '--android-studio-dir=/opt/My', 'Android', 'Studio']),
        throwsToolExit(),
      );
      verifyNoAnalytics();
    ...
    testUsingContext('warns the user to reload IDE', () async {
      ...
      expect(testLogger.statusText, containsIgnoringWhitespace('You may need to restart any open editors'));
    }, overrides: <Type, Generator>{Usage: () => testUsage});
    ...
    testUsingContext(
      ...
      () async {
        ...
        await commandRunner.run(<String>['config']);
    ...
    testUsingContext('no-analytics flag flips usage flag and sends event', () async {
      ...
      // info.
      expect(testUsage.events, equals(<TestUsageEvent>[const TestUsageEvent('analytics', 'enabled', label: 'false')]));
      expect(testUsage.commands, isEmpty);
      expect(testUsage.timings, isEmpty);
    }, overrides: <Type, Generator>{Usage: () => testUsage});
    ...
    testUsingContext('analytics flag flips usage flag and sends event', () async {
      ...
      // info.
      expect(testUsage.events, equals(<TestUsageEvent>[const TestUsageEvent('analytics', 'enabled', label: 'true')]));
      expect(testUsage.commands, isEmpty);
      expect(testUsage.timings, isEmpty);
    }, overrides: <Type, Generator>{Usage: () => testUsage});
    ...
    testUsingContext('analytics reported disabled when suppressed', () async {
      ...
      expect(testLogger.statusText, containsIgnoringWhitespace('Analytics reporting is currently disabled'));
    }, overrides: <Type, Generator>{Usage: () => testUsage});
  });
}
