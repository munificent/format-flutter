...
void main() {
  ...
  FakeCommand cmakeCommand(String buildMode, {String target = 'x64', void Function()? onRun}) {
    return FakeCommand(
      command: <String>[
        'cmake',
        '-G',
  ...
  testUsingContext(
    ...
    () async {
      ...
      processManager = FakeProcessManager.list(<FakeCommand>[cmakeCommand('release'), ninjaCommand('release')]);
...
''');
  ...
  testUsingContext(
    ...
    () async {
      ...
      setUpMockProjectFilesForBuild();
      processManager = FakeProcessManager.list(<FakeCommand>[cmakeCommand('debug'), ninjaCommand('debug')]);
  ...
  testUsingContext(
    ...
    () async {
      ...
      setUpMockProjectFilesForBuild();
      processManager = FakeProcessManager.list(<FakeCommand>[cmakeCommand('profile'), ninjaCommand('profile')]);
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(createTestCommandRunner(command)
          .run(const <String>['build', 'linux', '--no-pub', '--target-platform=linux-x64']), throwsToolExit());
    },
    overrides: <Type, Generator>{
      Platform: () => linuxPlatform,
      FeatureFlags: () => TestFeatureFlags(isLinuxEnabled: true),
...
''');
  ...
  testUsingContext('hidden when not enabled on Linux host', () {
    expect(
      BuildLinuxCommand(logger: BufferLogger.test(), operatingSystemUtils: FakeOperatingSystemUtils()).hidden,
      true,
    );
  }, overrides: <Type, Generator>{FeatureFlags: () => TestFeatureFlags(), Platform: () => notLinuxPlatform});
...
[
  {
    ...
    "s": 2400
      ...
      expect(usage.events, contains(const TestUsageEvent('code-size-analysis', 'linux')));
    },
    overrides: <Type, Generator>{
      FileSystem: () => fileSystem,
      ProcessManager: () => processManager,
