...
void main() {
  ...
  void setUpMockProjectFilesForBuild() {
    ...
    fileSystem.file(fileSystem.path.join('linux', 'CMakeLists.txt')).createSync(
      recursive: true,
    );
  }
  ...
  testUsingContext(
    ...
    () async {
      ...
      await createTestCommandRunner(command)
          .run(const <String>['build', 'linux', '--no-pub']);
      expect(
        fileSystem.file('linux/flutter/ephemeral/generated_config.cmake'),
        exists,
      );
    },
    overrides: <Type, Generator>{
...
[
  {
    ...
    "s": 2400
      ...
      expect(
        usage.events,
        contains(const TestUsageEvent('code-size-analysis', 'linux')),
      );
    },
    overrides: <Type, Generator>{
      FileSystem: () => fileSystem,
