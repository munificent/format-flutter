...
final Platform notMacosPlatform = FakePlatform(
  environment: <String, String>{'FLUTTER_ROOT': '/'},
);
...
void main() {
  ...
  void createMinimalMockProjectFiles() {
    fileSystem.directory(
      fileSystem.path.join('ios', 'Runner.xcodeproj'),
    ).createSync(recursive: true);
    fileSystem.directory(
      fileSystem.path.join('ios', 'Runner.xcworkspace'),
    ).createSync(recursive: true);
    fileSystem.file(
...
''',
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(createTestCommandRunner(command).run(const <String>[
        'build',
        'ios',
        '--no-pub',
  ...
  testUsingContext(
    ...
    () async {
      ...
      fileSystem.directory(
        fileSystem.path.join('ios', 'RenamedProj.xcodeproj'),
      ).createSync(recursive: true);
      fileSystem.directory(
        fileSystem.path.join('ios', 'RenamedWorkspace.xcworkspace'),
      ).createSync(recursive: true);
      fileSystem.file(
        fileSystem.path.join('ios', 'RenamedProj.xcodeproj', 'project.pbxproj'),
      ).createSync();
      createCoreMockProjectFiles();
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(
        usage.events,
        contains(const TestUsageEvent('code-size-analysis', 'ios')),
      );
    },
    overrides: <Type, Generator>{
      FileSystem: () => fileSystem,
  ...
  group('xcresults device', () {
    testUsingContext(
      ...
      () async {
        ...
        await expectLater(createTestCommandRunner(command)
            .run(const <String>['build', 'ios', '--no-pub']), throwsToolExit());
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(
          testLogger.statusText,
          isNot(contains('Lots of spew from Xcode')),
        );
      },
      overrides: <Type, Generator>{
        FileSystem: () => fileSystem,
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(testLogger.errorText, isNot(contains(
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(
          testLogger.errorText,
          contains('Some Provisioning profile issue.'),
        );
        expect(testLogger.errorText, contains(
        ...
        ProcessManager: () => FakeProcessManager.list(<FakeCommand>[
          ...
          setUpFakeXcodeBuildHandler(
            onRun: () {
              ...
              if (fileSystem.systemTempDirectory.childFile(
                _xcBundleFilePath,
              ).existsSync()) {
...
''',
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(
          testLogger.errorText,
          isNot(contains(noDevelopmentTeamInstruction)),
        );
      },
      overrides: <Type, Generator>{
        FileSystem: () => fileSystem,
  ...
  group('xcresults simulator', () {
    testUsingContext(
      ...
      () async {
        ...
        await expectLater(createTestCommandRunner(command).run(const <String>[
          'build',
          'ios',
          '--simulator',
          '--no-pub',
