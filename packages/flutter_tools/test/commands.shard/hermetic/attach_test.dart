  ...
  group('attach', () {
    ...
    testUsingContext(
      ...
      () async {
        ...
        await expectLater(createTestCommandRunner(command).run(<String>['attach']), throwsToolExit());
    ...
    testUsingContext(
      ...
      () async {
        ...
        testDeviceManager.devices = <Device>[FakeAndroidDevice(id: 'xx1'), FakeAndroidDevice(id: 'yy2')];
        await expectLater(createTestCommandRunner(command).run(<String>['attach']), throwsToolExit());
...
Future<void> expectLoggerInterruptEndsTask(Future<void> task, StreamLogger logger) async {
  ...
  await expectLater(() => task, throwsA(isA<ToolExit>().having((ToolExit error) => error.exitCode, 'exitCode', 2)));
}
...
class FakeIOSDevice extends Fake implements IOSDevice {
  ...
  @override
  DeviceLogReader getLogReader({IOSApp? app, bool includePastLogs = false, bool usingCISystem = false}) {
    if (onGetLogReader == null) {
      throw UnimplementedError(
