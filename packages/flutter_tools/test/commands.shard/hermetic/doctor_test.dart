...
void main() {
  ...
  testWithoutContext(
    ...
    () {
      const ValidationMessage messageA = ValidationMessage(
        'ab',
        contextUrl: 'a',
      );
      const ValidationMessage messageB = ValidationMessage(
        'ab',
        contextUrl: 'b',
      );
  ...
  group('doctor usage params', () {
    ...
    testUsingContext('sending events can be skipped', () async {
      await FakePassingDoctor(logger).diagnose(
        verbose: false,
        sendEvent: false,
      );
  ...
  group('doctor with fake validators', () {
    ...
    testUsingContext(
      ...
      () async {
        expect(
          await FakeCrashingDoctor(logger).diagnose(verbose: false),
          isFalse,
        );
        expect(logger.statusText, equals(
          'Doctor summary (to see all details, run flutter doctor -v):\n'
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(
          logger.statusText,
          contains('#0      CrashingValidator.validate'),
        );
      },
    );
    ...
    testUsingContext(
      ...
      () async {
        expect(
          await FakeSinglePassingDoctor(logger).diagnose(verbose: false),
          isTrue,
        );
        expect(logger.statusText, equals(
          'Doctor summary (to see all details, run flutter doctor -v):\n'
  ...
  group('grouped validator merging results', () {
    final PassingGroupedValidator installed = PassingGroupedValidator(
      'Category',
    );
    ...
    testUsingContext('validate partial + partial = partial', () async {
      expect(
        await FakeSmallGroupDoctor(logger, partial, partial).diagnose(),
        isTrue,
      );
      expect(logger.statusText, startsWith('[!]'));
    }, overrides: <Type, Generator>{AnsiTerminal: () => FakeTerminal()});
    ...
    testUsingContext('validate partial + missing = partial', () async {
      expect(
        await FakeSmallGroupDoctor(logger, partial, missing).diagnose(),
        isTrue,
      );
      expect(logger.statusText, startsWith('[!]'));
    }, overrides: <Type, Generator>{AnsiTerminal: () => FakeTerminal()});
    ...
    testUsingContext('validate missing + partial = partial', () async {
      expect(
        await FakeSmallGroupDoctor(logger, missing, partial).diagnose(),
        isTrue,
      );
      expect(logger.statusText, startsWith('[!]'));
    }, overrides: <Type, Generator>{AnsiTerminal: () => FakeTerminal()});
    ...
    testUsingContext('validate missing + missing = missing', () async {
      expect(
        await FakeSmallGroupDoctor(logger, missing, missing).diagnose(),
        isFalse,
      );
      expect(logger.statusText, startsWith('[✗]'));
    }, overrides: <Type, Generator>{AnsiTerminal: () => FakeTerminal()});
  ...
  group('FlutterValidator', () {
    ...
    testUsingContext(
      ...
      () async {
        ...
        fs.directory('/path/to/flutter/bin/cache/artifacts').createSync(
          recursive: true,
        );
  ...
  testUsingContext(
    ...
    () {
      ...
      expect(
        provider.validators,
        isNot(contains(isA<AndroidStudioValidator>())),
      );
      expect(
        provider.validators,
...
class VsCodeValidatorTestTargets extends VsCodeValidator {
  ...
  static VsCodeValidatorTestTargets
  get installedWithExtension => VsCodeValidatorTestTargets._(
    validInstall,
    validExtensions,
  );

  static VsCodeValidatorTestTargets
  get installedWithExtension64bit => VsCodeValidatorTestTargets._(
    validInstall,
    validExtensions,
  ...
  static VsCodeValidatorTestTargets
  get installedWithoutExtension => VsCodeValidatorTestTargets._(
    validInstall,
    missingExtensions,
  );

  static final String validInstall = globals.fs.path.join(
    'test',
    'data',
    'vscode',
    'application',
...
class FakeDevice extends Fake implements Device {
  ...
  @override
  Future<TargetPlatform> get targetPlatform => Future<TargetPlatform>.value(
