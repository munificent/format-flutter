  ...
  group('Usage', () {
    ...
    testUsingContext('indicate that project is a plugin', () async {
      final String projectPath =
          await createProject(tempDir, arguments: <String>['--no-pub', '--template=plugin', '--project-name=aar_test']);
    ...
    testUsingContext('logs success', () async {
      ...
      expect(testUsage.events, contains(const TestUsageEvent('tool-command-result', 'aar', label: 'success')));
  ...
  group('Gradle', () {
    ...
    group('throws ToolExit', () {
      testUsingContext('main.dart not found', () async {
        await expectLater(() async {
          await runBuildAarCommand('missing_project', mockAndroidSdk, arguments: <String>['--no-pub']);
...
Future<BuildAarCommand> runBuildAarCommand(String target, AndroidSdk? androidSdk, {List<String>? arguments}) async {
  final BuildAarCommand command = BuildAarCommand(
  ...
  final CommandRunner<void> runner = createTestCommandRunner(command);
  await runner.run(<String>['aar', '--no-pub', ...?arguments, globals.fs.path.join(target, 'lib', 'main.dart')]);
  return command;
}
