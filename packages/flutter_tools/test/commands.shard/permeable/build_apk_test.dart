...
void main() {
  ...
  group('Usage', () {
    ...
    testUsingContext('logs success', () async {
      ...
      expect(testUsage.events, contains(const TestUsageEvent('tool-command-result', 'apk', label: 'success')));
  ...
  group('Gradle', () {
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(
          testUsage.events,
          contains(const TestUsageEvent('build', 'gradle', label: 'gradle-r8-failure', parameters: CustomDimensions())),
        );
        expect(processManager, hasNoRemainingExpectations);
      },
      overrides: <Type, Generator>{
...
Future<BuildApkCommand> runBuildApkCommand(String target, {List<String>? arguments}) async {
  ...
  await runner.run(<String>['apk', ...?arguments, '--no-pub', globals.fs.path.join(target, 'lib', 'main.dart')]);
  return command;
}
