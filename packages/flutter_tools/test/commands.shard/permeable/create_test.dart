...
FakePlatform _kNoColorTerminalPlatform() =>
    FakePlatform.fromPlatform(const LocalPlatform())..stdoutSupportsAnsi =
        false;
...
void main() {
  ...
  test('createAndroidIdentifier emits a valid identifier', () {
    final String identifier = CreateBase.createAndroidIdentifier(
      '42org',
      '8project',
    );
  ...
  test('createWindowsIdentifier emits a GUID', () {
    final String identifier = CreateBase.createWindowsIdentifier(
      'org',
      'project',
    );
  ...
  testUsingContext(
    ...
    () async {
      ...
      final Directory workingDir = globals.fs.directory(
        r'X:\path\to\working\dir',
      );
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(
        logger.statusText,
        contains('In order to run your application, type:'),
      );
  ...
  testUsingContext(
    ...
    () async {
      ...
      await projectDir.absolute.childFile('.metadata').writeAsString(
        'project_type: blag\n',
      );
      expect(
        ...
        throwsToolExit(
          message: 'Sorry, unable to detect the type of project to recreate',
        ),
      );
    },
    overrides: <Type, Generator>{
  ...
  testUsingContext('androidx is used by default in an app project', () async {
    ...
    final String actualContents = await globals.fs.file(
      '${projectDir.path}/android/gradle.properties',
    ).readAsString();
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(projectDir.childDirectory('web'), isNot(exists));
      expect(
        projectDir.childDirectory('example').childDirectory('linux'),
        isNot(exists),
      );
      expect(
        projectDir.childDirectory('example').childDirectory('macos'),
        isNot(exists),
      );
      expect(
        projectDir.childDirectory('example').childDirectory('windows'),
        isNot(exists),
      );
      expect(
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(
        projectDir.childDirectory('linux').childFile('CMakeLists.txt'),
        exists,
      );
      expect(projectDir.childDirectory('android'), isNot(exists));
      expect(projectDir.childDirectory('ios'), isNot(exists));
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(
        projectDir.childDirectory('example').childDirectory('android'),
        isNot(exists),
      );
      expect(
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(
        projectDir.childDirectory('windows').childFile('CMakeLists.txt'),
        exists,
      );
      expect(projectDir.childDirectory('android'), isNot(exists));
      expect(projectDir.childDirectory('ios'), isNot(exists));
  ...
  testUsingContext('app creates maskable icons for web', () async {
    ...
    await runner
        .run(<String>['create', '--no-pub', '--platform=web', projectDir.path]);

    final Directory iconsDir = projectDir.childDirectory('web').childDirectory(
      'icons',
    );
  ...
  testUsingContext(
    ...
    () async {
      ...
      void expectExists(String relPath, [bool expectation = true]) {
        expect(
          globals.fs.isFileSync('${projectDir.path}/$relPath'),
          expectation,
        );
      }
      ...
      for (final FileSystemEntity file
          in projectDir.listSync(recursive: true)) {
      ...
      final String xcodeConfigPath = globals.fs.path.join(
        '.ios',
        'Flutter',
        'Generated.xcconfig',
      );
      ...
      expectExists(
        projectSharedData.childFile('IDEWorkspaceChecks.plist').path,
      );

      final String versionPath = globals.fs.path.join('.metadata');
      expectExists(versionPath);
      final String version = globals.fs.file(
        globals.fs.path.join(projectDir.path, versionPath),
      ).readAsStringSync();
      ...
      final String sdkMetaContents = globals.fs.file(
        globals.fs.path.join(projectDir.path, intelliJSdkMetadataPath),
      ).readAsStringSync();
...
1) 86f7e437faa5a7fce15d1ddcb9eaeaea377667b8 "iPhone Developer: Profile 1 (1111AAAA11)"
  ...
  testUsingContext(
    'Correct info.plist key-value pairs for objc iOS project.',
    () async {
      Cache.flutterRoot = '../..';
      ...
      final File plistFile = globals.fs.file(
        globals.fs.path.join(projectDir.path, plistPath),
      );
  ...
  testUsingContext(
    'Correct info.plist key-value pairs for objc swift project.',
    () async {
      Cache.flutterRoot = '../..';
  ...
  testUsingContext(
    'Correct info.plist key-value pairs for swift iOS plugin.',
    () async {
      Cache.flutterRoot = '../..';
  ...
  testUsingContext(
    'Correct info.plist key-value pairs for objc iOS plugin.',
    () async {
      Cache.flutterRoot = '../..';
  ...
  testUsingContext(
    ...
    () async {
      ...
      String tmpProjectDir = globals.fs.path.join(
        tempDir.path,
        'hello_flutter',
      );
      await runner.run(<String>[
      ...
      FlutterProject project = FlutterProject.fromDirectory(
        globals.fs.directory(tmpProjectDir),
      );
      expect(
  ...
  testUsingContext(
    ...
    () async {
      ...
      final String metadata = globals.fs.file(
        globals.fs.path.join(projectDir.path, '.metadata'),
      ).readAsStringSync();
  ...
  testUsingContext(
    ...
    () async {
      ...
      globals.fs.directory(
        globals.fs.path.join(projectDir.path, 'ios'),
  ...
  testUsingContext('can create an empty application project', () async {
    ...
    expect(
      projectDir.childDirectory('lib').childFile(
        'main.dart',
      ).readAsStringSync(),
    ...
    expect(
      projectDir.childFile('analysis_options.yaml').readAsStringSync(),
      isNot(contains('#')),
    );
    expect(
  ...
  testUsingContext(
    ...
    () async {
      final String outputFile = globals.fs.path.join(
        tempDir.path,
  ...
  testUsingContext('plugin creates platform interface by default', () async {
    ...
    expect(projectDir.childDirectory('lib').childFile(
      'flutter_project_method_channel.dart',
    ), exists);
    expect(projectDir.childDirectory('lib').childFile(
      'flutter_project_platform_interface.dart',
    ), exists);
  }, overrides: <Type, Generator>{FeatureFlags: () => TestFeatureFlags()});
  ...
  testUsingContext(
    ...
    () async {
      ...
      globals.fs.file(
        globals.fs.path.join(projectDir.path, 'android'),
      ).deleteSync(recursive: true);
      globals.fs.file(
        globals.fs.path.join(projectDir.path, 'example/android'),
      ).deleteSync(recursive: true);
      expect(projectDir.childDirectory('android'), isNot(exists));
      expect(
        projectDir.childDirectory('example').childDirectory('android'),
  ...
  testUsingContext(
    ...
    () async {
      ...
      final CreateCommand command = CreateCommand();
      final CommandRunner<void> runner = createTestCommandRunner(command);
      final String projectDirPath = globals.fs.path.normalize(
  ...
  testUsingContext(
    'newly created plugin has min flutter sdk version as 3.3.0',
    () async {
      Cache.flutterRoot = '../..';
  ...
  testUsingContext('default app uses flutter default versions', () async {
    ...
    expect(
      globals.fs.isFileSync('${projectDir.path}/android/app/build.gradle'),
      true,
    );

    final String buildContent = await globals.fs.file(
      '${projectDir.path}/android/app/build.gradle',
    ).readAsString();

    expect(
      buildContent.contains('compileSdkVersion flutter.compileSdkVersion'),
      true,
    );
    expect(buildContent.contains('ndkVersion flutter.ndkVersion'), true);
    expect(
      buildContent.contains('targetSdkVersion flutter.targetSdkVersion'),
      true,
    );
  });
  ...
  testUsingContext('Android Java plugin contains namespace', () async {
    ...
    final File buildGradleFile = globals.fs.file(
      '${projectDir.path}/android/build.gradle',
    );
    ...
    expect(buildGradleContent.contains(
  ...
  testUsingContext(
    ...
    () async {
      ...
      final String moduleBuildGradleFileContent = await globals.fs.file(
        globals.fs.path.join(projectDir.path, moduleBuildGradleFilePath),
      ).readAsString();
      final String moduleAppBuildGradleFileContent = await globals.fs.file(
        globals.fs.path.join(projectDir.path, moduleAppBuildGradleFlePath),
      ).readAsString();
      final String moduleFlutterBuildGradleFileContent = await globals.fs.file(
        globals.fs.path.join(projectDir.path, moduleFlutterBuildGradleFilePath),
      ).readAsString();
      ...
      expect(
        moduleFlutterBuildGradleFileContent.contains(expectedNameSpace),
        true,
      );
      const String expectedHostNameSpace =
          'namespace "com.bar.foo.flutter_project.host"';
      expect(
        moduleAppBuildGradleFileContent.contains(expectedHostNameSpace),
        true,
      );
  ...
  testUsingContext(
    ...
    () async {
      ...
      final File cApiImplFile = platformDir.childFile(
        '${classFilenameBase}_c_api.cpp',
      );
      final File pluginClassHeaderFile = platformDir.childFile(
        pluginClassHeaderName,
      );
      final File pluginClassImplFile = platformDir.childFile(
        '$classFilenameBase.cpp',
      );
      // Ensure that the files have the right names.
      ...
      expect(
        pluginClassImplFile.readAsLinesSync(),
        contains('#include "$pluginClassHeaderName"'),
      );
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(
        cmakeContents,
        contains('set(PLUGIN_NAME "foo_bar_plugin_plugin")'),
      );
    },
    overrides: <Type, Generator>{
      FeatureFlags: () => TestFeatureFlags(isLinuxEnabled: true),
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(
        logger.statusText,
        isNot(contains(_kDisabledPlatformRequestedMessage)),
      );
    },
    overrides: <Type, Generator>{
      FeatureFlags: () => TestFeatureFlags(isWindowsEnabled: true),
  ...
  testUsingContext(
    ...
    () async {
      ...
      final File toAnalyze = await globals.fs.file(
        globals.fs.path.join(dataPath, 'to_analyze.dart.test'),
      ).copy(globals.fs.path.join(projectDir.path, 'lib', 'to_analyze.dart'));
      final String relativePath = globals.fs.path.join(
        'lib',
        'to_analyze.dart',
      );
...
Future<void> _ensureFlutterToolsSnapshot() async {
  ...
  final ProcessResult snapshotResult =
      await Process.run('../../bin/cache/dart-sdk/bin/dart', snapshotArgs);
...
Future<void> _analyzeProject(
  ...
  expect(
    errors,
    unorderedEquals(expectedFailures),
...
Future<void> _getPackages(Directory workingDir) async {
  ...
  await Process.run(
    globals.artifacts!.getArtifactPath(Artifact.engineDartBinary),
    <String>[flutterToolsSnapshotPath, 'packages', 'get'],
...
String _getStringValueFromPlist({required File plistFile, String? key}) {
  ...
  assert(keyIndex > 0);
  return plist[keyIndex + 1].replaceAll('<string>', '').replaceAll(
    '</string>',
    '',
  );
}
