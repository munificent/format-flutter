...
final Map<Type, Generator> noColorTerminalOverride = <Type, Generator>{Platform: _kNoColorTerminalPlatform};
...
void main() {
  ...
  setUp(() {
    ...
    fakeFlutterVersion = FakeFlutterVersion(frameworkRevision: frameworkRevision, branch: frameworkChannel);
  ...
  testUsingContext(
    ...
    () async {
      ...
      final ProcessResult exec =
          await Process.run(flutterBin, <String>['create', 'flutter_project'], workingDirectory: Cache.flutterRoot);
  ...
  testUsingContext(
    ...
    () async {
      await _createAndAnalyzeProject(
        ...
        <String>['analysis_options.yaml', 'lib/flutter_project.dart', 'test/flutter_project_test.dart'],
        unexpectedPaths: <String>[
          'android/app/src/main/java/com/example/flutter_project/MainActivity.java',
          'android/src/main/java/com/example/flutter_project/FlutterProjectPlugin.java',
          'android/app/src/main/java/io/flutter/plugins/GeneratedPluginRegistrant.java',
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(
        logger.statusText,
        isNot(contains('https://github.com/flutter/flutter/wiki/Upgrading-pre-1.12-Android-projects')),
      );
    },
  ...
  testUsingContext('app supports Linux if requested', () async {
    ...
    expect(projectDir.childDirectory('web'), isNot(exists));
    expect(logger.errorText, isNot(contains(_kNoPlatformsMessage)));
  }, overrides: <Type, Generator>{FeatureFlags: () => TestFeatureFlags(isLinuxEnabled: true), Logger: () => logger});
  ...
  testUsingContext('app supports macOS if requested', () async {
    ...
    expect(projectDir.childDirectory('web'), isNot(exists));
    expect(logger.errorText, isNot(contains(_kNoPlatformsMessage)));
  }, overrides: <Type, Generator>{FeatureFlags: () => TestFeatureFlags(isMacOSEnabled: true), Logger: () => logger});
  ...
  testUsingContext('app supports Windows if requested', () async {
    ...
    expect(projectDir.childDirectory('web'), isNot(exists));
    expect(logger.errorText, isNot(contains(_kNoPlatformsMessage)));
  }, overrides: <Type, Generator>{FeatureFlags: () => TestFeatureFlags(isWindowsEnabled: true), Logger: () => logger});
  ...
  testUsingContext('app supports web if requested', () async {
    ...
    expect(projectDir.childDirectory('windows'), isNot(exists));
    expect(logger.errorText, isNot(contains(_kNoPlatformsMessage)));
  }, overrides: <Type, Generator>{FeatureFlags: () => TestFeatureFlags(isWebEnabled: true), Logger: () => logger});
  ...
  testUsingContext('has correct content and formatting with module template', () async {
    ...
    final String sdkMetaContents =
        globals.fs.file(globals.fs.path.join(projectDir.path, intelliJSdkMetadataPath)).readAsStringSync();
    expect(sdkMetaContents, contains('<root url="file:/'));
    expect(sdkMetaContents, contains('/bin/cache/dart-sdk/lib/core"'));
  }, overrides: <Type, Generator>{FlutterVersion: () => fakeFlutterVersion, Platform: _kNoColorTerminalPlatform});
...
1) 86f7e437faa5a7fce15d1ddcb9eaeaea377667b8 "iPhone Developer: Profile 1 (1111AAAA11)"
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(await project.ios.productBundleIdentifier(BuildInfo.debug), 'com.example.helloFlutter');
      expect(await project.ios.productBundleIdentifier(BuildInfo.profile), 'com.example.helloFlutter');
      expect(await project.ios.productBundleIdentifier(BuildInfo.release), 'com.example.helloFlutter');
  ...
  testUsingContext(
    ...
    () async {
      await _createProject(projectDir, <String>['--template=module', '--org', 'com.bar.foo'], <String>[]);
  ...
  testUsingContext(
    ...
    () async {
      ...
      final FlutterProject project = FlutterProject.fromDirectory(projectDir);
      expect(await project.ios.productBundleIdentifier(BuildInfo.debug), 'com.bar.foo.flutterProject');
    },
    overrides: <Type, Generator>{
      Pub: () => Pub.test(
            fileSystem: globals.fs,
  ...
  testUsingContext('can re-gen app android/ folder, reusing custom org', () async {
    ...
    return _createProject(
      ...
      unexpectedPaths: <String>['android/app/src/main/java/com/example/flutter_project/MainActivity.java'],
    );
  });

  testUsingContext('can re-gen app ios/ folder, reusing custom org', () async {
    await _createProject(projectDir, <String>['--no-pub', '--template=app', '--org', 'com.bar.foo'], <String>[]);
    projectDir.childDirectory('ios').deleteSync(recursive: true);
  ...
  testUsingContext('can re-gen plugin ios/ and example/ folders, reusing custom org', () async {
    ...
    final FlutterProject project = FlutterProject.fromDirectory(projectDir);
    expect(await project.example.ios.productBundleIdentifier(BuildInfo.debug), 'com.bar.foo.flutterProjectExample');
  });
  ...
  testUsingContext('fails to re-gen without specified org when org is ambiguous', () async {
    ...
    await _createProject(projectDir, <String>['--no-pub', '--template=app', '--org', 'com.bar.baz'], <String>[]);
    expect(() => _createProject(projectDir, <String>[], <String>[]), throwsToolExit(message: 'Ambiguous organization'));
  });
  ...
  testUsingContext('fails when file exists where output directory should be', () async {
    ...
    expect(runner.run(<String>['create', existingFile.path]), throwsToolExit(message: 'existing file'));
  });
  ...
  testUsingContext('fails overwrite when file exists where output directory should be', () async {
    ...
    expect(runner.run(<String>['create', '--overwrite', existingFile.path]), throwsToolExit(message: 'existing file'));
  });
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(loggingProcessManager.commands, contains(
        predicate((List<String> c) => dartCommand.hasMatch(c[0]) && c[1].contains('pub') && !c.contains('--offline')),
      ));
  ...
  testUsingContext('can create an empty application project', () async {
    ...
    expect(projectDir.childDirectory('lib').childFile('main.dart').readAsStringSync(), isNot(contains('int _counter')));
    expect(projectDir.childFile('analysis_options.yaml').readAsStringSync(), isNot(contains('#')));
    expect(projectDir.childFile('README.md').readAsStringSync(), isNot(contains('Getting Started')));
  });
  ...
  testUsingContext(
    ...
    () async {
      ...
      await expectLater(runner.run(args), throwsToolExit(exitCode: 2, message: 'Unable to download samples'));
    },
    overrides: <Type, Generator>{
      HttpClientFactory: () {
        return () {
          return FakeHttpClient.list(<FakeRequest>[
            FakeRequest(Uri.parse('https://master-api.flutter.dev/snippets/index.json')),
          ]);
        };
      },
    },
  ...
  testUsingContext('plugin includes native Swift unit tests', () async {
    ...
    expect(projectDir.childDirectory('example').childDirectory('ios').childDirectory('RunnerTests').childFile(
      'RunnerTests.swift',
    ), exists);
    expect(projectDir.childDirectory('example').childDirectory('macos').childDirectory('RunnerTests').childFile(
      'RunnerTests.swift',
    ), exists);
  }, overrides: <Type, Generator>{FeatureFlags: () => TestFeatureFlags(isMacOSEnabled: true), Logger: () => logger});
  ...
  testUsingContext('plugin includes native Objective-C unit tests', () async {
    ...
    await runner
        .run(<String>['create', '--no-pub', '--template=plugin', '--platforms=ios', '-i', 'objc', projectDir.path]);
  ...
  testUsingContext('plugin includes native Windows unit tests', () async {
    ...
    expect(
      projectDir.childDirectory('windows').childDirectory('test').childFile('flutter_project_plugin_test.cpp'),
      exists,
    );
  }, overrides: <Type, Generator>{FeatureFlags: () => TestFeatureFlags(isWindowsEnabled: true), Logger: () => logger});
  ...
  testUsingContext('plugin includes native Linux unit tests', () async {
    ...
    expect(
      projectDir.childDirectory('linux').childDirectory('test').childFile('flutter_project_plugin_test.cc'),
      exists,
    );
  }, overrides: <Type, Generator>{FeatureFlags: () => TestFeatureFlags(isLinuxEnabled: true), Logger: () => logger});
  ...
  testUsingContext('newly created iOS plugins has min iOS version of 11.0', () async {
    Cache.flutterRoot = '../..';
    final String flutterToolsAbsolutePath = globals.fs.path.join(Cache.flutterRoot!, 'packages', 'flutter_tools');
    final List<String> iosPluginTemplates = <String>[
      ...
      globals.fs.path.join(flutterToolsAbsolutePath, 'templates', 'plugin_ffi', 'ios.tmpl', 'projectName.podspec.tmpl'),
    ];
  ...
  testUsingContext(
    ...
    () async {
      ...
      await _createProject(
        ...
        <String>[moduleBuildGradleFilePath, moduleAppBuildGradleFlePath, moduleFlutterBuildGradleFilePath],
      );

      final String moduleBuildGradleFileContent =
          await globals.fs.file(globals.fs.path.join(projectDir.path, moduleBuildGradleFilePath)).readAsString();
      ...
      final String moduleFlutterBuildGradleFileContent =
          await globals.fs.file(globals.fs.path.join(projectDir.path, moduleFlutterBuildGradleFilePath)).readAsString();
  ...
  testUsingContext('Windows plugins handle partially camel-case project names correctly', () async {
    ...
    final File cApiHeaderFile = platformDir.childDirectory('include').childDirectory(projectName).childFile(
      cApiHeaderName,
    );
  ...
  testUsingContext('created plugin supports no platforms should print `no platforms` message', () async {
    ...
    expect(logger.statusText, contains(
      'To add platforms, run `flutter create -t plugin --platforms <platforms> .` under ${globals.fs.path.normalize(globals.fs.path.relative(
        projectDir.path,
      ))}.',
    ));
  ...
  testUsingContext('created FFI plugin supports no platforms should print `no platforms` message', () async {
    ...
    expect(logger.errorText, contains(_kNoPlatformsMessage));
    expect(logger.statusText, contains(
      'To add platforms, run `flutter create -t plugin_ffi --platforms <platforms> .` under ${globals.fs.path.normalize(globals.fs.path.relative(
        projectDir.path,
      ))}.',
    ));
    expect(logger.statusText, contains('For more information, see https://flutter.dev/go/plugin-platforms.'));
  ...
  testUsingContext(
    ...
    () async {
      ...
      final File toAnalyze = await globals.fs.file(globals.fs.path.join(dataPath, 'to_analyze.dart.test')).copy(
        globals.fs.path.join(projectDir.path, 'lib', 'to_analyze.dart'),
      );
      ...
      await _analyzeProject(projectDir.path, expectedFailures: expectedFailures);
    },
    overrides: <Type, Generator>{
      Pub: () => Pub.test(
            fileSystem: globals.fs,
...
Future<void> _ensureFlutterToolsSnapshot() async {
  final String flutterToolsPath = globals.fs.path.absolute(globals.fs.path.join('bin', 'flutter_tools.dart'));
  final String flutterToolsSnapshotPath = globals.fs.path.absolute(
    globals.fs.path.join('..', '..', 'bin', 'cache', 'flutter_tools.snapshot'),
  );
  final String packageConfig = globals.fs.path.absolute(globals.fs.path.join('.dart_tool', 'package_config.json'));
  ...
  final ProcessResult snapshotResult = await Process.run('../../bin/cache/dart-sdk/bin/dart', snapshotArgs);
...
Future<void> _runFlutterTest(Directory workingDir, {String? target}) async {
  ...
  final List<String> args = <String>[flutterToolsSnapshotPath, 'test', '--no-color', if (target != null) target];
