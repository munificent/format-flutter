...
void main() {
  ...
  testUsingContext('bundle getUsage indicate the target platform', () async {
    ...
    expect(
      (await command.usageValues).commandBuildBundleTargetPlatform,
      'android-arm',
    );
  });
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(
        () => runner
            .run(<String>['bundle', '--no-pub', '--target-platform=linux-x64']),
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(
        () => runner
            .run(<String>['bundle', '--no-pub', '--target-platform=darwin']),
  ...
  testUsingContext(
    ...
    () async {
      ...
      await runner
          .run(<String>['bundle', '--no-pub', '--target-platform=windows-x64']);
    },
    overrides: <Type, Generator>{
      BuildSystem: () => TestBuildSystem.all(BuildResult(success: true)),
      FileSystem: fsFactory,
  ...
  testUsingContext(
    ...
    () {
      ...
      expect(
        ...
        throwsToolExit(
          message:
              'Json config define file "--dart-define-from-file=config" is not a file, please fix first!',
        ),
      );
    },
    overrides: <Type, Generator>{
...
Iterable<String> _decodeDartDefines(Environment environment) {
  ...
  return encodedDefines.split(',').map<Uint8List>(base64.decode).map<String>(
    byteDecoder.convert,
  );
}
