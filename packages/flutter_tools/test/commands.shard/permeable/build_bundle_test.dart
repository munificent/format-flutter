...
void main() {
  ...
  Future<BuildBundleCommand> runCommandIn(String projectPath, {List<String>? arguments}) async {
    ...
    await runner.run(<String>['bundle', ...?arguments, '--target=$projectPath/lib/main.dart', '--no-pub']);
    return command;
  }
  ...
  testUsingContext(
    ...
    () async {
      ...
      final CommandRunner<void> runner = createTestCommandRunner(BuildBundleCommand(logger: BufferLogger.test()));
  ...
  }, overrides: <Type, Generator>{FileSystem: fsFactory, ProcessManager: () => FakeProcessManager.any()});
  ...
  testUsingContext(
    ...
    () async {
      ...
      await runner
          .run(<String>['bundle', '--no-pub', '--debug', '--target-platform=android-arm', '--track-widget-creation']);
    },
    overrides: <Type, Generator>{
  ...
  testUsingContext(
    ...
    () async {
      ...
      await runner
          .run(<String>['bundle', '--no-pub', '--debug', '--target-platform=android-arm', '--dart-define=foo=bar']);
    },
    overrides: <Type, Generator>{
      BuildSystem: () => TestBuildSystem.all(BuildResult(success: true), (Target target, Environment environment) {
        expect(environment.defines, <String, String>{
  ...
  testUsingContext(
    'test --dart-define-from-file option if conflict',
    () async {
      globals.fs.file(globals.fs.path.join('lib', 'main.dart')).createSync(recursive: true);
      globals.fs.file('pubspec.yaml').createSync();
      globals.fs.file('.packages').createSync();
  ...
  testUsingContext(
    'test --dart-define-from-file option by invalid file type',
    () {
      globals.fs.file(globals.fs.path.join('lib', 'main.dart')).createSync(recursive: true);
      globals.fs.file('pubspec.yaml').createSync();
      globals.fs.file('.packages').createSync();
      ...
      expect(() => runner.run(<String>['bundle', '--no-pub', '--dart-define-from-file=config']), throwsToolExit(
        message: 'Json config define file "--dart-define-from-file=config" is not a file, please fix first!',
      ));
    },
    overrides: <Type, Generator>{
      FileSystem: fsFactory,
  ...
  testUsingContext(
    'test --dart-define-from-file option by corrupted json',
    () async {
      globals.fs.file(globals.fs.path.join('lib', 'main.dart')).createSync(recursive: true);
      globals.fs.file('pubspec.yaml').createSync();
      globals.fs.file('.packages').createSync();
      ...
      expect(
        () => runner.run(<String>['bundle', '--no-pub', '--dart-define-from-file=config.json']),
...
Iterable<String> _decodeDartDefines(Environment environment) {
  ...
  return encodedDefines.split(',').map<Uint8List>(base64.decode).map<String>(byteDecoder.convert);
}
