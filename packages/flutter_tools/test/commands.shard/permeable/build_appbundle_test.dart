...
void main() {
  ...
  group('Usage', () {
    ...
    testUsingContext('logs success', () async {
      ...
      expect(testUsage.events, contains(const TestUsageEvent('tool-command-result', 'appbundle', label: 'success')));
    }, overrides: <Type, Generator>{AndroidBuilder: () => FakeAndroidBuilder(), Usage: () => testUsage});
  });
  ...
  group('Gradle', () {
    ...
    group('AndroidSdk', () {
      testUsingContext(
        ...
        () async {
          ...
          }, throwsToolExit(message: 'No Android SDK found. Try setting the ANDROID_SDK_ROOT environment variable'));
        },
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(testUsage.events, contains(
          const TestUsageEvent('build', 'gradle', label: 'app-not-using-android-x', parameters: CustomDimensions()),
        ));
      },
      overrides: <Type, Generator>{
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(testLogger.statusText, isNot(containsIgnoringWhitespace("Your app isn't using AndroidX")));
        expect(testLogger.statusText, isNot(containsIgnoringWhitespace(
          'To avoid potential build failures, you can quickly migrate your app by '
          'following the steps on https://goo.gl/CP92wY',
        )));

        expect(testUsage.events, contains(
          const TestUsageEvent('build', 'gradle', label: 'app-using-android-x', parameters: CustomDimensions()),
        ));
      },
      overrides: <Type, Generator>{
        AndroidSdk: () => fakeAndroidSdk,
...
Future<BuildAppBundleCommand> runBuildAppBundleCommand(String target, {List<String>? arguments}) async {
  final BuildAppBundleCommand command = BuildAppBundleCommand(logger: BufferLogger.test());
  final CommandRunner<void> runner = createTestCommandRunner(command);
  await runner.run(<String>['appbundle', ...?arguments, '--no-pub', globals.fs.path.join(target, 'lib', 'main.dart')]);
  return command;
}
