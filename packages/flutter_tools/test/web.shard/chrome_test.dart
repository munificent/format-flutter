...
void main() {
  ...
  testWithoutContext(
    'cannot have two concurrent instances of chrome',
    () async {
  ...
  testWithoutContext(
    'can launch new chrome after stopping a previous chrome',
    () async {
  ...
  testWithoutContext(
    ...
    () async {
      ...
      fileSystem.directory('$directoryPrefix/Local Storage').createSync(
        recursive: true,
      );
  ...
  testWithoutContext(
    ...
    () async {
      ...
      expect(
        logger.errorText,
        contains('Failed to restore Chrome preferences'),
      );
    },
  );
  ...
  testWithoutContext('can launch Chrome on x86_64 macOS', () async {
    ...
    await expectReturnsNormallyLater(
      chromiumLauncher.launch('example_url', skipCheck: true),
    );
  });
  ...
  testWithoutContext(
    ...
    () async {
      ...
      final File preferencesFile = dataDir.childDirectory('Default').childFile(
        'preferences',
      );
      preferencesFile
        ..createSync(recursive: true)
      ...
      for (final String cache in kCodeCache) {
        dataDir.childDirectory('Default').childDirectory(cache).createSync(
          recursive: true,
        );
      }
  ...
  testWithoutContext('can retry launch when glibc bug happens', () async {
    ...
    processManager.addCommand(
      const FakeCommand(command: args, stderr: kDevtoolsStderr),
    );
  ...
  testWithoutContext('can retry launch when chrome fails to start', () async {
    ...
    for (int i = 0; i < 3; i++) {
      processManager.addCommand(
        const FakeCommand(command: args, stderr: 'BLAH BLAH'),
      );
    }
