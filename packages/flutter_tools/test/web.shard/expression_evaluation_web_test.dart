...
void main() {
  group('Flutter run for web', () {
    ...
    testWithoutContext(
      'cannot evaluate expression if feature is disabled',
      () async {
    ...
    testWithoutContext(
      'shows no native javascript objects in static scope',
      () async {
...
Future<LibraryRef> getRootLibrary(FlutterTestDriver flutter) async {
  ...
  return isolate.libraries!.firstWhere(
    (LibraryRef l) => l.uri!.contains('org-dartlang-app'),
  );
}
...
void expectError(ObjRef result, String message) {
  expect(result, const TypeMatcher<ErrorRef>().having(
