...
void main() {
        ...
        FakeVmServiceRequest(
          ...
          jsonResponse: vm_service.Timeline(
            ...
            traceEvents: <vm_service.TimelineEvent>[
              vm_service.TimelineEvent.parse(<String, Object>{'name': kFlutterEngineMainEnterEventName, 'ts': 0})!,
              vm_service.TimelineEvent.parse(<String, Object>{'name': kFrameworkInitEventName, 'ts': 1})!,
              vm_service.TimelineEvent.parse(<String, Object>{'name': kFirstFrameBuiltEventName, 'ts': 2})!,
              vm_service.TimelineEvent.parse(<String, Object>{'name': kFirstFrameRasterizedEventName, 'ts': 3})!,
            ],
          ).toJson(),
        ),
        const FakeVmServiceRequest(method: 'setVMTimelineFlags', args: <String, Object>{'recordedStreams': <Object>[]}),
      ],
    );

    // Validate that old tracing data is deleted.
    final File outFile = fileSystem.currentDirectory.childFile('start_up_info.json')..writeAsStringSync('stale');

    await downloadStartupTrace(fakeVmServiceHost.vmService, output: fileSystem.currentDirectory, logger: logger);
  ...
  testWithoutContext('prints when first frame is taking a long time', () async {
    ...
    await FakeAsync().run((FakeAsync time) {
      final Map<String, String> extensionData = <String, String>{'test': 'data', 'renderedErrorText': 'error text'};
      final FakeVmServiceHost fakeVmServiceHost = FakeVmServiceHost(
      ...
      unawaited(downloadStartupTrace(fakeVmServiceHost.vmService, output: fileSystem.currentDirectory, logger: logger));
  ...
  testWithoutContext('downloadStartupTrace also downloads the timeline', () async {
    ...
    final Map<String, Object> expectedTimeline = <String, Object>{
      ...
      'traceEvents': <Object>[
        ...
        <String, Object>{'name': 'Widgets built first useful frame', 'ts': 2, 'type': 'TimelineEvent'},
