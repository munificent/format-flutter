...
void main() {
  testWithoutContext('Can trace application startup', () async {
    ...
    final File outFile = fileSystem.currentDirectory.childFile(
      'start_up_info.json',
  ...
  testWithoutContext('prints when first frame is taking a long time', () async {
    ...
    expect(
      logger.traceText,
      contains('No isolate ID associated with the view'),
    );
  ...
  testWithoutContext(
    'throws tool exit if first frame events are missing',
    () async {
      final BufferLogger logger = BufferLogger.test();
      final FileSystem fileSystem = MemoryFileSystem.test();
      final FakeVmServiceHost fakeVmServiceHost = FakeVmServiceHost(
  ...
  testWithoutContext(
    'downloadStartupTrace also downloads the timeline',
    () async {
      final BufferLogger logger = BufferLogger.test();
      final FileSystem fileSystem = MemoryFileSystem.test();
      final FakeVmServiceHost fakeVmServiceHost = FakeVmServiceHost(
      ...
      // Validate that old tracing data is deleted.
      final File timelineFile = fileSystem.currentDirectory.childFile(
