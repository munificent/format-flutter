  ...
  group('AssetBundle.build', () {
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(const StandardMessageCodec().decodeMessage(ByteData.sublistView(
          Uint8List.fromList(
...
''');
    ...
    testUsingContext(
      ...
      () async {
        ...
        globals.fs.file(
          globals.fs.path.join('assets', 'foo', 'bar.txt'),
...
''');
        ...
        final DateTime modifiedTime = pubspec.lastModifiedSync().add(
          const Duration(hours: 1),
        );
        globals.fs.directory(globals.fs.path.join('assets', 'foo')).deleteSync(
          recursive: true,
        );
        globals.fs.file('pubspec.yaml')
          ..createSync()
    ...
    testUsingContext(
      ...
      () async {
        ...
        globals.fs.directory(
          globals.fs.path.join('assets', 'foo', 'bar'),
        ).createSync();
        globals.fs.file('pubspec.yaml')
          ..createSync()
          ..writeAsStringSync(r'''
...
''');
    ...
    testUsingContext(
      ...
      () async {
        ...
        globals.fs.file(
          globals.fs.path.join('assets', 'bar', 'barbie.txt'),
        ).createSync(recursive: true);
        globals.fs.file(
          globals.fs.path.join('assets', 'wild', 'dash.txt'),
        ).createSync(recursive: true);
        globals.fs.file('pubspec.yaml')
          ..createSync()
          ..writeAsStringSync(r'''
...
''');
  ...
  testUsingContext(
    ...
    () async {
      globals.fs.file('.packages').createSync();
      globals.fs.file(globals.fs.path.join('assets', 'bar.txt')).createSync(
        recursive: true,
      );
      globals.fs.file('pubspec.yaml')
        ..createSync()
      ...
      () async {
        ...
        (globals.processManager as FakeProcessManager).addCommand(FakeCommand(
          command: <String>[
            ...
            '--iplr',
            '--json',
            '--sl=${fileSystem.path.join(
              output.path,
              'shaders',
              'ink_sparkle.frag',
            )}',
            '--spirv=${fileSystem.path.join(
              output.path,
              'shaders',
              'ink_sparkle.frag.spirv',
            )}',
            '--input=${fileSystem.path.join(
              materialDir.path,
              'shaders',
              'ink_sparkle.frag',
            )}',
...
''');
      ...
      expect(
        testLogger.errorText,
        isNot(contains('This asset was included from')),
      );
    },
    overrides: <Type, Generator>{
      FileSystem: () => MemoryFileSystem.test(),
...
''');
      ...
      expect(await bundle.build(packagesPath: '.packages'), 0);
      expect(
        (bundle.entries['FontManifest.json']! as DevFSStringContent).string,
        '[]',
      );
      expect(
        (bundle.entries['AssetManifest.json']! as DevFSStringContent).string,
        '{}',
      );
      expect(
        testLogger.errorText,
