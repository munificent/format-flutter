  ...
  group('Project', () {
    ...
    group('ensure ready for platform-specific tooling', () {
      ...
      _testInMemory('works if there is an "example" folder', () async {
        ...
        await project.regeneratePlatformSpecificTooling();
        expectExists(project.ios.hostAppRoot.childDirectory('Runner').childFile(
      ...
      _testInMemory(
        ...
        () async {
          final FlutterProject project =
              await someProject(includePubspec: true);
      ...
      testUsingContext(
        ...
        () async {
          ...
          await project.regeneratePlatformSpecificTooling();
          expectExists(project.linux.managedDirectory.childFile(
      ...
      testUsingContext(
        ...
        () async {
          ...
          await project.regeneratePlatformSpecificTooling();
          expectExists(project.windows.managedDirectory.childFile(
            'generated_plugin_registrant.h',
          ));
          expectExists(project.windows.managedDirectory.childFile(
            'generated_plugin_registrant.cc',
      ...
      _testInMemory('creates iOS pod in module', () async {
        final FlutterProject project = await aModuleProject();
        await project.regeneratePlatformSpecificTooling();
        final Directory flutter = project.ios.hostAppRoot.childDirectory(
          'Flutter',
        );
    ...
    ''', logger: BufferLogger.test())!;
      ...
      _testInMemory('gets xcworkspace directory', () async {
        ...
        project.ios.hostAppRoot.childFile('._Runner.xcworkspace').createSync(
          recursive: true,
        );
        project.ios.hostAppRoot.childFile('Runner.xcworkspace').createSync(
          recursive: true,
        );
...
''';
      ...
      group('_', () {
        ...
        _testInMemory(
          ...
          () async {
            final FlutterProject? project =
                await configureGradleAgpForTest(gradleV: '8.0', agpV: '7.4.2');
      ...
      group('_', () {
        ...
        _testInMemory(
          ...
          () async {
            final FlutterProject? project =
                await configureGradleAgpForTest(gradleV: gradleV, agpV: agpV);
            final CompatibilityResult value =
                await project!.android.hasValidJavaGradleAgpVersions();
...
''';
    ...
    group('With mocked context', () {
      ...
      group('universal link', () {
        testWithMocks('build with flavor', () async {
          ...
          project.ios.hostAppRoot.childFile(entitlementFilePath).createSync(
            recursive: true,
          );
          ...
          testPlistUtils.setProperty(
            ...
            <String>['applinks:example.com', 'applinks:example2.com'],
          );
          final XcodeUniversalLinkSettings settings = await project.ios
        ...
        testWithMocks(
          ...
          () async {
            ...
            addIosProjectFile(
              project.directory,
              projectFileContent: () {
                return projectFileWithBundleId(
                  'io.flutter.someProject',
                  qualifier: "'",
                );
              },
      ...
      testUsingContext(
        ...
        () async {
          ...
          mockXcodeProjectInterpreter
              .buildSettingsByBuildContext[buildContext] = <String, String>{
  ...
  group('watch companion', () {
    ...
    testUsingContext(
      ...
      () async {
        final FlutterProject project = await someProject();
        final XcodeProjectInfo projectInfo = XcodeProjectInfo(<String>[
          'WatchTarget',
...
''');
          ...
          mockXcodeProjectInterpreter
                  .buildSettingsByBuildContext[watchBuildContext] =
              <String, String>{
...
void transfer(FileSystemEntity entity, FileSystem target) {
  if (entity is Directory) {
    ...
    target.file(entity.absolute.path).writeAsBytesSync(
...
void addIosProjectFile(
  Directory directory, {
  required String Function() projectFileContent,
}) {
  directory.childDirectory('ios').childDirectory('Runner.xcodeproj').childFile(
    'project.pbxproj',
  )
    ..createSync(recursive: true)
...
void addAndroidGradleFile(
  ...
  directory.childDirectory('android').childDirectory('app').childFile(
    'build.gradle',
  )
    ..createSync(recursive: true)
