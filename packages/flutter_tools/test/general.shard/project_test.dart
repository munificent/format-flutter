  ...
  group('Project', () {
    group('construction', () {
      ...
      _testInMemory('reads dependencies from pubspec.yaml', () async {
        ...
        expect(FlutterProject.fromDirectory(directory).manifest.dependencies, <String>{'plugin_a', 'plugin_b'});
      });

      _testInMemory('sets up location', () async {
        final Directory directory = globals.fs.directory('myproject');
        expect(FlutterProject.fromDirectory(directory).directory.absolute.path, directory.absolute.path);
        expect(FlutterProject.fromDirectoryTest(directory).directory.absolute.path, directory.absolute.path);
        expect(FlutterProject.current().directory.absolute.path, globals.fs.currentDirectory.absolute.path);
      });
    });
    ...
    group('ensure ready for platform-specific tooling', () {
      ...
      _testInMemory('checkForDeprecation fails on invalid android app manifest file', () async {
        ...
        final FlutterProject project =
            await someProject(androidManifestOverride: invalidManifest, includePubspec: true);
      ...
      _testInMemory('Android project not on v2 embedding shows a warning', () async {
        ...
        expect(
          ...
          contains('https://github.com/flutter/flutter/wiki/Upgrading-pre-1.12-Android-projects'),
        );
      });
      ...
      _testInMemory('Android plugin project does not throw v1 embedding deprecation warning', () async {
        ...
        expect(
          testLogger.statusText,
          isNot(contains('No `<meta-data android:name="flutterEmbedding" android:value="2"/>` in ')),
        );
      });
      ...
      testUsingContext(
        ...
        overrides: <Type, Generator>{
          ...
          FlutterProjectFactory: () => FlutterProjectFactory(logger: logger, fileSystem: globals.fs),
        },
      );
      testUsingContext(
      ...
      _testInMemory('creates iOS pod in module', () async {
        ...
        final Directory pluginRegistrantClasses = flutter.childDirectory('FlutterPluginRegistrant').childDirectory(
          'Classes',
        );
    ...
    group('java gradle agp compatibility', () {
      Future<FlutterProject?> configureGradleAgpForTest({required String gradleV, required String agpV}) async {
        final FlutterProject project = await someProject();
        addRootGradleFile(
          project.directory,
          gradleFileContent: () {
...
''';
      ...
      group('_', () {
        ...
        _testInMemory('flamingo values are compatible', () async {
          final FlutterProject? project = await configureGradleAgpForTest(gradleV: '8.0', agpV: '7.4.2');
          final CompatibilityResult value = await project!.android.hasValidJavaGradleAgpVersions();
          expect(value.success, isTrue);
        }, java: java, androidStudio: androidStudio, processManager: processManager, androidSdk: androidSdk);
      });
      group('_', () {
        final FakeProcessManager processManager;
        final Java java;
        ...
        androidSdk = FakeAndroidSdkWithDir(fileSystem.currentDirectory);
        fileSystem.currentDirectory.childDirectory(androidStudio.javaPath!).createSync();
        _testInMemory('java 8 era values are compatible', () async {
          final FlutterProject? project = await configureGradleAgpForTest(gradleV: '6.7.1', agpV: '4.2.0');
          final CompatibilityResult value = await project!.android.hasValidJavaGradleAgpVersions();
          expect(value.success, isTrue);
        }, java: java, androidStudio: androidStudio, processManager: processManager, androidSdk: androidSdk);
      });
      ...
      group('_', () {
        ...
        androidSdk = FakeAndroidSdkWithDir(fileSystem.currentDirectory);
        fileSystem.currentDirectory.childDirectory(androidStudio.javaPath!).createSync();
        _testInMemory('electric eel era values are compatible', () async {
          final FlutterProject? project = await configureGradleAgpForTest(gradleV: '7.3.3', agpV: '7.2.0');
          final CompatibilityResult value = await project!.android.hasValidJavaGradleAgpVersions();
          expect(value.success, isTrue);
        }, java: java, androidStudio: androidStudio, processManager: processManager, androidSdk: androidSdk);
      });
      group('_', () {
        ...
        androidSdk = FakeAndroidSdkWithDir(fileSystem.currentDirectory);
        fileSystem.currentDirectory.childDirectory(androidStudio.javaPath!).createSync();
        _testInMemory('incompatible everything', () async {
          final FlutterProject? project = await configureGradleAgpForTest(gradleV: gradleV, agpV: agpV);
          final CompatibilityResult value = await project!.android.hasValidJavaGradleAgpVersions();
          expect(value.success, isFalse);
          // Should not have the valid string
          expect(value.description, isNot(contains(RegExp(AndroidProject.validJavaGradleAgpString))));
      ...
      group('_', () {
        ...
        androidSdk = FakeAndroidSdkWithDir(fileSystem.currentDirectory);
        fileSystem.currentDirectory.childDirectory(androidStudio.javaPath!).createSync();
        _testInMemory('incompatible java/gradle only', () async {
          final FlutterProject? project = await configureGradleAgpForTest(gradleV: gradleV, agpV: agpV);
          final CompatibilityResult value = await project!.android.hasValidJavaGradleAgpVersions();
          expect(value.success, isFalse);
          // Should not have the valid string.
      ...
      group('_', () {
        ...
        androidSdk = FakeAndroidSdkWithDir(fileSystem.currentDirectory);
        fileSystem.currentDirectory.childDirectory(androidStudio.javaPath!).createSync();
        _testInMemory('incompatible gradle/agp only', () async {
    ...
    group('With mocked context', () {
      ...
      group('universal link', () {
        testWithMocks('build with flavor', () async {
          ...
          xcodeProjectInterpreter.xcodeProjectInfo = XcodeProjectInfo(<String>[], <String>[], <String>[
            'Runner',
          ], logger);
          ...
          final XcodeUniversalLinkSettings settings =
              await project.ios.universalLinkSettings(target: 'Runner', scheme: 'Debug', configuration: 'config');
          expect(settings.associatedDomains, unorderedEquals(<String>['example.com', 'example2.com']));
    ...
    group('application bundle name', () {
      ...
      testUsingContext(
        ...
        () async {
          ...
          mockXcodeProjectInterpreter.xcodeProjectInfo = XcodeProjectInfo(<String>[], <String>[], <String>[
            'Runner',
          ], logger);
...
Future<FlutterProject> someProject({String? androidManifestOverride, bool includePubspec = false}) async {
  ...
  androidDirectory.childFile('AndroidManifest.xml').writeAsStringSync(
    androidManifestOverride ?? '<manifest></manifest>',
  );
...
void _testInMemory(
  ...
  testUsingContext(
    ...
    overrides: <Type, Generator>{
      ...
      FlutterProjectFactory: () => FlutterProjectFactory(fileSystem: testFileSystem, logger: globals.logger),
    },
  );
}
...
FileSystem getFileSystemForPlatform() {
  return MemoryFileSystem(style: globals.platform.isWindows ? FileSystemStyle.windows : FileSystemStyle.posix);
}
