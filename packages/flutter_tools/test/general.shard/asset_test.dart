...
void main() {
  group('Assets', () {
    ...
    testUsingContext('app font uses local font file', () async {
      ...
      await asset.build(manifestPath: manifestPath, packagesPath: packagesPath);
    ...
    testUsingContext('handles empty pubspec with .packages', () async {
      ...
      final AssetBundle asset = AssetBundleFactory.instance.createBundle();
      await asset.build(
        manifestPath: globals.fs.path.join(
          dataPath,
          'main',
          'pubspec.yaml',
        ), // file doesn't exist
      ...
      expect(asset.wasBuiltOnce(), true);
      expect(asset.inputFiles.map((File f) {
        return f.path;
      }), <String>[]);
    });
  });
...
''';
  ...
  testUsingContext(
    'Bundles material shaders on non-web platforms',
    () async {
  ...
  testUsingContext(
    'Does bundle material shaders on web platforms',
    () async {
      final String shaderPath = globals.fs.path.join(
        Cache.flutterRoot!,
        'packages',
