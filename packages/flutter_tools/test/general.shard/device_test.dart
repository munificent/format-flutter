...
void main() {
  group('DeviceManager', () {
    ...
    testWithoutContext('getDeviceById exact matcher', () async {
      ...
      final DeviceManager deviceManager = TestDeviceManager(
        devices,
        deviceDiscoveryOverrides: <DeviceDiscovery>[ThrowingPollingDeviceDiscovery(), LongPollingDeviceDiscovery()],
        logger: logger,
      );
  ...
  group('Filter devices', () {
    ...
    testUsingContext('returns null when multiple non ephemeral devices are found', () async {
      final List<Device> devices = <Device>[ephemeralOne, ephemeralTwo, nonEphemeralOne, nonEphemeralTwo];
      ...
      () async {
        ...
        final List<Device> filtered = await deviceManager.getDevices(
          filter: DeviceDiscoveryFilter(
            supportFilter: deviceManager.deviceSupportFilter(includeDevicesUnsupportedByProject: true),
          ),
        );
    ...
    testUsingContext('Removes devices unsupported by the project from --all', () async {
      final List<Device> devices = <Device>[nonEphemeralOne, nonEphemeralTwo, unsupported, unsupportedForProject];
      final DeviceManager deviceManager = TestDeviceManager(devices, logger: BufferLogger.test());
      deviceManager.specifiedDeviceId = 'all';
    ...
    testWithoutContext(
      ...
      () async {
        final List<Device> devices = <Device>[unsupported, unsupportedForProject];
        final TestDeviceManager deviceManager = TestDeviceManager(devices, logger: BufferLogger.test());
    ...
    testWithoutContext(
      ...
      () async {
        FakeAsync().run((FakeAsync time) {
          ...
          final Future<List<Device>> refreshCache = deviceManager.refreshAllDevices(timeout: timeToRefreshDevices);
    ...
    testWithoutContext('refreshAllDevices twice', () async {
      fakeAsync((FakeAsync async) {
        ...
        final TestDeviceManager deviceManager = TestDeviceManager(
          ...
          logger: BufferLogger.test(),
          fakeDiscoverer:
              FakePollingDeviceDiscoveryWithTimeout(<List<Device>>[firstRefreshDevices, secondRefreshDevices]),
        );

        // Expect that the cache is updated by each refresh in order of completion.
        final Future<List<Device>> firstRefresh = deviceManager.refreshAllDevices(timeout: timeToFirstRefresh);
        final Future<List<Device>> secondRefresh = deviceManager.refreshAllDevices(timeout: timeToSecondRefresh);
  ...
  testWithoutContext('computeDartVmFlags handles various combinations of Dart VM flags and null_assertions', () {
    ...
    expect(
      computeDartVmFlags(DebuggingOptions.enabled(BuildInfo.debug, dartFlags: '--foo', nullAssertions: true)),
      '--foo,--null_assertions',
    );
  });
  ...
  group('Get iOS launch arguments from DebuggingOptions', () {
    testWithoutContext('Get launch arguments for physical device with debugging enabled with all launch arguments', () {
      ...
      final List<String> launchArguments = original
          .getIOSLaunchArguments(EnvironmentType.physical, '/test', <String, dynamic>{'trace-startup': true});
    ...
    testWithoutContext('Get launch arguments for physical device with debugging enabled with no launch arguments', () {
      ...
      expect(
        launchArguments.join(' '),
        <String>['--enable-dart-profiling', '--enable-checked-mode', '--verify-entry-points'].join(' '),
      );
    });
    ...
    testWithoutContext('Get launch arguments for physical device with iPv6 network connection', () {
      ...
      expect(
        launchArguments.join(' '),
        <String>['--enable-dart-profiling', '--enable-checked-mode', '--verify-entry-points', '--vm-service-host=::0']
            .join(' '),
      );
    });
    ...
    testWithoutContext(
      ...
      () {
        ...
        final List<String> launchArguments = original
            .getIOSLaunchArguments(EnvironmentType.simulator, '/test', <String, dynamic>{'trace-startup': true});
    ...
    testWithoutContext('No --enable-dart-profiling flag when option is false', () {
      final DebuggingOptions original = DebuggingOptions.enabled(BuildInfo.debug, enableDartProfiling: false);
      ...
      expect(launchArguments.join(' '), <String>['--enable-checked-mode', '--verify-entry-points'].join(' '));
    });
  });
}
...
class TestDeviceDiscoverySupportFilter extends DeviceDiscoverySupportFilter {
  TestDeviceDiscoverySupportFilter.excludeDevicesUnsupportedByFlutterOrProject({required super.flutterProject})
