...
void main() {
  group('DeviceManager', () {
    ...
    testWithoutContext(
      'getDeviceById exact matcher with well known ID',
      () async {
    ...
    testWithoutContext(
      'getDeviceById two exact matches, matches on first',
      () async {
        final FakeDevice device1 = FakeDevice('Nexus 5', '0553790d0a4e726f');
    ...
    testWithoutContext('getAllDevices caches', () async {
      ...
      final FakeDevice attachedDevice = FakeDevice(
        'Nexus 5',
        '0553790d0a4e726f',
      );
      ...
      final FakeDevice newAttachedDevice = FakeDevice(
        'Nexus 5X',
        '01abfc49119c410e',
      );
  ...
  testWithoutContext('PollingDeviceDiscovery startPolling', () {
    FakeAsync().run((FakeAsync time) {
      ...
      expect(
        pollingDeviceDiscovery.lastPollingTimeout,
  ...
  group('Filter devices', () {
    ...
    final FakeDevice unsupported = FakeDevice(
      'unsupported',
      'unsupported',
      isSupported: false,
    );
    ...
    final FakeDevice webDevice = FakeDevice('webby', 'webby')..targetPlatform =
    ...
    testUsingContext(
      ...
      () async {
        ...
        final Device? ephemeralDevice = deviceManager.getSingleEphemeralDevice(
          devices,
        );
    ...
    testWithoutContext(
      ...
      () async {
        ...
        final List<Device> filtered =
            await deviceManager.getDevices(filter: filter);
  ...
  group('Simultaneous device discovery', () {
        ...
        FakeAsync().run((FakeAsync time) {
          ...
          final TestDeviceManager deviceManager = TestDeviceManager(
            ...
            fakeDiscoverer: FakePollingDeviceDiscoveryWithTimeout(
              <List<Device>>[initialDevices, refreshDevices],
          ...
          final Future<List<Device>> refreshCache = deviceManager
              .refreshAllDevices(timeout: timeToRefreshDevices);
    ...
    testWithoutContext('refreshAllDevices twice', () async {
      fakeAsync((FakeAsync async) {
        ...
        final Future<List<Device>> firstRefresh = deviceManager
            .refreshAllDevices(timeout: timeToFirstRefresh);
        final Future<List<Device>> secondRefresh = deviceManager
            .refreshAllDevices(timeout: timeToSecondRefresh);
  ...
  group('JSON encode DebuggingOptions', () {
    testWithoutContext('can preserve the original options', () {
      ...
      final DebuggingOptions deserialized = DebuggingOptions.fromJson(
        decoded,
        BuildInfo.debug,
      );
      expect(deserialized.startPaused, original.startPaused);
      expect(
        deserialized.disableServiceAuthCodes,
  ...
  group('Get iOS launch arguments from DebuggingOptions', () {
    ...
    testWithoutContext(
      'No --enable-dart-profiling flag when option is false',
      () {
        final DebuggingOptions original = DebuggingOptions.enabled(
          BuildInfo.debug,
          enableDartProfiling: false,
