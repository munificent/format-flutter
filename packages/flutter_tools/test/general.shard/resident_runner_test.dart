...
final vm_service.Isolate fakeUnpausedIsolate = vm_service.Isolate(
  ...
  libraries: <vm_service.LibraryRef>[vm_service.LibraryRef(id: '1', uri: 'file:///hello_world/main.dart', name: '')],
  livePorts: 0,
  name: 'test',
  number: '1',
  pauseOnExit: false,
...
final vm_service.Isolate fakePausedIsolate = vm_service.Isolate(
  ...
  breakpoints: <vm_service.Breakpoint>[
    vm_service.Breakpoint(
      ...
      id: 'test-breakpoint',
      location:
          vm_service.SourceLocation(tokenPos: 0, script: vm_service.ScriptRef(id: 'test-script', uri: 'foo.dart')),
      enabled: true,
      resolved: true,
    ),
  ],
...
void main() {
    ...
    () => testbed.run(() async {
      fakeVmServiceHost = FakeVmServiceHost(
        requests: <VmServiceExpectation>[
          ...
          FakeVmServiceRequest(method: 'getVM', jsonResponse: vm_service.VM.parse(<String, Object>{})!.toJson()),
          listViews,
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      fakeVmServiceHost = FakeVmServiceHost(requests: <VmServiceExpectation>[listViews, listViews, listViews]);
      final Completer<DebugConnectionInfo> futureConnectionInfo = Completer<DebugConnectionInfo>.sync();
      final Completer<void> futureAppStart = Completer<void>.sync();
      ...
      expect((globals.flutterUsage as TestUsage).events, contains(TestUsageEvent(
        'hot',
        'exception',
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      ...
      final Completer<void> futureAppStart = Completer<void>.sync();
      unawaited(
        residentRunner.attach(appStartedCompleter: futureAppStart, connectionInfoCompleter: futureConnectionInfo),
      );
      await futureAppStart.future;
      ...
      fakeVmServiceHost = FakeVmServiceHost(
        requests: <VmServiceExpectation>[
          ...
          const FakeVmServiceRequest(
            ...
            args: <String, Object>{'isolateId': '1', 'pause': false, 'rootLibUri': 'main.dart.incremental.dill'},
            jsonResponse: <String, Object>{
              'type': 'ReloadReport',
              'success': true,
        ...
        fakeVmServiceHost = FakeVmServiceHost(
          requests: <VmServiceExpectation>[
            ...
            FakeVmServiceRequest(
              ...
              args: <String, Object?>{'isolateId': fakeUnpausedIsolate.id, 'className': 'FOO'},
            ),
          ],
        );
  ...
  testUsingContext(
    ...
    () => testbed.run(
      () async {
        ...
        expect(testLogger.statusText, contains(RegExp(
...
''');
      ...
      final File generatedMain = globals.fs.directory('.dart_tool').childDirectory('flutter_build').childFile(
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      ...
      // to app_en.arb.
      final File arbFile = globals.fs.file(globals.fs.path.join('lib', 'l10n', 'foo.arb'))..createSync(recursive: true);
      arbFile.writeAsStringSync('''
{
  "helloWorld": "Hello, World!",
  "@helloWorld": {
      ...
      arbFile.writeAsStringSync('''
{
  ...
  testUsingContext(
    ...
    () => testbed.run(() {
      ...
      // commands
      expect(testLogger.statusText, equals(<dynamic>[
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      fakeVmServiceHost = FakeVmServiceHost(requests: <VmServiceExpectation>[listViews, listViews], wsAddress: testUri);
      globals.fs.file(globals.fs.path.join('lib', 'main.dart')).createSync(recursive: true);
      residentRunner = HotRunner(
        <FlutterDevice>[flutterDevice],
        stayResident: false,
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      ...
      residentRunner = HotRunner(
        ...
        stayResident: false,
        debuggingOptions: DebuggingOptions.enabled(
          const BuildInfo(BuildMode.debug, '', treeShakeIcons: false, dartDefines: <String>['a', 'b']),
        ),
        target: 'main.dart',
        devtoolsHandler: createNoOpHandler,
      );
      ...
      expect(
        await globals.fs.file(
          globals.fs.path.join('build', '187ef4436122d1cc2f40dc2b92f0eba0.cache.dill'),
        ).readAsString(),
        'ABC',
      );
    }),
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      ...
      residentRunner = HotRunner(
        ...
        stayResident: false,
        debuggingOptions: DebuggingOptions.enabled(
          const BuildInfo(BuildMode.debug, '', treeShakeIcons: false, trackWidgetCreation: true),
        ),
        target: 'main.dart',
        devtoolsHandler: createNoOpHandler,
      );
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      ...
      expect(fakeVmServiceHost?.hasRemainingExpectations, false);
    }, overrides: <Type, Generator>{FileSystem: () => ThrowingForwardingFileSystem(MemoryFileSystem.test())}),
  );

  testUsingContext(
    'ColdRunner writes vm service file when providing debugging option',
    () => testbed.run(() async {
      fakeVmServiceHost = FakeVmServiceHost(requests: <VmServiceExpectation>[listViews], wsAddress: testUri);
      globals.fs.file(globals.fs.path.join('lib', 'main.dart')).createSync(recursive: true);
      residentRunner = ColdRunner(
        <FlutterDevice>[flutterDevice],
        stayResident: false,
  ...
  testUsingContext(
    ...
    () async {
      ...
      final DefaultResidentCompiler? residentCompiler = (await FlutterDevice.create(
            device,
            buildInfo:
                const BuildInfo(BuildMode.debug, '', treeShakeIcons: false, nullSafetyMode: NullSafetyMode.unsound),
            target: null,
            platform: FakePlatform(),
          ))
      ...
      expect(residentCompiler.sdkRoot, '${globals.artifacts!.getHostArtifact(HostArtifact.flutterWebSdk).path}/');
  ...
  testUsingContext(
    ...
    () async {
      ...
      final DefaultResidentCompiler? residentCompiler = (await FlutterDevice.create(
            device,
            buildInfo: const BuildInfo(BuildMode.debug, '', treeShakeIcons: false, extraFrontEndOptions: <String>[]),
            target: null,
            platform: FakePlatform(),
          ))
          .generator as DefaultResidentCompiler?;

      expect(residentCompiler!.extraFrontEndOptions, contains('--flutter-widget-cache'));
    },
    overrides: <Type, Generator>{
      Artifacts: () => Artifacts.test(),
      FileSystem: () => MemoryFileSystem.test(),
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(residentCompiler!.extraFrontEndOptions, contains('--enable-experiment=alternative-invalidation-strategy'));
    },
    overrides: <Type, Generator>{
      Artifacts: () => Artifacts.test(),
      FileSystem: () => MemoryFileSystem.test(),
  ...
  testUsingContext(
    ...
    () => testbed.run(
      () async {
        fakeVmServiceHost = FakeVmServiceHost(requests: <VmServiceExpectation>[]);
        final FakeDevice device = FakeDevice()..dds = DartDevelopmentService();
        ddsLauncherCallback = (
          Uri uri, {
          ...
          throw FakeDartDevelopmentServiceException(
            message: 'Existing VM service clients prevent DDS from taking control.',
          );
        };
        final TestFlutterDevice flutterDevice = TestFlutterDevice(device, vmServiceUris: Stream<Uri>.value(testUri));
        bool caught = false;
        ...
        runZonedGuarded(
          ...
          (Object e, StackTrace st) {
            ...
            expect((e as ToolExit).message, contains('Existing VM service clients prevent DDS from taking control.'));
            done.complete();
            caught = true;
          },
        );
      ...
      () async {
        ...
        runZonedGuarded(
          ...
          (Object e, StackTrace st) {
            ...
            expect(testLogger.errorText, contains('DDS has failed to start and there is not an existing DDS instance'));
            done.complete();
            caught = true;
          },
        );
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      fakeVmServiceHost = FakeVmServiceHost(requests: <VmServiceExpectation>[listViews, setAssetBundlePath, evict]);
      residentRunner = HotRunner(
        <FlutterDevice>[flutterDevice],
        stayResident: false,
        debuggingOptions: DebuggingOptions.enabled(BuildInfo.debug),
...
class FakeDelegateFlutterDevice extends FlutterDevice {
  FakeDelegateFlutterDevice(super.device, BuildInfo buildInfo, ResidentCompiler residentCompiler, this.fakeDevFS)
...
class FakeProjectFileInvalidator extends Fake implements ProjectFileInvalidator {
  ...
  Future<InvalidationResult> findInvalidated({
    ...
    return InvalidationResult(packageConfig: packageConfig, uris: <Uri>[Uri.parse('file:///hello_world/main.dart')]);
  }
}
