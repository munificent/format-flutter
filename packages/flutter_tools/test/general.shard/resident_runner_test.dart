...
void main() {
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      globals.fs.file(globals.fs.path.join('lib', 'main.dart')).createSync(
        recursive: true,
      );
      fakeVmServiceHost = FakeVmServiceHost(
        requests: <VmServiceExpectation>[listViews, listViews],
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      ...
      await futureAppStart.future;
      flutterDevice.report = UpdateFSReport(
        success: true,
    ...
    () => testbed.run(() async {
      fakeVmServiceHost = FakeVmServiceHost(
        requests: <VmServiceExpectation>[
          ...
          const FakeVmServiceRequest(
            method: 'resume',
            args: <String, String>{'isolateId': '1'},
          ),
          listViews,
          const FakeVmServiceRequest(
            method: 'streamListen',
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      final File arbFile = globals.fs.file(
        globals.fs.path.join('lib', 'l10n', 'app_en.arb'),
      )..createSync(recursive: true);
      arbFile.writeAsStringSync('''
{
  ...
  "@helloWorld": {
    "description": "Sample description"
      ...
      final File packageConfig = globals.fs.directory('.dart_tool').childFile(
        'package_config.json',
      );
...
''');
      final Directory fakePluginDir = globals.fs.directory(
        'path_provider_linux',
      );
...
''');
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      ...
      // to app_en.arb.
      final File arbFile = globals.fs.file(
        globals.fs.path.join('lib', 'l10n', 'foo.arb'),
      )..createSync(recursive: true);
      arbFile.writeAsStringSync('''
{
  "helloWorld": "Hello, World!",
      ...
      arbFile.writeAsStringSync('''
{
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      ...
      residentRunner = HotRunner(
        ...
        stayResident: false,
        debuggingOptions:
            DebuggingOptions.enabled(BuildInfo.debug, vmserviceOutFile: 'foo'),
        target: 'main.dart',
        devtoolsHandler: createNoOpHandler,
      );
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      ...
      residentRunner.artifactDirectory.childFile('app.dill').writeAsStringSync(
        'ABC',
      );
      ...
      expect(
        await globals.fs.file(
          globals.fs.path.join('build', 'cache.dill'),
        ).readAsString(),
        'ABC',
      );
    }),
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      ...
      expect(
        await globals.fs.file(globals.fs.path.join(
          'build',
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      ...
      expect(
        globals.fs.file(globals.fs.path.join('build', 'cache.dill')),
        isNot(exists),
      );
    }),
  );
  ...
  testUsingContext(
    ...
    () async {
      ...
      expect(residentCompiler!.initializeFromDill, globals.fs.path.join(
        getBuildDirectory(),
      ...
      expect(
        ...
        '${globals.artifacts!.getHostArtifact(
  ...
  testUsingContext(
    ...
    () => testbed.run(
      () async {
        ...
        runZonedGuarded(
          () {
            flutterDevice.connect(allowExistingDdsInstance: true).then(
              (_) => done.complete(),
            );
          },
          (Object e, StackTrace st) {
            expect(e, isA<ToolExit>());
            expect((e as ToolExit).message, contains(
              'Existing VM service clients prevent DDS from taking control.',
            ));
            done.complete();
            caught = true;
      ...
      overrides: <Type, Generator>{
        VMServiceConnector: () => (
              ...
              required Logger logger,
                ...
                FakeVmServiceHost(requests: <VmServiceExpectation>[]).vmService,
      },
    ),
  );
  ...
  testUsingContext(
    ...
    () => testbed.run(() async {
      fakeVmServiceHost = FakeVmServiceHost(
        requests: <VmServiceExpectation>[listViews, evict],
      );
      residentRunner = HotRunner(
        <FlutterDevice>[flutterDevice],
        stayResident: false,
