  ...
  group('Fuchsia app start and stop: ', () {
    ...
    setUp(() {
      ...
      for (final BuildMode mode in <BuildMode>[
        ...
        memoryFileSystem.file(artifacts.getArtifactPath(
      ...
      fakeFailedProcessManagerForHostAddress = FakeProcessManager.list(
        <FakeCommand>[
          FakeCommand(
            command: <String>[
              'ssh',
    ...
    testUsingContext(
      ...
      overrides: <Type, Generator>{
        ...
        ProcessManager: () => fakeSuccessfulProcessManager,
        FuchsiaDeviceTools:
            () => FakeFuchsiaDeviceTools(pkgctl: FailingPkgctl()),
        FuchsiaArtifacts: () => FuchsiaArtifacts(sshConfig: sshConfig),
        FuchsiaSdk: () => fuchsiaSdk,
        OperatingSystemUtils: () => osUtils,
      },
...
class FakeFuchsiaPM implements FuchsiaPM {
  ...
  Future<bool> init(String buildPath, String appName) async {
    ...
    globals.fs.file(
      globals.fs.path.join(buildPath, 'meta', 'package'),
    ).createSync(recursive: true);
    _appName = appName;
    return true;
  }
  ...
  Future<bool> build(String buildPath, String manifestPath) async {
    ...
    globals.fs.file(globals.fs.path.join(buildPath, 'meta.far')).createSync(
      recursive: true,
    );
    return true;
  }
  ...
  Future<bool> archive(String buildPath, String manifestPath) async {
    ...
    globals.fs.file(
      globals.fs.path.join(buildPath, '$_appName-0.far'),
    ).createSync(recursive: true);
    return true;
  }
