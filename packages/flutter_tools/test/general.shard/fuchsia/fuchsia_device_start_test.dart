...
void main() {
  group('Fuchsia app start and stop: ', () {
    ...
    setUp(() {
      ...
      for (final BuildMode mode in <BuildMode>[BuildMode.debug, BuildMode.release]) {
        memoryFileSystem.file(
          artifacts.getArtifactPath(Artifact.fuchsiaKernelCompiler, platform: TargetPlatform.fuchsia_arm64, mode: mode),
        ).createSync();
        ...
        memoryFileSystem.file(
          artifacts.getArtifactPath(Artifact.flutterPatchedSdkPath, platform: TargetPlatform.fuchsia_arm64, mode: mode),
        ).createSync();

        memoryFileSystem.file(
          artifacts.getArtifactPath(Artifact.fuchsiaFlutterRunner, platform: TargetPlatform.fuchsia_arm64, mode: mode),
        ).createSync();
      }
    ...
    Future<LaunchResult> setupAndStartApp({required bool prebuilt, required BuildMode mode}) async {
      ...
      return device.startApp(app!, prebuiltApplication: prebuilt, debuggingOptions: debuggingOptions);
    }
    ...
    testUsingContext(
      ...
      () async {
        expect(() async => setupAndStartApp(prebuilt: false, mode: BuildMode.release), throwsToolExit(
    ...
    testUsingContext(
      ...
      () async {
        expect(() async => setupAndStartApp(prebuilt: false, mode: BuildMode.debug), throwsToolExit(
          message: 'This tool does not currently build apps for fuchsia.\n'
              'Build the app using a supported Fuchsia workflow.\n'
              'Then use the --use-application-binary flag.',
        ));
    ...
    testUsingContext(
      ...
      () async {
        expect(() async => setupAndStartApp(prebuilt: true, mode: BuildMode.release), throwsToolExit(
    ...
    testUsingContext(
      ...
      () async {
        expect(
          () async => FuchsiaDeviceWithFakeDiscovery('123').hostAddress,
    ...
    testUsingContext(
      'fail with correct LaunchResult when pkgctl fails',
      () async {
        final LaunchResult launchResult = await setupAndStartApp(prebuilt: true, mode: BuildMode.release);
        expect(launchResult.started, isFalse);
        expect(launchResult.hasVmService, isFalse);
...
Process _createFakeProcess({int exitCode = 0, String stdout = '', String stderr = '', bool persistent = false}) {
  final Stream<List<int>> stdoutStream = Stream<List<int>>.fromIterable(<List<int>>[utf8.encode(stdout)]);
  final Stream<List<int>> stderrStream = Stream<List<int>>.fromIterable(<List<int>>[utf8.encode(stderr)]);
...
class FakeFuchsiaDeviceTools implements FuchsiaDeviceTools {
  FakeFuchsiaDeviceTools({FuchsiaPkgctl? pkgctl, FuchsiaFfx? ffx})
...
class FakeFuchsiaSdk extends Fake implements FuchsiaSdk {
  FakeFuchsiaSdk({FuchsiaPM? pm, FuchsiaKernelCompiler? compiler, FuchsiaFfx? ffx})
