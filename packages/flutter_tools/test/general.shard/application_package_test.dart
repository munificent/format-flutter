...
void main() {
  group('Apk with partial Android SDK works', () {
    ...
    setUp(() async {
      ...
      fs
          .file(
            project.android.hostAppGradleRoot.childFile(
    ...
    testUsingContext('correct debug filename in module projects', () async {
      ...
      project.android.hostAppGradleRoot.childFile('build.gradle').createSync(
        recursive: true,
      );
    ...
    testUsingContext(
      ...
      () async {
        ...
        final Directory gradleWrapperDir = cache.getArtifactDirectory(
          'gradle_wrapper',
        );
        ...
        gradleWrapperDir.childFile('gradlew.bat').writeAsStringSync(
          'irrelevant',
        );
  ...
  group('PrebuiltIOSApp', () {
    ...
    testUsingContext('Error on non-existing file', () {
      final PrebuiltIOSApp? iosApp = IOSApp.fromPrebuiltApp(
        globals.fs.file('not_existing.ipa'),
      ) as PrebuiltIOSApp?;
    ...
    testUsingContext('Error on non-app-bundle folder', () {
      ...
      final PrebuiltIOSApp? iosApp = IOSApp.fromPrebuiltApp(
        globals.fs.file('regular_folder'),
      ) as PrebuiltIOSApp?;
      expect(iosApp, isNull);
      expect(
        testLogger.errorText,
    ...
    testUsingContext('Success with app bundle', () {
      ...
      expect(
        iosApp.applicationPackage.path,
    ...
    testUsingContext('Success with ipa', () {
      ...
      os.onUnzip = (File zipFile, Directory targetDirectory) {
        ...
        testPlistParser.setProperty('CFBundleIdentifier', 'fooBundleId');
        globals.fs.file(
          globals.fs.path.join(bundleAppDir.path, 'Info.plist'),
        ).createSync();
      };
      final PrebuiltIOSApp iosApp =
          IOSApp.fromPrebuiltApp(globals.fs.file('app.ipa'))! as PrebuiltIOSApp;
    ...
    testUsingContext(
      'returns null when there is no Runner.xcodeproj',
      () async {
        globals.fs.file('pubspec.yaml').createSync();
        globals.fs.file('.packages').createSync();
    ...
    testUsingContext(
      'returns null when there with no product identifier',
      () async {
        globals.fs.file('pubspec.yaml').createSync();
        globals.fs.file('.packages').createSync();
    ...
    testUsingContext('returns project app icon dirname', () async {
      final BuildableIOSApp iosApp = BuildableIOSApp(IosProject.fromFlutter(
    ...
    testUsingContext(
      'returns template app icon dirname for Contents.json',
      () async {
        final BuildableIOSApp iosApp = BuildableIOSApp(IosProject.fromFlutter(
          FlutterProject.fromDirectory(globals.fs.currentDirectory),
        ), 'com.foo.bar', 'Runner');
        ...
        expect(iosApp.templateAppIconDirNameForContentsJson, globals.fs.path
            .join(
              Cache.flutterRoot!,
              'packages',
              'flutter_tools',
...
''');
      ...
      expect(await iosApp.templateAppIconDirNameForImages, globals.fs.path
          .absolute(
    ...
    testUsingContext(
      'returns template launch image dirname for Contents.json',
      () async {
        final BuildableIOSApp iosApp = BuildableIOSApp(IosProject.fromFlutter(
          FlutterProject.fromDirectory(globals.fs.currentDirectory),
        ), 'com.foo.bar', 'Runner');
        ...
        expect(iosApp.templateLaunchImageDirNameForContentsJson, globals.fs.path
            .join(
              Cache.flutterRoot!,
              'packages',
              'flutter_tools',
    ...
    testUsingContext(
      'returns template launch image dirname for images',
      () async {
        final String toolsDir = globals.fs.path.join(
          Cache.flutterRoot!,
          'packages',
...
''');
        ...
        expect(await iosApp.templateLaunchImageDirNameForImages, globals.fs.path
            .absolute(
              'flutter_template_images',
              'templates',
              'app_shared',
  ...
  group('FuchsiaApp', () {
    ...
    testUsingContext('Error on non-existing file', () {
      final PrebuiltFuchsiaApp? fuchsiaApp = FuchsiaApp.fromPrebuiltApp(
        globals.fs.file('not_existing.far'),
    ...
    testUsingContext('Success with far file', () {
      ...
      final PrebuiltFuchsiaApp fuchsiaApp = FuchsiaApp.fromPrebuiltApp(
        globals.fs.file('bundle.far'),
      )! as PrebuiltFuchsiaApp;
      expect(testLogger.errorText, isEmpty);
      expect(fuchsiaApp.id, 'bundle.far');
      expect(
        fuchsiaApp.applicationPackage.path,
