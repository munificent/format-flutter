...
void main() {
  group('Apk with partial Android SDK works', () {
    ...
    setUp(() async {
      ...
      fs
          .file(
            project.android.hostAppGradleRoot.childFile(globals.platform.isWindows ? 'gradlew.bat' : 'gradlew').path,
          )
    ...
    testUsingContext('correct debug filename in module projects', () async {
      ...
      final File appGradle = project.android.hostAppGradleRoot.childFile(fs.path.join('app', 'build.gradle'));
    ...
    testUsingContext('Licenses available, build tools not, apk exists', () async {
      ...
      final FlutterProject project = FlutterProject.fromDirectoryTest(fs.currentDirectory);
      project.android.hostAppGradleRoot.childFile('gradle.properties').writeAsStringSync('irrelevant');

      final Directory gradleWrapperDir = cache.getArtifactDirectory('gradle_wrapper');

      gradleWrapperDir.fileSystem.directory(
        gradleWrapperDir.childDirectory('gradle').childDirectory('wrapper'),
      ).createSync(recursive: true);
  ...
  group('ApkManifestData', () {
    testWithoutContext(
      ...
      () {
        final ApkManifestData data =
            ApkManifestData.parseFromXmlDump(_aaptDataWithExplicitEnabledAndMainLauncherActivity, BufferLogger.test())!;
    ...
    testWithoutContext(
      ...
      () {
        final ApkManifestData data =
            ApkManifestData.parseFromXmlDump(_aaptDataWithDefaultEnabledAndMainLauncherActivity, BufferLogger.test())!;
    ...
    testWithoutContext('Parses manifest with a dist namespace', () {
      final ApkManifestData data = ApkManifestData.parseFromXmlDump(_aaptDataWithDistNamespace, BufferLogger.test())!;
    ...
    testWithoutContext(
      ...
      () {
        final BufferLogger logger = BufferLogger.test();
        final ApkManifestData? data = ApkManifestData.parseFromXmlDump(_aaptDataWithNoEnabledActivity, logger);
    ...
    testWithoutContext(
      ...
      () {
        final BufferLogger logger = BufferLogger.test();
        final ApkManifestData? data = ApkManifestData.parseFromXmlDump(_aaptDataWithNoMainActivity, logger);
    ...
    testWithoutContext(
      ...
      () {
        final BufferLogger logger = BufferLogger.test();
        final ApkManifestData? data = ApkManifestData.parseFromXmlDump(_aaptDataWithNoLauncherActivity, logger);
    ...
    testWithoutContext(
      ...
      () {
        final ApkManifestData data =
            ApkManifestData.parseFromXmlDump(_aaptDataWithLauncherAndDefaultActivity, BufferLogger.test())!;
    ...
    testWithoutContext('Parses manifest with missing application tag', () async {
      final ApkManifestData? data = ApkManifestData.parseFromXmlDump(_aaptDataWithoutApplication, BufferLogger.test());
  ...
  group('PrebuiltIOSApp', () {
    ...
    testUsingContext('Error on non-existing file', () {
      final PrebuiltIOSApp? iosApp = IOSApp.fromPrebuiltApp(globals.fs.file('not_existing.ipa')) as PrebuiltIOSApp?;
      expect(iosApp, isNull);
      expect(testLogger.errorText, 'File "not_existing.ipa" does not exist. Use an app bundle or an ipa.\n');
    }, overrides: overrides);

    testUsingContext('Error on non-app-bundle folder', () {
      globals.fs.directory('regular_folder').createSync();
      final PrebuiltIOSApp? iosApp = IOSApp.fromPrebuiltApp(globals.fs.file('regular_folder')) as PrebuiltIOSApp?;
      expect(iosApp, isNull);
    ...
    testUsingContext('Bad ipa zip-file, no payload dir', () {
      ...
      expect(iosApp, isNull);
      expect(testLogger.errorText, 'Invalid prebuilt iOS ipa. Does not contain a "Payload" directory.\n');
    }, overrides: overrides);
    ...
    testUsingContext('Bad ipa zip-file, two app bundles', () {
      ...
      os.onUnzip = (File zipFile, Directory targetDirectory) {
        ...
        final String bundlePath1 = globals.fs.path.join(targetDirectory.path, 'Payload', 'bundle1.app');
        final String bundlePath2 = globals.fs.path.join(targetDirectory.path, 'Payload', 'bundle2.app');
        globals.fs.directory(bundlePath1).createSync(recursive: true);
        globals.fs.directory(bundlePath2).createSync(recursive: true);
      };
      final PrebuiltIOSApp? iosApp = IOSApp.fromPrebuiltApp(globals.fs.file('app.ipa')) as PrebuiltIOSApp?;
      expect(iosApp, isNull);
      expect(testLogger.errorText, 'Invalid prebuilt iOS ipa. Does not contain a single app bundle.\n');
    }, overrides: overrides);
    ...
    testUsingContext('returns project app icon dirname', () async {
      ...
      final String iconDirSuffix = globals.fs.path.join('Runner', 'Assets.xcassets', 'AppIcon.appiconset');
    ...
    testUsingContext('returns template app icon dirname for Contents.json', () async {
      ...
      final String iconDirSuffix = globals.fs.path.join('Runner', 'Assets.xcassets', 'AppIcon.appiconset');
      expect(iosApp.templateAppIconDirNameForContentsJson, globals.fs.path.join(
        Cache.flutterRoot!,
        'packages',
        'flutter_tools',
        'templates',
    ...
    testUsingContext('returns template app icon dirname for images', () async {
      ...
      final String packageConfigPath = globals.fs.path.join(toolsDir, '.dart_tool', 'package_config.json');
...
''');
      ...
      expect(
        ...
        globals.fs.path.absolute('flutter_template_images', 'templates', 'app_shared', 'ios.tmpl', iconDirSuffix),
      );
    }, overrides: overrides);
    ...
    testUsingContext('returns project launch image dirname', () async {
      ...
      final String launchImageDirSuffix = globals.fs.path.join('Runner', 'Assets.xcassets', 'LaunchImage.imageset');
    ...
    testUsingContext('returns template launch image dirname for Contents.json', () async {
      ...
      final String launchImageDirSuffix = globals.fs.path.join('Runner', 'Assets.xcassets', 'LaunchImage.imageset');
      expect(iosApp.templateLaunchImageDirNameForContentsJson, globals.fs.path.join(
        Cache.flutterRoot!,
        'packages',
        'flutter_tools',
        'templates',
...
''');
      ...
      final String launchImageDirSuffix = globals.fs.path.join('Runner', 'Assets.xcassets', 'LaunchImage.imageset');
      expect(await iosApp.templateLaunchImageDirNameForImages, globals.fs.path.absolute(
  ...
  group('FuchsiaApp', () {
    ...
    testUsingContext('Error on non-existing file', () {
      ...
      expect(testLogger.errorText, 'File "not_existing.far" does not exist or is not a .far file. Use far archive.\n');
    }, overrides: overrides);
    ...
    testUsingContext('Error on non-far file', () {
      ...
      expect(fuchsiaApp, isNull);
      expect(testLogger.errorText, 'File "regular_folder" does not exist or is not a .far file. Use far archive.\n');
    }, overrides: overrides);

    testUsingContext('Success with far file', () {
      globals.fs.file('bundle.far').createSync();
      final PrebuiltFuchsiaApp fuchsiaApp =
          FuchsiaApp.fromPrebuiltApp(globals.fs.file('bundle.far'))! as PrebuiltFuchsiaApp;
      expect(testLogger.errorText, isEmpty);
