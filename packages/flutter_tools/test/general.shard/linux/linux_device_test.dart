...
void main() {
  ...
  testWithoutContext(
    'LinuxDevice.isSupportedForProject is false with no host app',
    () async {
      final FileSystem fileSystem = MemoryFileSystem.test();
      fileSystem.file('pubspec.yaml').createSync();
      fileSystem.file('.packages').createSync();
  ...
  testWithoutContext(
    ...
    () async {
      ...
      expect(
        device.executablePathForDevice(mockApp, BuildInfo.debug),
        'debug/executable',
      );
      expect(
        device.executablePathForDevice(mockApp, BuildInfo.profile),
        'profile/executable',
      );
      expect(
        device.executablePathForDevice(mockApp, BuildInfo.release),
        'release/executable',
      );
    },
  );
