...
void main() {
  ...
  testWithoutContext('DevFSFileContent', () async {
    ...
    final DateTime fiveSecondsAgo = file.statSync().modified.subtract(
  ...
  testWithoutContext(
    'DevFS destroy is resilient to vmservice disconnection',
    () async {
      final FileSystem fileSystem = MemoryFileSystem.test();
      final OperatingSystemUtils osUtils = FakeOperatingSystemUtils();
      final FakeVmServiceHost fakeVmServiceHost = FakeVmServiceHost(
  ...
  testWithoutContext('DevFS can reset compilation time', () async {
    ...
    final LocalDevFSWriter localDevFSWriter = LocalDevFSWriter(
      fileSystem: fileSystem,
    );
  ...
  testWithoutContext('Local DevFSWriter can copy and write files', () async {
    ...
    expect(
      fileSystem.file('/foo/bar/devfs/goodbye').readAsStringSync(),
      'goodbye',
    );
  });
  ...
  testUsingContext(
    'DevFS actually starts compile before processing bundle',
    () async {
      final FileSystem fileSystem = MemoryFileSystem.test();
      final FakeVmServiceHost fakeVmServiceHost = FakeVmServiceHost(
        requests: <VmServiceExpectation>[createDevFSRequest],
      ...
      final int processingBundleIndex = logger.messages.indexOf(
        'Processing bundle.',
      );
      final int bundleProcessingDoneIndex = logger.messages.indexOf(
