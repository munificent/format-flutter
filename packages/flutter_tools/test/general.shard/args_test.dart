...
void verifyCommand(Command<Object?> runner) {
  ...
  if (!runner.hidden && runner.parent == null) {
    expect(
      runner.category,
      anyOf(FlutterCommandCategory.sdk, FlutterCommandCategory.project, FlutterCommandCategory.tools),
...
void verifyOptions(String? command, Iterable<Option> options) {
  ...
  for (final Option option in options) {
    ...
    expect(
      option.help,
      isNot(endsWith(':')),
      reason: '${_header}Help for $target--${option.name}" ends with a colon, which seems unlikely to be correct.',
    );
    expect(
      option.help,
