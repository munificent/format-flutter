...
void main() {
  ...
  test(
    ...
    () => Testbed().run(() {
      ...
      executable.generateCommands(verboseHelp: true, verbose: true).forEach(
        runner.addCommand,
      );
  ...
  testUsingContext(
    'Global arg results are available in FlutterCommands',
    () async {
  ...
  testUsingContext('String? safe argResults', () async {
    ...
    expect(command.stringArg('key'), 'value');
    expect(
      () => command.stringArg('non-existent'),
      throwsA(const TypeMatcher<ArgumentError>()),
    );
  });
  ...
  testUsingContext('List<String> safe argResults', () async {
    ...
    expect(
      () => command.stringsArg('non-existent'),
      throwsA(const TypeMatcher<ArgumentError>()),
    );
...
void verifyCommandRunner(CommandRunner<Object?> runner) {
  ...
  expect(
    runner.argParser.allowsAnything,
    isFalse,
...
final RegExp _allowedArgumentNamePatternForPrecache = RegExp(
  r'^([-a-z0-9_]+)$',
);
...
final RegExp _allowedTrailingPatterns = RegExp(
  r'([^ ]([^.^!^:][.!:])\)?|: https?://[^ ]+[^.]|^)$',
);
