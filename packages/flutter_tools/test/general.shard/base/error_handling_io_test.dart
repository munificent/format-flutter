...
void main() {
  ...
  group('throws ToolExit on Windows', () {
    ...
    testWithoutContext(
      ...
      () {
        ...
        exceptionHandler.addError(file, FileSystemOp.write, FileSystemException(
          '',
          file.path,
          const OSError('', kUserPermissionDenied),
        ));
    ...
    testWithoutContext('when access is denied', () async {
      ...
      exceptionHandler.addError(file, FileSystemOp.open, FileSystemException(
        '',
        file.path,
        const OSError('', kUserPermissionDenied),
      ));
      ...
      expect(
        () async => file.writeAsBytes(<int>[0]),
        throwsToolExit(message: expectedMessage),
      );
      expect(
        () async => file.writeAsString(''),
        throwsToolExit(message: expectedMessage),
      );
      expect(
        () => file.writeAsBytesSync(<int>[0]),
        throwsToolExit(message: expectedMessage),
      );
      expect(
        () => file.writeAsStringSync(''),
    ...
    testWithoutContext(
      ...
      () async {
        ...
        exceptionHandler.addError(file, FileSystemOp.write, FileSystemException(
          '',
          file.path,
          const OSError('', kUserMappedSectionOpened),
        ));
    ...
    testWithoutContext('when the device driver has a fatal error', () async {
      ...
      exceptionHandler.addError(file, FileSystemOp.write, FileSystemException(
        '',
        file.path,
        const OSError('', kFatalDeviceHardwareError),
      ));
      exceptionHandler.addError(file, FileSystemOp.open, FileSystemException(
        '',
        file.path,
    ...
    testWithoutContext('when the device does not exist', () async {
      ...
      exceptionHandler.addError(file, FileSystemOp.write, FileSystemException(
        '',
        file.path,
        const OSError('', kDeviceDoesNotExist),
      ));
      exceptionHandler.addError(file, FileSystemOp.open, FileSystemException(
        '',
        file.path,
    ...
    testWithoutContext(
      ...
      () async {
        ...
        exceptionHandler.addTempError(FileSystemOp.create, FileSystemException(
          '',
          directory.path,
          const OSError('', kDeviceFull),
        ));

        const String expectedMessage = 'The target device is full';
        expect(
          () async => directory.createTemp('prefix'),
          throwsToolExit(message: expectedMessage),
        );
        expect(
    ...
    testWithoutContext('When reading from a file without permission', () {
      ...
      exceptionHandler.addError(file, FileSystemOp.read, FileSystemException(
        '',
        file.path,
        const OSError('', kUserPermissionDenied),
      ));
    ...
    testWithoutContext(
      ...
      () {
        ...
        expect(() => fileSystem.currentDirectory, throwsToolExit(
  ...
  group('throws ToolExit on Linux', () {
    ...
    testWithoutContext('when access is denied', () async {
      ...
      expect(() async => file.createSync(recursive: true), throwsA(
        isA<ToolExit>().having(
      ...
      expect(
        () => file.readAsStringSync(),
        throwsToolExit(message: readMessage),
      );
    });
    ...
    testWithoutContext('when access is denied for directories', () async {
      ...
      // Recursive does not contain the "sudo chown" suggestion.
      expect(() async => directory.createSync(recursive: true), throwsA(
        isA<ToolExit>().having(
          (ToolExit e) => e.message,
          'message',
          isNot(contains('sudo chown')),
      ...
      expect(
        () async => directory.delete(),
        throwsToolExit(message: deleteMessage),
      );

      // Recursive does not contain the "sudo chown" suggestion.
      expect(() async => directory.deleteSync(recursive: true), throwsA(
        isA<ToolExit>().having(
          (ToolExit e) => e.message,
          'message',
          isNot(contains('sudo chown')),
    ...
    testWithoutContext('Rethrows os error $kSystemCannotFindFile', () {
      ...
      exceptionHandler.addError(file, FileSystemOp.read, FileSystemException(
        '',
        file.path,
        const OSError('', kSystemCannotFindFile),
      ));
  ...
  group('toString() gives toString() of delegate', () {
    ...
    testWithoutContext('ErrorHandlingDirectory', () {
      ...
      expect(
        fs.currentDirectory.toString(),
        delegate.currentDirectory.toString(),
      );
