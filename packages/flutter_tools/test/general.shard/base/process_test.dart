...
void main() {
  ...
  group('output formatting', () {
    ...
    testWithoutContext('Command output is not wrapped.', () async {
      ...
      processManager.addCommand(
        FakeCommand(command: const <String>['command'], stdout: testString.join(), stderr: testString.join()),
      );
    ...
    testWithoutContext('Command output is filtered by mapFunction', () async {
      processManager.addCommand(
        const FakeCommand(command: <String>['command'], stdout: 'match\nno match', stderr: 'match\nno match'),
      );
  ...
  group('run', () {
    ...
    testWithoutContext(' does not throw on allowed Failures', () async {
      fakeProcessManager.addCommand(const FakeCommand(command: <String>['kaboom'], exitCode: 1));
      expect(
        (await processUtils.run(<String>['kaboom'], throwOnError: true, allowedFailures: (int c) => c == 1)).exitCode,
        1,
      );
    });
    ...
    testWithoutContext(' throws on disallowed failure', () async {
      ...
      expect(
        () => processUtils.run(<String>['kaboom'], throwOnError: true, allowedFailures: (int c) => c == 1),
  ...
  group('runSync', () {
    ...
    testWithoutContext('throws on failure with throwOnError', () async {
      ...
      fakeProcessManager.addCommand(const FakeCommand(command: <String>['kaboom'], exitCode: 1, stderr: stderr));
      expect(() => processUtils.runSync(<String>['kaboom'], throwOnError: true), throwsA(
        isA<ProcessException>().having((ProcessException error) => error.message, 'message', isNot(contains(stderr))),
      ));
    });

    testWithoutContext('throws with stderr in exception on failure with verboseExceptions', () async {
      const String stderr = 'Something went wrong.';
      fakeProcessManager.addCommand(const FakeCommand(command: <String>['verybad'], exitCode: 1, stderr: stderr));
      expect(
        () => processUtils.runSync(<String>['verybad'], throwOnError: true, verboseExceptions: true),
    ...
    testWithoutContext(' does not throw on allowed Failures', () async {
      fakeProcessManager.addCommand(const FakeCommand(command: <String>['kaboom'], exitCode: 1));
      expect(
        processUtils.runSync(<String>['kaboom'], throwOnError: true, allowedFailures: (int c) => c == 1).exitCode,
        1,
      );
    });
    ...
    testWithoutContext(' throws on disallowed failure', () async {
      fakeProcessManager.addCommand(const FakeCommand(command: <String>['kaboom'], exitCode: 2));
      expect(
        () => processUtils.runSync(<String>['kaboom'], throwOnError: true, allowedFailures: (int c) => c == 1),
        throwsProcessException(),
      );
    });
    ...
    testWithoutContext(' prints stdout to status and stderr to error on failure with throwOnError', () async {
      fakeProcessManager.addCommand(
        const FakeCommand(command: <String>['kaboom'], exitCode: 1, stdout: 'stdout', stderr: 'stderr'),
      );
