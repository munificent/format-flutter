...
void main() {
  ...
  group('which on POSIX', () {
    testWithoutContext('returns null when executable does not exist', () async {
      fakeProcessManager.addCommand(const FakeCommand(command: <String>['which', kExecutable], exitCode: 1));
    ...
    testWithoutContext('returns exactly one result', () async {
      fakeProcessManager.addCommand(const FakeCommand(command: <String>['which', 'foo'], stdout: kPath1));
      final OperatingSystemUtils utils = createOSUtils(FakePlatform());
    ...
    testWithoutContext('returns all results for whichAll', () async {
      fakeProcessManager.addCommand(
        const FakeCommand(command: <String>['which', '-a', kExecutable], stdout: '$kPath1\n$kPath2'),
      );
      final OperatingSystemUtils utils = createOSUtils(FakePlatform());
  ...
  group('which on Windows', () {
    ...
    testWithoutContext('returns null when executable does not exist', () async {
      fakeProcessManager.addCommand(const FakeCommand(command: <String>['where', kExecutable], exitCode: 1));
    ...
    testWithoutContext('returns exactly one result', () async {
      fakeProcessManager.addCommand(const FakeCommand(command: <String>['where', 'foo'], stdout: '$kPath1\n$kPath2'));
      final OperatingSystemUtils utils = createOSUtils(FakePlatform(operatingSystem: 'windows'));
      expect(utils.which(kExecutable)!.path, kPath1);
    });
  ...
  group('host platform', () {
    testWithoutContext('unknown defaults to Linux', () async {
      fakeProcessManager.addCommand(const FakeCommand(command: <String>['uname', '-m'], stdout: 'x86_64'));
    ...
    testWithoutContext('Linux ARM', () async {
      fakeProcessManager.addCommand(const FakeCommand(command: <String>['uname', '-m'], stdout: 'aarch64'));
    ...
    testWithoutContext('Linux name', () async {
      ...
      final OperatingSystemUtils utils = OperatingSystemUtils(
        ...
        logger: BufferLogger.test(),
        platform: FakePlatform(operatingSystemVersion: 'Linux 1.2.3-abcd #1 SMP PREEMPT Sat Jan 1 00:00:00 UTC 2000'),
        processManager: fakeProcessManager,
      );
    ...
    testWithoutContext('Windows validates paths when unzipping', () {
      ...
      expect(() => utils.unzip(fakeZipFile, targetDirectory), throwsA(isA<StateError>().having(
  ...
  group('display an install message when unzip cannot be run', () {
    testWithoutContext('Linux', () {
      ...
      expect(() => linuxOsUtils.unzip(fileSystem.file('foo.zip'), fileSystem.currentDirectory), throwsToolExit(
    ...
    testWithoutContext('macOS', () {
      ...
      expect(() => macOSUtils.unzip(fileSystem.file('foo.zip'), fileSystem.currentDirectory), throwsToolExit(
        message: 'Missing "unzip" tool. Unable to extract foo.zip.\n'
    ...
    testWithoutContext('unknown OS', () {
      ...
      expect(() => unknownOsUtils.unzip(fileSystem.file('foo.zip'), fileSystem.currentDirectory), throwsToolExit(
        message: 'Missing "unzip" tool. Unable to extract foo.zip.\n'
