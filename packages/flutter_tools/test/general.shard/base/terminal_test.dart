...
void main() {
  ...
  group('ANSI coloring, bold, and clearing', () {
    ...
    testWithoutContext('adding bold works', () {
      expect(terminal.bolden('output'), equals('${AnsiTerminal.bold}output${AnsiTerminal.resetBold}'));
    });
    ...
    testWithoutContext('nesting bold within bold works', () {
      expect(terminal.bolden(terminal.bolden('output')), equals('${AnsiTerminal.bold}output${AnsiTerminal.resetBold}'));
      expect(
  ...
  group('character input prompt', () {
    ...
    testWithoutContext('character prompt throws if usesTerminalUi is false', () async {
      expect(terminalUnderTest.promptForCharInput(<String>[
        'a',
        'b',
        'c',
      ], prompt: 'Please choose something', logger: BufferLogger.test()), throwsStateError);
    });
    ...
    testWithoutContext('character prompt', () async {
      ...
      final String choice = await terminalUnderTest
          .promptForCharInput(<String>['a', 'b', 'c'], prompt: 'Please choose something', logger: bufferLogger);
      expect(choice, 'b');
      expect(
  ...
  testWithoutContext('AnsiTerminal.preferredStyle', () {
    ...
    expect(
      AnsiTerminal(stdio: stdio, platform: const LocalPlatform(), now: DateTime(2018, 1, 10, 1)).preferredStyle,
      2,
    );
    expect(
      AnsiTerminal(stdio: stdio, platform: const LocalPlatform(), now: DateTime(2018, 1, 11, 1)).preferredStyle,
      3,
    );

    expect(
      AnsiTerminal(stdio: stdio, platform: const LocalPlatform(), now: DateTime(2018, 1, 1, 23)).preferredStyle,
      0,
    );
    expect(
      AnsiTerminal(stdio: stdio, platform: const LocalPlatform(), now: DateTime(2018, 1, 2, 23)).preferredStyle,
      1,
    );
    expect(
      AnsiTerminal(stdio: stdio, platform: const LocalPlatform(), now: DateTime(2018, 1, 3, 23)).preferredStyle,
      2,
    );
    expect(
      AnsiTerminal(stdio: stdio, platform: const LocalPlatform(), now: DateTime(2018, 1, 4, 23)).preferredStyle,
      3,
    );
    expect(
      AnsiTerminal(stdio: stdio, platform: const LocalPlatform(), now: DateTime(2018, 1, 5, 23)).preferredStyle,
      4,
    );
    expect(
      AnsiTerminal(stdio: stdio, platform: const LocalPlatform(), now: DateTime(2018, 1, 6, 23)).preferredStyle,
      28,
    );
    expect(
      AnsiTerminal(stdio: stdio, platform: const LocalPlatform(), now: DateTime(2018, 1, 7, 23)).preferredStyle,
      28,
    );
    expect(
      AnsiTerminal(stdio: stdio, platform: const LocalPlatform(), now: DateTime(2018, 1, 8, 23)).preferredStyle,
      0,
    );
    expect(
      AnsiTerminal(stdio: stdio, platform: const LocalPlatform(), now: DateTime(2018, 1, 9, 23)).preferredStyle,
      1,
    );
    expect(
      AnsiTerminal(stdio: stdio, platform: const LocalPlatform(), now: DateTime(2018, 1, 10, 23)).preferredStyle,
      2,
    );
    expect(
      AnsiTerminal(stdio: stdio, platform: const LocalPlatform(), now: DateTime(2018, 1, 11, 23)).preferredStyle,
      3,
    );
  });
}
