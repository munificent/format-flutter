...
void main() {
  ...
  group('successful fetch', () {
    ...
    testWithoutContext('fetchUrl() gets the data', () async {
      final Net net = createNet(FakeHttpClient.list(<FakeRequest>[
        FakeRequest(Uri.parse('http://example.invalid/'), response: FakeResponse(body: utf8.encode(responseString))),
      ]));
    ...
    testWithoutContext('fetchUrl(destFile) writes the data to a file', () async {
      ...
      final List<int>? data = await net.fetchUrl(Uri.parse('http://example.invalid/'), destFile: destFile);
  ...
  testWithoutContext('check for bad override on ArgumentError', () async {
    ...
    final Net net = Net(
      httpClientFactory: () {
        return FakeHttpClient.list(<FakeRequest>[FakeRequest(invalid, responseError: ArgumentError())]);
      },
      logger: testLogger,
      platform: FakePlatform(environment: <String, String>{'FLUTTER_STORAGE_BASE_URL': 'example.invalid'}),
    );
    String? error;
    FakeAsync().run((FakeAsync time) {
  ...
  testWithoutContext('max attempts', () async {
    final Uri invalid = Uri.parse('http://example.invalid/');
    final Net net = createNet(FakeHttpClient.list(<FakeRequest>[
      FakeRequest(invalid, response: const FakeResponse(statusCode: HttpStatus.internalServerError)),
      FakeRequest(invalid, response: const FakeResponse(statusCode: HttpStatus.internalServerError)),
      FakeRequest(invalid, response: const FakeResponse(statusCode: HttpStatus.internalServerError)),
    ]));
    String? error;
  ...
  testWithoutContext('remote file non-existent', () async {
    final Uri invalid = Uri.parse('http://example.invalid/');
    final Net net = createNet(FakeHttpClient.list(<FakeRequest>[
      FakeRequest(invalid, method: HttpMethod.head, response: const FakeResponse(statusCode: HttpStatus.notFound)),
    ]));
  ...
  testWithoutContext('remote file exists', () async {
    final Uri valid = Uri.parse('http://example.valid/');
    final Net net = createNet(FakeHttpClient.list(<FakeRequest>[FakeRequest(valid, method: HttpMethod.head)]));
    final bool result = await net.doesRemoteFileExist(valid);
    expect(result, true);
  });
}
