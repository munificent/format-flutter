...
void main() {
  ...
  testWithoutContext(
    ...
    () {
      ...
      fileSystem.file(
        fileSystem.path.join('', 'resources', 'app', 'package.json'),
      )
  ...
  testWithoutContext('can locate VS Code installed via Snap', () {
    ...
    fileSystem.directory(
      fileSystem.path.join('/snap/code/current/', '.vscode'),
    ).createSync(recursive: true);
    ...
    final List<VsCode> installed = VsCode.allInstalled(
      fileSystem,
      platform,
      processManager,
    );
  ...
  testWithoutContext('can locate installations on macOS', () {
    ...
    fileSystem.directory(
      fileSystem.path.join(randomLocation, 'Contents'),
    ).createSync(recursive: true);
    ...
    fileSystem.directory(
      fileSystem.path.join(randomInsidersLocation, 'Contents'),
    ).createSync(recursive: true);
