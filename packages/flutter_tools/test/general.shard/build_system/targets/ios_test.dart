...
void main() {
  ...
  testUsingContext(
    ...
    () async {
      ...
      processManager.addCommands(<FakeCommand>[
        ...
        FakeCommand(command: <String>['xattr', '-r', '-d', 'com.apple.FinderInfo', appFrameworkPath]),
        FakeCommand(command: <String>['codesign', '--force', '--sign', '-', '--timestamp=none', appFrameworkPath]),
      ]);
  ...
  testUsingContext(
    ...
    () async {
      ...
      processManager.addCommands(<FakeCommand>[
        FakeCommand(command: <String>['xattr', '-r', '-d', 'com.apple.FinderInfo', frameworkDirectoryBinary.path]),
        FakeCommand(
          command: <String>[
            'codesign',
            '--force',
  ...
  testUsingContext(
    ...
    () async {
      ...
      processManager.addCommands(<FakeCommand>[
        FakeCommand(command: <String>['xattr', '-r', '-d', 'com.apple.FinderInfo', frameworkDirectoryBinary.path]),
        FakeCommand(command: <String>['codesign', '--force', '--sign', 'ABC123', frameworkDirectoryBinary.path]),
      ]);
  ...
  testUsingContext(
    ...
    () async {
      ...
      processManager.addCommands(<FakeCommand>[
        FakeCommand(command: <String>['xattr', '-r', '-d', 'com.apple.FinderInfo', frameworkDirectoryBinary.path]),
        FakeCommand(command: <String>['codesign', '--force', '--sign', '-', frameworkDirectoryBinary.path]),
      ]);
  ...
  group('copies Flutter.framework', () {
    ...
    setUp(() {
      ...
      lipoCommandNonFatResult = FakeCommand(command: <String>['lipo', '-info', binary.path], stdout: 'Non-fat file:');

      lipoVerifyArm64Command = FakeCommand(command: <String>['lipo', binary.path, '-verify_arch', 'arm64']);

      xattrCommand = FakeCommand(command: <String>['xattr', '-r', '-d', 'com.apple.FinderInfo', binary.path]);
    ...
    testWithoutContext('fails when requested archs missing from framework', () async {
      ...
      final Environment environment = Environment.test(
        ...
        fileSystem: fileSystem,
        outputDir: outputDir,
        defines: <String, String>{kIosArchs: 'arm64 armv7', kSdkRoot: 'path/to/iPhoneOS.sdk'},
      );

      processManager.addCommands(<FakeCommand>[
        copyPhysicalFrameworkCommand,
        FakeCommand(command: <String>['lipo', '-info', binary.path], stdout: 'Architectures in the fat file:'),
        FakeCommand(command: <String>['lipo', binary.path, '-verify_arch', 'arm64', 'armv7'], exitCode: 1),
      ]);
    ...
    testWithoutContext('fails when lipo extract fails', () async {
      ...
      processManager.addCommands(<FakeCommand>[
        copyPhysicalFrameworkCommand,
        FakeCommand(command: <String>['lipo', '-info', binary.path], stdout: 'Architectures in the fat file:'),
        FakeCommand(command: <String>['lipo', binary.path, '-verify_arch', 'arm64', 'armv7']),
        FakeCommand(
          command: <String>['lipo', '-output', binary.path, '-extract', 'arm64', '-extract', 'armv7', binary.path],
          exitCode: 1,
          stderr: 'lipo error',
        ),
      ]);
      ...
      await expectLater(
        const DebugUnpackIOS().build(environment),
        throwsA(isException.having((Exception exception) => exception.toString(), 'description', contains(
    ...
    testWithoutContext('fails when bitcode strip fails', () async {
      ...
      processManager.addCommands(<FakeCommand>[
        ...
        lipoVerifyArm64Command,
        FakeCommand(
          command: <String>['xcrun', 'bitcode_strip', binary.path, '-r', '-o', binary.path],
          exitCode: 1,
          stderr: 'bitcode_strip error',
        ),
      ]);
    ...
    testWithoutContext('fails when codesign fails', () async {
      ...
      final Environment environment = Environment.test(
        ...
        fileSystem: fileSystem,
        outputDir: outputDir,
        defines: <String, String>{kIosArchs: 'arm64', kSdkRoot: 'path/to/iPhoneOS.sdk', kCodesignIdentity: 'ABC123'},
      );
      ...
      processManager.addCommands(<FakeCommand>[
        ...
        xattrCommand,
        FakeCommand(
          command: <String>['codesign', '--force', '--sign', 'ABC123', '--timestamp=none', binary.path],
          exitCode: 1,
    ...
    testWithoutContext('codesigns framework', () async {
      ...
      processManager.addCommands(<FakeCommand>[
        ...
        lipoVerifyArm64Command,
        xattrCommand,
        FakeCommand(command: <String>['codesign', '--force', '--sign', 'ABC123', '--timestamp=none', binary.path]),
      ]);
      await const DebugUnpackIOS().build(environment);
