...
void main() {
  ...
  test(
    ...
    () => testbed.run(() async {
      ...
      final Directory webResources = environment.projectDir.childDirectory(
        'web',
      );
      webResources.childFile('index.html').createSync(recursive: true);
      environment.buildDir.childFile('main.dart.js').createSync();
      await const WebReleaseBundle(WebRendererMode.auto, isWasm: false).build(
        environment,
      );
  ...
  test(
    ...
    () => testbed.run(() async {
      ...
      final String versionFile =
          environment.outputDir.childFile('version.json').readAsStringSync();
...
''');
      ...
      expect(
        environment.outputDir.childFile('main.dart.js').existsSync(),
        true,
      );
      expect(
        environment.outputDir.childDirectory('assets').childFile(
          'AssetManifest.json',
        ).existsSync(),
        true,
      );
  ...
  test(
    ...
    () => testbed.run(
      () async {
        ...
        expect(
          generated,
          contains("import 'file:///other/lib/main.dart' as entrypoint;"),
        );
      },
      overrides: <Type, Generator>{
        TemplateRenderer: () => const MustacheTemplateRenderer(),
      ...
      () async {
        final File mainFile =
            globals.fs.file(globals.fs.path.join('foo', 'lib', 'main.dart'))
              ..createSync(recursive: true)
              ..writeAsStringSync('void main() {}');
        globals.fs.file(globals.fs.path.join('pubspec.yaml')).writeAsStringSync(
          'name: foo\n',
        );
        environment.defines[kTargetFile] = mainFile.path;
        await const WebEntrypointTarget().build(environment);
  ...
  test(
    ...
    () => testbed.run(() async {
      ...
      processManager.addCommand(FakeCommand(
        ...
        onRun: () {
          environment.buildDir.childFile('app.dill.deps').writeAsStringSync(
            'file:///a.dart',
          );
        },
      ));
  ...
  test(
    ...
    () => testbed.run(() async {
      environment.defines[kBuildMode] = 'release';
      environment.defines[kDartDefines] = encodeDartDefines(<String>[
        'FOO=bar',
        'BAZ=qux',
      ]);
      processManager.addCommand(FakeCommand(
  ...
  test(
    ...
    () => testbed.run(() async {
      environment.defines[kBuildMode] = 'profile';
      environment
              .defines[JsCompilerConfig.kDart2jsNoFrequencyBasedMinification] =
          'true';
      processManager.addCommand(FakeCommand(
        command: <String>[
          ..._kDart2jsLinuxArgs,
  ...
  test(
    ...
    () => testbed.run(() async {
      ...
      final File outputJsFile = environment.buildDir.childFile(
        'main.dart.unopt.mjs',
      );
      processManager.addCommand(FakeCommand(
        command: <String>[
  ...
  test(
    ...
    () => testbed.run(() async {
      environment.outputDir.childDirectory('a').childFile('a.txt')
        ..createSync(recursive: true)
        ..writeAsStringSync('A');
      await WebServiceWorker(globals.fs, WebRendererMode.auto, isWasm: false)
          .build(environment);
  ...
  test(
    ...
    () => testbed.run(() async {
      environment.outputDir.childFile('main.dart.js').createSync(
        recursive: true,
      );
      environment.outputDir.childFile('main.dart.js.map').createSync(
        recursive: true,
      );
      await WebServiceWorker(globals.fs, WebRendererMode.auto, isWasm: false)
          .build(environment);
  ...
  test(
    ...
    () => testbed.run(() async {
      ...
      await WebBuiltInAssets(globals.fs, WebRendererMode.auto, isWasm: false)
          .build(environment);
  ...
  test(
    ...
    () => testbed.run(() async {
      ...
      await WebBuiltInAssets(globals.fs, WebRendererMode.auto, isWasm: true)
          .build(environment);
      ...
      expect(
        environment.outputDir.childDirectory('canvaskit').childFile(
          'canvaskit.wasm',
        ).existsSync(),
        true,
      );
