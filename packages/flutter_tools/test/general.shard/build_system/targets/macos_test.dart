...
void main() {
  ...
  setUp(() {
    ...
    environment = Environment.test(
      ...
      defines: <String, String>{kBuildMode: 'debug', kTargetPlatform: 'darwin', kDarwinArchs: 'x86_64'},
    ...
    lipoInfoNonFatCommand = FakeCommand(command: <String>['lipo', '-info', binary.path], stdout: 'Non-fat file:');
    ...
    lipoVerifyX86_64Command = FakeCommand(command: <String>['lipo', binary.path, '-verify_arch', 'x86_64']);
  });

  testUsingContext('Copies files to correct cache directory', () async {
    binary.createSync(recursive: true);
    processManager.addCommands(<FakeCommand>[copyFrameworkCommand, lipoInfoNonFatCommand, lipoVerifyX86_64Command]);

    await const DebugUnpackMacOS().build(environment);

    expect(processManager, hasNoRemainingExpectations);
  }, overrides: <Type, Generator>{FileSystem: () => fileSystem, ProcessManager: () => processManager});
  ...
  testUsingContext('lipo fails when arch missing from framework', () async {
    ...
    processManager.addCommands(<FakeCommand>[
      ...
      lipoInfoFatCommand,
      FakeCommand(command: <String>['lipo', binary.path, '-verify_arch', 'arm64', 'x86_64'], exitCode: 1),
    ]);
  ...
  testUsingContext('thins fat framework', () async {
    ...
    processManager.addCommands(<FakeCommand>[
      ...
      lipoInfoFatCommand,
      lipoVerifyX86_64Command,
      FakeCommand(command: <String>['lipo', '-output', binary.path, '-extract', 'x86_64', binary.path]),
    ]);
  ...
  testUsingContext('debug macOS application fails if App.framework missing', () async {
    fileSystem.directory(artifacts.getArtifactPath(Artifact.flutterMacOSFramework, mode: BuildMode.debug)).createSync();
  ...
  testUsingContext('debug macOS application creates correctly structured framework', () async {
    ...
    fileSystem.file(
      artifacts.getArtifactPath(Artifact.vmSnapshotData, platform: TargetPlatform.darwin, mode: BuildMode.debug),
    ).createSync(recursive: true);
    fileSystem.file(
      artifacts.getArtifactPath(Artifact.isolateSnapshotData, platform: TargetPlatform.darwin, mode: BuildMode.debug),
    ).createSync(recursive: true);
    ...
    expect(
      fileSystem.file('App.framework/Versions/A/Resources/flutter_assets/kernel_blob.bin').readAsStringSync(),
      'testing',
    );
    expect(
    ...
    expect(fileSystem.file('App.framework/Versions/A/Resources/flutter_assets/vm_snapshot_data'), exists);
    expect(fileSystem.file('App.framework/Versions/A/Resources/flutter_assets/isolate_snapshot_data'), exists);
  ...
  testUsingContext('release/profile macOS application has no blob or precompiled runtime', () async {
    fileSystem.file('bin/cache/artifacts/engine/darwin-x64/vm_isolate_snapshot.bin').createSync(recursive: true);
    fileSystem.file('bin/cache/artifacts/engine/darwin-x64/isolate_snapshot.bin').createSync(recursive: true);
    fileSystem.file('${environment.buildDir.path}/App.framework/App').createSync(recursive: true);

    await const ProfileMacOSBundleFlutterAssets().build(environment..defines[kBuildMode] = 'profile');

    expect(fileSystem.file('App.framework/Versions/A/Resources/flutter_assets/kernel_blob.bin'), isNot(exists));
    expect(fileSystem.file('App.framework/Versions/A/Resources/flutter_assets/vm_snapshot_data'), isNot(exists));
    expect(fileSystem.file('App.framework/Versions/A/Resources/flutter_assets/isolate_snapshot_data'), isNot(exists));
  }, overrides: <Type, Generator>{FileSystem: () => fileSystem, ProcessManager: () => processManager});
  ...
  testUsingContext('release macOS application creates App.framework.dSYM', () async {
    ...
    fileSystem.file('bin/cache/artifacts/engine/darwin-x64/isolate_snapshot.bin').createSync(recursive: true);
    fileSystem.file('${environment.buildDir.path}/App.framework/App').createSync(recursive: true);
    fileSystem.file('${environment.buildDir.path}/App.framework.dSYM/Contents/Resources/DWARF/App').createSync(
      recursive: true,
    );

    await const ReleaseMacOSBundleFlutterAssets().build(environment..defines[kBuildMode] = 'release');
  ...
  testUsingContext('ReleaseMacOSBundleFlutterAssets sends archive success event', () async {
    ...
    fileSystem.file('bin/cache/artifacts/engine/darwin-x64/vm_isolate_snapshot.bin').createSync(recursive: true);
    fileSystem.file('bin/cache/artifacts/engine/darwin-x64/isolate_snapshot.bin').createSync(recursive: true);
    fileSystem.file(fileSystem.path.join(environment.buildDir.path, 'App.framework', 'App')).createSync(
      recursive: true,
    );
  ...
  testUsingContext('CompileMacOSFramework creates universal binary', () async {
    ...
    environment.buildDir.childFile('arm64/App.framework.dSYM/Contents/Resources/DWARF/App').createSync(recursive: true);
    environment.buildDir.childFile('x86_64/App.framework.dSYM/Contents/Resources/DWARF/App').createSync(
      recursive: true,
    );
