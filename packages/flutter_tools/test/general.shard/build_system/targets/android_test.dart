...
void main() {
  ...
  testUsingContext('debug bundle contains expected resources', () async {
    ...
    fileSystem.file(
      artifacts.getArtifactPath(Artifact.vmSnapshotData, mode: BuildMode.debug),
    ...
    expect(
      fileSystem.file(
        fileSystem.path.join('out', 'flutter_assets', 'isolate_snapshot_data'),
      ).existsSync(),
      true,
    );
    expect(
  ...
  testUsingContext(
    'debug bundle contains expected resources with bundle SkSL',
    () async {
      final Environment environment = Environment.test(
        fileSystem.currentDirectory,
        outputDir: fileSystem.directory('out')..createSync(),
      ...
      expect(fileSystem.file(fileSystem.path.join(
        'out',
        'flutter_assets',
  ...
  testWithoutContext('profile bundle contains expected resources', () async {
    ...
    expect(
      fileSystem.file(fileSystem.path.join('out', 'app.so')).existsSync(),
      true,
    );
  });
  ...
  testUsingContext('AndroidAot can build provided target platform', () async {
    ...
    processManager.addCommand(FakeCommand(
      command: <String>[
        ...
        '--elf=${environment.buildDir.childDirectory('arm64-v8a').childFile(
          'app.so',
        ).path}',
        '--strip',
  ...
  testWithoutContext(
    ...
    () async {
      ...
      expect(
        environment.outputDir.childDirectory('arm64-v8a').childFile(
          'app.so',
        ).existsSync(),
        true,
      );
  ...
  test(
    'copyDeferredComponentSoFiles copies all files to correct locations',
    () {
      final Environment environment = Environment.test(
        fileSystem.currentDirectory,
        outputDir: fileSystem.directory('/out')..createSync(),
      ...
      final List<LoadingUnit> loadingUnits = <LoadingUnit>[
        LoadingUnit(
          ...
          path: '/unit2/abi1/part.so',
        ),
        LoadingUnit(
          id: 3,
          libraries: <String>['lib2'],
          path: '/unit3/abi1/part.so',
        ),
        LoadingUnit(
          id: 4,
          libraries: <String>['lib3'],
          path: '/unit4/abi1/part.so',
        ),
        LoadingUnit(
          id: 2,
          libraries: <String>['lib1'],
          path: '/unit2/abi2/part.so',
        ),
        LoadingUnit(
          id: 3,
          libraries: <String>['lib2'],
          path: '/unit3/abi2/part.so',
        ),
        LoadingUnit(
          id: 4,
          libraries: <String>['lib3'],
          path: '/unit4/abi2/part.so',
        ),
      ];
  ...
  testUsingContext(
    ...
    () async {
      ...
      fileSystem.file(
        artifacts.getHostArtifact(HostArtifact.impellerc),
      ).createSync(recursive: true);
