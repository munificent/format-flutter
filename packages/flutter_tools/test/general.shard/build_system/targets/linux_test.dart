...
void main() {
  testWithoutContext(
    ...
    () async {
      ...
      expect(
        fileSystem.file('linux/flutter/ephemeral/libflutter_linux_gtk.so'),
        exists,
      );
      expect(
        fileSystem.file('linux/flutter/ephemeral/unrelated-stuff'),
        isNot(exists),
      );
      ...
      expect(
        fileSystem.file('linux/flutter/ephemeral/$headersPathForX64/foo.h'),
        exists,
      );
      expect(
      ...
      expect(
        fileSystem.file('linux/flutter/ephemeral/$icuDataPathForX64'),
        exists,
      );
      expect(
        fileSystem.file('linux/flutter/ephemeral/$icuDataPathForArm64'),
        isNot(exists),
      );
    },
  );
  ...
  testWithoutContext(
    ...
    () async {
      ...
      await const UnpackLinux(TargetPlatform.linux_arm64).build(
        testEnvironment,
      );
  ...
  testUsingContext(
    ...
    () async {
      ...
      await const DebugBundleLinuxAssets(TargetPlatform.linux_x64).build(
        testEnvironment,
      );

      final Directory output = testEnvironment.outputDir.childDirectory(
        'flutter_assets',
      );
      ...
      expect(
        output.childFile('io.flutter.shaders.json').readAsStringSync(),
        '{"data":{"A":"B"}}',
      );
  ...
  testWithoutContext(
    "DebugBundleLinuxAssets' name depends on target platforms",
    () async {
      expect(
  ...
  testUsingContext(
    ...
    () async {
      ...
      await const LinuxAotBundle(AotElfProfile(TargetPlatform.linux_x64)).build(
        testEnvironment,
      );
      await const ProfileBundleLinuxAssets(TargetPlatform.linux_x64).build(
        testEnvironment,
      );
      final Directory libDir = testEnvironment.outputDir.childDirectory('lib');
      final Directory assetsDir = testEnvironment.outputDir.childDirectory(
        'flutter_assets',
      );
  ...
  testWithoutContext(
    "ProfileBundleLinuxAssets' name depends on target platforms",
    () async {
      expect(
  ...
  testUsingContext(
    ...
    () async {
      ...
      await const LinuxAotBundle(AotElfRelease(TargetPlatform.linux_x64)).build(
        testEnvironment,
      );
      await const ReleaseBundleLinuxAssets(TargetPlatform.linux_x64).build(
        testEnvironment,
      );
      final Directory libDir = testEnvironment.outputDir.childDirectory('lib');
      final Directory assetsDir = testEnvironment.outputDir.childDirectory(
  ...
  testWithoutContext(
    "ReleaseBundleLinuxAssets' name depends on target platforms",
    () async {
      expect(
...
void setUpCacheDirectory(FileSystem fileSystem, Artifacts artifacts) {
  ...
  fileSystem.file('$desktopPathForX64/unrelated-stuff').createSync(
    recursive: true,
  );
  fileSystem.file('$desktopPathForX64/libflutter_linux_gtk.so').createSync(
    recursive: true,
  );
  fileSystem.file('$desktopPathForArm64/unrelated-stuff').createSync(
    recursive: true,
  );
  fileSystem.file('$desktopPathForArm64/libflutter_linux_gtk.so').createSync(
    recursive: true,
  );
  ...
  fileSystem.file(artifacts.getArtifactPath(
    Artifact.icuData,
    platform: TargetPlatform.linux_x64,
  )).createSync();
  fileSystem.file(artifacts.getArtifactPath(
