...
void main() {
  ...
  testWithoutContext(
    ...
    () async {
      ...
      final BuildResult result =
          await buildSystem.build(badTarget, environment);
  ...
  testWithoutContext('Re-invoke build if output is modified', () async {
    ...
    environment.buildDir.childFile('out').writeAsStringSync(
      'Something different',
    );
  ...
  testWithoutContext(
    'Automatically cleans old outputs when build graph changes',
    () async {
      final BuildSystem buildSystem = setUpBuildSystem(fileSystem);
  ...
  testWithoutContext(
    'Does not crash when filesystem and cache are out of sync',
    () async {
      final BuildSystem buildSystem = setUpBuildSystem(fileSystem);
  ...
  testWithoutContext(
    'handles a throwing build action without crashing',
    () async {
      final BuildSystem buildSystem = setUpBuildSystem(fileSystem);
      final BuildResult result =
          await buildSystem.build(fizzTarget, environment);
  ...
  testWithoutContext(
    ...
    () async {
      ...
      final TestTarget target = TestTarget((Environment environment) async {
        environment.buildDir.childFile('example.d').writeAsStringSync(
          'a.txt: b.txt',
        );
  ...
  testWithoutContext(
    'Target with depfile dependency will not run twice without '
        'invalidation in incremental builds',
    () async {
      final BuildSystem buildSystem = setUpBuildSystem(fileSystem);
      int called = 0;
      final TestTarget target = TestTarget((Environment environment) async {
  ...
  testWithoutContext(
    ...
    () {
      environment.outputDir.childFile('.last_build_id').writeAsStringSync(
        'foo',
      );
      environment.buildDir.parent.childDirectory('foo').createSync(
        recursive: true,
      );
  ...
  testWithoutContext(
    ...
    () {
      ...
      final Directory otherBuildDir = environment.buildDir.parent
          .childDirectory('foo')..createSync(recursive: true);
  ...
  testWithoutContext(
    ...
    () async {
      ...
      expect(
        testEnvironmentProfile.outputDir.childFile('.last_build_id'),
        exists,
      );
      expect(
        testEnvironmentProfile.outputDir.childFile(
          '.last_build_id',
        ).readAsStringSync(),
  ...
  testWithoutContext(
    'A target using canSkip can create a conditional output',
    () async {
      final BuildSystem buildSystem = setUpBuildSystem(fileSystem);
      ...
      expect(foo, exists);
      expect(secondResult.inputFiles.map((File file) => file.path), <String>[
        bar.path,
      ]);
      expect(secondResult.outputFiles.map((File file) => file.path), <String>[
        foo.path,
      ]);
  ...
  testWithoutContext(
    ...
    () async {
      ...
      final Future<BuildResult> pendingResult = buildSystem.build(
        a,
        environment,
      );
      await startB.future;
...
class TestTarget extends Target {
  TestTarget([
    Future<void> Function(Environment environment)? build,
    this._canSkip,
