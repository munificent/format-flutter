...
void main() {
  ...
  testWithoutContext('VmService registers flutterGetIOSBuildOptions service', () async {
    ...
    await setUpVmService(flutterProject: mockedFlutterProject, vmService: mockVMService);
  ...
  testUsingContext('VM Service prints messages for connection failures', () {
    ...
    FakeAsync().run((FakeAsync time) {
      ...
      expect(statusText, containsIgnoringWhitespace('Connecting to the VM Service is taking longer than expected...'));
      expect(statusText, containsIgnoringWhitespace('try re-running with --host-vmservice-port'));
      expect(statusText, containsIgnoringWhitespace('Exception attempting to connect to the VM Service:'));
  ...
  testWithoutContext('setAssetDirectory forwards arguments correctly', () async {
    ...
    final vm_service.VmService vmService = vm_service.VmService(const Stream<String>.empty(), completer.complete);
  ...
  testWithoutContext('getSkSLs forwards arguments correctly', () async {
    ...
    unawaited(flutterVmService.getSkSLs(viewId: 'abc'));

    final Map<String, Object?>? rawRequest = json.decode(await completer.future) as Map<String, Object?>?;

    expect(rawRequest, allOf(<Matcher>[
      containsPair('method', kGetSkSLsMethod),
      containsPair('params', allOf(<Matcher>[containsPair('viewId', 'abc')])),
    ]));
  });
  ...
  testWithoutContext('flushUIThreadTasks forwards arguments correctly', () async {
    ...
    unawaited(flutterVmService.flushUIThreadTasks(uiIsolateId: 'def'));

    final Map<String, Object?>? rawRequest = json.decode(await completer.future) as Map<String, Object?>?;

    expect(rawRequest, allOf(<Matcher>[
      containsPair('method', kFlushUIThreadTasksMethod),
      containsPair('params', allOf(<Matcher>[containsPair('isolateId', 'def')])),
    ]));
  });
  ...
  testWithoutContext('VmService forward flutterGetIOSBuildOptions request and response correctly', () async {
    ...
    await setUpVmService(flutterProject: mockedFlutterProject, vmService: vmService);
  ...
  testWithoutContext(
    ...
    () async {
      final MockVMService vmService = MockVMService();
      final FlutterProject mockedFlutterProject = MockFlutterProject(mockedIos: MockIosProject());
      await setUpVmService(flutterProject: mockedFlutterProject, vmService: vmService);
      final vm_service.ServiceCallback cb = vmService.serviceCallBacks[kFlutterGetIOSBuildOptionsServiceName]!;
    ...
    final FakeVmServiceHost fakeVmServiceHost = FakeVmServiceHost(
      requests: <VmServiceExpectation>[
        ...
        const FakeVmServiceRequest(
          ...
          args: <String, Object>{'viewId': '1234', 'mainScript': 'main.dart', 'assetDirectory': 'flutter_assets/'},
        ),
  ...
  testWithoutContext('flutterDebugDumpSemanticsTreeInTraversalOrder handles missing method', () async {
    ...
    expect(await fakeVmServiceHost.vmService.flutterDebugDumpSemanticsTreeInTraversalOrder(isolateId: '1'), '');
    expect(fakeVmServiceHost.hasRemainingExpectations, false);
  });
  ...
  testWithoutContext('flutterDebugDumpSemanticsTreeInInverseHitTestOrder handles missing method', () async {
    ...
    expect(await fakeVmServiceHost.vmService.flutterDebugDumpSemanticsTreeInInverseHitTestOrder(isolateId: '1'), '');
    expect(fakeVmServiceHost.hasRemainingExpectations, false);
  });
  ...
  testWithoutContext('flutterDebugDumpFocusTree returns data', () async {
    ...
    expect(await fakeVmServiceHost.vmService.flutterDebugDumpFocusTree(isolateId: '1'), 'Hello world');
    expect(fakeVmServiceHost.hasRemainingExpectations, false);
  });
  ...
  testWithoutContext('Framework service extension invocations return null if service disappears ', () async {
    final FakeVmServiceHost fakeVmServiceHost = FakeVmServiceHost(
      requests: <VmServiceExpectation>[
        ...
        const FakeVmServiceRequest(method: kScreenshotMethod, errorCode: RPCErrorCodes.kServiceDisappeared),
        const FakeVmServiceRequest(method: kScreenshotSkpMethod, errorCode: RPCErrorCodes.kServiceDisappeared),
        const FakeVmServiceRequest(
        ...
        const FakeVmServiceRequest(method: 'getVMTimeline', errorCode: RPCErrorCodes.kServiceDisappeared),
        const FakeVmServiceRequest(
          method: kRenderFrameWithRasterStatsMethod,
          args: <String, dynamic>{'viewId': '1', 'isolateId': '12'},
          errorCode: RPCErrorCodes.kServiceDisappeared,
        ),
      ],
    );

    final Map<String, Object?>? skSLs = await fakeVmServiceHost.vmService.getSkSLs(viewId: '1234');
    expect(skSLs, isNull);
  ...
  testWithoutContext('getIsolateOrNull returns null if service disappears ', () async {
    ...
    final vm_service.Isolate? isolate = await fakeVmServiceHost.vmService.getIsolateOrNull('isolate/123');
    expect(isolate, null);
  ...
  testWithoutContext('renderWithStats forwards stats correctly', () async {
    ...
    const Map<String, dynamic> response = {
      ...
      'snapshots': <dynamic>[
        // ignore: always_specify_types
        {'layer_unique_id': 1512, 'duration_micros': 477, 'snapshot': ''},
      ],
    };
    final FakeVmServiceHost fakeVmServiceHost = FakeVmServiceHost(
      requests: <VmServiceExpectation>[
        const FakeVmServiceRequest(
          method: kRenderFrameWithRasterStatsMethod,
          args: <String, Object>{'isolateId': 'isolate/123', 'viewId': 'view/1'},
  ...
  testWithoutContext('getFlutterViews polls until a view is returned', () async {
    final FakeVmServiceHost fakeVmServiceHost = FakeVmServiceHost(
      requests: <VmServiceExpectation>[
        const FakeVmServiceRequest(method: kListViewsMethod, jsonResponse: <String, Object>{'views': <Object>[]}),
        const FakeVmServiceRequest(method: kListViewsMethod, jsonResponse: <String, Object>{'views': <Object>[]}),
        listViewsRequest,
      ],
    );
    ...
    testWithoutContext('returns an isolate with the registered extensionRPC', () async {
      final FakeVmServiceHost fakeVmServiceHost = FakeVmServiceHost(
        requests: <VmServiceExpectation>[
          ...
          const FakeVmServiceRequest(method: 'streamCancel', args: <String, Object>{'streamId': 'Isolate'}),
        ],
      );
...
class MockFlutterProject extends Fake implements FlutterProject {
  MockFlutterProject({IosProject? mockedIos, AndroidProject? mockedAndroid})
...
Future<io.WebSocket> failingWebSocketConnector(String url, {io.CompressionOptions? compression, Logger? logger}) {
