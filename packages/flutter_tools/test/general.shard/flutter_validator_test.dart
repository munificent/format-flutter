...
void main() {
  testWithoutContext('FlutterValidator shows an error message if gen_snapshot is '
    ...
    final FakeFlutterVersion flutterVersion = FakeFlutterVersion(frameworkVersion: '1.0.0', branch: 'beta');
  ...
  testWithoutContext('FlutterValidator shows an error message if Rosetta is needed', () async {
    ...
    final Artifacts artifacts = Artifacts.test();
    final FlutterValidator flutterValidator = FlutterValidator(
      platform: FakePlatform(operatingSystem: 'macos', localeName: 'en_US.UTF-8', environment: <String, String>{}),
      flutterVersion: () => flutterVersion,
      devToolsVersion: () => '2.8.0',
      userMessages: UserMessages(),
      artifacts: artifacts,
  ...
  testWithoutContext(
    ...
    () async {
      final FakeFlutterVersion flutterVersion = FakeFlutterVersion(frameworkVersion: '1.0.0', branch: 'beta');
      final FlutterValidator flutterValidator = FlutterValidator(
        platform: FakePlatform(operatingSystem: 'windows', localeName: 'en_US.UTF-8', environment: <String, String>{}),
        flutterVersion: () => flutterVersion,
        devToolsVersion: () => '2.8.0',
        userMessages: UserMessages(),
  ...
  group('FlutterValidator shows flutter upstream remote', () {
    ...
    testWithoutContext('as unknown if upstream is null', () async {
      final FlutterValidator flutterValidator = FlutterValidator(
        platform: FakePlatform(localeName: 'en_US.UTF-8'),
        flutterVersion: () => FakeFlutterVersion(frameworkVersion: '1.0.0', branch: 'beta', repositoryUrl: null),
        devToolsVersion: () => '2.8.0',
        userMessages: UserMessages(),
        artifacts: Artifacts.test(),
        fileSystem: MemoryFileSystem.test(),
  ...
  testWithoutContext('detects no flutter and dart on path', () async {
    ...
    final FlutterValidator flutterValidator = FlutterValidator(
      ...
      fileSystem: MemoryFileSystem.test(),
      processManager: FakeProcessManager.any(),
      operatingSystemUtils: FakeOperatingSystemUtils(name: 'Linux', whichLookup: const <String, File>{}),
  ...
  testWithoutContext('allows case differences in paths on Windows', () async {
    ...
    final File flutterBinary = fs.file('${flutterRoot.toUpperCase()}\\bin\\flutter')..createSync(recursive: true);
    final FlutterValidator flutterValidator = FlutterValidator(
      platform: FakePlatform(operatingSystem: 'windows', localeName: 'en_US.UTF-8'),
      flutterVersion: () => FakeFlutterVersion(frameworkVersion: '1.0.0', branch: 'beta'),
      devToolsVersion: () => '2.8.0',
      ...
      operatingSystemUtils:
          FakeOperatingSystemUtils(name: osName, whichLookup: <String, File>{'flutter': flutterBinary}),
      flutterRoot: () => flutterRoot,
    );
