...
''');
  ...
  group('Version', () {
    testWithoutContext('can parse and compare', () {
      ...
      expect(
        const Version.withText(1, 2, 3, 'versionText').toString(),
        'versionText',
      );
    ...
    group('isWithinVersionRange', () {
      test('unknown not included', () {
        expect(
          isWithinVersionRange('unknown', min: '1.0.0', max: '1.1.3'),
          isFalse,
        );
      });
      ...
      test('pre java 8 format included', () {
        expect(
          isWithinVersionRange('1.0.0_201', min: '1.0.0', max: '1.1.3'),
          isTrue,
        );
      });
      ...
      test('min included by default', () {
        expect(
          isWithinVersionRange('1.0.0', min: '1.0.0', max: '1.1.3'),
          isTrue,
        );
      });
      ...
      test('max included by default', () {
        expect(
          isWithinVersionRange('1.1.3', min: '1.0.0', max: '1.1.3'),
          isTrue,
        );
      });

      test('inclusive min excluded', () {
        expect(isWithinVersionRange(
          '1.0.0',
          min: '1.0.0',
          max: '1.1.3',
          inclusiveMin: false,
      ...
      test('lower value excluded', () {
        expect(
          isWithinVersionRange('0.1.0', min: '1.0.0', max: '1.1.3'),
          isFalse,
        );
      });
      ...
      test('higher value excluded', () {
        expect(
          isWithinVersionRange('1.1.4', min: '1.0.0', max: '1.1.3'),
          isFalse,
        );
      });
      ...
      test('middle value included', () {
        expect(
          isWithinVersionRange('1.1.0', min: '1.0.0', max: '1.1.3'),
          isTrue,
        );
      });
    });
  ...
  group('text wrapping', () {
    ...
    testWithoutContext('does not wrap by default in tests', () {
      expect(
        wrapText(longLine, columnWidth: 80, shouldWrap: true),
        equals(longLine),
      );
    });
...
wrapped.'''),
    ...
    testWithoutContext('can override wrap preference if preference is on', () {
      expect(
        wrapText(longLine, shouldWrap: false, columnWidth: 80),
        equals(longLine),
      );
    });
    ...
    testWithoutContext('does not wrap at all if not told to wrap', () {
      expect(
        wrapText(longLine, columnWidth: 80, shouldWrap: false),
        equals(longLine),
      );
    });
    ...
    testWithoutContext('does not wrap short lines.', () {
      expect(
        wrapText(shortLine, columnWidth: lineLength, shouldWrap: true),
        equals(shortLine),
      );
    });
...
0123456789'''),
    ...
    testWithoutContext(
      'refuses to wrap to a column smaller than 10 characters',
      () {
        expect(
          wrapText(
            '$longLine ${'0123456789' * 4}',
            columnWidth: 1,
            shouldWrap: true,
          ),
          equals('''
...
This is a long line that needs to be
    ...
    testWithoutContext(
      'handles hangingIndent with a single unwrapped line.',
      () {
        expect(
          wrapText(
            shortLine,
