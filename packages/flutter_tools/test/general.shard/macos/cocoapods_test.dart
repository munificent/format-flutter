...
void main() {
  ...
  void podsIsInHomeDir() {
    fileSystem.directory(
      fileSystem.path.join('.cocoapods', 'repos', 'master'),
  ...
  group('Setup Podfile', () {
    testUsingContext('creates objective-c Podfile when not present', () async {
      ...
      expect(
        projectUnderTest.ios.podfile.readAsStringSync(),
    ...
    testUsingContext('creates macOS Podfile when not present', () async {
      ...
      expect(
        projectUnderTest.macos.podfile.readAsStringSync(),
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(
          releaseContents,
          isNot(contains('Pods-Runner/Pods-Runner.release')),
        );
...
Note: as of CocoaPods 1.0, `pod repo update` does not happen on `pod install` by default.''',
    ...
    possibleErrors.forEach((String errorName, String cocoaPodsError) {
      void testToolExitsWithCocoapodsMessage(_StdioStream outputStream) {
        final String streamName = outputStream == _StdioStream.stdout
            ? 'stdout'
            : 'stderr';
        testUsingContext(
          ...
          () async {
            ...
            fakeProcessManager.addCommands(<FakeCommand>[
              FakeCommand(
                ...
                exitCode: 1,
                stdout: outputStream == _StdioStream.stdout
                    ? cocoaPodsError
                    : '',
                stderr: outputStream == _StdioStream.stderr
                    ? cocoaPodsError
                    : '',
              ),
