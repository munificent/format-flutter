  ...
  group('update deployment target version', () {
    ...
    testWithoutContext('skipped if files are missing', () {
      final MacOSDeploymentTargetMigration macOSProjectMigration =
          MacOSDeploymentTargetMigration(project, testLogger);
      macOSProjectMigration.migrate();
      expect(xcodeProjectInfoFile.existsSync(), isFalse);
...
''');
      ...
      expect(
        'Updating minimum macOS deployment target to 10.14'.allMatches(
  ...
  group('update NSPrincipalClass from FlutterApplication to NSApplication', () {
    ...
    testWithMocks('skipped if files are missing', () async {
      final FlutterApplicationMigration macOSProjectMigration =
          FlutterApplicationMigration(project, testLogger);
      macOSProjectMigration.migrate();
    ...
    testWithMocks(
      'skipped if no NSPrincipalClass key exists to upgrade',
      () async {
        final FlutterApplicationMigration macOSProjectMigration =
            FlutterApplicationMigration(project, testLogger);
        ...
        macOSProjectMigration.migrate();
        expect(fakePlistParser.getValueFromFile<String>(
          infoPlistFile.path,
    ...
    testWithMocks(
      'skipped if already de-upgraded (or never migrated)',
      () async {
