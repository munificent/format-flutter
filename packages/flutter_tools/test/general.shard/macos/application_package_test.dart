...
void main() {
  group('PrebuiltMacOSApp', () {
    ...
    testUsingContext('Error on non-existing file', () {
      final PrebuiltMacOSApp? macosApp = MacOSApp.fromPrebuiltApp(
    ...
    testUsingContext('Error on info.plist missing bundle identifier', () {
      final String contentsDirectory = fileSystem.path.join(
        'bundle.app',
        'Contents',
      );
      fileSystem.directory(contentsDirectory).createSync(recursive: true);
      fileSystem.file(
        fileSystem.path.join('bundle.app', 'Contents', 'Info.plist'),
    ...
    testUsingContext('Success with app bundle', () {
      ...
      fileSystem.file(
        fileSystem.path.join(appDirectory, executableName),
      ).createSync();
      final PrebuiltMacOSApp macosApp = MacOSApp.fromPrebuiltApp(
        fileSystem.file('bundle.app'),
      )! as PrebuiltMacOSApp;
    ...
    testUsingContext('Success with zipped app', () {
      ...
      os.unzipOverride = (File zipFile, Directory targetDirectory) {
        ...
        fileSystem.file(
          fileSystem.path.join(bundleAppContentsDir.path, 'Info.plist'),
        ).writeAsStringSync(plistData);
        fileSystem.directory(
          fileSystem.path.join(bundleAppContentsDir.path, 'MacOS'),
        ).createSync();
        fileSystem.file(fileSystem.path.join(
          bundleAppContentsDir.path,
          'MacOS',
          executableName,
        )).createSync();
      };
