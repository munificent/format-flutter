  ...
  group('FakeProcessManager', () {
    ...
    group('Xcode', () {
      ...
      group('macOS', () {
        ...
        setUp(() {
          xcodeProjectInterpreter = FakeXcodeProjectInterpreter();
          xcode = Xcode.test(processManager: fakeProcessManager, xcodeProjectInterpreter: xcodeProjectInterpreter);
        });

        testWithoutContext('xcodeSelectPath returns path when xcode-select is installed', () {
          const String xcodePath = '/Applications/Xcode8.0.app/Contents/Developer';
          fakeProcessManager.addCommand(
            const FakeCommand(command: <String>['/usr/bin/xcode-select', '--print-path'], stdout: xcodePath),
          );
        ...
        testWithoutContext('eulaSigned is true when clang output indicates EULA has been accepted', () {
          fakeProcessManager.addCommands(const <FakeCommand>[
            FakeCommand(command: <String>['xcrun', 'clang'], exitCode: 1, stderr: 'clang: error: no input files'),
          ]);
        ...
        group('SDK location', () {
          ...
          testWithoutContext('--show-sdk-path iphoneos', () async {
            fakeProcessManager.addCommand(
              const FakeCommand(command: <String>['xcrun', '--sdk', 'iphoneos', '--show-sdk-path'], stdout: sdkroot),
            );
    ...
    group('xcdevice', () {
      ...
      group('observe device events', () {
        testUsingContext('relays events', () async {
          fakeProcessManager.addCommand(
            const FakeCommand(
              command: <String>['script', '-t', '0', '/dev/null', 'xcrun', 'xcdevice', 'observe', '--usb'],
          ...
          fakeProcessManager.addCommand(
            const FakeCommand(
              command: <String>['script', '-t', '0', '/dev/null', 'xcrun', 'xcdevice', 'observe', '--wifi'],
      ...
      group('wait device events', () {
        testUsingContext('relays events', () async {
          ...
          fakeProcessManager.addCommand(
            const FakeCommand(
              command: <String>['script', '-t', '0', '/dev/null', 'xcrun', 'xcdevice', 'wait', '--usb', deviceId],
              stdout: 'Waiting for $deviceId to appear, on USB.\n',
            ),
          );
          fakeProcessManager.addCommand(
            const FakeCommand(
              command: <String>['script', '-t', '0', '/dev/null', 'xcrun', 'xcdevice', 'wait', '--wifi', deviceId],
...
''';
          ...
          fakeProcessManager.addCommand(
            const FakeCommand(command: <String>['xcrun', 'xcdevice', 'list', '--timeout', '2'], stdout: devicesOutput),
          );
          ...
          expect(fakeProcessManager, hasNoRemainingExpectations);
        }, overrides: <Type, Generator>{Platform: () => macPlatform, Artifacts: () => Artifacts.test()});
        ...
        testWithoutContext('uses timeout', () async {
          fakeProcessManager.addCommand(
            const FakeCommand(command: <String>['xcrun', 'xcdevice', 'list', '--timeout', '20'], stdout: '[]'),
          );
