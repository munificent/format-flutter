...
void main() {
  ...
  test(
    ...
    () => testbed.run(() async {
      ...
      expect(
        webAssetServer.write(source, manifestOutOfBounds, sourcemap, metadata),
        isEmpty,
      );
    }),
  );
  ...
  test(
    ...
    () => testbed.run(() async {
      ...
      expect(response.statusCode, HttpStatus.ok);
      expect(
        (await response.read().toList()).first,
  ...
  test(
    ...
    () => testbed.run(() async {
      final File source = globals.fs.file(globals.fs.path.join(
        'build',
        'flutter_assets',
  ...
  test(
    ...
    () => testbed.run(() async {
      final File source = globals.fs.file(
        globals.fs.path.join('build', 'flutter_assets', 'foo'),
      )
        ..createSync(recursive: true)
  ...
  test(
    ...
    () => testbed.run(() async {
      ...
      final String webPrecompiledSdk = globals.artifacts!.getHostArtifact(
      ...
      final String webPrecompiledCanvaskitSdkSourcemaps = globals.artifacts!
          ...
          .path;
      globals.fs.currentDirectory.childDirectory('lib').childFile(
        'web_entrypoint.dart',
      )
        ..createSync(recursive: true)
      ...
      expect(
        webDevFS.webAssetServer.getFile('stack_trace_mapper.js'),
        isNotNull,
      );
      ...
      expect(
        webDevFS.webAssetServer.getFile('flutter_service_worker.js'),
        isNotNull,
      );
      ...
      expect(
        await webDevFS.webAssetServer.dartSourceContents('dart_sdk.js.map'),
        'THERE',
      );
  ...
  test(
    ...
    () => testbed.run(() async {
      ...
      expect(
        await webAssetServer.metadataContents(
      ...
      // Not base href.
      expect(
        () async => webAssetServer.metadataContents(
