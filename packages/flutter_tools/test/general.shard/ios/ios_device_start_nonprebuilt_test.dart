...
void main() {
  ...
  group('IOSDevice.startApp succeeds in release mode', () {
    ...
    setUp(() {
      ...
      projectInfo = XcodeProjectInfo(<String>['Runner'], <String>['Debug', 'Release'], <String>['Runner'], logger);
    ...
    testUsingContext(
      ...
      () async {
        ...
        processManager.addCommand(
          const FakeCommand(command: kRunReleaseArgs, exitCode: 1, stdout: kConcurrentBuildErrorMessage),
        );
        processManager.addCommand(const FakeCommand(command: kRunReleaseArgs));
        processManager.addCommand(FakeCommand(
          command: <String>[
            ...
            '--justlaunch',
            '--args',
            const <String>['--enable-dart-profiling', '--disable-service-auth-codes'].join(' '),
          ],
        ));
        ...
        await FakeAsync().run((FakeAsync time) async {
          ...
          expect(logger.statusText, contains('Xcode build failed due to concurrent builds, will retry in 2 seconds'));
          expect(launchResult.started, true);
          expect(processManager, hasNoRemainingExpectations);
        });
      },
      ...
      skip: true,
    ); // TODO(zanderso): clean up with https://github.com/flutter/flutter/issues/60675
  });
}
...
class FakeXcodeProjectInterpreter extends Fake implements XcodeProjectInterpreter {
  ...
  @override
  Future<XcodeProjectInfo?> getInfo(String projectPath, {String? projectFilename}) async => projectInfo;
