...
void main() {
  ...
  FakeCommand setUpFakeXCResultGetCommand({
    ...
    return FakeCommand(
      command: <String>[...xcode.xcrunCommand(), 'xcresulttool', 'get', '--path', tempResultPath, '--format', 'json'],
      stdout: stdout,
      stderr: stderr,
      exitCode: exitCode,
      onRun: () {},
  ...
  XCResultGenerator setupGenerator({required String resultJson, int exitCode = 0, String stderr = ''}) {
    ...
    final Xcode xcode = Xcode.test(
      ...
      xcodeProjectInterpreter: XcodeProjectInterpreter.test(processManager: fakeProcessManager, version: null),
    );
  ...
  testWithoutContext('correctly parse sample result json when there are issues but invalid url.', () async {
  ...
  testWithoutContext('correctly parse sample result json with multiple discarders.', () async {
    ...
    final XCResult result =
        await generator.generate(issueDiscarders: <XCResultIssueDiscarder>[discardWarnings, discardSemanticIssues]);
  ...
  testWithoutContext(
    ...
    () async {
      ...
      final XCResultGenerator generator = setupGenerator(resultJson: '', exitCode: 1, stderr: fakeStderr);
