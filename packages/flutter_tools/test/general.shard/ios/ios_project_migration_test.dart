...
''');
    ...
    group('new Xcode build system', () {
      ...
      setUp(() {
        memoryFileSystem = MemoryFileSystem.test();
        xcodeWorkspaceSharedSettings = memoryFileSystem.file(
      ...
      testWithoutContext('skipped if files are missing', () {
        final XcodeBuildSystemMigration iosProjectMigration =
            XcodeBuildSystemMigration(project, testLogger);
        iosProjectMigration.migrate();
      ...
      testWithoutContext(
        'skipped if _xcodeWorkspaceSharedSettings is null',
        () {
          final XcodeBuildSystemMigration iosProjectMigration =
              XcodeBuildSystemMigration(project, testLogger);
...
<dict>
    ...
    group('Xcode default build location', () {
      ...
      setUp(() {
        ...
        xcodeProjectWorkspaceData = memoryFileSystem.file(
      ...
      testWithoutContext('skipped if files are missing', () {
        final ProjectBuildLocationMigration iosProjectMigration =
            ProjectBuildLocationMigration(project, testLogger);
        iosProjectMigration.migrate();
    ...
    group('update deployment target version', () {
      ...
      testWithoutContext('skipped if files are missing', () {
        final IOSDeploymentTargetMigration iosProjectMigration =
            IOSDeploymentTargetMigration(project, testLogger);
        iosProjectMigration.migrate();
        expect(xcodeProjectInfoFile.existsSync(), isFalse);
...
''';
        ...
        expect(xcodeProjectInfoFile.lastModifiedSync(), projectLastModified);
        expect(
          xcodeProjectInfoFile.readAsStringSync(),
          xcodeProjectInfoFileContents,
        );
        expect(
          appFrameworkInfoPlist.readAsStringSync(),
...
''');
        ...
        expect(
          'Updating minimum iOS deployment target to 11.0'.allMatches(
            testLogger.statusText,
          ).length,
          1,
        );
...
''';
        ...
        expect(
          xcodeProjectSchemeFile.readAsStringSync(),
          xcodeProjectSchemeFileContents,
        );
      ...
      testWithoutContext(
        'Xcode project is migrated to newest objectVersion',
        () {
          xcodeProjectInfoFile.writeAsStringSync('''
	classes = {
	};
...
''');
          ...
          expect(
            'Updating project for Xcode compatibility'.allMatches(
              testLogger.statusText,
            ).length,
            1,
          );
    ...
    group('update info.plist migration', () {
      ...
      testWithoutContext('skipped if files are missing', () {
        final HostAppInfoPlistMigration iosProjectMigration =
            HostAppInfoPlistMigration(project, testLogger);
        iosProjectMigration.migrate();
...
''');
        // Only print once even though 2 lines were changed.
        expect(
          'Disabling deprecated bitcode Xcode build setting'.allMatches(
    ...
    group('CocoaPods script readlink', () {
      ...
      setUp(() {
        memoryFileSystem = MemoryFileSystem();
        podRunnerFrameworksScript = memoryFileSystem.file(
  ...
  group('update Xcode script build phase', () {
    ...
    testWithoutContext('skipped if files are missing', () {
      final XcodeScriptBuildPhaseMigration iosProjectMigration =
          XcodeScriptBuildPhaseMigration(project, testLogger);
      iosProjectMigration.migrate();
      expect(xcodeProjectInfoFile.existsSync(), isFalse);
...
class FakeIosProject extends Fake implements IosProject {
  @override
  File xcodeProjectWorkspaceData = MemoryFileSystem.test().file(
    'xcodeProjectWorkspaceData',
  );

  @override
  File? xcodeWorkspaceSharedSettings = MemoryFileSystem.test().file(
    'xcodeWorkspaceSharedSettings',
  );

  @override
  File xcodeProjectInfoFile = MemoryFileSystem.test().file(
    'xcodeProjectInfoFile',
  );

  @override
  File xcodeProjectSchemeFile = MemoryFileSystem.test().file(
    'xcodeProjectSchemeFile',
  );

  @override
  File appFrameworkInfoPlist = MemoryFileSystem.test().file(
    'appFrameworkInfoPlist',
  );

  @override
  File defaultHostInfoPlist = MemoryFileSystem.test().file(
  ...
  @override
  File podRunnerFrameworksScript = MemoryFileSystem.test().file(
