...
final Platform macosPlatform = FakePlatform(operatingSystem: 'macos', environment: <String, String>{'HOME': '/'});
...
void main() {
  ...
  group('compareIosVersions', () {
    testWithoutContext('compares correctly', () {
      ...
      final List<String> testList = <String>['8', '8.0', '8.1', '8.2', '9', '9.0', '9.1', '9.2', '10', '10.0', '10.1'];
  ...
  group('Simulator screenshot', () {
    testWithoutContext('supports screenshots', () async {
      ...
      final FakeProcessManager fakeProcessManager = FakeProcessManager.list(<FakeCommand>[
        const FakeCommand(command: <String>['xcrun', 'simctl', 'io', 'x', 'screenshot', 'screenshot.png']),
      ]);

      // Test a real one. Screenshot doesn't require instance states.
      final SimControl simControl = SimControl(processManager: fakeProcessManager, logger: logger, xcode: xcode);
  ...
  group('device log tool', () {
    ...
    testUsingContext(
      ...
      () async {
        ...
        fakeProcessManager.addCommand(
          const FakeCommand(command: <String>['tail', '-n', '0', '-F', '/Library/Logs/CoreSimulator/x/system.log']),
        );
    ...
    testUsingContext('unified logging with app name', () async {
      ...
      expect(fakeProcessManager, hasNoRemainingExpectations);
    }, overrides: <Type, Generator>{ProcessManager: () => fakeProcessManager, FileSystem: () => fileSystem});
...
Dec 20 17:04:32 md32-11-vm1 Another App[88374]: Ignore this text''',
          ...
          expect(lines, <String>['flutter: The Dart VM service is listening on http://127.0.0.1:64213/1Uoeu523990=/']);
          expect(fakeProcessManager, hasNoRemainingExpectations);
        },
        overrides: <Type, Generator>{
          ProcessManager: () => fakeProcessManager,
    ...
    const String validSimControlOutput = '''
{
  "devices" : {
    ...
    setUp(() {
      fakeProcessManager = FakeProcessManager.empty();
      xcode = Xcode.test(processManager: FakeProcessManager.any());
      simControl = SimControl(logger: BufferLogger.test(), processManager: fakeProcessManager, xcode: xcode);
    });
    ...
    testWithoutContext('.install() handles exceptions', () async {
      ...
      expect(() async => simControl.install(deviceId, appId), throwsToolExit(message: r'Unable to install'));
    });
    ...
    testWithoutContext('.uninstall() handles exceptions', () async {
      ...
      expect(() async => simControl.uninstall(deviceId, appId), throwsToolExit(message: r'Unable to uninstall'));
    });
    ...
    testWithoutContext('.launch() handles exceptions', () async {
      ...
      expect(() async => simControl.launch(deviceId, appId), throwsToolExit(message: r'Unable to launch'));
    });
    ...
    testWithoutContext('.stopApp() handles exceptions', () async {
      ...
      expect(() async => simControl.stopApp(deviceId, appId), throwsToolExit(message: 'Unable to terminate'));
      expect(fakeProcessManager, hasNoRemainingExpectations);
    });
