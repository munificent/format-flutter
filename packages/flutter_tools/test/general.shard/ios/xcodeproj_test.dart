...
void main() {
    ...
    () async {
      ...
      expect(
        await xcodeProjectInterpreter.getBuildSettings(
          '',
          buildContext:
              const XcodeProjectBuildContext(deviceId: '123', scheme: 'Free'),
        ),
        const <String, String>{},
      );
      expect(fakeProcessManager, hasNoRemainingExpectations);
  ...
  testUsingContext(
    'build settings uses watch destination if isWatch is true',
    () async {
      platform.environment = const <String, String>{};
  ...
  testWithoutContext(
    ...
    () async {
      ...
      expect(
        await xcodeProjectInterpreter.getInfo(workingDirectory),
        isNotNull,
      );
      expect(fakeProcessManager, hasNoRemainingExpectations);
    },
...
''';
      final XcodeProjectInfo info = XcodeProjectInfo.fromXcodeBuildOutput(
        output,
        logger,
      );
...
''';
  ...
  testWithoutContext(
    ...
    () {
      expect(XcodeProjectInfo.expectedSchemeFor(
  ...
  testWithoutContext(
    ...
    () {
      ...
      expect(
        info.buildConfigurationFor(BuildInfo.profile, 'Runner'),
        'Profile',
      );
      expect(
        info.buildConfigurationFor(BuildInfo.release, 'Runner'),
        'Release',
      );
    },
  );
  ...
  testWithoutContext('reports default scheme error and exit', () {
    ...
    expect(defaultInfo.reportFlavorNotFoundAndExit, throwsToolExit(
      message:
  ...
  testWithoutContext('reports custom scheme error and exit', () {
    ...
    expect(info.reportFlavorNotFoundAndExit, throwsToolExit(
      message:
  ...
  testWithoutContext(
    ...
    () {
      ...
      expect(info.buildConfigurationFor(
        const BuildInfo(BuildMode.debug, 'free', treeShakeIcons: false),
        'Free',
      ), 'debug (free)');
      expect(info.buildConfigurationFor(
    ...
    group('arm simulator', () {
      ...
      testUsingContext(
        ...
        () async {
          ...
          final String buildDirectory = fileSystem.path.absolute(
            'build',
            'ios',
          );
          fakeProcessManager.addCommands(<FakeCommand>[
...
Build settings for action build and target plugin2:
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(
          buildPhaseScriptContents.contains('export "ARCHS=arm64"'),
          isTrue,
        );
      },
      overrides: <Type, Generator>{
    ...
    testUsingContext(
      ...
      () async {
        ...
        final String buildPhaseScriptContents =
            buildPhaseScript.readAsStringSync();
        expect(
          buildPhaseScriptContents.contains('export "ARCHS=x86_64"'),
          isTrue,
        );
      },
      overrides: <Type, Generator>{
    ...
    testUsingOsxContext(
      'does not exclude arm64 simulator when there are no plugins',
      () async {
        const BuildInfo buildInfo = BuildInfo.debug;
        final FlutterProject project = FlutterProject.fromDirectoryTest(
          fs.directory('path/to/project'),
    ...
    testUsingOsxContext(
      ...
      () async {
        ...
        final String buildPhaseScriptContents =
            buildPhaseScript.readAsStringSync();
        expect(
          buildPhaseScriptContents.contains('TRACK_WIDGET_CREATION=true'),
          isTrue,
        );
      },
    );
    ...
    group('sim local engine', () {
      testUsingContext(
        ...
        overrides: <Type, Generator>{
          Artifacts:
              () => Artifacts.test(localEngine: 'out/ios_debug_sim_unopt'),
          Platform: () => macOS,
          FileSystem: () => fs,
          ProcessManager: () => FakeProcessManager.any(),
        },
      ...
      testUsingContext(
        ...
        overrides: <Type, Generator>{
          Artifacts:
              () => Artifacts.test(localEngine: 'out/ios_debug_sim_arm64'),
          Platform: () => macOS,
          FileSystem: () => fs,
          ProcessManager: () => FakeProcessManager.any(),
        },
    ...
    Future<void> checkBuildVersion({
      ...
      final File localPropertiesFile = fs.file(
        'path/to/project/ios/Flutter/Generated.xcconfig',
      );
      expect(
        propertyFor('FLUTTER_BUILD_NAME', localPropertiesFile),
        expectedBuildName,
      );
      expect(
        propertyFor('FLUTTER_BUILD_NUMBER', localPropertiesFile),
        expectedBuildNumber,
      );
      expect(
        propertyFor('FLUTTER_BUILD_NUMBER', localPropertiesFile),
    ...
    testUsingOsxContext(
      'extract build name and number from pubspec.yaml',
      () async {
...
''';
    ...
    testUsingOsxContext(
      'allow build info to override build name and number',
      () async {
        const String manifest = '''
name: test
version: 1.0.0+1
...
''';
    ...
    testUsingOsxContext(
      'allow build info to override build name and set number',
      () async {
        const String manifest = '''
name: test
version: 1.0.0
...
''';
    ...
    testUsingOsxContext(
      'allow build info to set build name and number',
      () async {
        const String manifest = '''
name: test
dependencies:
...
''';
    ...
    testUsingOsxContext(
      'default build name and number when version is missing',
      () async {
        const String manifest = '''
name: test
dependencies:
