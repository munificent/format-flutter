  ...
  group('IOSDeploy.prepareDebuggerForLaunch', () {
    testWithoutContext(
      ...
      () async {
        ...
        final IOSDeploy iosDeploy = setUpIOSDeploy(
          processManager,
          artifacts: artifacts,
        );
        final IOSDeployDebugger iosDeployDebugger = iosDeploy
  ...
  group('IOSDeployDebugger', () {
    group('launch', () {
      ...
      testWithoutContext(
        ...
        () async {
          final StreamController<List<int>> stdin =
    ...
    testWithoutContext('stop with backtrace', () async {
      final StreamController<List<int>> stdin = StreamController<List<int>>();
      final Stream<String> stdinStream = stdin.stream.transform<String>(
        const Utf8Decoder(),
      );
      final FakeProcessManager processManager = FakeProcessManager.list(
        <FakeCommand>[
...
''';
    ...
    group('Check for symbols', () {
      ...
      testWithoutContext('and find status', () async {
        ...
        final Directory symbolsDirectory = fileSystem.directory(
          symbolsDirectoryPath,
        );
        symbolsDirectory.createSync(recursive: true);

        final File copyingStatusFile = symbolsDirectory.parent.childFile(
          '.copying_lock',
        );
        copyingStatusFile.createSync();

        final File processingStatusFile = symbolsDirectory.parent.childFile(
          '.processing_lock',
        );
