    ...
    () async {
      final BuildSystem buildSystem = TestBuildSystem.all(
        ...
        (Target target, Environment environment) {
          environment.outputDir.childFile('kernel_blob.bin').createSync(
            recursive: true,
          );
          ...
          environment.outputDir.childFile('LICENSE').createSync(
            recursive: true,
          );
        },
      );
      ...
      expect(
        globals.fs.file(
          globals.fs.path.join('example', 'kernel_blob.bin'),
        ).existsSync(),
        true,
      );
      expect(
        globals.fs.file(
          globals.fs.path.join('example', 'LICENSE'),
        ).existsSync(),
        true,
      );
      expect(
        globals.fs.file(globals.fs.path.join('example.d')).existsSync(),
        false,
      );
    },
