...
void main() {
  group('channel', () {
    ...
    testUsingContext(
      ...
      overrides: <Type, Generator>{ProcessManager: () => fakeProcessManager, FileSystem: () => MemoryFileSystem.test()},
    );
    ...
    testUsingContext(
      ...
      () async {
        fakeProcessManager.addCommands(const <FakeCommand>[
          ...
          FakeCommand(command: <String>['bin/flutter', '--no-color', '--no-version-check', 'precache']),
        ]);
        ...
        expect(fakeProcessManager, hasNoRemainingExpectations);
        expect(testLogger.statusText, containsIgnoringWhitespace("Switching to flutter channel 'beta'..."));
        expect(testLogger.errorText, hasLength(0));

        fakeProcessManager.addCommands(const <FakeCommand>[
          FakeCommand(command: <String>['git', 'fetch']),
          FakeCommand(command: <String>['git', 'show-ref', '--verify', '--quiet', 'refs/heads/stable']),
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(testLogger.statusText, containsIgnoringWhitespace("Successfully switched to flutter channel 'beta'."));
    ...
    testUsingContext(
      ...
      () async {
        ...
        final File versionCheckFile = globals.cache.getStampFileFor(VersionCheckStamp.flutterVersionCheckStampFile);
