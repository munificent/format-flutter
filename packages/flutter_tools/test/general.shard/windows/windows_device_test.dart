...
void main() {
  ...
  testWithoutContext(
    'WindowsDevices lists a devices if the workflow is supported',
    () async {
      expect(
        await WindowsDevices(
          windowsWorkflow: WindowsWorkflow(
  ...
  testWithoutContext(
    ...
    () async {
      ...
      fileSystem.file(
        fileSystem.path.join('windows', 'CMakeLists.txt'),
      ).createSync();
      final FlutterProject flutterProject = setUpFlutterProject(
  ...
  testWithoutContext(
    'isSupportedForProject is false with no build file',
    () async {
      final FileSystem fileSystem = MemoryFileSystem.test();
      final WindowsDevice windowsDevice = setUpWindowsDevice(
        fileSystem: fileSystem,
  ...
  testWithoutContext(
    ...
    () async {
      ...
      expect(
        windowsDevice.executablePathForDevice(fakeApp, BuildInfo.debug),
        'debug/executable',
      );
      expect(
        windowsDevice.executablePathForDevice(fakeApp, BuildInfo.profile),
        'profile/executable',
      );
      expect(
        windowsDevice.executablePathForDevice(fakeApp, BuildInfo.release),
        'release/executable',
      );
    },
  );
