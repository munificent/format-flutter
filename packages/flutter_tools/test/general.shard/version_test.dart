...
void main() {
  ...
  testUsingContext('Channel enum and string transform to each other', () {
    ...
    expect(
      kOfficialChannels.toList().map(
        (String str) => getChannelForName(str),
      ).toList(),
      Channel.values,
    );
  });
    ...
    group('$FlutterVersion for $channel', () {
      ...
      testWithoutContext(
        ...
        () async {
          ...
          final Duration frameworkAge = _testClock.now().difference(
      ...
      group('$VersionCheckStamp for $channel', () {
        ...
        testWithoutContext('loads valid JSON', () async {
          final String value = '''
        {
          "lastKnownRemoteVersion": "${_testClock.ago(
                const Duration(days: 1),
              )}",
          "lastTimeVersionWasChecked": "${_testClock.ago(
  ...
  group('VersionUpstreamValidator', () {
    ...
    testWithoutContext(
      ...
      () {
        expect(
          runUpstreamValidator(versionUpstreamUrl: flutterStandardUrlDotGit),
          isNull,
        );
      },
    );
    ...
    testWithoutContext('stripDotGit removes ".git" suffix if any', () {
      expect(VersionUpstreamValidator.stripDotGit(
        'https://github.com/flutter/flutter.git',
    ...
    () async {
      processManager.addCommands(<FakeCommand>[
        ...
        FakeCommand(
          ...
          stdout: _testClock.ago(
            VersionFreshnessValidator.versionAgeConsideredUpToDate('stable') ~/
                2,
          ).toString(),
        ),
      ]);
      ...
      flutterRoot.childDirectory('bin').childDirectory('cache').createSync(
        recursive: true,
      );
      final FlutterVersion flutterVersion = FlutterVersion(
        clock: _testClock,
      ...
      final File versionFile = fs.file(
        '/path/to/flutter/bin/cache/flutter.version.json',
      );
      ...
      expect(versionFile.readAsStringSync(), '''
{
  ...
  testUsingContext(
    ...
    () async {
      ...
      final Directory cacheDir = flutterRoot
          .childDirectory('bin')
          .childDirectory('cache')..createSync(recursive: true);
  ...
  testUsingContext('GitTagVersion', () {
    ...
    expect(
      GitTagVersion.parse('10.20.30-0-g$hash').frameworkVersionFor(hash),
      '10.20.30',
    );
    ...
    expect(
      GitTagVersion.parse('x1.2.3-4-g$hash').frameworkVersionFor(hash),
      '0.0.0-unknown',
    );
    expect(
    ...
    expect(
      GitTagVersion.parse('beta-1-g$hash').frameworkVersionFor(hash),
      '0.0.0-unknown',
    );
    expect(
      GitTagVersion.parse('1.2.3-4-gx$hash').frameworkVersionFor(hash),
      '0.0.0-unknown',
    );
  ...
  testUsingContext(
    ...
    () {
      ...
      expect(
        gitTagVersion.frameworkVersionFor(headRevision),
        '1.2.0-3.0.pre.12',
      );
    },
  );
