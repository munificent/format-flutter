...
const FakeCommand unameCommandForX64 = FakeCommand(command: <String>['uname', '-m'], stdout: 'x86_64');

const FakeCommand unameCommandForArm64 = FakeCommand(command: <String>['uname', '-m'], stdout: 'aarch64');
...
void main() {
  ...
  group('Cache', () {
    ...
    testWithoutContext('Gradle wrapper should not be up to date, if some cached artifact is not available', () {
      ...
      fileSystem.file(fileSystem.path.join(directory.path, 'gradle', 'wrapper', 'gradle-wrapper.jar')).createSync(
        recursive: true,
      );
    ...
    testWithoutContext("getter dyLdLibEntry concatenates the output of each artifact's dyLdLibEntry getter", () async {
      ...
      artifact1.environment = <String, String>{'DYLD_LIBRARY_PATH': '/path/to/alpha:/path/to/beta'};
      artifact2.environment = <String, String>{'DYLD_LIBRARY_PATH': '/path/to/gamma:/path/to/delta:/path/to/epsilon'};
      ...
      expect(cache.dyLdLibEntry.value, '/path/to/alpha:/path/to/beta:/path/to/gamma:/path/to/delta:/path/to/epsilon');
    });
    ...
    testWithoutContext('failed storage.googleapis.com download shows China warning', () async {
      ...
      await expectLater(() => cache.updateAll(<DevelopmentArtifact>{DevelopmentArtifact.universal}), throwsException);
      expect(artifact1.didUpdate, true);
      // Don't continue when retrieval fails.
      expect(artifact2.didUpdate, false);
      expect(logger.errorText, contains('https://flutter.dev/community/china'));
    });

    testWithoutContext('Invalid URI for FLUTTER_STORAGE_BASE_URL throws ToolExit', () async {
      final Cache cache = Cache.test(
        platform: FakePlatform(environment: <String, String>{'FLUTTER_STORAGE_BASE_URL': ' http://foo'}),
        processManager: FakeProcessManager.any(),
      );
  ...
  testWithoutContext('IosUsbArtifacts verifies executables for libimobiledevice in isUpToDateInner', () async {
    ...
    final File ideviceScreenshotFile = iosUsbArtifacts.location.childFile('idevicescreenshot')..createSync();
  ...
  testWithoutContext('FontSubset artifacts on macos', () {
    fakeProcessManager.addCommands(<FakeCommand>[
      ...
      const FakeCommand(command: <String>['sysctl', 'hw.optional.arm64'], stdout: 'hw.optional.arm64: 0'),
    ]);
  ...
  testWithoutContext('Windows desktop artifacts ignore filtering when requested', () {
    final Cache cache = Cache.test(processManager: FakeProcessManager.any());
    final WindowsEngineArtifacts artifacts = WindowsEngineArtifacts(cache, platform: FakePlatform());
    cache.includeAllPlatforms = false;
  ...
  testWithoutContext('FlutterWebSdk fetches web artifacts and deletes previous directory contents', () async {
    ...
    expect(locations, <String>['cache/bin/cache/flutter_web_sdk']);
  ...
  testWithoutContext('FlutterWebSdk CanvasKit URL can be overridden via FLUTTER_STORAGE_BASE_URL', () async {
    ...
    final Cache cache = Cache.test(
      ...
      fileSystem: fileSystem,
      platform: FakePlatform(
        environment: <String, String>{'FLUTTER_STORAGE_BASE_URL': 'https://flutter.storage.com/override'},
      ),
    );
    final Directory webCacheDirectory = cache.getWebSdkDirectory();
    final FakeArtifactUpdater artifactUpdater = FakeArtifactUpdater();
  ...
  testWithoutContext('LegacyCanvasKitRemover removes old canvaskit artifacts if they exist', () async {
    ...
    await remover.update(FakeArtifactUpdater(), BufferLogger.test(), fileSystem, FakeOperatingSystemUtils());
...
''');
    ...
    fileSystem.file('.pub-cache/hosted/pub.dartlang.org/example-7.0.0/pubspec.yaml').createSync(recursive: true);
  ...
  testWithoutContext('PubDependencies updates via pub get', () async {
    ...
    expect(pub.invocations.first, predicate<FakePubInvocation>(
...
class FakeCache extends Cache {
  FakeCache({required super.logger, required super.fileSystem, required super.platform, required super.osUtils})
