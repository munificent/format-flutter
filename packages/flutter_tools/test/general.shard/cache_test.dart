...
void main() {
  ...
  group('Cache.checkLockAcquired', () {
    ...
    testWithoutContext('should not throw when lock is acquired', () async {
      ...
      try {
        ...
        fileSystem.file(
          fileSystem.path.join('bin', 'cache', 'lockfile'),
  ...
  group('Cache', () {
    ...
    testWithoutContext(
      ...
      () async {
        ...
        final File noticeFile = fileSystem.file(
          fileSystem.path.join(artifactDir.path, 'NOTICE'),
        )..createSync(recursive: true);
    ...
    testWithoutContext(
      ...
      () {
        ...
        fileSystem.file(
          fileSystem.path.join(directory.path, 'gradlew'),
        ).createSync(recursive: true);
        fileSystem.file(
          fileSystem.path.join(directory.path, 'gradlew.bat'),
        ).createSync(recursive: true);
    ...
    testWithoutContext('overridden storage base url prints warning', () async {
      ...
      final Cache cache = Cache.test(
        platform: FakePlatform(
          environment: <String, String>{'FLUTTER_STORAGE_BASE_URL': baseUrl},
        ),
        processManager: FakeProcessManager.any(),
        logger: logger,
      );
  ...
  testWithoutContext('flattenNameSubdirs', () {
    expect(flattenNameSubdirs(
      Uri.parse('http://flutter.dev/foo/bar'),
      MemoryFileSystem.test(),
    ), 'flutter.dev/foo/bar');
    expect(flattenNameSubdirs(
      Uri.parse('http://api.flutter.dev/foo/bar'),
      MemoryFileSystem.test(),
  ...
  testWithoutContext(
    ...
    () async {
      ...
      final Directory unzippedFramework = binDir.childDirectory(
  ...
  testWithoutContext(
    ...
    () async {
      ...
      expect(
        iosUsbArtifacts.archiveUri.toString(),
        isNot(contains('/unsigned/')),
      );
    },
  );
  ...
  testWithoutContext(
    'FontSubset artifacts for all platforms on arm64 hosts',
    () {
      fakeProcessManager.addCommand(unameCommandForArm64);
  ...
  testWithoutContext(
    ...
    () {
      final Cache cache = Cache.test(processManager: FakeProcessManager.any());
      final MacOSEngineArtifacts artifacts = MacOSEngineArtifacts(
        cache,
        platform: FakePlatform(),
      );
      cache.includeAllPlatforms = false;
  ...
  testWithoutContext(
    ...
    () {
      ...
      expect(artifacts.getBinaryDirs(), <List<String>>[
        <String>[
          'linux-arm64',
          'linux-arm64-debug/linux-arm64-flutter-gtk.zip',
        ],
        <String>[
          'linux-arm64-profile',
  ...
  testWithoutContext(
    ...
    () {
      ...
      final File toolStampFile = fileSystem.file(
  ...
  testWithoutContext(
    ...
    () async {
      ...
      final File canvasKitVersionFile = internalDir.childFile(
        'canvaskit.version',
      );
  ...
  testWithoutContext('PubDependencies needs to be updated if the package config'
    ...
    expect(
      await pubDependencies.isUpToDate(fileSystem),
      false,
    ); // no package config
  ...
  group('AndroidMavenArtifacts', () {
    ...
    testWithoutContext(
      ...
      () async {
        ...
        expect(
          mavenArtifacts.developmentArtifact,
