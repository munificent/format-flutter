...
void main() {
  testWithoutContext(
    ...
    () async {
      ...
      final GroupedValidator groupedValidator = FakeGroupedValidator(
        <DoctorValidator>[
  ...
  testWithoutContext('Reports null safe analytics events', () {
    ...
    NullSafetyAnalysisEvent(packageConfig, NullSafetyMode.sound, 'foo', usage)
        .send();
  ...
  testWithoutContext('Does not crash if main package is missing', () {
    ...
    final PackageConfig packageConfig = PackageConfig(<Package>[
      Package(
        'foo',
        Uri.parse('file:///foo/lib/'),
        languageVersion: LanguageVersion(2, 12),
      ),
      Package(
        'bar',
        Uri.parse('file:///fizz/lib/'),
        languageVersion: LanguageVersion(2, 1),
      ),
      Package(
        'baz',
        Uri.parse('file:///bar/lib/'),
        languageVersion: LanguageVersion(2, 2),
      ),
    ]);
