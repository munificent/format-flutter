...
void main() {
  group('Features', () {
    ...
    testWithoutContext('retrieves the correct setting for each branch', () {
      const FeatureChannelSetting masterSetting = FeatureChannelSetting(
        available: true,
      );
      const FeatureChannelSetting betaSetting = FeatureChannelSetting(
        available: true,
      );
      const FeatureChannelSetting stableSetting = FeatureChannelSetting(
        available: true,
      );
    ...
    testWithoutContext('Flutter web wasm only enable on master', () {
      expect(
        flutterWebWasm.getSettingForChannel('master').enabledByDefault,
        isTrue,
      );
      expect(
        flutterWebWasm.getSettingForChannel('beta').enabledByDefault,
        isFalse,
      );
      expect(
        flutterWebWasm.getSettingForChannel('stable').enabledByDefault,
        isFalse,
      );
    });
    ...
    for (final String channel in <String>['master', 'beta', 'stable']) {
      testWithoutContext(
        'Custom devices are enabled with flag on $channel',
        () {
