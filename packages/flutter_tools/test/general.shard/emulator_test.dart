...
const FakeEmulator emulator2 = FakeEmulator(
  'Nexus_5X_API_27_x86',
  'Nexus 5X',
  'Google',
);
  ...
  group('EmulatorManager', () {
    ...
    testUsingContext('getEmulators', () async {
      ...
      final EmulatorManager emulatorManager = EmulatorManager(
        ...
        androidSdk: sdk,
        androidWorkflow:
            AndroidWorkflow(androidSdk: sdk, featureFlags: TestFeatureFlags()),
      );

      await expectLater(
        () async => emulatorManager.getAllAvailableEmulators(),
        returnsNormally,
      );
    });
    ...
    testWithoutContext('getEmulatorsById', () async {
      ...
      expect(
        await testEmulatorManager.getEmulatorsMatching('Nexus_5'),
        <Emulator>[emulator1],
      );
      expect(
        await testEmulatorManager.getEmulatorsMatching('Nexus_5X'),
        <Emulator>[emulator2],
      );
      expect(
      ...
      expect(
        await testEmulatorManager.getEmulatorsMatching('Nexus'),
        <Emulator>[emulator1, emulator2],
      );
      expect(
        await testEmulatorManager.getEmulatorsMatching('iOS Simulator'),
        <Emulator>[emulator3],
      );
      expect(await testEmulatorManager.getEmulatorsMatching('ios'), <Emulator>[
        emulator3,
      ]);
    });
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(
          result.error,
          contains('avdmanager is missing from the Android SDK'),
        );
      },
    );
