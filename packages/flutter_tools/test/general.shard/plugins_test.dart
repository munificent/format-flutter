...
void main() {
  group('plugins', () {
    ...
    void setUpProject(FileSystem fileSystem) {
      ...
      flutterProject
        ...
        ..flutterPluginsFile = flutterProject.directory.childFile(
          '.flutter-plugins',
        )
      ...
      final Directory iosDirectory = flutterProject.directory.childDirectory(
        'ios',
      );
      iosProject
        ..pluginRegistrantHost = flutterProject.directory.childDirectory(
          'Runner',
        )
      ...
      final Directory macosDirectory = flutterProject.directory.childDirectory(
        'macos',
      );
      final Directory macosManagedDirectory = macosDirectory.childDirectory(
        'Flutter',
      );
      macosProject
      ...
      final Directory androidDirectory = flutterProject.directory
      ...
      windowsProject
        ...
        ..generatedPluginCmakeFile = windowsManagedDirectory.childFile(
      ...
      final Directory linuxManagedDirectory = flutterProject.directory
          ...
          .childDirectory('flutter');
      final Directory linuxEphemeralDirectory = linuxManagedDirectory
          .childDirectory('ephemeral');
      linuxProject
        ..managedDirectory = linuxManagedDirectory
        ..ephemeralDirectory = linuxEphemeralDirectory
        ..pluginSymlinkDirectory = linuxEphemeralDirectory.childDirectory(
          '.plugin_symlinks',
        )
        ..cmakeFile = linuxManagedDirectory.parent.childFile('CMakeLists.txt')
        ..generatedPluginCmakeFile = linuxManagedDirectory.childFile(
          'generated_plugins.mk',
        )
        ..exists = false;
    }
    ...
    setUp(() async {
      ...
      setUpProject(fs);
      flutterProject.directory.childFile('.packages').createSync(
        recursive: true,
      );
    });
    ...
    List<Directory> createFakePlugins(
      ...
      List<String> pluginNamesOrPaths,
    ) {
  ...
  ''';
      ...
      final Directory fakePubCache = fileSystem.systemTempDirectory
  ...
  plugin:
    ...
    pluginClass: UseNewEmbedding
      ...
      flutterProject.directory.childFile('.packages').writeAsStringSync(
        'plugin1:${pluginUsingJavaAndNewEmbeddingDir.childDirectory(
          'lib',
        ).uri}\n',
...
flutter:
  plugin:
    ...
    pluginClass: UseNewEmbedding
      ...
      flutterProject.directory.childFile('.packages').writeAsStringSync(
        'plugin2:${pluginUsingKotlinAndNewEmbeddingDir.childDirectory(
          'lib',
        ).uri}\n',
        mode: FileMode.append,
      );
...
''');
      ...
      flutterProject.directory.childFile('.packages').writeAsStringSync(
        'plugin4:${pluginUsingJavaAndNewEmbeddingDir.childDirectory(
          'lib',
        ).uri}',
        mode: FileMode.append,
      );
    ...
    group('refreshPlugins', () {
      testUsingContext(
        ...
        () async {
          ...
          expect(
            flutterProject.flutterPluginsDependenciesFile.existsSync(),
            false,
          );
        },
    ...
    group('injectPlugins', () {
      ...
      testUsingContext(
        ...
        () async {
          ...
          final File registrant = flutterProject.directory.childDirectory(
            fs.path.join(
              'android',
              'app',
              'src',
          ...
          expect(registrant.readAsStringSync(), contains(
      ...
      testUsingContext(
        'App using the v1 embedding shows warning',
        () async {
          flutterProject.isModule = false;
          androidProject.embeddingVersion = AndroidEmbeddingVersion.v1;
      ...
      testUsingContext(
        'Module using multiple old plugins all show warnings',
        () async {
          flutterProject.isModule = true;
          androidProject.embeddingVersion = AndroidEmbeddingVersion.v2;
      ...
      testUsingContext(
        'App using multiple old plugins all show warnings',
        () async {
          flutterProject.isModule = false;
          androidProject.embeddingVersion = AndroidEmbeddingVersion.v2;
      ...
      testUsingContext(
        ...
        () async {
          ...
          for (final String plugin in <String>[newPluginName, oldPluginName]) {
            ...
            expect(
              content.split(plugin).first.trim().endsWith('try {'),
              isTrue,
            );
            expect(content.split(plugin).last.trim().startsWith(
              '} catch (Exception e) {',
            ), isTrue);
          }
        },
...
''');
          ...
          final Directory destination = flutterProject.directory.childDirectory(
            'lib',
          );
      ...
      testUsingContext(
        ...
        () async {
          ...
          for (final File file in <File>[pluginCmakeFile, pluginRegistrant]) {
            ...
            expect(
              contents.indexOf('plugin_a'),
              lessThan(contents.indexOf('plugin_b')),
            );
            expect(
              contents.indexOf('plugin_b'),
              lessThan(contents.indexOf('plugin_c')),
            );
            expect(
              contents.indexOf('plugin_c'),
...
flutter:
  ...
  ''';
      ...
      testUsingContext(
        'createPlatformsYamlMap should create the correct map',
        () async {
          final YamlMap map = Plugin.createPlatformsYamlMap(<String>[
            'ios',
            'android',
            'linux',
          ], 'PluginClass', 'some.android.package');
      ...
      testUsingContext(
        'createPlatformsYamlMap should create empty map',
        () async {
    ...
    testWithoutContext(
      ...
      () async {
        ...
        const FileSystemException e = FileSystemException(
          '',
          '',
          OSError('', 1314),
        );
...
class FakeIosProject extends Fake implements IosProject {
  ...
  @override
  File get pluginRegistrantHeader => pluginRegistrantHost.childFile(
    'GeneratedPluginRegistrant.h',
  );

  @override
  File get pluginRegistrantImplementation => pluginRegistrantHost.childFile(
