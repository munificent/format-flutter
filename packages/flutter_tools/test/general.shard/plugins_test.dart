...
void main() {
  group('plugins', () {
    ...
    void setUpProject(FileSystem fileSystem) {
      ...
      final Directory windowsManagedDirectory = flutterProject.directory.childDirectory('windows').childDirectory(
        'flutter',
      );
      windowsProject
        ...
        ..pluginSymlinkDirectory = windowsManagedDirectory.childDirectory('ephemeral').childDirectory(
          '.plugin_symlinks',
        )
        ..exists = false;
      ...
      final Directory linuxManagedDirectory = flutterProject.directory.childDirectory('linux').childDirectory(
        'flutter',
      );
...
flutter:
  plugin:
    ...
    void createNewKotlinPlugin2() {
      final Directory pluginUsingKotlinAndNewEmbeddingDir = fs.systemTempDirectory.createTempSync(
...
flutter:
  plugin:
    ...
    void createOldJavaPlugin(String pluginName) {
      final Directory pluginUsingOldEmbeddingDir = fs.systemTempDirectory.createTempSync(
...
''');
    ...
    Directory createLegacyPluginWithDependencies({required String name, required List<String> dependencies}) {
...
''');
    ...
    group('refreshPlugins', () {
      ...
      testUsingContext(
        ...
        () async {
          ...
          final List<String> expectedKeys = <String>['info', 'plugins', 'dependencyGraph', 'date_created', 'version'];
  ...
  ''');
        webPluginWithNestedFile.childDirectory('lib').childDirectory('src').childFile('web_plugin.dart').createSync(
          recursive: true,
        );
...
''');
        ...
        final File registrant = flutterProject.directory.childDirectory('lib').childFile('web_plugin_registrant.dart');
...
flutter:
  plugin:
    ...
    ''');
          ...
          final File registrantFile = androidProject.pluginRegistrantHost.childDirectory(
...
flutter:
  plugin:
    ...
    ''');
      ...
      }, overrides: <Type, Generator>{FileSystem: () => fsWindows, ProcessManager: () => FakeProcessManager.any()});
    });
  ...
  ''';
      ...
      testUsingContext('createPlatformsYamlMap should create the correct map', () async {
        ...
        expect(map['android'], <String, String>{'pluginClass': 'PluginClass', 'package': 'some.android.package'});
