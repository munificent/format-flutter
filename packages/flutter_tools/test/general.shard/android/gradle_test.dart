...
void main() {
  ...
  group('Gradle local.properties', () {
    ...
    String? propertyFor(String key, File file) {
      final Iterable<String> result = file.readAsLinesSync().where((String line) => line.startsWith('$key=')).map(
...
''';
      ...
      await checkBuildVersion(manifest: manifest, expectedBuildName: '1.0.3', expectedBuildNumber: '4');
      // Values get unset.
      await checkBuildVersion(
        manifest: manifest,
        buildInfo: const BuildInfo(BuildMode.release, null, treeShakeIcons: false),
  ...
  test('Current settings.gradle is in our legacy settings.gradle file set', () {
    ...
    final File legacySettingsDotGradleFiles = globals.fs.file(globals.fs.path.join(
      Cache.flutterRoot!,
      'packages',
      'flutter_tools',
      'gradle',
