...
void main() {
  ...
  testWithoutContext('Finds app bundle when no flavor is used in debug mode', () {
    ...
    final File bundle = findBundleFile(project, BuildInfo.debug, BufferLogger.test(), TestUsage());
  ...
  testWithoutContext('AAB not found', () {
    ...
    expect(testUsage.events, contains(TestUsageEvent(
      ...
      parameters:
          CustomDimensions.fromMap(<String, String>{'cd37': 'androidGradlePluginVersion: 7.5, fileExtension: .aab'}),
    )));
  });
}
