...
void main() {
  group('android_emulator', () {
    ...
    testWithoutContext('uses cleaned up ID if no displayname is set', () {
      ...
      final Map<String, String> properties = <String, String>{'avd.ini.notadisplayname': 'this is not a display name'};
      final AndroidEmulator emulator = AndroidEmulator(
        emulatorID,
        properties: properties,
        logger: BufferLogger.test(),
  ...
  group('Android emulator launch ', () {
    ...
    testWithoutContext('succeeds', () async {
      final AndroidEmulator emulator = AndroidEmulator(
        emulatorID,
        processManager: FakeProcessManager.list(<FakeCommand>[const FakeCommand(command: kEmulatorLaunchCommand)]),
        androidSdk: mockSdk,
        logger: BufferLogger.test(),
      );
    ...
    testWithoutContext('succeeds with coldboot launch', () async {
      ...
      final AndroidEmulator emulator = AndroidEmulator(
        emulatorID,
        processManager: FakeProcessManager.list(<FakeCommand>[FakeCommand(command: kEmulatorLaunchColdBootCommand)]),
        androidSdk: mockSdk,
        logger: BufferLogger.test(),
      );
    ...
    testWithoutContext('prints error on failure', () async {
      ...
      final AndroidEmulator emulator = AndroidEmulator(
        ...
        processManager: FakeProcessManager.list(<FakeCommand>[
          const FakeCommand(command: kEmulatorLaunchCommand, exitCode: 1, stderr: errorText, stdout: 'dummy text'),
        ]),
        androidSdk: mockSdk,
        logger: logger,
      );
    ...
    testWithoutContext('prints nothing on late failure with empty stderr', () async {
      ...
      final AndroidEmulator emulator = AndroidEmulator(
        emulatorID,
        processManager: FakeProcessManager.list(<FakeCommand>[
          FakeCommand(command: kEmulatorLaunchCommand, exitCode: 1, stdout: 'dummy text', completer: Completer<void>()),
        ]),
        androidSdk: mockSdk,
        logger: logger,
      );
    ...
    testWithoutContext('throws if emulator not found', () async {
      ...
      await expectLater(() => emulator.launch(startupDuration: Duration.zero), throwsA(isException.having(
