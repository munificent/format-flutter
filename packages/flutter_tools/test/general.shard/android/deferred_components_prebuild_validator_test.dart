...
void main() {
  ...
  testUsingContext('androidComponentSetup build.gradle does not exist', () async {
    ...
    final File androidManifestTemplate = deferredComponentDir.childDirectory('src').childDirectory('main').childFile(
    ...
    await validator.checkAndroidDynamicFeature(<DeferredComponent>[DeferredComponent(name: 'component1')]);
    validator.displayResults();
    validator.attemptToolExit();
  ...
  testWithoutContext('androidComponentSetup all files exist passes', () async {
    ...
    final File androidManifestTemplate = templatesDir.childDirectory('src').childDirectory('main').childFile(
      'AndroidManifest.xml.tmpl',
    );
