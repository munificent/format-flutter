...
[
  {
    ...
    "s": 2400
      ...
      await builder.buildGradleApp(
        ...
        androidBuildInfo: const AndroidBuildInfo(
          BuildInfo(BuildMode.release, null, treeShakeIcons: false, codeSizeDirectory: 'foo'),
      ...
      expect(testUsage.events, contains(const TestUsageEvent('code-size-analysis', 'apk')));
    }, overrides: <Type, Generator>{AndroidStudio: () => FakeAndroidStudio()});
    ...
    testUsingContext('indicates that an APK has been built successfully', () async {
      ...
      expect(logger.statusText, contains('Built build/app/outputs/flutter-apk/app-release.apk (0.0MB)'));
      expect(processManager, hasNoRemainingExpectations);
    }, overrides: <Type, Generator>{AndroidStudio: () => FakeAndroidStudio()});
    ...
    testUsingContext('Uses namespace attribute if manifest lacks a package attribute', () async {
      ...
      fileSystem.directory(project.android.hostAppGradleRoot).childFile('build.gradle').createSync(recursive: true);

      fileSystem.directory(project.android.hostAppGradleRoot).childDirectory('app').childFile('build.gradle')
        ..createSync(recursive: true)
        ..writeAsStringSync('''
...
BuildVariant: paidProfile
      ...
      final List<String> actual =
          await builder.getBuildVariants(project: FlutterProject.fromDirectoryTest(fileSystem.currentDirectory));
    ...
    testUsingContext('honors --no-android-gradle-daemon setting', () async {
      ...
      await expectLater(() async {
        await builder.buildGradleApp(
          project: FlutterProject.fromDirectoryTest(fileSystem.currentDirectory),
          androidBuildInfo: const AndroidBuildInfo(
            BuildInfo(BuildMode.release, null, treeShakeIcons: false, androidGradleDaemon: false),
          ),
          target: 'lib/main.dart',
          isBuildingBundle: false,
          configOnly: false,
