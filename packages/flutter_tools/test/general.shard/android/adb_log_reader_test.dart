...
void main() {
  testWithoutContext('AdbLogReader ignores spam from SurfaceSyncer', () async {
    ...
    final AdbLogReader logReader = await AdbLogReader.createLogReader(createFakeDevice(null), processManager)
      ..appPid = appPid;
  ...
  testWithoutContext('AdbLogReader calls adb logcat with expected flags apiVersion 21', () async {
    final FakeProcessManager processManager = FakeProcessManager.list(<FakeCommand>[
      const FakeCommand(
        command: <String>['adb', '-s', '1234', 'shell', '-x', 'logcat', '-v', 'time', '-T', "'$kLastLogcatTimestamp'"],
      ),
    ]);
    await AdbLogReader.createLogReader(createFakeDevice(kLollipopVersionCode), processManager);
  ...
  testWithoutContext('AdbLogReader calls adb logcat with expected flags apiVersion < 21', () async {
    final FakeProcessManager processManager = FakeProcessManager.list(<FakeCommand>[
      const FakeCommand(command: <String>['adb', '-s', '1234', 'shell', '-x', 'logcat', '-v', 'time']),
    ]);
    await AdbLogReader.createLogReader(createFakeDevice(kLollipopVersionCode - 1), processManager);
  ...
  testWithoutContext('AdbLogReader calls adb logcat with expected flags null apiVersion', () async {
    ...
    await AdbLogReader.createLogReader(createFakeDevice(null), processManager);
  ...
  testWithoutContext('AdbLogReader calls adb logcat with expected flags when requesting past logs', () async {
    final FakeProcessManager processManager = FakeProcessManager.list(<FakeCommand>[
      const FakeCommand(command: <String>['adb', '-s', '1234', 'shell', '-x', 'logcat', '-v', 'time', '-s', 'flutter']),
    ]);
    await AdbLogReader.createLogReader(createFakeDevice(null), processManager, includePastLogs: true);
