...
class FakeDaemonStreams implements DaemonStreams {
  ...
  void send(Map<String, dynamic> message, [List<int>? binary]) {
    outputs.add(DaemonMessage(
      message,
      binary != null ? Stream<List<int>>.value(binary) : null,
    ));
  }
...
void main() {
  ...
  group('DaemonConnection receiving end', () {
    ...
    testWithoutContext('listenToEvent can receive the right events', () async {
      ...
      expect(
        (await events).map((DaemonEventData event) => event.data).toList(),
        <dynamic>['1', null, 3],
      );
    });
  });
  ...
  group('DaemonConnection request and response', () {
    testWithoutContext('receiving response from requests', () async {
      final Future<dynamic> requestFuture = daemonConnection.sendRequest(
        'some_method',
        'param',
      );
      final DaemonMessage message = await daemonStreams.outputs.stream.first;
  ...
  group('DaemonInputStreamConverter', () {
    ...
    List<int> testCommandBinary(int id, [int? binarySize]) => utf8.encode(
      '[${json.encode(testCommand(id, binarySize))}]\n',
    );

    testWithoutContext('can parse a single message', () async {
      final Stream<List<int>> inputStream = Stream<List<int>>.fromIterable(
        <List<int>>[testCommandBinary(10)],
      );
      final DaemonInputStreamConverter converter = DaemonInputStreamConverter(
        inputStream,
      );
    ...
    testWithoutContext('can parse multiple messages', () async {
      final Stream<List<int>> inputStream = Stream<List<int>>.fromIterable(
        <List<int>>[testCommandBinary(10), testCommandBinary(20)],
      );
      final DaemonInputStreamConverter converter = DaemonInputStreamConverter(
        inputStream,
      );
    ...
    testWithoutContext(
      'can parse a single message with binary stream',
      () async {
        final List<int> binary = <int>[1, 2, 3, 4, 5];
        final Stream<List<int>> inputStream = Stream<List<int>>.fromIterable(
          <List<int>>[testCommandBinary(10, binary.length), binary],
        );
        final DaemonInputStreamConverter converter = DaemonInputStreamConverter(
          inputStream,
        );
    ...
    testWithoutContext(
      'can parse multiple messages with binary stream',
      () async {
  ...
  group('DaemonStreams', () {
    ...
    testWithoutContext('handles sending to a closed sink', () async {
      ...
      daemonStreams.send(testCommand);
      expect(bufferLogger.errorText, contains(
...
Future<List<_DaemonMessageAndBinary>> _readAllBinaries(
  Stream<DaemonMessage> inputStream,
) async {
  final StreamIterator<DaemonMessage> iterator = StreamIterator<DaemonMessage>(
    inputStream,
  );
  ...
  while (await iterator.moveNext()) {
    ...
    if (iterator.current.binary != null) {
      binary = await iterator.current.binary!.reduce(
        (List<int> a, List<int> b) => a + b,
      );
    }
