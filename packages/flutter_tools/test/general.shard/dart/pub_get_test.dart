...
''';
  ...
  testWithoutContext('pub get shows working directory on process exception', () async {
    ...
    final FakeProcessManager processManager = FakeProcessManager.list(<FakeCommand>[
      FakeCommand(
        ...
        onRun: () {
          throw const ProcessException('bin/cache/dart-sdk/bin/dart', <String>[
            'pub',
            '--suppress-analytics',
            '--directory',
            '.',
  ...
  testWithoutContext('pub cache in flutter root is ignored', () async {
    ...
    expect(mockStdio.stdout.writes.map(utf8.decode), <String>["FakeCommand's env successfully matched"]);
    expect(processManager, hasNoRemainingExpectations);
  });
  ...
  testWithoutContext('Preloaded packages are added to the pub cache', () async {
    ...
    final FakeProcessManager processManager = FakeProcessManager.list(<FakeCommand>[
      ...
      FakeCommand(
        ...
        environment: const <String, String>{'FLUTTER_ROOT': '', 'PUB_ENVIRONMENT': 'flutter_cli:flutter_tests'},
        onRun: () {
          fileSystem.currentDirectory.childDirectory('.dart_tool').childFile('package_config.json').createSync(
            recursive: true,
          );
        },
      ),
  ...
  testWithoutContext('Notifies about existing caches, on first run only', () async {
    ...
    expect(logger.statusText, contains('It can be reset by running `dart pub cache clean`'));
    expect(logger.statusText, contains('Found an existing Dart Analysis Server cache at /global/.dartServer'));
    expect(preloadCache.existsSync(), false);
    logger.clear();
    await pub.get(
      project: FlutterProject.fromDirectoryTest(fileSystem.currentDirectory),
  ...
  testWithoutContext('pub cache in environment is used', () async {
    ...
    final Pub pub = Pub.test(
      ...
      botDetector: const BotDetectorAlwaysNo(),
      stdio: mockStdio,
      platform: FakePlatform(environment: const <String, String>{'PUB_CACHE': 'custom/pub-cache/path'}),
    );
  ...
  testWithoutContext('analytics sent on success', () async {
    ...
    expect(usage.events, contains(const TestUsageEvent('pub-result', 'flutter-tests', label: 'success')));
  });
...
''');
  ...
  testWithoutContext('analytics sent on failure', () async {
    ...
    expect(usage.events, contains(const TestUsageEvent('pub-result', 'flutter-tests', label: 'failure')));
    expect(processManager, hasNoRemainingExpectations);
  });
