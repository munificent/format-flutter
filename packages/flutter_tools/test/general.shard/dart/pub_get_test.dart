...
void main() {
  ...
  testWithoutContext(
    ...
    () async {
      ...
      fileSystem.file('pubspec.yaml').createSync();
      fileSystem.file('pubspec.lock').createSync();
      fileSystem.file('.dart_tool/package_config.json').createSync(
        recursive: true,
      );
      fileSystem.file('.dart_tool/version').writeAsStringSync('a');
...
''';
  ...
  testWithoutContext('Preloaded packages are added to the pub cache', () async {
    final FileSystem fileSystem = MemoryFileSystem.test();
    final Directory preloadCache = fileSystem.currentDirectory.childDirectory(
      '.pub-preload-cache',
    );
  ...
  testWithoutContext(
    ...
    () async {
      ...
      final FakeProcessManager processManager = FakeProcessManager.list(
        <FakeCommand>[dartPreloadCommand, dartPubGetCommand, dartPubGetCommand],
      );
...
''');
  ...
  testWithoutContext('Pub error handling', () async {
    ...
    fileSystem.file('.dart_tool/package_config.json').setLastModifiedSync(
      DateTime(2000),
    );
