...
void main() {
  ...
  testWithoutContext(
    'Config does not error on a file with a deprecated field',
    () {
...
''');
  ...
  testWithoutContext(
    ...
    () {
      ...
      expect(
        bufferLogger.errorText,
        contains(r'sudo chown -R $(whoami) /testfile'),
      );
    },
  );
  ...
  testWithoutContext('Config in home dir is used if it exists', () {
    memoryFileSystem.file('.flutter_example').writeAsStringSync(
      '{"hello":"bar"}',
    );
    config = Config(
      'example',
    ...
    expect(
      memoryFileSystem.file('.config/flutter/example').existsSync(),
      false,
    );
  });
