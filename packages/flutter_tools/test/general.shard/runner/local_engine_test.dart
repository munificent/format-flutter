...
void main() {
  testWithoutContext(
    ...
    () async {
      ...
      fileSystem.file('bin/cache/pkg/sky_engine/lib').createSync(
        recursive: true,
      );
  ...
  testWithoutContext(
    ...
    () async {
      ...
      final Directory localEngine = fileSystem.directory(
        '$kArbitraryEngineRoot/src/out/ios_debug/',
      )..createSync(recursive: true);
      fileSystem
          .directory('$kArbitraryEngineRoot/src/out/host_debug/')
          .createSync(recursive: true);
  ...
  testWithoutContext('works if local engine is host engine', () async {
    final FileSystem fileSystem = MemoryFileSystem.test();
    final Directory localEngine = fileSystem.directory(
      '$kArbitraryEngineRoot/src/out/host_debug/',
    )..createSync(recursive: true);
  ...
  testWithoutContext(
    'works if local engine is host engine with suffixes',
    () async {
      final FileSystem fileSystem = MemoryFileSystem.test();
      final Directory localEngine = fileSystem.directory(
  ...
  testWithoutContext(
    ...
    () async {
      final FileSystem fileSystem = MemoryFileSystem.test();
      fileSystem.file(kDotPackages).writeAsStringSync('\n');
      fileSystem.directory('$kEngineRoot/src/out/ios_debug').createSync(
        recursive: true,
      );
      fileSystem.directory('$kEngineRoot/src/out/host_debug').createSync(
        recursive: true,
      );
      fileSystem.file('bin/cache/pkg/sky_engine/lib').createSync(
        recursive: true,
    ...
    () async {
      final LocalEngineLocator localWebEngineLocator = LocalEngineLocator(
        ...
        platform: FakePlatform(
          environment: <String, String>{'FLUTTER_ENGINE': 'blah'},
        ),
      );
...
Matcher matchesEngineBuildPaths({String? hostEngine, String? targetEngine}) {
  return const TypeMatcher<EngineBuildPaths>()
      .having(
        (EngineBuildPaths paths) => paths.hostEngine,
        'hostEngine',
        hostEngine,
      )
      .having(
        (EngineBuildPaths paths) => paths.targetEngine,
        'targetEngine',
        targetEngine,
      );
}
