...
void main() {
  group('FlutterCommandRunner', () {
    ...
    setUp(() {
      ...
      platform = FakePlatform(environment: <String, String>{'FLUTTER_ROOT': _kFlutterRoot}, version: '1 2 3 4 5');
    });
    ...
    group('run', () {
      ...
      group('getRepoPackages', () {
        ...
        setUp(() {
          ...
          fileSystem.directory(fileSystem.path.join(_kFlutterRoot, 'examples')).createSync(recursive: true);
          fileSystem.directory(fileSystem.path.join(_kFlutterRoot, 'packages')).createSync(recursive: true);
          fileSystem.directory(fileSystem.path.join(_kFlutterRoot, 'dev', 'tools', 'aatool')).createSync(
            recursive: true,
          );

          fileSystem.file(fileSystem.path.join(_kFlutterRoot, 'dev', 'tools', 'pubspec.yaml')).createSync();
          fileSystem.file(fileSystem.path.join(_kFlutterRoot, 'dev', 'tools', 'aatool', 'pubspec.yaml')).createSync();
        });
        ...
        testUsingContext(
          ...
          () {
            final FlutterCommandRunner runner = createTestCommandRunner(DummyFlutterCommand()) as FlutterCommandRunner;
            final List<String> packagePaths = runner.getRepoPackages().map((Directory d) => d.path).toList();
