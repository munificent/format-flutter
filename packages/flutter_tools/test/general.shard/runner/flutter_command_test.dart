...
void main() {
  group('Flutter Command', () {
    ...
    testUsingContext(
      ...
      () async {
        final DummyFlutterCommand flutterCommand = DummyFlutterCommand();
        await expectToolExitLater(flutterCommand.run(), contains('Flutter SDK installation appears corrupted'));
      },
      overrides: <Type, Generator>{
        Cache: () => cache,
        FileSystem: () => fileSystem,
    ...
    testUsingContext('deprecated command should warn', () async {
      ...
      expect(flutterCommand.usage, contains(
    ...
    testUsingContext('devToolsServerAddress returns parsed uri', () async {
      ...
      await createTestCommandRunner(command)
          .run(<String>['dummy', '--${FlutterCommand.kDevToolsServerAddress}', 'http://127.0.0.1:9105']);
    ...
    testUsingContext('devToolsServerAddress returns null for bad input', () async {
      ...
      await runner.run(<String>['dummy', '--${FlutterCommand.kDevToolsServerAddress}', 'hello-world']);
      ...
      await runner.run(<String>['dummy', '--${FlutterCommand.kDevToolsServerAddress}', '127.0.0.1:9101']);
      expect(command.devToolsServerAddress, isNull);
    }, overrides: <Type, Generator>{FileSystem: () => fileSystem, ProcessManager: () => processManager});
    ...
    group('signals tests', () {
      ...
      testUsingContext(
        ...
        overrides: <Type, Generator>{
          ...
          ProcessManager: () => processManager,
          ProcessInfo: () => processInfo,
          Signals: () => FakeSignals(subForSigTerm: signalUnderTest, exitSignals: <ProcessSignal>[signalUnderTest]),
          SystemClock: () => clock,
          Usage: () => usage,
        },
      );
    ...
    testUsingCommandContext('report execution timing by default', () async {
      ...
      expect(
        usage.timings,
        contains(const TestTimingEvent('flutter', 'dummy', Duration(milliseconds: 1000), label: 'fail')),
      );
    });
    ...
    testUsingCommandContext('report additional FlutterCommandResult data', () async {
      ...
      expect(usage.timings, contains(
        const TestTimingEvent('flutter', 'dummy', Duration(milliseconds: 500), label: 'success-blah1-blah2-blah3'),
      ));
    });
