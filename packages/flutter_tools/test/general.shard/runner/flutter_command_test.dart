  ...
  group('Flutter Command', () {
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(
          () async => runner.run(<String>['test', '-t', 'lib/foo.dart']),
          throwsToolExit(),
        );
      },
      overrides: <Type, Generator>{
    ...
    testUsingContext(
      ...
      () async {
        ...
        expect(
          command.devToolsServerAddress.toString(),
...
''');
    ...
    testUsingContext(
      ...
      () async {
        ...
        await runner.run(<String>[
          'dummy',
          '--no-assume-initialize-from-dill-up-to-date',
        ]);
        final BuildInfo buildInfo =
            await flutterCommand.getBuildInfo(forcedBuildMode: BuildMode.debug);
...
class FakeSignals implements Signals {
  ...
  @override
  Future<bool> removeHandler(ProcessSignal signal, Object token) => delegate
      .removeHandler(signal, token);
