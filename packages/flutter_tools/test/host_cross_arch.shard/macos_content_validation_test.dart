...
void main() {
  ...
  setUpAll(() {
    processManager
        .runSync(<String>[flutterBin, 'config', '--enable-macos-desktop']);
  });
  ...
  for (final String buildMode in <String>['Debug', 'Release']) {
    ...
    test(
      'flutter build macos --$buildModeLower builds a valid app',
      () {
        ...
        final Directory outputApp = buildPath.childDirectory(
          'Flutter Gallery.app',
        );
        final Directory outputAppFramework = fileSystem.directory(fileSystem
            .path
            .join(outputApp.path, 'Contents', 'Frameworks', 'App.framework'));
        ...
        final List<String> libSymbols = AppleTestUtils.getExportedSymbols(
          libBinary.path,
        );
...
void _checkFatBinary(File file, String buildModeLower, String expectedType) {
  final String archs =
      processManager.runSync(<String>['file', file.path]).stdout as String;
