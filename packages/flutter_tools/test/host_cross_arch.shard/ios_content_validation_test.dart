    ...
    () {
      ...
      setUpAll(() {
        ...
        final Directory xcframeworkArtifact = fileSystem.directory(
          fileSystem.path.join(flutterRoot, 'bin', 'cache', 'artifacts', 'engine', 'ios', 'Flutter.xcframework'),
        );
      ...
      for (final BuildMode buildMode in <BuildMode>[BuildMode.debug, BuildMode.release]) {
        group('build in ${buildMode.cliName} mode', () {
          ...
          setUpAll(() {
            ...
            outputPath = fileSystem.directory(fileSystem.path.join(projectRoot, 'build', 'ios', 'iphoneos'));
            ...
            buildPath = fileSystem.directory(
              fileSystem.path.join(projectRoot, 'build', 'ios', '${sentenceCase(buildMode.cliName)}-iphoneos'),
            );
          ...
          testWithoutContext('Info.plist dart VM Service Bonjour service', () {
            ...
            final ProcessResult bonjourServices = processManager
                .runSync(<String>['plutil', '-extract', 'NSBonjourServices', 'xml1', '-o', '-', infoPlistPath]);
          ...
          testWithoutContext('check symbols', () {
            final List<String> symbols = AppleTestUtils.getExportedSymbols(outputAppFrameworkBinary.path);
          ...
          testWithoutContext('check symbols in dSYM', () {
            if (buildMode == BuildMode.debug) {
              ...
              final List<String> symbols = AppleTestUtils.getExportedSymbols(buildAppFrameworkDsymBinary.path);
          ...
          testWithoutContext('xcode_backend embed_and_thin', () {
            ...
            final ProcessResult xcodeBackendResult = processManager.runSync(
              ...
              environment: <String, String>{
                ...
                'BUILT_PRODUCTS_DIR': fileSystem.path.join(projectRoot, 'build', 'ios', 'Release-iphoneos'),
          ...
          testWithoutContext('validate obfuscation', () {
            ...
            ProcessResult grepResult = processManager.runSync(<String>['grep', '-r', 'HelloPlugin', pluginRoot.path]);
            // Matches exits 0.
            expect(grepResult.exitCode, 0);

            // Not present in binary.
            grepResult = processManager.runSync(<String>['grep', 'HelloPlugin', outputAppFrameworkBinary.path]);
      ...
      testWithoutContext('builds all plugin architectures for simulator', () {
        ...
        final ProcessResult archs = processManager.runSync(<String>['file', pluginFrameworkBinary.path]);
      ...
      testWithoutContext('build for simulator with all available architectures', () {
        ...
        final ProcessResult archs = processManager.runSync(<String>['file', simulatorAppFrameworkBinary.path]);
        expect(archs.stdout, contains('Mach-O 64-bit dynamically linked shared library x86_64'));
        expect(archs.stdout, contains('Mach-O 64-bit dynamically linked shared library arm64'));
      });
    },
...
bool _containsBitcode(String pathToBinary, ProcessManager processManager) {
  ...
  final ProcessResult result = processManager.runSync(<String>['otool', '-l', '-arch', 'arm64', pathToBinary]);
  ...
  lines.asMap().forEach((int index, String line) {
    if (line.contains('segname __LLVM') && lines.length - index - 1 > 3) {
      final bool bitcodeMarkerFound = lines.skip(index - 1).take(4).any(
