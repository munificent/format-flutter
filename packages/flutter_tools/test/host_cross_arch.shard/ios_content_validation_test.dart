  ...
  group(
    ...
    () {
      ...
      setUpAll(() {
        ...
        hiddenFile = xcframeworkArtifact.childFile('._Info.plist')
          ..createSync();
      ...
      for (final BuildMode buildMode in <BuildMode>[
        ...
        group('build in ${buildMode.cliName} mode', () {
          ...
          setUpAll(() {
            ...
            outputFlutterFramework = frameworkDirectory.childDirectory(
              'Flutter.framework',
            );
            outputFlutterFrameworkBinary = outputFlutterFramework.childFile(
              'Flutter',
            );

            outputAppFramework = frameworkDirectory.childDirectory(
              'App.framework',
            );
            ...
            buildAppFrameworkDsym = buildPath.childDirectory(
              'App.framework.dSYM',
            );
          ...
          testWithoutContext('flutter build ios builds a valid app', () {
            ...
            expect(
              buildAppFrameworkDsymBinary.existsSync(),
      ...
      testWithoutContext('builds all plugin architectures for simulator', () {
        ...
        final ProcessResult archs = processManager
            .runSync(<String>['file', pluginFrameworkBinary.path]);
        expect(
          archs.stdout,
      ...
      testWithoutContext(
        ...
        () {
          ...
          final ProcessResult archs = processManager
              .runSync(<String>['file', simulatorAppFrameworkBinary.path]);
          expect(
            archs.stdout,
            contains('Mach-O 64-bit dynamically linked shared library x86_64'),
          );
...
bool _containsBitcode(String pathToBinary, ProcessManager processManager) {
  ...
  final ProcessResult result = processManager
      .runSync(<String>['otool', '-l', '-arch', 'arm64', pathToBinary]);
