...
final String _flutterBin = path.join(
...
Future<void> main() async {
  group('Integration binding result', () {
    test('when multiple tests pass', () async {
      ...
      expect(results, equals(<String, dynamic>{
    ...
    test('when multiple tests fail', () async {
      ...
      expect(
        results,
        containsPair('failing test 1', contains(_failureExcerpt)),
      );
      expect(
        results,
        containsPair('failing test 2', contains(_failureExcerpt)),
      );
    });
