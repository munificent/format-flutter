...
class _ConsistentTextRangeImplementationMatcher extends Matcher {
  _ConsistentTextRangeImplementationMatcher(
    int length,
  ...
  Description describeMismatch(
    ...
    return mismatchDescription.add(
      'at position $position, expected ${TextRange(
        start: leading,
        end: trailing,
      )} but got ${boundary.getTextBoundaryAt(position)}',
    );
  }
...
Matcher _hasConsistentTextRangeImplementationWithinRange(
  int length,
...
void main() {
  ...
  test('line boundary works', () {
    ...
    expect(
      boundary.getLeadingTextBoundaryAt(3),
      TestTextLayoutMetrics.lineAt3.start,
    );
    expect(
      boundary.getTrailingTextBoundaryAt(3),
...
class TestTextLayoutMetrics extends TextLayoutMetrics {
  static const TextSelection lineAt3 = TextSelection(
    baseOffset: 0,
    extentOffset: 10,
  );
