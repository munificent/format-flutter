...
void main() {
  ...
  group('TextSelection', () {
    test('The invalid selection is a singleton', () {
      const TextSelection invalidSelection1 = TextSelection(baseOffset: -1, extentOffset: 0, isDirectional: true);
    ...
    test('TextAffinity does not affect equivalence when the selection is not collapsed', () {
      ...
      const TextSelection selection2 = TextSelection(baseOffset: 1, extentOffset: 2, affinity: TextAffinity.upstream);
    ...
    group('replaced', () {
      ...
      test('replace before selection', () {
        ...
        expect(
          ...
          const TextEditingValue(text: testText, selection: selection).replaced(
      ...
      test('replace after selection', () {
        ...
        expect(
          ...
          const TextEditingValue(text: testText, selection: selection).replaced(
            const TextRange(start: 13, end: 14),
            'AA',
          ),
  ...
  group('TextInputConfiguration', () {
    ...
    test('text serializes to JSON', () async {
      ...
      expect(json['inputType'], <String, dynamic>{'name': 'TextInputType.text', 'signed': null, 'decimal': null});
    ...
    test('number serializes to JSON', () async {
      ...
      final Map<String, dynamic> json = configuration.toJson();
      expect(json['inputType'], <String, dynamic>{'name': 'TextInputType.number', 'signed': false, 'decimal': true});
    ...
    test('basic structure', () async {
      ...
      expect(
        ...
        'TextEditingValue(text: \u2524\u251C, selection: ${const TextSelection.collapsed(
          offset: -1,
        )}, composing: ${TextRange.empty})',
      );
      expect(
        const TextEditingValue(text: 'Sample Text').toString(),
        'TextEditingValue(text: \u2524Sample Text\u251C, selection: ${const TextSelection.collapsed(
          offset: -1,
        )}, composing: ${TextRange.empty})',
      );
    });
    ...
    test('TextInputClient performPrivateCommand method is called', () async {
      ...
      final ByteData? messageBytes = const JSONMessageCodec().encodeMessage(<String, dynamic>{
        'args': <dynamic>[1, jsonDecode('{"action": "actionCommand", "data": {"input_context" : "abcdefg"}}')],
    ...
    test('TextInputClient performPrivateCommand method is called with float', () async {
      ...
      // Send performPrivateCommand message.
      final ByteData? messageBytes = const JSONMessageCodec().encodeMessage(<String, dynamic>{
        'args': <dynamic>[1, jsonDecode('{"action": "actionCommand", "data": {"input_context" : 0.5}}')],
        'method': 'TextInputClient.performPrivateCommand',
      });
      await binding.defaultBinaryMessenger.handlePlatformMessage('flutter/textinput', messageBytes, (ByteData? _) {});
    ...
    test('TextInputClient performPrivateCommand method is called with CharSequence array', () async {
      ...
      // Send performPrivateCommand message.
      final ByteData? messageBytes = const JSONMessageCodec().encodeMessage(<String, dynamic>{
        'args': <dynamic>[1, jsonDecode('{"action": "actionCommand", "data": {"input_context" : ["abc", "efg"]}}')],
        'method': 'TextInputClient.performPrivateCommand',
      });
      await binding.defaultBinaryMessenger.handlePlatformMessage('flutter/textinput', messageBytes, (ByteData? _) {});
    ...
    test('TextInputClient performPrivateCommand method is called with CharSequence', () async {
      ...
      // Send performPrivateCommand message.
      final ByteData? messageBytes = const JSONMessageCodec().encodeMessage(<String, dynamic>{
        'args': <dynamic>[1, jsonDecode('{"action": "actionCommand", "data": {"input_context" : "abc"}}')],
        'method': 'TextInputClient.performPrivateCommand',
      });
      await binding.defaultBinaryMessenger.handlePlatformMessage('flutter/textinput', messageBytes, (ByteData? _) {});
    ...
    test('TextInputClient performPrivateCommand method is called with float array', () async {
      ...
      // Send performPrivateCommand message.
      final ByteData? messageBytes = const JSONMessageCodec().encodeMessage(<String, dynamic>{
        'args': <dynamic>[1, jsonDecode('{"action": "actionCommand", "data": {"input_context" : [0.5, 0.8]}}')],
        'method': 'TextInputClient.performPrivateCommand',
      });
      await binding.defaultBinaryMessenger.handlePlatformMessage('flutter/textinput', messageBytes, (ByteData? _) {});
  ...
  group('Scribble interactions', () {
    ...
    test('TextInputClient requestElementsInRect', () async {
      ...
      void registerElements(FakeScribbleElement element) => TextInput.registerScribbleElement(
        element.elementIdentifier,
        element,
      );
      void unregisterElements(FakeScribbleElement element) => TextInput.unregisterScribbleElement(
        element.elementIdentifier,
      );
  ...
  test('TextEditingValue.isComposingRangeValid', () async {
    ...
    expect(const TextEditingValue(text: 'test', composing: TextRange(start: 1, end: 0)).isComposingRangeValid, isFalse);

    // The composing range is out of range for the text.
    expect(const TextEditingValue(text: 'test', composing: TextRange(start: 1, end: 5)).isComposingRangeValid, isFalse);
    ...
    expect(const TextEditingValue(text: 'test', composing: TextRange(start: 1, end: 4)).isComposingRangeValid, isTrue);
  });
  ...
  group('TextInputControl', () {
    ...
    test('does not interfere with platform text input', () {
      ...
      fakeTextChannel.incoming!(
        MethodCall('TextInputClient.updateEditingState', <dynamic>[1, TextEditingValue.empty.toJSON()]),
      );
