...
void main() {
  ...
  group('Standard method codec', () {
    ...
    test('Should encode and decode objects produced from codec', () {
      final ByteData? data = messageCodec
          .encodeMessage(<Object, Object>{'foo': true, 3: 'fizz'});
    ...
    test('should decode error envelope with native stacktrace.', () {
      ...
      expect(() => method.decodeEnvelope(errorData), throwsA(
        predicate((PlatformException e) => e.stacktrace == 'errorStacktrace'),
      ));
    });
  ...
  group('Json method codec', () {
    ...
    test('should decode error envelope with native stacktrace.', () {
      final ByteData? errorData = stringCodec.encodeMessage(json.encode(<
        dynamic
      >['errorCode', 'errorMessage', 'errorDetails', 'errorStacktrace']));
      expect(() => jsonMethodCodec.decodeEnvelope(errorData!), throwsA(
        predicate((PlatformException e) => e.stacktrace == 'errorStacktrace'),
      ));
    });
  });
  ...
  group('Standard message codec', () {
    ...
    test('should encode sizes correctly at boundary cases', () {
      checkEncoding<dynamic>(standard, Uint8List(253), <int>[
        8,
        253,
        ...List<int>.filled(253, 0),
      ]);
