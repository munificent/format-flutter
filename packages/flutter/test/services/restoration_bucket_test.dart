...
void main() {
  test('root bucket values', () {
    ...
    final RestorationBucket bucket = RestorationBucket.root(
      manager: manager,
      rawData: rawData,
    );
  ...
  test('child bucket values', () {
    ...
    manager.doSerialization();
    expect(((rootRawData[childrenMapKey] as Map<String, dynamic>)['child1']
            as Map<String, dynamic>)
        .containsKey('foo'), isFalse);
    expect(manager.updateScheduled, isFalse);
  ...
  test('claim child with existing data', () {
    ...
    final Object debugOwner = Object();
    final RestorationBucket child = bucket.claimChild(
      'child1',
      debugOwner: debugOwner,
    );
  ...
  test('claim child that is already claimed throws if not given up', () {
    ...
    final RestorationBucket child1 = bucket.claimChild(
      'child1',
      debugOwner: 'FirstClaim',
    );
    ...
    final RestorationBucket child2 = bucket.claimChild(
      'child1',
      debugOwner: 'SecondClaim',
    );
    expect(child2.restorationId, 'child1');
    expect(
      child2.read<int>('foo'),
      isNull,
    ); // Value does not exist in this child.

    // child1 is not given up before running finalizers.
    expect(() => manager.doSerialization(), throwsA(isFlutterError.having(
  ...
  test('claiming a claimed child twice and only giving it up once throws', () {
    ...
    expect(child2.restorationId, 'child1');
    child1.dispose();
    final RestorationBucket child3 = bucket.claimChild(
      'child1',
      debugOwner: 'ThirdClaim',
    );
  ...
  test('cleans up raw data if last value/child is dropped', () {
    ...
    final RestorationBucket child = root.claimChild(
      'child1',
      debugOwner: 'owner',
    );
    child.dispose();
  ...
  test('rename to unused id', () {
    ...
    expect(
      (rawData[childrenMapKey] as Map<Object?, Object?>).containsKey('child1'),
      isFalse,
    );
    expect(
      (rawData[childrenMapKey] as Map<Object?, Object?>)['new-name'],
      rawChildData,
    );
  });
  ...
  test('renaming a to be added child', () {
    ...
    expect(
      (rawData[childrenMapKey] as Map<String, dynamic>)['foo'],
      isEmpty,
    ); // new bucket
  });
  ...
  test('adopt fresh child', () {
    ...
    expect((rawData[childrenMapKey] as Map<String, dynamic>).containsKey(
      'fresh-child',
    ), isTrue);
    expect(
  ...
  test('adopt child that already had a parent', () {
    ...
    final RestorationBucket childOfChild = child.claimChild(
      'childOfChild',
      debugOwner: 'owner2',
    );
    ...
    expect(
      (rawData[childrenMapKey] as Map<String, dynamic>)['childOfChild'],
      childOfChildData,
    );
  });
  ...
  test(
    'adopting child does not throw if id is already in use and given up',
    () {
      final MockRestorationManager manager = MockRestorationManager();
      final Map<String, dynamic> rawData = _createRawDataSet();
      final RestorationBucket root = RestorationBucket.root(
  ...
  test('adopting a to-be-added child under an already in use id', () {
    ...
    final RestorationBucket child1OfChild1 = child1.claimChild(
      'child2',
      debugOwner: 'owner2',
    );
    child1OfChild1.write<String>('hello', 'world');
    final RestorationBucket child2OfChild1 = child1.claimChild(
      'child2',
      debugOwner: 'owner2',
    );
  ...
  test('throws when used after dispose', () {
    ...
    expect(
      () => bucket.claimChild('child', debugOwner: null),
      throwsFlutterError,
    );
    final RestorationBucket child = RestorationBucket.empty(
      restorationId: 'child',
