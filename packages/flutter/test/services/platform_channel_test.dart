...
void main() {
    ...
    test('can send string message and get reply', () async {
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMessageHandler(
            'ch',
            (ByteData? message) async => string.encodeMessage(
              '${string.decodeMessage(message)!} world',
            ),
          );
  ...
  group('MethodChannel', () {
    ...
    const OptionalMethodChannel optionalMethodChannel = OptionalMethodChannel(
      'ch8',
      jsonMethod,
    );
    tearDown(() {
      ...
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMessageHandler('ch7', (ByteData? message) async {
            ...
            if (methodCall['method'] == 'sayHello') {
              ...
              return jsonMessage
                  .encodeMessage(<dynamic>['unknown', null, null]);
            }
          });
    ...
    test('can invoke list method and get result', () async {
      ...
      expect(
        channel.invokeMethod<List<String>>('sayHello', 'hello'),
        throwsA(isA<TypeError>()),
      );
      expect(
        await channel.invokeListMethod<String>('sayHello', 'hello'),
        <String>['hello', 'world'],
      );
    });
    ...
    test('can invoke map method and get result', () async {
      ...
      expect(
        await channel.invokeMapMethod<String, String>('sayHello', 'hello'),
    ...
    test('can invoke method and get error', () async {
      ...
      expect(() => channel.invokeMethod<dynamic>('sayHello', 'hello'), throwsA(
    ...
    test('can invoke unimplemented method', () async {
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMessageHandler('ch7', (ByteData? message) async => null);
      expect(() => channel.invokeMethod<void>('sayHello', 'hello'), throwsA(
    ...
    test('can invoke unimplemented method (optional)', () async {
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMessageHandler('ch8', (ByteData? message) async => null);
      final String? result =
    ...
    test(
      'can handle method call with no registered plugin (setting before)',
      () async {
        channel.setMethodCallHandler(null);
        final ByteData call = jsonMethod.encodeMethodCall(
          const MethodCall('sayHello', 'hello'),
        );
        ByteData? envelope;
        await TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
    ...
    test('can handle method call with expressive error result', () async {
      ...
      expect(() => jsonMethod.decodeEnvelope(envelope!), throwsA(
        isA<PlatformException>().having(
          (PlatformException e) => e.code,
          'code',
          equals('bad'),
    ...
    test('can check the mock handler', () async {
      ...
      expect(TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .checkMockMessageHandler(channel.name, null), true);
      expect(TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .checkMockMessageHandler(channel.name, handler), false);
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(channel, handler);
      expect(TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .checkMockMessageHandler(channel.name, handler), true);
    });
  });
  ...
  group('EventChannel', () {
    ...
    test('can receive error event', () async {
      ...
      channel.receiveBroadcastStream('hello').listen(
        events.add,
        onError: errors.add,
      );
      await Future<void>.delayed(Duration.zero);
