...
void main() {
  testWidgetsWithLeakTracking('debugPrintGestureArenaDiagnostics', (WidgetTester tester) async {
    ...
    expect(
      log[1],
      equalsIgnoringHashCodes('Gesture arena 1    ❙ Adding: TapGestureRecognizer#00000(state: ready, button: 1)'),
    );
    log.clear();
    ...
    expect(log[1], equalsIgnoringHashCodes(
  ...
  testWidgetsWithLeakTracking('debugPrintRecognizerCallbacksTrace', (WidgetTester tester) async {
    ...
    expect(log[0], equalsIgnoringHashCodes(
    ...
    expect(log[2], equalsIgnoringHashCodes(
  ...
  testWidgetsWithLeakTracking(
    ...
    (WidgetTester tester) async {
      ...
      expect(log[3], equalsIgnoringHashCodes(
        '                   ❙ TapGestureRecognizer#00000(state: ready, won arena, finalPosition: Offset(12.0, 8.0), button: 1, sent tap down) calling onTapUp callback.',
      ));
      expect(log[4], equalsIgnoringHashCodes(
        '                   ❙ TapGestureRecognizer#00000(state: ready, won arena, finalPosition: Offset(12.0, 8.0), button: 1, sent tap down) calling onTap callback.',
      ));
      log.clear();
  ...
  test('TapGestureRecognizer _sentTapDown toString', () {
    final TapGestureRecognizer tap = TapGestureRecognizer()..onTap = () {}; // Add a callback so that event can be added
    ...
    expect(
      tap.toString(),
      equalsIgnoringHashCodes('TapGestureRecognizer#00000(state: possible, button: 1, sent tap down)'),
    );
    GestureBinding.instance.gestureArena.close(1);
    tap.dispose();
  });
