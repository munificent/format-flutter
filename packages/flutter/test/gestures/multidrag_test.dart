...
void main() {
  ...
  testGesture('MultiDrag: moving before delay rejects', (GestureTester tester) {
    ...
    expect(didStartDrag, isFalse);
    tester.route(
      pointer.move(const Offset(30.0, 120.0)),
    ); // move some more after delay expires
    expect(didStartDrag, isFalse);
    drag.dispose();
  });
  ...
  test(
    'allowedButtonsFilter should work the same when null or not specified',
    () {
      ...
      const PointerDownEvent allowedPointer = PointerDownEvent(
