...
void main() {
  ...
  testGesture(
    'DragGestureRecognizer.onStart behavior test',
    (GestureTester tester) {
      final HorizontalDragGestureRecognizer drag =
          HorizontalDragGestureRecognizer()
            ..dragStartBehavior = DragStartBehavior.down;
      ...
      tester.route(pointer.move(
        const Offset(20.0, 25.0),
  ...
  DragEndDetails performDragToEnd(
    ...
    tester.route(down);
    tester.route(pointer.move(
      pointerVelocity * 0.025,
      timeStamp: const Duration(milliseconds: 25),
    ));
    tester.route(pointer.move(
      pointerVelocity * 0.050,
  ...
  testGesture(
    'Clamp max pan velocity in 2D, isotropically',
    (GestureTester tester) {
      ...
      void checkDrag(Offset pointerVelocity, Offset expectedVelocity) {
        final DragEndDetails actual = performDragToEnd(
          tester,
          recognizer,
          pointerVelocity,
        );
  ...
  testGesture(
    'Clamp max vertical-drag velocity vertically',
    (GestureTester tester) {
  ...
  testGesture(
    'Clamp max horizontal-drag velocity horizontally',
    (GestureTester tester) {
  ...
  testGesture(
    'Synthesized pointer events are ignored for velocity tracking',
    (GestureTester tester) {
      final HorizontalDragGestureRecognizer drag =
          HorizontalDragGestureRecognizer()
            ..dragStartBehavior = DragStartBehavior.down;
  ...
  testGesture('Can filter drags based on device kind', (GestureTester tester) {
    ...
    final PointerDownEvent touchDown = touchPointer.down(
      const Offset(10.0, 10.0),
    );
    ...
    final PointerDownEvent mouseDown = mousePointer.down(
      const Offset(10.0, 10.0),
    );
  ...
  testGesture(
    'Should recognize pan gestures from platform',
    (GestureTester tester) {
      final PanGestureRecognizer pan = PanGestureRecognizer();
      ...
      final PointerPanZoomStartEvent start = pointer.panZoomStart(
        const Offset(10.0, 10.0),
      );
  ...
  testGesture(
    'Pointer pan/zooms drags should allow touches to join them',
    (GestureTester tester) {
      final PanGestureRecognizer pan = PanGestureRecognizer();
      // We need a competing gesture recognizer so that the gesture is not immediately claimed.
      final PanGestureRecognizer competingPan = PanGestureRecognizer();
      ...
      final PointerPanZoomStartEvent start = panZoomPointer.panZoomStart(
        const Offset(10.0, 10.0),
      );
      pan.addPointerPanZoom(start);
      competingPan.addPointerPanZoom(start);
  ...
  testGesture(
    'Touch drags should allow pointer pan/zooms to join them',
    (GestureTester tester) {
      final PanGestureRecognizer pan = PanGestureRecognizer();
      // We need a competing gesture recognizer so that the gesture is not immediately claimed.
      final PanGestureRecognizer competingPan = PanGestureRecognizer();
