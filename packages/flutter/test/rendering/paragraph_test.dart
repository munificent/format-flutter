...
void main() {
  ...
  test('getOffsetForCaret control test', () {
    ...
    final Offset offset5 = paragraph.getOffsetForCaret(
      const TextPosition(offset: 5),
      caret,
    );
    expect(offset5.dx, greaterThan(0.0));

    final Offset offset25 = paragraph.getOffsetForCaret(
      const TextPosition(offset: 25),
      caret,
    );
    expect(offset25.dx, greaterThan(offset5.dx));

    final Offset offset50 = paragraph.getOffsetForCaret(
  ...
  test('getPositionForOffset control test', () {
    ...
    final TextPosition position20 = paragraph.getPositionForOffset(
      const Offset(20.0, 5.0),
    );
    expect(position20.offset, greaterThan(0.0));

    final TextPosition position40 = paragraph.getPositionForOffset(
      const Offset(40.0, 5.0),
    );
    expect(position40.offset, greaterThan(position20.offset));

    final TextPosition positionBelow = paragraph.getPositionForOffset(
  ...
  test('getBoxesForSelection control test', () {
    ...
    expect(
      boxes.any((ui.TextBox box) => box.left == 250 && box.top == 0),
      isTrue,
    );
    expect(
      boxes.any((ui.TextBox box) => box.right == 100 && box.top == 10),
      isTrue,
    );
  ...
  test('getWordBoundary control test', () {
    ...
    final TextRange range5 = paragraph.getWordBoundary(
      const TextPosition(offset: 5),
    );
    expect(range5.textInside(_kText), equals('polished'));

    final TextRange range50 = paragraph.getWordBoundary(
      const TextPosition(offset: 50),
    );
    expect(range50.textInside(_kText), equals(' '));

    final TextRange range85 = paragraph.getWordBoundary(
  ...
  test(
    'Does not include the semantics node of truncated rendering children',
    () {
  ...
  test('Supports gesture recognizer semantics', () {
    ...
    expect(
      children[2].getSemanticsData().hasAction(SemanticsAction.longPress),
      true,
    );
  ...
  test(
    ...
    () {
      ...
      paragraph = RenderParagraphWithEmptySelectionBoxList(
        TextSpan(
          text: '',
          children: <InlineSpan>[
            TextSpan(
  ...
  group('Selection', () {
    void selectionParagraph(
      ...
      for (final Selectable selectable
        ...
        selectable.dispatchSelectionEvent(SelectionEdgeUpdateEvent.forEnd(
          globalPosition:
              paragraph.getOffsetForCaret(end, Rect.zero) + const Offset(0, 5),
        ));
      }
    }
    ...
    test('paints selection highlight', () async {
      ...
      paragraph.paint(paintingContext, Offset.zero);
      expect(
        paintingContext.canvas.drawnRect,
