...
void main() {
  ...
  test(
    ...
    () {
      ...
      final _TestAlwaysNeedsAddToSceneLayer d =
  ...
  test('LeaderLayer.applyTransform can be called after retained rendering', () {
    ...
    final RenderRepaintBoundary root = RenderRepaintBoundary(
      child:
          RenderRepaintBoundary(child: leader = RenderLeaderLayer(link: link)),
    );
  ...
  test('Layer prints engineLayer if it is not null in debug info', () {
    ...
    expect(
      info,
      contains('engine layer: ${describeIdentity(layer.engineLayer)}'),
    );
  });
  ...
  test(
    'OpacityLayer does not push an OffsetLayer if there are no children',
    () {
  ...
  test('Observe layer tree composition - asserts on mutation', () {
    ...
    b1.addCompositionCallback((Layer layer) {
      ...
      expect(() => layer.remove(), throwsAssertionError);
      expect(
        () => (layer as ContainerLayer).append(ContainerLayer()),
  ...
  test('Layer types that support rasterization', () {
    ...
    final TextureLayer textureLayer = TextureLayer(
      rect: Rect.zero,
      textureId: 1,
    );
    ...
    // Unsupported.
    final PlatformViewLayer platformViewLayer = PlatformViewLayer(
      rect: Rect.zero,
      viewId: 1,
    );
