...
void main() {
  ...
  test('Set size error messages', () {
        ...
        MissingPerformLayoutRenderBox();
    {
      ...
      expect(result.diagnostics.where(
        (DiagnosticsNode node) => node.level == DiagnosticLevel.hint,
      ), isEmpty);
    }
    {
  ...
  group('ConstraintsTransformBox', () {
    ...
    test('handles flow layout', () {
      ...
      final RenderConstraintsTransformBox box = RenderConstraintsTransformBox(
        alignment: Alignment.center,
        textDirection: TextDirection.ltr,
        constraintsTransform: (BoxConstraints constraints) => constraints
  ...
  group('hit testing', () {
    ...
    test('localToGlobal with ancestor', () {
      ...
      final RenderPositionedBox innerCenter = RenderPositionedBox(
        child: innerConstrained,
      );
      ...
      final RenderPositionedBox outerCentered = RenderPositionedBox(
        child: outerConstrained,
      );

      layout(outerCentered);

      expect(
        innerConstrained.localToGlobal(
          Offset.zero,
          ancestor: outerConstrained,
        ).dy,
        25.0,
      );
    });
