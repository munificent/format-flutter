...
class _PathMatcher extends Matcher {
  ...
  Description describe(Description description) {
    ...
    return description.add(
      'A Path that contains ${points(includes)} but does not contain ${points(
        excludes,
      )}.',
    );
  }
...
abstract class _TestRecordingCanvasMatcher extends Matcher {
  ...
  bool _evaluatePredicates(
    Iterable<RecordedInvocation> calls,
...
class _TestRecordingCanvasPaintsCountMatcher
  ...
  Description describe(Description description) {
    return description.add(
      'Object or closure painting $_methodName exactly $_count times',
    );
  }
...
class _TestRecordingCanvasPaintsNothingMatcher
  ...
  bool _evaluatePredicates(
    ...
    final Iterable<RecordedInvocation> paintingCalls = _filterCanvasCalls(
      calls,
    );
...
class _TestRecordingCanvasPatternMatcher extends _TestRecordingCanvasMatcher
  ...
  Description describe(Description description) {
    ...
    return description.addAll('', '\n', '', _predicates.map<String>(
      (_PaintPredicate predicate) => predicate.toString(),
    ));
  }
      ...
      if (!call.moveNext()) {
        throw _MismatchedCall(
          'It called $others other method${others == 1 ? "" : "s"} on the canvas, '
              ...
              'call ${_symbolName(
            symbol,
          )}() at the time where $this was expected.',
          'The first method that was called when the call to ${_symbolName(
            symbol,
          )}() '
  ...
  void verifyArguments(List<dynamic> arguments) {
    ...
    if (radius != null && radiusArgument != radius) {
      throw 'It called $methodName with radius, ${radiusArgument.toStringAsFixed(
        1,
      )}, which was not exactly the expected radius (${radius!.toStringAsFixed(
        1,
      )}).';
    }
  }
  ...
  void match(Iterator<RecordedInvocation> call) {
    ...
    for (int index = 0; index < arguments.length; index += 1) {
      ...
      if (desiredArgument is Matcher) {
        ...
        throw 'It called ${_symbolName(
          symbol,
        )} with argument $index having value ${_valueName(
          actualArgument,
