...
void main() {
  ...
  test('needsCompositingBitsUpdate paint error', () {
    ...
    final RenderBox root = RenderRepaintBoundary(
      child: RenderSizedBox(const Size(100, 100)),
    );
    try {
      layout(root);
    ...
    // The lines in the middle of the error message contain the stack trace
    // which will change depending on where the test is run.
    expect(flutterError.toStringDeep(), equalsIgnoringHashCodes(
      'FlutterError\n'
    ...
    expect(
      flutterError.diagnostics.singleWhere(
