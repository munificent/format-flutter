...
void main() {
  ...
  test(
    ...
    () {
      ...
      RendererBinding.instance.platformDispatcher.onPointerDataPacket!(
        ui.PointerDataPacket(
          data: <ui.PointerData>[_pointerData(PointerChange.add, Offset.zero)],
        ),
      );
  ...
  test('should not crash if the first event is a Removed event', () {
    final List<PointerEvent> events = <PointerEvent>[];
    setUpWithOneAnnotation(logEvents: events);
    binding.platformDispatcher.onPointerDataPacket!(ui.PointerDataPacket(
      data: <ui.PointerData>[_pointerData(PointerChange.remove, Offset.zero)],
    ));
    events.clear();
  });
  ...
  test('should correctly handle multiple devices', () {
    ...
    RendererBinding
        .instance
        .platformDispatcher
        .onPointerDataPacket!(ui.PointerDataPacket(
      data: <ui.PointerData>[
  ...
  test(
    ...
    () {
      ...
      binding.flushPostFrameCallbacks(Duration.zero);
      expect(events, _equalToEventsOnCriticalFields(<BaseEventMatcher>[
        EventMatcher<PointerExitEvent>(const PointerExitEvent(
          position: Offset(0.0, 100.0),
        ).transformed(translate10by20)),
      ]));
  ...
  test(
    ...
    () {
      ...
      expect(events, _equalToEventsOnCriticalFields(<BaseEventMatcher>[
        ...
        EventMatcher<PointerHoverEvent>(const PointerHoverEvent(
          position: Offset(0.0, 100.0),
        ).transformed(translate10by20)),
      ]));
      events.clear();
  ...
  test(
    'should not schedule post-frame callbacks when no mouse is connected',
    () {
      setUpMouseAnnotationFinder((Offset position) sync* {});
...
class BaseEventMatcher extends Matcher {
  ...
  Description describe(Description description) {
    return description.add('event (critical fields only) ').addDescriptionOf(
      expected,
    );
  }
