...
void main() {
  ...
  test('pointer is added and removed out of any annotations', () {
    ...
    annotation = const TestAnnotationTarget(
      cursor: SystemMouseCursors.grabbing,
    );
    RendererBinding.instance.platformDispatcher.onPointerDataPacket!(
      ui.PointerDataPacket(
    ...
    expect(logCursors, <_CursorUpdateDetails>[
      _CursorUpdateDetails.activateSystemCursor(
        device: 0,
        kind: SystemMouseCursors.grabbing.kind,
      ),
    ]);
    logCursors.clear();
    ...
    RendererBinding.instance.platformDispatcher.onPointerDataPacket!(
      ui.PointerDataPacket(
        data: <ui.PointerData>[_pointerData(PointerChange.hover, Offset.zero)],
      ),
    );
  ...
  test('The first annotation with non-deferring cursor is used', () {
    ...
    expect(logCursors, <_CursorUpdateDetails>[
      _CursorUpdateDetails.activateSystemCursor(
        device: 0,
        kind: SystemMouseCursors.click.kind,
      ),
    ]);
    logCursors.clear();
  ...
  test('Pointing devices display cursors separately', () {
    ...
    expect(logCursors, <_CursorUpdateDetails>[
      _CursorUpdateDetails.activateSystemCursor(
        device: 2,
        kind: SystemMouseCursors.forbidden.kind,
      ),
    ]);
    logCursors.clear();
  });
...
class _CursorUpdateDetails extends MethodCall {
  ...
  }) : this('activateSystemCursor', <String, dynamic>{
         'device': device,
         'kind': kind,
       });
  @override
