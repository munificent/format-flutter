...
void main() {
        ...
        child: SizedBox(
          ...
          child: CustomScrollView(
            ...
            slivers: List<Widget>.generate(20, (int i) {
              final Widget sliver = SliverToBoxAdapter(child: SizedBox(height: 100.0, child: Text('Tile $i')));
              children.add(sliver);
              return SliverPadding(padding: const EdgeInsets.only(top: 22.0, bottom: 23.0), sliver: sliver);
            }),
          ),
        ),
      ),
      ...
      child: Center(
        child: SizedBox(
          ...
          child: CustomScrollView(
            ...
            controller: ScrollController(initialScrollOffset: 300.0),
            slivers: List<Widget>.generate(20, (int i) {
              final Widget sliver = SliverToBoxAdapter(child: SizedBox(width: 100.0, child: Text('Tile $i')));
              children.add(sliver);
              return SliverPadding(padding: const EdgeInsets.only(left: 22.0, right: 23.0), sliver: sliver);
            }),
          ),
        ),
      ),
  ...
  testWidgets('Viewport getOffsetToReveal Sliver - up - reverse growth', (WidgetTester tester) async {
    ...
    const Widget lowerSliver = SliverPadding(padding: padding, sliver: SliverToBoxAdapter(child: lowerItem));
      ...
      const Directionality(
        ...
        child: Center(
          child: SizedBox(
            height: 200.0,
            width: 300.0,
            child: CustomScrollView(center: centerKey, reverse: true, slivers: <Widget>[lowerSliver, centerSliver]),
          ),
        ),
      ),
    );
        ...
        child: SizedBox(
          ...
          child: ListView(
            ...
            children: List<Widget>.generate(10, (int y) {
              return SizedBox(
                height: 100.0,
                child: ListView(scrollDirection: Axis.horizontal, controller: controllersX[y], children: children[y]),
              );
            }),
          ),
        ),
          ...
          child: SizedBox(
            ...
            child: ListView(
              ...
              children: <Widget>[
                ...
                SizedBox(height: 200.0, width: 300.0, child: ListView(controller: inner, children: children)),
                const SizedBox(height: 200.0),
              ],
            ),
          ),
        ...
        child: CustomScrollView(
          ...
          slivers: <Widget>[
            ...
            SliverList(
              key: centerKey,
              delegate: SliverChildListDelegate(
                List<Widget>.generate(1, (int index) => const SizedBox(height: itemHeight, child: Text('Item 0'))),
              ),
            ),
            SliverList(
              delegate: SliverChildListDelegate(
              ...
              children: <Widget>[
                ...
                SizedBox(
                  ...
                  child: ListView(
                    ...
                    children: slivers = <Widget>[Container(width: 150.0), Container(width: 150.0)],
                  ),
                ),
                const SizedBox(height: 150.0),
              ],
              ...
              slivers: children = List<Widget>.generate(20, (int i) {
                return i == 10
                    ? SliverPersistentHeader(
                        pinned: true,
                        delegate: _TestSliverPersistentHeaderDelegate(minExtent: 100, maxExtent: 300, key: headerKey),
                      )
  ...
  void testFloatingHeaderShowOnScreen({bool animated = true, Axis axis = Axis.vertical}) {
    ...
    group('animated: $animated, scrollDirection: $axis', () {
      ...
      testWidgets('RenderViewportBase.showOnScreen but no child', (WidgetTester tester) async {
        ...
        // The persistent header is pinned to the leading edge thus still visible,
        // the viewport should not scroll.
        tester.renderObject(pinnedHeaderContent).showOnScreen(rect: Offset.zero & const Size(300, 300));
        await tester.pumpAndSettle();
        // The header expands but doesn't move.
        expect(controller.offset, 300.0 * 15);
        expect(mainAxisExtent(tester, pinnedHeaderContent), 300);
        ...
        //
        // See: https://github.com/flutter/flutter/issues/25507.
        tester.renderObject(pinnedHeaderContent).showOnScreen(rect: const Offset(-1, -1) & const Size(300, 300));
        await tester.pumpAndSettle();
        expect(controller.offset, 300.0 * 15);
        expect(mainAxisExtent(tester, pinnedHeaderContent), 300);
      });

      testWidgets('RenderViewportBase.showOnScreen with maxShowOnScreenExtent ', (WidgetTester tester) async {
        await tester.pumpWidget(buildList(
          floatingHeader: SliverPersistentHeader(
            pinned: true,
            floating: true,
      ...
      testWidgets('RenderViewportBase.showOnScreen with minShowOnScreenExtent ', (WidgetTester tester) async {
        await tester.pumpWidget(buildList(
          floatingHeader: SliverPersistentHeader(
            pinned: true,
            floating: true,
      ...
      testWidgets('RenderViewportBase.showOnScreen should not scroll if the rect is already visible, '
          'even if it does not scroll linearly (reversed order version)', (WidgetTester tester) async {
        await tester.pumpWidget(buildList(
          floatingHeader: SliverPersistentHeader(
            pinned: true,
            floating: true,
              ...
              slivers: List<Widget>.generate(6, (int i) {
                return SliverPadding(
                  ...
                  sliver: SliverToBoxAdapter(
                    child: Container(padding: padding, height: 300.0, width: 300.0, child: Text('Tile $i')),
                  ),
                );
              }),
            ),
                ...
                child: SizedBox(
                  ...
                  child: CustomScrollView(
                    ...
                    slivers: List<Widget>.generate(5, (int i) {
                      return SliverToBoxAdapter(child: SizedBox(height: 300.0, child: Text('Tile $i')));
                    }),
                  ),
                ),
              ),
  ...
  group('Viewport childrenInPaintOrder control test', () {
    test('RenderViewport', () async {
      ...
      expect(renderViewport.childrenInPaintOrder, equals(renderViewport.childrenInHitTestOrder.toList().reversed));
    });
  ...
  group('Overscrolling RenderShrinkWrappingViewport', () {
      ...
      return Directionality(
        ...
        child: MediaQuery(
          ...
          child: ListView.builder(
            ...
            shrinkWrap: true,
            itemBuilder:
                (BuildContext context, int index) => SizedBox(height: 50, width: 50, child: Text('Item $index')),
            itemCount: 20,
            itemExtent: 50,
          ),
        ),
          ...
          child: ColoredBox(
            ...
            child: Column(
              children: <Widget>[
                ...
                // clipped properly.
                Opacity(opacity: 0.5, child: Container(height: 100, color: const Color(0xFF00B0FF))),
                Container(
    ...
    testWidgets('correctly clips overflow without constraints', (WidgetTester tester) async {
      ...
      expect(controller.offset, -100.0);
      expect(tester.getTopLeft(find.text('Item 0')).dy, 200.0);
      await expectLater(find.byType(Directionality), matchesGoldenFile('shrinkwrap_clipped_overscroll.png'));
      await overscrollGesture.up();
      await tester.pumpAndSettle();
      expect(controller.offset, 0.0);
      expect(tester.getTopLeft(find.text('Item 0')).dy, 100.0);
    ...
    testWidgets('allows overscrolling on default platforms - horizontal', (WidgetTester tester) async {
      ...
      // Scrollables should overscroll by default on iOS and macOS
      final ScrollController controller = ScrollController();
      await tester.pumpWidget(buildSimpleShrinkWrap(controller: controller, scrollDirection: Axis.horizontal));
      expect(controller.offset, 0.0);
    ...
    testWidgets('allows overscrolling per physics - vertical', (WidgetTester tester) async {
      ...
      final ScrollController controller = ScrollController();
      await tester.pumpWidget(buildSimpleShrinkWrap(controller: controller, physics: const BouncingScrollPhysics()));
      expect(controller.offset, 0.0);
      expect(tester.getTopLeft(find.text('Item 0')).dy, 0.0);
      // Check overscroll at both ends
      // Start
    ...
    testWidgets('allows overscrolling per physics - horizontal', (WidgetTester tester) async {
      ...
      // Scrollables should overscroll when the scroll physics allow
      final ScrollController controller = ScrollController();
      await tester.pumpWidget(buildSimpleShrinkWrap(
        controller: controller,
        scrollDirection: Axis.horizontal,
  ...
  testWidgets('Viewport describeApproximateClip respects clipBehavior', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      const Directionality(
        textDirection: TextDirection.ltr,
        child: CustomScrollView(slivers: <Widget>[SliverToBoxAdapter(child: SizedBox(width: 20, height: 20))]),
      ),
    );
  ...
  testWidgets('Shrinkwrapping viewport asserts bounded cross axis', (WidgetTester tester) async {
    ...
    expect(error.toString(), contains('Viewports expand in the cross axis to fill their container'));
    errors.clear();
    ...
    await tester.pumpWidget(Directionality(
      textDirection: TextDirection.ltr,
      child: ListView(scrollDirection: Axis.horizontal, children: <Widget>[ListView(shrinkWrap: true)]),
    ));
