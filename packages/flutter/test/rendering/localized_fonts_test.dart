...
void main() {
  testWidgets('RichText TextSpan styles with different locales', (WidgetTester tester) async {
    ...
    await expectLater(find.byType(RichText), matchesGoldenFile('localized_fonts.rich_text.styled_text_span.png'));
  });

  testWidgets('Text with locale-specific glyphs, ambient locale', (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
      theme: ThemeData(useMaterial3: false),
      supportedLocales: const <Locale>[Locale('en', 'US'), Locale('ja'), Locale('zh')],
      home: Builder(
    ...
    await expectLater(find.byType(Row), matchesGoldenFile('localized_fonts.text_ambient_locale.chars.png'));
  });

  testWidgets('Text with locale-specific glyphs, explicit locale', (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
      theme: ThemeData(useMaterial3: false),
      supportedLocales: const <Locale>[Locale('en', 'US'), Locale('ja'), Locale('zh')],
      home: Builder(
    ...
    await expectLater(find.byType(Row), matchesGoldenFile('localized_fonts.text_explicit_locale.chars.png'));
  });
}
