...
void main() {
  ...
  test('Reports the height of the first line when maxLines is 1', () {
    final InlineSpan multilineSpan = TextSpan(text: 'liiiiines\n' * 10, style: const TextStyle(fontSize: 10));
    final BoxConstraints constraints = BoxConstraints.loose(const Size(600, 600));
    final RenderEditable editable = RenderEditable(
      textDirection: TextDirection.ltr,
      startHandleLayerLink: LayerLink(),
  ...
  test('Can change cursor color, radius, visibility', () {
    ...
    expect(
      editable,
      paints..rect(color: const Color.fromARGB(0xFF, 0xFF, 0x00, 0x00), rect: const Rect.fromLTWH(40, 0, 1, 10)),
    );
  ...
  test('Cursor with ideographic script', () {
    ...
    final RenderEditable editable = RenderEditable(
      ...
      offset: ViewportOffset.zero(),
      textSelectionDelegate: delegate,
      text: const TextSpan(text: '中文测试文本是否正确', style: TextStyle(fontSize: 10.0, fontFamily: 'FlutterTest')),
      startHandleLayerLink: LayerLink(),
      endHandleLayerLink: LayerLink(),
      selection: const TextSelection.collapsed(offset: 4, affinity: TextAffinity.upstream),
    );
  ...
  test('text is painted above selection', () {
    ...
    final RenderEditable editable = RenderEditable(
      ...
      startHandleLayerLink: LayerLink(),
      endHandleLayerLink: LayerLink(),
      selection: const TextSelection(baseOffset: 0, extentOffset: 3, affinity: TextAffinity.upstream),
    );
  ...
  group('WidgetSpan support', () {
    test('able to render basic WidgetSpan', () async {
      final TextSelectionDelegate delegate = _FakeEditableTextState()
        ..textEditingValue = const TextEditingValue(text: 'test', selection: TextSelection.collapsed(offset: 3));
      ...
      final RenderEditable editable = RenderEditable(
        ...
        text: const TextSpan(
          ...
          children: <InlineSpan>[
            ...
            TextSpan(children: <InlineSpan>[WidgetSpan(child: Text('b')), WidgetSpan(child: Text('c'))]),
          ],
        ),
        selection: const TextSelection.collapsed(offset: 3),
        maxLines: 2,
  ...
  test('Floating cursor position is independent of viewport offset', () {
    ...
    final RRect expectedRRect = RRect.fromRectAndRadius(const Rect.fromLTWH(49.5, 51, 2, 8), const Radius.circular(1));
