...
void main() {
  ...
  testWidgets(
    'Verify Image does not use disposed handles',
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final _TestImageProvider imageProvider = _TestImageProvider();
      await tester.pumpWidget(
        Image(image: imageProvider, excludeFromSemantics: true),
      );
      final State<Image> image = tester.state /*State<Image>*/ (
        find.byType(Image),
      );
  ...
  testWidgets(
    'Listeners are only removed if callback tuple matches',
    (WidgetTester tester) async {
  ...
  testWidgets(
    'Removing listener removes one listener and error listener',
    (WidgetTester tester) async {
      int errorListenerCalled = 0;
      ImageInfo? capturedImage;
      void errorListener(dynamic exception, StackTrace? stackTrace) {
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final RenderImage renderer = tester.renderObject<RenderImage>(
        find.byType(Image),
      );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        lastFrame,
        1,
      ); // missed a frame because we weren't animating at the time
      expect(buildCount, 4);
    },
  );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        Container(key: key, child: Image(image: imageProvider1)),
      );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final ImageCacheStatus providerLocation = (await provider
  ...
  testWidgets('Reports image size when painted', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(Center(
      child:
          SizedBox(height: 50, width: 50, child: Image(image: imageProvider)),
    ));
  ...
  testWidgets(
    'Disposes image handle when disposed',
    (WidgetTester tester) async {
      ...
      final ImageProvider provider = _TestImageProvider(
        streamCompleter: OneFrameImageStreamCompleter(
          Future<ImageInfo>.value(
            ImageInfo(image: image, debugLabel: '_TestImage'),
          ),
        ),
      );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        TickerMode(enabled: true, child: Image(image: provider)),
      );
      expect(find.byType(Image), findsOneWidget);

      await tester.pumpWidget(
        TickerMode(enabled: false, child: Image(image: provider)),
      );
      expect(find.byType(Image), findsOneWidget);
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(find.byType(Padding), findsOneWidget);
      expect(
        tester.widget<Padding>(find.byType(Padding)).child,
        isA<RawImage>(),
      );
      expect(find.byKey(errorKey), findsNothing);
    },
    ...
    expect(
      ...
      kIsWeb
          // Web does not support file access, expect AssertionError
            ? throwsA(
              predicate((AssertionError e) => e.message == expectedError),
            )
...
class _DebouncingImageProvider extends ImageProvider<Object> {
  ...
  @override
  Future<Object> obtainKey(ImageConfiguration configuration) => imageProvider
...
class _FailingImageProvider extends ImageProvider<int> {
  ...
  ImageStreamCompleter load(int key, DecoderCallback decode) {
    ...
    return OneFrameImageStreamCompleter(
      Future<ImageInfo>.value(ImageInfo(image: image, scale: 0)),
    );
  }
}
