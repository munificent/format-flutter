...
class TestRoute extends Route<String?> with LocalHistoryRoute<String?> {
  ...
  void install() {
    log('install');
    final OverlayEntry entry = OverlayEntry(builder: (BuildContext context) => Container(), opaque: true);
    _entries.add(entry);
    routes.add(this);
    super.install();
  }
...
void main() {
  ...
  testWidgets('Route management - push, replace, pop sequence', (WidgetTester tester) async {
    ...
    await runNavigatorTest(
      ...
      host,
      () {},
      <String>['initial: install', 'initial: didAdd', 'initial: didChangeNext null'],
    );
  ...
  testWidgets('Route management - push, remove, pop', (WidgetTester tester) async {
    ...
    await runNavigatorTest(
      ...
      host,
      () {},
      <String>['first: install', 'first: didAdd', 'first: didChangeNext null'],
    );
    late TestRoute second;
    await runNavigatorTest(tester, host, () {
      host.push(second = TestRoute('second'));
    }, <String>['second: install', 'second: didPush', 'second: didChangeNext null', 'first: didChangeNext second']);
    await runNavigatorTest(tester, host, () {
      host.push(TestRoute('third'));
    }, <String>['third: install', 'third: didPush', 'third: didChangeNext null', 'second: didChangeNext third']);
    await runNavigatorTest(tester, host, () {
      host.removeRouteBelow(second);
    }, <String>['first: dispose']);
    await runNavigatorTest(tester, host, () {
      host.pop('good bye');
    }, <String>['third: didPop good bye', 'second: didPopNext third'], <String>['third: dispose']);
    await runNavigatorTest(tester, host, () {
      host.push(TestRoute('three'));
    }, <String>['three: install', 'three: didPush', 'three: didChangeNext null', 'second: didChangeNext three']);
    late TestRoute four;
    await runNavigatorTest(tester, host, () {
      host.push(four = TestRoute('four'));
    }, <String>['four: install', 'four: didPush', 'four: didChangeNext null', 'three: didChangeNext four']);
    await runNavigatorTest(tester, host, () {
      host.removeRouteBelow(four);
    }, <String>['second: didChangeNext four', 'three: dispose']);
    await runNavigatorTest(tester, host, () {
      host.pop('the end');
    }, <String>['four: didPop the end', 'second: didPopNext four'], <String>['four: dispose']);
    await tester.pumpWidget(Container());
  ...
  testWidgets('Route management - push, replace, popUntil', (WidgetTester tester) async {
    ...
    final NavigatorState host = navigatorKey.currentState!;
    await runNavigatorTest(tester, host, () {}, <String>['A: install', 'A: didAdd', 'A: didChangeNext null']);
    await runNavigatorTest(tester, host, () {
    ...
    }, <String>['b: install', 'b: didReplace B', 'b: didChangeNext C', 'A: didChangeNext b', 'B: dispose']);
    await runNavigatorTest(tester, host, () {
      host.popUntil((Route<dynamic> route) => route == routeB);
    }, <String>['C: didPop null', 'b: didPopNext C'], <String>['C: dispose']);
    await tester.pumpWidget(Container());
        ...
        onGenerateRoute: (RouteSettings settings) {
          return PageRouteBuilder<void>(
            ...
            pageBuilder: (BuildContext context, Animation<double> input, Animation<double> out) {
              return Focus(child: TextField(autofocus: true, focusNode: focusNode, controller: controller));
            },
          );
        },
      ),
  ...
  group('TransitionRoute', () {
    ...
    testWidgets('secondary animation is kDismissed after train hopping finishes and pop', (WidgetTester tester) async {
      ...
      navigator.currentState!.pushReplacement(TestPageRouteBuilder(
        pageBuilder: (_, Animation<double> animation, Animation<double> secondaryAnimation) {
        ...
        onGenerateRoute: (RouteSettings settings) {
          return MaterialPageRoute<dynamic>(
            builder: (BuildContext context) {
              return ElevatedButton(
                onPressed: () {
                  Navigator.of(context).push<void>(ModifiedReverseTransitionDurationRoute<void>(
                    builder: (BuildContext innerContext) {
                      return Container(key: containerKey, color: Colors.green);
                    },
  ...
  group('ModalRoute', () {
    testWidgets('default barrierCurve', (WidgetTester tester) async {
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    Navigator.of(context).push<void>(
                      _TestDialogRouteWithCustomBarrierCurve<void>(child: const Text('Hello World')),
                    );
                  },
                ),
              );
      ...
      modalBarrierAnimation = tester.widget<AnimatedModalBarrier>(animatedModalBarrier).color;
      expect(modalBarrierAnimation.value!.alpha, closeTo(getExpectedBarrierTweenAlphaValue(0.25), 1));

      await tester.pump(const Duration(milliseconds: 25));
      modalBarrierAnimation = tester.widget<AnimatedModalBarrier>(animatedModalBarrier).color;
      expect(modalBarrierAnimation.value!.alpha, closeTo(getExpectedBarrierTweenAlphaValue(0.50), 1));

      await tester.pump(const Duration(milliseconds: 25));
      modalBarrierAnimation = tester.widget<AnimatedModalBarrier>(animatedModalBarrier).color;
      expect(modalBarrierAnimation.value!.alpha, closeTo(getExpectedBarrierTweenAlphaValue(0.75), 1));
            ...
            builder: (BuildContext context) {
              return Center(
                child: ElevatedButton(
                  child: const Text('X'),
                  onPressed: () {
                    Navigator.of(context).push<void>(_TestDialogRouteWithCustomBarrierCurve<void>(
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        // Pushes one page.
        navigatorKey.currentState!.push<void>(
          MaterialPageRoute<void>(builder: (BuildContext context) => const Material(child: TextField())),
        );
        await tester.pumpAndSettle();
  ...
  testWidgets('can be dismissed with escape keyboard shortcut', (WidgetTester tester) async {
    ...
    showDialog<void>(context: textOnPageOne, builder: (BuildContext context) => const Text('dialog1'));
    await tester.pumpAndSettle();
