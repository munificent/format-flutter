...
void main() {
      ...
      Widget buildItem(BuildContext context, int index) {
        return !skip || index.isEven
            ? Card(
                child: ListTile(
                  title:
                      Text('item$index', style: const TextStyle(fontSize: 80)),
                ),
              )
            : Container();
      }
      ...
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(
          body: CustomScrollView(
            slivers: <Widget>[
              SliverList(
                delegate: SliverChildBuilderDelegate(buildItem, childCount: 30),
              ),
            ],
          ),
        ),
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        await tester.pumpAndSettle(),
        1,
      ); // there should be no animation here
      expect(controller.offset, 800.0);
    },
  );
  ...
  group('SliverOffstage - ', () {
    testWidgets('offstage true', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(boilerPlate(
        const SliverOffstage(sliver: SliverToBoxAdapter(child: Text('a'))),
      ));

      expect(semantics.nodesWith(label: 'a'), hasLength(0));
      expect(find.byType(Text), findsNothing);
      final RenderViewport renderViewport = tester.renderObject(
    ...
    testWidgets('offstage false', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(boilerPlate(
        const SliverOffstage(
          offstage: false,
          sliver: SliverToBoxAdapter(child: Text('a')),
        ),
      ));
  ...
  group('SliverIgnorePointer - ', () {
    ...
    testWidgets(
      'ignoring only block semantics actions',
      (WidgetTester tester) async {
        final SemanticsTester semantics = SemanticsTester(tester);
        await tester.pumpWidget(boilerPlate(SliverIgnorePointer(
          sliver: SliverToBoxAdapter(
  ...
  testWidgets(
    'SliverList.builder can build children',
    (WidgetTester tester) async {
      int firstTapped = 0;
      int secondTapped = 0;
      final Key key = UniqueKey();
        ...
        body: CustomScrollView(
          slivers: <Widget>[
            SliverList.separated(
              ...
              separatorBuilder:
                  (BuildContext context, int index) => Text('Separator $index'),
            ),
          ],
        ),
      ),
        ...
        body: CustomScrollView(
          slivers: <Widget>[
            SliverList.separated(
              itemCount: 2,
              itemBuilder:
                  (BuildContext context, int index) => const Text('item'),
              separatorBuilder:
                  (BuildContext context, int index) => const Text('separator'),
            ),
          ],
        ),
      ),
  ...
  testWidgets(
    'SliverList.list can build children',
    (WidgetTester tester) async {
      int firstTapped = 0;
      int secondTapped = 0;
      final Key key = UniqueKey();
