...
void main() {
  ...
  testWidgets('AnimatedSwitcher can handle back-to-back changes.', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      AnimatedSwitcher(duration: const Duration(milliseconds: 100), child: Container(key: container1)),
    );
    ...
    await tester.pumpWidget(
      AnimatedSwitcher(duration: const Duration(milliseconds: 100), child: Container(key: container2)),
    );
    expect(find.byKey(container1), findsOneWidget);
    ...
    await tester.pumpWidget(
      AnimatedSwitcher(duration: const Duration(milliseconds: 100), child: Container(key: container3)),
    );
    expect(find.byKey(container1), findsOneWidget);
    expect(find.byKey(container2), findsNothing);
  ...
  testWidgets("AnimatedSwitcher doesn't transition in a new child of the same type.", (WidgetTester tester) async {
    await tester.pumpWidget(
      AnimatedSwitcher(duration: const Duration(milliseconds: 100), child: Container(color: const Color(0x00000000))),
    );
    ...
    await tester.pumpWidget(
      AnimatedSwitcher(duration: const Duration(milliseconds: 100), child: Container(color: const Color(0xff000000))),
    );
  ...
  testWidgets('AnimatedSwitcher uses custom layout.', (WidgetTester tester) async {
    Widget newLayoutBuilder(Widget? currentChild, List<Widget> previousChildren) {
      return Column(children: <Widget>[...previousChildren, if (currentChild != null) currentChild]);
    }
  ...
  testWidgets('AnimatedSwitcher uses custom transitions.', (WidgetTester tester) async {
    ...
    for (final Widget child in foundChildren) {
      expect(child, isA<KeyedSubtree>());
      expect(find.descendant(of: find.byWidget(child), matching: find.byType(SizeTransition)), findsOneWidget);
    }
  });
  ...
  testWidgets("AnimatedSwitcher doesn't reset state of the children in transitions.", (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      AnimatedSwitcher(duration: const Duration(milliseconds: 100), child: StatefulTest(key: statefulOne)),
    );
    ...
    await tester.pumpWidget(
      AnimatedSwitcher(duration: const Duration(milliseconds: 100), child: StatefulTest(key: statefulTwo)),
    );
    ...
    await tester.pumpWidget(
      AnimatedSwitcher(duration: const Duration(milliseconds: 100), child: StatefulTest(key: statefulThree)),
    );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      for (final Widget child in foundChildren) {
        expect(child, isA<KeyedSubtree>());
        expect(find.descendant(of: find.byWidget(child), matching: find.byType(FadeTransition)), findsOneWidget);
      }
      ...
      for (final Widget child in foundChildren) {
        expect(child, isA<KeyedSubtree>());
        expect(find.descendant(of: find.byWidget(child), matching: find.byType(ScaleTransition)), findsOneWidget);
      }
    },
  );
