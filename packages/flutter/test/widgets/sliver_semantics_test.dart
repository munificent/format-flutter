...
void _tests() {
  testWidgets(
    'excludeFromScrollable works correctly',
  ...
  testWidgets(
    'Offscreen sliver are hidden in semantics tree',
    (WidgetTester tester) async {
      final SemanticsTester semantics = SemanticsTester(tester);
  ...
  testWidgets('SemanticsNodes of Slivers are in paint order', (
    ...
    final List<Widget> slivers = List<Widget>.generate(5, (int i) {
      return SliverToBoxAdapter(
        child: SizedBox(height: 20.0, child: Text('Item $i')),
      );
    });
    await tester.pumpWidget(Semantics(
      textDirection: TextDirection.ltr,
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final GlobalKey forwardAppBarKey = GlobalKey(
        debugLabel: 'forward app bar',
      );
