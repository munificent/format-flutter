...
void main() {
  ...
  testWidgets('SliverConstrainedCrossAxis is laid out properly', (WidgetTester tester) async {
    ...
    final List<RenderSliverConstrainedCrossAxis> renderSliversConstrained =
        tester.renderObjectList<RenderSliverConstrainedCrossAxis>(find.byType(SliverConstrainedCrossAxis)).toList();
  ...
  testWidgets('Mix of slivers is laid out properly', (WidgetTester tester) async {
    ...
    final RenderSliverCrossAxisGroup sliverCrossAxisRenderObject = tester.renderObject<RenderSliverCrossAxisGroup>(
      find.byType(SliverCrossAxisGroup),
    );
      ...
      const Directionality(
        ...
        child: Center(
          child: CustomScrollView(
            slivers: <Widget>[SliverCrossAxisExpanded(flex: 2, sliver: SliverToBoxAdapter(child: Text('Hello World')))],
          ),
        ),
      ),
    );
      ...
      slivers: <Widget>[
        _buildSliverList(
          ...
          label: (int item) => tile == item && group == 0
              ? TextButton(onPressed: () => clickedTile = 'Group 0 Tile $item', child: Text('Group 0 Tile $item'))
              : Text('Group 0 Tile $item'),
        ),
        _buildSliverList(
          items: items,
          label: (int item) => tile == item && group == 1
              ? TextButton(onPressed: () => clickedTile = 'Group 1 Tile $item', child: Text('Group 1 Tile $item'))
  ...
  testWidgets('Assertion error when constrained widget runs out of cross axis extent', (WidgetTester tester) async {
    ...
    expect(errors, isNotEmpty);
    final AssertionError error = errors.first.exception as AssertionError;
    expect(error.toString(), contains('SliverCrossAxisGroup ran out of extent before child could be laid out.'));
  });
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(_buildSliverCrossAxisGroup(
        ...
        slivers: <Widget>[
          const SliverToBoxAdapter(child: SizedBox(height: 600)),
          SliverPersistentHeader(delegate: TestDelegate(minExtent: 40.0), pinned: true, floating: true),
        ],
        otherSlivers: <Widget>[const SliverToBoxAdapter(child: SizedBox(height: 2400))],
      ));
      await tester.pumpAndSettle();
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(_buildSliverCrossAxisGroup(
        ...
        slivers: <Widget>[
          const SliverToBoxAdapter(child: SizedBox(height: 600)),
          const SliverAppBar(toolbarHeight: 30, expandedHeight: 60, floating: true, pinned: true, snap: true),
        ],
        otherSlivers: <Widget>[const SliverToBoxAdapter(child: SizedBox(height: 2400))],
      ));
      await tester.pumpAndSettle();
    ...
    delegate: SliverChildBuilderDelegate(
      (BuildContext context, int i) {
        return scrollDirection == Axis.vertical
            ? SizedBox(key: ValueKey<int>(items[i]), height: itemMainAxisExtent, child: label(items[i]))
            : SizedBox(key: ValueKey<int>(items[i]), width: itemMainAxisExtent, child: label(items[i]));
      },
