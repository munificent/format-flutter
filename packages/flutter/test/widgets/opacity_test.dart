...
void main() {
  testWidgets('Opacity', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(const Opacity(opacity: 1.0, child: Text('a', textDirection: TextDirection.rtl)));
    ...
    // Opacity 0.0: Nothing
    await tester.pumpWidget(const Opacity(opacity: 0.0, child: Text('a', textDirection: TextDirection.rtl)));
    expect(semantics, hasSemantics(TestSemantics.root()));
    expect(find.byType(Opacity), paintsNothing);

    // Opacity 0.0 with semantics: Just semantics
    await tester.pumpWidget(
      const Opacity(opacity: 0.0, alwaysIncludeSemantics: true, child: Text('a', textDirection: TextDirection.rtl)),
    );
    expect(semantics, hasSemantics(TestSemantics.root(
      children: <TestSemantics>[
        TestSemantics.rootChild(
    ...
    await tester.pumpWidget(const Opacity(opacity: 0.1, child: Text('a', textDirection: TextDirection.rtl)));
    expect(semantics, hasSemantics(TestSemantics.root(
      children: <TestSemantics>[
        TestSemantics.rootChild(
          id: 1,
    ...
    await tester.pumpWidget(
      const Opacity(opacity: 0.1, alwaysIncludeSemantics: true, child: Text('a', textDirection: TextDirection.rtl)),
    );
    expect(semantics, hasSemantics(TestSemantics.root(
      children: <TestSemantics>[
        TestSemantics.rootChild(
  ...
  testWidgets('offset is correctly handled in Opacity', (WidgetTester tester) async {
          ...
          child: RepaintBoundary(
            child: Column(
              ...
              children: List<Widget>.generate(10, (int index) {
                return Opacity(
                  opacity: 0.5,
                  child: Padding(padding: const EdgeInsets.all(5.0), child: Container(color: Colors.blue, height: 50)),
                );
              }),
            ),
          ),
    ...
    await expectLater(find.byType(RepaintBoundary).first, matchesGoldenFile('opacity_test.offset.png'));
  });
        ...
        child: Stack(
          children: <Widget>[
            Positioned(
              top: 40,
              left: 140,
              child: Opacity(opacity: .5, child: Container(height: 100, width: 100, color: Colors.red)),
            ),
          ],
        ),
      ),
