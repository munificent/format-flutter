...
void main() {
  ...
  testWidgets('SliverList should not dispose widget with key during in screen reordering', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(SwitchingSliverListTest(viewportFraction: 0.1, children: childList));
    final _WidgetTest0State state0 = tester.state(find.byType(WidgetTest0));
    final _WidgetTest1State state1 = tester.state(find.byType(WidgetTest1));
    final _WidgetTest2State state2 = tester.state(find.byType(WidgetTest2));
    expect(find.text('child 0'), findsOneWidget);
          ...
          child: CustomScrollView(
            ...
            slivers: <Widget>[
              SliverFillViewport(
                ...
                }, childCount: children.length, findChildIndexCallback: (Key key) => _mapKeyToIndex[key] ?? -1),
              ),
            ],
          ),
        ),
      ...
      child: Center(
        child: SizedBox(
          ...
          child: CustomScrollView(
            cacheExtent: 0,
            slivers: <Widget>[
              SliverFillViewport(viewportFraction: viewportFraction, delegate: SliverChildListDelegate(children)),
            ],
          ),
        ),
      ),
...
class SwitchingSliverListTest extends StatelessWidget {
  const SwitchingSliverListTest({required this.children, this.viewportFraction = 1.0, super.key});
