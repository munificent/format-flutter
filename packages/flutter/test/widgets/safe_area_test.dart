...
void main() {
  group('SafeArea', () {
    ...
    testWidgets('SafeArea - with minimums', (WidgetTester tester) async {
      await tester.pumpWidget(
        const MediaQuery(
          data: MediaQueryData(padding: EdgeInsets.all(20.0)),
          child: SafeArea(top: false, minimum: EdgeInsets.fromLTRB(0.0, 10.0, 20.0, 30.0), child: Placeholder()),
        ),
      );
    ...
    testWidgets('SafeArea - changing', (WidgetTester tester) async {
      const Widget child = SafeArea(bottom: false, child: SafeArea(left: false, bottom: false, child: Placeholder()));
      await tester.pumpWidget(const MediaQuery(data: MediaQueryData(padding: EdgeInsets.all(20.0)), child: child));
      expect(tester.getTopLeft(find.byType(Placeholder)), const Offset(20.0, 20.0));
    ...
    testWidgets('SafeArea - properties', (WidgetTester tester) async {
      ...
      expect(
        properties.properties.any((DiagnosticsNode n) => n is FlagProperty && n.toString() == 'avoid top padding'),
        true,
      );
      expect(
  ...
  group('SliverSafeArea', () {
    ...
    testWidgets('SliverSafeArea - changing', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(buildWidget(const EdgeInsets.only(left: 100.0, top: 30.0, bottom: 40.0), sliver));
      verify(tester, <Rect>[
        const Rect.fromLTWH(0.0, 0.0, 800.0, 100.0),
