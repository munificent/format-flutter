...
void main() {
  group('SafeArea', () {
    ...
    testWidgets('SafeArea - nested', (WidgetTester tester) async {
      await tester.pumpWidget(
        const MediaQuery(
          ...
          child: SafeArea(
            top: false,
            child: SafeArea(right: false, child: Placeholder()),
          ),
        ),
      );
      expect(
    ...
    testWidgets('SafeArea - changing', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        const MediaQuery(
          data: MediaQueryData(
            padding: EdgeInsets.only(left: 100.0, top: 30.0, bottom: 40.0),
          ),
          child: child,
        ),
      );
    ...
    testWidgets('SafeArea - properties', (WidgetTester tester) async {
      ...
      expect(properties.properties.any(
        (DiagnosticsNode n) =>
    ...
    group(
      ...
      () {
        ...
        testWidgets('SafeArea alone.', (WidgetTester tester) async {
          ...
          final Offset initialPoint = tester.getCenter(
        ...
        testWidgets('SafeArea with nested Scaffold', (
          WidgetTester tester,
        ) async {
          final Widget child = boilerplate(
            const SafeArea(
              ...
              child: Scaffold(
                ...
                body:
                    Column(children: <Widget>[Expanded(child: Placeholder())]),
              ),
            ),
          );
        ...
        testWidgets(
          ...
          (WidgetTester tester) async {
            ...
            final Offset finalPoint = tester.getCenter(
              find.byType(Placeholder),
            );
            expect(initialPoint, finalPoint);
          },
  ...
  group('SliverSafeArea', () {
        ...
        child: Directionality(
          ...
          child: Viewport(
            ...
            slivers: <Widget>[
              ...
              const SliverToBoxAdapter(
                child:
                    SizedBox(width: 800.0, height: 100.0, child: Text('after')),
              ),
            ],
          ),
        ),
    ...
    void verify(WidgetTester tester, List<Rect> expectedRects) {
      final List<Rect> testAnswers = tester.renderObjectList<RenderBox>(
        find.byType(SizedBox),
      ).map<Rect>((RenderBox target) {
        final Offset topLeft = target.localToGlobal(Offset.zero);
        final Offset bottomRight = target.localToGlobal(
    ...
    testWidgets('SliverSafeArea - changing', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(buildWidget(
        const EdgeInsets.only(left: 100.0, top: 30.0, bottom: 40.0),
        sliver,
      ));
      verify(tester, <Rect>[
        const Rect.fromLTWH(0.0, 0.0, 800.0, 100.0),
