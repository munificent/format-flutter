...
void main() {
  group('UnmanagedRestorationScope', () {
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        await tester.pumpWidget(
          UnmanagedRestorationScope(bucket: bucket1, child: const BucketSpy()),
        );
        ...
        await tester.pumpWidget(
          UnmanagedRestorationScope(bucket: bucket2, child: const BucketSpy()),
        );
    ...
    testWidgets(
      'null bucket disables restoration',
      (WidgetTester tester) async {
        await tester.pumpWidget(
          const UnmanagedRestorationScope(child: BucketSpy()),
        );
        final BucketSpyState state = tester.state(find.byType(BucketSpy));
  ...
  group('RestorationScope', () {
    ...
    testWidgets('bucket for descendants contains data claimed from parent', (
      ...
      await tester.pumpWidget(UnmanagedRestorationScope(
        bucket: root,
        child:
            const RestorationScope(restorationId: 'child1', child: BucketSpy()),
      ));
      manager.doSerialization();
    ...
    testWidgets(
      'no bucket for descendants when scope is null',
      (WidgetTester tester) async {
        final Key scopeKey = GlobalKey();

        await tester.pumpWidget(RestorationScope(
          key: scopeKey,
          restorationId: 'foo',
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        await tester.pumpWidget(
          const RestorationScope(restorationId: null, child: BucketSpy()),
        );
        final BucketSpyState state = tester.state(find.byType(BucketSpy));
        expect(state.bucket, isNull);
      },
