...
Widget primaryScrollControllerBoilerplate({required Widget child, required ScrollController controller}) {
  return Directionality(
    textDirection: TextDirection.ltr,
    child:
        MediaQuery(data: const MediaQueryData(), child: PrimaryScrollController(controller: controller, child: child)),
  );
}
...
void main() {
      ...
      child: ListView(
        ...
        children: focusNodes.map((FocusNode focusNode) {
          return Container(
            height: 50,
            color: Colors.green,
            child: TextField(focusNode: focusNode, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
          );
        }).toList(),
      ),
    ));
  ...
  testWidgets('Can jumpTo during drag', (WidgetTester tester) async {
    ...
    expect(log, equals(<Type>[ScrollStartNotification, UserScrollNotification, ScrollUpdateNotification]));
    log.clear();
  ...
  test('PrimaryScrollController.automaticallyInheritOnPlatforms defaults to all mobile platforms', () {
    ...
    expect(primaryScrollController.automaticallyInheritForPlatforms, TargetPlatformVariant.mobile().values);
  });
  ...
  testWidgets('Vertical ListViews use PrimaryScrollController by default on mobile', (WidgetTester tester) async {
    final ScrollController controller = ScrollController();
    await tester.pumpWidget(primaryScrollControllerBoilerplate(child: ListView(), controller: controller));
    expect(controller.hasClients, isTrue);
  }, variant: TargetPlatformVariant.mobile());
  ...
  testWidgets('Vertical GridViews use PrimaryScrollController by default on mobile', (WidgetTester tester) async {
    final ScrollController controller = ScrollController();
    await tester.pumpWidget(
      primaryScrollControllerBoilerplate(child: GridView.count(crossAxisCount: 1), controller: controller),
    );
    expect(controller.hasClients, isTrue);
  }, variant: TargetPlatformVariant.mobile());
  ...
  testWidgets('Horizontal CustomScrollViews are non-primary by default', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(primaryScrollControllerBoilerplate(
      child: CustomScrollView(scrollDirection: Axis.horizontal, controller: ScrollController()),
      controller: controller,
    ));
    expect(controller.hasClients, isFalse);
  });
  ...
  testWidgets('Horizontal ListViews are non-primary by default', (WidgetTester tester) async {
    final ScrollController controller = ScrollController();
    await tester.pumpWidget(primaryScrollControllerBoilerplate(
      child: ListView(scrollDirection: Axis.horizontal, controller: ScrollController()),
      controller: controller,
    ));
    expect(controller.hasClients, isFalse);
  });
  ...
  testWidgets('Horizontal GridViews are non-primary by default', (WidgetTester tester) async {
    final ScrollController controller = ScrollController();
    await tester.pumpWidget(primaryScrollControllerBoilerplate(
      child: GridView.count(scrollDirection: Axis.horizontal, controller: ScrollController(), crossAxisCount: 1),
      controller: controller,
    ));
    expect(controller.hasClients, isFalse);
  });
  ...
  testWidgets('ListViews with controllers are non-primary by default', (WidgetTester tester) async {
    final ScrollController controller = ScrollController();
    await tester.pumpWidget(
      primaryScrollControllerBoilerplate(child: ListView(controller: ScrollController()), controller: controller),
    );
    expect(controller.hasClients, isFalse);
  });
  ...
  testWidgets('CustomScrollView sets PrimaryScrollController when primary', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(Directionality(
      textDirection: TextDirection.ltr,
      child: PrimaryScrollController(controller: primaryScrollController, child: const CustomScrollView(primary: true)),
    ));
  ...
  testWidgets('ListView sets PrimaryScrollController when primary', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(Directionality(
      textDirection: TextDirection.ltr,
      child: PrimaryScrollController(controller: primaryScrollController, child: ListView(primary: true)),
    ));
    final Scrollable scrollable = tester.widget(find.byType(Scrollable));
    expect(scrollable.controller, primaryScrollController);
  });
      ...
      home: CustomScrollView(
        ...
        slivers: List<Widget>.generate(20, (int index) {
          return SliverToBoxAdapter(
            child: Focus(autofocus: index == 0, child: SizedBox(key: ValueKey<String>('Box $index'), height: 50.0)),
          );
        }),
      ),
    ));
    final ScrollController controller = PrimaryScrollController.of(tester.element(find.byType(CustomScrollView)));
    await tester.pumpAndSettle();
    ...
    Widget getScrollView() {
      return SizedBox(
        ...
        child: CustomScrollView(
          primary: true,
          slivers: List<Widget>.generate(20, (int index) {
            return SliverToBoxAdapter(child: Focus(child: SizedBox(key: ValueKey<String>('Box $index'), height: 50.0)));
          }),
        ),
      );
    }

    await tester.pumpWidget(MaterialApp(home: Row(children: <Widget>[getScrollView(), getScrollView()])));
    await tester.pumpAndSettle();
    expect(
