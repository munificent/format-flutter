...
void verify(WidgetTester tester, List<Rect> answerKey) {
  final List<Rect> testAnswers =
      tester.renderObjectList<RenderBox>(find.byType(SizedBox, skipOffstage: false)).map<Rect>((RenderBox target) {
...
void main() {
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(tester.renderObject<RenderBox>(find.text('x')).localToGlobal(Offset.zero), const Offset(0.0, 200.0));
      expect(tester.renderObject<RenderSliverPadding>(find.byType(SliverPadding)).geometry!.paintExtent, 200.0);
      controller.jumpTo(50.0);
      await tester.pump();
      expect(tester.renderObject<RenderSliverPadding>(find.byType(SliverPadding)).geometry!.paintExtent, 150.0);
    },
  );
  ...
  testWidgets('SliverPadding propagates geometry offset corrections', (WidgetTester tester) async {
        ...
        child: CustomScrollView(
          ...
          slivers: <Widget>[
            SliverPadding(
              ...
              sliver: SliverList(delegate: SliverChildBuilderDelegate(sliverChildBuilder, childCount: 10)),
            ),
          ],
        ),
      );
    ...
    await tester.pumpWidget(listBuilder((BuildContext context, int index) {
    ...
    expect(tester.getRect(find.widgetWithText(SizedBox, '2')), const Rect.fromLTRB(0.0, 100.0, 800.0, 300.0));

    // Now item 0 is 400.0px and going back will underflow.
    await tester.pumpWidget(listBuilder((BuildContext context, int index) {
      return SizedBox(height: index == 0 ? 400.0 : 200.0, child: Center(child: Text(index.toString())));
    }));
    ...
    expect(tester.getRect(find.widgetWithText(SizedBox, '0')), const Rect.fromLTRB(0.0, -200.0, 800.0, 200.0));
  });
        ...
        child: CustomScrollView(
          slivers: <Widget>[
            SliverPadding(
              ...
              sliver: SliverFillRemaining(hasScrollBody: false, child: Container(key: key, color: Colors.red)),
            ),
          ],
        ),
      ));
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final RenderSliverPadding renderObject = RenderSliverPadding(padding: const EdgeInsets.only(top: 20));
