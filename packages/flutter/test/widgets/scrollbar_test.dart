...
void main() {
      ...
      final List<ScrollMetrics> metricsList = <ScrollMetrics>[
        ...
        ...List<ScrollMetrics>.generate(
          ...
          (int index) => startingMetrics.copyWith(
            pixels: (index + 1) * viewportDimension,
          ),
  ...
  testWidgets('thumb resizes gradually on overscroll', (
    ...
    painter.update(
      metrics.copyWith(pixels: scrollExtent / 2),
      AxisDirection.down,
    );
    painter.paint(testCanvas, size);
    expect(
      captureRect().height,
      moreOrLessEquals(fullThumbExtent, epsilon: 1e-6),
    );
    ...
    painter.update(
      metrics.copyWith(pixels: scrollExtent * 1.001),
      AxisDirection.down,
    );
    painter.paint(testCanvas, size);
    expect(
      captureRect().height,
      moreOrLessEquals(fullThumbExtent, epsilon: 2.0),
    );

    // Scrolling way past the end shrinks the thumb to minimum.
    painter.update(
      metrics.copyWith(pixels: double.infinity),
      AxisDirection.down,
    );
    painter.paint(testCanvas, size);
      ...
      await tester.pumpWidget(Directionality(
        ...
        child: MediaQuery(
          ...
          child: RawScrollbar(
            ...
            thumbColor: const Color(0x11111111),
            child: const SingleChildScrollView(
              child: SizedBox(height: 1000.0, width: 50.0),
            ),
          ),
        ),
      ));
  ...
  test(
    ...
    () {
      ...
      expect(
        painter.shouldRepaint(createPainter(color: const Color(0xFFFFFFFF))),
        true,
      );
      expect(painter.shouldRepaint(
        createPainter(fadeoutOpacityAnimation: kAlwaysDismissedAnimation),
      ), true);
      expect(painter.shouldRepaint(
        createPainter(trackColor: const Color(0xFFFFFFFF)),
      ), true);
      expect(painter.shouldRepaint(
      ...
      expect(
        painter.shouldRepaint(createPainter(thickness: _kThickness + 1.0)),
        true,
      );
      expect(painter.shouldRepaint(
        createPainter(padding: const EdgeInsets.all(1.0)),
      ), true);
      ...
      expect(painter.shouldRepaint(
        createPainter(radius: const Radius.circular(1.0)),
      ), true);
      expect(painter.shouldRepaint(
        createPainter(trackRadius: const Radius.circular(1.0)),
      ), true);
      expect(painter.shouldRepaint(
        createPainter(shape: const CircleBorder(side: BorderSide(width: 2.0))),
      ), true);
      expect(
      ...
      expect(
        painter.shouldRepaint(createPainter(minOverscrollLength: 1.0)),
        true,
      );
      expect(painter.shouldRepaint(
  ...
  testWidgets(
    'RawScrollbar correctly assigns colors',
    (WidgetTester tester) async {
      final ScrollController scrollController = ScrollController();
      await tester.pumpWidget(Directionality(
        textDirection: TextDirection.ltr,
  ...
  testWidgets(
    'Track offset respects MediaQuery padding',
    (WidgetTester tester) async {
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        ...
        greaterThan((100.0 * 10 + 500.0 * 90) / 3),
      );
      expect(
        find.byType(RawScrollbar),
        paints
