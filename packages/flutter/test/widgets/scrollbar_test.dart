...
void main() {
  ...
  test('Scrollbar is not smaller than minLength with large scroll views, '
    ...
    final ScrollMetrics metrics = defaultMetrics.copyWith(maxScrollExtent: 100000, viewportDimension: size.height);
  ...
  test('When scrolling normally (no overscrolling), the size of the scrollbar stays the same, '
    ...
    painter = _buildPainter(minLength: minLen, minOverscrollLength: minLen, scrollMetrics: defaultMetrics);
  ...
  test('mainAxisMargin is respected', () {
    ...
    for (final double margin in margins) {
      painter = _buildPainter(mainAxisMargin: margin, minLength: minLen, scrollMetrics: defaultMetrics);

      // Overscroll to double.negativeInfinity (top).
      painter.update(startingMetrics.copyWith(pixels: double.negativeInfinity), startingMetrics.axisDirection);

      painter.paint(testCanvas, size);
      expect(captureRect().top, margin);

      // Overscroll to double.infinity (down).
      painter.update(startingMetrics.copyWith(pixels: double.infinity), startingMetrics.axisDirection);
  ...
  test('crossAxisMargin & text direction are respected', () {
    ...
    for (final TextDirection textDirection in TextDirection.values) {
      painter = _buildPainter(crossAxisMargin: margin, scrollMetrics: startingMetrics, textDirection: textDirection);
      ...
      for (final AxisDirection direction in AxisDirection.values) {
        ...
        switch (direction) {
          ...
          case AxisDirection.down:
            expect(margin, textDirection == TextDirection.ltr ? size.width - rect.right : rect.left);
  ...
  test('scrollbarOrientation are respected', () {
    ...
    for (final ScrollbarOrientation scrollbarOrientation in ScrollbarOrientation.values) {
      ...
      painter = _buildPainter(scrollMetrics: startingMetrics, scrollbarOrientation: scrollbarOrientation);
  ...
  test('scrollbarOrientation default values are correct', () {
    ...
    // Horizontal scroll
    painter = _buildPainter(scrollMetrics: startingMetrics);
    painter.update(startingMetrics.copyWith(axisDirection: AxisDirection.right), AxisDirection.right);
    painter.paint(testCanvas, size);
    rect = captureRect();
    expect(rect.left, 0);
    expect(rect.top, 23 - _kThickness);
  ...
  group('Padding works for all scroll directions', () {
    ...
    testWidgets('down', (WidgetTester tester) async {
      ...
      // Bottom overscroll.
      painter.update(metrics.copyWith(viewportDimension: size.height, pixels: double.infinity), AxisDirection.down);
    ...
    testWidgets('up', (WidgetTester tester) async {
      painter.update(
        metrics.copyWith(viewportDimension: size.height, pixels: double.infinity, axisDirection: AxisDirection.up),
        AxisDirection.up,
      );
      ...
      // Bottom overscroll.
      painter.update(metrics.copyWith(
    ...
    testWidgets('left', (WidgetTester tester) async {
      ...
      // Left overscroll.
      painter.update(
        metrics.copyWith(viewportDimension: size.width, pixels: double.infinity, axisDirection: AxisDirection.left),
        AxisDirection.left,
      );
    ...
    testWidgets('right', (WidgetTester tester) async {
      painter.update(
        metrics.copyWith(viewportDimension: size.width, pixels: double.infinity, axisDirection: AxisDirection.right),
        AxisDirection.right,
      );
  ...
  test('should scroll towards the right direction', () {
    ...
    for (final double minLength in <double>[_kMinThumbExtent, double.infinity]) {
      ...
      painter = _buildPainter(minLength: minLength, minOverscrollLength: minLength, scrollMetrics: startingMetrics);
  ...
  testWidgets('Tapping the track area pages the Scroll View', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(Directionality(
      ...
      child: MediaQuery(
        ...
        child: RawScrollbar(
          ...
          child:
              SingleChildScrollView(controller: scrollController, child: const SizedBox(width: 1000.0, height: 1000.0)),
        ),
      ),
    ));
    ...
    expect(
      ...
      paints
        ..rect(rect: const Rect.fromLTRB(794.0, 0.0, 800.0, 600.0))
        ..rect(rect: const Rect.fromLTRB(794.0, 240.0, 800.0, 600.0), color: const Color(0x66BCBCBC)),
    );
  ...
  testWidgets('Scrollbar never goes away until finger lift', (WidgetTester tester) async {
    await tester.pumpWidget(
      const Directionality(
        ...
        child: MediaQuery(
          data: MediaQueryData(),
          child: RawScrollbar(child: SingleChildScrollView(child: SizedBox(width: 4000.0, height: 4000.0))),
        ),
      ),
    );
  ...
  testWidgets('Scrollbar thumb can be dragged', (WidgetTester tester) async {
    ...
    expect(
      ...
      paints
        ..rect(rect: const Rect.fromLTRB(794.0, 0.0, 800.0, 600.0))
        ..rect(rect: const Rect.fromLTRB(794.0, 10.0, 800.0, 100.0), color: const Color(0x66BCBCBC)),
    );
  });
      ...
      await tester.pumpWidget(Directionality(
        ...
        child: MediaQuery(
          ...
          child: ScrollConfiguration(
            ...
            behavior:
                const ScrollBehavior().copyWith(scrollbars: false, physics: const AlwaysScrollableScrollPhysics()),
            child: PrimaryScrollController(
              controller: scrollController,
              child: RawScrollbar(
                thumbVisibility: true,
                controller: scrollController,
                child: const SingleChildScrollView(primary: true, child: SizedBox(width: 4000.0, height: 4000.0)),
              ),
            ),
          ),
        ),
  ...
  testWidgets('Scrollbar thumb can be dragged into overscroll if the platform allows it', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(Directionality(
      ...
      child: MediaQuery(
        ...
        child: ScrollConfiguration(
          // Don't apply a scrollbar automatically for this test.
          behavior: const ScrollBehavior().copyWith(scrollbars: false, physics: const AlwaysScrollableScrollPhysics()),
          child: PrimaryScrollController(
            controller: scrollController,
            child: RawScrollbar(
              thumbVisibility: true,
                ...
                children: <Widget>[
                  RawScrollbar(
                    ...
                    child: SizedBox(
                      ...
                      child: SingleChildScrollView(
                        key: innerKey,
                        child: const SizedBox(key: Key('Inner scrollable'), height: 1000.0, width: double.infinity),
                      ),
                    ),
                  ),
                ],
          ...
          child: RawScrollbar(
            ...
            child: SingleChildScrollView(
              child: GestureDetector(
                onTap: () => onTap = true,
                child: const SizedBox(width: 4000.0, height: 4000.0, child: ColoredBox(color: Color(0x00000000))),
              ),
            ),
          ),
        ),
  ...
  testWidgets('RawScrollbar.thumbVisibility asserts that a ScrollPosition is attached', (WidgetTester tester) async {
    ...
    expect(exception.message, contains("The Scrollbar's ScrollController has no ScrollPosition attached."));
  ...
  testWidgets('Scrollbar thumb can be dragged in reverse', (WidgetTester tester) async {
      ...
      child: MediaQuery(
        ...
        child: PrimaryScrollController(
          ...
          child: RawScrollbar(
            thumbVisibility: true,
            controller: scrollController,
            child: const SingleChildScrollView(reverse: true, child: SizedBox(width: 4000.0, height: 4000.0)),
          ),
        ),
      ),
    ));
    ...
    expect(
      ...
      paints
        ..rect(rect: const Rect.fromLTRB(794.0, 0.0, 800.0, 600.0))
        ..rect(rect: const Rect.fromLTRB(794.0, 510.0, 800.0, 600.0), color: const Color(0x66BCBCBC)),
    );
    ...
    expect(
      ...
      paints
        ..rect(rect: const Rect.fromLTRB(794.0, 0.0, 800.0, 600.0))
        ..rect(rect: const Rect.fromLTRB(794.0, 500.0, 800.0, 590.0), color: const Color(0x66BCBCBC)),
    );
  });
    ...
    await tester.pumpWidget(Directionality(
      ...
      child: MediaQuery(
        ...
        child: RawScrollbar(
          ...
          controller: scrollController,
          thumbVisibility: true,
          child: SingleChildScrollView(controller: scrollController, child: const SizedBox(height: 1000.0)),
        ),
      ),
    ));
    await tester.pumpAndSettle();
    ...
    await tester.pumpWidget(Directionality(
      ...
      child: MediaQuery(
        ...
        child: RawScrollbar(
          ...
          thumbVisibility: true,
          child:
              SingleChildScrollView(controller: scrollController, child: const SizedBox(height: 1000.0, width: 1000)),
        ),
      ),
    ));
    await tester.pumpAndSettle();
  ...
  testWidgets('shape property of RawScrollbar can draw a RoundedRectangleBorder', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(Directionality(
      ...
      child: MediaQuery(
        ...
        child: RawScrollbar(
          ...
          thumbVisibility: true,
          child:
              SingleChildScrollView(controller: scrollController, child: const SizedBox(height: 1000.0, width: 1000.0)),
        ),
      ),
    ));
    await tester.pumpAndSettle();
    expect(
      ...
      paints
        ...
        ..path(includes: const <Offset>[Offset(800.0, 0.0)], excludes: const <Offset>[Offset(780.0, 0.0)]),
    );
  });
        ...
        child: MediaQuery(
          ...
          child: PrimaryScrollController(
            ...
            child: RawScrollbar(
              thumbVisibility: true,
              controller: scrollController,
              child: const SingleChildScrollView(child: SizedBox(width: double.infinity, height: 600.0)),
            ),
          ),
        ),
      );
      ...
      return Directionality(
        ...
        child: MediaQuery(
          ...
          child: RawScrollbar(
            ...
            controller: scrollController,
            thumbVisibility: true,
            child: SingleChildScrollView(controller: scrollController, child: const SingleChildScrollView()),
          ),
        ),
      );
    }
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        ...
        paints
          ..rect(rect: const Rect.fromLTRB(794.0, 0.0, 800.0, 600.0))
          ..rect(rect: const Rect.fromLTRB(794.0, 200.0, 800.0, 400.0), color: const Color(0x66BCBCBC)),
      );
      ...
      expect(
        ...
        paints
          ..rect(rect: const Rect.fromLTRB(794.0, 0.0, 800.0, 600.0))
          ..rect(rect: const Rect.fromLTRB(794.0, 190.0, 800.0, 390.0), color: const Color(0x66BCBCBC)),
      );
    },
          ...
          child: PrimaryScrollController(
            ...
            child: LayoutBuilder(
              builder: (BuildContext context, BoxConstraints constraints) {
                Widget content = const SingleChildScrollView(child: SizedBox(width: 4000.0, height: 4000.0));
                if (buildBar) {
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        ...
        paints
          ..rect(rect: const Rect.fromLTRB(794.0, 0.0, 800.0, 600.0))
          ..rect(rect: const Rect.fromLTRB(794.0, 200.0, 800.0, 218.0), color: const Color(0x66BCBCBC)),
      );
    },
  );
        ...
        child: MediaQuery(
          ...
          child: PrimaryScrollController(
            ...
            child: RawScrollbar(
              ...
              controller: scrollController,
              child:
                  const SingleChildScrollView(primary: true, child: SizedBox(width: double.infinity, height: 1200.0)),
            ),
          ),
        ),
      );
