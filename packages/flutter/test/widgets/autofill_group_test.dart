...
final Matcher _matchesCommit = isMethodCall(
  'TextInput.finishAutofillContext',
  arguments: true,
);
final Matcher _matchesCancel = isMethodCall(
  'TextInput.finishAutofillContext',
  arguments: false,
);
...
void main() {
  testWidgets('AutofillGroup has the right clients', (
            ...
            child: Column(
              children: <Widget>[
                ...
                AutofillGroup(
                  key: innerKey,
                  child: Column(
                    children: <Widget>[client2, TextField(autofillHints: null)],
                  ),
                ),
              ],
            ),
    ...
    final AutofillGroupState innerState = tester.state<AutofillGroupState>(
      find.byKey(innerKey),
    );
    final AutofillGroupState outerState = tester.state<AutofillGroupState>(
      find.byKey(outerKey),
    );

    final State<TextField> clientState1 = tester.state<State<TextField>>(
      find.byWidget(client1),
    );
    final State<TextField> clientState2 = tester.state<State<TextField>>(
      find.byWidget(client2),
    );

    expect(outerState.autofillClients.toList(), <State<TextField>>[
      clientState1,
    ]);
    // The second TextField in the AutofillGroup doesn't have autofill enabled.
    expect(innerState.autofillClients.toList(), <State<TextField>>[
      clientState2,
    ]);
  });
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final AutofillGroupState scopeState = tester.state<AutofillGroupState>(
        find.byKey(scopeKey),
      );
      ...
      expect(scopeState.autofillClients.toList(), <State<TextField>>[
        clientState1,
      ]);
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final State<TextField> clientState3 = tester.state<State<TextField>>(
        find.byKey(keyClient3),
      );
  ...
  testWidgets('disposing AutofillGroups', (WidgetTester tester) async {
    ...
    const TextField placeholder = TextField(
