...
void verify(WidgetTester tester, List<Offset> answerKey, String text) {
  ...
  final String foundText = tester.widgetList<Text>(find.byType(Text)).map<String>((Text widget) => widget.data!).reduce(
    (String value, String element) => value + element,
  );
...
void main() {
  ...
  testWidgets('Viewport with GlobalKey reparenting', (WidgetTester tester) async {
    ...
    verify(tester, <Offset>[Offset.zero, const Offset(0.0, 251.0), const Offset(0.0, 503.0)], 'abc');
    await tester.pumpWidget(Directionality(
      textDirection: TextDirection.ltr,
      child: Viewport(
        offset: offset,
    ...
    verify(tester, <Offset>[Offset.zero, const Offset(0.0, 253.0), const Offset(0.0, 504.0)], 'cab');
    await tester.pumpWidget(Directionality(
      textDirection: TextDirection.ltr,
      child: Viewport(
        offset: offset,
    ...
    verify(tester, <Offset>[Offset.zero, const Offset(0.0, 251.0), const Offset(0.0, 504.0)], 'acb');
    await tester.pumpWidget(Directionality(
      textDirection: TextDirection.ltr,
      child: Viewport(
        offset: offset,
  ...
  testWidgets('Viewport overflow clipping of SliverToBoxAdapter', (WidgetTester tester) async {
    await tester.pumpWidget(Directionality(
      ...
      child: Viewport(
        ...
        slivers: const <Widget>[SliverToBoxAdapter(child: SizedBox(height: 400.0, child: Text('a')))],
      ),
    ));
    ...
    await tester.pumpWidget(Directionality(
      ...
      child: Viewport(
        offset: ViewportOffset.fixed(100.0),
        slivers: const <Widget>[SliverToBoxAdapter(child: SizedBox(height: 4000.0, child: Text('a')))],
      ),
    ));
  ...
  testWidgets('Viewport overflow clipping of SliverBlock', (WidgetTester tester) async {
    await tester.pumpWidget(Directionality(
      ...
      child: Viewport(
        offset: ViewportOffset.zero(),
        slivers: <Widget>[
          SliverList(delegate: SliverChildListDelegate(const <Widget>[SizedBox(height: 400.0, child: Text('a'))])),
        ],
      ),
    ));
    ...
    await tester.pumpWidget(Directionality(
      ...
      child: Viewport(
        offset: ViewportOffset.fixed(100.0),
        slivers: <Widget>[
          SliverList(delegate: SliverChildListDelegate(const <Widget>[SizedBox(height: 4000.0, child: Text('a'))])),
        ],
      ),
    ));
