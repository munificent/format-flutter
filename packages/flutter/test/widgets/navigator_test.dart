...
void main() {
  ...
  testWidgets('Navigator can set clip behavior', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MediaQuery(
      ...
      child: Directionality(
        textDirection: TextDirection.ltr,
        child: Navigator(pages: const <Page<void>>[page], clipBehavior: Clip.none, onPopPage: (_, __) => false),
      ),
    ));
  ...
  testWidgets('Zero transition page-based route correctly notifies observers when it is popped', (
    ...
    final TestObserver observer = TestObserver()
      ..onPopped = (Route<dynamic>? route, Route<dynamic>? previousRoute) {
        observations.add(
          NavigatorObservation(current: route?.settings.name, previous: previousRoute?.settings.name, operation: 'pop'),
        );
      };
    final GlobalKey<NavigatorState> navigator = GlobalKey<NavigatorState>();
    await tester.pumpWidget(TestDependencies(
      child: Navigator(
        key: navigator,
        pages: pages,
  ...
  testWidgets('Push and pop should trigger the observers', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(routes: routes, navigatorObservers: <NavigatorObserver>[observer]));
    expect(find.text('/'), findsOneWidget);
    expect(find.text('A'), findsNothing);
  ...
  testWidgets('Add and remove an observer should work', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(routes: routes, navigatorObservers: <NavigatorObserver>[observer1]));
    expect(isPushed, isFalse);
    expect(isPopped, isFalse);

    await tester.pumpWidget(MaterialApp(routes: routes, navigatorObservers: <NavigatorObserver>[observer1, observer2]));
    await tester.tap(find.text('/'));
    await tester.pump();
    await tester.pump(const Duration(seconds: 1));
    expect(isPushed, isTrue);
  ...
  testWidgets('initial route trigger observer in the right order', (WidgetTester tester) async {
    ...
    final TestObserver observer = TestObserver()
      ..onPushed = (Route<dynamic>? route, Route<dynamic>? previousRoute) {
        // Pushes the initial route.
        observations.add(NavigatorObservation(
    ...
    await tester.pumpWidget(
      MaterialApp(routes: routes, initialRoute: '/A/B', navigatorObservers: <NavigatorObserver>[observer]),
    );
  ...
  testWidgets('Route didAdd and dispose in same frame work', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(TestDependencies(
      child:
          TabBarView(controller: controller, children: <Widget>[buildNavigator(), buildNavigator(), buildNavigator()]),
    ));
  ...
  testWidgets('pushReplacement correctly reports didReplace to the observer', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(routes: routes, navigatorObservers: <NavigatorObserver>[observer], initialRoute: '/A/B'),
    );
    await tester.pumpAndSettle();
    expect(find.text('B'), isOnstage);
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(MaterialApp(navigatorKey: navigator, routes: routes, initialRoute: '/A/B'));
      await tester.pumpAndSettle();
      await tester.tap(find.text('B'));
      await tester.pumpAndSettle();
  ...
  testWidgets('route semantics', (WidgetTester tester) async {
    ...
    expect(
      semantics,
      includesNodeWith(label: 'Page 1', flags: <SemanticsFlag>[SemanticsFlag.namesRoute, SemanticsFlag.isHeader]),
    );
    ...
    expect(
      semantics,
      includesNodeWith(label: 'Page 2', flags: <SemanticsFlag>[SemanticsFlag.namesRoute, SemanticsFlag.isHeader]),
    );
    ...
    expect(
      semantics,
      includesNodeWith(label: 'Page 3', flags: <SemanticsFlag>[SemanticsFlag.namesRoute, SemanticsFlag.isHeader]),
    );
  ...
  testWidgets('arguments for named routes on Navigator', (WidgetTester tester) async {
    ...
    Navigator.pushReplacementNamed(currentRouteKey.currentContext!, '/D', arguments: 'pushReplacementNamed');
    await tester.pumpAndSettle();
  ...
  group('error control test', () {
    ...
    testWidgets('onUnknownRoute null and onGenerateRoute returns null', (WidgetTester tester) async {
      final GlobalKey<NavigatorState> navigatorKey = GlobalKey<NavigatorState>();
      await tester.pumpWidget(Navigator(key: navigatorKey, onGenerateRoute: (_) => null, onUnknownRoute: (_) => null));
      final dynamic exception = tester.takeException();
      expect(exception, isNotNull);
      expect(exception, isFlutterError);
      final FlutterError error = exception as FlutterError;
  ...
  testWidgets('OverlayEntry of topmost route is set to opaque after Push', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(
      ...
      initialRoute: '/',
      onGenerateRoute: (RouteSettings settings) {
        return NoAnimationPageRoute(pageBuilder: (_) => Container(key: ValueKey<String>(settings.name!)));
      },
    ));
  ...
  testWidgets('OverlayEntry of topmost route is set to opaque after Replace', (WidgetTester tester) async {
    ...
    final Route<dynamic> oldRoute =
        ModalRoute.of(tester.element(find.byKey(const ValueKey<String>('/A'), skipOffstage: false)))!;
    final Route<void> newRoute = NoAnimationPageRoute(pageBuilder: (_) => Container(key: const ValueKey<String>('/C')));
      ...
      child: Navigator(
        ...
        onGenerateInitialRoutes: (NavigatorState navigator, String initialRoute) {
          ...
          for (final String route in initialRoute.split(' ')) {
            result.add(MaterialPageRoute<void>(
              builder: (BuildContext context) {
                return Text(route);
              },
            ));
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(MaterialApp(
        ...
        home: Navigator(
          key: sub,
          onGenerateRoute: (RouteSettings settings) {
            return MaterialPageRoute<void>(settings: settings, builder: (BuildContext context) => const Text('dummy'));
          },
        ),
      ));
  ...
  testWidgets('hero controller scope works', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(HeroControllerScope(
      controller: spy,
      child: TestDependencies(
        child: Navigator(
          key: top,
  ...
  testWidgets('hero controller can correctly transfer subscription - swapping navigator', (WidgetTester tester) async {
    ...
    final HeroControllerSpy spy1 = HeroControllerSpy()
      ..onPushed = (Route<dynamic>? route, Route<dynamic>? previousRoute) {
        observations1.add(NavigatorObservation(
          current: route?.settings.name,
          previous: previousRoute?.settings.name,
          operation: 'didPush',
        ));
    ...
    final HeroControllerSpy spy2 = HeroControllerSpy()
      ..onPushed = (Route<dynamic>? route, Route<dynamic>? previousRoute) {
        observations2.add(NavigatorObservation(
          current: route?.settings.name,
          previous: previousRoute?.settings.name,
          operation: 'didPush',
        ));
  ...
  group('Page api', () {
    ...
    testWidgets('can initialize with pages list', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(buildNavigator(view: tester.view, pages: myPages, onPopPage: onPopPage, key: navigator));
    ...
    testWidgets('can handle duplicate page key if update before transition finishes', (WidgetTester tester) async {
      ...
      final GlobalKey<NavigatorState> navigator = GlobalKey<NavigatorState>();
      final List<TestPage> myPages1 = <TestPage>[const TestPage(key: ValueKey<String>('1'), name: 'initial')];
      final List<TestPage> myPages2 = <TestPage>[const TestPage(key: ValueKey<String>('2'), name: 'second')];

      bool onPopPage(Route<dynamic> route, dynamic result) => false;

      await tester.pumpWidget(buildNavigator(view: tester.view, pages: myPages1, onPopPage: onPopPage, key: navigator));
      await tester.pump();
      expect(find.text('initial'), findsOneWidget);
      ...
      await tester.pumpWidget(buildNavigator(view: tester.view, pages: myPages2, onPopPage: onPopPage, key: navigator));
      await tester.pump();
      await tester.pumpWidget(buildNavigator(view: tester.view, pages: myPages1, onPopPage: onPopPage, key: navigator));
      await tester.pump();
      await tester.pumpWidget(buildNavigator(view: tester.view, pages: myPages2, onPopPage: onPopPage, key: navigator));
    ...
    testWidgets('throw if page list is empty', (WidgetTester tester) async {
      ...
      expect(firstError!.exception.toString(), 'The Navigator.pages must not be empty to use the Navigator.pages API');
    });
    ...
    testWidgets('complex case 1', (WidgetTester tester) async {
      final GlobalKey<NavigatorState> navigator = GlobalKey<NavigatorState>();
      List<TestPage> myPages = <TestPage>[const TestPage(key: ValueKey<String>('1'), name: 'initial')];
      bool onPopPage(Route<dynamic> route, dynamic result) {
        myPages.removeWhere((Page<dynamic> page) => route.settings == page);
        return route.didPop(result);
      }
      ...
      navigator.currentState!.push(
        MaterialPageRoute<void>(builder: (BuildContext context) => const Text('initial-pageless1')),
  ...
  testWidgets('Navigator requests focus if requestFocus is true', (WidgetTester tester) async {
    ...
    final Map<String, Widget> routes = <String, Widget>{'/': const Text('A'), '/second': Text('B', key: innerKey)};
...
class RouteAnnouncementSpy extends Route<void> {
  RouteAnnouncementSpy({this.onDidChangePrevious, this.onDidChangeNext, this.onDidPopNext, super.settings});
...
class ZeroDurationPageRoute extends PageRoute<void> {
  ZeroDurationPageRoute({required ZeroDurationPage page}) : super(settings: page, allowSnapshotting: false);
