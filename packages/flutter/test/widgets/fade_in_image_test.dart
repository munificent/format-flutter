...
FadeInImageParts findFadeInImage(WidgetTester tester) {
  ...
  final Iterable<Element> rawImageElements = tester.elementList(
...
Future<void> main() async {
  ...
  group('FadeInImage', () {
    testWidgets('animates an uncached image', (WidgetTester tester) async {
      final TestImageProvider placeholderProvider = TestImageProvider(
        placeholderImage,
      );
      ...
      await tester.pump();
      expect(findFadeInImage(tester).placeholder!.rawImage.image!.isCloneOf(
        placeholderImage,
      ), true);
      expect(findFadeInImage(tester).target.rawImage.image, null);
      ...
      for (int i = 0; i < 5; i += 1) {
        ...
        expect(
          parts.placeholder!.rawImage.image!.isCloneOf(placeholderImage),
          true,
        );
    ...
    testWidgets("FadeInImage's image obeys gapless playback", (
      ...
      final TestImageProvider imageProvider = TestImageProvider(targetImage);
      final TestImageProvider secondImageProvider = TestImageProvider(
        replacementImage,
      );
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        parts = findFadeInImage(tester);
        expect(
          parts.target.rawImage.image!.isCloneOf(replacementImage),
          isTrue,
        );
        expect(parts.target.opacity, 1);
      },
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        final TestImageProvider secondPlaceholderProvider = TestImageProvider(
          replacementImage,
        );
        final TestImageProvider imageProvider = TestImageProvider(targetImage);
        ...
        parts = findFadeInImage(tester);
        expect(
          parts.placeholder!.rawImage.image!.isCloneOf(replacementImage),
          true,
        );
        expect(parts.placeholder!.opacity, 1);
      },
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        expect(findFadeInImage(tester).state, same(state));
        expect(
          findFadeInImage(tester).placeholder!.opacity,
          moreOrLessEquals(0),
        );
    ...
    group('semantics', () {
      ...
      group('label', () {
        ...
        testWidgets(
          ...
          (WidgetTester tester) async {
            Semantics semanticsWidget() => tester.widget(
    ...
    group("placeholder's FilterQuality", () {
      ...
      testWidgets('should be the given value when set', (
        ...
        expect(
          findFadeInImage(tester).target.filterQuality,
          equals(FilterQuality.medium),
        );
        expect(
          findFadeInImage(tester).placeholder!.filterQuality,
