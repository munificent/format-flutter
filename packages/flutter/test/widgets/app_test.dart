...
void main() {
  ...
  testWidgets('WidgetsApp can override default key bindings', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(WidgetsApp(
      ...
      shortcuts: const <ShortcutActivator, Intent>{SingleActivator(LogicalKeyboardKey.space): TestIntent()},
      builder: (BuildContext context, Widget? child) {
        return Material(
          child: Checkbox(
            value: checked,
  ...
  testWidgets('WidgetsApp.router route information parser is optional', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(WidgetsApp.router(routerDelegate: delegate, color: const Color(0xFF123456)));
    expect(find.text('initial'), findsOneWidget);
  ...
  testWidgets('WidgetsApp.router throw if route information provider is provided but no route information parser', (
    ...
    await expectLater(() async {
      await tester.pumpWidget(
        WidgetsApp.router(routeInformationProvider: provider, routerDelegate: delegate, color: const Color(0xFF123456)),
      );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await expectLater(() async {
        await tester.pumpWidget(
          WidgetsApp.router(routerDelegate: delegate, routerConfig: routerConfig, color: const Color(0xFF123456)),
        );
      }, throwsAssertionError);
    },
  );
  ...
  testWidgets('WidgetsApp.router router config works', (WidgetTester tester) async {
    final RouterConfig<RouteInformation> routerConfig = RouterConfig<RouteInformation>(
      routeInformationProvider:
          PlatformRouteInformationProvider(initialRouteInformation: RouteInformation(uri: Uri.parse('initial'))),
      routeInformationParser: SimpleRouteInformationParser(),
    ...
    await tester.pumpWidget(WidgetsApp.router(routerConfig: routerConfig, color: const Color(0xFF123456)));
    expect(find.text('initial'), findsOneWidget);
  ...
  test('basicLocaleListResolution', () {
    ...
    expect(basicLocaleListResolution(
  ...
  testWidgets("WidgetsApp doesn't have dependency on MediaQuery", (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MediaQuery(data: const MediaQueryData(textScaleFactor: 10), child: widget));

    expect(routeBuildCount, equals(1));

    await tester.pumpWidget(MediaQuery(data: const MediaQueryData(textScaleFactor: 20), child: widget));
  ...
  testWidgets('WidgetsApp provides meta based shortcuts for iOS and macOS', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(WidgetsApp(
      builder: (BuildContext context, Widget? child) {
        return Actions(actions: actions, child: Focus(focusNode: focusNode, child: const Placeholder()));
      },
      color: const Color(0xFF123456),
    ));
