...
void main() {
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      await tester.pumpWidget(
        boilerplateWidget(null, maxChildSize: .8, snapSizes: <double>[.9]),
      );
      ...
      await tester.pumpWidget(
        boilerplateWidget(null, snapSizes: <double>[.6, .6, .9]),
      );
      expect(tester.takeException(), isAssertionError);
    },
  );
  ...
  group('Scroll Physics', () {
    ...
    testWidgets('Ballistic animation on fling should not leak Ticker', (
      WidgetTester tester,
    ) async {
  ...
  testWidgets('Fling snaps in direction of momentum', (
    WidgetTester tester,
  ) async {
    const Key stackKey = ValueKey<String>('stack');
    const Key containerKey = ValueKey<String>('container');
  ...
  testWidgets('Can get size and pixels', (WidgetTester tester) async {
    ...
    expect(
      controller.pixels,
      closeTo(.6 * screenHeight, precisionErrorTolerance),
    );
    ...
    expect(
      controller.pixels,
      closeTo(.4 * screenHeight, precisionErrorTolerance),
    );
  });
  ...
  testWidgets('Can listen for changes in sheet size', (
    WidgetTester tester,
  ) async {
    const Key stackKey = ValueKey<String>('stack');
    const Key containerKey = ValueKey<String>('container');
    ...
    expect(
      loggedSizes,
      <double>[.45, .5].map((double v) => closeTo(v, precisionErrorTolerance)),
    );
    loggedSizes.clear();
    ...
    await tester.pumpAndSettle();
    expect(loggedSizes, <double>[.7, .8, .9, 1].map(
      (double v) => closeTo(v, precisionErrorTolerance),
    ));
    loggedSizes.clear();
  ...
  testWidgets('DraggableScrollableSheet controller can be changed', (
    WidgetTester tester,
  ) async {
