...
void main() {
  ...
  testWidgets('Scrolls correct amount when maxChildSize < 1.0', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      boilerplateWidget(null, maxChildSize: .6, initialChildSize: .25, itemExtent: 25.0, containerKey: key),
    );
  ...
  testWidgets('Scrolls correct amount when maxChildSize == 1.0', (WidgetTester tester) async {
    const Key key = ValueKey<String>('container');
    await tester.pumpWidget(boilerplateWidget(null, initialChildSize: .25, itemExtent: 25.0, containerKey: key));
  ...
  testWidgets('Does not snap away from initial child on build', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      boilerplateWidget(null, snap: true, initialChildSize: .7, containerKey: containerKey, stackKey: stackKey),
    );
    await tester.pumpAndSettle();
  ...
  for (final bool useActuator in <bool>[false, true]) {
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        await tester.pumpWidget(
          boilerplateWidget(null, controller: controller, snap: true, containerKey: containerKey, stackKey: stackKey),
        );
        await tester.pumpAndSettle();
        final double screenHeight = tester.getSize(find.byKey(stackKey)).height;
        ...
        await tester.pumpAndSettle();
        expect(tester.getSize(find.byKey(containerKey)).height / screenHeight, closeTo(1.0, precisionErrorTolerance));
        ...
        expect(tester.getSize(find.byKey(containerKey)).height / screenHeight, closeTo(.5, precisionErrorTolerance));
      },
    );
  }
  ...
  for (final Duration? snapAnimationDuration in <Duration?>[null, const Duration(seconds: 2)]) {
    testWidgets('Zero velocity drag snaps to nearest snap target with '
      ...
      await tester.pumpAndSettle();
      expect(tester.getSize(find.byKey(containerKey)).height / screenHeight, closeTo(.75, precisionErrorTolerance));
  ...
  for (final List<double>? snapSizes in <List<double>?>[null, <double>[]]) {
    testWidgets('Setting snapSizes to $snapSizes resolves to min and max', (WidgetTester tester) async {
      ...
      const Key containerKey = ValueKey<String>('container');
      await tester.pumpWidget(
        boilerplateWidget(null, snap: true, stackKey: stackKey, containerKey: containerKey, snapSizes: snapSizes),
      );
      await tester.pumpAndSettle();
      final double screenHeight = tester.getSize(find.byKey(stackKey)).height;
      ...
      await tester.pumpAndSettle();
      expect(tester.getSize(find.byKey(containerKey)).height / screenHeight, closeTo(.25, precisionErrorTolerance));
    }, variant: TargetPlatformVariant.all());
  }
  ...
  testWidgets('Min and max are implicitly added to snapSizes', (WidgetTester tester) async {
    ...
    const Key containerKey = ValueKey<String>('container');
    await tester.pumpWidget(
      boilerplateWidget(null, snap: true, stackKey: stackKey, containerKey: containerKey, snapSizes: <double>[.5]),
    );
    await tester.pumpAndSettle();
    final double screenHeight = tester.getSize(find.byKey(stackKey)).height;
  ...
  testWidgets('Changes to widget parameters are propagated', (WidgetTester tester) async {
    const Key stackKey = ValueKey<String>('stack');
    const Key containerKey = ValueKey<String>('container');
    await tester.pumpWidget(boilerplateWidget(null, stackKey: stackKey, containerKey: containerKey));
    await tester.pumpAndSettle();
    final double screenHeight = tester.getSize(find.byKey(stackKey)).height;
    expect(tester.getSize(find.byKey(containerKey)).height / screenHeight, closeTo(.5, precisionErrorTolerance));
    ...
    expect(tester.getSize(find.byKey(containerKey)).height / screenHeight, closeTo(.6, precisionErrorTolerance));

    // Pump the same widget but with a new max child size.
    await tester.pumpWidget(
      boilerplateWidget(null, stackKey: stackKey, containerKey: containerKey, initialChildSize: .6, maxChildSize: .9),
    );
    await tester.pumpAndSettle();
    ...
    await tester.pumpAndSettle();
    expect(tester.getSize(find.byKey(containerKey)).height / screenHeight, closeTo(.9, precisionErrorTolerance));

    // Pump the same widget but with a new max child size and initial size.
    await tester.pumpWidget(
      boilerplateWidget(null, stackKey: stackKey, containerKey: containerKey, maxChildSize: .8, initialChildSize: .7),
    );
    await tester.pumpAndSettle();
    ...
    expect(tester.getSize(find.byKey(containerKey)).height / screenHeight, closeTo(.8, precisionErrorTolerance));
  ...
  testWidgets('Fling snaps in direction of momentum', (WidgetTester tester) async {
    ...
    const Key containerKey = ValueKey<String>('container');
    await tester.pumpWidget(
      boilerplateWidget(null, snap: true, stackKey: stackKey, containerKey: containerKey, snapSizes: <double>[.5, .75]),
    );
    await tester.pumpAndSettle();
    final double screenHeight = tester.getSize(find.byKey(stackKey)).height;
  ...
  testWidgets('Transitioning between scrollable children sharing a scroll controller will not throw', (
    WidgetTester tester,
  ) async {
    int s = 0;
                ...
                return PrimaryScrollController(
                  ...
                  child: AnimatedSwitcher(
                    ...
                    child: (s.isEven)
                        ? ListView(
                            children: <Widget>[
                              ElevatedButton(onPressed: () => setState(() => ++s), child: const Text('Switch to 2')),
                        ...
                        : SingleChildScrollView(
                            child: Column(
                              children: <Widget>[
                                ElevatedButton(onPressed: () => setState(() => ++s), child: const Text('Switch to 1')),
                                Container(height: 400, color: Colors.blue),
                              ],
                            ),
                          ),
  ...
  for (final bool shouldAnimate in <bool>[true, false]) {
    testWidgets('Can ${shouldAnimate ? 'animate' : 'jump'} to arbitrary positions', (WidgetTester tester) async {
      ...
      final DraggableScrollableController controller = DraggableScrollableController();
      await tester.pumpWidget(
        boilerplateWidget(null, controller: controller, stackKey: stackKey, containerKey: containerKey),
      );
      await tester.pumpAndSettle();
      final double screenHeight = tester.getSize(find.byKey(stackKey)).height;
      ...
      goTo(.4);
      expect(await tester.pumpAndSettle(), expectedPumpCount);
      expect(tester.getSize(find.byKey(containerKey)).height / screenHeight, closeTo(.4, precisionErrorTolerance));
      expect(find.text('Item 1'), findsOneWidget);
      ...
      await tester.pumpAndSettle();
      expect(tester.getSize(find.byKey(containerKey)).height / screenHeight, closeTo(1, precisionErrorTolerance));
      expect(find.text('Item 1'), findsNothing);
      expect(find.text('Item 20'), findsOneWidget);
      expect(find.text('Item 70'), findsNothing);
  ...
  testWidgets('Can animateTo with a nonlinear curve', (WidgetTester tester) async {
    ...
    expect(tester.getSize(find.byKey(containerKey)).height / screenHeight, closeTo(.55, precisionErrorTolerance));
    await tester.pumpAndSettle();
    expect(tester.getSize(find.byKey(containerKey)).height / screenHeight, closeTo(.6, precisionErrorTolerance));
    ...
    expect(tester.getSize(find.byKey(containerKey)).height / screenHeight, closeTo(.65, precisionErrorTolerance));
    await tester.pumpAndSettle();
    expect(tester.getSize(find.byKey(containerKey)).height / screenHeight, closeTo(.7, precisionErrorTolerance));
  });
  ...
  testWidgets('animateTo interrupts other animations', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(Directionality(
      textDirection: TextDirection.ltr,
      child: boilerplateWidget(null, controller: controller, stackKey: stackKey, containerKey: containerKey),
    ));
    await tester.pumpAndSettle();
    final double screenHeight = tester.getSize(find.byKey(stackKey)).height;
  ...
  testWidgets('Listener does not fire on parameter change and persists after change', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      boilerplateWidget(null, minChildSize: .1, controller: controller, stackKey: stackKey, containerKey: containerKey),
    );
    expect(loggedSizes.isEmpty, true);
  ...
  testWidgets('Listener fires if a parameter change forces a change in size', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      boilerplateWidget(null, minChildSize: .4, controller: controller, stackKey: stackKey, containerKey: containerKey),
    );
    expect(loggedSizes, <double>[.4].map((double v) => closeTo(v, precisionErrorTolerance)));
    loggedSizes.clear();
  });
          ...
          body: DraggableScrollableSheet(
            ...
            builder: (BuildContext context, ScrollController scrollController) {
              return ListView(
                physics: physics,
                controller: scrollController,
                children: <Widget>[const Text('Drag me!'), Container(height: 10000.0, color: Colors.blue)],
              );
            },
          ),
        ),
