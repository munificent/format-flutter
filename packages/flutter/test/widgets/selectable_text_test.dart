...
void main() {
      ...
      home: Material(
        child: StatefulBuilder(
          builder: (BuildContext context, StateSetter setState) {
            setter = setState;
            if (isShow) {
              return const SelectableText('abc def ghi', dragStartBehavior: DragStartBehavior.down);
            } else {
      ...
      const MediaQuery(
        ...
        child: Directionality(
          ...
          child: SelectableText.rich(TextSpan(
            ...
            children: <TextSpan>[
              TextSpan(text: 'Second line!\n', style: TextStyle(fontSize: 30, fontFamily: 'Roboto')),
              TextSpan(text: 'Third line!\n', style: TextStyle(fontSize: 14, fontFamily: 'Roboto')),
            ],
          )),
        ),
      ),
      ...
      const MediaQuery(
        ...
        child: Directionality(
          ...
          child: SelectableText.rich(TextSpan(
            ...
            children: <InlineSpan>[
              WidgetSpan(
                child: SizedBox(width: 120, height: 50, child: Card(child: Center(child: Text('Hello World!')))),
              ),
              TextSpan(text: 'Third line!\n', style: TextStyle(fontSize: 14, fontFamily: 'Roboto')),
            ],
          )),
  ...
  testWidgets('can scale with textScaleFactor', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(boilerplate(child: const SelectableText('selectable text', textScaleFactor: 1.9)));
  ...
  testWidgets('can switch between textWidthBasis', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(boilerplate(child: const SelectableText(text, textWidthBasis: TextWidthBasis.parent)));
    RenderBox textBox = findTextBox();
    expect(textBox.size, const Size(800.0, 28.0));

    await tester.pumpWidget(boilerplate(child: const SelectableText(text, textWidthBasis: TextWidthBasis.longestLine)));
    textBox = findTextBox();
  ...
  testWidgets('can switch between textHeightBehavior', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(boilerplate(child: const SelectableText(text, textHeightBehavior: textHeightBehavior)));
  ...
  testWidgets('enableInteractiveSelection = false, tap', (WidgetTester tester) async {
    await tester.pumpWidget(overlay(child: const SelectableText('abc def ghi', enableInteractiveSelection: false)));
    final EditableText editableText = tester.widget(find.byType(EditableText));
    expect(editableText.controller.selection.baseOffset, -1);
    expect(editableText.controller.selection.extentOffset, -1);
  ...
  testWidgets('selectable text can disable toolbar options', (WidgetTester tester) async {
    await tester.pumpWidget(
      overlay(child: const SelectableText('a selectable text', toolbarOptions: ToolbarOptions(selectAll: true))),
    );
    const int dIndex = 5;
    final Offset dPos = textOffsetToPosition(tester, dIndex);
    await tester.longPressAt(dPos);
  ...
  testWidgets('Can select text by dragging with a mouse', (WidgetTester tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Material(child: SelectableText('abc def ghi', dragStartBehavior: DragStartBehavior.down)),
      ),
    );
    final EditableText editableTextWidget = tester.widget(find.byType(EditableText));
    final TextEditingController controller = editableTextWidget.controller;
  ...
  testWidgets('minLines cannot be greater than maxLines', (WidgetTester tester) async {
    expect(
      () async {
        await tester.pumpWidget(
          overlay(child: SizedBox(width: 300.0, child: SelectableText('abcd', minLines: 4, maxLines: 3))),
        );
      },
  ...
  testWidgets('Can align to center', (WidgetTester tester) async {
    await tester.pumpWidget(
      overlay(child: const SizedBox(width: 300.0, child: SelectableText('abcd', textAlign: TextAlign.center))),
    );

    final RenderEditable editable = findRenderEditable(tester);

    final Offset topLeft = editable.localToGlobal(editable.getLocalRectForCaret(const TextPosition(offset: 2)).topLeft);
  ...
  testWidgets('Can align to center within center', (WidgetTester tester) async {
    await tester.pumpWidget(overlay(
      child: const SizedBox(width: 300.0, child: Center(child: SelectableText('abcd', textAlign: TextAlign.center))),
    ));
  ...
  testWidgets('Selectable text identifies as text field in semantics', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(const MaterialApp(home: Material(child: Center(child: SelectableText('some text')))));

    expect(semantics, includesNodeWith(
      flags: <SemanticsFlag>[SemanticsFlag.isTextField, SemanticsFlag.isReadOnly, SemanticsFlag.isMultiline],
    ));
  ...
  testWidgets('Selectable text rich text with spell out in semantics', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      const MaterialApp(
        home: Material(child: Center(child: SelectableText.rich(TextSpan(text: 'some text', spellOut: true)))),
      ),
    );
    ...
    expect(semantics, includesNodeWith(
      attributedValue: AttributedString(
        'some text',
        attributes: <StringAttribute>[SpellOutStringAttribute(range: const TextRange(start: 0, end: 9))],
      ),
      flags: <SemanticsFlag>[SemanticsFlag.isTextField, SemanticsFlag.isReadOnly, SemanticsFlag.isMultiline],
    ));
  ...
  testWidgets('Selectable text rich text with locale in semantics', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      const MaterialApp(
        home: Material(
          child: Center(child: SelectableText.rich(TextSpan(text: 'some text', locale: Locale('es', 'MX')))),
        ),
      ),
    );
    ...
    await tester.pumpWidget(overlay(
      child: SelectableText.rich(TextSpan(
        children: <TextSpan>[
          ...
          TextSpan(text: 'link', recognizer: TapGestureRecognizer()..onTap = () {}),
        ],
      )),
    ));
  ...
  group('Keyboard Tests', () {
    ...
    Future<void> setupWidget(WidgetTester tester, String text) async {
      ...
      await tester.pumpWidget(MaterialApp(
        home: Material(child: RawKeyboardListener(focusNode: focusNode, child: SelectableText(text, maxLines: 3))),
      ));
      await tester.tap(find.byType(SelectableText));
      await tester.pumpAndSettle();
  ...
  testWidgets('Copy test', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(
      home: Material(
        child: RawKeyboardListener(focusNode: focusNode, child: const SelectableText(testValue, maxLines: 3)),
      ),
    ));
    final EditableText editableTextWidget = tester.widget(find.byType(EditableText).first);
    final TextEditingController controller = editableTextWidget.controller;
  ...
  testWidgets('SelectableText baseline alignment', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(overlay(
      child: Row(
        ...
        children: <Widget>[
          Expanded(
            child: SelectableText('A', key: keyA, style: const TextStyle(fontFamily: 'FlutterTest', fontSize: 10.0)),
          ),
          const Text('abc', style: TextStyle(fontFamily: 'FlutterTest', fontSize: 20.0)),
          Expanded(
            child: SelectableText('B', key: keyB, style: const TextStyle(fontFamily: 'FlutterTest', fontSize: 30.0)),
          ),
        ],
      ),
    ));
  ...
  testWidgets('SelectableText semantics, with semanticsLabel', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      overlay(child: SelectableText('Guten Tag', semanticsLabel: 'German greeting for good day', key: key)),
    );
  ...
  testWidgets('SelectableText semantics, enableInteractiveSelection = false', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(overlay(child: SelectableText('Guten Tag', key: key, enableInteractiveSelection: false)));
    ...
    expect(semantics, hasSemantics(TestSemantics.root(
      children: <TestSemantics>[
        TestSemantics.rootChild(
          ...
          textDirection: TextDirection.ltr,
          actions: <SemanticsAction>[SemanticsAction.longPress],
          flags: <SemanticsFlag>[SemanticsFlag.isReadOnly, SemanticsFlag.isTextField, SemanticsFlag.isMultiline],
        ),
      ],
    ), ignoreTransform: true, ignoreRect: true));
      ...
      home: SingleChildScrollView(
        ...
        child: SelectableText.rich(TextSpan(
          children: <TextSpan>[
            ...
            TextSpan(text: offScreenText, recognizer: TapGestureRecognizer()..onTap = () {}),
          ],
          style: textStyle,
        ), textDirection: TextDirection.ltr),
      ),
                          ...
                          children: <TestSemantics>[
                            TestSemantics(
                              children: <TestSemantics>[
                                ...
                                TestSemantics(
                                  flags: <SemanticsFlag>[SemanticsFlag.isHidden, SemanticsFlag.isLink],
                                  actions: <SemanticsAction>[SemanticsAction.tap],
                                  label: 'off screen',
                                  textDirection: TextDirection.ltr,
                                ),
    ...
    expect(semantics, hasSemantics(TestSemantics.root(
      children: <TestSemantics>[
        TestSemantics(
          ...
          value: testValue,
          textDirection: TextDirection.ltr,
          textSelection: const TextSelection(baseOffset: testValue.length, extentOffset: testValue.length),
        ),
      ],
    ), ignoreRect: true, ignoreTransform: true));
  ...
  testWidgets('SelectableText style is merged with default text style', (WidgetTester tester) async {
    ...
    Widget buildFrame(TextStyle style) {
      return MaterialApp(
        home: Material(
          child: DefaultTextStyle(style: defaultStyle, child: Center(child: SelectableText('something', style: style))),
        ),
      );
    }
  ...
  testWidgets('tap moves cursor to the edge of the word it tapped', (WidgetTester tester) async {
    await tester.pumpWidget(
      const MaterialApp(home: Material(child: Center(child: SelectableText('Atwater Peel Sherbrooke Bonaventure')))),
    );
  ...
  testWidgets('tap moves cursor to the position tapped (Android)', (WidgetTester tester) async {
    ...
    // We moved the cursor.
    expect(controller.selection, const TextSelection.collapsed(offset: 4, affinity: TextAffinity.upstream));
  ...
  testWidgets('double tap on top of cursor also selects word (Android)', (WidgetTester tester) async {
    await tester.pumpWidget(
      const MaterialApp(home: Material(child: Center(child: SelectableText('Atwater Peel Sherbrooke Bonaventure')))),
    );
  ...
  testWidgets('double tap selects word with semantics label', (WidgetTester tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Material(
          child: Center(
            child: SelectableText.rich(TextSpan(text: 'Atwater Peel Sherbrooke Bonaventure', semanticsLabel: '')),
          ),
        ),
      ),
    );
  ...
  testWidgets('long press selects word and shows toolbar (iOS)', (WidgetTester tester) async {
    await tester.pumpWidget(
      const MaterialApp(home: Material(child: Center(child: SelectableText('Atwater Peel Sherbrooke Bonaventure')))),
    );
  ...
  testWidgets('long press selects word and shows custom toolbar (Android)', (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
      home: Material(
        child: Center(
          child:
              SelectableText('Atwater Peel Sherbrooke Bonaventure', selectionControls: cupertinoTextSelectionControls),
        ),
      ),
    ));
  ...
  testWidgets('long press selects word and shows custom toolbar (iOS)', (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
      home: Material(
        child: Center(
          child:
              SelectableText('Atwater Peel Sherbrooke Bonaventure', selectionControls: materialTextSelectionControls),
        ),
      ),
    ));
  ...
  testWidgets('textSelectionControls is passed to EditableText', (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
      home: Material(
        child: Scaffold(
          body: SelectableText('Atwater Peel Sherbrooke Bonaventure', selectionControls: materialTextSelectionControls),
        ),
      ),
    ));
  ...
  testWidgets('long press drag extends the selection to the word under the drag and shows toolbar on lift (iOS)', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      const MaterialApp(home: Material(child: Center(child: SelectableText('Atwater Peel Sherbrooke Bonaventure')))),
    ...
    (WidgetTester tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: Material(
            child: Center(
              child: SelectableText('Atwater Peel Sherbrooke Bonaventure Angrignon Peel Côte-des-Neiges', maxLines: 1),
            ),
          ),
        ),
      );
  ...
  testWidgets('long tap still selects after a double tap select (iOS)', (WidgetTester tester) async {
    await tester.pumpWidget(
      const MaterialApp(home: Material(child: Center(child: SelectableText('Atwater Peel Sherbrooke Bonaventure')))),
    );
  ...
  testWidgets('long tap still selects after a double tap select (macOS)', (WidgetTester tester) async {
    await tester.pumpWidget(
      const MaterialApp(home: Material(child: Center(child: SelectableText('Atwater Peel Sherbrooke Bonaventure')))),
    );
...
//convert
  ...
  testWidgets('force press does not select a word on (android)', (WidgetTester tester) async {
    await tester.pumpWidget(
      const MaterialApp(home: Material(child: SelectableText('Atwater Peel Sherbrooke Bonaventure'))),
    );
  ...
  testWidgets('strut TextStyle increases height', (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
      theme: ThemeData(platform: TargetPlatform.android),
      home: const Material(child: Center(child: SelectableText('something', style: TextStyle(fontSize: 20)))),
    ));
    ...
    await tester.pumpWidget(MaterialApp(
      ...
      home: const Material(
        child:
            Center(child: SelectableText('something', style: TextStyle(fontSize: 20), strutStyle: StrutStyle.disabled)),
      ),
    ));
  ...
  testWidgets('strut no force large strut', (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
      theme: ThemeData(platform: TargetPlatform.android),
      home: const Material(
        child: Center(child: SelectableText('something', maxLines: 6, strutStyle: StrutStyle(fontSize: 25))),
      ),
    ));
  ...
  testWidgets('strut height override', (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
      ...
      home: const Material(
        child: Center(
          child: SelectableText('something', maxLines: 3, strutStyle: StrutStyle(fontSize: 8, forceStrutHeight: true)),
        ),
      ),
    ));
  ...
  testWidgets('Long press shows handles and toolbar', (WidgetTester tester) async {
    await tester.pumpWidget(const MaterialApp(home: Material(child: SelectableText('abc def ghi'))));
  ...
  testWidgets('Mouse tap does not show handles nor toolbar', (WidgetTester tester) async {
    ...
    final Offset ePos = textOffsetToPosition(tester, 5);
    final TestGesture gesture = await tester.startGesture(ePos, pointer: 7, kind: PointerDeviceKind.mouse);
    await tester.pump();
    await gesture.up();
    await tester.pump();
  ...
  testWidgets('Mouse double tap does not show handles nor toolbar', (WidgetTester tester) async {
    ...
    final TestGesture gesture = await tester.startGesture(selectableTextPos, pointer: 7, kind: PointerDeviceKind.mouse);
    await tester.pump(const Duration(milliseconds: 50));
    await gesture.up();
    await tester.pump();
  ...
  testWidgets('SelectableText changes mouse cursor when hovered', (WidgetTester tester) async {
    await tester.pumpWidget(const MaterialApp(home: Material(child: Center(child: SelectableText('test')))));
  ...
  testWidgets('The Select All calls on selection changed with a mouse on windows and linux', (
    WidgetTester tester,
  ) async {
    ...
    final TestGesture gesture =
        await tester.startGesture(ePos, kind: PointerDeviceKind.mouse, buttons: kSecondaryMouseButton);
    await tester.pump();
    await gesture.up();
    await tester.pumpAndSettle();
    ...
    expect(newSelection!.baseOffset, 0);
    expect(newSelection!.extentOffset, 11);
  }, variant: const TargetPlatformVariant(<TargetPlatform>{TargetPlatform.windows, TargetPlatform.linux}));
  ...
  testWidgets('text selection style 1', (WidgetTester tester) async {
    ...
    await expectLater(find.byType(MaterialApp), matchesGoldenFile('selectable_text_golden.TextSelectionStyle.1.png'));
  });
  ...
  testWidgets('text selection style 2', (WidgetTester tester) async {
    ...
    await expectLater(find.byType(MaterialApp), matchesGoldenFile('selectable_text_golden.TextSelectionStyle.2.png'));
  });
        ...
        child: Scaffold(
          body: NestedScrollView(
            headerSliverBuilder: (BuildContext context, bool innerBoxIsScrolled) {
              return <Widget>[
                SliverToBoxAdapter(
                  child: Container(height: 200, color: Colors.black12, child: const Center(child: Text('Sliver 1'))),
                ),
            ...
            body: const TabBarView(
              children: <Widget>[
                ...
                Padding(padding: EdgeInsets.only(top: 100.0), child: SelectableText('Selectable Text')),
              ],
            ),
          ),
        ),
  ...
  testWidgets('SelectableText text span style is merged with default text style', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(const MaterialApp(home: SelectableText.rich(TextSpan(text: 'Abcd', style: textStyle))));
