    ...
    : super(
        builder: (BuildContext context) {
          return Hero(
            tag: 'a',
            key: UniqueKey(),
            child: const Text('MutatingRoute'),
          );
        },
      );
...
Future<void> main() async {
  ...
  testWidgets('Heroes animation is fastOutSlowIn', (WidgetTester tester) async {
    ...
    expect(tester.getSize(find.byKey(secondKey)).height, moreOrLessEquals(
      curve.transform(0.25) * deltaHeight + initialHeight,
      epsilon: epsilon,
    ));

    await tester.pump(duration * 0.25);
    expect(tester.getSize(find.byKey(secondKey)).height, moreOrLessEquals(
      curve.transform(0.50) * deltaHeight + initialHeight,
      epsilon: epsilon,
    ));

    await tester.pump(duration * 0.25);
    expect(tester.getSize(find.byKey(secondKey)).height, moreOrLessEquals(
      curve.transform(0.75) * deltaHeight + initialHeight,
      epsilon: epsilon,
    ));

    await tester.pump(duration * 0.25);
    expect(tester.getSize(find.byKey(secondKey)).height, moreOrLessEquals(
      curve.transform(1.0) * deltaHeight + initialHeight,
      epsilon: epsilon,
    ));
  });
  ...
  testWidgets('Heroes are not interactive', (WidgetTester tester) async {
      ...
      home: Center(
        child: Hero(
          ...
          child: GestureDetector(
            ...
            child:
                const SizedBox(width: 100.0, height: 100.0, child: Text('foo')),
          ),
        ),
      ),
    ...
    expect(log, isEmpty);
    await tester.tap(
      find.text('foo', skipOffstage: false),
      warnIfMissed: false,
    );
    expect(log, isEmpty);
    ...
    expect(log, isEmpty);
    await tester.tap(
      find.text('bar', skipOffstage: false),
      warnIfMissed: false,
    );
    expect(log, isEmpty);
      ...
      await tester.pumpWidget(MaterialApp(
        onGenerateRoute: (RouteSettings settings) {
          return MaterialPageRoute<void>(
            settings: settings,
            builder:
                (BuildContext context) => Hero(tag: 'test', child: Container()),
          );
        },
      ));
      await tester.pump();
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(last.toStringDeep(), equalsIgnoringHashCodes(
  ...
  testWidgets(
    'Hero push transition interrupted by a pop',
    (WidgetTester tester) async {
      await tester.pumpWidget(MaterialApp(routes: routes));
      ...
      expect(find.descendant(
        of: find.byKey(homeRouteKey),
        matching: find.byKey(firstKey),
      ), findsNothing);
      expect(find.descendant(
        of: find.byKey(routeTwoKey),
        matching: find.byKey(secondKey),
      ), findsNothing);
      expect(find.byKey(firstKey), findsNothing);
      expect(find.byKey(secondKey), isOnstage);
              ...
              children: <Widget>[
                const Card(
                  child: Hero(
                    tag: 'H',
                    child:
                        SizedBox(key: homeHeroKey, height: 100.0, width: 100.0),
                  ),
                ),
                TextButton(
                  child: const Text('PUSH'),
            ...
            children: <Widget>[
              ...
              Container(
                ...
                child: const Hero(
                  tag: 'H',
                  child:
                      SizedBox(key: routeHeroKey, height: 200.0, width: 200.0),
                ),
              ),
              TextButton(
                child: const Text('POP'),
  ...
  testWidgets('Hero createRectTween for Navigator that is not full screen', (
    ...
    await tester.pump();
    expect(
      tester.getCenter(find.byKey(secondKey)),
  ...
  testWidgets(
    'Can override flight shuttle in to hero',
    (WidgetTester tester) async {
      await tester.pumpWidget(MaterialApp(
        home: Material(
          child: ListView(
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        find.byKey(nestedRouteHeroBottom, skipOffstage: false),
        findsOneWidget,
      );
      ...
      expect(
        find.byKey(nestedRouteHeroTop, skipOffstage: false),
        findsOneWidget,
      );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        find.byKey(nestedRouteHeroOne, skipOffstage: false),
        findsOneWidget,
      );
      ...
      expect(
        find.byKey(nestedRouteHeroTwo, skipOffstage: false),
        findsOneWidget,
      );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pump();
      final RenderImage renderImage = tester.renderObject(find.descendant(
        of: find.byKey(imageKey1),
    ...
    navigatorKey.currentState?.push(MaterialPageRoute<void>(
      builder: (BuildContext context) {
        return const Scaffold(
          body: Center(child: Hero(tag: 'a', child: Placeholder())),
        );
      },
    ));
    await tester.pumpAndSettle();
