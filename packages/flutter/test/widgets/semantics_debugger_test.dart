...
void main() {
  ...
  testWidgets('SemanticsDebugger scroll test', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(Directionality(
      textDirection: TextDirection.ltr,
      child: SemanticsDebugger(
        child: ListView(children: <Widget>[Container(key: childKey, height: 5000.0, color: Colors.green[500])]),
      ),
    ));
  ...
  testWidgets('SemanticsDebugger checkbox message', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(Directionality(
      ...
      child: SemanticsDebugger(
        ...
        child: Material(
          child: ListView(
            children: <Widget>[
              Semantics(container: true, key: checkbox, child: Checkbox(value: true, onChanged: (bool? _) {})),
              Semantics(
                container: true,
                key: checkboxUnchecked,
                child: Checkbox(value: false, onChanged: (bool? _) {}),
              ),
              Semantics(container: true, key: checkboxDisabled, child: const Checkbox(value: true, onChanged: null)),
              Semantics(
                container: true,
    ...
    expect(_getMessageShownInSemanticsDebugger(widgetKey: checkbox, debuggerKey: debugger, tester: tester), 'checked');
    expect(
  ...
  testWidgets('SemanticsDebugger ignores duplicated label and tooltip for Android', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(Directionality(
      ...
      child: SemanticsDebugger(
        key: debugger,
        child: Material(child: Semantics(container: true, key: child, label: 'text', tooltip: 'text')),
      ),
    ));
  ...
  testWidgets('SemanticsDebugger textfield', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(home: SemanticsDebugger(key: debugger, child: Material(child: TextField(key: textField)))),
    );
...
dynamic _getSemanticsDebuggerPainter({required Key debuggerKey, required WidgetTester tester}) {
  final CustomPaint customPaint = tester.widgetList(
