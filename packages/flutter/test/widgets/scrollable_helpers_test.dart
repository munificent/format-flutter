...
void main() {
  group('ScrollableDetails', () {
    ...
    test('copyWith / == / hashCode', () {
      ...
      expect(copiedDetails, ScrollableDetails(
        direction: AxisDirection.left,
        controller: controller,
        physics: const ClampingScrollPhysics(),
        decorationClipBehavior: Clip.none,
    ...
    test('toString', () {
      ...
      expect(fullDetails.toString(), equalsIgnoringHashCodes(
  ...
  testWidgets('Horizontal scrollables are scrolled when activated via keyboard.', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(
      ...
      home: CustomScrollView(
        ...
        slivers: List<Widget>.generate(20, (int index) {
          return SliverToBoxAdapter(
            child: Focus(autofocus: index == 0, child: SizedBox(key: ValueKey<String>('Box $index'), width: 50.0)),
          );
        }),
      ),
    ));
  ...
  testWidgets('Reversed vertical scrollables are scrolled when activated via keyboard.', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(
      ...
      home: CustomScrollView(
        ...
        slivers: List<Widget>.generate(20, (int index) {
          return SliverToBoxAdapter(
            child: Focus(focusNode: focusNode, child: SizedBox(key: ValueKey<String>('Box $index'), height: 50.0)),
          );
        }),
      ),
    ));
  ...
  testWidgets('Reversed horizontal scrollables are scrolled when activated via keyboard.', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(
      ...
      home: CustomScrollView(
        ...
        slivers: List<Widget>.generate(20, (int index) {
          return SliverToBoxAdapter(
            child: Focus(focusNode: focusNode, child: SizedBox(key: ValueKey<String>('Box $index'), width: 50.0)),
          );
        }),
      ),
    ));
          ...
          slivers: items.map<Widget>((String item) {
            return SliverToBoxAdapter(
              ...
              child: Focus(
                ...
                child:
                    Container(key: ValueKey<String>(item), alignment: Alignment.center, height: 100, child: Text(item)),
              ),
            );
          }).toList(),
        ),
