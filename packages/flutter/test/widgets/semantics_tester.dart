...
class TestSemantics {
  ...
  bool _matches(
    ...
    final int childrenCount = node.mergeAllDescendantsIntoThisNode
        ? 0
        : node.childrenCount;
  ...
  String toString([int indentAmount = 0]) {
    ...
    if (flags is int && flags != 0 ||
      ...
      buf.writeln(
        '$indent  flags: ${SemanticsTester._flagsToSemanticsFlagExpression(
          flags,
        )},',
      );
    }
    if (actions is int && actions != 0 ||
      ...
      buf.writeln(
        '$indent  actions: ${SemanticsTester._actionsToSemanticsActionExpression(
          actions,
        )},',
      );
    }
...
class SemanticsTester {
  ...
  static String _flagsToSemanticsFlagExpression(dynamic flags) {
    ...
    if (flags is int) {
      list = SemanticsFlag.values.where(
        (SemanticsFlag flag) => (flag.index & flags) != 0,
      );
    } else {
  ...
  static String _tagsToSemanticsTagExpression(Set<SemanticsTag> tags) {
    return '<SemanticsTag>[${tags.map<String>(
      (SemanticsTag tag) => "const SemanticsTag('${tag.name}')",
    ).join(', ')}]';
  }
...
class _IncludesNodeWith extends Matcher {
  ...
  Description describeMismatch(
    ...
    return mismatchDescription.add(
      'could not find node with $_configAsString.\n$_matcherHelp',
    );
  }
