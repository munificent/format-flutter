...
void main() {
  ...
  testWidgets('MediaQuery.fromView updates on notifications (with parent data)', (WidgetTester tester) async {
    ...
    int rebuildCount = 0;
    await tester.pumpWidget(MediaQuery(
      data: const MediaQueryData(textScaleFactor: 44, platformBrightness: Brightness.dark, accessibleNavigation: true),
      child: MediaQuery.fromView(
        view: tester.view,
        child: Builder(
          builder: (BuildContext context) {
  ...
  testWidgets('MediaQuery.copyWith copies specified values', (WidgetTester tester) async {
    ...
    const List<DisplayFeature> customDisplayFeatures = <DisplayFeature>[
      DisplayFeature(bounds: Rect.zero, type: DisplayFeatureType.cutout, state: DisplayFeatureState.unknown),
    ];
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(subScreenMediaQuery.viewPadding, const EdgeInsets.only(top: 6.0, left: 4.0, right: 8.0, bottom: 12.0));
      expect(subScreenMediaQuery.viewInsets, const EdgeInsets.only(top: 5.0, right: 6.0, bottom: 8.0));
      expect(subScreenMediaQuery.alwaysUse24HourFormat, true);
      expect(subScreenMediaQuery.accessibleNavigation, true);
      expect(subScreenMediaQuery.invertColors, true);
      expect(subScreenMediaQuery.disableAnimations, true);
  ...
  testWidgets('MediaQuery can be partially depended-on', (WidgetTester tester) async {
    MediaQueryData data = const MediaQueryData(size: Size(800, 600), textScaleFactor: 1.1);
  ...
  testWidgets(
    ...
    variant: _MediaQueryAspectVariant(
      values: <_MediaQueryAspectCase>[
        ...
        const _MediaQueryAspectCase(MediaQuery.displayFeaturesOf, MediaQueryData(
          displayFeatures: <DisplayFeature>[
            DisplayFeature(bounds: Rect.zero, type: DisplayFeatureType.unknown, state: DisplayFeatureState.unknown),
          ],
        )),
        const _MediaQueryAspectCase(MediaQuery.maybeDisplayFeaturesOf, MediaQueryData(
          displayFeatures: <DisplayFeature>[
            DisplayFeature(bounds: Rect.zero, type: DisplayFeatureType.unknown, state: DisplayFeatureState.unknown),
          ],
        )),
