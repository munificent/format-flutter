...
void main() {
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(Directionality(
        textDirection: TextDirection.ltr,
        child:
            Overlay(key: overlayKey, initialEntries: <OverlayEntry>[rootEntry]),
      ));
  ...
  testWidgets('OverlayEntries do not rebuild when opaqueness changes', (
    ...
    expect(
      tester.state<StatefulTestState>(
        find.byKey(bottom, skipOffstage: false),
      ).rebuildCount,
      1,
    );
    expect(tester.state<StatefulTestState>(find.byKey(middle)).rebuildCount, 1);
  ...
  testWidgets('OverlayEntries do not rebuild when opaque entry is added', (
    ...
    await tester.pumpWidget(Directionality(
      ...
      child: Overlay(
        key: overlayKey,
        initialEntries: <OverlayEntry>[bottomEntry, topEntry],
      ),
    ));
    ...
    overlayKey.currentState!
        .rearrange(<OverlayEntry>[bottomEntry, middleEntry, topEntry]);
    await tester.pump();
  ...
  testWidgets(
    'Can use Positioned within OverlayEntry',
    (WidgetTester tester) async {
      await tester.pumpWidget(Directionality(
        textDirection: TextDirection.ltr,
        child: Overlay(
      ...
      expect(
        tester.getTopLeft(find.text('positioned child')),
  ...
  group('LookupBoundary', () {
    testWidgets(
      'hides Overlay from Overlay.maybeOf',
      (WidgetTester tester) async {
    ...
    testWidgets(
      'hides Overlay from debugCheckHasOverlay',
      (WidgetTester tester) async {
        await tester.pumpWidget(Directionality(
          textDirection: TextDirection.ltr,
          child: Overlay(
