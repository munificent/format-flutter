...
void main() {
  testWidgets('Flings on different platforms', (WidgetTester tester) async {
    ...
    ); // Android is slipperier than macOS
    expect(linuxResult, lessThan(iOSResult)); // iOS is slipperier than Linux
    expect(
      macOSResult,
      lessThan(linuxResult),
    ); // Linux is slipperier than macOS
    expect(
      windowsResult,
      lessThan(iOSResult),
    ); // iOS is slipperier than Windows
    expect(
      macOSResult,
      lessThan(windowsResult),
    ); // Windows is slipperier than macOS
  ...
  testWidgets('fling and tap to stop', (WidgetTester tester) async {
    ...
    await tester.tap(
      find.byType(Scrollable),
    ); // should stop the fling but not tap anything
    await tester.pump(const Duration(milliseconds: 50));
    expect(log, equals(<String>['tap 21']));
    await tester.tap(find.byType(Scrollable));
    await tester.pump(const Duration(milliseconds: 50));
