...
void main() {
  ...
  testWidgets('Check onstage/offstage handling around transitions', (
    ...
    expect(
      state(skipOffstage: false),
      equals('BDEF'),
    ); // F is offstage since we're at 0.0

    await tester.pump(kFourTenthsOfTheTransitionDuration);
    expect(
      state(),
      equals('BCEF'),
    ); // transition 1<-2 is at 0.2, 1->3 is at 0.4
    expect(
      state(skipOffstage: false),
      equals('BCEF'),
    ...
    ); // notice that dT=0.2 here, not 0.4
    ...
    expect(
      state(),
      equals('BCF'),
    ); // transition 1<-3 is at 0.2, 1->4 is not yet built
    await tester.pump();
    ...
    expect(
      state(skipOffstage: false),
      equals('G'),
    ); // route 1 is not around any more
  });
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        tester.widget<ModalBarrier>(find.byType(ModalBarrier).first).color,
        isNull,
      );
      expect(
      ...
      expect(find.byType(ModalBarrier), findsOneWidget);
      expect(
        tester.widget<ModalBarrier>(find.byType(ModalBarrier)).color,
