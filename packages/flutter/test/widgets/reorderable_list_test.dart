...
void main() {
  ...
  testWidgets('SliverReorderableList item has correct semantics', (WidgetTester tester) async {
    ...
    expect(semantics, includesNodeWith(label: 'item 0', actions: <SemanticsAction>[SemanticsAction.customAction]));
        ...
        builder: (BuildContext outerContext, StateSetter setState) {
          return CustomScrollView(
            slivers: <Widget>[
              ...
              SliverFixedExtentList(itemExtent: 50.0, delegate: SliverChildListDelegate(<Widget>[const Text('after')])),
            ],
          );
        },
      ),
  ...
  testWidgets('SliverReorderableList, items inherit DefaultTextStyle, IconTheme', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(TestList(items: List<int>.from(<int>[0]), textColor: textColor, iconColor: iconColor));
    expect(tester.getTopLeft(find.text('item 0')), Offset.zero);
                ...
                child: Column(
                  children: <Widget>[
                    ListView(
                      ...
                      children: const <Widget>[Text('Other data'), Text('Other data'), Text('Other data')],
                    ),
                  ],
                ),
              );
      ...
      home: ReorderableList(
        ...
        itemBuilder: (BuildContext context, int index) {
          return SizedBox(
            key: ValueKey<int>(items[index]),
            height: 100,
            child: ReorderableDelayedDragStartListener(index: index, child: Text('item ${items[index]}')),
          );
        },
        onReorder: handleReorder,
        onReorderStart: (int index) {
        ...
        home: ReorderableList(
          ...
          itemBuilder: (BuildContext context, int index) {
            return SizedBox(
              key: ValueKey<int>(items[index]),
              height: 100,
              child: ReorderableDragStartListener(index: index, enabled: false, child: Text('item ${items[index]}')),
            );
          },
          onReorder: handleReorder,
        ),
