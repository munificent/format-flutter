...
void main() {
  group(ActionDispatcher, () {
    testWidgets(
      'ActionDispatcher invokes actions when asked.',
  ...
  group(Actions, () {
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        final GlobalKey containerKey = GlobalKey();
        final ActionDispatcher testDispatcher = TestDispatcher1(
          postInvoke: collect,
        );
    ...
    testWidgets('Action can be found with find', (WidgetTester tester) async {
      ...
      await tester.pump();
      expect(
        Actions.find<TestIntent>(containerKey.currentContext!),
        equals(testAction),
      );
      expect(
      ...
      expect(
        Actions.maybeFind<DoNothingIntent>(containerKey.currentContext!),
        isNull,
      );
  ...
  group('Action subclasses', () {
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        expect(DefaultToKeyEventResultAction(consumesKey: false)
            .toKeyEventResult(
              const DefaultToKeyEventResultIntent(),
              null,
            ), KeyEventResult.skipRemainingHandlers);
        expect(DefaultToKeyEventResultAction(consumesKey: true)
            .toKeyEventResult(
              const DefaultToKeyEventResultIntent(),
              null,
        ...
        builder: (BuildContext context1) {
          return Actions(
            actions: <Type, Action<Intent>>{
              LogIntent: Action<LogIntent>.overridable(
                defaultAction:
                    LogInvocationAction(actionName: 'action1', enabled: false),
                context: context1,
              ),
            },
            child: Builder(
