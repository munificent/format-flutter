...
void main() {
  ...
  testWidgets('showOnScreen works in scrollable', (WidgetTester tester) async {
    ...
    final List<Widget> containers = List<Widget>.generate(
      ...
      (int i) => MergeSemantics(
        child: SizedBox(height: kItemHeight, child: Text('container $i', textDirection: TextDirection.ltr)),
      ),
    );

    final ScrollController scrollController = ScrollController(initialScrollOffset: kItemHeight / 2);
  ...
  testWidgets('showOnScreen works with pinned app bar and individual slivers', (WidgetTester tester) async {
    ...
    final ScrollController scrollController = ScrollController(initialScrollOffset: 2.5 * kItemHeight);
  ...
  testWidgets('Semantics tree is populated mid-scroll', (WidgetTester tester) async {
    ...
    final List<Widget> children = List<Widget>.generate(80, (int i) => SizedBox(height: 40.0, child: Text('Item $i')));
    await tester.pumpWidget(Directionality(textDirection: TextDirection.ltr, child: ListView(children: children)));
  ...
  group('showOnScreen', () {
    ...
    setUp(() {
      ...
      widgetUnderTest = Directionality(
        textDirection: TextDirection.ltr,
        child: Center(
          child: SizedBox(height: 2 * kItemHeight, child: ListView(controller: scrollController, children: children)),
        ),
      );
    });
    ...
    setUp(() {
      ...
      children = List<Widget>.generate(10, (int i) {
        return SliverToBoxAdapter(
          key: i == 5 ? center : null,
          child:
              MergeSemantics(key: ValueKey<int>(i), child: SizedBox(height: kItemHeight, child: Text('container $i'))),
        );
      });
        ...
        child: Center(
          child: SizedBox(
            ...
            child: Scrollable(
              controller: scrollController,
              viewportBuilder: (BuildContext context, ViewportOffset offset) {
                return Viewport(cacheExtent: 0.0, offset: offset, center: center, slivers: children);
              },
            ),
          ),
        ),
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(Directionality(
        ...
        child: SingleChildScrollView(
          child: ListView(shrinkWrap: true, children: <Widget>[for (int i = 0; i < 50; ++i) Text('$i')]),
        ),
      ));
...
Future<void> flingUp(WidgetTester tester, {int repetitions = 1}) => fling(
  tester,
  const Offset(0.0, -200.0),
  repetitions,
);

Future<void> flingDown(WidgetTester tester, {int repetitions = 1}) => fling(
  tester,
  const Offset(0.0, 200.0),
  repetitions,
);
