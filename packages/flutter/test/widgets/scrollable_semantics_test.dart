...
void main() {
  ...
  testWidgets('scrollable exposes the correct semantic actions', (
    ...
    expect(
      semantics,
      includesNodeWith(actions: <SemanticsAction>[SemanticsAction.scrollDown]),
    );
  ...
  testWidgets('showOnScreen works with pinned app bar and sliver list', (
    ...
    await tester.pump(const Duration(seconds: 5));
    expect(
      tester.getTopLeft(find.byWidget(containers.first)).dy,
    ...
    (WidgetTester tester) async {
      ...
      final List<Widget> slivers = List<Widget>.generate(30, (int i) {
        final Widget child = MergeSemantics(
          child: SizedBox(height: 72.0, child: Text('Item $i')),
        );
        children.add(child);
  ...
  group('showOnScreen with negative children', () {
    ...
    testWidgets('brings item above leading edge to leading edge', (
      ...
      final int firstContainerId = tester.renderObject(
    ...
    testWidgets('brings item below trailing edge to trailing edge', (
      ...
      final int firstContainerId = tester.renderObject(
        find.byKey(const ValueKey<int>(4)),
      ).debugSemantics!.id;
      tester.binding.pipelineOwner.semanticsOwner!.performAction(
        firstContainerId,
        SemanticsAction.showOnScreen,
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final SemanticsNode rootScrollNode = semantics.nodesWith(
        actions: <SemanticsAction>[SemanticsAction.scrollUp],
      ).single;
      final SemanticsNode innerListPane = semantics.nodesWith(
        ancestor: rootScrollNode,
        scrollExtentMax: 0,
      ).single;
      final SemanticsNode outerListPane = innerListPane.parent!;
      final List<SemanticsNode> hiddenNodes = semantics.nodesWith(
        flags: <SemanticsFlag>[SemanticsFlag.isHidden],
      ).toList();
