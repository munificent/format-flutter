...
void main() {
  testWidgets('Align smoke test', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      const Directionality(textDirection: TextDirection.rtl, child: Align(alignment: AlignmentDirectional.topStart)),
    );
    await tester.pumpWidget(const Align(alignment: Alignment.topLeft));
  });
  ...
  testWidgets('Align control test (LTR)', (WidgetTester tester) async {
    await tester.pumpWidget(
      const Directionality(
        ...
        child: Align(alignment: AlignmentDirectional.topStart, child: SizedBox(width: 100.0, height: 80.0)),
      ),
    );
  ...
  testWidgets('Shrink wraps in finite space', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      SingleChildScrollView(child: Align(key: alignKey, child: const SizedBox(width: 10.0, height: 10.0))),
    );
    ...
    await tester.pumpWidget(
      const Directionality(
        ...
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: <Widget>[Align(heightFactor: 0.5, child: SizedBox(height: 100.0, width: 100.0))],
        ),
      ),
    );
    final RenderBox box = tester.renderObject<RenderBox>(find.byType(Align));
