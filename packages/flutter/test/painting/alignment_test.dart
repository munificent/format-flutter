...
void main() {
  ...
  test('Alignment.lerp()', () {
    ...
    expect(
      Alignment.lerp(a, null, 0.25),
      equals(const Alignment(-0.75, -0.75)),
    );
  });
  ...
  test('Alignment.lerp identical a,b', () {
    ...
    expect(
      identical(Alignment.lerp(alignment, alignment, 0.5), alignment),
      true,
    );
  });
  ...
  test('AlignmentDirectional.lerp identical a,b', () {
    ...
    expect(identical(
      AlignmentDirectional.lerp(alignment, alignment, 0.5),
      alignment,
    ), true);
  });
  ...
  test('AlignmentGeometry invariants', () {
    ...
    for (final TextDirection x in TextDirection.values) {
      ...
      expect(
        (topEnd * 0.0).add(topLeft).resolve(x),
  ...
  test('AlignmentGeometry.resolve()', () {
    ...
    expect(
      AlignmentDirectional.bottomEnd.resolve(TextDirection.ltr),
      Alignment.bottomRight,
    );
    expect(
      AlignmentDirectional.bottomEnd.resolve(TextDirection.rtl),
  ...
  test('AlignmentGeometry.lerp ad hoc tests', () {
    final AlignmentGeometry mixed1 = const Alignment(10.0, 20.0).add(
      const AlignmentDirectional(30.0, 50.0),
    );
    final AlignmentGeometry mixed2 = const Alignment(70.0, 110.0).add(
      const AlignmentDirectional(130.0, 170.0),
    );
    final AlignmentGeometry mixed3 = const Alignment(25.0, 42.5).add(
      const AlignmentDirectional(55.0, 80.0),
    );
  ...
  test('lerp commutes with resolve', () {
    final List<AlignmentGeometry?> offsets = <AlignmentGeometry?>[
      ...
      const Alignment(0.0625, 0.5625).add(
        const AlignmentDirectional(0.1875, 0.6875),
      ),
      ...
      const Alignment(70.0, 110.0).add(
        const AlignmentDirectional(130.0, 170.0),
      ),
    ...
    for (final TextDirection direction in TextDirection.values) {
      final Alignment defaultValue = AlignmentDirectional.center.resolve(
        direction,
      );
      for (final AlignmentGeometry? a in offsets) {
        ...
        for (final AlignmentGeometry? b in offsets) {
          ...
          approxExpect((AlignmentGeometry.lerp(a, b, 0.0) ?? defaultValue)
              .resolve(direction), resolvedA);
          approxExpect((AlignmentGeometry.lerp(a, b, 1.0) ?? defaultValue)
              .resolve(direction), resolvedB);
          for (final double t in times) {
            assert(t > 0.0);
            assert(t < 1.0);
            final Alignment value = (AlignmentGeometry.lerp(a, b, t) ??
                    defaultValue)
                .resolve(direction);
  ...
  test('AlignmentGeometry add/subtract', () {
    ...
    expect(
      directional.add(normal).resolve(TextDirection.ltr),
      const Alignment(4.0, 7.0),
    );
    expect(
      directional.add(normal).resolve(TextDirection.rtl),
  ...
  test('AlignmentGeometry operators', () {
    expect(
      const AlignmentDirectional(1.0, 2.0) * 2.0,
  ...
  test('AlignmentGeometry operators', () {
    expect(
      const Alignment(1.0, 2.0) + const Alignment(3.0, 5.0),
      const Alignment(4.0, 7.0),
    );
    expect(
      const Alignment(1.0, 2.0) - const Alignment(3.0, 5.0),
