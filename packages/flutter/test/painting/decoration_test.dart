...
void main() {
  ...
  test('Decoration.lerp()', () {
    ...
    expect(c.color, equals(
      Color.lerp(const Color(0xFFFFFFFF), const Color(0x00000000), 0.25),
    ));
  ...
  test('Decoration.lerp identical a,b', () {
    ...
    expect(
      identical(Decoration.lerp(decoration, decoration, 0.5), decoration),
      true,
    );
  });
  ...
  test(
    'BoxDecorationImageListener does not change when image is clone',
    () async {
      ...
      final MultiFrameImageProvider imageProvider = MultiFrameImageProvider(
        completer,
      );
      final DecorationImage backgroundImage = DecorationImage(
        image: imageProvider,
  ...
  test('DecorationImage test', () async {
    const ColorFilter colorFilter = ui.ColorFilter.mode(
      Color(0xFF00FF00),
      BlendMode.src,
    );
    final ui.Image image = await createTestImage(width: 100, height: 100);
  ...
  test(
    ...
    () async {
      ...
      expect(
        error.diagnostics[3],
        isA<DiagnosticsProperty<ImageConfiguration>>(),
      );
      expect(
        error.toStringDeep(),
        'FlutterError\n'
  ...
  test('paintImage BoxFit.none scale test', () async {
    for (double scale = 1.0; scale <= 4.0; scale += 1.0) {
      ...
      final Rect expectedTileRect = Rect.fromPoints(outputRect.bottomRight
          .translate(
            -expectedTileSize.width,
            -expectedTileSize.height,
  ...
  test('paintImage with repeatX and fitHeight', () async {
    ...
    final List<Invocation> calls = canvas.invocations.where(
      (Invocation call) => call.memberName == #drawImageRect,
    ).toList();
  ...
  test('DecorationImage scale test', () async {
    ...
    expect(
      call.positionalArguments[2],
      const Rect.fromLTRB(0.0, 0.0, 25.0, 25.0),
    );
  });

  test('DecorationImagePainter disposes of image when disposed', () async {
    final ImageProvider provider = MemoryImage(
    ...
    final DecorationImagePainter painter = DecorationImage(image: provider)
        .createPainter(() {});
