...
void main() {
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        Directionality(textDirection: TextDirection.ltr, child: WidgetToRenderBoxAdapter(renderBox: child)),
      );
      const Map<String, dynamic> data = <String, dynamic>{'type': 'fontsChange'};
      await tester.binding.defaultBinaryMessenger.handlePlatformMessage(
        'flutter/system',
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      await tester.pumpWidget(CupertinoApp(home: CupertinoDatePicker(onDateTimeChanged: (DateTime dateTime) {})));
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      await tester.pumpWidget(CupertinoApp(
        home: CupertinoDatePicker(mode: CupertinoDatePickerMode.date, onDateTimeChanged: (DateTime dateTime) {}),
      ));
      final dynamic state = tester.state(find.byType(CupertinoDatePicker));
      // ignore: avoid_dynamic_calls
      final Map<int, double> cache = state.estimatedColumnWidths as Map<int, double>;
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      await tester.pumpWidget(CupertinoApp(home: CupertinoTimerPicker(onTimerDurationChanged: (Duration d) {})));
  ...
  testWidgets('RangeSlider relayout upon system fonts changes', (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
      home: Material(child: RangeSlider(values: const RangeValues(0.0, 1.0), onChanged: (RangeValues values) {})),
    ));
    const Map<String, dynamic> data = <String, dynamic>{'type': 'fontsChange'};
    await tester.binding.defaultBinaryMessenger.handlePlatformMessage(
      'flutter/system',
    ...
    final RenderObject renderObject = tester.renderObject(
      find.byWidgetPredicate((Widget widget) => widget.runtimeType.toString() == '_RangeSliderRenderObjectWidget'),
    );
    await verifyMarkedNeedsLayoutDuringTransientCallbacksPhase(tester, renderObject);
  });

  testWidgets('Slider relayout upon system fonts changes', (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(home: Material(child: Slider(value: 0.0, onChanged: (double value) {}))));
  ...
  testWidgets('TimePicker relayout upon system fonts changes', (WidgetTester tester) async {
                ...
                onPressed: () {
                  showTimePicker(
                    ...
                    builder: (BuildContext context, Widget? child) {
                      return Directionality(key: const Key('parent'), textDirection: TextDirection.ltr, child: child!);
                    },
                  );
                },
              );
    ...
    final RenderObject renderObject = tester.renderObject(find.descendant(
