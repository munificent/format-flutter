  ...
  group('ResizeImage', () {
    group('resizing', () {
      test('upscales to the correct dimensions', () async {
        ...
        const ImageConfiguration resizeConfig = ImageConfiguration(
          size: resizeDims,
        );
      ...
      group('with policy=fit and allowResize=false', () {
        test('constrains square image to bounded portrait rect', () async {
          final MemoryImage rawImage = MemoryImage(
        ...
        test('constrains portrait image to bounded portrait rect', () async {
          final MemoryImage rawImage = MemoryImage(
            Uint8List.fromList(kBluePortraitPng),
          );
        ...
        test('constrains landscape image to bounded portrait rect', () async {
          final MemoryImage rawImage = MemoryImage(
            Uint8List.fromList(kBlueLandscapePng),
          );
        ...
        test(
          'leaves image as-is if constraints are bigger than image',
          () async {
            final MemoryImage rawImage = MemoryImage(
              Uint8List.fromList(kBlueSquarePng),
            );
