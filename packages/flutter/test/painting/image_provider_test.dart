...
void main() {
  ...
  test('File image with empty file throws expected error (load)', () async {
    ...
    expect(provider.loadBuffer(provider, (
  ...
  test('Asset image sets tag', () async {
    ...
    final ExactAssetImage provider = ExactAssetImage(
      asset,
  ...
  test('Resize image sets tag', () async {
    final Uint8List bytes = Uint8List.fromList(kBlueSquarePng);
    final ResizeImage provider = ResizeImage(
      MemoryImage(bytes),
      width: 40,
      height: 40,
    );
  ...
  test(
    'File image throws error when given a real but non-image file',
    () async {
      ...
      expect(await error.future, isException.having(
