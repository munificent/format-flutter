...
void main() {
  ...
  test(
    ...
    () {
      ...
      expect(caretOffset.dx, 42); // 👦
      caretOffset = painter.getOffsetForCaret(
        const ui.TextPosition(offset: 8),
        ui.Rect.zero,
      );
      expect(caretOffset.dx, 42); // 👩‍
      caretOffset = painter.getOffsetForCaret(
        const ui.TextPosition(offset: 9),
        ui.Rect.zero,
      );
      expect(caretOffset.dx, 98); // 👩‍
      caretOffset = painter.getOffsetForCaret(
        const ui.TextPosition(offset: 10),
        ui.Rect.zero,
      );
      ...
      expect(caretOffset.dx, 98); // 👩‍
      caretOffset = painter.getOffsetForCaret(
        const ui.TextPosition(offset: 13),
        ui.Rect.zero,
      );
      expect(caretOffset.dx, 98); // <zwj>
      caretOffset = painter.getOffsetForCaret(
        const ui.TextPosition(offset: 14),
        ui.Rect.zero,
      );
      expect(caretOffset.dx, 98); // 👧‍
      caretOffset = painter.getOffsetForCaret(
        const ui.TextPosition(offset: 15),
        ui.Rect.zero,
      );
      expect(caretOffset.dx, 98); // 👧‍
      caretOffset = painter.getOffsetForCaret(
        const ui.TextPosition(offset: 16),
        ui.Rect.zero,
      );
      expect(caretOffset.dx, 98); // <zwj>
      caretOffset = painter.getOffsetForCaret(
        const ui.TextPosition(offset: 17),
        ui.Rect.zero,
      );
      expect(caretOffset.dx, 98); // 👧
      caretOffset = painter.getOffsetForCaret(
        const ui.TextPosition(offset: 18),
        ui.Rect.zero,
      );
      expect(caretOffset.dx, 98); // 👧
      caretOffset = painter.getOffsetForCaret(
        const ui.TextPosition(offset: 19),
        ui.Rect.zero,
      );
      expect(caretOffset.dx, 98); // 👏
      caretOffset = painter.getOffsetForCaret(
        const ui.TextPosition(offset: 20),
        ui.Rect.zero,
      );
      expect(caretOffset.dx, 98); // 👏
      caretOffset = painter.getOffsetForCaret(
        const ui.TextPosition(offset: 21),
        ui.Rect.zero,
      );
      expect(caretOffset.dx, 98); // <medium skin tone modifier>
      caretOffset = painter.getOffsetForCaret(
        const ui.TextPosition(offset: 22),
        ui.Rect.zero,
      );
      expect(caretOffset.dx, 98); // <medium skin tone modifier>
      caretOffset = painter.getOffsetForCaret(
        const ui.TextPosition(offset: 23),
        ui.Rect.zero,
      );
  ...
  test(
    'TextPainter caret emoji tests: single, long emoji',
    () {
  ...
  test(
    'TextPainter caret Devanagari test',
    () {
  ...
  test('TextPainter error test', () {
    ...
    expect(() => painter.paint(MockCanvas(), Offset.zero), throwsA(
  ...
  test('TextPainter handles newlines properly', () {
    ...
    expect(
      caretOffset.dx,
      moreOrLessEquals(SIZE_OF_A * offset, epsilon: 0.0001),
    );
    ...
    expect(caretOffset.dx, moreOrLessEquals(0.0, epsilon: 0.0001));
    expect(caretOffset.dy, moreOrLessEquals(SIZE_OF_A, epsilon: 0.0001));
    caretOffset = painter.getOffsetForCaret(ui.TextPosition(
  ...
  group(
    ...
    () {
      test('half-leading', () {
        ...
        final RelativeRect insets = RelativeRect.fromSize(
          glyphBox,
          painter.size,
        );
  ...
  test('TextPainter throws with stack trace when accessing text layout', () {
    ...
    expect(() => painter.getPositionForOffset(Offset.zero), throwsA(
  ...
  test(
    ...
    () {
      ...
      expect(() => painter.paint(MockCanvas(), Offset.zero), isNot(throwsA(
        isA<StateError>().having(
          (StateError error) => error.message,
          'message',
          contains(
  ...
  test('TextPainter computeWidth', () {
    ...
    expect(painter.width, TextPainter.computeWidth(
      text: text,
      textDirection: TextDirection.ltr,
      minWidth: 500,
    ));
  ...
  test('TextPainter computeMaxIntrinsicWidth', () {
    ...
    painter.layout();
    expect(painter.maxIntrinsicWidth, TextPainter.computeMaxIntrinsicWidth(
      text: text,
      textDirection: TextDirection.ltr,
    ));
  ...
  test('TextPainter infinite width - centered', () {
    ...
    expect(
      () => painter.paint(
        mockCanvas = MockCanvasWithDrawParagraph(),
        Offset.zero,
      ),
      returnsNormally,
    );
