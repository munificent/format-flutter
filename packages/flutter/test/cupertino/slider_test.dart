...
void main() {
  ...
  testWidgets(
    'Slider does not move when tapped (LTR)',
  ...
  testWidgets('Slider moves when dragged (RTL)', (WidgetTester tester) async {
    ...
    const double delta = 3.0 * unit;
    await tester.dragFrom(
      bottomRight - const Offset(unit, unit),
      const Offset(-delta, 0.0),
    );
    ...
    expect(semantics, hasSemantics(TestSemantics.root(
      children: <TestSemantics>[
        TestSemantics.rootChild(
          ...
          actions:
              SemanticsAction.decrease.index | SemanticsAction.increase.index,
        ),
      ],
  ...
  testWidgets('Slider respects themes', (WidgetTester tester) async {
    await tester.pumpWidget(CupertinoApp(
      home: Center(
        child: CupertinoSlider(onChanged: (double value) {}, value: 0.5),
      ),
    ));
    expect(
  ...
  testWidgets(
    'Themes can be overridden by dynamic colors',
    (WidgetTester tester) async {
      ...
      expect(
        find.byType(CupertinoSlider),
        paints..rrect(color: activeColor.color),
      );
