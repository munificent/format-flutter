...
void main() {
  ...
  Future<void> showCupertinoMagnifier(
    ...
    final Future<void> magnifierShown = magnifierController.show(
      context: context,
      builder: (_) => CupertinoTextMagnifier(controller: magnifierController, magnifierInfo: magnifierInfo),
    );
    ...
    group('position', () {
      ...
      testWidgets(
        ...
        (WidgetTester tester) async {
          ...
          final ValueNotifier<MagnifierInfo> magnifier = ValueNotifier<MagnifierInfo>(MagnifierInfo(
          ...
          await expectLater(find.byKey(outerKey), matchesGoldenFile('cupertino_magnifier.position.default.png'));
        },
      );

      testWidgets('should never horizontally be outside of Screen Padding', (WidgetTester tester) async {
        await tester.pumpWidget(const MaterialApp(color: Color.fromARGB(7, 0, 129, 90), home: Placeholder()));

        final BuildContext context = tester.firstElement(find.byType(Placeholder));

        await showCupertinoMagnifier(context, tester, ValueNotifier<MagnifierInfo>(MagnifierInfo(
      ...
      testWidgets('should have some vertical drag', (WidgetTester tester) async {
        ...
        await showCupertinoMagnifier(context, tester, ValueNotifier<MagnifierInfo>(MagnifierInfo(
          ...
          caretRect: reasonableTextField,
          // The tap position is dragBelow units below the text field.
          globalGesturePosition: Offset(MediaQuery.sizeOf(context).width / 2, dragPositionBelowTextField),
        )));
