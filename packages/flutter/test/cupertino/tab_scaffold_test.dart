...
void main() {
  ...
  testWidgets('Tab switching', (WidgetTester tester) async {
    ...
    RichText tab1 = tester.widget(find.descendant(of: find.text('Tab 1'), matching: find.byType(RichText)));
    expect(tab1.text.style!.color, CupertinoColors.activeBlue);
    RichText tab2 = tester.widget(find.descendant(of: find.text('Tab 2'), matching: find.byType(RichText)));
        ...
        tabBuilder: (BuildContext context, int index) {
          return Column(
            children: <Widget>[
              ...
              CupertinoTextField(focusNode: focusNodes[index * 2 + 1], placeholder: 'TextField 2'),
            ],
          );
        },
      ),
  ...
  testWidgets('Tab bar respects themes', (WidgetTester tester) async {
    ...
    BoxDecoration tabDecoration = tester.widget<DecoratedBox>(
      find.descendant(of: find.byType(CupertinoTabBar), matching: find.byType(DecoratedBox)),
    ...
    await tester.pumpWidget(CupertinoApp(
      theme: const CupertinoThemeData(brightness: Brightness.dark, primaryColor: CupertinoColors.destructiveRed),
      home: CupertinoTabScaffold(
        tabBar: _buildTabBar(),
        tabBuilder: (BuildContext context, int index) {
          return const Placeholder();
    ...
    tabDecoration = tester.widget<DecoratedBox>(
      find.descendant(of: find.byType(CupertinoTabBar), matching: find.byType(DecoratedBox)),
    ).decoration as BoxDecoration;

    expect(tabDecoration.color, isSameColorAs(const Color(0xF01D1D1D)));

    final RichText tab1 = tester.widget(find.descendant(of: find.text('Tab 1'), matching: find.byType(RichText)));
    // Tab 2 should still be selected after changing theme.
    expect(tab1.text.style!.color!.value, 0xFF757575);
    final RichText tab2 = tester.widget(find.descendant(of: find.text('Tab 2'), matching: find.byType(RichText)));
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(CupertinoApp(
        home: MediaQuery(data: const MediaQueryData(viewInsets: EdgeInsets.only(bottom: 20.0)), child: child),
      ));
  ...
  testWidgets('Opaque tab bar consumes bottom padding while non opaque tab bar does not', (WidgetTester tester) async {
    ...
    Future<EdgeInsets> getContentPaddingWithTabBarColor(Color color) async {
      ...
      await tester.pumpWidget(CupertinoApp(
        home: MediaQuery(
          ...
          child: CupertinoTabScaffold(
            tabBar:
                CupertinoTabBar(backgroundColor: color, items: List<BottomNavigationBarItem>.generate(2, tabGenerator)),
            tabBuilder: (BuildContext context, int index) {
        ...
        child: Stack(
          children: <Widget>[
            CupertinoTabScaffold(
              ...
              controller: controller,
              tabBuilder: (BuildContext context, int index) {
                return CustomPaint(painter: TestCallbackPainter(onPaint: () => tabsPainted0.add(index)));
              },
            ),
            CupertinoTabScaffold(
              tabBar: CupertinoTabBar(items: List<BottomNavigationBarItem>.generate(3, tabGenerator)),
              controller: controller,
              tabBuilder: (BuildContext context, int index) {
                return CustomPaint(painter: TestCallbackPainter(onPaint: () => tabsPainted1.add(index)));
              },
            ),
          ],
        ),
  ...
  testWidgets('textScaleFactor is set to 1.0', (WidgetTester tester) async {
    ...
    final Iterable<RichText> contents = tester.widgetList<RichText>(
      find.descendant(of: find.text('content'), matching: find.byType(RichText), skipOffstage: false),
    );
