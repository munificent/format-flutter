...
void main() {
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final DefaultTextStyle helperTextStyle = tester.widget(
        find.byType(DefaultTextStyle).first,
      );

      expect(helperTextStyle.style.color, CupertinoColors.label);

      final DefaultTextStyle errorTextStyle = tester.widget(
  ...
  testWidgets('CupertinoFormRow adapts to MaterialApp dark mode', (
    WidgetTester tester,
  ) async {
    ...
    expect(
      helperParagraph.text.style!.color.toString().contains('UNRESOLVED'),
      isFalse,
    );
    ...
    // Text style should not return unresolved color.
    expect(
      prefixParagraph.text.style!.color.toString().contains('UNRESOLVED'),
      isFalse,
    );
