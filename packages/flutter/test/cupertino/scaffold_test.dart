...
void main() {
  testWidgets(
    'Contents are behind translucent bar',
  ...
  testWidgets('Contents padding from viewInsets', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(Directionality(
      ...
      child: MediaQuery(
        ...
        child: CupertinoPageScaffold(
          navigationBar:
              const CupertinoNavigationBar(middle: Text('Transparent')),
          child: Builder(
            builder: (BuildContext context) {
              childContext = context;
              return Container();
  ...
  testWidgets('Contents are between opaque bars', (WidgetTester tester) async {
    ...
    expect(
      tester.getSize(find.byWidget(page1Center)).height,
      600.0 - 44.0 - 50.0,
    );
  });
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        tester.getBottomLeft(find.byWidget(content)).dy,
        600 - 20.0 - 50.0,
      );
    },
  );
  ...
  testWidgets('iOS independent tab navigation', (WidgetTester tester) async {
    ...
    expect(
      find.text('Page 1 of tab 2'),
      findsNothing,
    ); // Lazy building so not built yet.
  ...
  testWidgets(
    'Decorated with white background by default',
    (WidgetTester tester) async {
      await tester.pumpWidget(
        const CupertinoApp(home: CupertinoPageScaffold(child: Center())),
      );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final Offset positionNoInsetWithNavBar = tester.getTopLeft(
      ...
      await tester.pumpAndSettle();
      final Offset positionWithInsetWithNavBar = tester.getTopLeft(
        find.byType(CupertinoTextField),
      );
      expect(
        positionWithInsetWithNavBar.dy,
        lessThan(positionNoInsetWithNavBar.dy),
      );

      // When there is no nav bar and no keyboard, the CupertinoTextField is still
      // centered.
      await tester.pumpWidget(buildFrame(false, false));
      final Offset positionNoInsetNoNavBar = tester.getTopLeft(
        find.byType(CupertinoTextField),
      );
      ...
      await tester.pumpAndSettle();
      final Offset positionWithInsetNoNavBar = tester.getTopLeft(
        find.byType(CupertinoTextField),
      );
      expect(
        positionWithInsetNoNavBar.dy,
        lessThan(positionNoInsetNoNavBar.dy),
      );
  ...
  testWidgets('textScaleFactor is set to 1.0', (WidgetTester tester) async {
    ...
    final Iterable<RichText> richTextList = tester.widgetList<RichText>(find
        .descendant(
    ...
    expect(
      richTextList.any((RichText text) => text.textScaleFactor != 1),
      isFalse,
    );
