...
void main() {
  group(consolidateHttpClientResponseBytes, () {
    ...
    test('Notifies onBytesReceived for every chunk of bytes', () async {
      ...
      expect(records, <int>[chunkOne.length, syntheticTotal, chunkOne.length + chunkTwo.length, syntheticTotal]);
    });
    ...
    group('when gzipped', () {
      ...
      test('Notifies onBytesReceived with gzipped numbers', () async {
        ...
        expect(records, <int>[gzippedChunkOne.length, gzipped.length, gzipped.length, gzipped.length]);
      });
