...
void main() {
  test(
    ...
    () {
      ...
      expect(() => objectTween.transform(0.1), throwsA(isA<FlutterError>()
          .having(
            (FlutterError error) => error.diagnostics.map(
              (DiagnosticsNode node) => node.toString(),
            ),
            'diagnostics',
            <String>[
  ...
  test(
    ...
    () {
      ...
      expect(() => colorTween.transform(0.1), throwsA(isA<FlutterError>()
          .having(
            (FlutterError error) => error.diagnostics.map(
              (DiagnosticsNode node) => node.toString(),
            ),
  ...
  test(
    ...
    () {
      ...
      expect(() => rectTween.transform(0.1), throwsA(isA<FlutterError>().having(
        (FlutterError error) => error.diagnostics.map(
          (DiagnosticsNode node) => node.toString(),
        ),
        'diagnostics',
  ...
  test('Can chain tweens', () {
    ...
    final Animatable<double> chain = tween.chain(
      Tween<double>(begin: 0.50, end: 1.0),
    );
  ...
  test('ReverseTween', () {
    final ReverseTween<int> tween = ReverseTween<int>(
      IntTween(begin: 5, end: 9),
    );
    expect(tween.lerp(0.5), 7);
