...
void main() {
  test('toString control test', () {
    ...
    expect(
      const Interval(0.25, 0.75, curve: Curves.ease),
  ...
  test('Invalid transform parameter should assert', () {
    ...
    expect(
      () => const Interval(0.0, 1.0).transform(-0.0001),
      throwsAssertionError,
    );
    expect(
      () => const Interval(0.0, 1.0).transform(1.0001),
      throwsAssertionError,
    );
    ...
    expect(
      () => const ElasticInCurve().transform(-0.0001),
      throwsAssertionError,
    );
    expect(
      () => const ElasticInCurve().transform(1.0001),
      throwsAssertionError,
    );

    expect(
      () => const ElasticOutCurve().transform(-0.0001),
      throwsAssertionError,
    );
    expect(
      () => const ElasticOutCurve().transform(1.0001),
      throwsAssertionError,
    );

    expect(
      () => const Cubic(0.42, 0.0, 0.58, 1.0).transform(-0.0001),
      throwsAssertionError,
    );
    expect(
      () => const Cubic(0.42, 0.0, 0.58, 1.0).transform(1.0001),
      throwsAssertionError,
    );

    expect(
      () => Curves.easeInOutCubicEmphasized.transform(-0.0001),
      throwsAssertionError,
    );
    expect(
      () => Curves.easeInOutCubicEmphasized.transform(1.0001),
      throwsAssertionError,
    );
  ...
  test('CatmullRomSpline interpolates values properly', () {
    final CatmullRomSpline curve = CatmullRomSpline(const <Offset>[
      Offset.zero,
      Offset(0.01, 0.25),
      Offset(0.2, 0.25),
      Offset(0.33, 0.25),
      ...
      Offset(0.66, 0.75),
      Offset(1.0, 1.0),
    ], startHandle: const Offset(0.0, -0.3), endHandle: const Offset(1.3, 1.3));
    ...
    expect(
      curve.transform(0.25).dx,
      moreOrLessEquals(0.0966945, epsilon: tolerance),
    );
    expect(
      curve.transform(0.25).dy,
    ...
    expect(
      curve.transform(0.75).dx,
      moreOrLessEquals(0.570260, epsilon: tolerance),
    );
    expect(
      curve.transform(0.75).dy,
  ...
  test('CatmullRomSpline interpolates values properly when precomputed', () {
    final CatmullRomSpline curve = CatmullRomSpline.precompute(const <Offset>[
      Offset.zero,
      Offset(0.01, 0.25),
      Offset(0.2, 0.25),
      Offset(0.33, 0.25),
  ...
  test(
    'CatmullRomCurve interpolates given points correctly when precomputed',
    () {
  ...
  test('CatmullRomCurve enforces contract', () {
    ...
    expect(CatmullRomCurve.validateControlPoints(const <Offset>[
      Offset(0.2, 0.25),
      Offset(0.01, 0.25),
    ]), isFalse);
    expect(() {
