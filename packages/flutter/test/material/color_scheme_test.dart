...
void main() {
  ...
  test('can override specific colors in a generated scheme', () {
    ...
    final ColorScheme scheme = ColorScheme.fromSeed(seedColor: Colors.blue, primary: primaryOverride);
  ...
  test(
    ...
    () async {
      ...
      final ColorScheme scheme = await ColorScheme.fromImageProvider(provider: image, brightness: Brightness.dark);
  ...
  test('fromImageProvider() propogates TimeoutException when image cannot be rendered', () async {
    ...
    expect(() async => ColorScheme.fromImageProvider(provider: image), throwsA(isA<Exception>().having(
