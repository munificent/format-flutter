...
void main() {
  ...
  List<ContextMenuButtonItem> buildSuggestionButtons(List<String> suggestions) {
    ...
    for (final String suggestion in suggestions) {
      buttonItems.add(ContextMenuButtonItem(onPressed: () {}, label: suggestion));
    }
  ...
  Finder findSpellCheckSuggestionsToolbar() {
    return find.descendant(
      ...
      matching: find.byWidgetPredicate((Widget w) => '${w.runtimeType}' == '_SpellCheckSuggestionsToolbarContainer'),
    );
  }
  ...
  test('buildSuggestionButtons only considers the first three suggestions', () {
    ...
    final List<ContextMenuButtonItem>? buttonItems = SpellCheckSuggestionsToolbar.buildButtonItems(editableTextState);
