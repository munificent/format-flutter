...
void main() {
  group('$ReorderableListView', () {
    ...
    Widget listItemToWidget(String listItem) {
      return SizedBox(key: Key(listItem), height: itemHeight, width: itemHeight, child: Text(listItem));
    }
    ...
    group('in vertical mode', () {
      testWidgets('reorder is not triggered when children length is less or equals to 1', (WidgetTester tester) async {
        ...
        await tester.pumpWidget(MaterialApp(home: SizedBox(height: itemHeight * 10, child: reorderableListView)));
      ...
      testWidgets('allows reordering from the very bottom to the very top', (WidgetTester tester) async {
        await tester.pumpWidget(build());
        expect(listItems, orderedEquals(originalListItems));
        await longPressDrag(tester, tester.getCenter(find.text('Item 4')), tester.getCenter(find.text('Item 1')));
        await tester.pumpAndSettle();
      ...
      testWidgets('Vertical drag in progress golden image', (WidgetTester tester) async {
        ...
        final Widget reorderableListView = ReorderableListView(
          children: <Widget>[
            Container(key: const Key('pink'), width: double.infinity, height: itemHeight, color: Colors.pink),
            Container(key: const Key('blue'), width: double.infinity, height: itemHeight, color: Colors.blue),
            Container(key: const Key('green'), width: double.infinity, height: itemHeight, color: Colors.green),
          ],
          onReorder: (int oldIndex, int newIndex) {},
        );
        ...
        // the dragged item.
        await expectLater(find.byType(Overlay).last, matchesGoldenFile('reorderable_list_test.vertical.drop_area.png'));
      ...
      testWidgets('Preserves children states when the list parent changes the order', (WidgetTester tester) async {
        _StatefulState findState(Key key) {
          ...
          ).evaluate().first.findAncestorStateOfType<_StatefulState>()!;
        }
      ...
      testWidgets('Test custom ScrollController behavior when set', (WidgetTester tester) async {
        ...
        customController.animateTo(40.0, duration: const Duration(milliseconds: 200), curve: Curves.linear);
        await tester.pumpAndSettle();
      ...
      testWidgets('Still builds when no PrimaryScrollController is available', (WidgetTester tester) async {
        ...
        final Widget overlay = Overlay(
          initialEntries: <OverlayEntry>[OverlayEntry(builder: (BuildContext context) => reorderableList)],
        );
      ...
      group('Accessibility (a11y/Semantics)', () {
        ...
        testWidgets("Doesn't hide accessibility when a child declares its own semantics", (WidgetTester tester) async {
          ...
          final Widget reorderableListView = ReorderableListView(
            onReorder: (int oldIndex, int newIndex) {},
            children: <Widget>[
              const SizedBox(key: Key('List tile 1'), height: itemHeight, child: Text('List tile 1')),
              SizedBox(
                key: const Key('Switch tile'),
                height: itemHeight,
                child: Material(
                  child: SwitchListTile(title: const Text('Switch tile'), value: true, onChanged: (bool? newValue) {}),
                ),
              ),
              const SizedBox(key: Key('List tile 2'), height: itemHeight, child: Text('List tile 2')),
            ],
          );
          await tester.pumpWidget(MaterialApp(home: SizedBox(height: itemHeight * 10, child: reorderableListView)));
    ...
    group('in horizontal mode', () {
      ...
      testWidgets('properly reorders with a header', (WidgetTester tester) async {
        ...
        expect(listItems, orderedEquals(<String>['Item 2', 'Item 3', 'Item 4', 'Item 1']));
        await tester.pumpWidget(build(header: const Text('Header Text'), scrollDirection: Axis.horizontal));
        await longPressDrag(tester, tester.getCenter(find.text('Item 4')), tester.getCenter(find.text('Item 3')));
        await tester.pumpAndSettle();
        expect(find.text('Header Text'), findsOneWidget);
      ...
      testWidgets('properly determines the horizontal drop area extents', (WidgetTester tester) async {
        ...
        await tester.pumpWidget(MaterialApp(home: SizedBox(width: itemHeight * 10, child: reorderableListView)));
      ...
      testWidgets('Horizontal drag in progress golden image', (WidgetTester tester) async {
        ...
        final Widget reorderableListView = ReorderableListView(
          ...
          onReorder: (int oldIndex, int newIndex) {},
          children: <Widget>[
            Container(key: const Key('pink'), height: double.infinity, width: itemHeight, color: Colors.pink),
            Container(key: const Key('blue'), height: double.infinity, width: itemHeight, color: Colors.blue),
            Container(key: const Key('green'), height: double.infinity, width: itemHeight, color: Colors.green),
          ],
        );
        await tester.pumpWidget(MaterialApp(
          home: Container(
          ...
          itemBuilder: (BuildContext context, int index) {
            return SizedBox(
              ...
              child: ReorderableDragStartListener(
                index: index,
                child:
                    Row(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[Text('item ${items[index]}')]),
              ),
            );
          },
        ...
        () => ReorderableListView.builder(
          itemBuilder: (BuildContext context, int index) {
            return SizedBox(
              ...
              child: ReorderableDragStartListener(index: index, child: Text(numbers[index].toString())),
            );
          },
  ...
  testWidgets('ReorderableListView auto scrolls speed is configurable', (WidgetTester tester) async {
    Future<void> pumpFor({required Duration duration, Duration interval = const Duration(milliseconds: 50)}) async {
      await tester.pump();
...
class _StatefulState extends State<_Stateful> {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      width: 48.0,
      height: 48.0,
      child: Material(child: Checkbox(value: checked, onChanged: (bool? newValue) => checked = newValue)),
    );
  }
}
