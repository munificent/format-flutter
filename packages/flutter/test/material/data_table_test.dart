...
void main() {
  testWidgets('DataTable control test', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(home: Material(child: buildTable(sortColumnIndex: 1, sortAscending: false))));
    await tester.pumpAndSettle(const Duration(milliseconds: 200));
  ...
  testWidgets('DataTable control test - tristate', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(home: Material(child: buildTable(<bool>[false, false, false]))));
    await tester.tap(find.byType(Checkbox).first);
    ...
    await tester.pumpWidget(MaterialApp(home: Material(child: buildTable(<bool>[true, false, true]))));
    await tester.tap(find.byType(Checkbox).first);
    ...
    await tester.pumpWidget(MaterialApp(home: Material(child: buildTable(<bool>[true, true, true]))));
    await tester.tap(find.byType(Checkbox).first);
  ...
  testWidgets('DataTable sort indicator orientation', (WidgetTester tester) async {
    Widget buildTable({bool sortAscending = true}) {
      return DataTable(
        ...
        sortAscending: sortAscending,
        columns: <DataColumn>[
          DataColumn(label: const Text('Name'), tooltip: 'Name', onSort: (int columnIndex, bool ascending) {}),
        ],
        rows: kDesserts.map<DataRow>((Dessert dessert) {
  ...
  testWidgets('DataTable sort indicator orientation does not change on state update', (WidgetTester tester) async {
    ...
    Widget buildTable({String title = 'Name1'}) {
      return DataTable(
        sortColumnIndex: 0,
        columns: <DataColumn>[
          DataColumn(label: Text(title), tooltip: 'Name', onSort: (int columnIndex, bool ascending) {}),
        ],
        rows: kDesserts.map<DataRow>((Dessert dessert) {
          return DataRow(cells: <DataCell>[DataCell(Text(dessert.name))]);
        }).toList(),
  ...
  testWidgets('DataTable custom row height', (WidgetTester tester) async {
    ...
    // CUSTOM VALUES
    await tester.pumpWidget(MaterialApp(home: Material(child: buildCustomTable(headingRowHeight: 48.0))));
    expect(tester.getSize(findFirstContainerFor('Name')).height, 48.0);

    await tester.pumpWidget(MaterialApp(home: Material(child: buildCustomTable(headingRowHeight: 64.0))));
    expect(tester.getSize(findFirstContainerFor('Name')).height, 64.0);

    await tester.pumpWidget(
      MaterialApp(home: Material(child: buildCustomTable(dataRowMinHeight: 30.0, dataRowMaxHeight: 30.0))),
    );
    expect(tester.getSize(findFirstContainerFor('Frozen yogurt')).height, 30.0);

    await tester.pumpWidget(
      MaterialApp(home: Material(child: buildCustomTable(dataRowMinHeight: 0.0, dataRowMaxHeight: double.infinity))),
    );
  ...
  testWidgets('DataTable custom row height - separate test for deprecated dataRowHeight', (WidgetTester tester) async {
    ...
    // CUSTOM VALUES
    await tester.pumpWidget(MaterialApp(home: Material(child: buildCustomTable(dataRowHeight: 30.0))));
    expect(tester.getSize(findFirstContainerFor('Frozen yogurt')).height, 30.0);
  });
  ...
  testWidgets('DataTable custom horizontal padding - checkbox', (WidgetTester tester) async {
    ...
    expect(tester.getRect(padding).right - tester.getRect(checkbox).right, defaultHorizontalMargin / 2);
    ...
    expect(tester.getRect(cellContent).left - tester.getRect(padding).left, defaultHorizontalMargin / 2);
    expect(tester.getRect(padding).right - tester.getRect(cellContent).right, defaultColumnSpacing / 2);

    // default middle column padding
    padding = find.widgetWithText(Padding, '159');
    cellContent = find.widgetWithText(Align, '159');
    expect(tester.getRect(cellContent).left - tester.getRect(padding).left, defaultColumnSpacing / 2);
    expect(tester.getRect(padding).right - tester.getRect(cellContent).right, defaultColumnSpacing / 2);
    ...
    expect(tester.getRect(cellContent).left - tester.getRect(padding).left, defaultColumnSpacing / 2);
    expect(tester.getRect(padding).right - tester.getRect(cellContent).right, defaultHorizontalMargin);
    ...
    await tester.pumpWidget(MaterialApp(
      home: Material(
        child: buildCustomTable(horizontalMargin: customHorizontalMargin, columnSpacing: customColumnSpacing),
      ),
    ));
    ...
    expect(tester.getRect(padding).right - tester.getRect(checkbox).right, customHorizontalMargin / 2);

    // custom first column padding
    padding = find.widgetWithText(Padding, 'Frozen yogurt').first;
    cellContent = find.widgetWithText(Align, 'Frozen yogurt'); // DataTable wraps its DataCells in an Align widget
    expect(tester.getRect(cellContent).left - tester.getRect(padding).left, customHorizontalMargin / 2);
    expect(tester.getRect(padding).right - tester.getRect(cellContent).right, customColumnSpacing / 2);

    // custom middle column padding
    padding = find.widgetWithText(Padding, '159');
    cellContent = find.widgetWithText(Align, '159');
    expect(tester.getRect(cellContent).left - tester.getRect(padding).left, customColumnSpacing / 2);
    expect(tester.getRect(padding).right - tester.getRect(cellContent).right, customColumnSpacing / 2);
    ...
    cellContent = find.widgetWithText(Align, '6.0');
    expect(tester.getRect(cellContent).left - tester.getRect(padding).left, customColumnSpacing / 2);
    expect(tester.getRect(padding).right - tester.getRect(cellContent).right, customHorizontalMargin);
  });
  ...
  testWidgets('DataTable custom horizontal padding - no checkbox', (WidgetTester tester) async {
    ...
    padding = find.widgetWithText(Padding, 'Frozen yogurt');
    cellContent = find.widgetWithText(Align, 'Frozen yogurt'); // DataTable wraps its DataCells in an Align widget
    expect(tester.getRect(cellContent).left - tester.getRect(padding).left, defaultHorizontalMargin);
    expect(tester.getRect(padding).right - tester.getRect(cellContent).right, defaultColumnSpacing / 2);
  ...
  testWidgets('DataTable set border width test', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(home: Material(child: DataTable(columns: columns, rows: rows))));
    ...
    await tester.pumpWidget(
      MaterialApp(home: Material(child: DataTable(dividerThickness: thickness, columns: columns, rows: rows))),
    );
  ...
  testWidgets('DataTable set show bottom border', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(home: Material(child: DataTable(showBottomBorder: true, columns: columns, rows: rows))),
    );
  ...
  testWidgets('DataTable correctly renders with a mouse', (WidgetTester tester) async {
    ...
    Widget buildTable({int? sortColumnIndex}) {
      return DataTable(
        sortColumnIndex: sortColumnIndex,
        columns: <DataColumn>[
          const DataColumn(label: Expanded(child: Center(child: Text('column1'))), tooltip: 'Column1'),
          DataColumn(
    ...
    Widget buildTable({bool disabled = false}) {
      return Material(
        child: DataTable(
          columns: const <DataColumn>[DataColumn(label: Text('Column1'))],
          rows: <DataRow>[
            DataRow(cells: const <DataCell>[DataCell(Text('Content1'))], onSelectChanged: (bool? value) {}),
            DataRow(
              color: MaterialStateProperty.resolveWith<Color>((Set<MaterialState> states) {
  ...
  testWidgets('DataRow renders custom colors when pressed', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(theme: ThemeData(useMaterial3: false), home: Material(child: buildTable())));
  ...
  testWidgets('DataTable renders with border and background decoration', (WidgetTester tester) async {
    ...
    expect(
      ...
      paints..rect(rect: const Rect.fromLTRB(0.0, 0.0, width, height), color: backgroundColor),
    );
    expect(find.ancestor(of: find.byType(Table), matching: find.byType(Container)), paints..drrect(color: borderColor));
    expect(tester.getTopLeft(find.byType(Table)), const Offset(borderVertical, borderHorizontal));
    expect(tester.getBottomRight(find.byType(Table)), const Offset(width - borderVertical, height - borderHorizontal));
  });
  ...
  testWidgets('checkboxHorizontalMargin properly applied', (WidgetTester tester) async {
    ...
    checkbox = find.byType(Checkbox).first;
    padding = find.ancestor(of: checkbox, matching: find.byType(Padding));
    expect(tester.getRect(checkbox).left - tester.getRect(padding).left, customCheckboxHorizontalMargin);
    expect(tester.getRect(padding).right - tester.getRect(checkbox).right, customCheckboxHorizontalMargin);
  ...
  testWidgets('DataRow is disabled when onSelectChanged is not set', (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
      home: Material(
        child: DataTable(
          columns: const <DataColumn>[DataColumn(label: Text('Col1')), DataColumn(label: Text('Col2'))],
          rows: <DataRow>[
            DataRow(
  ...
  testWidgets('DataTable set interior border test', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(
      home: Material(
        child: DataTable(border: TableBorder.all(width: 2, color: Colors.red), columns: columns, rows: rows),
      ),
    ));
    ...
    await tester.pumpWidget(MaterialApp(
      home: Material(child: DataTable(border: TableBorder.all(color: Colors.red), columns: columns, rows: rows)),
    ));
  ...
  testWidgets('DataTable dataRowMinHeight smaller or equal dataRowMaxHeight validation', (WidgetTester tester) async {
    ...
    expect(
      () => createDataTable(),
      throwsA(predicate((AssertionError e) => e.toString().contains('dataRowMaxHeight >= dataRowMinHeight'))),
    );
  });

  testWidgets('DataTable dataRowHeight is not used together with dataRowMinHeight or dataRowMaxHeight', (
    WidgetTester tester,
  ) async {
    DataTable createDataTable({double? dataRowHeight, double? dataRowMinHeight, double? dataRowMaxHeight}) => DataTable(
      columns: const <DataColumn>[DataColumn(label: Text('Column1'))],
      rows: const <DataRow>[],
      dataRowHeight: dataRowHeight,
      dataRowMinHeight: dataRowMinHeight,
    ...
    expect(() => createDataTable(dataRowHeight: 1.0, dataRowMinHeight: 2.0, dataRowMaxHeight: 2.0), throwsA(predicate(
      (AssertionError e) => e.toString().contains(
        'dataRowHeight == null || (dataRowMinHeight == null && dataRowMaxHeight == null)',
      ),
    )));

    expect(() => createDataTable(dataRowHeight: 1.0, dataRowMaxHeight: 2.0), throwsA(predicate(
      (AssertionError e) => e.toString().contains(
        'dataRowHeight == null || (dataRowMinHeight == null && dataRowMaxHeight == null)',
      ),
    )));

    expect(() => createDataTable(dataRowHeight: 1.0, dataRowMinHeight: 2.0), throwsA(predicate(
      (AssertionError e) => e.toString().contains(
        'dataRowHeight == null || (dataRowMinHeight == null && dataRowMaxHeight == null)',
      ),
    )));
