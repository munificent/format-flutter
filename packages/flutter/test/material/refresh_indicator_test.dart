...
void main() {
  ...
  testWidgets('RefreshIndicator does not force child to relayout', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(home: RefreshIndicator(onRefresh: refresh, child: LayoutBuilder(builder: layoutCallback))),
    );
  ...
  testWidgets('RefreshIndicator responds to edgeOffset', (WidgetTester tester) async {
    ...
    expect(tester.widget<RefreshIndicator>(find.byType(RefreshIndicator)).edgeOffset, kToolbarHeight);
  });
  ...
  testWidgets('RefreshIndicator appears at edgeOffset', (WidgetTester tester) async {
    ...
    expect(tester.getTopLeft(find.byType(RefreshProgressIndicator)).dy, greaterThanOrEqualTo(2.0 * kToolbarHeight));
  });
  ...
  testWidgets('RefreshIndicator color defaults to ColorScheme.primary', (WidgetTester tester) async {
    ...
    await tester.pump();
    expect(
      tester.widget<RefreshProgressIndicator>(find.byType(RefreshProgressIndicator)).valueColor!.value,
      primaryColor,
    );
  });
    ...
    await tester.pumpWidget(MaterialApp(
      home: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: <Widget>[for (int i = 0; i < 4; i++) SizedBox(height: 200.0, child: Text('X - $i'))],
        ),
      ),
    ));
            ...
            return NotificationListener<ScrollNotification>(
              onNotification: (ScrollNotification notification) {
                if (notification is OverscrollNotification && lastNotification is! OverscrollNotification) {
                  final OverscrollIndicatorNotification confirmationNotification = OverscrollIndicatorNotification(
                    leading: true,
                  );
