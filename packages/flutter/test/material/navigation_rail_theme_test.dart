...
void main() {
  ...
  testWidgets('Default values are used when no NavigationRail or NavigationRailThemeData properties are specified', (
    ...
    await tester.pumpWidget(
      MaterialApp(theme: theme, home: Scaffold(body: NavigationRail(selectedIndex: 0, destinations: _destinations()))),
    );
        ...
        home: Scaffold(
          body: NavigationRailTheme(
            data: const NavigationRailThemeData(
              ...
              elevation: elevation,
              selectedIconTheme:
                  IconThemeData(size: selectedIconSize, color: selectedIconColor, opacity: selectedIconOpacity),
              unselectedIconTheme:
                  IconThemeData(size: unselectedIconSize, color: unselectedIconColor, opacity: unselectedIconOpacity),
        ...
        home: Scaffold(
          body: NavigationRailTheme(
            data: const NavigationRailThemeData(
              ...
              elevation: 5,
              selectedIconTheme: IconThemeData(size: 31.0, color: Color(0x00000098), opacity: 0.81),
              unselectedIconTheme: IconThemeData(size: 37.0, color: Color(0x00000097), opacity: 0.82),
            ...
            child: NavigationRail(
              ...
              elevation: elevation,
              selectedIconTheme:
                  const IconThemeData(size: selectedIconSize, color: selectedIconColor, opacity: selectedIconOpacity),
  ...
  testWidgets('NavigationRailThemeData lerps correctly with null iconThemes', (WidgetTester tester) async {
    final NavigationRailThemeData lerp =
        NavigationRailThemeData.lerp(const NavigationRailThemeData(), const NavigationRailThemeData(), 0.5)!;
...
List<NavigationRailDestination> _destinations() {
  return const <NavigationRailDestination>[
    ...
    NavigationRailDestination(icon: Icon(Icons.star_border), selectedIcon: Icon(Icons.star), label: Text('Def')),
  ];
}
...
IconThemeData _iconTheme(WidgetTester tester, IconData icon) {
  ...
  return tester.firstWidget<IconTheme>(find.ancestor(of: find.byIcon(icon), matching: find.byType(IconTheme))).data;
}

TextStyle _selectedLabelStyle(WidgetTester tester) {
  return tester.widget<RichText>(find.descendant(of: find.text('Abc'), matching: find.byType(RichText))).text.style!;
}

TextStyle _unselectedLabelStyle(WidgetTester tester) {
  return tester.widget<RichText>(find.descendant(of: find.text('Def'), matching: find.byType(RichText))).text.style!;
}

Size _destinationSize(WidgetTester tester) {
  return tester.getSize(find.ancestor(of: find.byIcon(Icons.favorite), matching: find.byType(Material)).first);
}

Align _destinationsAlign(WidgetTester tester) {
  // The first Expanded widget is the one within the main Column for the rail
  // content.
  return tester.firstWidget<Align>(find.descendant(of: find.byType(Expanded), matching: find.byType(Align)));
}
