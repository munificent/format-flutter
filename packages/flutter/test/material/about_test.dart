...
void main() {
  ...
  testWidgetsWithLeakTracking('About box logic defaults to executable name for app name', (WidgetTester tester) async {
    await tester.pumpWidget(const MaterialApp(title: 'flutter_tester', home: Material(child: AboutListTile())));
  ...
  testWidgetsWithLeakTracking('LicensePage control test', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(theme: ThemeData(useMaterial3: false), home: const Center(child: LicensePage())),
    );
  ...
  testWidgetsWithLeakTracking('_PackageLicensePage title style without AppBarTheme', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(
      theme: ThemeData(
        useMaterial3: false,
        primaryTextTheme: const TextTheme(titleLarge: titleTextStyle, titleSmall: subtitleTextStyle),
      ),
  ...
  testWidgetsWithLeakTracking('_PackageLicensePage title style with AppBarTheme', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(
      theme: ThemeData(
        ...
        appBarTheme: const AppBarTheme(titleTextStyle: titleTextStyle, foregroundColor: Colors.indigo),
      ),
      home: const Center(child: LicensePage()),
    ));
    await tester.pumpAndSettle();
  ...
  testWidgetsWithLeakTracking('LicensePage respects the notch', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(
      theme: ThemeData(useMaterial3: false),
      home: const MediaQuery(data: MediaQueryData(padding: EdgeInsets.all(safeareaPadding)), child: LicensePage()),
    ));
    ...
    expect(tester.getTopLeft(find.text('Licenses')), const Offset(16.0 + safeareaPadding, 18.0 + safeareaPadding));
  });
  ...
  testWidgetsWithLeakTracking(
    ...
    (WidgetTester tester) async {
      await tester.pumpWidget(const MaterialApp(title: 'flutter_tester', home: Material(child: LicensePage())));
  ...
  testWidgetsWithLeakTracking('AboutListTile dense property is applied', (WidgetTester tester) async {
    await tester.pumpWidget(const MaterialApp(home: Material(child: Center(child: AboutListTile()))));
    Rect tileRect = tester.getRect(find.byType(AboutListTile));
    expect(tileRect.height, 56.0);

    await tester.pumpWidget(const MaterialApp(home: Material(child: Center(child: AboutListTile(dense: false)))));
    tileRect = tester.getRect(find.byType(AboutListTile));
    expect(tileRect.height, 56.0);

    await tester.pumpWidget(const MaterialApp(home: Material(child: Center(child: AboutListTile(dense: true)))));
    tileRect = tester.getRect(find.byType(AboutListTile));
                  ...
                  onGenerateRoute: (RouteSettings settings) {
                    return PageRouteBuilder<dynamic>(
                      pageBuilder: (BuildContext context, _, __) {
                        return ElevatedButton(
                          onPressed: () {
                            showLicensePage(context: context, useRootNavigator: true, applicationName: 'A');
                          },
                          child: const Text('Show License Page'),
                        );
                      },
  ...
  testWidgetsWithLeakTracking(
    ...
    (WidgetTester tester) async {
      await tester.pumpWidget(const MaterialApp(home: Scaffold(body: AboutListTile(child: Text('About')))));

      expect(find.descendant(of: find.byType(AboutListTile), matching: find.byType(Icon)), findsNothing);
    },
  );
              ...
              return ElevatedButton(
                onPressed: () {
                  showAboutDialog(
                    ...
                    useRootNavigator: false,
                    applicationName: 'A',
                    children: <Widget>[Container(key: contentKey, color: Colors.orange, height: 500)],
                  );
                },
                child: const Text('Show About Dialog'),
              );
  ...
  testWidgetsWithLeakTracking("LicensePage's color must be same whether loading or done", (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(
      theme:
          ThemeData.light(useMaterial3: false).copyWith(scaffoldBackgroundColor: scaffoldColor, cardColor: cardColor),
      home: Scaffold(
        body: Center(
          child: Builder(
            builder: (BuildContext context) => GestureDetector(
                  ...
                  onTap: () {
                    showLicensePage(context: context, applicationName: 'MyApp', applicationVersion: '1.0.0');
                  },
                ),
          ),
        ),
  ...
  testWidgetsWithLeakTracking(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        const MaterialApp(title: 'Pirate app', home: Center(child: LicensePage(applicationIcon: logo))),
      );
  ...
  testWidgetsWithLeakTracking('Error handling test', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(theme: ThemeData(useMaterial3: false), home: const Material(child: AboutListTile())),
    );
  ...
  testWidgetsWithLeakTracking('LicensePage master view layout position - ltr', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(
      theme: ThemeData(useMaterial3: false),
      title: title,
      home: const Scaffold(body: Directionality(textDirection: textDirection, child: LicensePage())),
    ));
  ...
  testWidgetsWithLeakTracking(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(MaterialApp(title: title, theme: theme, home: const Scaffold(body: LicensePage())));
