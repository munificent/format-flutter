...
Widget boilerplate({required Widget child, ThemeData? theme}) {
  return MaterialApp(
    ...
    home: Localizations(
      locale: const Locale('en', 'US'),
      delegates: <LocalizationsDelegate<dynamic>>[WidgetsLocalizationsDelegate(), MaterialLocalizationsDelegate()],
      child: Directionality(
        textDirection: TextDirection.ltr,
        child: MediaQuery(
          data: const MediaQueryData(size: Size(800.0, 600.0)),
...
double getOpacity(WidgetTester tester, Finder finder) {
  return tester.widget<FadeTransition>(find.ancestor(of: finder, matching: find.byType(FadeTransition))).opacity.value;
}
...
void main() {
  ...
  testWidgets('Composing change does not hide selection handle caret', (WidgetTester tester) async {
    ...
    tester.testTextInput.updateEditingValue(controller.value.copyWith(composing: const TextRange(start: 7, end: 15)));
    await skipPastScrollingAnimation(tester);
    ...
    tester.testTextInput.updateEditingValue(controller.value.copyWith(
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final TestGesture gesture =
          await tester.startGesture(midBlah1, kind: PointerDeviceKind.mouse, buttons: kSecondaryMouseButton);
      await tester.pump();
      await gesture.up();
      await tester.pumpAndSettle();
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      gesture = await tester.startGesture(midBlah1, kind: PointerDeviceKind.mouse, buttons: kSecondaryMouseButton);
      await tester.pump();
      await gesture.up();
      await gesture.removePointer();
      await tester.pumpAndSettle();
  ...
  testWidgets('sets cursorOpacityAnimates on EditableText correctly', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      const MaterialApp(home: Material(child: TextField(autofocus: true, cursorOpacityAnimates: true))),
    );
    await tester.pump();
    ...
    await tester.pumpWidget(
      const MaterialApp(home: Material(child: TextField(autofocus: true, cursorOpacityAnimates: false))),
    );
    await tester.pump();
                ...
                TestSemantics(
                  ...
                  children: <TestSemantics>[
                    TestSemantics(
                      ...
                      actions: <SemanticsAction>[SemanticsAction.tap, SemanticsAction.didGainAccessibilityFocus],
                      textDirection: TextDirection.ltr,
                    ),
                  ],
                ),
  ...
  testWidgets('TextField passes onEditingComplete to EditableText', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(home: Material(child: TextField(onEditingComplete: onEditingComplete))));
  ...
  testWidgets('Cursor blinks', (WidgetTester tester) async {
    await tester.pumpWidget(overlay(child: const TextField(decoration: InputDecoration(hintText: 'Placeholder'))));
    await tester.showKeyboard(find.byType(TextField));
      ...
      home: Material(
        child: TextField(
          ...
          decoration: InputDecoration(
            suffixIcon: IconButton(key: icon, icon: const Icon(Icons.cancel), onPressed: () => controller.clear()),
          ),
        ),
      ),
    ));
          ...
          child: SizedBox(
            ...
            child: Center(
              child: SizedBox(
                ...
                height: 50,
                child: TextField(controller: OverflowWidgetTextEditingController(), clipBehavior: Clip.none),
              ),
            ),
          ),
        ),
  ...
  testWidgets('Material cursor android golden', (WidgetTester tester) async {
    final Widget widget = Theme(
      ...
      child: overlay(
        child: const RepaintBoundary(
          key: ValueKey<int>(1),
          child: TextField(cursorColor: Colors.blue, cursorWidth: 15, cursorRadius: Radius.circular(3.0)),
        ),
      ),
    );
    await tester.pumpWidget(widget);
    ...
    await expectLater(find.byKey(const ValueKey<int>(1)), matchesGoldenFile('text_field_cursor_test.material.0.png'));
  });
  ...
  testWidgets('TextInputFormatter gets correct selection value', (WidgetTester tester) async {
    ...
    expect(actualNewValue, const TextEditingValue(text: '12', selection: TextSelection.collapsed(offset: 2)));
      ...
      home: const Scaffold(
        body: Center(
          child: SizedBox(
            width: 100,
            height: 100,
            child: Opacity(opacity: 0.5, child: TextField(decoration: InputDecoration(hintText: 'Placeholder'))),
          ),
        ),
      ),
    ));
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      final TextEditingController controller = TextEditingController(text: 'Atwater Peel Sherbrooke Bonaventure');
      // On iOS/iPadOS, during a tap we select the edge of the word closest to the tap.
      // On macOS, we select the precise position of the tap.
      await tester.pumpWidget(MaterialApp(
        home: Material(
          child: Center(child: TextField(controller: controller, toolbarOptions: const ToolbarOptions(copy: true))),
        ),
      ));
  ...
  testWidgets('text selection style 1', (WidgetTester tester) async {
    ...
    await expectLater(find.byType(MaterialApp), matchesGoldenFile('text_field_golden.TextSelectionStyle.1.png'));
  });
  ...
  testWidgets('text selection style 2', (WidgetTester tester) async {
    ...
    await expectLater(find.byType(MaterialApp), matchesGoldenFile('text_field_golden.TextSelectionStyle.2.png'));
  ...
  testWidgets('cursor layout has correct width', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(Theme(
      data: ThemeData(useMaterial3: false),
      child: overlay(
        child: RepaintBoundary(child: TextField(cursorWidth: 15.0, controller: controller, focusNode: focusNode)),
      ),
    ));
    focusNode.requestFocus();
    await tester.pump();

    await expectLater(find.byType(TextField), matchesGoldenFile('text_field_cursor_width_test.0.png'));
  ...
  testWidgets('cursor layout has correct radius', (WidgetTester tester) async {
    ...
    await expectLater(find.byType(TextField), matchesGoldenFile('text_field_cursor_width_test.1.png'));
    EditableText.debugDeterministicCursor = false;
  });
  ...
  testWidgets('cursor layout has correct height', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(Theme(
      ...
      child: overlay(
        child: RepaintBoundary(
          child: TextField(cursorWidth: 15.0, cursorHeight: 30.0, controller: controller, focusNode: focusNode),
        ),
      ),
    ));
    focusNode.requestFocus();
    ...
    await expectLater(find.byType(TextField), matchesGoldenFile('text_field_cursor_width_test.2.png'));
    EditableText.debugDeterministicCursor = false;
  });
  ...
  testWidgets('Overflowing a line with spaces stops the cursor at the end', (WidgetTester tester) async {
    ...
    final Offset cursorOffsetSpaces =
        findRenderEditable(tester).getLocalRectForCaret(const TextPosition(offset: testValueSpaces.length)).bottomRight;
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      await tester.pumpWidget(overlay(child: const TextField(textDirection: TextDirection.rtl, maxLines: null)));
      ...
      final Offset cursorOffsetSpaces =
          findRenderEditable(tester).getLocalRectForCaret(const TextPosition(offset: testValueSpaces.length)).topLeft;
  ...
  testWidgets('mobile obscureText control test', (WidgetTester tester) async {
    await tester.pumpWidget(
      overlay(child: const TextField(obscureText: true, decoration: InputDecoration(hintText: 'Placeholder'))),
    );
    await tester.showKeyboard(find.byType(TextField));

    const String testValue = 'ABC';
    tester.testTextInput.updateEditingValue(
      const TextEditingValue(text: testValue, selection: TextSelection.collapsed(offset: testValue.length)),
    );
  ...
  testWidgets('enableInteractiveSelection = false, tap', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(overlay(child: TextField(controller: controller, enableInteractiveSelection: false)));
    expect(controller.selection.baseOffset, -1);
    expect(controller.selection.extentOffset, -1);
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final OverlayEntry entry = OverlayEntry(
        builder: (BuildContext context) {
          return Center(child: Material(child: TextField(controller: controller, readOnly: readOnly)));
        },
      );
      await tester.pumpWidget(overlayWithEntry(entry));
      await tester.tap(find.byType(TextField));
  ...
  testWidgets('Can select text by dragging with a mouse', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(home: Material(child: TextField(dragStartBehavior: DragStartBehavior.down, controller: controller))),
    );
  ...
  testWidgets('Cursor should not move on a quick touch drag when touch does not begin on previous selection (iOS)', (
    WidgetTester tester,
  ) async {
    final TextEditingController controller = TextEditingController();
  ...
  testWidgets('Can move cursor when dragging, when tap is on collapsed selection (iOS) - multiline', (
    WidgetTester tester,
  ) async {
    final TextEditingController controller = TextEditingController();
    ...
    await tester.pumpWidget(MaterialApp(
      home:
          Material(child: TextField(dragStartBehavior: DragStartBehavior.down, controller: controller, maxLines: null)),
    ));
  ...
  testWidgets('Can move cursor when dragging, when tap is on collapsed selection (iOS) - ListView', (
    WidgetTester tester,
  ) async {
    ...
    await tester.pumpWidget(MaterialApp(
      home: Material(
        child: ListView(
          children: <Widget>[
            TextField(dragStartBehavior: DragStartBehavior.down, controller: controller, maxLines: null),
          ],
        ),
      ),
    ));
  ...
  testWidgets('Can drag handles to change selection on Apple platforms', (WidgetTester tester) async {
    ...
    final RenderEditable renderEditable = findRenderEditable(tester);
    List<TextSelectionPoint> endpoints = globalize(renderEditable.getEndpointsForSelection(selection), renderEditable);
    ...
    endpoints = globalize(renderEditable.getEndpointsForSelection(controller.selection), renderEditable);
    handlePos = endpoints[0].point + const Offset(-1.0, 1.0);
  ...
  testWidgets('Can drag the left handle while the right handle remains off-screen', (WidgetTester tester) async {
  ...
  testWidgets('Can drag the right handle while the left handle remains off-screen', (WidgetTester tester) async {
    // Text is longer than textfield width.
    const String testValue = 'aaaaaaaaaaaaaaaaaaaaaaaaaaa bbbbbbbbbbbbbbbbbbbbbbbbbbb';
    final TextEditingController controller = TextEditingController(text: testValue);
    final ScrollController scrollController = ScrollController();
  ...
  testWidgets('Dragging between multiple lines keeps the contact point at the same place on the handle on Android', (
    WidgetTester tester,
  ) async {
    final TextEditingController controller = TextEditingController(
    ...
    await tester.pumpWidget(overlay(
      child: TextField(dragStartBehavior: DragStartBehavior.down, controller: controller, maxLines: 3, minLines: 3),
    ));
  ...
  testWidgets('Dragging between multiple lines keeps the contact point at the same place on the handle on iOS', (
    WidgetTester tester,
  ) async {
    final TextEditingController controller = TextEditingController(
      // 11 first line, 19 second line, 17 third line = length 49
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: Padding(padding: const EdgeInsets.all(30.0), child: TextField(controller: controller))),
      ));
      ...
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: Padding(padding: const EdgeInsets.all(150.0), child: TextField(controller: controller))),
      ));
          ...
          body: Center(
            child: Padding(
              ...
              child: Column(
                ...
                children: <Widget>[
                  IntrinsicHeight(child: TextField(controller: controller, expands: true, maxLines: null)),
  ...
  testWidgets('An obscured TextField is not selectable when read-only', (WidgetTester tester) async {
    ...
    Widget buildFrame(bool obscureText, bool readOnly) {
      return overlay(child: TextField(controller: controller, obscureText: obscureText, readOnly: readOnly));
    }
  ...
  testWidgets('Multiline text when wrapped in IntrinsicHeight', (WidgetTester tester) async {
    ...
    Widget intrinsicTextFieldBuilder(bool wrapInIntrinsic) {
      ...
      final Widget widget = wrapInIntrinsic ? IntrinsicHeight(key: intrinsicHeightKey, child: textField) : textField;
      return boilerplate(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: <Widget>[widget]));
    }
  ...
  testWidgets('Growable TextField when content height exceeds parent', (WidgetTester tester) async {
    ...
    Widget containedTextFieldBuilder({Widget? counter, String? helperText, String? labelText, Widget? prefix}) {
      return boilerplate(
        theme: ThemeData(useMaterial3: false),
        child: SizedBox(
          height: height,
          child: TextField(
            key: textFieldKey,
            maxLines: null,
            decoration: InputDecoration(counter: counter, helperText: helperText, labelText: labelText, prefix: prefix),
          ),
        ),
      );
    }
    ...
    await tester.pumpWidget(
      containedTextFieldBuilder(counter: Container(height: counterHeight), helperText: 'I am helperText'),
    );
    expect(findEditableText(), equals(inputBox));
    expect(inputBox.size.height, height - padding - counterSpace);
    ...
    await tester.pumpWidget(
      containedTextFieldBuilder(counter: Container(height: counterHeight), labelText: 'I am labelText'),
    );
    expect(findEditableText(), equals(inputBox));
  ...
  testWidgets('Can drag handles to change selection in multiline', (WidgetTester tester) async {
    ...
    Offset newHandlePos = textOffsetToPosition(tester, testValue.indexOf('Third') + 5) + downLineOffset;
    gesture = await tester.startGesture(handlePos, pointer: 7);
    await tester.pump();
    await gesture.moveTo(newHandlePos);
    await tester.pump();
  ...
  testWidgets('Can scroll multiline input', (WidgetTester tester) async {
    final Key textFieldKey = UniqueKey();
    final TextEditingController controller = TextEditingController(text: kMoreThanFourLines);
    ...
    final Offset selectedWordPos = textOffsetToPosition(tester, kMoreThanFourLines.indexOf('Fourth line') + 14);
    ...
    expect(controller.text.substring(controller.selection.baseOffset, controller.selection.extentOffset), "won't");
  ...
  testWidgets('TextField errorText trumps helperText', (WidgetTester tester) async {
    await tester.pumpWidget(Theme(
      data: ThemeData(useMaterial3: false),
      child: overlay(
        child: const TextField(decoration: InputDecoration(errorText: 'error text', helperText: 'helper text')),
      ),
    ));
  ...
  testWidgets('TextField with default helperStyle', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(overlay(
      child: Theme(data: themeData, child: const TextField(decoration: InputDecoration(helperText: 'helper text'))),
    ));
  ...
  testWidgets('TextField with specified helperStyle', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      overlay(child: TextField(decoration: InputDecoration(helperText: 'helper text', helperStyle: style))),
    );
    final Text helperText = tester.widget(find.text('helper text'));
  ...
  testWidgets('TextField with default hintStyle', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(overlay(
      child: Theme(
        data: themeData,
        child: TextField(decoration: const InputDecoration(hintText: 'Placeholder'), style: style),
      ),
    ));
  ...
  testWidgets('TextField with specified hintStyle', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      overlay(child: TextField(decoration: InputDecoration(hintText: 'Placeholder', hintStyle: hintStyle))),
    );
  ...
  testWidgets('TextField with specified prefixStyle', (WidgetTester tester) async {
    final TextStyle prefixStyle = TextStyle(inherit: false, color: Colors.pink[500], fontSize: 10.0);

    await tester.pumpWidget(
      overlay(child: TextField(decoration: InputDecoration(prefixText: 'Prefix:', prefixStyle: prefixStyle))),
    );
  ...
  testWidgets('TextField with specified suffixStyle', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      overlay(child: TextField(decoration: InputDecoration(suffixText: '.com', suffixStyle: suffixStyle))),
    );
    ...
    await tester.pumpWidget(overlay(
      child: Column(
        children: <Widget>[
          ...
          TextField(key: secondKey, decoration: const InputDecoration(prefixText: 'Prefix', suffixText: 'Suffix')),
        ],
      ),
    ));
      ...
      child: Column(
        children: <Widget>[
          ...
          TextField(
            key: secondKey,
            decoration:
                InputDecoration(hintText: 'Hint', hintStyle: hintStyle, prefixText: 'Prefix', suffixText: 'Suffix'),
          ),
        ],
      ),
    ));
  ...
  testWidgets('Icon is separated from input/label by 16+12', (WidgetTester tester) async {
    await tester.pumpWidget(overlay(
      child: const TextField(decoration: InputDecoration(icon: Icon(Icons.phone), labelText: 'label', filled: true)),
    ));
  ...
  testWidgets('Collapsed hint text placement', (WidgetTester tester) async {
    await tester.pumpWidget(Theme(
      ...
      child: overlay(
        child:
            const TextField(decoration: InputDecoration.collapsed(hintText: 'hint'), strutStyle: StrutStyle.disabled),
      ),
    ));
  ...
  testWidgets('Can align to center', (WidgetTester tester) async {
    await tester.pumpWidget(
      overlay(child: const SizedBox(width: 300.0, child: TextField(textAlign: TextAlign.center, decoration: null))),
    );

    final RenderEditable editable = findRenderEditable(tester);
    assert(editable.size.width == 300);
    Offset topLeft = editable.localToGlobal(editable.getLocalRectForCaret(const TextPosition(offset: 0)).topLeft);
    ...
    topLeft = editable.localToGlobal(editable.getLocalRectForCaret(const TextPosition(offset: 2)).topLeft);
  ...
  testWidgets('Can align to center within center', (WidgetTester tester) async {
    await tester.pumpWidget(overlay(
      child:
          const SizedBox(width: 300.0, child: Center(child: TextField(textAlign: TextAlign.center, decoration: null))),
    ));
  ...
  testWidgets('Controller can update server', (WidgetTester tester) async {
    final TextEditingController controller1 = TextEditingController(text: 'Initial Text');
    final TextEditingController controller2 = TextEditingController(text: 'More Text');
  ...
  testWidgets('Cannot enter new lines onto single line TextField', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(boilerplate(child: TextField(controller: textController, decoration: null)));
  ...
  testWidgets('Do not add LengthLimiting formatter to the user supplied list', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(overlay(child: TextField(decoration: null, maxLength: 5, inputFormatters: formatters)));
  ...
  testWidgets('Text field scrolls the caret into view', (WidgetTester tester) async {
    ...
    editableTextState.userUpdateTextEditingValue(
      editableTextState.textEditingValue.copyWith(selection: TextSelection.collapsed(offset: longText.length)),
      null,
    );
  ...
  testWidgets('Multiline text field scrolls the caret into view', (WidgetTester tester) async {
    ...
    final EditableTextState editableTextState = tester.firstState(find.byType(EditableText));
    editableTextState.userUpdateTextEditingValue(
      editableTextState.textEditingValue.copyWith(selection: const TextSelection.collapsed(offset: tallText.length)),
      null,
    );
    await tester.pump();
    await skipPastScrollingAnimation(tester);
  ...
  testWidgets('haptic feedback', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(overlay(child: SizedBox(width: 100.0, child: TextField(controller: controller))));
  ...
  testWidgets('Text field drops selection color when losing focus', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(overlay(
      child: DefaultSelectionStyle(
        selectionColor: selectionColor,
        cursorColor: cursorColor,
        child: Column(children: <Widget>[TextField(key: key1, controller: controller1), TextField(key: key2)]),
      ),
    ));
  ...
  testWidgets('Clearing text field with suffixIcon does not cause text selection exception', (
    ...
    final TextEditingController controller = TextEditingController(text: 'Prefilled text.');

    await tester.pumpWidget(boilerplate(
      child: TextField(
        controller: controller,
        decoration: InputDecoration(suffixIcon: IconButton(icon: const Icon(Icons.close), onPressed: controller.clear)),
      ),
    ));
  ...
  testWidgets('maxLength limits input.', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(boilerplate(child: TextField(controller: textController, maxLength: 10)));
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.showKeyboard(find.byType(TextField));
      tester.testTextInput.updateEditingValue(
        const TextEditingValue(text: '${testValue}9999999', selection: TextSelection.collapsed(offset: 10 + 7)),
      );
      await tester.pump();
      ...
      await tester.showKeyboard(find.byType(TextField));
      tester.testTextInput.updateEditingValue(
        const TextEditingValue(text: '0123455555555556789', selection: TextSelection.collapsed(offset: 19)),
      );
      await tester.pump();
  ...
  testWidgets('maxLength limits input in the center of a maxed-out field, with non-collapsed selection', (
    WidgetTester tester,
  ) async {
    final TextEditingController textController = TextEditingController();
    const String testValue = '0123456789';
    ...
    await tester.pumpWidget(boilerplate(
      child: TextField(controller: textController, maxLength: 10, maxLengthEnforcement: MaxLengthEnforcement.enforced),
    ));

    // Max out the character limit in the field.
    await tester.showKeyboard(find.byType(TextField));
    tester.testTextInput.updateEditingValue(
      const TextEditingValue(text: testValue, selection: TextSelection(baseOffset: 8, extentOffset: 10)),
    );
    await tester.pump();
    expect(textController.text, testValue);
    ...
    await tester.showKeyboard(find.byType(TextField));
    tester.testTextInput.updateEditingValue(
      const TextEditingValue(text: '01234569999999', selection: TextSelection.collapsed(offset: 14)),
    );
    await tester.pump();
  ...
  testWidgets('maxLength limits input length even if decoration is null.', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(boilerplate(child: TextField(controller: textController, decoration: null, maxLength: 10)));
  ...
  testWidgets("maxLength isn't enforced when maxLengthEnforcement.none.", (WidgetTester tester) async {
    ...
    await tester.pumpWidget(boilerplate(
      child: TextField(controller: textController, maxLength: 10, maxLengthEnforcement: MaxLengthEnforcement.none),
    ));
  ...
  testWidgets('setting maxLength shows counter', (WidgetTester tester) async {
    await tester.pumpWidget(const MaterialApp(home: Material(child: Center(child: TextField(maxLength: 10)))));
  ...
  testWidgets('setting maxLength to TextField.noMaxLength shows only entered length', (WidgetTester tester) async {
    await tester.pumpWidget(
      const MaterialApp(home: Material(child: Center(child: TextField(maxLength: TextField.noMaxLength)))),
    );
    ...
    await tester.pumpWidget(MaterialApp(
      home: Material(
        child: Center(
          child: TextField(
            buildCounter:
                (BuildContext context, {required int currentLength, int? maxLength, required bool isFocused}) {
  ...
  testWidgets('Disabled text field does not have tap action', (WidgetTester tester) async {
    final SemanticsTester semantics = SemanticsTester(tester);
    await tester.pumpWidget(
      const MaterialApp(home: Material(child: Center(child: TextField(maxLength: 10, enabled: false)))),
    );
  ...
  testWidgets('Disabled text field semantics node still contains value', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(
      home: Material(
        child: Center(child: TextField(controller: TextEditingController(text: 'text'), maxLength: 10, enabled: false)),
      ),
    ));
  ...
  testWidgets('Readonly text field does not have tap action', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      const MaterialApp(home: Material(child: Center(child: TextField(maxLength: 10, readOnly: true)))),
    );
  ...
  testWidgets('currentValueLength/maxValueLength are in the tree', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(home: Material(child: Center(child: TextField(controller: controller, maxLength: 10)))),
    );

    expect(
      semantics,
      includesNodeWith(flags: <SemanticsFlag>[SemanticsFlag.isTextField], maxValueLength: 10, currentValueLength: 0),
    );
  ...
  testWidgets('Read only TextField identifies as read only text field in semantics', (WidgetTester tester) async {
    ...
    expect(semantics, includesNodeWith(flags: <SemanticsFlag>[SemanticsFlag.isTextField, SemanticsFlag.isReadOnly]));
  ...
  group('Keyboard Tests', () {
    ...
    Future<void> setupWidget(WidgetTester tester) async {
      ...
      await tester.pumpWidget(MaterialApp(
        home: Material(
          child: RawKeyboardListener(focusNode: focusNode, child: TextField(controller: controller, maxLines: 3)),
        ),
      ));
      await tester.pump();
    }
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(home: Material(child: RawKeyboardListener(focusNode: focusNode, child: textField))),
      );
      focusNode.requestFocus();
      await tester.pump();
  ...
  testWidgets('TextField semantics always include label when no hint is given', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(overlay(
      child: TextField(key: key, controller: controller, decoration: const InputDecoration(labelText: 'label')),
    ));
  ...
  testWidgets('TextField semantics only include hint when it is visible', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      overlay(child: TextField(key: key, controller: controller, decoration: const InputDecoration(hintText: 'hint'))),
    );
  ...
  testWidgets('TextField semantics, enableInteractiveSelection = false', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      overlay(child: TextField(key: key, controller: controller, enableInteractiveSelection: false)),
    );
  ...
  testWidgets('TextField change selection with semantics', (WidgetTester tester) async {
    ...
    semanticsOwner
        .performAction(inputFieldId, SemanticsAction.setSelection, <dynamic, dynamic>{'base': 4, 'extent': 4});
    await tester.pump();
    expect(controller.selection, const TextSelection.collapsed(offset: 4));

    // move cursor to front
    semanticsOwner
        .performAction(inputFieldId, SemanticsAction.setSelection, <dynamic, dynamic>{'base': 0, 'extent': 0});
    await tester.pump();
    expect(controller.selection, const TextSelection.collapsed(offset: 0));

    // select all
    semanticsOwner
        .performAction(inputFieldId, SemanticsAction.setSelection, <dynamic, dynamic>{'base': 0, 'extent': 5});
    await tester.pump();
    expect(controller.selection, const TextSelection(baseOffset: 0, extentOffset: 5));
    expect(semantics, hasSemantics(TestSemantics.root(
      children: <TestSemantics>[
  ...
  testWidgets('Can activate TextField with explicit controller via semantics ', (WidgetTester tester) async {
    ...
    final SemanticsTester semantics = SemanticsTester(tester);
    final SemanticsOwner semanticsOwner = tester.binding.pipelineOwner.semanticsOwner!;
    final TextEditingController controller = TextEditingController()..text = textInTextField;
    final Key key = UniqueKey();
    ...
    expect(semantics, hasSemantics(TestSemantics.root(
      children: <TestSemantics>[
        TestSemantics(
          ...
          value: textInTextField,
          textDirection: TextDirection.ltr,
          textSelection: const TextSelection(baseOffset: textInTextField.length, extentOffset: textInTextField.length),
        ),
      ],
    ), ignoreRect: true, ignoreTransform: true));
  ...
  testWidgets('TextField loses focus when disabled', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(boilerplate(child: TextField(focusNode: focusNode, autofocus: true, enabled: true)));
    expect(focusNode.hasFocus, isTrue);

    await tester.pumpWidget(boilerplate(child: TextField(focusNode: focusNode, autofocus: true, enabled: false)));
    expect(focusNode.hasFocus, isFalse);
  ...
  testWidgets('TextField displays text with text direction', (WidgetTester tester) async {
    ...
    await tester.enterText(find.byType(TextField), '0123456789101112');
    await tester.pumpAndSettle();
    Offset topLeft = editable.localToGlobal(editable.getLocalRectForCaret(const TextPosition(offset: 10)).topLeft);
    ...
    await tester.enterText(find.byType(TextField), '0123456789101112');
    await tester.pumpAndSettle();
    topLeft = editable.localToGlobal(editable.getLocalRectForCaret(const TextPosition(offset: 10)).topLeft);
  ...
  testWidgets('TextField semantics', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(overlay(
      child: TextField(
        ...
        controller: controller,
        maxLength: 10,
        decoration: const InputDecoration(labelText: 'label', hintText: 'hint', helperText: 'helper'),
      ),
    ));
      ...
      children: <TestSemantics>[
        TestSemantics.rootChild(
          ...
          children: <TestSemantics>[
            ...
            TestSemantics(id: 3, label: '10 characters remaining', textDirection: TextDirection.ltr),
          ],
        ),
      ],
    ), ignoreTransform: true, ignoreRect: true));
  ...
  testWidgets('InputDecoration errorText semantics', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(overlay(
      child: TextField(
        key: key,
        controller: controller,
        decoration: const InputDecoration(labelText: 'label', hintText: 'hint', errorText: 'oh no!'),
      ),
    ));
    ...
    expect(semantics, hasSemantics(TestSemantics.root(
      children: <TestSemantics>[
        TestSemantics.rootChild(
          ...
          actions: <SemanticsAction>[SemanticsAction.tap],
          flags: <SemanticsFlag>[SemanticsFlag.isTextField],
          children: <TestSemantics>[TestSemantics(label: 'oh no!', textDirection: TextDirection.ltr)],
        ),
      ],
    ), ignoreTransform: true, ignoreRect: true, ignoreId: true));
  ...
  testWidgets('Includes cursor for TextField', (WidgetTester tester) async {
    ...
    Widget buildFrame({double? stepWidth, required double cursorWidth, required TextAlign textAlign}) {
      return MaterialApp(
        theme: ThemeData(useMaterial3: false),
        home: Scaffold(
          body: Center(
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: <Widget>[
                IntrinsicWidth(stepWidth: stepWidth, child: TextField(textAlign: textAlign, cursorWidth: cursorWidth)),
              ],
            ),
          ),
        ),
    ...
    await tester.pumpWidget(buildFrame(stepWidth: 80.0, cursorWidth: 2.0, textAlign: TextAlign.left));
    ...
    await tester.pumpWidget(buildFrame(stepWidth: stepWidth, cursorWidth: 18.0, textAlign: TextAlign.left));
    await tester.enterText(find.byType(TextField), text);
    await tester.pumpAndSettle();
    ...
    await tester.pumpWidget(buildFrame(stepWidth: stepWidth, cursorWidth: 18.0, textAlign: TextAlign.right));
    await tester.enterText(find.byType(TextField), text);
    await tester.pumpAndSettle();
    expect(tester.getSize(find.byType(TextField)).width, WIDTH_OF_CHAR * text.length + 18.0 + CARET_GAP);
  });
  ...
  testWidgets('TextField style is merged with theme', (WidgetTester tester) async {
    ...
    Widget buildFrame(TextStyle style) {
      return MaterialApp(theme: themeData, home: Material(child: Center(child: TextField(style: style))));
    }
    ...
    await tester.pumpWidget(
      buildFrame(const TextStyle(fontSize: 24.0, textBaseline: TextBaseline.alphabetic, inherit: false)),
    );
    editableText = tester.widget(find.byType(EditableText));
  ...
  testWidgets('style enforces required fields', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      buildFrame(const TextStyle(inherit: false, fontSize: 12.0, textBaseline: TextBaseline.alphabetic)),
    );
    expect(tester.takeException(), isNull);
  ...
  testWidgets('tap moves cursor to the edge of the word it tapped', (WidgetTester tester) async {
    final TextEditingController controller = TextEditingController(text: 'Atwater Peel Sherbrooke Bonaventure');
    await tester.pumpWidget(MaterialApp(home: Material(child: Center(child: TextField(controller: controller)))));
    ...
    // We moved the cursor.
    expect(controller.selection, const TextSelection.collapsed(offset: 7, affinity: TextAffinity.upstream));

    // But don't trigger the toolbar.
    expect(find.byType(CupertinoButton), findsNothing);
  }, variant: const TargetPlatformVariant(<TargetPlatform>{TargetPlatform.iOS}));

  testWidgets('tap with a mouse does not move cursor to the edge of the word', (WidgetTester tester) async {
    final TextEditingController controller = TextEditingController(text: 'Atwater Peel Sherbrooke Bonaventure');
    await tester.pumpWidget(MaterialApp(home: Material(child: Center(child: TextField(controller: controller)))));
    ...
    final TestGesture gesture =
        await tester.startGesture(textfieldStart + const Offset(50.0, 9.0), pointer: 1, kind: PointerDeviceKind.mouse);
    await gesture.up();
  ...
  testWidgets('Tapping on a collapsed selection toggles the toolbar', (WidgetTester tester) async {
    ...
    // On iOS/iPadOS, during a tap we select the edge of the word closest to the tap.
    await tester.pumpWidget(
      MaterialApp(home: Material(child: Center(child: TextField(controller: controller, maxLines: 2)))),
    );

    final double lineHeight = findRenderEditable(tester).preferredLineHeight;
    final Offset begPos = textOffsetToPosition(tester, 0);
    final Offset endPos = textOffsetToPosition(tester, 35) +
        const Offset(200.0, 0.0); // Index of 'Bonaventure|' + Offset(200.0,0), which is at the end of the first line.
  ...
  testWidgets('double tap selects word and first tap of double tap moves cursor (iOS)', (WidgetTester tester) async {
    final TextEditingController controller = TextEditingController(text: 'Atwater Peel Sherbrooke Bonaventure');
    await tester.pumpWidget(MaterialApp(home: Material(child: Center(child: TextField(controller: controller)))));
    ...
    await tester.pump(const Duration(milliseconds: 50));
    // First tap moved the cursor.
    expect(controller.selection, const TextSelection.collapsed(offset: 12, affinity: TextAffinity.upstream));
    await tester.tapAt(pPos);
    await tester.pumpAndSettle();
  ...
  testWidgets('double tap does not select word on read-only obscured field', (WidgetTester tester) async {
    final TextEditingController controller = TextEditingController(text: 'Atwater Peel Sherbrooke Bonaventure');
    await tester.pumpWidget(MaterialApp(
      home: Material(child: Center(child: TextField(obscureText: true, readOnly: true, controller: controller))),
    ));
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(MaterialApp(
        home: Material(
          child: Center(child: TextField(controller: controller, selectionControls: materialTextSelectionControls)),
        ),
      ));
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(MaterialApp(
        home: Material(
          child: Center(child: TextField(controller: controller, selectionControls: cupertinoTextSelectionControls)),
        ),
      ));
  ...
  testWidgets('selectionControls is passed to EditableText', (WidgetTester tester) async {
    await tester.pumpWidget(
      MaterialApp(home: Material(child: Scaffold(body: TextField(selectionControls: materialTextSelectionControls)))),
    );
  ...
  testWidgets('Can double click + drag with a mouse to select word by word', (WidgetTester tester) async {
    final TextEditingController controller = TextEditingController();
  ...
  group('Triple tap/click', () {
    ...
    testWidgets('Can triple tap to select a paragraph on mobile platforms when tapping at a word edge', (
      WidgetTester tester,
    ) async {
      ...
      await tester.pumpWidget(MaterialApp(
        theme: ThemeData(useMaterial3: false),
        home: Material(
          child: TextField(dragStartBehavior: DragStartBehavior.down, controller: controller, maxLines: null),
        ),
      ));
    ...
    testWidgets('Can triple tap to select a paragraph on mobile platforms', (WidgetTester tester) async {
      final TextEditingController controller = TextEditingController();
      final bool isTargetPlatformApple = defaultTargetPlatform == TargetPlatform.iOS;
    ...
    testWidgets('triple tap chains work on Non-Apple mobile platforms', (WidgetTester tester) async {
      final TextEditingController controller = TextEditingController(text: 'Atwater Peel Sherbrooke Bonaventure');
      await tester.pumpWidget(MaterialApp(
        theme: ThemeData(useMaterial3: false),
    ...
    testWidgets('triple tap chains work on Apple platforms', (WidgetTester tester) async {
      ...
      await tester.pump(const Duration(milliseconds: 50));
      // First tap moved the cursor and hid the toolbar.
      expect(controller.selection, const TextSelection.collapsed(offset: 50, affinity: TextAffinity.upstream));
      expect(find.byType(CupertinoButton), findsNothing);
      // Second tap selects the word.
      await tester.tapAt(textfieldStart + const Offset(150.0, 50.0));
      await tester.pump();
    ...
    testWidgets('triple click chains work', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(home: Material(child: Center(child: TextField(controller: controller, maxLines: null)))),
      );
      ...
      await tester.pumpAndSettle(kDoubleTapTimeout);
      expect(controller.selection, TextSelection(baseOffset: 0, extentOffset: platformSelectsByLine ? 19 : 20));
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        await tester.pumpWidget(MaterialApp(
          home: Material(child: TextField(dragStartBehavior: DragStartBehavior.down, controller: controller)),
        ));
        ...
        final TestGesture gesture = await tester.startGesture(firstLinePos, pointer: 7, kind: PointerDeviceKind.mouse);
        await tester.pump();
        await gesture.up();
        await tester.pump();
    ...
    testWidgets('Can triple click + drag to select line by line on Linux', (WidgetTester tester) async {
      final TextEditingController controller = TextEditingController();
    ...
    testWidgets('Can triple click + drag to select paragraph by paragraph', (WidgetTester tester) async {
      final TextEditingController controller = TextEditingController();
    ...
    testWidgets('Going past triple click retains the selection on Apple platforms', (WidgetTester tester) async {
      final TextEditingController controller = TextEditingController(text: testValueA);
      await tester.pumpWidget(
        MaterialApp(home: Material(child: Center(child: TextField(controller: controller, maxLines: null)))),
      );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final TestGesture gesture =
          await tester.startGesture(midBlah2, kind: PointerDeviceKind.mouse, buttons: kSecondaryMouseButton);
      await tester.pump();
      await gesture.up();
      await tester.pumpAndSettle();
      ...
      final TestGesture gesture2 = await tester.startGesture(midBlah1, kind: PointerDeviceKind.mouse);
      await tester.pump();
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      final TextEditingController controller = TextEditingController(text: '');
      await tester.pumpWidget(
        MaterialApp(home: Material(child: Center(child: TextField(autofocus: true, controller: controller)))),
      );
  ...
  testWidgets('double tap hold selects word', (WidgetTester tester) async {
    ...
    await tester.tapAt(textfieldStart + const Offset(150.0, 9.0));
    await tester.pump(const Duration(milliseconds: 50));
    final TestGesture gesture = await tester.startGesture(textfieldStart + const Offset(150.0, 9.0));
    // Hold the press.
    await tester.pumpAndSettle();
  ...
  testWidgets('tap after a double tap select is not affected', (WidgetTester tester) async {
    final TextEditingController controller = TextEditingController(text: 'Atwater Peel Sherbrooke Bonaventure');
    // On iOS/iPadOS, during a tap we select the edge of the word closest to the tap.
    // On macOS, we select the precise position of the tap.
    final bool isTargetPlatformMobile = defaultTargetPlatform == TargetPlatform.iOS;
  ...
  testWidgets('long press moves cursor to the exact long press position and shows toolbar when the field is focused', (
    WidgetTester tester,
  ) async {
    final TextEditingController controller = TextEditingController(text: 'Atwater Peel Sherbrooke Bonaventure');
    await tester.pumpWidget(
    ...
    expect(find.byType(CupertinoButton), isContextMenuProvidedByPlatform ? findsNothing : findsNWidgets(buttons));
  }, variant: const TargetPlatformVariant(<TargetPlatform>{TargetPlatform.iOS, TargetPlatform.macOS}));
  ...
  testWidgets('long press drag on a focused TextField moves the cursor under the drag and shows toolbar on lift', (
    WidgetTester tester,
  ) async {
    final TextEditingController controller = TextEditingController(text: 'Atwater Peel Sherbrooke Bonaventure');
    await tester.pumpWidget(
    ...
    final TestGesture gesture = await tester.startGesture(textfieldStart + const Offset(50.0, 9.0));
    await tester.pump(const Duration(milliseconds: 500));
  ...
  testWidgets('long press drag on an unfocused TextField selects word-by-word and shows toolbar on lift', (
    WidgetTester tester,
  ) async {
    final TextEditingController controller = TextEditingController(text: 'Atwater Peel Sherbrooke Bonaventure');
    await tester.pumpWidget(MaterialApp(home: Material(child: Center(child: TextField(controller: controller)))));
  ...
  testWidgets('long press drag can edge scroll on Apple platforms - unfocused TextField', (WidgetTester tester) async {
    ...
    expect(controller.selection, const TextSelection(baseOffset: 0, extentOffset: 7, affinity: TextAffinity.upstream));
    expect(find.byType(CupertinoButton), findsNothing);
    ...
    // The selection isn't affected by the gesture lift.
    expect(controller.selection, const TextSelection(baseOffset: 0, extentOffset: 66, affinity: TextAffinity.upstream));
    // The toolbar now shows up.
    expect(find.byType(CupertinoButton), isContextMenuProvidedByPlatform ? findsNothing : findsNWidgets(3));
  ...
  testWidgets('long press drag can edge scroll on Apple platforms - focused TextField', (WidgetTester tester) async {
    ...
    expect(controller.selection, const TextSelection.collapsed(offset: 19, affinity: TextAffinity.upstream));
    expect(find.byType(CupertinoButton), findsNothing);
    ...
    // The selection isn't affected by the gesture lift.
    expect(controller.selection, const TextSelection.collapsed(offset: 66, affinity: TextAffinity.upstream));
    // The toolbar now shows up.
    final int buttons = defaultTargetPlatform == TargetPlatform.iOS ? 2 : 1;
    expect(find.byType(CupertinoButton), isContextMenuProvidedByPlatform ? findsNothing : findsNWidgets(buttons));
  ...
  testWidgets('mouse click and drag can edge scroll', (WidgetTester tester) async {
    ...
    final TestGesture gesture =
        await tester.startGesture(textOffsetToPosition(tester, 19), pointer: 7, kind: PointerDeviceKind.mouse);
    ...
    await tester.pumpAndSettle();
    expect(controller.selection, const TextSelection(baseOffset: 19, extentOffset: 66)); // We're at the edge now.
    expect(find.byType(CupertinoButton), findsNothing);
    ...
    // The last character is now on screen near the right edge.
    expect(textOffsetToPosition(tester, 66).dx, moreOrLessEquals(TestSemantics.fullScreen.width, epsilon: 2.0));
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpAndSettle();
      expect(controller.selection, const TextSelection.collapsed(offset: 66)); // We're at the edge now.
  ...
  testWidgets('long press drag can edge scroll vertically', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(
      home: Material(child: Center(child: TextField(autofocus: true, maxLines: 2, controller: controller))),
    ));
    ...
    expect(textOffsetToPosition(tester, textLength).dy, moreOrLessEquals(firstCharY + lineHeight * 2, epsilon: 1));
    ...
    expect(textOffsetToPosition(tester, 65).dy, moreOrLessEquals(firstCharY + lineHeight, epsilon: 1));
    ...
    // down by one line.
    expect(textOffsetToPosition(tester, 110).dy, moreOrLessEquals(firstCharY + lineHeight, epsilon: 1));

    // Likewise, the first character is now scrolled out of the top of the field
    // by one line.
    expect(textOffsetToPosition(tester, 0).dy, moreOrLessEquals(firstCharY - lineHeight, epsilon: 1));
  }, variant: const TargetPlatformVariant(<TargetPlatform>{TargetPlatform.iOS, TargetPlatform.macOS}));
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(home: Material(child: Center(child: TextField(maxLines: 2, controller: controller)))),
      );
      ...
      // Still hasn't scrolled.
      expect(textOffsetToPosition(tester, 66).dy, moreOrLessEquals(firstCharY + lineHeight, epsilon: 1));
      ...
      // the field, it's now only two line heights down, because it has scrolled
      // down by one line.
      expect(textOffsetToPosition(tester, textLength).dy, moreOrLessEquals(firstCharY + lineHeight, epsilon: 1));
  ...
  testWidgets('mouse click and drag can edge scroll vertically', (WidgetTester tester) async {
    ...
    // Still hasn't scrolled.
    expect(textOffsetToPosition(tester, 60).dy, moreOrLessEquals(firstCharY + lineHeight, epsilon: 1));
  ...
  testWidgets('double click after a click on desktop platforms', (WidgetTester tester) async {
    ...
    final TestGesture gesture =
        await tester.startGesture(textFieldStart + const Offset(50.0, 9.0), pointer: 7, kind: PointerDeviceKind.mouse);
    await tester.pump();
    await gesture.up();
    await tester.pumpAndSettle(kDoubleTapTimeout);
    expect(controller.selection, const TextSelection.collapsed(offset: 3));
  ...
  testWidgets('double tapping a space selects the previous word on iOS', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(home: Material(child: Center(child: TextField(maxLines: null, controller: controller)))),
    );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      // Put the cursor at the end of the field.
      final TestGesture gesture =
          await tester.startGesture(textOffsetToPosition(tester, 10), pointer: 7, kind: PointerDeviceKind.mouse);
      await tester.pump();
      await gesture.up();
      expect(controller.value.selection, isNotNull);
      expect(controller.value.selection.baseOffset, 10);
  ...
  testWidgets('Force press does not set selection on Android or Fuchsia touch devices', (WidgetTester tester) async {
    ...
    await gesture.downWithCustomEvent(
      offset,
      PointerDownEvent(pointer: pointerValue, position: offset, pressure: 0.0, pressureMax: 6.0, pressureMin: 0.0),
    );
  ...
  testWidgets('tap on non-force-press-supported devices work', (WidgetTester tester) async {
    ...
    final Offset offset = textfieldStart + const Offset(150.0, 9.0);
    final TestGesture gesture = await tester.createGesture();
    await gesture.downWithCustomEvent(offset, PointerDownEvent(
      pointer: pointerValue,
      position: offset,
  ...
  testWidgets('strut TextStyle increases height', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(
      theme: ThemeData(platform: TargetPlatform.android),
      home: const Material(
        child: Center(child: TextField(style: TextStyle(fontSize: 20), strutStyle: StrutStyle.disabled)),
      ),
    ));
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      await tester.pumpWidget(MaterialApp(
        theme: ThemeData(platform: TargetPlatform.android, useMaterial3: false),
        home: const Material(child: Center(child: TextField(maxLines: 6, strutStyle: StrutStyle(fontSize: 25)))),
      ));
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      await tester.pumpWidget(MaterialApp(
        theme: ThemeData(platform: TargetPlatform.android, useMaterial3: false),
        home: const Material(
          child: Center(child: TextField(maxLines: 3, strutStyle: StrutStyle(fontSize: 8, forceStrutHeight: true))),
        ),
      ));
  ...
  testWidgets('Caret center position', (WidgetTester tester) async {
    await tester.pumpWidget(overlay(
      child: Theme(
        data: ThemeData(useMaterial3: false),
        child: const SizedBox(width: 300.0, child: TextField(textAlign: TextAlign.center, decoration: null)),
      ),
    ));
    ...
    Offset topLeft = editable.localToGlobal(editable.getLocalRectForCaret(const TextPosition(offset: 4)).topLeft);
    expect(topLeft.dx, equals(431));

    topLeft = editable.localToGlobal(editable.getLocalRectForCaret(const TextPosition(offset: 3)).topLeft);
    expect(topLeft.dx, equals(415));

    topLeft = editable.localToGlobal(editable.getLocalRectForCaret(const TextPosition(offset: 2)).topLeft);
    expect(topLeft.dx, equals(399));

    topLeft = editable.localToGlobal(editable.getLocalRectForCaret(const TextPosition(offset: 1)).topLeft);
  ...
  testWidgets('Caret indexes into trailing whitespace center align', (WidgetTester tester) async {
    ...
    Offset topLeft = editable.localToGlobal(editable.getLocalRectForCaret(const TextPosition(offset: 7)).topLeft);
    expect(topLeft.dx, equals(479));

    topLeft = editable.localToGlobal(editable.getLocalRectForCaret(const TextPosition(offset: 8)).topLeft);
    expect(topLeft.dx, equals(495));

    topLeft = editable.localToGlobal(editable.getLocalRectForCaret(const TextPosition(offset: 4)).topLeft);
    expect(topLeft.dx, equals(431));
  ...
  testWidgets('iPad Scribble selection change shows selection handles', (WidgetTester tester) async {
    ...
    tester.testTextInput.updateEditingValue(
      const TextEditingValue(text: testText, selection: TextSelection(baseOffset: 2, extentOffset: 7)),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('Tap in empty text field does not show handles nor toolbar', (WidgetTester tester) async {
    final TextEditingController controller = TextEditingController();
  ...
  testWidgets('Long press in empty text field shows handles and toolbar', (WidgetTester tester) async {
    final TextEditingController controller = TextEditingController();
  ...
  testWidgets('Double tap in empty text field shows toolbar but not handles', (WidgetTester tester) async {
    final TextEditingController controller = TextEditingController();
  ...
  testWidgets('Mouse tap does not show handles nor toolbar', (WidgetTester tester) async {
    ...
    final TestGesture gesture = await tester.startGesture(textFieldPos, pointer: 7, kind: PointerDeviceKind.mouse);
    await tester.pump();
    await gesture.up();
    await tester.pump();
  ...
  testWidgets('Does not show handles when updated from the web engine', (WidgetTester tester) async {
    ...
    final TestGesture gesture =
        await tester.startGesture(topLeft + const Offset(0.0, 5.0), kind: PointerDeviceKind.mouse);
    await tester.pump(const Duration(milliseconds: 50));
    await gesture.up();
    await tester.pumpAndSettle();
    ...
    if (kIsWeb) {
      tester.testTextInput.updateEditingValue(
        const TextEditingValue(text: 'abc def ghi', selection: TextSelection(baseOffset: 2, extentOffset: 7)),
      );
      // Wait for all the `setState` calls to be flushed.
      await tester.pumpAndSettle();
      expect(state.currentTextEditingValue.selection, const TextSelection(baseOffset: 2, extentOffset: 7));
      expect(state.selectionOverlay!.handlesAreVisible, isFalse);
    }
  });
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      tester.testTextInput.updateEditingValue(
        TextEditingValue(text: testValue, selection: TextSelection.collapsed(offset: testValue.length)),
      );
      await tester.pump();
  ...
  group('height', () {
    testWidgets('By default, TextField is at least kMinInteractiveDimension high', (WidgetTester tester) async {
      await tester.pumpWidget(MaterialApp(theme: ThemeData(), home: const Scaffold(body: Center(child: TextField()))));
    ...
    testWidgets('When isDense, TextField can go below kMinInteractiveDimension height', (WidgetTester tester) async {
      await tester.pumpWidget(MaterialApp(
        theme: ThemeData(),
        home: const Scaffold(body: Center(child: TextField(decoration: InputDecoration(isDense: true)))),
      ));
      ...
      home: Scaffold(
        body: Center(
          child: Column(
            ...
            mainAxisSize: MainAxisSize.min,
            children: <Widget>[
              SizedBox(width: 100.0, child: TextField(controller: controller1, focusNode: focusNode1)),
              Row(
                mainAxisAlignment: MainAxisAlignment.center,
                mainAxisSize: MainAxisSize.min,
                children: <Widget>[
                  SizedBox(width: 100.0, child: TextField(controller: controller2, focusNode: focusNode2)),
                  SizedBox(width: 100.0, child: TextField(controller: controller3, focusNode: focusNode3)),
                  SizedBox(width: 100.0, child: TextField(controller: controller4, focusNode: focusNode4)),
                ],
              ),
              SizedBox(width: 100.0, child: TextField(controller: controller5, focusNode: focusNode5)),
            ],
          ),
        ),
      ),
        ...
        body: Column(
          children: <Widget>[
            ...
            TextField(
              ...
              maxLength: 1,
              buildCounter:
                  (BuildContext context, {required int currentLength, required bool isFocused, int? maxLength}) => null,
            ),
          ],
        ),
      ),
  ...
  testWidgets('The selection menu displays in an Overlay without error', (WidgetTester tester) async {
    // This is a regression test for
            ...
            child: Container(
              ...
              child: Overlay(
                initialEntries: <OverlayEntry>[
                  OverlayEntry(builder: (BuildContext context) => Center(child: TextField(controller: controller))),
                ],
              ),
            ),
          ),
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(MaterialApp(
        home: Material(
          child: MouseRegion(cursor: SystemMouseCursors.forbidden, child: TextField(controller: controller)),
        ),
      ));
  ...
  testWidgets('Caret rtl with changing width', (WidgetTester tester) async {
      ...
      child: StatefulBuilder(
        builder: (BuildContext context, StateSetter setter) {
          ...
          return SizedBox(
            ...
            child: TextField(key: textFieldKey, controller: controller, textDirection: TextDirection.rtl),
          );
        },
      ),
    ));
    ...
    RenderEditable editable = findRenderEditable(tester);
    double cursorRight = editable.getLocalRectForCaret(TextPosition(offset: controller.value.text.length)).topRight.dx;
    ...
    await tester.pump();
    editable = findRenderEditable(tester);
    cursorRight = editable.getLocalRectForCaret(TextPosition(offset: controller.value.text.length)).topRight.dx;
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final String selectAll = defaultTargetPlatform == TargetPlatform.macOS ? 'Select All' : 'Select all';
  ...
  testWidgets('TextFields can inherit [FloatingLabelBehaviour] from InputDecorationTheme.', (
    ...
    Widget textFieldBuilder({FloatingLabelBehavior behavior = FloatingLabelBehavior.auto}) {
      return MaterialApp(
        theme:
            ThemeData(useMaterial3: false, inputDecorationTheme: InputDecorationTheme(floatingLabelBehavior: behavior)),
        home: Scaffold(body: TextField(focusNode: focusNode, decoration: const InputDecoration(labelText: 'Label'))),
      );
    }
  ...
  testWidgets('autofill info has hint text', (WidgetTester tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Material(child: Center(child: TextField(decoration: InputDecoration(hintText: 'placeholder text')))),
      ),
    );

    await tester.tap(find.byType(TextField));

    expect(tester.testTextInput.setClientArgs?['autofill'], containsPair('hintText', 'placeholder text'));
  });
  ...
  testWidgets('Focused TextField does not push any layers with alwaysNeedsAddToScene', (WidgetTester tester) async {
    final FocusNode focusNode = FocusNode();
    await tester.pumpWidget(MaterialApp(home: Material(child: Center(child: TextField(focusNode: focusNode)))));
    await tester.showKeyboard(find.byType(TextField));
  ...
  testWidgets('cursor blinking respects TickerMode', (WidgetTester tester) async {
    ...
    Widget builder({required bool tickerMode}) {
      return MaterialApp(
        home: Material(child: Center(child: TickerMode(enabled: tickerMode, child: TextField(focusNode: focusNode)))),
      );
    }
  ...
  testWidgets('shift tapping an unfocused field', (WidgetTester tester) async {
    ...
    final FocusNode focusNode = FocusNode();
    await tester.pumpWidget(
      MaterialApp(home: Material(child: Center(child: TextField(controller: controller, focusNode: focusNode)))),
    );
    expect(focusNode.hasFocus, isFalse);
  ...
  testWidgets('can shift + tap + drag to select with a keyboard (Apple platforms)', (WidgetTester tester) async {
    ...
    await tester.sendKeyDownEvent(LogicalKeyboardKey.shift);
    final TestGesture gesture =
        await tester.startGesture(textOffsetToPosition(tester, 23), pointer: 7, kind: PointerDeviceKind.mouse);
  ...
  testWidgets('can shift + tap + drag to select with a keyboard, reversed (Apple platforms)', (
    WidgetTester tester,
  ) async {
    final TextEditingController controller = TextEditingController(text: 'Atwater Peel Sherbrooke Bonaventure');
    final bool isTargetPlatformMobile = defaultTargetPlatform == TargetPlatform.iOS;
    ...
    await tester.sendKeyDownEvent(LogicalKeyboardKey.shift);
    final TestGesture gesture =
        await tester.startGesture(textOffsetToPosition(tester, 8), pointer: 7, kind: PointerDeviceKind.mouse);
    if (isTargetPlatformMobile) {
      await gesture.up();
    }
    await tester.pumpAndSettle();
  ...
  testWidgets('Cannot request focus when canRequestFocus is false', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(boilerplate(child: TextField(focusNode: focusNode, canRequestFocus: false)));
  ...
  group('Right click focus', () {
    ...
    testWidgets('Right clicking cannot request focus if canRequestFocus is false', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(MaterialApp(
        home: Material(
          child: Column(children: <Widget>[TextField(key: key, focusNode: focusNode, canRequestFocus: false)]),
        ),
      ));
  ...
  group('context menu', () {
    testWidgets(
      ...
      (WidgetTester tester) async {
        final TextEditingController controller = TextEditingController(text: '');
        await tester.pumpWidget(
          MaterialApp(home: Material(child: Column(children: <Widget>[TextField(controller: controller)]))),
        );
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        await tester.pumpWidget(
          MaterialApp(home: Material(child: TextField(key: key, controller: controller, contextMenuBuilder: null))),
        );
  ...
  group('magnifier builder', () {
    ...
    group('defaults', () {
      testWidgets('should build Magnifier on Android', (WidgetTester tester) async {
        ...
        expect(editableText.magnifierConfiguration.magnifierBuilder(
          context,
          MagnifierController(),
          ValueNotifier<MagnifierInfo>(MagnifierInfo.empty),
        ), isA<TextMagnifier>());
      ...
      testWidgets('should build nothing on Android and iOS', (WidgetTester tester) async {
        await tester.pumpWidget(const MaterialApp(home: Scaffold(body: TextField())));
      ...
      }, variant: TargetPlatformVariant.all(excluding: <TargetPlatform>{TargetPlatform.iOS, TargetPlatform.android}));
    });
  });
      ...
      await tester.pumpWidget(overlay(
        child: TextField(
          ...
          magnifierConfiguration: TextMagnifierConfiguration(
            magnifierBuilder: (_, MagnifierController controller, ValueNotifier<MagnifierInfo> localMagnifierInfo) {
  ...
  group('TapRegion integration', () {
            ...
            child: SizedBox(
              ...
              child: Opacity(
                ...
                child: TextField(
                  autofocus: true,
                  focusNode: focusNode,
                  decoration: const InputDecoration(hintText: 'Placeholder', border: OutlineInputBorder()),
                ),
              ),
            ),
          ),
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        // Right clicking shows the menu.
        final TestGesture gesture =
            await tester.startGesture(aPosition, kind: PointerDeviceKind.mouse, buttons: kSecondaryMouseButton);
        await tester.pump();
        await gesture.up();
        await tester.pumpAndSettle();
    ...
    for (final PointerDeviceKind pointerDeviceKind in PointerDeviceKind.values.toSet()
      ...
      testWidgets(
        ...
        (WidgetTester tester) async {
          ...
          await tester.pumpWidget(MaterialApp(
            home: Scaffold(
              body: Column(children: <Widget>[const Text('Outside'), TextField(autofocus: true, focusNode: focusNode)]),
            ),
          ));
          await tester.pump();

          Future<void> click(Finder finder) async {
            final TestGesture gesture = await tester.startGesture(tester.getCenter(finder), kind: pointerDeviceKind);
            await gesture.up();
            await gesture.removePointer();
          }
        ...
        body: Center(
          child: Builder(
            builder: (BuildContext context) {
              ...
              return const TextField(autofocus: true, spellCheckConfiguration: SpellCheckConfiguration());
            },
          ),
        ),
      ),
  ...
  testWidgets('Builds the corresponding default spell check configuration by platform', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(
          body: Center(child: TextField(autofocus: true, spellCheckConfiguration: SpellCheckConfiguration())),
        ),
      ),
    );
    ...
    expect(editableTextState.spellCheckConfiguration.misspelledTextStyle, expectedConfiguration.misspelledTextStyle);
    expect(
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      final TextEditingController controller = TextEditingController(text: 'blah1 blah2');
      await tester.pumpWidget(MaterialApp(home: Scaffold(body: Center(child: TextField(controller: controller)))));
      ...
      gesture = await tester.startGesture(textOffsetToPosition(tester, 2), kind: PointerDeviceKind.mouse);
      await tester.pump();
