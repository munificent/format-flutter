...
void main() {
  ...
  group('canSelectAll', () {
    Widget createEditableText({required Key key, String? text, TextSelection? selection}) {
      final TextEditingController controller = TextEditingController(text: text)..selection =
    ...
    testWidgets('should return true when there is text and partial uncollapsed selection', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        createEditableText(key: key, text: '123', selection: const TextSelection(baseOffset: 1, extentOffset: 2)),
      );
      expect(materialTextSelectionControls.canSelectAll(key.currentState!), true);
    });
    ...
    testWidgets('should return false when there is text and full selection', (WidgetTester tester) async {
      final GlobalKey<EditableTextState> key = GlobalKey();
      await tester.pumpWidget(
        createEditableText(key: key, text: '123', selection: const TextSelection(baseOffset: 0, extentOffset: 3)),
      );
      expect(materialTextSelectionControls.canSelectAll(key.currentState!), false);
    });
  });
      ...
      (WidgetTester tester) async {
        ...
        await tester.pumpWidget(MaterialApp(
          ...
          home: Directionality(
            ...
            child: MediaQuery(
              data: const MediaQueryData(size: Size(800.0, 600.0)),
              child: Align(alignment: Alignment.topLeft, child: Material(child: TextField(controller: controller))),
            ),
          ),
        ));
  ...
  group('menu position', () {
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        final Offset selectionOffset = tester.getTopLeft(find.byType(TextSelectionToolbarTextButton).first);
  ...
  group('material handles', () {
    testWidgets('draws transparent handle correctly', (WidgetTester tester) async {
      await tester.pumpWidget(RepaintBoundary(
        child: Theme(
          data: ThemeData(textSelectionTheme: const TextSelectionThemeData(selectionHandleColor: Color(0x550000AA))),
          child: Builder(
            builder: (BuildContext context) {
              return Container(
                ...
                child: Padding(
                  ...
                  child: FittedBox(
                    child: materialTextSelectionControls.buildHandle(context, TextSelectionHandleType.right, 10.0),
                  ),
                ),
              );
            },
