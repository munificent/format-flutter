...
void main() {
  ...
  testWidgets('Tooltip margin - ThemeData', (WidgetTester tester) async {
    ...
    ).parent!.parent!.parent!.parent!.parent! as RenderBox;
    final RenderBox tooltipContent = tester.renderObject(
      find.text(tooltipText),
    );

    final Offset topLeftTipInGlobal = tip.localToGlobal(
      tip.size.topLeft(Offset.zero),
    );
    ...
    final Offset topRightTipInGlobal = tip.localToGlobal(
    ...
    final Offset bottomLeftTipInGlobal = tip.localToGlobal(
      tip.size.bottomLeft(Offset.zero),
    );
    ...
    final Offset bottomRightTipInGlobal = tip.localToGlobal(
      tip.size.bottomRight(Offset.zero),
    );
  ...
  testWidgets(
    'Tooltip message textAlign - TooltipTheme',
    (WidgetTester tester) async {
  ...
  testWidgets(
    'Tooltip decoration - ThemeData.tooltipTheme',
    (WidgetTester tester) async {
      final GlobalKey key = GlobalKey();
      ...
      final RenderBox tip = tester.renderObject(
        find.text(tooltipText),
      ).parent!.parent!.parent!.parent! as RenderBox;
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(MaterialApp(
        home: Theme(
          data: ThemeData(
            tooltipTheme:
                const TooltipThemeData(waitDuration: customWaitDuration),
          ),
          child: const Center(
            child: Tooltip(
              message: tooltipText,
      ...
      expect(
        find.text(tooltipText),
        findsOneWidget,
      ); // Should appear after customWaitDuration
  ...
  testWidgets(
    'Tooltip waitDuration - TooltipTheme',
    (WidgetTester tester) async {
      const Duration customWaitDuration = Duration(milliseconds: 500);
      final TestGesture gesture =
          await tester.createGesture(kind: PointerDeviceKind.mouse);
  ...
  testWidgets(
    'Tooltip showDuration - ThemeData.tooltipTheme',
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(MaterialApp(
        home: Theme(
          data: ThemeData(
            tooltipTheme:
                const TooltipThemeData(showDuration: customShowDuration),
          ),
          child: const Center(
            child: Tooltip(
              message: tooltipText,
  ...
  testWidgets(
    'Tooltip showDuration - TooltipTheme',
    (WidgetTester tester) async {
      const Duration customShowDuration = Duration(milliseconds: 3000);
      await tester.pumpWidget(
        const MaterialApp(
  ...
  testWidgets(
    'Semantics excluded - ThemeData.tooltipTheme',
    (WidgetTester tester) async {
      final SemanticsTester semantics = SemanticsTester(tester);
