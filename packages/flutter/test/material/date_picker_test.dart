...
void main() {
  ...
  group('showDatePicker Dialog', () {
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        expect(
          defaultDialogMaterial.elevation,
        ...
        await tester.pumpWidget(MaterialApp(
          theme: ThemeData.fallback(useMaterial3: false).copyWith(
  ...
  group('Calendar mode', () {
    testWidgets('Default Calendar mode layout (Landscape)', (
      ...
      const EdgeInsets insetPadding = EdgeInsets.symmetric(
        horizontal: 16.0,
        vertical: 24.0,
      );
      ...
      final Offset dialogTopLeft = tester.getTopLeft(
      ...
      expect(
        subHeaderTextTopLeft.dy,
        dialogTopLeft.dy + 16.0 - (hasIssue99933 ? 0.5 : 0.0),
      );
      ...
      final Offset calendarPageViewTopLeft = tester.getTopLeft(
        calendarPageView,
      );
      final Offset subHeaderTextBottomLeft = tester.getBottomLeft(
        subHeaderText,
      );
      expect(calendarPageViewTopLeft.dx, dividerTopRight.dx);
      expect(
        calendarPageViewTopLeft.dy,
        subHeaderTextBottomLeft.dy + 16.0 - (hasIssue99933 ? 0.5 : 0.0),
      );

      // Test month navigation icons position.
      final Finder previousMonthButton = find.widgetWithIcon(
        IconButton,
        Icons.chevron_left,
      );
      final Finder nextMonthButton = find.widgetWithIcon(
        IconButton,
        Icons.chevron_right,
      );
      final Offset previousMonthButtonTopRight = tester.getTopRight(
        previousMonthButton,
      );
      final Offset nextMonthButtonTopRight = tester.getTopRight(
        nextMonthButton,
      );
      final Offset dialogTopRight = tester.getTopRight(
        find.byType(AnimatedContainer),
      );
      ...
      final Offset dialogBottomRight = tester.getBottomRight(
        find.byType(AnimatedContainer),
      );
      final Offset okButtonTopRight = tester.getTopRight(
        find.widgetWithText(TextButton, 'OK'),
      );
      final Offset cancelButtonTopRight = tester.getTopRight(
        find.widgetWithText(TextButton, 'Cancel'),
      );
      final Offset calendarPageViewBottomRight = tester.getBottomRight(
        calendarPageView,
      );
      expect(okButtonTopRight.dx, dialogBottomRight.dx - 8);
      expect(okButtonTopRight.dy, calendarPageViewBottomRight.dy + 2);
      final Offset okButtonTopLeft = tester.getTopLeft(
        find.widgetWithText(TextButton, 'OK'),
      );
    ...
    testWidgets('Default Calendar mode layout (Portrait)', (
      ...
      const bool hasIssue99933 =
          kIsWeb && !bool.fromEnvironment('FLUTTER_WEB_USE_SKIA');
      expect(
        headerTextTextTopLeft.dy,
        helpTextBottomLeft.dy + 28.0 - (hasIssue99933 ? 1.0 : 0.0),
      );
      ...
      expect(
        subHeaderTextTopLeft.dy,
        dividerBottomLeft.dy + 16.0 - (hasIssue99933 ? 0.5 : 0.0),
      );
    ...
    testWidgets('Cannot select disabled year', (WidgetTester tester) async {
      ...
      await prepareDatePicker(tester, (Future<DateTime?> date) async {
        ...
        await tester.tap(
          find.text('2016'),
        ); // Disabled, doesn't change the year.
        await tester.pumpAndSettle();
        await tester.tap(find.text('OK'));
        await tester.pumpAndSettle();
  ...
  group('showDatePicker avoids overlapping display features', () {
    testWidgets('positioning with anchorPoint', (WidgetTester tester) async {
      ...
      expect(
        tester.getTopLeft(find.byType(DatePickerDialog)),
        const Offset(410.0, 0.0),
      );
      expect(
        tester.getBottomRight(find.byType(DatePickerDialog)),
  ...
  group('Material 2', () {
    ...
    group('showDatePicker Dialog', () {
      testWidgets('Default dialog size', (WidgetTester tester) async {
        ...
        Size dialogContainerSize = tester.getSize(
          find.byType(AnimatedContainer),
        );
        expect(dialogContainerSize, calendarLandscapeDialogSize);
