...
class TallSliderTickMarkShape extends SliderTickMarkShape {
  ...
  void paint(
    ...
    context.canvas.drawRect(
      Rect.fromLTWH(offset.dx, offset.dy, 10.0, 20.0),
      paint,
    );
  }
}
...
void main() {
  ...
  testWidgets(
    'discrete Slider repaints when dragged',
    (WidgetTester tester) async {
      final Key sliderKey = UniqueKey();
      double value = 0.0;
      final List<Offset> log = <Offset>[];
  ...
  testWidgets(
    'Slider can tap in vertical scroller',
    (WidgetTester tester) async {
      double value = 0.0;
      await tester.pumpWidget(MaterialApp(
        home: Directionality(
    ...
    await tester.pumpWidget(
      const MaterialApp(
        home: Directionality(
          textDirection: TextDirection.ltr,
          child: Material(
            child: Center(child: Slider(value: 0.5, onChanged: null)),
          ),
        ),
      ),
    );
    ...
    await tester.pumpWidget(
      const MaterialApp(
        home: Directionality(
          ...
          child: Material(
            child: Center(
              child: IntrinsicWidth(child: Slider(value: 0.5, onChanged: null)),
            ),
          ),
        ),
      ),
  ...
  testWidgets(
    'Slider has correct animations when reparented',
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(MaterialApp(
        home: Theme(
          ...
          child: Directionality(
            textDirection: TextDirection.ltr,
            child: Material(
              child: Slider(value: 100.0, max: 200.0, onChanged: (double v) {}),
            ),
          ),
        ),
      ));
  ...
  testWidgets(
    'Slider semantics with custom formatter',
    (WidgetTester tester) async {
      final SemanticsTester semantics = SemanticsTester(tester);
  ...
  testWidgets('Slider is draggable and has correct dragged color', (
    WidgetTester tester,
  ) async {
    tester.binding.focusManager.highlightStrategy =
        FocusHighlightStrategy.alwaysTraditional;
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final RenderObject valueIndicatorBox = tester.renderObject(
        find.byType(Overlay),
      );
      final Offset topRight = tester.getTopRight(find.byType(Slider)).translate(
        -24,
        0,
      );
  ...
  testWidgets(
    'Slider changes mouse cursor when hovered',
    (WidgetTester tester) async {
  ...
  testWidgets('Slider track paints correctly when the shape is rectangular', (
    ...
    await tester.pumpWidget(MaterialApp(
      theme: ThemeData(
        sliderTheme:
            const SliderThemeData(trackShape: RectangularSliderTrackShape()),
      ),
      home: const Directionality(
        textDirection: TextDirection.ltr,
        child:
            Material(child: Center(child: Slider(value: 0.5, onChanged: null))),
      ),
    ));
    ...
    expect(
      ...
      paints
        ..rect(
          rect: const Rect.fromLTRB(24.0, 298.0, 400.0, 302.0),
        ) // active track Rect.
        ..rect(
          rect: const Rect.fromLTRB(400.0, 298.0, 776.0, 302.0),
        ), // inactive track Rect.
    );
  });
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      final Widget sliderAdaptive = MaterialApp(
        theme: ThemeData(platform: TargetPlatform.iOS),
        home: Material(
          child: Slider.adaptive(value: 0, onChanged: (double newValue) {}),
        ),
      );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final TestGesture gesture = await tester.startGesture(
        tester.getRect(find.byType(Slider)).centerLeft,
      );
