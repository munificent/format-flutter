  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(
          body: Center(
            child: InkWell(splashFactory: InkSparkle.splashFactory, child: const Text('Sparkle!'), onTap: () {}),
          ),
        ),
      ));
      final Finder buttonFinder = find.text('Sparkle!');
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(
          body: Center(
            child: InkResponse(splashFactory: InkSparkle.splashFactory, child: const Text('Sparkle!'), onTap: () {}),
          ),
        ),
      ));
      final Finder buttonFinder = find.text('Sparkle!');
...
Future<void> _runTest(WidgetTester tester, String positionName, double distanceFromTopLeft) async {
  ...
  for (int i = 0; i <= 5; i++) {
    ...
    await expectLater(repaintFinder, matchesGoldenFile('ink_sparkle.$positionName.$i.png'));
  }
}
