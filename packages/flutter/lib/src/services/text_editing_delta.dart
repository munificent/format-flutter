...
abstract class TextEditingDelta with Diagnosticable {
  ...
  factory TextEditingDelta.fromJSON(Map<String, dynamic> encoded) {
    ...
    assert(
      _debugTextRangeIsValid(TextRange(
        start: replacementDestinationStart,
        end: replacementDestinationEnd,
      ), oldText),
      'The delta range: ${TextRange(
        start: replacementSourceStart,
        end: replacementSourceEnd,
      )} is not within the bounds of text: $oldText of length: ${oldText.length}',
    );

    final String newText = _replace(oldText, replacementSource, TextRange(
      start: replacementDestinationStart,
      end: replacementDestinationEnd,
    ));
    ...
    if (isEqual) {
      ...
      return TextEditingDeltaDeletion(
        oldText: oldText,
        deletedRange:
            TextRange(start: actualStart, end: replacementDestinationEnd),
        selection: newSelection,
        composing: newComposing,
      );
...
class TextEditingDeltaInsertion extends TextEditingDelta {
  ...
  TextEditingValue apply(TextEditingValue value) {
    ...
    newText = _replace(
      newText,
      textInserted,
      TextRange.collapsed(insertionOffset),
    );
    assert(
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      DiagnosticsProperty<int>('insertionOffset', insertionOffset),
    );
...
class TextEditingDeltaDeletion extends TextEditingDelta {
  ...
  String get textDeleted => oldText.substring(
    deletedRange.start,
    deletedRange.end,
  );
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      DiagnosticsProperty<TextRange>('deletedRange', deletedRange),
    );
    properties.add(DiagnosticsProperty<TextSelection>('selection', selection));
    properties.add(DiagnosticsProperty<TextRange>('composing', composing));
  }
...
class TextEditingDeltaReplacement extends TextEditingDelta {
  ...
  String get textReplaced => oldText.substring(
    replacedRange.start,
    replacedRange.end,
  );
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      DiagnosticsProperty<String>('replacementText', replacementText),
    );
    properties.add(
      DiagnosticsProperty<TextRange>('replacedRange', replacedRange),
    );
    properties.add(DiagnosticsProperty<TextSelection>('selection', selection));
    properties.add(DiagnosticsProperty<TextRange>('composing', composing));
  }
