...
class _ProfiledBinaryMessenger implements BinaryMessenger {
  ...
  @override
  Future<ByteData?>? send(String channel, ByteData? message) => sendWithPostfix(channel, '', message);
...
Future<void> _debugLaunchProfilePlatformChannels() async {
  if (!_debugProfilePlatformChannelsIsRunning) {
    ...
    final List<_PlatformChannelStats> allStats = _debugProfilePlatformChannelsStats.values.toList();
    // Sort highest combined bandwidth first.
    allStats.sort(
      (_PlatformChannelStats x, _PlatformChannelStats y) => (y.upBytes + y.downBytes) - (x.upBytes + x.downBytes),
    );
    for (final _PlatformChannelStats stats in allStats) {
      log.writeln(
        '  (name:"${stats.channel}" type:"${stats.type}" codec:"${stats.codec}" upBytes:${stats.upBytes} upBytes_avg:${stats.averageUpPayload.toStringAsFixed(
          1,
        )} downBytes:${stats.downBytes} downBytes_avg:${stats.averageDownPayload.toStringAsFixed(1)})',
      );
    }
...
void _debugRecordUpStream(String channelTypeName, String name, String codecTypeName, ByteData? bytes) {
  final _PlatformChannelStats stats =
      _debugProfilePlatformChannelsStats[name] ??= _PlatformChannelStats(name, codecTypeName, channelTypeName);
