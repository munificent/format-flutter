...
class _ProfiledBinaryMessenger implements BinaryMessenger {
  ...
  @override
  Future<ByteData?>? send(String channel, ByteData? message) => sendWithPostfix(
    channel,
    '',
    message,
  );
    ...
    for (final _PlatformChannelStats stats in allStats) {
      log.writeln(
        '  (name:"${stats.channel}" type:"${stats.type}" codec:"${stats.codec}" upBytes:${stats.upBytes} upBytes_avg:${stats.averageUpPayload.toStringAsFixed(
          1,
        )} downBytes:${stats.downBytes} downBytes_avg:${stats.averageDownPayload.toStringAsFixed(
          1,
        )})',
      );
    }
...
void _debugRecordUpStream(
  ...
  final _PlatformChannelStats stats =
      _debugProfilePlatformChannelsStats[name] ??= _PlatformChannelStats(
    name,
    codecTypeName,
    channelTypeName,
  );
