...
abstract class KeyEvent with Diagnosticable {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      DiagnosticsProperty<LogicalKeyboardKey>('logicalKey', logicalKey),
    );
...
class HardwareKeyboard {
  ...
  Future<void> syncKeyboardState() async {
    final Map<int, int>? keyboardState = await SystemChannels.keyboard
    ...
    if (keyboardState != null) {
      for (final int key in keyboardState.keys) {
        ...
        final LogicalKeyboardKey logicalKey = LogicalKeyboardKey(
          keyboardState[key]!,
        );
  ...
  bool handleKeyEvent(KeyEvent event) {
    ...
    if (event is KeyDownEvent) {
      _pressedKeys[physicalKey] = logicalKey;
      final KeyboardLockMode? lockMode = KeyboardLockMode.findLockByLogicalKey(
        event.logicalKey,
      );
      if (lockMode != null) {
...
class KeyEventManager {
  ...
  Future<Map<String, dynamic>> handleRawKeyMessage(dynamic message) async {
    ...
    final RawKeyEvent rawEvent = RawKeyEvent.fromMessage(
  ...
  void _convertRawEventAndStore(RawKeyEvent rawEvent) {
    ...
    final LogicalKeyboardKey? recordedLogicalMain = _hardwareKeyboard
        .lookUpLayout(physicalKey);
    final Duration timeStamp =
        ServicesBinding.instance.currentSystemFrameTimeStamp;
    final String? character = rawEvent.character == ''
        ? null
        : rawEvent.character;
    if (rawEvent is RawKeyDownEvent) {
