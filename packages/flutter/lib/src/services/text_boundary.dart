...
class CharacterBoundary extends TextBoundary {
  ...
  int? getTrailingTextBoundaryAt(int position) {
    ...
    final CharacterRange rangeAtPosition = CharacterRange.at(
      _text,
      max(0, position + 1),
    );
  ...
  TextRange getTextBoundaryAt(int position) {
    if (position < 0) {
      ...
      return TextRange(
        start: getLeadingTextBoundaryAt(position) ?? -1,
        end: -1,
      );
    }
...
class LineBoundary extends TextBoundary {
  ...
  @override
  TextRange getTextBoundaryAt(int position) => _textLayout.getLineAtOffset(
