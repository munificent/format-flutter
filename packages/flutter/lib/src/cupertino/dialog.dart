    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: ConstrainedBox(
          constraints:
              const BoxConstraints(minHeight: _kActionSheetButtonHeight),
          child: Semantics(
            button: true,
            child: Container(
...
class _CupertinoDialogRenderElement extends RenderObjectElement {
  ...
  void removeRenderObjectChild(RenderObject child, _AlertDialogSections slot) {
    assert(
      child == renderObject.contentSection ||
...
class _RenderCupertinoDialog extends RenderBox {
  ...
  _AlertDialogSizes performRegularLayout({
    ...
    final Size contentSize = layoutChild(contentSection!, constraints.deflate(
      EdgeInsets.only(bottom: minActionsHeight + dividerThickness),
    ));

    final Size actionsSize = layoutChild(actionsSection!, constraints.deflate(
      EdgeInsets.only(top: contentSize.height + dividerThickness),
    ));
  ...
  _AlertDialogSizes performAccessibilityLayout({
    ...
    final double maxContentHeight = contentSection!.getMaxIntrinsicHeight(
      _dialogWidth,
    );
    final double maxActionsHeight = actionsSection!.getMaxIntrinsicHeight(
      _dialogWidth,
    );
    ...
    if (maxContentHeight + dividerThickness + maxActionsHeight >
      ...
      contentSize = layoutChild(contentSection!, constraints.deflate(
...
class _CupertinoDialogActionsRenderWidget extends MultiChildRenderObjectWidget {
  ...
  void updateRenderObject(
    ...
    renderObject
      ...
      ..dialogPressedColor = CupertinoDynamicColor.resolve(
        _kPressedColor,
        context,
      )
      ..dividerColor = CupertinoDynamicColor.resolve(
    ...
    if (!isActionSheet && _isSingleButtonRow(dialogWidth!)) {
      if (childCount == 1) {
        ...
        if (!dry) {
          ...
          secondButtonParentData.offset = Offset(
            firstChildSize.width + dividerThickness,
            0.0,
          );
        }
        ...
        return constraints.constrain(Size(
          dialogWidth!,
          math.max(firstChildSize.height, lastChildSize.height),
        ));
      }
    } else {
