...
class _CupertinoContextMenuState extends State<CupertinoContextMenu>
  ...
  void _onTapDown(TapDownDetails details) {
    ...
    Overlay.of(context, rootOverlay: true, debugRequiredFor: widget).insert(
      _lastOverlayEntry!,
    );
  ...
  void initState() {
    ...
    _rect = TweenSequence<Rect?>(<TweenSequenceItem<Rect?>>[
      TweenSequenceItem<Rect?>(
        tween: RectTween(begin: widget.beginRect, end: widget.beginRect).chain(
          CurveTween(curve: Curves.linear),
        ),
        weight: beginPause,
      ),
      TweenSequenceItem<Rect?>(
        tween: RectTween(begin: widget.beginRect, end: widget.endRect).chain(
          CurveTween(curve: Curves.easeOutSine),
        ),
        weight: openAnimationLength,
      ),
      TweenSequenceItem<Rect?>(
        tween: RectTween(begin: widget.endRect, end: widget.endRect).chain(
          CurveTween(curve: Curves.linear),
        ),
        weight: endPause,
      ),
...
class _ContextMenuRoute<T> extends PopupRoute<T> {
  ...
  void _onDismiss(BuildContext context, double scale, double opacity) {
    ...
    _sheetOpacity = _opacityTween.animate(
      CurvedAnimation(parent: animation!, curve: const Interval(0.9, 1.0)),
    );
...
class _ContextMenuRouteStaticState extends State<_ContextMenuRouteStatic>
  ...
  List<Widget> _getChildren(
    ...
    final Expanded child = Expanded(
      child: Align(
        alignment:
            _getChildAlignment(widget.orientation, widget.contextMenuLocation),
  ...
  void initState() {
    ...
    _sheetOpacityAnimation = Tween<double>(begin: 1.0, end: 0.0).animate(
      _sheetController,
    );
    _setDragOffset(Offset.zero);
  }
