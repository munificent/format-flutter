...
class _SegmentState<T> extends State<_Segment<T>> with TickerProviderStateMixin<_Segment<T>> {
  ...
  void initState() {
    ...
    highlightPressScaleAnimation = highlightPressScaleController.drive(Tween<double>(begin: 1.0, end: _kMinThumbScale));
  }
  ...
  void didUpdateWidget(_Segment<T> oldWidget) {
    ...
    if (oldWidget.shouldScaleContent != widget.shouldScaleContent) {
      highlightPressScaleAnimation = highlightPressScaleController.drive(Tween<double>(
    ...
    return MetaData(
      ...
      child: IndexedStack(
        ...
        children: <Widget>[
          ...
          Offstage(
            child: DefaultTextStyle.merge(style: const TextStyle(fontWeight: FontWeight.w500), child: widget.child),
          ),
        ],
      ),
    );
...
class _SegmentSeparator extends StatefulWidget {
  const _SegmentSeparator({required ValueKey<int> key, required this.highlighted}) : super(key: key);
...
class _SegmentedControlState<T> extends State<CupertinoSlidingSegmentedControl<T>>
  ...
  Widget build(BuildContext context) {
    ...
    for (final MapEntry<T, Widget> entry in widget.children.entries) {
      ...
      if (index != 0) {
        children.add(_SegmentSeparator(
