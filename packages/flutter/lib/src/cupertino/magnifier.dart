...
class _CupertinoTextMagnifierState extends State<CupertinoTextMagnifier> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _ioAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(parent: _ioAnimationController, curve: widget.animationCurve),
    );
  }
...
class CupertinoMagnifier extends StatelessWidget {
  ...
  const CupertinoMagnifier({
    ...
    this.shadows = const <BoxShadow>[BoxShadow(color: Color.fromARGB(25, 0, 0, 0), blurRadius: 11, spreadRadius: 0.2)],
  ...
  Widget build(BuildContext context) {
    ...
    return Transform.translate(
      offset: Offset.lerp(const Offset(0, -kMagnifierAboveFocalPoint), Offset.zero, inOutAnimation?.value ?? 1)!,
      child: RawMagnifier(
        size: size,
