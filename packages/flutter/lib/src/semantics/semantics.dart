...
/// value.
typedef ChildSemanticsConfigurationsDelegate = ChildSemanticsConfigurationsResult
    Function(List<SemanticsConfiguration>);
...
class CustomSemanticsAction {
  ...
  @override
  String toString() {
    return 'CustomSemanticsAction(${_ids[
      this
    ]}, label:$label, hint:$hint, action:$action)';
  }
...
class AttributedString {
  ...
  AttributedString operator +(AttributedString other) {
    ...
    final List<StringAttribute> newAttributes = List<StringAttribute>.of(
      attributes,
    );
    if (other.attributes.isNotEmpty) {
      ...
      for (final StringAttribute attribute in other.attributes) {
        ...
        final StringAttribute adjustedAttribute = attribute.copy(
          range: newRange,
        );
...
class SemanticsData with Diagnosticable {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    final List<String?> customSemanticsActionSummary = customSemanticsActionIds!
        ...
        .toList();
    properties.add(
      IterableProperty<String>('actions', actionSummary, ifEmpty: null),
    );
    ...
    properties.add(
      IterableProperty<String>('flags', flagSummary, ifEmpty: null),
    );
    ...
    properties.add(
      IntProperty('platformViewId', platformViewId, defaultValue: null),
    );
    properties.add(
      IntProperty('maxValueLength', maxValueLength, defaultValue: null),
    );
    properties.add(
      IntProperty('currentValueLength', currentValueLength, defaultValue: null),
    );
    properties.add(
  ...
  bool operator ==(Object other) {
    return other is SemanticsData &&
        ...
        _sortedListsEqual(
          other.customSemanticsActionIds,
...
class SemanticsProperties extends DiagnosticableTree {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super.debugFillProperties(properties);
    properties.add(
      DiagnosticsProperty<bool>('checked', checked, defaultValue: null),
    );
    properties.add(
      DiagnosticsProperty<bool>('mixed', mixed, defaultValue: null),
    );
    properties.add(
    ...
    properties.add(DiagnosticsProperty<SemanticsSortKey>(
      'sortKey',
      sortKey,
      defaultValue: null,
    ));
  ...
  @override
  String toStringShort() => objectRuntimeType(
    this,
    'SemanticsProperties',
  ); // the hashCode isn't important since we're immutable
}
...
class SemanticsNode with DiagnosticableTreeMixin {
  ...
  set transform(Matrix4? value) {
    if (!MatrixUtils.matrixEquals(_transform, value)) {
      _transform = value == null || MatrixUtils.isIdentity(value)
          ? null
          : value;
      _markDirty();
    }
  ...
  bool _canPerformAction(SemanticsAction action) => _actions.containsKey(
    action,
  );
  ...
  SemanticsData getSemanticsData() {
    ...
    Set<SemanticsTag>? mergedTags = tags == null
        ? null
        : Set<SemanticsTag>.of(tags!);
    ...
    return SemanticsData(
      ...
      actions: _areUserActionsBlocked
          ? actions & _kUnblockedUserActions
          : actions,
  ...
  void _addToUpdate(
    ...
    if (data.customSemanticsActionIds?.isNotEmpty ?? false) {
      customSemanticsActionIds = Int32List(
        data.customSemanticsActionIds!.length,
      );
    ...
    builder.updateNode(
      ...
      textSelectionBase: data.textSelection != null
          ? data.textSelection!.baseOffset
          : -1,
      textSelectionExtent: data.textSelection != null
  ...
  List<SemanticsNode> _childrenInTraversalOrder() {
    ...
    if (inheritedTextDirection != null) {
      childrenInDefaultOrder = _childrenInDefaultOrder(
        _children!,
    ...
    for (int position = 0;
      ...
      sortNodes.add(
        _TraversalSortNode(node: child, sortKey: sortKey, position: position),
      );
    }
    ...
    return everythingSorted.map<SemanticsNode>(
      (_TraversalSortNode sortNode) => sortNode.node,
    ).toList();
  }
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(DiagnosticsProperty<SemanticsOwner>(
      'owner',
      owner,
    ...
    final Offset? offset = transform != null
        ? MatrixUtils.getAsTranslation(transform!)
        : null;
    if (offset != null) {
      properties.add(
      ...
      final double? scale = transform != null
          ? MatrixUtils.getAsScale(transform!)
          : null;
      String? description;
      if (scale != null) {
    ...
    final List<String> actions = _actions.keys.map<String>(
    ...
    properties.add(FlagProperty(
      'isHidden',
      value: hasFlag(SemanticsFlag.isHidden),
      ifTrue: 'HIDDEN',
    ));
  ...
  List<DiagnosticsNode> debugDescribeChildren({
    ...
    return debugListChildrenInOrder(childOrder).map<DiagnosticsNode>(
...
class _SemanticsSortGroup implements Comparable<_SemanticsSortGroup> {
  ...
  List<SemanticsNode> sortedWithinVerticalGroup() {
    ...
    return horizontalGroups.expand(
      (_SemanticsSortGroup group) => group.sortedWithinKnot(),
    ).toList();
  }
  ...
  List<SemanticsNode> sortedWithinKnot() {
    ...
    for (final SemanticsNode node in nodes) {
      ...
      for (final SemanticsNode nextNode in nodes) {
        ...
        final Offset nextCenter = _pointInParentCoordinates(
          nextNode,
    ...
    return sortedIds.map<SemanticsNode>(
      (int id) => nodeMap[id]!,
...
List<SemanticsNode> _childrenInDefaultOrder(
  ...
  return verticalGroups.expand(
    (_SemanticsSortGroup group) => group.sortedWithinVerticalGroup(),
  ).toList();
}
...
class SemanticsOwner extends ChangeNotifier {
  ...
  void sendSemanticsUpdate() {
    ...
    while (_dirtyNodes.isNotEmpty) {
      final List<SemanticsNode> localDirtyNodes = _dirtyNodes.where(
        (SemanticsNode node) => !_detachedNodes.contains(node),
      ).toList();
      _dirtyNodes.clear();
      _detachedNodes.clear();
      localDirtyNodes.sort(
        (SemanticsNode a, SemanticsNode b) => a.depth - b.depth,
      );
  ...
  void performAction(int id, SemanticsAction action, [Object? args]) {
    final SemanticsActionHandler? handler = _getSemanticsActionHandlerForId(
      id,
      action,
    );
    if (handler != null) {
...
class SemanticsConfiguration {
  ...
  void _onCustomSemanticsAction(Object? args) {
    final CustomSemanticsAction? action = CustomSemanticsAction.getAction(
      args! as int,
    );
    if (action == null) {
      return;
  ...
  bool get isInMutuallyExclusiveGroup => _hasFlag(
  ...
  void absorb(SemanticsConfiguration child) {
    ...
    if (child.isBlockingUserActions) {
      child._actions.forEach(
        (SemanticsAction key, SemanticsActionHandler value) {
