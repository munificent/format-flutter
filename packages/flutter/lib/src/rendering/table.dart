...
class RenderTable extends RenderBox {
  ...
  get columnWidths => Map<int, TableColumnWidth>.unmodifiable(_columnWidths);
  ...
  /// either.
  List<Decoration> get rowDecorations => List<Decoration>.unmodifiable(
  ...
  double computeMinIntrinsicWidth(double height) {
    ...
    for (int x = 0; x < columns; x += 1) {
      ...
      totalMinWidth += columnWidth.minIntrinsicWidth(
        columnCells,
        double.infinity,
      );
    }
  ...
  double computeMaxIntrinsicWidth(double height) {
    ...
    for (int x = 0; x < columns; x += 1) {
      final TableColumnWidth columnWidth =
          _columnWidths[x] ?? defaultColumnWidth;
      final Iterable<RenderBox> columnCells = column(x);
      totalMaxWidth += columnWidth.maxIntrinsicWidth(
        columnCells,
        double.infinity,
      );
    }
  ...
  double computeMinIntrinsicHeight(double width) {
    ...
    assert(_children.length == rows * columns);
    final List<double> widths = _computeColumnWidths(
    ...
    for (int y = 0; y < rows; y += 1) {
      ...
      for (int x = 0; x < columns; x += 1) {
        ...
        if (child != null) {
          rowHeight = math.max(
            rowHeight,
            child.getMaxIntrinsicHeight(widths[x]),
          );
        }
      }
      rowTop += rowHeight;
  ...
  void performLayout() {
    ...
    for (int y = 0; y < rows; y += 1) {
      ...
      if (haveBaseline) {
        ...
        rowHeight = math.max(
          rowHeight,
          beforeBaselineDistance + afterBaselineDistance,
        );
      }
      for (int x = 0; x < columns; x += 1) {
        final int xy = x + y * columns;
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(IterableProperty<String>(
      'row offsets',
      _rowTops.map(debugFormatDouble),
      ifNull: 'unknown',
    ));
  }
    ...
    for (int y = 0; y < rows; y += 1) {
      for (int x = 0; x < columns; x += 1) {
        ...
        if (child != null) {
          ...
          children.add(DiagnosticsProperty<Object>(
            name,
            null,
            ifNull: 'is null',
            showSeparator: false,
