...
class _MouseState {
  _MouseState({required PointerEvent initialEvent})
  ...
  String toString() {
    ...
    return '${describeIdentity(
      this,
    )}($describeLatestEvent, $describeAnnotations)';
  }
}
...
class _MouseTrackerUpdateDetails with Diagnosticable {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      DiagnosticsProperty<PointerEvent>('previousEvent', previousEvent),
    );
    properties.add(
...
class MouseTracker extends ChangeNotifier {
  ...
  void _handleDeviceUpdate(_MouseTrackerUpdateDetails details) {
    ...
    _mouseCursorMixin.handleDeviceCursorUpdate(
      ...
      details.nextAnnotations.keys.map(
        (MouseTrackerAnnotation annotation) => annotation.cursor,
      ),
    );
  }
  ...
  static void _handleDeviceUpdateMouseEvents(
    ...
    // hit-test order.
    final PointerExitEvent baseExitEvent = PointerExitEvent.fromMouseEvent(
      latestEvent,
    );
    lastAnnotations.forEach(
      (MouseTrackerAnnotation annotation, Matrix4 transform) {
        if (!nextAnnotations.containsKey(annotation)) {
          if (annotation.validForMouseTracker && annotation.onExit != null) {
            annotation.onExit!(
              baseExitEvent.transformed(lastAnnotations[annotation]),
            );
          }
        }
      },
    ...
    final List<MouseTrackerAnnotation> enteringAnnotations =
        nextAnnotations.keys.where(
          (MouseTrackerAnnotation annotation) =>
              !lastAnnotations.containsKey(annotation),
        ).toList();
    final PointerEnterEvent baseEnterEvent = PointerEnterEvent.fromMouseEvent(
      latestEvent,
    );
    for (final MouseTrackerAnnotation annotation
      ...
      if (annotation.validForMouseTracker && annotation.onEnter != null) {
        annotation.onEnter!(
          baseEnterEvent.transformed(nextAnnotations[annotation]),
        );
      }
    }
  }
