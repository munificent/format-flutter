...
class PaintingContext extends ClipContext {
  ...
  String toString() => '${objectRuntimeType(
        this,
        'PaintingContext',
      )}#$hashCode(layer: $_containerLayer, canvas bounds: $estimatedBounds)';
}
...
class PipelineOwner {
  ...
  void flushLayout() {
    if (!kReleaseMode) {
      ...
      Timeline.startSync('LAYOUT', arguments: debugTimelineArguments);
    }
    assert(
      () {
  ...
  void flushPaint() {
    if (!kReleaseMode) {
      ...
      Timeline.startSync('PAINT', arguments: debugTimelineArguments);
    }
    try {
      assert(
        () {
...
abstract class RenderObject with DiagnosticableTreeMixin implements HitTestTarget {
      ...
      () {
        ...
        if (activeLayoutRoot == null) {
          throw FlutterError.fromParts(<DiagnosticsNode>[
            ...
            ErrorDescription('The RenderObject was mutated when none of its ancestors is actively performing layout.'),
            DiagnosticsProperty<RenderObject>(
  ...
  void layout(Constraints constraints, {bool parentUsesSize = false}) {
    ...
    if (!kReleaseMode && debugProfileLayoutsEnabled) {
      ...
      Timeline.startSync('$runtimeType', arguments: debugTimelineArguments);
    }
  ...
  _SemanticsFragment _getSemanticsForParent({required bool mergeIntoParent, required bool blockUserActions}) {
    ...
    if (explicitChildNode) {
      ...
      mergeUpFragments.addAll(result.mergeUp.map<_InterestingSemanticsFragment>((SemanticsConfiguration config) {
    ...
    if (parent is! RenderObject) {
      ...
      result = _RootSemanticsFragment(owner: this, dropsSemanticsOfPreviousSiblings: dropSemanticsOfPreviousSiblings);
...
class _ContainerSemanticsFragment extends _SemanticsFragment {
  _ContainerSemanticsFragment({required super.dropsSemanticsOfPreviousSiblings, required this.siblingMergeGroups});
...
abstract class _InterestingSemanticsFragment extends _SemanticsFragment {
  _InterestingSemanticsFragment({required RenderObject owner, required super.dropsSemanticsOfPreviousSiblings})
...
class _RootSemanticsFragment extends _InterestingSemanticsFragment {
  ...
  void compileChildren({
    ...
    owner._semantics ??= SemanticsNode.root(showOnScreen: owner.showOnScreen, owner: owner.owner!.semanticsOwner!);
...
class _SwitchableSemanticsFragment extends _InterestingSemanticsFragment {
  ...
  void compileChildren({
    ...
    if (!_isExplicit) {
      ...
      _mergeSiblingGroup(parentSemanticsClipRect, parentPaintClipRect, siblingNodes, usedSemanticsIds);
    ...
    final List<SemanticsNode> children = <SemanticsNode>[];
    _mergeSiblingGroup(node.parentSemanticsClipRect, node.parentPaintClipRect, siblingNodes, usedSemanticsIds);
    for (final _InterestingSemanticsFragment fragment in compilingFragments) {
      if (fragment is _SwitchableSemanticsFragment) {
        // Cached semantics node may be part of sibling merging group prior
        // to this update. In this case, the semantics node may continue to
...
class DiagnosticsDebugCreator extends DiagnosticsProperty<Object> {
  ...
  DiagnosticsDebugCreator(Object value) : super('debugCreator', value, level: DiagnosticLevel.hidden);
}
