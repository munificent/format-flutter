...
class RenderPositionedBox extends RenderAligningShiftedBox {
      ...
      () {
        ...
        if (child != null && !child!.size.isEmpty) {
          ...
          if (childParentData.offset.dy > 0.0) {
            ...
            final double headSize = math.min(
              childParentData.offset.dy * 0.2,
              10.0,
            );
            path
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super.debugFillProperties(properties);
    properties.add(
      DoubleProperty('widthFactor', _widthFactor, ifNull: 'expand'),
    );
    properties.add(
      DoubleProperty('heightFactor', _heightFactor, ifNull: 'expand'),
    );
  }
}
...
class RenderCustomSingleChildLayoutBox extends RenderShiftedBox {
  ...
  void performLayout() {
    ...
    if (child != null) {
      final BoxConstraints childConstraints = delegate.getConstraintsForChild(
        constraints,
      );
...
class RenderBaseline extends RenderShiftedBox {
  ...
  void performLayout() {
    if (child != null) {
      ...
      size = constraints.constrain(
        Size(childSize.width, top + childSize.height),
      );
    } else {
      size = constraints.smallest;
    }
