...
class RenderEditable extends RenderBox
  ...
  void _updateForegroundPainter(RenderEditablePainter? newPainter) {
    final _CompositeRenderEditablePainter effectivePainter = newPainter == null
        ...
        : _CompositeRenderEditablePainter(painters: <RenderEditablePainter>[_builtInForegroundPainters, newPainter]);
  ...
  _CompositeRenderEditablePainter _createBuiltInForegroundPainters() {
    return _CompositeRenderEditablePainter(painters: <RenderEditablePainter>[if (paintCursorAboveText) _caretPainter]);
  }
  ...
  List<TextBox> getBoxesForSelection(TextSelection selection) {
    ...
    return _textPainter.getBoxesForSelection(selection).map(
  ...
  void _handleMoveCursorForwardByCharacter(bool extendSelection) {
    ...
    _setSelection(TextSelection(baseOffset: baseOffset, extentOffset: extentOffset), SelectionChangedCause.keyboard);
  }
  ...
  void _handleMoveCursorForwardByWord(bool extendSelection) {
    ...
    _setSelection(TextSelection(baseOffset: baseOffset, extentOffset: nextWord.start), SelectionChangedCause.keyboard);
  }
  ...
  Rect? getRectForComposingRange(TextRange range) {
    ...
    return boxes
        .fold(null, (Rect? accum, TextBox incoming) => accum?.expandToInclude(incoming.toRect()) ?? incoming.toRect())
        ?.shift(_paintOffset);
  }
  ...
  Rect getLocalRectForCaret(TextPosition caretPosition) {
    ...
    switch (defaultTargetPlatform) {
      ...
      case TargetPlatform.macOS:
        ...
        caretRect = Rect.fromLTWH(caretRect.left, caretRect.top + heightDiff / 2, caretRect.width, caretRect.height);
      ...
      case TargetPlatform.windows:
        ...
        caretRect = Rect.fromLTWH(caretRect.left, caretRect.top - _kCaretHeightOffset, caretRect.width, caretHeight);
    }
  ...
  TextSelection _getWordAtOffset(TextPosition position) {
    ...
    // the position.
    if (TextLayoutMetrics.isWhitespace(plainText.codeUnitAt(effectiveOffset)) && effectiveOffset > 0) {
  ...
  void _paintHandleLayers(PaintingContext context, List<TextSelectionPoint> endpoints, Offset offset) {
    ...
    context.pushLayer(LeaderLayer(link: startHandleLayerLink, offset: startPoint + offset), super.paint, Offset.zero);
    if (endpoints.length == 2) {
      Offset endPoint = endpoints[1].point;
      endPoint = Offset(clampDouble(endPoint.dx, 0.0, size.width), clampDouble(endPoint.dy, 0.0, size.height));
      context.pushLayer(LeaderLayer(link: endHandleLayerLink, offset: endPoint + offset), super.paint, Offset.zero);
    }
  }
  ...
  List<DiagnosticsNode> debugDescribeChildren() {
    return <DiagnosticsNode>[
      if (text != null) text!.toDiagnosticsNode(name: 'text', style: DiagnosticsTreeStyle.transition),
    ];
  }
}
...
class _TextHighlightPainter extends RenderEditablePainter {
  ...
  void paint(Canvas canvas, Size size, RenderEditable renderEditable) {
    ...
    for (final TextBox box in boxes) {
      canvas.drawRect(box.toRect().shift(renderEditable._paintOffset).intersect(
