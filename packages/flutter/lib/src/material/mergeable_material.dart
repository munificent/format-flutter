...
class _MergeableMaterialState extends State<MergeableMaterial> with TickerProviderStateMixin {
  ...
  void didUpdateWidget(MergeableMaterial oldWidget) {
    ...
    final Set<LocalKey> oldKeys = oldWidget.children.map<LocalKey>((MergeableMaterialItem child) => child.key).toSet();
    final Set<LocalKey> newKeys = widget.children.map<LocalKey>((MergeableMaterialItem child) => child.key).toSet();
...
class _MergeableMaterialListBody extends ListBody {
  ...
  RenderListBody createRenderObject(BuildContext context) {
    return _RenderMergeableMaterialListBody(axisDirection: _getDirection(context), elevation: elevation);
  }
