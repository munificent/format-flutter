...
class _RangeSliderState extends State<RangeSlider> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    enableController = AnimationController(duration: enableAnimationDuration, vsync: this, value: _enabled ? 1.0 : 0.0);
    ...
    if (overlayEntry == null) {
      overlayEntry = OverlayEntry(
        builder: (BuildContext context) {
          return CompositedTransformFollower(link: _layerLink, child: _ValueIndicatorRenderObjectWidget(state: this));
        },
      );
...
class _RenderRangeSlider extends RenderBox with RelayoutWhenSystemFontsChangeMixin {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (isDiscrete) {
      final double tickMarkWidth =
          _sliderTheme.rangeTickMarkShape!.getPreferredSize(isEnabled: isEnabled, sliderTheme: _sliderTheme).width;
  ...
  void assembleSemanticsNode(SemanticsNode node, SemanticsConfiguration config, Iterable<SemanticsNode> children) {
    ...
    final List<SemanticsNode> finalChildren = <SemanticsNode>[_startSemanticsNode!, _endSemanticsNode!];
