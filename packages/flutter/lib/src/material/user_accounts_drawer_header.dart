    ...
    return Stack(
      children: <Widget>[
        PositionedDirectional(
          ...
          child: Row(
            children: (otherAccountsPictures ?? <Widget>[]).take(3).map<Widget>(
              (Widget picture) {
                return Padding(
...
class _AccountDetailsLayout extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    if (bottomLine != null) {
      ...
      final Size bottomLineSize = layoutChild(
        bottomLine,
        BoxConstraints.loose(constraintSize),
      );
      final Offset bottomLineOffset = _offsetForBottomLine(
        size,
        iconSize,
        bottomLineSize,
      );
      ...
      if (bottomLine == accountEmail && hasChild(accountName)) {
        final Size nameSize = layoutChild(
          accountName,
          BoxConstraints.loose(constraintSize),
        );
        positionChild(
          accountName,
          _offsetForName(size, nameSize, bottomLineOffset),
        );
      }
    }
  }
