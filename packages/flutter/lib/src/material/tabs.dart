  ...
  MaterialStateColor _resolveWithLabelColor(BuildContext context) {
    ...
    if (selectedColor is MaterialStateColor) {
      ...
      selectedColor = selectedColor
          .resolve(const <MaterialState>{MaterialState.selected});
    } else {
...
class _TabBarState extends State<TabBar> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget>
    wrappedTabs = List<Widget>.generate(widget.tabs.length, (int index) {
    ...
    if (widget.isScrollable) {
      final EdgeInsetsGeometry? effectivePadding =
          effectiveTabAlignment == TabAlignment.startOffset
              ? const EdgeInsetsDirectional.only(start: _kStartOffset).add(
