...
class MenuAnchor extends StatefulWidget {
  ...
  List<DiagnosticsNode> debugDescribeChildren() {
    return menuChildren.map<DiagnosticsNode>(
      (Widget child) => child.toDiagnosticsNode(),
    ).toList();
  }
...
class _MenuAnchorState extends State<MenuAnchor> {
  ...
  final FocusScopeNode _menuScopeNode = FocusScopeNode(
    debugLabel: kReleaseMode ? null : 'MenuAnchor sub menu',
  );
...
class SubmenuButton extends StatefulWidget {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(DiagnosticsProperty<MenuStyle>(
      'menuStyle',
      menuStyle,
      defaultValue: null,
    ));
  ...
  Widget build(BuildContext context) {
    ...
    return MenuAnchor(
      ...
      builder:
        ...
        // once.
        ButtonStyle mergedStyle = widget.themeStyleOf(context)?.merge(
              widget.defaultStyleOf(context),
            ) ??
            widget.defaultStyleOf(context);
        if (widget.style != null) {
...
class _MenuDirectionalFocusAction extends DirectionalFocusAction {
  ...
  bool _moveToNext(_MenuAnchorState currentMenu) {
    ...
    if (currentMenu.widget.childFocusNode != null) {
      final FocusTraversalPolicy? policy = FocusTraversalGroup.maybeOf(
    ...
    return Row(
      ...
      children: <Widget>[
        ...
        if (showDecoration && shortcut != null)
          Padding(
            padding: EdgeInsetsDirectional.only(start: horizontalPadding),
            child: Text(_LocalizedShortcutLabeler.instance.getShortcutLabel(
              shortcut!,
...
class _MenuLayout extends SingleChildLayoutDelegate {
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    ...
    if (menuPosition == null) {
      Offset desiredPosition = alignment.resolve(textDirection).withinRect(
        anchorRect,
      );
...
class _MenuPanelState extends State<_MenuPanel> {
  ...
  Widget build(BuildContext context) {
    ...
    final EdgeInsetsGeometry resolvedPadding = padding.add(
      EdgeInsets.symmetric(horizontal: dx, vertical: dy),
    ).clamp(
      EdgeInsets.zero,
...
class _Submenu extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Offset upperLeft = anchorBox.localToGlobal(
      Offset.zero,
      ancestor: overlay,
    );
    ...
    final double dx = math.max(0, densityAdjustment.dx);
    final EdgeInsetsGeometry resolvedPadding = padding.add(
      EdgeInsets.fromLTRB(dx, dy, dx, dy),
    ).clamp(
      EdgeInsets.zero,
      EdgeInsetsGeometry.infinity,
    ); // ignore_clamp_double_lint
...
class _MenuBarDefaultsM3 extends MenuStyle {
  _MenuBarDefaultsM3(this.context)
    : super(
        ...
        shape:
            const MaterialStatePropertyAll<OutlinedBorder>(_defaultMenuBorder),
