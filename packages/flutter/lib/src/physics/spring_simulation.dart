...
class SpringDescription {
  ...
  /// ratio is unitless.
  SpringDescription.withDampingRatio({required this.mass, required this.stiffness, double ratio = 1.0})
  ...
  @override
  String toString() => '${objectRuntimeType(this, 'SpringDescription')}(mass: ${mass.toStringAsFixed(
    1,
  )}, stiffness: ${stiffness.toStringAsFixed(1)}, damping: ${damping.toStringAsFixed(1)})';
}
...
class SpringSimulation extends Simulation {
  ...
  /// [SpringSimulation].
  SpringSimulation(SpringDescription spring, double start, double end, double velocity, {super.tolerance})
    : _endPosition = end,
...
abstract class _SpringSolution {
  factory _SpringSolution(SpringDescription spring, double initialPosition, double initialVelocity) {
...
class _OverdampedSolution implements _SpringSolution {
  ...
  _OverdampedSolution.withArgs(double r1, double r2, double c1, double c2) : _r1 = r1, _r2 = r2, _c1 = c1, _c2 = c2;
...
class _UnderdampedSolution implements _SpringSolution {
  factory _UnderdampedSolution(SpringDescription spring, double distance, double velocity) {
    final double w =
        math.sqrt(4.0 * spring.mass * spring.stiffness - spring.damping * spring.damping) / (2.0 * spring.mass);
    final double r = -(spring.damping / 2.0 * spring.mass);
    final double c1 = distance;
  ...
  _UnderdampedSolution.withArgs(double w, double r, double c1, double c2) : _w = w, _r = r, _c1 = c1, _c2 = c2;

  final double _w, _r, _c1, _c2;

  @override
  double x(double time) {
    return (math.pow(math.e, _r * time) as double) * (_c1 * math.cos(_w * time) + _c2 * math.sin(_w * time));
  }
