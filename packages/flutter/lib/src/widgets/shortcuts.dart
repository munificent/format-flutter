...
class LogicalKeySet extends KeySet<LogicalKeyboardKey>
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(DiagnosticsProperty<Set<LogicalKeyboardKey>>(
      'keys',
      _keys,
...
class ShortcutMapProperty
  ...
  String valueToString({TextTreeConfiguration? parentConfiguration}) {
    return '{${value.keys.map<String>(
      (ShortcutActivator keySet) =>
          '{${keySet.debugDescribeKeys()}}: ${value[keySet]}',
    ).join(', ')}}';
  }
}
...
class SingleActivator
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(FlagProperty(
      'includeRepeats',
      value: includeRepeats,
      ifFalse: 'excluding repeats',
    ));
  }
}
  ...
  static Map<LogicalKeyboardKey?, List<_ActivatorIntentPair>> _indexShortcuts(
    ...
    source.forEach((ShortcutActivator activator, Intent intent) {
      ...
      final Iterable<LogicalKeyboardKey?>? nullableTriggers =
          activator.triggers;
      for (final LogicalKeyboardKey? trigger in nullableTriggers ??
          <LogicalKeyboardKey?>[null]) {
        result.putIfAbsent(trigger, () => <_ActivatorIntentPair>[]).add(
      ...
      if (primaryContext != null) {
        ...
        if (action != null) {
          final (bool enabled, Object? invokeResult) = Actions.of(
            primaryContext,
          ).invokeActionIfEnabled(action, matchedIntent, primaryContext);
          if (enabled) {
...
class Shortcuts extends StatefulWidget {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super.debugFillProperties(properties);
    properties.add(DiagnosticsProperty<ShortcutManager>(
      'manager',
      manager,
      defaultValue: null,
    ));
...
class ShortcutRegistry with ChangeNotifier {
  ...
  Map<ShortcutActivator, Intent> get shortcuts {
    ...
    return <ShortcutActivator, Intent>{
      for (final MapEntry<
          ...
          > entry in _registeredShortcuts.entries)
        ...entry.value,
    };
  }
  ...
  void _disposeEntry(ShortcutRegistryEntry entry) {
    assert(_debugCheckEntryIsValid(entry));
    final Map<ShortcutActivator, Intent>? removedShortcut = _registeredShortcuts
        .remove(entry);
      ...
      for (final ShortcutActivator shortcut in tokenEntry.value.keys) {
        if (previous.containsKey(shortcut)) {
          throw FlutterError(
            ...
            'shortcut $shortcut in ${describeIdentity(
              tokenEntry.key,
            )} and ${previous[shortcut]}.',
          );
        }
