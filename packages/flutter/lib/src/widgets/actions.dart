...
abstract class Action<T extends Intent> with Diagnosticable {
  ...
  /// {@endtemplate}
  @mustCallSuper
  void addActionListener(ActionListenerCallback listener) => _listeners.add(
    listener,
  );
  ...
  @mustCallSuper
  void removeActionListener(ActionListenerCallback listener) => _listeners
      .remove(listener);
...
class Actions extends StatefulWidget {
  ...
  ///    no [Actions] ancestor is found.
  static Action<T>? maybeFind<T extends Intent>(
    BuildContext context, {
    T? intent,
  }) {
    Action<T>? action;
  ...
  static Action<T>? _castAction<T extends Intent>(
  ...
  static Object? invoke<T extends Intent>(BuildContext context, T intent) {
    ...
    final bool actionFound = _visitActionsAncestors(
      context,
      (InheritedElement element) {
        ...
        if (result != null && result._isEnabled(intent, context)) {
          ...
          // Element we found.
          returnValue = _findDispatcher(element).invokeAction(
            result,
            intent,
            context,
          );
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      DiagnosticsProperty<ActionDispatcher>('dispatcher', dispatcher),
    );
    properties.add(
...
class _ActionsState extends State<Actions> {
  ...
  void _updateActionListeners() {
    ...
    final Set<Action<Intent>> removedActions = listenedActions!.difference(
      widgetActions,
    );
    final Set<Action<Intent>> addedActions = widgetActions.difference(
      listenedActions!,
    );
...
class _FocusableActionDetectorState extends State<FocusableActionDetector> {
  ...
  void _mayTriggerCallback({
    VoidCallback? task,
    FocusableActionDetector? oldWidget,
  }) {
...
mixin _OverridableActionMixin<T extends Intent> on Action<T> {
  ...
  bool isEnabled(T intent, [BuildContext? context]) {
    ...
    final Action<T>? overrideAction = getOverrideAction();
    overrideAction?._updateCallingAction(defaultAction);
    final bool returnValue = (overrideAction ?? defaultAction)._isEnabled(
      intent,
      context,
    );
    overrideAction?._updateCallingAction(null);
  ...
  bool consumesKey(T intent) {
    ...
    final Action<T>? overrideAction = getOverrideAction();
    overrideAction?._updateCallingAction(defaultAction);
    final bool isEnabled = (overrideAction ?? defaultAction).consumesKey(
      intent,
    );
    overrideAction?._updateCallingAction(null);
    assert(
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super.debugFillProperties(properties);
    properties.add(
      DiagnosticsProperty<Action<T>>('defaultAction', defaultAction),
    );
  }
}
