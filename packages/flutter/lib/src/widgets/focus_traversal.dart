...
abstract class FocusTraversalPolicy with Diagnosticable {
  ...
  ///
  /// The [currentNode] argument must not be null.
  bool previous(FocusNode currentNode) => _moveFocus(
    currentNode,
    forward: false,
  );
  ...
  Map<FocusNode?, _FocusTraversalGroupInfo> _findGroups(
  ...
  List<FocusNode> _sortAllDescendants(
    ...
    for (final FocusNode? key in groups.keys) {
      final List<FocusNode> sortedMembers = groups[key]!.policy.sortDescendants(
        groups[key]!.members,
        currentNode,
      ).toList();
    ...
    assert(
      sortedDescendants.length <= scope.traversalDescendants.length &&
          sortedDescendants.toSet().difference(
      ...
      'These are the different nodes: ${sortedDescendants.toSet().difference(
        scope.traversalDescendants.toSet(),
      )}',
    );
  ...
  bool _moveFocus(FocusNode currentNode, {required bool forward}) {
    ...
    final List<FocusNode> sortedNodes = _sortAllDescendants(
      nearestScope,
      currentNode,
    );
    ...
    final Iterable<FocusNode> maybeFlipped = forward
        ? sortedNodes
...
mixin DirectionalFocusTraversalPolicyMixin on FocusTraversalPolicy {
  ...
  void changedScope({FocusNode? node, FocusScopeNode? oldScope}) {
    ...
    if (oldScope != null) {
      _policyData[oldScope]?.history.removeWhere(
        (_DirectionalPolicyDataEntry entry) {
  ...
  static Iterable<FocusNode> _sortClosestEdgesByDistancePreferHorizontal(
    ...
    mergeSort<FocusNode>(
      sorted,
      compare: (FocusNode nodeA, FocusNode nodeB) {
        final int horizontal = _horizontalCompareClosestEdge(
          target,
          nodeA.rect,
          nodeB.rect,
        );
  ...
  Iterable<FocusNode> _sortAndFilterHorizontally(
    ...
    mergeSort<FocusNode>(
      sorted,
      compare: (FocusNode a, FocusNode b) => a.rect.center.dx.compareTo(
        b.rect.center.dx,
      ),
    );
    return sorted;
  ...
  Iterable<FocusNode> _sortAndFilterVertically(
    ...
    mergeSort<FocusNode>(
      sorted,
      compare: (FocusNode a, FocusNode b) => a.rect.center.dy.compareTo(
        b.rect.center.dy,
      ),
    );
    return sorted;
...
class _ReadingOrderSortData with Diagnosticable {
  ...
  static TextDirection? commonDirectionalityOf(
    ...
    final Iterable<Set<Directionality>> allAncestors = list
        .map<Set<Directionality>>(
    ...
    return list.first.directionalAncestors.firstWhere(
      common.contains,
    ).textDirection;
  }
...
class ReadingOrderTraversalPolicy extends FocusTraversalPolicy
  ...
  _ReadingOrderSortData _pickNext(List<_ReadingOrderSortData> candidates) {
    ...
    mergeSort<_ReadingOrderSortData>(
      candidates,
      compare: (_ReadingOrderSortData a, _ReadingOrderSortData b) => a.rect.top
          .compareTo(b.rect.top),
    );
...
class OrderedTraversalPolicy extends FocusTraversalPolicy
  ...
  Iterable<FocusNode> sortDescendants(
    ...
    final Iterable<FocusNode> sortedDescendants = secondaryPolicy
