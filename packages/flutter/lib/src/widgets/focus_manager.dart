...
class FocusNode with DiagnosticableTreeMixin, ChangeNotifier {
  ...
  void _reparent(FocusNode child) {
    ...
    child._parent?._removeChild(
      child,
      removeScopeFocus: oldScope != nearestScope,
    );
    _children.add(child);
    child._parent = this;
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      DiagnosticsProperty<BuildContext>('context', context, defaultValue: null),
    );
  ...
  String toStringShort() {
    ...
    return '${describeIdentity(
      this,
    )}${extraData.isNotEmpty ? '($extraData)' : ''}';
  }
}
...
class FocusScopeNode extends FocusNode {
  ...
  void autofocus(FocusNode node) {
    ...
    _manager?._pendingAutofocuses.add(
      _Autofocus(scope: this, autofocusNode: node),
    );
...
class FocusManager with DiagnosticableTreeMixin, ChangeNotifier {
  ...
  void _applyFocusChange() {
    ...
    assert(_focusDebug(
      () => 'Notified ${_dirtyNodes.length} dirty nodes:',
      () => _dirtyNodes,
    ));
    _dirtyNodes.clear();
    if (previousFocus != _primaryFocus) {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    if (element != null) {
      properties.add(DiagnosticsProperty<String>(
        'primaryFocusCreator',
        element.debugGetCreatorChain(20),
      ));
    }
  }
}
...
class _HighlightModeManager {
  ...
  void addListener(ValueChanged<FocusHighlightMode> listener) => _listeners.add(
    listener,
  );

  /// Remove a previously registered closure from the list of closures that the
  /// [FocusManager] notifies.
  void removeListener(ValueChanged<FocusHighlightMode> listener) => _listeners
      .remove(listener);
