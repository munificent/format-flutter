  ...
  void _dragEnd(_DragInfo item) {
    setState(() {
      if (_insertIndex == item.index) {
        ...
        // down by the gap.
        final int itemIndex = _items.length > 1
            ? _insertIndex! - 1
            : _insertIndex!;
        if (_reverse) {
  ...
  void _dragUpdateItems() {
    ...
    for (final _ReorderableItemState item in _items.values) {
      ...
      if (!_dragStartTransitionComplete && _dragIndex! <= item.index) {
        ...
        final Offset transitionOffset = _extentOffset(
          _reverse ? -gapExtent : gapExtent,
          _scrollDirection,
        );
...
class ReorderableDragStartListener extends StatelessWidget {
  ...
  void _startDragging(BuildContext context, PointerDownEvent event) {
    ...
    final SliverReorderableListState? list = SliverReorderableList.maybeOf(
      context,
    );
