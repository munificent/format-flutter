...
class FlutterErrorDetails with Diagnosticable {
  ...
  String toString({DiagnosticLevel minLevel = DiagnosticLevel.info}) {
    return toDiagnosticsNode(style: DiagnosticsTreeStyle.error).toStringDeep(
      minLevel: minLevel,
    );
  }
...
class FlutterError extends Error
  ...
  factory FlutterError(String message) {
    ...
    return FlutterError.fromParts(<DiagnosticsNode>[
      ...
      ...lines.skip(1).map<DiagnosticsNode>(
        (String line) => ErrorDescription(line),
      ),
    ]);
  }
  ...
  FlutterError.fromParts(
    this.diagnostics,
  ) : assert(diagnostics.isNotEmpty, FlutterError.fromParts(<DiagnosticsNode>[
  ...
  static Iterable<String> defaultStackFilter(Iterable<String> frames) {
    ...
    final List<String?> reasons = List<String?>.filled(
      parsedFrames.length,
      null,
    );
