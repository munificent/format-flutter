      ...
      if (onPeak != null && pressure > peakPressure && (_state == _ForceState.started)) {
        ...
        if (onPeak != null) {
          invokeCallback<void>(
            'onPeak',
            () => onPeak!(
              ForcePressDetails(pressure: pressure, globalPosition: event.position, localPosition: event.localPosition),
            ),
          );
        }
      }
    ...
    if (wasAccepted && onEnd != null) {
      if (onEnd != null) {
        invokeCallback<void>(
          'onEnd',
          () => onEnd!(
            ForcePressDetails(pressure: 0.0, globalPosition: _lastPosition.global, localPosition: _lastPosition.local),
          ),
        );
      }
    }
