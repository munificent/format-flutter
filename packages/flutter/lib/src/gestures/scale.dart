...
/// zooming in.
const Offset kDefaultTrackpadScrollToScaleFactor = Offset(
  0,
...
class ScaleGestureRecognizer extends OneSequenceGestureRecognizer {
  ...
  void handleEvent(PointerEvent event) {
    ...
    if (event is PointerMoveEvent) {
      ...
      _pointerPanZooms[event.pointer] = _PointerPanZoomData.fromStartEvent(
        this,
        event,
      );
      didChangeConfiguration = true;
      ...
      _pointerPanZooms[event.pointer] = _PointerPanZoomData.fromUpdateEvent(
        this,
        event,
      );
      _lastTransform = event.transform;
  ...
  void _update() {
    ...
    _currentFocalPoint = _pointerCount > 0
        ? focalPoint / _pointerCount.toDouble()
        : Offset.zero;
        ...
        if (_isFlingGesture(velocity)) {
          ...
          invokeCallback<void>(
            ...
            () => onEnd!(ScaleEndDetails(
              velocity: velocity,
              scaleVelocity:
                  _scaleVelocityTracker?.getVelocity().pixelsPerSecond.dx ?? -1,
  ...
  void _advanceStateMachine(bool shouldStartIfAccepted, PointerEvent event) {
    ...
    if (_state == _ScaleState.possible) {
      ...
      if (spanDelta > computeScaleSlop(event.kind) ||
          ...
          math.max(
                _scaleFactor / _pointerScaleFactor,
