...
class _PointerPanZoomData {
  ...
  double get scale {
    if (parent.trackpadScrollCausesScale) {
      return _scale *
          math.exp(
            (_pan.dx * parent.trackpadScrollToScaleFactor.dx) + (_pan.dy * parent.trackpadScrollToScaleFactor.dy),
          );
    }
    return _scale;
  }
...
class ScaleGestureRecognizer extends OneSequenceGestureRecognizer {
  ...
  bool _reconfigure(int pointer) {
    ...
    if (_pointerPanZooms.isEmpty) {
      ...
      _initialPanZoomRotationFactor = _pointerPanZooms.values.map((_PointerPanZoomData x) => x.rotation).reduce(
