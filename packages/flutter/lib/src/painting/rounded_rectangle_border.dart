    ...
    switch (side.style) {
      ...
      case BorderStyle.solid:
        if (side.width == 0.0) {
          ...
          final RRect borderRect = borderRadius.resolve(textDirection).toRRect(
            rect,
          );
...
class _RoundedRectangleToCircleBorder extends OutlinedBorder {
  ...
  BorderRadius? _adjustBorderRadius(Rect rect, TextDirection? textDirection) {
    ...
    if (eccentricity != 0.0) {
      if (rect.width < rect.height) {
        return BorderRadius.lerp(resolvedRadius, BorderRadius.all(
          Radius.elliptical(
            rect.width / 2,
  ...
  @override
  Path getInnerPath(Rect rect, {TextDirection? textDirection}) {
    final RRect borderRect = _adjustBorderRadius(rect, textDirection)!.toRRect(
      _adjustRect(rect),
    );
  ...
  void paint(Canvas canvas, Rect rect, {TextDirection? textDirection}) {
    switch (side.style) {
      ...
      case BorderStyle.solid:
        final BorderRadius adjustedBorderRadius =
            _adjustBorderRadius(rect, textDirection)!;
        final RRect borderRect = adjustedBorderRadius.toRRect(
          _adjustRect(rect),
        );
        canvas.drawRRect(
