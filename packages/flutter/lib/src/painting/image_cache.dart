...
class ImageCache {
  ...
  bool evict(Object key, {bool includeLive = true}) {
    ...
    if (!kReleaseMode) {
      Timeline.instantSync(
        'ImageCache.evict',
        arguments: <String, dynamic>{'type': 'miss'},
      );
    }
    return false;
  }
  ...
  ImageStreamCompleter? putIfAbsent(
    ...
    if (!kReleaseMode) {
      timelineTask = TimelineTask()
        ..start(
          'ImageCache.putIfAbsent',
          arguments: <String, dynamic>{'key': key.toString()},
        );
    }
    ...
    if (image != null) {
      if (!kReleaseMode) {
        timelineTask!.finish(
          arguments: <String, dynamic>{'result': 'keepAlive'},
        );
      }
...
class ImageCacheStatus {
  ...
  String toString() => '${objectRuntimeType(
        this,
        'ImageCacheStatus',
      )}(pending: $pending, live: $live, keepAlive: $keepAlive)';
}
...
abstract class _CachedImageBase {
  _CachedImageBase(this.completer, {this.sizeBytes})
