...
class StarBorder extends OutlinedBorder {
      ...
      return _twoPhaseLerp(
        ...
        (double t) {
          return _twoPhaseLerp(
            t,
            0.5,
            (double t) => StadiumBorder(side: lerpedSide).lerpTo(
              CircleBorder(side: lerpedSide),
              t,
            ),
            (double t) => lerpFrom(CircleBorder(side: lerpedSide), t),
          );
      ...
      return _twoPhaseLerp(
        ...
        (double t) {
          return _twoPhaseLerp(
            ...
            0.5,
            (double t) => lerpTo(CircleBorder(side: lerpedSide), t),
            (double t) => StadiumBorder(side: lerpedSide).lerpFrom(
              CircleBorder(side: lerpedSide),
              t,
            ),
          );
  ...
  String toString() {
    return '${objectRuntimeType(
      this,
      'StarBorder',
    )}($side, points: $points, innerRadiusRatio: $innerRadiusRatio)';
  }
}
...
class _StarGenerator {
  ...
  Path generate(Rect rect) {
    ...
    // covers.
    final Matrix4 squashMatrix = Matrix4.translationValues(
      rect.center.dx,
      rect.center.dy,
      0,
    );
  ...
  double _generatePoints({
    ...
    // non-zero.
    return clampDouble(
      math.max(valleyRadius, pointRadius),
      double.minPositive,
      double.maxFinite,
    );
  }
  ...
  void _drawPoints(Path path, List<_PointInfo> points) {
    ...
    final double pointAngle = _getAngle(
      points[0].valley,
      points[0].point,
      points[1].valley,
    );
    final double pointWeight = _getWeight(pointAngle);
    final double valleyAngle = _getAngle(
      points[1].point,
      points[1].valley,
      points[0].point,
    );
