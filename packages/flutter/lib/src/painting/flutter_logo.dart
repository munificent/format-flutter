...
class FlutterLogoDecoration extends Decoration {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    if (_inTransition) {
      properties.add(
        DiagnosticsNode.message('transition ${debugFormatDouble(_position)}:${debugFormatDouble(_opacity)}'),
      );
    }
  }
}
  ...
  void paint(Canvas canvas, Offset offset, ImageConfiguration configuration) {
    ...
    if (_config._opacity < 1.0) {
      canvas.saveLayer(
        offset & canvasSize,
        Paint()
          ..colorFilter = ColorFilter.mode(const Color(0xFFFFFFFF).withOpacity(_config._opacity), BlendMode.modulate),
      );
    }
    if (_config._position != 0.0) {
      if (_config._position > 0.0) {
