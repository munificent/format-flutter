...
class _StadiumToCircleBorder extends OutlinedBorder {
  ...
  BorderRadius _adjustBorderRadius(Rect rect) {
    final BorderRadius circleRadius = BorderRadius.circular(
      rect.shortestSide / 2,
    );
    if (eccentricity != 0.0) {
      if (rect.width < rect.height) {
        return BorderRadius.lerp(circleRadius, BorderRadius.all(
          Radius.elliptical(
            rect.width / 2,
            (0.5 + eccentricity / 2) * rect.height / 2,
          ),
        ), circularity)!;
      } else {
        return BorderRadius.lerp(circleRadius, BorderRadius.all(
          Radius.elliptical(
            (0.5 + eccentricity / 2) * rect.width / 2,
            rect.height / 2,
          ),
        ), circularity)!;
      }
  ...
  Path getInnerPath(Rect rect, {TextDirection? textDirection}) {
    return Path()
      ..addRRect(_adjustBorderRadius(rect).toRRect(_adjustRect(rect)).deflate(
        side.strokeInset,
      ));
  }
  ...
  void paint(Canvas canvas, Rect rect, {TextDirection? textDirection}) {
    switch (side.style) {
      case BorderStyle.none:
        break;
      case BorderStyle.solid:
        final RRect borderRect = _adjustBorderRadius(rect).toRRect(
          _adjustRect(rect),
        );
        canvas.drawRRect(
          borderRect.inflate(side.strokeOffset / 2),
  ...
  String toString() {
    if (eccentricity != 0.0) {
      return 'StadiumBorder($side, ${(circularity * 100).toStringAsFixed(
        1,
      )}% of the way to being a CircleBorder that is ${(eccentricity * 100).toStringAsFixed(
        1,
      )}% oval)';
    }
    return 'StadiumBorder($side, ${(circularity * 100).toStringAsFixed(
...
class _StadiumToRoundedRectangleBorder extends OutlinedBorder {
  ...
  @override
  Path getInnerPath(Rect rect, {TextDirection? textDirection}) {
    final RRect borderRect = _adjustBorderRadius(rect)
  ...
  void paint(Canvas canvas, Rect rect, {TextDirection? textDirection}) {
    switch (side.style) {
      case BorderStyle.none:
        break;
      case BorderStyle.solid:
        final BorderRadiusGeometry adjustedBorderRadius = _adjustBorderRadius(
          rect,
        );
