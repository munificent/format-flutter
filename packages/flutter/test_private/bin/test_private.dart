...
final Directory flutterRoot = Directory(path.fromUri(Platform.script))
    ...
    .parent
    .parent;
final Directory flutterPackageDir = Directory(
  path.join(flutterRoot.path, 'packages', 'flutter'),
);
final Directory testPrivateDir = Directory(
  path.join(flutterPackageDir.path, 'test_private'),
);
final Directory privateTestsDir = Directory(
  path.join(testPrivateDir.path, 'test'),
);
...
class TestCase {
  ...
  bool setUp() {
    ...
    for (final File file in dependencies) {
      try {
        final Directory destDir = Directory(
          path.join(tmpdir.absolute.path, file.parent.path),
        );
        destDir.createSync(recursive: true);
        final File absFile = makeAbsolute(
          file,
          workingDirectory: flutterPackageDir,
        );
    ...
    makeAbsolute(pubspec, workingDirectory: privateTestsDir).copySync(
      path.join(tmpdir.absolute.path, 'pubspec.yaml'),
    );

    // Use Flutter's analysis_options.yaml file from packages/flutter.
    File(path.join(tmpdir.absolute.path, 'analysis_options.yaml'))
        .writeAsStringSync(
          'include: ${path.toUri(path.join(
            flutterRoot.path,
            'packages',
            'flutter',
  ...
  Future<bool> runTests() async {
    ...
    for (final File test in tests) {
      ...
      final ProcessRunnerResult result = await runner
          .runProcess(<String>[flutter, 'test', testPath], failOk: true);
      if (result.exitCode != 0) {
        return false;
      }
    }
...
Stream<TestCase> getTestCases(Directory tmpdir) async* {
  ...
  await for (final FileSystemEntity entity in testDir.list(recursive: true)) {
    if (path.split(entity.path).where(
