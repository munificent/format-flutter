...
class RenderSectorRing extends RenderSectorWithChildren {
  ...
  SectorDimensions getIntrinsicDimensions(
    ...
    final double outerDeltaRadius = constraints.constrainDeltaRadius(
      desiredDeltaRadius,
    );
    final double innerDeltaRadius = math.max(
      0.0,
      outerDeltaRadius - padding * 2.0,
    );
    final double childRadius = radius + padding;
    final double paddingTheta = math.atan(
      padding / (radius + outerDeltaRadius),
    );
  ...
  void performLayout() {
    ...
    final double paddingTheta = math.atan(
      padding / (parentData!.radius + deltaRadius),
    );
    double innerTheta = paddingTheta; // increments with each child
...
class RenderSectorSlice extends RenderSectorWithChildren {
  ...
  SectorDimensions getIntrinsicDimensions(
    ...
    final double outerDeltaTheta = constraints.constrainDeltaTheta(
      desiredDeltaTheta,
    );
...
class RenderBoxToRenderSectorAdapter extends RenderBox
  ...
  Size getIntrinsicDimensions({
    ...
    final double maxChildDeltaRadius = math.max(
      0.0,
      math.min(width, height) / 2.0 - innerRadius,
    );
    final SectorDimensions childDimensions = child!.getIntrinsicDimensions(
  ...
  void performLayout() {
    if (child == null ||
      ...
      child?.layout(
        SectorConstraints(maxDeltaRadius: innerRadius),
        parentUsesSize: true,
      );
      return;
    }
  ...
  bool hitTest(BoxHitTestResult result, {required Offset position}) {
    ...
    child!.hitTest(
      SectorHitTestResult.wrap(result),
      radius: radius,
      theta: theta,
    );
